...
namespace log4cpp
{
    ...
    void PropertyConfiguratorImpl::configureCategory(const std::string& categoryName)
    {
        ...
        if (iter == _properties.end()) throw ConfigureFailure(std::string("Unable to find category: ") + tempCatName);
        ...
        for (/**/; i != iEnd; ++i)
        {
            ...
            {
                // appender not found;
                throw ConfigureFailure(std::string("Appender '") + appenderName + "' not found for category '" + categoryName
                                       + "'");
            }
            else
            {
    ...
    Appender* PropertyConfiguratorImpl::instantiateAppender(const std::string& appenderName)
    {
        ...
        if (key == _properties.end()) throw ConfigureFailure(std::string("Appender '") + appenderName + "' not defined");
        ...
        else if (appenderType == "SyslogAppender")
        {
            ...
            int         facility =
                _properties.getInt(appenderPrefix + ".facility", -1) * 8;  // * 8 to get LOG_KERN, etc. compatible values.
    ...
    void PropertyConfiguratorImpl::setLayout(Appender* appender, const std::string& appenderName)
    {
        ...
        {
            throw ConfigureFailure(std::string("Unknown layout type '" + layoutType + "' for appender '") + appenderName
                                   + "'");
        }
