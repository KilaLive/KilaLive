...
namespace log4cpp
{
    ...
    {
        static int priorities[8] = {LOG_EMERG, LOG_ALERT, LOG_CRIT, LOG_ERR, LOG_WARNING, LOG_NOTICE, LOG_INFO, LOG_DEBUG};
        int        result;
    ...
    std::auto_ptr<Appender> create_syslog_appender(const FactoryParams& params)
    {
        ...
        int         facility = 0;
        params.get_for("syslog appender").required("name", name)("syslog_name", syslog_name).optional("facility", facility);
