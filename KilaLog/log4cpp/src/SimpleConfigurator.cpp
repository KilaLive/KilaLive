...
namespace log4cpp
{
    ...
    void SimpleConfigurator::configure(std::istream& initFile)
    {
        ...
        while (initFile >> nextCommand)
        {
            ...
            if (!(initFile >> categoryName)) break;

            log4cpp::Category& category = (categoryName.compare("root") == 0) ? log4cpp::Category::getRoot()
            ...
            if (nextCommand.compare("appender") == 0)
            {
                ...
                if (initFile >> layout >> appenderName)
                {
                    ...
                    if (appenderName.compare("file") == 0)
                    {
                        ...
                        {
                            throw ConfigureFailure("Missing filename for log file logging configuration file for category: "
                                                   + categoryName);
                        }
                    ...
                    else if (appenderName.compare("remotesyslog") == 0)
                    {
                        ...
                        appender = new log4cpp::RemoteSyslogAppender(categoryName, syslogName, relayer, facility, portNumber);
                    }
