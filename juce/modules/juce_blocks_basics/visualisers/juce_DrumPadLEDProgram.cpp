...
namespace juce
{
    DrumPadGridProgram::DrumPadGridProgram(Block& b)
        : Program(b)
    {
    }
    ...
    void DrumPadGridProgram::endTouch(float startX, float startY)
    {
        ...
        for (size_t i = 0; i < 4; ++i)
            if (block.getDataByte(touchedPads_byte + i) == (padIdx + 1)) block.setDataByte(touchedPads_byte + i, 0);
    }
    ...
    void DrumPadGridProgram::sendTouch(float x, float y, float z, LEDColour colour)
    {
        ...
        e.values[0] = 0x20000000 + (juce::jlimit(0, 255, juce::roundToInt(x * (255.0f / block.getWidth()))) << 16)
    ...
    void DrumPadGridProgram::triggerSlideTransition(int newNumColumns, int newNumRows, const juce::Array<GridFill>& newFills,
    ...
    {
        if (block.versionNumber.isEmpty() || block.versionNumber.compare("0.2.5") < 0) return getLittleFootProgramPre25();
