...
namespace juce
{
    struct RuleBasedTopologySource::Internal : public TopologySource::Listener, private juce::AsyncUpdater
    {
        Internal(RuleBasedTopologySource& da, TopologySource& bd)
            : owner(da)
            , detector(bd)
        {
        ...
        void regenerateTopology()
        {
            ...
            for (auto rule : rules) rule->transformTopology(newTopology);
    ...
    void RuleBasedTopologySource::addRule(Rule* r)
    {
        internal->addRule(r);
    }
