...
#define JUCE_ASSERT_MESSAGE_MANAGER_IS_LOCKED                                                                                \
    ...
    namespace
    {
        struct PortIOStats
        {
            ...
            {
                return juce::String(name) + ": " + "count=" + juce::String(messageCount).paddedRight(' ', 7)
    ...
    struct PhysicalTopologySource::Internal
    {
        ...
        //==============================================================================
        struct MIDIDeviceConnection : public DeviceConnection, public juce::MidiInputCallback
        {
            ...
            ~MIDIDeviceConnection()
            {
                ...
                if (interprocessLock != nullptr) interprocessLock->exit();
            }

            bool lockAgainstOtherProcesses(const String& midiInName, const String& midiOutName)
            {
                interprocessLock.reset(new juce::InterProcessLock("blocks_sdk_" + File::createLegalFileName(midiInName) + "_"
                                                                  + File::createLegalFileName(midiOutName)));
                if (interprocessLock->enter(500)) return true;
            ...
            void handleIncomingMidiMessage(juce::MidiInput*, const juce::MidiMessage& message) override
            {
                ...
                if (bodySize > 0
                    && memcmp(data, BlocksProtocol::roliSysexHeader, sizeof(BlocksProtocol::roliSysexHeader)) == 0)
        ...
        struct MIDIDeviceDetector : public DeviceDetector
        {
            ...
            juce::StringArray scanForDevices() override
            {
                ...
                for (auto& pair : findDevices()) result.add(pair.inputName + " & " + pair.outputName);
            ...
            static String cleanBlocksDeviceName(juce::String name)
            {
                ...
                if (name.endsWith(" IN)")) return name.dropLastCharacters(4);

                if (name.endsWith(" OUT)")) return name.dropLastCharacters(5);
        ...
        static juce::Array<DeviceInfo> getArrayOfDeviceInfo(const juce::Array<BlocksProtocol::DeviceStatus>& devices)
        {
            ...
            for (auto& device : devices)
            {
                ...
                result.add({getBlockUIDFromSerialNumber(device.serialNumber), device.index, device.serialNumber, version,
                            name, isFirst});
        ...
        static bool versionNumberAddedToBlock(const juce::Array<DeviceInfo>& devices, Block::UID uid,
                                              juce::String version) noexcept
        {
            for (auto&& d : devices)
            {
                ...
                if (d.uid == uid && deviceVersion != version && deviceVersion.isNotEmpty()) return true;
            }
        ...
        {
            for (auto&& d : devices)
                if (d.uid == uid && d.name.length) return true;
        ...
        {
            for (auto&& d : devices)
                if (d.uid == block.uid) block.versionNumber = juce::String((const char*)d.version.version, d.version.length);
        }

        static void setNameForBlock(const juce::Array<DeviceInfo>& devices, Block& block) noexcept
        {
            for (auto&& d : devices)
                if (d.uid == block.uid) block.name = juce::String((const char*)d.name.name, d.name.length);
        }

        //==============================================================================
        struct ConnectedDeviceGroup : private juce::AsyncUpdater, private juce::Timer
        {
            ...
            const BlocksProtocol::DeviceStatus* getLastStatus(Block::UID deviceID) const noexcept
            {
                for (auto&& status : currentTopologyDevices)
                    if (getBlockUIDFromSerialNumber(status.serialNumber) == deviceID) return &status;
            ...
            bool hasAnyBlockStoppedPinging() const noexcept
            {
                ...
                for (auto& ping : blockPings)
                    if (ping.lastPing < now - juce::RelativeTime::seconds(pingTimeoutSeconds)) return true;
            ...
            void timerCallback() override
            {
                ...
                if ((now > lastTopologyReceiveTime + juce::RelativeTime::seconds(30.0) || hasAnyBlockStoppedPinging())
                    && now > lastTopologyRequestTime + juce::RelativeTime::seconds(1.0) && numTopologyRequestsSent < 4)
                    sendTopologyRequest();
            }
            ...
            void handleVersion(BlocksProtocol::DeviceVersion version)
            {
                for (auto i = 0; i < currentDeviceInfo.size(); ++i)
                {
                    ...
                    {
                        if (memcmp(currentDeviceInfo.getReference(i).version.version, version.version.version,
            ...
            void handleTouchChange(BlocksProtocol::TopologyIndex deviceIndex, uint32 timestamp,
                                   BlocksProtocol::TouchIndex touchIndex, BlocksProtocol::TouchPosition position,
                                   BlocksProtocol::TouchVelocity velocity, bool isStart, bool isEnd)
            {
                if (auto deviceID = getDeviceIDFromMessageIndex(deviceIndex))
                {
            ...
            void setTouchStartPosition(TouchSurface::Touch& touch)
            {
                ...
                if (touch.isTouchStart) startPos = {touch.x, touch.y};
            ...
            {
                if (auto deviceID = getDeviceIDFromMessageIndex(deviceIndex)) detector.handleSharedDataACK(deviceID, counter);
            }
            ...
            void handleConfigUpdateMessage(BlocksProtocol::TopologyIndex deviceIndex, int32 item, int32 value, int32 min,
                                           int32 max)
            {
                if (auto deviceID = getDeviceIDFromMessageIndex(deviceIndex))
            ...
            {
                if (auto deviceID = getDeviceIDFromMessageIndex(deviceIndex)) detector.handleLogMessage(deviceID, message);
            }
        ...
        private:
            ...
            juce::Array<BlockDeviceConnection> getArrayOfConnections(
                const juce::Array<BlocksProtocol::DeviceConnection>& connections)
            {
            ...
            void handleAsyncUpdate() override
            {
                ...
                for (auto& packet : packets)
                {
                    ...
                    BlocksProtocol::HostPacketDecoder<ConnectedDeviceGroup>::processNextPacket(*this, *data, data + 1,
                                                                                               (int)packet.getSize() - 1);
                }
        ...
        struct Detector : public juce::ReferenceCountedObject, private juce::Timer
        {
            Detector()
                : defaultDetector(new MIDIDeviceDetector())
                , deviceDetector(*defaultDetector)
            {
                topologyBroadcastThrottle.detector = this;
                startTimer(10);
            }

            Detector(DeviceDetector& dd)
                : deviceDetector(dd)
            {
                topologyBroadcastThrottle.detector = this;
                startTimer(10);
            ...
            void detach(PhysicalTopologySource* pts)
            {
                ...
                if (activeTopologySources.isEmpty())
                {
                    for (auto& b : currentTopology.blocks)
                        if (auto bi = BlockImplementation::getFrom(*b)) bi->sendCommandMessage(BlocksProtocol::endAPIMode);

                    currentTopology = {};

                    auto& d = getDefaultDetectorPointer();

                    if (d != nullptr && d->getReferenceCount() == 2) getDefaultDetectorPointer() = nullptr;
                }
            }
            ...
            bool isConnected(Block::UID deviceID) const noexcept
            {
                JUCE_ASSERT_MESSAGE_MANAGER_IS_LOCKED  // This method must only be called from the message thread!
                    ...
                    for (auto&& b : currentTopology.blocks) if (b->uid == deviceID) return true;
            ...
            const BlocksProtocol::DeviceStatus* getLastStatus(Block::UID deviceID) const noexcept
            {
                for (auto d : connectedDeviceGroups)
                    if (auto status = d->getLastStatus(deviceID)) return status;
            ...
            void handleTopologyChange()
            {
...

                {
                    ...
                    for (int i = currentTopology.blocks.size(); --i >= 0;)
                    {
                        ...
                        {
                            if (auto bi = BlockImplementation::getFrom(*block)) bi->invalidate();
                        ...
                        else
                        {
                            ...
                            if (nameAddedToBlock(newDeviceInfo, block->uid)) setNameForBlock(newDeviceInfo, *block);
                        }
                    }
                    ...
                    for (auto& info : newDeviceInfo)
                        if (info.serial.isValid())
                            if (!containsBlockWithUID(currentTopology.blocks, getBlockUIDFromSerialNumber(info.serial)))
                                currentTopology.blocks.add(
                                    new BlockImplementation(info.serial, *this, info.version, info.name, info.isMaster));
            ...
            void handleSharedDataACK(Block::UID deviceID, uint32 packetCounter) const
            {
                ...
                for (auto&& b : currentTopology.blocks)
                    if (b->uid == deviceID)
                        if (auto bi = BlockImplementation::getFrom(*b)) bi->handleSharedDataACK(packetCounter);
            }
            ...
            void handleFirmwareUpdateACK(Block::UID deviceID, uint8 resultCode, uint32 resultDetail)
            {
                for (auto&& b : currentTopology.blocks)
                    if (b->uid == deviceID)
                        if (auto bi = BlockImplementation::getFrom(*b)) bi->handleFirmwareUpdateACK(resultCode, resultDetail);
            }
            ...
            void handleConfigUpdateMessage(Block::UID deviceID, int32 item, int32 value, int32 min, int32 max)
            {
                for (auto&& b : currentTopology.blocks)
                    if (b->uid == deviceID)
                        if (auto bi = BlockImplementation::getFrom(*b)) bi->handleConfigUpdateMessage(item, value, min, max);
            }
            ...
            void handleLogMessage(Block::UID deviceID, const String& message) const
            {
                ...
                for (auto&& b : currentTopology.blocks)
                    if (b->uid == deviceID)
                        if (auto bi = BlockImplementation::getFrom(*b)) bi->handleLogMessage(message);
            }
            ...
            {
                for (auto surface : activeTouchSurfaces) surface->cancelAllActiveTouches();
            }
            ...
            void handleCustomMessage(Block::UID deviceID, Block::Timestamp timestamp, const int32* data)
            {
                for (auto&& b : currentTopology.blocks)
                    if (b->uid == deviceID)
                        if (auto bi = BlockImplementation::getFrom(*b)) bi->handleCustomMessage(timestamp, data);
            }
            ...
            {
                for (auto* c : connectedDeviceGroups)
                    if (c->getIndexFromDeviceID(deviceID) >= 0) return c->sendMessageToDevice(builder);
            ...
            {
                if (auto* bi = BlockImplementation::getFrom(b)) return &(bi->detector);
            ...
            DeviceConnection* getDeviceConnectionFor(const Block& b)
            {
                for (const auto& d : connectedDeviceGroups)
                {
                    ...
                    {
                        if (info.uid == b.uid) return d->getDeviceConnection();
                    }
                }
        ...
        private:
            ...
            void handleDevicesRemoved(const juce::StringArray& detectedDevices)
            {
                ...
                if (anyDevicesRemoved) handleTopologyChange();
            }
            ...
            void handleDevicesAdded(const juce::StringArray& detectedDevices)
            {
                ...
                if (anyDevicesAdded) handleTopologyChange();
            }

            bool hasDeviceFor(const juce::String& devName) const
            {
                for (auto d : connectedDeviceGroups)
                    if (d->deviceName == devName) return true;
        ...
        //==============================================================================
        struct BlockImplementation : public Block, private MIDIDeviceConnection::Listener, private Timer
        {
            BlockImplementation(const BlocksProtocol::BlockSerialNumber& serial, Detector& detectorToUse,
                                BlocksProtocol::VersionNumber version, BlocksProtocol::BlockName name, bool master)
                ...
                , isMaster(master)
            {
                ...
                if (modelData.hasTouchSurface) touchSurface.reset(new TouchSurfaceImplementation(*this));

                int i = 0;
                for (auto&& b : modelData.buttons) controlButtons.add(new ControlButtonImplementation(*this, i++, b));

                if (modelData.lightGridWidth > 0 && modelData.lightGridHeight > 0)
                    ledGrid.reset(new LEDGridImplementation(*this));

                for (auto&& s : modelData.statusLEDs) statusLights.add(new StatusLightImplementation(*this, s));

                if (modelData.numLEDRowLEDs > 0) ledRow.reset(new LEDRowImplementation(*this));

                listenerToMidiConnection = dynamic_cast<MIDIDeviceConnection*>(detector.getDeviceConnectionFor(*this));

                if (listenerToMidiConnection != nullptr) listenerToMidiConnection->addListener(this);
            ...
            {
                return modelData.apiType;
            }
            juce::String getDeviceDescription() const override
            {
                return modelData.description;
            }
            int getWidth() const override
            {
                return modelData.widthUnits;
            }
            int getHeight() const override
            {
                return modelData.heightUnits;
            }
            float getMillimetersPerUnit() const override
            {
                return 47.0f;
            }
            bool isHardwareBlock() const override
            {
                return true;
            }
            juce::Array<Block::ConnectionPort> getPorts() const override
            {
                return modelData.ports;
            }
            bool isConnected() const override
            {
                return isStillConnected && detector.isConnected(uid);
            }
            bool isMasterBlock() const override
            {
                return isMaster;
            }

            TouchSurface* getTouchSurface() const override
            {
                return touchSurface.get();
            }
            LEDGrid* getLEDGrid() const override
            {
                return ledGrid.get();
            }
            LEDRow* getLEDRow() const override
            {
                return ledRow.get();
            }
            ...
            {
                if (auto status = detector.getLastStatus(uid)) return status->batteryLevel.toUnipolarFloat();
            ...
            {
                if (auto status = detector.getLastStatus(uid)) return status->batteryCharging.get() != 0;
            ...
            void handleCustomMessage(Block::Timestamp, const int32* data)
            {
                ...
                for (uint32 i = 0; i < BlocksProtocol::numProgramMessageInts; ++i) m.values[i] = data[i];
            ...
            {
                if (auto bi = dynamic_cast<BlockImplementation*>(&b)) return bi;
            ...
            bool isControlBlock() const
            {
                ...
                return type == Block::Type::liveBlock || type == Block::Type::loopBlock || type == Block::Type::touchBlock
            ...
            {
                if (logger != nullptr) logger(message);
            }
            ...
            void addDataInputPortListener(DataInputPortListener* listener) override
            {
                ...
                if (auto midiInput = getMidiInput()) midiInput->start();
            }

            void sendMessage(const void* message, size_t messageSize) override
            {
                if (auto midiOutput = getMidiOutput()) midiOutput->sendMessageNow({message, (int)messageSize});
            }
            ...
            void handleTimerTick()
            {
                ...
                {
                    if (resetMessagesSent == 1) sendCommandMessage(BlocksProtocol::endAPIMode);
                ...
                if (ledGrid != nullptr)
                    if (auto renderer = ledGrid->getRenderer()) renderer->renderLEDGrid(*ledGrid);
            ...
            bool setName(const juce::String& newName) override
            {
                ...
                if (index >= 0)
                {
                    ...
                    if (p.addSetBlockName(newName))
                    {
                        ...
                        if (sendMessageToDevice(p)) return true;
                    }
                }
                else
                {
        ...
        private:
            ...
            {
                dataInputPortListeners.call([&](DataInputPortListener& l) {
                    l.handleIncomingDataPortMessage(*this, message.getRawData(), (size_t)message.getRawDataSize());
                });
            }
        ...
        //==============================================================================
        struct LEDRowImplementation : public LEDRow, private Timer
        {
            LEDRowImplementation(BlockImplementation& b)
                : LEDRow(b)
            {
                startTimer(300);
            }
        ...
        private:
            ...
            {
                if (block.getProgram() != nullptr)
                    for (uint32 i = 0; i < (uint32)numElementsInArray(colours); ++i) write565Colour(16 * i, colours[i]);
            }
        ...
        //==============================================================================
        struct TouchSurfaceImplementation : public TouchSurface, private juce::Timer
        {
            TouchSurfaceImplementation(BlockImplementation& b)
                : TouchSurface(b)
                , blockImpl(b)
            {
                if (auto det = Detector::getFrom(block)) det->activeTouchSurfaces.add(this);
            ...
            {
                if (auto det = Detector::getFrom(block)) det->activeTouchSurfaces.removeFirstMatchingValue(this);
            }
            ...
            void broadcastTouchChange(const TouchSurface::Touch& touchEvent)
            {
                ...
                if (touchEvent.isTouchStart) status.lastStrikePressure = touchEvent.zVelocity;
            ...
            void timerCallback() override
            {
                ...
                for (auto& t : touches)
                {
                    ...
                    if (status.isActive && now > status.lastEventTime + touchTimeOutMs) killTouch(t.touch, status, now);
                }
            }
            ...
            void cancelAllActiveTouches() noexcept override
            {
                ...
                for (auto& t : touches)
                    if (t.value.isActive) killTouch(t.touch, t.value, now);
        ...
        struct ControlButtonImplementation : public ControlButton
        {
            ControlButtonImplementation(BlockImplementation& b, int index, BlocksProtocol::BlockDataSheet::ButtonInfo info)
                ...
                , buttonIndex(index)
            {
                if (auto det = Detector::getFrom(block)) det->activeControlButtons.add(this);
            }

            ~ControlButtonImplementation()
            {
                if (auto det = Detector::getFrom(block)) det->activeControlButtons.removeFirstMatchingValue(this);
            }

            ButtonFunction getType() const override
            {
                return buttonInfo.type;
            }
            juce::String getName() const override
            {
                return BlocksProtocol::getButtonNameForFunction(buttonInfo.type);
            }
            ...
            {
                return blockImpl.isControlBlock();
            }
            ...
            void broadcastButtonChange(Block::Timestamp timestamp, ControlButton::ButtonFunction button, bool isDown)
            {
                ...
                {
                    if (wasDown == isDown) sendButtonChangeToListeners(timestamp, !isDown);
        ...
        {
            StatusLightImplementation(Block& b, BlocksProtocol::BlockDataSheet::StatusLEDInfo i)
                : StatusLight(b)
                , info(i)
            {
            }
        ...
        {
            LEDGridImplementation(BlockImplementation& b)
                : LEDGrid(b)
                , blockImpl(b)
            {
            }
            ...
            {
                return blockImpl.modelData.lightGridWidth;
            }
            int getNumRows() const override
            {
                return blockImpl.modelData.lightGridHeight;
            }
        ...
        static juce::String idToSerialNum(const BlockTopology& topology, Block::UID uid)
        {
            for (auto* b : topology.blocks)
                if (b->uid == uid) return b->serialNumber;
        ...
        static juce::String portEdgeToString(Block::ConnectionPort port)
        {
            switch (port.edge)
            {
            case Block::ConnectionPort::DeviceEdge::north:
                return "north";
            case Block::ConnectionPort::DeviceEdge::south:
                return "south";
            case Block::ConnectionPort::DeviceEdge::east:
                return "east";
            case Block::ConnectionPort::DeviceEdge::west:
                return "west";
            }
        ...
        static void dumpTopology(const BlockTopology& topology)
        {
            ...
            m << "=============================================================================" << newLine
              << "Topology:  " << topology.blocks.size() << " device(s)" << newLine << newLine;
            ...
            for (auto block : topology.blocks)
            {
                ...
                m << "  Description: " << block->getDeviceDescription() << newLine << "  Serial: " << block->serialNumber
                  << newLine;
                ...
                m << "  Battery level: " + juce::String(juce::roundToInt(100.0f * block->getBatteryLevel())) + "%" << newLine
                  ...
                  << "  Width: " << block->getWidth() << newLine << "  Height: " << block->getHeight() << newLine
                  << "  Millimeters per unit: " << block->getMillimetersPerUnit() << newLine << newLine;
            }

            for (auto& connection : topology.connections)
            {
                m << idToSerialNum(topology, connection.device1) << ":" << portToString(connection.connectionPortOnDevice1)
    ...
    struct PhysicalTopologySource::DetectorHolder : private juce::Timer
    {
        ...
        void timerCallback() override
        {
            if (!topologySource.hasOwnServiceTimer()) handleTimerTick();
        }

        void handleTimerTick()
        {
            for (auto& b : detector->currentTopology.blocks)
                if (auto bi = Internal::BlockImplementation::getFrom(*b)) bi->handleTimerTick();
        }
    ...
    bool PhysicalTopologySource::hasOwnServiceTimer() const
    {
        return false;
    }
    ...
    static bool blocksMatch(const Block::Array& list1, const Block::Array& list2) noexcept
    {
        ...
        for (auto&& b : list1)
            if (!list2.contains(b)) return false;
