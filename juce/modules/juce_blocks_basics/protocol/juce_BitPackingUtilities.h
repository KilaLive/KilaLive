    ...
    namespace BlocksProtocol
    {
        ...
        */
        ...
        static uint8 calculatePacketChecksum(const uint8* data, uint32 size) noexcept
        {
            ...
            for (uint32 i = 0; i < size; ++i) checksum += checksum * 2 + data[i];
        ...
        struct IntegerWithBitSize
        {
            ...
            {
                return (uint8)(numBits < 8 ? (uint32)(value << (8 - numBits)) : (uint32)(value >> (numBits - 8)));
            }
            ...
            {
                return static_cast<int32>(value << (32 - numBits)) / (float)0x80000000u;
            }
            ...
            {
                static_assert(numBits <= 31, "numBits must be <= 31");
                return IntegerWithBitSize(
                    maxValue & (uint32)jlimit((int)-(maxValue / 2), (int)(maxValue / 2), (int)(value * (maxValue / 2))));
            }
        ...
        */
        ...
        struct Packed7BitArrayBuilder
        {
            ...
            {
                return bytesWritten + (bitsInCurrentByte > 0 ? 1 : 0);
            }
            ...
            void writeHeaderSysexBytes(uint8 deviceIndex) noexcept
            {
                ...
                for (int i = 0; i < (int)sizeof(roliSysexHeader); ++i) data[bytesWritten++] = roliSysexHeader[i];
        ...
        */
        struct Packed7BitArrayReader
        {
            Packed7BitArrayReader(const void* sourceData, int numBytes) noexcept
                : data(static_cast<const uint8*>(sourceData))
                , totalBits(numBytes * 7)
            {
            }
