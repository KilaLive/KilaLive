    ...
    namespace BlocksProtocol
    {
        ...
        */
        ...
        struct HostPacketBuilder
        {
            ...
            {
                if (!data.hasCapacity(MessageType::bits + DeviceCommand::bits)) return false;
            ...
            {
                if (!data.hasCapacity(MessageType::bits + PacketIndex::bits + DataChangeCommand::bits)) return false;
            ...
            {
                if (!data.hasCapacity(DataChangeCommand::bits)) return false;
            ...
            bool setMultipleBytes(const uint8* values, int num) noexcept
            {
                ...
                if (!data.hasCapacity(DataChangeCommand::bits * 2 + num * (1 + ByteValue::bits))) return false;

                data << DataChangeCommand((uint32)setSequenceOfBytes);

                for (int i = 0; i < num; ++i)
                    data << ByteValue((uint32)values[i]) << ByteSequenceContinues(i < num - 1 ? 1 : 0);
            ...
            bool setMultipleBytes(uint8 value, uint8 lastValue, int num) noexcept
            {
                ...
                if (num == 1) return setMultipleBytes(&value, 1);  // (this is a more compact message)
                ...
                if (num > ByteCountFew::maxValue)
                {
                    ...
                    data << DataChangeCommand((uint32)setManyBytesWithValue) << ByteCountMany((uint32)num)
            ...
            {
                if (!data.hasCapacity(BitSizes::programEventMessage)) return false;

                writeMessageType(MessageFromHost::programEventMessage);

                for (uint32 i = 0; i < numProgramMessageInts; ++i) data << IntegerWithBitSize<32>((uint32)messageData[i]);
            ...
            {
                if (!data.hasCapacity(MessageType::bits + FirmwareUpdatePacketSize::bits + 7 * size)) return false;

                writeMessageType(MessageFromHost::firmwareUpdatePacket);
                data << FirmwareUpdatePacketSize(size);

                for (uint8 i = 0; i < size; ++i) data << IntegerWithBitSize<7>((uint32)packetData[i]);
            ...
            {
                if (!data.hasCapacity(BitSizes::configSetMessage)) return false;
            ...
            {
                if (!data.hasCapacity(MessageType::bits + ConfigCommand::bits)) return false;
            ...
            {
                if (!data.hasCapacity(MessageType::bits)) return false;
            ...
            {
                if (name.length() > 32 || !data.hasCapacity(MessageType::bits + 7 + (7 * name.length()))) return false;

                writeMessageType(MessageFromHost::setName);

                data << IntegerWithBitSize<7>((uint32)name.length());

                for (auto i = 0; i < name.length(); ++i) data << IntegerWithBitSize<7>((uint32)name.toRawUTF8()[i]);
