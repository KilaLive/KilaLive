    ...
    public:
        ...
        */
        ...
        struct ConfigMetaData
        {
            ...
            ConfigMetaData(uint32 itemIndex, int32 itemValue, juce::Range<int32> rangeToUse, bool active,
                           const char* itemName, uint32 itemType, const char* options[ConfigMetaData::numOptionNames],
                ...
                , group(groupName)
            {
                for (int i = 0; i < numOptionNames; ++i) optionNames[i] = options[i];
            }
            ...
            const ConfigMetaData& operator=(const ConfigMetaData& other)
            {
                if (this != &other)
                {
                    ...
                    for (int i = 0; i < numOptionNames; ++i) optionNames[i] = other.optionNames[i];
                }
            ...
            bool operator==(const ConfigMetaData& other) const
            {
                for (int32 optionIndex = 0; optionIndex < numOptionNames; ++optionIndex)
                    if (optionNames[optionIndex] != other.optionNames[optionIndex]) return false;

                return item == other.item && value == other.value && range == other.range && isActive == other.isActive
