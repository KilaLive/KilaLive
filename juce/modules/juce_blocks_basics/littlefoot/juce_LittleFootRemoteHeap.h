    ...
    */
    ...
    struct LittleFootRemoteHeap
    {
        ...
        void resetDataRangeToUnknown(size_t offset, size_t size) noexcept
        {
            ...
            for (size_t i = 0; i < size; ++i) latestState[offset + i] = unknownByte;
        }
        ...
        void setByte(size_t offset, uint8 value) noexcept
        {
            if (offset < blockSize)
            {
                if (targetData[offset] != value)
                {
                    ...
                    if (programStateKnown && offset < programSize) programStateKnown = false;
                }
            }
            else
            {
        ...
        {
            for (size_t i = 0; i < num; ++i) setByte(offset + i, data[i]);
        }
        ...
        {
            if (offset < blockSize) return targetData[offset];
        ...
        void sendChanges(ImplementationClass& bi, bool forceSend)
        {
            if ((needsSyncing && messagesSent.isEmpty()) || forceSend)
            {
                ...
                {
                    if (isAllZero(targetData, blockSize)) break;

                    uint16 data[ImplementationClass::maxBlockSize];
                    auto*  latestState = getLatestExpectedDataState();

                    for (uint32 i = 0; i < blockSize; ++i) data[i] = latestState[i];

                    uint32 packetIndex =
                        messagesSent.isEmpty() ? lastPacketIndexReceived : messagesSent.getLast()->packetIndex;

                    packetIndex = (packetIndex + 1) & ImplementationClass::maxPacketCounter;

                    if (!Diff(data, targetData, blockSize).createChangeMessage(bi, data, messagesSent, packetIndex)) break;
            ...
            {
                if (m->dispatchTime >= Time::getCurrentTime() - RelativeTime::milliseconds(250)) break;

                m->dispatchTime = Time::getCurrentTime();
                bi.sendMessageToDevice(m->packet);
                // DBG ("Sending packet " << (int) m->packetIndex << " - " << m->packet.size() << " bytes, device " <<
                // bi.getDeviceIndex());

                if (getTotalSizeOfMessagesSent() > 200) break;
            }
        }
        ...
        void handleACKFromDevice(ImplementationClass& bi, uint32 packetIndex) noexcept
        {
            ...
            if (lastPacketIndexReceived != packetIndex)
            {
                ...
                for (int i = messagesSent.size(); --i >= 0;)
                {
                    ...
                    {
                        for (uint32 j = 0; j < blockSize; ++j) deviceState[j] = m.resultDataState[j];

                        messagesSent.removeRange(0, i + 1);
                        dumpStatus();
                        sendChanges(bi, false);

                        if (messagesSent.isEmpty()) needsSyncing = false;
        ...
        bool isProgramLoaded() noexcept
        {
            if (!programStateKnown)
            {
                ...
                for (size_t i = 0; i < blockSize; ++i) deviceMemory[i] = (uint8)deviceState[i];
    ...
    private:
        ...
        {
            return messagesSent.isEmpty() ? deviceState : messagesSent.getLast()->resultDataState;
        }
        ...
        int getTotalSizeOfMessagesSent() const noexcept
        {
            ...
            for (auto* m : messagesSent)
                if (m->dispatchTime != Time()) total += m->packet.size();
        ...
        void dumpStatus()
        {
            ...
            for (int i = 0; i < diffLen; ++i) areas[i] = '.';
        ...
        struct Diff
        {
                ...
                , blockSize(blockSizeToUse)
            {
                ...
                for (int i = 0; i < (int)blockSize; ++i) ranges.add({i, 1, newData[i] == current[i], false});
            ...
            bool createChangeMessage(const ImplementationClass& bi, const uint16* currentState,
                                     OwnedArray<ChangeMessage>& messagesCreated, uint32 nextPacketIndex)
            {
                ...
                for (uint32 i = 0; i < blockSize; ++i) message.resultDataState[i] = currentState[i];
                ...
                for (auto& r : ranges)
                {
                    ...
                    else if (r.isMixed)
                    {
                        ...
                        if (!packetOverflow) lastValue = newData[r.index + r.length - 1];
                    }
                    else
                    {
                        ...
                        if (!packetOverflow) lastValue = value;
                    }

                    if (packetOverflow) break;

                    if (!r.isSkipped)
                        for (int i = r.index; i < r.index + r.length; ++i) message.resultDataState[i] = newData[i];
                }
        ...
        private:
            ...
            void coalesceUniformRegions()
            {
                for (int i = ranges.size(); --i > 0;)
                {
                    ...
                    if (r1.isSkipped == r2.isSkipped && (r1.isSkipped || newData[r1.index] == newData[r2.index]))
                    {
                        r1.length += r2.length;
                        ranges.remove(i);
            ...
            void trim()
            {
                while (ranges.size() > 0 && ranges.getLast().isSkipped) ranges.removeLast();
            }
        };
    };
