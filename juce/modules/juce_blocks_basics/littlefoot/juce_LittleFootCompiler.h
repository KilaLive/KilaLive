    ...
    */
    struct Compiler
    {
        ...
        */
        ...
        {
            for (int i = 0; i < runner.getNumNativeFunctions(); ++i) nativeFunctions.add(runner.getNativeFunction(i));
        }
    ...
    private:
        ...
        {
            virtual ~AllocatedObject() noexcept {}
        };
...
#define LITTLEFOOT_KEYWORDS(X)                                                                                               \
    X(if_, "if")                                                                                                             \
    X(else_, "else") X(do_, "do") X(while_, "while") X(for_, "for") X(break_, "break") X(continue_, "continue")              \
        X(void_, "void") X(int_, "int") X(float_, "float") X(bool_, "bool") X(return_, "return") X(true_, "true")            \
            X(false_, "false") X(const_, "const")

#define LITTLEFOOT_OPERATORS(X)                                                                                              \
    X(semicolon, ";")                                                                                                        \
    X(dot, ".") X(comma, ",") X(hash, "#") X(openParen, "(") X(closeParen, ")") X(openBrace, "{") X(closeBrace, "}")         \
        X(openBracket, "[") X(closeBracket, "]") X(colon, ":") X(question, "?") X(equals, "==") X(assign, "=")               \
            X(notEquals, "!=") X(logicalNot, "!") X(plusEquals, "+=") X(plusplus, "++") X(plus, "+") X(minusEquals, "-=")    \
                X(minusminus, "--") X(minus, "-") X(timesEquals, "*=") X(times, "*") X(divideEquals, "/=") X(divide, "/")    \
                    X(moduloEquals, "%=") X(modulo, "%") X(xorEquals, "^=") X(bitwiseXor, "^") X(bitwiseNot, "~")            \
                        X(andEquals, "&=") X(logicalAnd, "&&") X(bitwiseAnd, "&") X(orEquals, "|=") X(logicalOr, "||")       \
                            X(bitwiseOr, "|") X(leftShiftEquals, "<<=") X(lessThanOrEqual, "<=") X(leftShift, "<<")          \
                                X(lessThan, "<") X(rightShiftUnsigned, ">>>") X(rightShiftEquals, ">>=") X(rightShift, ">>") \
...
#define DECLARE_LITTLEFOOT_TOKEN(name, str) static constexpr const char* name = str;
        ...
        struct TokenIterator
        {
            ...
            {
                if (currentType != expected) throwErrorExpecting(getTokenDescription(expected));
            ...
            {
                return currentType == t1 || matchesAny(others...);
            }
            bool matchesAny(TokenType t1) const noexcept
            {
                return currentType == t1;
            }
            ...
            {
                location.throwError("Found " + getTokenDescription(currentType) + " when expecting " + expected);
            }
...
#define LITTLEFOOT_COMPARE_KEYWORD(name, str)                                                                                \
    if (len == sizeof(str) - 1 && matchToken(Token::name, len)) return Token::name;
                ...
                if (parseStringLiteral(*p) || (*p == '.' && parseFloatLiteral())) return Token::literal;

#define LITTLEFOOT_COMPARE_OPERATOR(name, str)                                                                               \
    if (matchToken(Token::name, sizeof(str) - 1)) return Token::name;
                LITTLEFOOT_OPERATORS(LITTLEFOOT_COMPARE_OPERATOR)

                if (!p.isEmpty()) location.throwError("Unexpected character '" + String::charToString(*p) + "' in source");
        ...
        struct SyntaxTreeBuilder : private TokenIterator
        {
            SyntaxTreeBuilder(const String& code, const Array<NativeFunction>& nativeFns, uint32 defaultHeapSize)
                : TokenIterator(code)
                , nativeFunctions(nativeFns)
                , heapSizeRequired(defaultHeapSize)
            {
            }
            ...
            void compile()
            {
                ...
                while (currentType != Token::eof)
                {
                    ...
                    if (!matchesAnyTypeOrVoid()) throwErrorExpecting("a global variable or function");
                    ...
                    {
                        if (isConstVariable) location.throwError("Return type of a function cannot be const");

                        parseFunctionDeclaration(type, name);
                        continue;
                    }

                    if (type == Type::void_) location.throwError("A variable type cannot be 'void'");
            ...
            void simplify()
            {
                for (auto f : functions) f->block->simplify(*this);
            }

            const Function* findFunction(FunctionID functionID) const noexcept
            {
                for (auto f : functions)
                    if (f->functionID == functionID) return f;
            ...
            {
                auto o = new Type(args...);
                allAllocatedObjects.add(o);
                return o;
            }
        ...
        private:
            ...
            void parseGlobalVariableDeclaraion(bool isConst, Type type, String name)
            {
                for (;;)
                {
                    ...
                    if (arraySize > 0) location.throwError("Arrays not yet implemented!");

                    var constantInitialiser;

                    if (isConst) constantInitialiser = parseConstantExpressionInitialiser(type);
            ...
            void parseFunctionDeclaration(Type returnType, const String& name)
            {
                ...
                while (matchesAnyType())
                {
                    ...
                    if (f->arguments.size() > 127) location.throwError("Too many function arguments");

                    if (currentType == Token::closeParen) break;
                ...
                {
                    if (returnType != Type::void_) location.throwError("This function must return a value");
            ...
            BlockPtr parseBlock(bool isMainBlockOfFunction)
            {
                ...
                while (!matchIf(Token::closeBrace)) b->statements.add(parseStatement());
            ...
            StatementPtr parseStatement()
            {
                ...
                if (matchIf(Token::continue_))
                    return matchEndOfStatement(allocate<ContinueStatement>(location, blockBeingParsed));
            ...
            ExpPtr parseExpression()
            {
                ...
                {
                    auto loc = location;
                    return allocate<Assignment>(loc, blockBeingParsed, getIdentifierFromExpression(lhs), parseExpression(),
                                                false);
                }
            ...
            ExpPtr parseLogicOperator()
            {
                ...
                {
                    if (!matchesAny(Token::logicalAnd, Token::logicalOr, Token::bitwiseOr, Token::bitwiseAnd,
                                    Token::bitwiseXor))
                        return a;
            ...
            ExpPtr parseComparator()
            {
                ...
                {
                    if (!matchesAny(Token::equals, Token::notEquals, Token::lessThan, Token::lessThanOrEqual,
            ...
            ExpPtr parseShiftOperator()
            {
                ...
                {
                    if (!matchesAny(Token::leftShift, Token::rightShift, Token::rightShiftUnsigned)) return a;
            ...
            ExpPtr parseAdditionSubtraction()
            {
                ...
                {
                    if (!matchesAny(Token::plus, Token::minus)) return a;
            ...
            ExpPtr parseMultiplyDivide()
            {
                ...
                {
                    if (!matchesAny(Token::times, Token::divide, Token::modulo)) return a;
            ...
            ExpPtr parseFactor()
            {
                if (currentType == Token::identifier)
                    return parseSuffixes(allocate<Identifier>(location, blockBeingParsed, parseIdentifier()));
                ...
                if (matchesAny(Token::int_, Token::float_, Token::bool_)) return parseSuffixes(parseFunctionCall(skip()));
            ...
            {
                if (isConst && !matchesAnyType()) throwErrorExpecting("a type");
                ...
                for (StatementPtr result = nullptr;;)
                {
                    ...
                    else
                    {
                        ...
                        if (auto literal = dynamic_cast<LiteralValue*>(assignedValue))
                            if (static_cast<double>(literal->value) == 0) assignedValue = nullptr;

                        if (assignedValue != nullptr
                            || !blockBeingParsed
                                    ->isMainBlockOfFunction)  // no need to assign 0 for variables in the outer scope
                        {
                            if (assignedValue == nullptr)
            ...
            StatementPtr parseForLoop()
            {
                ...
                loopStatement->condition   = matchIf(Token::semicolon)
                                               ? allocate<LiteralValue>(location, blockBeingParsed, true)
            ...
            StatementPtr parseDoOrWhileLoop(bool isDoLoop)
            {
                ...
                if (!isDoLoop) loopStatement->body = parseStatement();
            ...
            {
                if (auto i = dynamic_cast<Identifier*>(e)) return i->name;
            ...
            {
                return matchesAny(Token::int_, Token::float_, Token::bool_);
            }
            bool matchesAnyTypeOrVoid() const noexcept
            {
                return matchesAnyType() || currentType == Token::void_;
            }
        ...
        struct CodeGenerator
        {
            ...
            void generateCode(BlockPtr outerBlock, uint32 heapSizeBytesRequired)
            {
                ...
                for (auto f : syntaxTree.functions) emit(f->functionID, f->address);

                auto codeStart = outputCode.size();

                for (auto f : syntaxTree.functions) f->emit(*this);
            ...
            {
                resolvedMarkers.add({m, outputCode.size()});
            }

            int getResolvedMarkerAddress(Marker marker) const
            {
                for (auto m : resolvedMarkers)
                    if (m.marker.index == marker.index) return m.address;
            ...
            Marker getMarkerAtAddress(int address) const noexcept
            {
                for (auto m : markersToResolve)
                    if (m.address == address) return m.marker;
            ...
            void removeCode(int address, int size)
            {
                ...
                for (auto& m : resolvedMarkers)
                    if (m.address >= address + size) m.address -= size;
            }
            ...
            void emit(Marker m)
            {
                markersToResolve.add({m, outputCode.size()});
                emit((int16)0);
            }
            void emit(int8 value)
            {
                outputCode.add((uint8)value);
            }
            void emit(int16 value)
            {
                uint8 d[2];
                Program::writeInt16(d, value);
                outputCode.insertArray(-1, d, (int)sizeof(d));
            }
            void emit(int32 value)
            {
                uint8 d[4];
                Program::writeInt32(d, value);
                outputCode.insertArray(-1, d, (int)sizeof(d));
            }
            ...
            void emitPush(const var& value)
            {
                if (value.isDouble())
                {
                    ...
                    if (v == 0)
                        ...
                        emit(OpCode::push32, Program::floatToInt(v));
                }
                else
                {
                    const int v = value;
                    ...
                    if (v == 0)
                        ...
                        emit(OpCode::push8, (int8)v);
                    else if (v > 0 && v < 32768)
                        emit(OpCode::push16, (int16)v);
                    else
                        emit(OpCode::push32, (int32)v);
                }
            }
            ...
            void emitCast(Type source, Type dest, const CodeLocation& location)
            {
                ...
                if ((source == Type::int_ || source == Type::bool_) && dest == Type::float_)
                    return emit(OpCode::int32ToFloat);
        ...
        struct Statement : public AllocatedObject
        {
            ...
            virtual Statement* simplify(SyntaxTreeBuilder&)
            {
                return this;
            }
        ...
        {
            Expression(const CodeLocation& l, BlockPtr parent) noexcept
                : Statement(l, parent)
            {
            }
        ...
        struct Function : public AllocatedObject
        {
            ...
            Array<Type> getArgumentTypes() const
            {
                ...
                for (auto& arg : arguments) argTypes.add(arg.type);
            ...
            static int countMaxNumLocalVariables(StatementPtr s) noexcept
            {
                ...
                if (s != nullptr) s->visitSubStatements(counter);

                if (auto block = dynamic_cast<BlockPtr>(s)) counter.num += block->variables.size();
        ...
        struct BlockStatement : public Statement
        {
            BlockStatement(const CodeLocation& l, BlockPtr parent, Function* f, bool isMainFunctionBlock)
                : Statement(l, parent)
                , function(f)
                , isMainBlockOfFunction(isMainFunctionBlock)
            {
            }

            void emit(CodeGenerator& cg, Type requiredType, int stackDepth) const override
            {
                jassert(requiredType == Type::void_);
                ignoreUnused(requiredType);
                jassert(function != nullptr);

                for (auto s : statements) s->emit(cg, Type::void_, stackDepth);
            }
            ...
            {
                for (auto s : statements) visit(s);
            }

            Statement* simplify(SyntaxTreeBuilder& stb) override
            {
                for (int i = 0; i < statements.size(); ++i) statements.set(i, statements.getReference(i)->simplify(stb));
            ...
            int getVariableDepth(const String& name, const CodeLocation& locationForError) const
            {
                ...
                if (index >= 0) return getNumVariablesInParentBlocks() + index;

                if (!isMainBlockOfFunction) return parentBlock->getVariableDepth(name, locationForError);

                for (int i = function->arguments.size(); --i >= 0;)
                    if (function->arguments.getReference(i).name == name) return i + 1 + function->getNumLocals();
            ...
            {
                return isMainBlockOfFunction ? 0
                                             : (parentBlock->getNumVariablesInParentBlocks() + parentBlock->variables.size());
            }
            ...
            {
                return parentBlock != nullptr ? parentBlock->getGlobalConstants() : constants;
            }
            ...
            const Variable& getVariable(const String& name, const CodeLocation& locationForError) const
            {
                ...
                if (!isMainBlockOfFunction) return parentBlock->getVariable(name, locationForError);
            ...
            static int indexOf(const Array<Variable>& vars, const String& name) noexcept
            {
                for (int i = 0; i < vars.size(); ++i)
                    if (vars.getReference(i).name == name) return i;
            ...
            Statement* simplify(SyntaxTreeBuilder& stb) override
            {
                ...
                if (auto literal = dynamic_cast<LiteralValue*>(condition))
                    return literal->value
                               ? trueBranch
                               : (falseBranch != nullptr ? falseBranch : stb.allocate<Statement>(location, parentBlock));
        ...
        struct TernaryOp : public Expression
        {
            ...
            Type getType(CodeGenerator& cg) const override
            {
                ...
                if (type != falseBranch->getType(cg))
                    location.throwError("Expected both branches of this ternary operator to have the same type");
            ...
            ExpPtr simplify(SyntaxTreeBuilder& stb) override
            {
                ...
                if (auto literal = dynamic_cast<LiteralValue*>(condition)) return literal->value ? trueBranch : falseBranch;
        ...
        {
            ReturnStatement(const CodeLocation& l, BlockPtr parent, ExpPtr v) noexcept
                : Statement(l, parent)
                , returnValue(v)
            {
            }
            ...
            StatementPtr simplify(SyntaxTreeBuilder& stb) override
            {
                if (returnValue != nullptr) returnValue = returnValue->simplify(stb);
        ...
        {
            BreakStatement(const CodeLocation& l, BlockPtr parent)
                : Statement(l, parent)
            {
            }
            ...
            {
                if (cg.breakTarget.index == 0) location.throwError("The break statement can only be used inside a loop");
        ...
        {
            ContinueStatement(const CodeLocation& l, BlockPtr parent)
                : Statement(l, parent)
            {
            }
        ...
        struct LiteralValue : public Expression
        {
            ...
            Type getType(CodeGenerator&) const override
            {
                ...
                if (t == Type::void_) location.throwError("Unsupported literal type");
        ...
        struct Identifier : public Expression
        {
            ...
            void emit(CodeGenerator& cg, Type requiredType, int stackDepth) const override
            {
                cg.emitVariableRead(getType(cg), requiredType, stackDepth, parentBlock->getVariableDepth(name, location),
                                    location);
            }
            ...
            ExpPtr simplify(SyntaxTreeBuilder& stb) override
            {
                ...
                if (v.isConst) return stb.allocate<LiteralValue>(location, parentBlock, v.constantValue);
        ...
        struct UnaryOp : public Expression
        {
            ...
            void emit(CodeGenerator& cg, Type requiredType, int stackDepth) const override
            {
                ...
                {
                    if (sourceType == Type::float_) location.throwError("Cannot perform this operation on a float");
            ...
            ExpPtr simplify(SyntaxTreeBuilder& stb) override
            {
                ...
                if (auto literal = dynamic_cast<LiteralValue*>(source))
                {
                    ...
                    if (type == Type::int_ && operation == Token::minus)
                    {
                        literal->value = -static_cast<int>(literal->value);
                        return literal;
                    }
                    if (type == Type::int_ && operation == Token::bitwiseNot)
                    {
                        literal->value = ~static_cast<int>(literal->value);
                        return literal;
                    }
                    if (type == Type::int_ && operation == Token::logicalNot)
                    {
                        literal->value = !static_cast<int>(literal->value);
                        return literal;
                    }
                    if (type == Type::bool_ && operation == Token::logicalNot)
                    {
                        literal->value = !static_cast<bool>(literal->value);
                        return literal;
                    }
                    if (type == Type::float_ && operation == Token::minus)
                    {
                        literal->value = -static_cast<double>(literal->value);
                        return literal;
                    }
                }
        ...
        struct BinaryOperator : public Expression
        {
            ...
            void emit(CodeGenerator& cg, Type requiredType, int stackDepth) const override
            {
                ...
                if (typeA == Type::float_ || typeB == Type::float_)
                {
                    ...
                    location.throwError("The operator " + getTokenDescription(operation)
                                        + " cannot take floating point arguments");
                }

                auto type = (typeA == Type::bool_ || typeB == Type::bool_) ? Type::bool_ : Type::int_;
            ...
            {
                if (operation == Token::logicalOr || operation == Token::logicalAnd || operation == Token::equals
                    || operation == Token::notEquals || operation == Token::lessThan || operation == Token::lessThanOrEqual
                    || operation == Token::greaterThan || operation == Token::greaterThanOrEqual)
                    return Type::bool_;

                if (operation == Token::plus || operation == Token::minus || operation == Token::times
                    || operation == Token::divide)
                {
                    if (typeA == Type::float_ || typeB == Type::float_) return Type::float_;
                }
            ...
            ExpPtr simplifyFloat(double a, double b, LiteralValue* literal)
            {
                ...
                if (operation == Token::equals)
                {
                    literal->value = a == b;
                    return literal;
                }
                if (operation == Token::notEquals)
                {
                    literal->value = a != b;
                    return literal;
                }
                if (operation == Token::lessThan)
                {
                    literal->value = a < b;
                    return literal;
                }
                if (operation == Token::lessThanOrEqual)
                {
                    literal->value = a <= b;
                    return literal;
                }
                if (operation == Token::greaterThan)
                {
                    literal->value = a > b;
                    return literal;
                }
                if (operation == Token::greaterThanOrEqual)
                {
                    literal->value = a >= b;
                    return literal;
                }
            ...
            {
                if (operation == Token::logicalOr)
                {
                    literal->value = a || b;
                    return literal;
                }
                if (operation == Token::logicalAnd)
                {
                    literal->value = a && b;
                    return literal;
                }
            ...
            ExpPtr simplifyInt(int a, int b, LiteralValue* literal)
            {
                ...
                if (operation == Token::bitwiseOr)
                {
                    literal->value = a | b;
                    return literal;
                }
                if (operation == Token::bitwiseAnd)
                {
                    literal->value = a & b;
                    return literal;
                }
                if (operation == Token::bitwiseXor)
                {
                    literal->value = a ^ b;
                    return literal;
                }
                if (operation == Token::leftShift)
                {
                    literal->value = a << b;
                    return literal;
                }
                if (operation == Token::rightShift)
                {
                    literal->value = a >> b;
                    return literal;
                }
            ...
            ExpPtr simplify(SyntaxTreeBuilder& stb) override
            {
                ...
                if (auto literal1 = dynamic_cast<LiteralValue*>(lhs))
                {
                    ...
                    {
                        auto resultType = getResultType(getTypeOfVar(literal1->value), getTypeOfVar(literal2->value));
        ...
        struct Assignment : public Expression
        {
            ...
            void emit(CodeGenerator& cg, Type requiredType, int stackDepth) const override
            {
                ...
                if (requiredType != Type::void_) cg.emitCast(variableType, requiredType, location);
            }
        ...
        struct FunctionCall : public Expression
        {
            ...
            Type getType(CodeGenerator& cg) const override
            {
                ...
                if (auto fn = cg.syntaxTree.findFunction(functionID)) return fn->returnType;

                if (auto nativeFn = cg.syntaxTree.findNativeFunction(functionID)) return nativeFn->returnType;

                if (auto b = findBuiltInFunction(functionID)) return b->returnType;
            ...
            {
                static constexpr const BuiltInFunction builtIns[] = {{OpCode::getHeapByte, Type::int_, "getHeapByte/ii"},
                ...
                for (auto& b : builtIns)
                    if (functionID == NativeFunction::createID(b.name)) return &b;
            ...
            void emitArgs(CodeGenerator& cg, const Array<Type>& argTypes, int stackDepth) const
            {
                for (int i = arguments.size(); --i >= 0;)
                {
                    ...
                    if (argValue->getType(cg) != argType)
                        location.throwError("Argument " + String(i + 1) + " requires an expression of type "
                                            + getTypeName(argType));
            ...
            void emitCast(CodeGenerator& cg, Type destType, int stackDepth) const
            {
                ...
                if (sourceIsFloat != destIsFloat) cg.emit(destIsFloat ? OpCode::int32ToFloat : OpCode::floatToInt32);
            }
            ...
            FunctionID getFunctionID(CodeGenerator& cg) const
            {
                ...
                for (auto arg : arguments) argTypes.add(arg->getType(cg));

                return createFunctionID(functionName, Type::void_,
                                        argTypes);  // NB: the ID ignores the return type so void is OK
            }

            void throwCannotFindFunctionError(CodeGenerator& cg, Type returnType) const
            {
                StringArray args;
                for (auto arg : arguments) args.add(getTypeName(arg->getType(cg)));

                auto desc = getTypeName(returnType) + " " + functionName + "(" + args.joinIntoString(", ") + ")";
            ...
            void visitSubStatements(Statement::Visitor& visit) const override
            {
                for (auto& arg : arguments) visit(arg);
            }

            ExpPtr simplify(SyntaxTreeBuilder& stb) override
            {
                for (auto& arg : arguments) arg = arg->simplify(stb);
        ...
        static Array<Type> getArgTypesFromFunctionName(const char* nameAndTypes)
        {
            ...
            for (int i = 0; i < args.length(); ++i) types.add(static_cast<Type>(args[i]));
