    ...
    struct NativeFunction
    {
        ...
        */
        NativeFunction(const char* nameAndArgumentTypes, ImplementationFunction fn) noexcept
            : nameAndArguments(nameAndArgumentTypes)
            , function(fn)
            , functionID(createID(nameAndArgumentTypes))
            , returnType()
            , numArgs()
        {
            ...
            if (slash > 0)
            {
                ...
                while (nameAndArgumentTypes[slash + 2 + numArgs] != 0) ++numArgs;
            }
        }
        ...
        static FunctionID createID(const char* nameAndArgTypes) noexcept
        {
            ...
            jassert(juce::String(nameAndArgTypes)
                        .substring(0, slash)
                        .containsOnly("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_"));
            ...
            jassert(juce::String(nameAndArgTypes)
                        .substring(slash + 2)
                        .containsOnly("ifb"));  // arguments must only be of these types

            for (; nameAndArgTypes[i] != 0; ++i)
                if (i != slash + 1) hash = hash * 31 + nameAndArgTypes[i];
    ...
    */
    struct Program
    {
        Program(const void* data, uint32 totalMemorySize) noexcept
            : programStart(static_cast<const uint8*>(data))
            , maxProgramSize(totalMemorySize)
        {
        ...
        uint16 calculateChecksum() const noexcept
        {
            ...
            for (uint32 i = 2; i < size; ++i) n += (n * 2) + programStart[i];
        ...
        {
            if (auto f = getFunctionEntry(functionIndex)) return static_cast<FunctionID>(readInt16(f));
        ...
        const uint8* getFunctionStartAddress(uint32 functionIndex) const noexcept
        {
            if (auto f = getFunctionEntry(functionIndex))
            {
                ...
                if (address < getProgramSize()) return programStart + address;
            }
        ...
        uint32 getProgramSize() const noexcept
        {
            ...
            return size < programHeaderSize ? programHeaderSize : (size > maxProgramSize ? maxProgramSize : size);
        }
        ...
        void dumpFunctionDisassembly(juce::OutputStream& out, uint32 functionIndex) const
        {
            ...
            if (auto codeStart = getFunctionStartAddress(functionIndex))
                if (auto codeEnd = getFunctionEndAddress(functionIndex))
                    for (auto prog = codeStart; prog < codeEnd;) out << getOpDisassembly(prog) << juce::newLine;
        }
...
#define LITTLEFOOT_OP_INT8(name)                                                                                             \
    case OpCode::name:                                                                                                       \
        s << #name << " " << juce::String::toHexString((int)*prog++).paddedLeft('0', 2);                                     \
        break;
#define LITTLEFOOT_OP_INT16(name)                                                                                            \
    case OpCode::name:                                                                                                       \
        s << #name << " " << juce::String::toHexString((int)readInt16(prog)).paddedLeft('0', 4);                             \
        prog += 2;                                                                                                           \
        break;
#define LITTLEFOOT_OP_INT32(name)                                                                                            \
    case OpCode::name:                                                                                                       \
        s << #name << " " << juce::String::toHexString((int)readInt32(prog)).paddedLeft('0', 8);                             \
        prog += 4;                                                                                                           \
        break;
...
#undef LITTLEFOOT_OP_INT32
        ...
        void dumpAllFunctions(juce::OutputStream& out) const
        {
            if (programStart != nullptr)
            {
                ...
                for (uint32 i = 0; i < getNumFunctions(); ++i) dumpFunctionDisassembly(out, i);
            }
        }
#endif
...
#undef LITTLEFOOT_OP_INT32
        ...
        {
            return (int16)(d[0] | (((uint16)d[1]) << 8));
        }
        static int32 readInt32(const uint8* d) noexcept
        {
            return (int32)(d[0] | (((uint32)d[1]) << 8) | (((uint32)d[2]) << 16) | (((uint32)d[3]) << 24));
        }
    ...
    private:
        ...
        {
            for (int i = 0; i < 4; ++i) ((uint8*)dest)[i] = ((const uint8*)src)[i];
        }
    };
    ...
    */
    ...
    {
        Runner() noexcept
            : program(allMemory, sizeof(allMemory))
        {
            reset();
        }
        ...
        */
        ...
        {
            jassert(index >= 0 && index < numNativeFunctions);
        ...
        void reset() noexcept
        {
            for (uint32 i = 0; i < sizeof(allMemory); ++i) allMemory[i] = 0;
        }
        ...
        static const char* getErrorDescription(ErrorCode e) noexcept
        {
            ...
            case ErrorCode::ok:
                return "OK";
            case ErrorCode::executionTimedOut:
                return "Timed-out";
            case ErrorCode::unknownInstruction:
                return "Illegal instruction";
            case ErrorCode::stackOverflow:
                return "Stack overflow";
            case ErrorCode::stackUnderflow:
                return "Stack underflow";
            case ErrorCode::illegalAddress:
        ...
        {
            return programAndHeapSpace;
        }

        /** */
        uint8* getProgramAndDataStart() const noexcept
        {
            return const_cast<uint8*>(allMemory);
        }
        /** */
        uint8* getProgramAndDataEnd() const noexcept
        {
            return reinterpret_cast<uint8*>(stackStart);
        }
        /** */
        uint32 getProgramAndDataSize() const noexcept
        {
            return (uint32)(getProgramAndDataEnd() - getProgramAndDataStart());
        }
        ...
        void setHeapByte(uint32 index, uint8 value) noexcept
        {
            ...
            if (addr < getProgramHeapEnd()) *addr = value;
        }
        ...
        {
            if (byteOffset + 3 < getProgramHeapSize()) Program::writeInt32(getProgramHeapStart() + byteOffset, (int32)value);
        ...
        struct FunctionExecutionContext
        {
            ...
            FunctionExecutionContext(Runner& r, FunctionID function) noexcept
                ...
                , programBase(r.program.programStart)
            ...
            {
                if (!isValid()) return ErrorCode::unknownFunction;
                ...
                for (;;)
                {
                    if (programCounter >= programEnd) return error;

                    if ((++opsPerformed & 63) == 0 && hasTimedOut()) return ErrorCode::executionTimedOut;
...
#define LITTLEFOOT_PERFORM_OP_INT32(name)                                                                                    \
    case OpCode::name:                                                                                                       \
        ...
        break;
                    ...
                    switch (op)
                    {
                        LITTLEFOOT_OPCODES(LITTLEFOOT_PERFORM_OP, LITTLEFOOT_PERFORM_OP_INT8, LITTLEFOOT_PERFORM_OP_INT16,
                                           LITTLEFOOT_PERFORM_OP_INT32)
                    default:
                        setError(ErrorCode::unknownInstruction);
                        break;
                    }
        ...
        private:
            ...
            {
                push32(Program::floatToInt(arg1));
            }

            int16 readProgram16() noexcept
            {
                auto v = Program::readInt16(programCounter);
                programCounter += sizeof(int16);
                return v;
            }
            int32 readProgram32() noexcept
            {
                auto v = Program::readInt32(programCounter);
                programCounter += sizeof(int32);
                return v;
            }
            ...
            void setError(ErrorCode e) noexcept
            {
                ...
                jassert(error == ErrorCode::ok);
            }

            bool checkStackUnderflow() noexcept
            {
                if (stack <= stackEnd) return true;
                setError(ErrorCode::stackUnderflow);
                return false;
            }
            ...
            {
                if (checkStackUnderflow()) tos = f(*stack++, tos);
            }
            void binaryOp(FloatBinaryOp f) noexcept
            {
                if (checkStackUnderflow())
                    tos = Program::floatToInt(f(Program::intToFloat(*stack++), Program::intToFloat(tos)));
            }
            ...
            void jump(int16 addr) noexcept
            {
                ...
                programCounter = programBase + (uint16)addr;
            }
            ...
            void call(int16 fnAddr) noexcept
            {
                if (flushTopToStack())
                {
                    tos = (int32)(programCounter - programBase);
                    jump(fnAddr);
                }
            }
            void retVoid(int8 numArgs) noexcept
            {
                if (tos == 0) return setError(ErrorCode::ok);
                auto retAddr = (int16)tos;
                stack += (uint8)numArgs;
                if (checkStackUnderflow())
                {
                    tos = *stack++;
            ...
            {
                auto retAddr = (int16)*stack++;
                if (retAddr == 0) return setError(ErrorCode::ok);
                stack += (uint8)numArgs;
                if (checkStackUnderflow()) jump(retAddr);
            }
            void drop() noexcept
            {
                if (checkStackUnderflow()) tos = *stack++;
            }
            ...
            {
                if (stack - num <= stackStart) return setError(ErrorCode::stackOverflow);
                flushTopToStack();
                for (int i = (uint8)num; --i > 0;) *--stack = 0;
                tos = 0;
            }
            void push0() noexcept
            {
            ...
            {
                dupOffset16((uint8)offset);
            }
            void dupOffset16(int16 offset) noexcept
            {
                if (flushTopToStack())
                {
                    auto addr = stack + offset;
                    if (addr < stackStart || addr >= stackEnd) return setError(ErrorCode::illegalAddress);
                    tos = *addr;
                }
            }
            void dropToStack(int8 offset) noexcept
            {
                dropToStack16((uint8)offset);
            }
            ...
            void dupFromGlobal(int16 index) noexcept
            {
                if (flushTopToStack())
                {
                    if (((uint16)index) >= numGlobals) return setError(ErrorCode::illegalAddress);
                    tos = globals[(uint16)index];
                }
            }
            void dropToGlobal(int16 index) noexcept
            {
                if (((uint16)index) >= numGlobals) return setError(ErrorCode::illegalAddress);
                globals[(uint16)index] = tos;
                drop();
            }
            void int32ToFloat() noexcept
            {
                tos = Program::floatToInt(static_cast<float>(tos));
            }
            void floatToInt32() noexcept
            {
                tos = static_cast<int32>(Program::intToFloat(tos));
            }
            void add_int32() noexcept
            {
                binaryOp([](int32 a, int32 b) { return a + b; });
            }
            ...
            {
                if (tos == 0) return setError(ErrorCode::divisionByZero);
                binaryOp([](int32 a, int32 b) { return a / b; });
            }
            ...
            {
                binaryOp([](int32 a, int32 b) { return (int32)(a || b); });
            }
            ...
            {
                tos = (Program::intToFloat(tos) == 0.0f);
            }
            ...
            {
                tos = runner->getHeapByte((uint32)tos);
            }
            void getHeapInt() noexcept
            {
                tos = runner->getHeapInt((uint32)tos);
            }
            void getHeapBits() noexcept
            {
                if (checkStackUnderflow()) tos = runner->getHeapBits((uint32)tos, (uint32)*stack++);
            }
            void setHeapByte() noexcept
            {
                if (checkStackUnderflow()) runner->setHeapByte((uint32)tos, (uint8)*stack++);
                drop();
            }
            void setHeapInt() noexcept
            {
                if (checkStackUnderflow()) runner->setHeapInt((uint32)tos, (uint32)*stack++);
                drop();
            }
                ...
                for (int i = 0; i < numFunctions; ++i)
                {
                    ...
                    if (f.functionID == functionID)
                    {
                        if (flushTopToStack())
                        {
                            ...
                            if (checkStackUnderflow() && f.returnType == Type::void_) drop();
                        }
            ...
            void dumpDebugTrace() const
            {
                ...
                for (auto s = stack; s < stackEnd; ++s) dump << juce::String::toHexString(*s) << ' ';
    ...
    private:
        ...
        Runner& reinitialiseProgramLayoutIfProgramHasChanged() noexcept
        {
            if (heapStart == nullptr && program.checksumMatches())
            {
                ...
                {
                    for (uint32 i = 0; i < numGlobals; ++i) globals[i] = 0;  // clear globals
