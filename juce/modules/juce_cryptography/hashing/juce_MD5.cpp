...
namespace juce
{
    ...
    public:
        ...
        void processBlock(const void* data, size_t dataSize) noexcept
        {
            ...
            if (count[0] < ((uint32)dataSize << 3)) count[1]++;
            ...
            if (dataSize >= spaceLeft)
            {
                ...
                for (i = spaceLeft; i + 64 <= dataSize; i += 64) transform(static_cast<const char*>(data) + i);
    ...
    private:
        ...
        static inline uint32 rotateLeft(const uint32 x, const uint32 n) noexcept
        {
            return (x << n) | (x >> (32 - n));
        }

        static inline uint32 F(const uint32 x, const uint32 y, const uint32 z) noexcept
        {
            return (x & y) | (~x & z);
        }
        static inline uint32 G(const uint32 x, const uint32 y, const uint32 z) noexcept
        {
            return (x & z) | (y & ~z);
        }
        static inline uint32 H(const uint32 x, const uint32 y, const uint32 z) noexcept
        {
            return x ^ y ^ z;
        }
        static inline uint32 I(const uint32 x, const uint32 y, const uint32 z) noexcept
        {
            return y ^ (x | ~z);
        }

        static void FF(uint32& a, const uint32 b, const uint32 c, const uint32 d, const uint32 x, const uint32 s,
                       const uint32 ac) noexcept
        {
            a += F(b, c, d) + x + ac;
            a = rotateLeft(a, s) + b;
        }

        static void GG(uint32& a, const uint32 b, const uint32 c, const uint32 d, const uint32 x, const uint32 s,
                       const uint32 ac) noexcept
        {
            a += G(b, c, d) + x + ac;
            a = rotateLeft(a, s) + b;
        ...
        static void HH(uint32& a, const uint32 b, const uint32 c, const uint32 d, const uint32 x, const uint32 s,
                       const uint32 ac) noexcept
        {
            a += H(b, c, d) + x + ac;
            a = rotateLeft(a, s) + b;
        ...
        static void II(uint32& a, const uint32 b, const uint32 c, const uint32 d, const uint32 x, const uint32 s,
                       const uint32 ac) noexcept
        {
            a += I(b, c, d) + x + ac;
            a = rotateLeft(a, s) + b;
