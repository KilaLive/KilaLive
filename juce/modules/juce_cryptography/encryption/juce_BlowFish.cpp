...
namespace juce
{
    BlowFish::BlowFish(const void* const keyData, const int keyBytes)
    {
        ...
        static const uint32 initialPValues[18] = {0x243f6a88, 0x85a308d3, 0x13198a2e, 0x03707344, 0xa4093822, 0x299f31d0,
        ...
        for (int i = 0; i < 18; ++i)
        {
            ...
            for (int k = 0; k < 4; ++k)
            {
                ...
                if (++keyIndex >= keyBytes) keyIndex = 0;
            }
    ...
    {
        for (auto& block : s) block.malloc(256);
    ...
    BlowFish& BlowFish::operator=(const BlowFish& other) noexcept
    {
        ...
        for (int i = 4; --i >= 0;) memcpy(s[i], other.s[i], 256 * sizeof(uint32));
    ...
    int BlowFish::encrypt(void* data, size_t size, size_t bufferSize) const noexcept
    {
        ...
        if (encryptedSize >= 0 && apply(data, static_cast<size_t>(encryptedSize), &BlowFish::encrypt)) return encryptedSize;
    ...
    {
        if (apply(data, size, &BlowFish::decrypt)) return unpad(data, size);
    ...
    bool BlowFish::apply(void* data, size_t size, void (BlowFish::*op)(uint32&, uint32&) const) const
    {
        ...
        if ((size % 8u) != 0) return false;

        auto  n   = size / 8u;
        auto* ptr = reinterpret_cast<AlignedAccessHelper*>(data);

        for (size_t i = 0; i < n; ++i) (this->*op)(ptr[i].data[0], ptr[i].data[1]);
    ...
    int BlowFish::pad(void* data, size_t size, size_t bufferSize) noexcept
    {
        ...
        if (n > bufferSize) return -1;

        auto* dst = reinterpret_cast<uint8*>(data);

        for (size_t i = size; i < n; ++i) dst[i] = paddingSize;
    ...
    int BlowFish::unpad(const void* data, size_t size) noexcept
    {
        ...
        if (paddingSize == 0 || paddingSize > 8 || paddingSize > size) return -1;
...
//==============================================================================
    ...
    public:
        BlowFishTests()
            : UnitTest("BlowFish", "Cryptography")
        {
        }
        ...
        static void fillMemoryBlockWithRandomData(MemoryBlock& block, Random& random)
        {
            ...
            for (size_t i = 0; i < n; ++i) dst[i] = static_cast<uint8>(random.nextInt(255));
        }
        ...
        void expectEqualData(const void* dataA, const void* dataB, size_t size, const String& failureMessage)
        {
            ...
            for (size_t i = 0; i < size; ++i) expectEquals((int)a[i], (int)b[i], failureMessage);
        }
