...
namespace juce
{
    namespace PrimesHelpers
    {
        static void createSmallSieve(const int numBits, BigInteger& result)
        {
            ...
            do
            {
                for (int i = n + n; i < numBits; i += n) result.setBit(i);

                n = result.findNextClearBit(n + 1);
            } while (n <= (numBits >> 1));
        }

        static void bigSieve(const BigInteger& base, const int numBits, BigInteger& result, const BigInteger& smallSieve,
        ...
        static bool findCandidate(const BigInteger& base, const BigInteger& sieve, const int numBits, BigInteger& result,
                                  const int certainty)
        {
            for (int i = 0; i < numBits; ++i)
            {
                if (!sieve[i])
                {
                    ...
                    if (Primes::isProbablyPrime(result, certainty)) return true;
                }
            }
        ...
        static bool passesMillerRabin(const BigInteger& n, int iterations)
        {
            ...
            for (;;)
            {
                ...
                if (numPrimesFound > iterations + 1) break;
            }
            ...
            while (--iterations >= 0)
            {
                ...
                if (r != one && r != nMinusOne)
                {
                    for (int j = 0; j < s; ++j)
                    {
                        ...
                        if (r == nMinusOne) break;
                    }

                    if (r != nMinusOne) return false;
                }
            }
    ...
    //==============================================================================
    BigInteger Primes::createProbablePrime(const int bitLength, const int certainty, const int* randomSeeds,
                                           int numRandomSeeds)
    {
        ...
        if (numRandomSeeds <= 0)
        {
            ...
            for (int j = 10; --j >= 0;)
            {
                ...
                for (int i = numRandomSeeds; --i >= 0;) defaultSeeds[i] ^= r1.nextInt() ^ r2.nextInt();
            }
        }
        ...
        while (p.getHighestBit() < bitLength)
        {
            ...
            BigInteger sieve;
            bigSieve(p, searchLen, sieve, smallSieve, smallSieveSize);

            BigInteger candidate;

            if (findCandidate(p, sieve, searchLen, candidate, certainty)) return candidate;
        }
    ...
    bool Primes::isProbablyPrime(const BigInteger& number, const int certainty)
    {
        ...
        {
            if (number.findGreatestCommonDivisor(2 * 3 * 5 * 7 * 11 * 13 * 17 * 19 * 23) != 1) return false;
