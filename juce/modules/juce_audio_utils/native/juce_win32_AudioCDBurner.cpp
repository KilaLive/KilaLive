...
namespace juce
{
    namespace CDBurnerHelpers
    {
        IDiscRecorder* enumCDBurners(StringArray* list, int indexToOpen, IDiscMaster** master)
        {
            ...
            if (SUCCEEDED(CoCreateInstance(CLSID_MSDiscMasterObj, 0, CLSCTX_INPROC_SERVER | CLSCTX_LOCAL_SERVER,
                                           IID_IDiscMaster, (void**)&dm)))
            {
                    ...
                    if (SUCCEEDED(dm->EnumDiscRecorders(&drEnum)))
                    {
                        ...
                        while (drEnum->Next(1, &dr, &dummy) == S_OK)
                        {
                            ...
                            else if (list != nullptr)
                            {
                                ...
                                if (SUCCEEDED(dr->GetPath(&path))) list->add((const WCHAR*)path);
                            }
    ...
    //==============================================================================
    class AudioCDBurner::Pimpl : public ComBaseClassHelper<IDiscMasterProgressEvents>, public Timer
    {
    public:
        ...
        {
            discRecorder->Close();
            if (redbook != nullptr) redbook->Release();
        ...
        {
            if (listener != nullptr && !shouldCancel) shouldCancel = listener->audioCDBurnProgress(progress);
        ...
        DiskState getDiskState()
        {
            ...
            if (type != 0 && (flags & MEDIA_WRITABLE) != 0) return writableDiskPresent;

            if (type == 0) return noDisc;
        ...
        int getIntProperty(const LPOLESTR name, const int defaultReturn) const
        {
            ...
            if (FAILED(discRecorder->GetRecorderProperties(prop.resetAndGetPointerAddress()))) return defaultReturn;
            ...
            return FAILED(prop->ReadMultiple(1, &iPropSpec, &iPropVariant)) ? defaultReturn : (int)iPropVariant.lVal;
        }

        bool setIntProperty(const LPOLESTR name, const int value) const
        {
            ComSmartPtr<IPropertyStorage> prop;
            if (FAILED(discRecorder->GetRecorderProperties(prop.resetAndGetPointerAddress()))) return false;
            ...
            PROPVARIANT iPropVariant;
            if (FAILED(prop->ReadMultiple(1, &iPropSpec, &iPropVariant))) return false;
    ...
    AudioCDBurner::AudioCDBurner(const int deviceIndex)
    {
        ...
        if (discRecorder != nullptr) pimpl.reset(new Pimpl(*this, discMaster, discRecorder));
    }

    AudioCDBurner::~AudioCDBurner()
    {
        if (pimpl != nullptr) pimpl.release()->releaseObjects();
    }
    ...
    Array<int> AudioCDBurner::getAvailableWriteSpeeds() const
    {
        ...
        for (int i = 0; i < numElementsInArray(speeds); ++i)
            if (speeds[i] <= maxSpeed) results.add(speeds[i]);
    ...
    {
        if (pimpl->getIntProperty(L"BufferUnderrunFreeCapable", 0) == 0) return false;
                               ...
                               bool performFakeBurnForTesting, int writeSpeed)
    {
        ...
        hr = pimpl->discMaster->RecordDisc(performFakeBurnForTesting, ejectDiscAfterwards);
    ...
    bool AudioCDBurner::addAudioTrack(AudioSource* audioSource, int numSamples)
    {
        ...
        while (ok)
        {
            ...
            typedef AudioData::Pointer<AudioData::Int16, AudioData::LittleEndian, AudioData::Interleaved, AudioData::NonConst>
                CDSampleFormat;
            ...
            if (samplesDone >= numSamples) break;
        }
