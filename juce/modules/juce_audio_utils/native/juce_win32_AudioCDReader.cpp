...
#define IOCTL_SCSI_PASS_THROUGH_DIRECT                                                                                       \
    CTL_CODE2(IOCTL_SCSI_BASE, 0x0405, METHOD_BUFFERED, FILE_READ_ACCESS | FILE_WRITE_ACCESS)
...
#pragma pack()
        ...
        public:
            ...
            bool isZero() const noexcept
            {
                for (int i = 0; i < dataLength; ++i)
                    if (buffer[dataStartOffset + i] != 0) return false;
        ...
        public:
            ...
            ~CDDeviceHandle()
            {
                ...
                if (scsiHandle != 0) CloseHandle(scsiHandle);
            }
        ...
        HANDLE createSCSIDeviceHandle(const char driveLetter)
        {
            ...
            HANDLE h =
                CreateFile(devicePath, flags, FILE_SHARE_WRITE | FILE_SHARE_READ, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);

            if (h == INVALID_HANDLE_VALUE)
            {
                flags ^= GENERIC_WRITE;
                h = CreateFile(devicePath, flags, FILE_SHARE_WRITE | FILE_SHARE_READ, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL,
                               0);
            }
            ...
            for (char driveLetter = 'b'; driveLetter <= 'z'; ++driveLetter)
            {
                ...
                if (GetDriveType(drivePath) == DRIVE_CDROM)
                {
                    ...
                    if (h != INVALID_HANDLE_VALUE)
                    {
                        ...
                        if (DeviceIoControl(h, IOCTL_SCSI_PASS_THROUGH_DIRECT, &p, sizeof(p), &p, sizeof(p), &bytesReturned,
                                            0)
                            != 0)
                        {
                            ...
                            if (DeviceIoControl(h, IOCTL_SCSI_GET_ADDRESS, 0, 0, &scsiAddr, sizeof(scsiAddr), &bytesReturned,
                                                0)
                                != 0)
                            {
        ...
        DWORD performScsiPassThroughCommand(SRB_ExecSCSICmd* const srb, const char driveLetter, HANDLE& deviceHandle,
                                            const bool retryOnFailure)
        {
            ...
            s.spt.DataIn = (BYTE)((srb->SRB_Flags & SRB_DIR_IN)
                                      ? SCSI_IOCTL_DATA_IN
                                      : ((srb->SRB_Flags & SRB_DIR_OUT) ? SCSI_IOCTL_DATA_OUT : SCSI_IOCTL_DATA_UNSPECIFIED));
            ...
            if (DeviceIoControl(deviceHandle, IOCTL_SCSI_PASS_THROUGH_DIRECT, &s, sizeof(s), &s, sizeof(s), &bytesReturned, 0)
                != 0)
            {
            ...
            else if (retryOnFailure)
            {
                ...
                {
                    if (error != ERROR_INVALID_HANDLE) CloseHandle(deviceHandle);
        ...
        public:
            ...
            {
                if (rb.numFrames * 2352 > rb.bufferSize) return false;
        ...
        public:
            ...
            bool read(CDReadBuffer& rb)
            {
                ...
                if (!initialised)
                {
                    ...
                    if (deviceInfo->readType == READTYPE_READ_D4_1) selectD4Mode();
        ...
        bool CDController::readAudio(CDReadBuffer& rb, CDReadBuffer* overlapBuffer)
        {
            if (overlapBuffer != nullptr)
            {
                ...
                if (doJitter && overlapBuffer->startFrame > 0 && overlapBuffer->numFrames > 0
                    && overlapBuffer->dataLength > 0)
                {
                    ...
                    if (overlapBuffer->startFrame == (rb.startFrame - framesToCheck))
                    {
                        ...
                        if (framesToCheck < framesOverlap && numFrames + framesOverlap <= rb.bufferSize / 2352)
                ...
                if (doJitter)
                {
                    ...
                    if (overlapBuffer->dataLength == 0 || overlapBuffer->isZero()) return true;
                ...
                if (canDoJitter)
                {
                    memcpy(overlapBuffer->buffer, rb.buffer + rb.dataStartOffset + 2352 * (rb.numFrames - framesToCheck),
                           2352 * framesToCheck);
        ...
        void CDDeviceHandle::performScsiCommand(HANDLE event, SRB_ExecSCSICmd& s)
        {
            ...
            if (status == SS_PENDING) WaitForSingleObject(event, 4000);
        ...
        bool CDDeviceHandle::readAudio(CDReadBuffer& buffer, CDReadBuffer* overlapBuffer)
        {
            ...
            {
                if (buffer.wantsIndex) buffer.index = controller->getLastIndex();
        ...
        struct CDDeviceWrapper
        {
            CDDeviceWrapper(const CDDeviceDescription& device, HANDLE scsiHandle)
                : deviceHandle(device, scsiHandle)
                , overlapBuffer(3)
                , jitter(false)
            {
        ...
        {
            return (((DWORD)t.addr[0]) << 24) + (((DWORD)t.addr[1]) << 16) + (((DWORD)t.addr[2]) << 8) + ((DWORD)t.addr[3]);
        }
    ...
    StringArray AudioCDReader::getAvailableCDNames()
    {
        ...
        {
            String s;
            if (list[i].scsiDriveLetter > 0) s << String::charToString(list[i].scsiDriveLetter).toUpperCase() << ": ";
    ...
    AudioCDReader* AudioCDReader::createReaderForCD(const int deviceIndex)
    {
        ...
        if (isPositiveAndBelow(deviceIndex, list.size()))
        {
            ...
            if (h != INVALID_HANDLE_VALUE)
            {
                ...
                if (cd->lengthInSamples > 0) return cd.release();
            }
        }
                                    ...
                                    int64 startSampleInFile, int numSamples)
    {
        ...
        while (numSamples > 0)
        {
            ...
            if (startSampleInFile >= bufferStartSample && startSampleInFile < bufferEndSample)
            {
    ...
    int AudioCDReader::getIndexAt(int samplePos)
    {
        ...
        for (i = 5; --i >= 0;)
        {
            ...
            if (device->deviceHandle.readAudio(readBuffer)) break;
        }

        if (i >= 0) return readBuffer.index;
        ...
        if (trackEnd - trackStart > 20 * 44100)
        {
            ...
            while (pos <= trackEnd - samplesPerFrame)
            {
                ...
                if (index == 0)
                {
                    ...
                    if (seenAnIndex) break;

                    pos -= 44100 * 5;

                    if (pos < trackStart) break;
                }
                else
                {
                    if (index > 0) seenAnIndex = true;
