...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD)                                                          \
    ...
    METHOD(getHumanReadableStringForBluetoothAddress, "getHumanReadableStringForBluetoothAddress",                           \
...
#undef JNI_CLASS_MEMBERS
    ...
    struct AndroidBluetoothMidiInterface
    {
        static void startStopScan(bool startScanning)
        {
            ...
            if (btManager.get() != nullptr)
                env->CallVoidMethod(btManager.get(), AndroidBluetoothManager.startStopScan,
                                    (jboolean)(startScanning ? 1 : 0));
        }
        ...
        static StringArray getBluetoothMidiDevicesNearby()
        {
            ...
            if (btManager.get() == nullptr) return {};

            jobjectArray jDevices =
                (jobjectArray)env->CallObjectMethod(btManager.get(), AndroidBluetoothManager.getMidiBluetoothAddresses);
        ...
        static bool pairBluetoothMidiDevice(const String& bluetoothAddress)
        {
            ...
            LocalRef<jobject> btManager(android.activity.callObjectMethod(JuceAppActivity.getAndroidBluetoothManager));
            if (btManager.get() == nullptr) return false;
        ...
        static String getHumanReadableStringForBluetoothAddress(const String& address)
        {
            ...
            if (btManager.get() == nullptr) return address;

            LocalRef<jstring> string((jstring)env->CallObjectMethod(
                btManager.get(), AndroidBluetoothManager.getHumanReadableStringForBluetoothAddress,
                javaString(address).get()));

            if (string.get() == nullptr) return address;
        ...
        static PairStatus isBluetoothDevicePaired(const String& address)
        {
            ...
            if (btManager.get() == nullptr) return unpaired;

            return static_cast<PairStatus>(env->CallIntMethod(
                btManager.get(), AndroidBluetoothManager.getBluetoothDeviceStatus, javaString(address).get()));
        }
    };
    ...
    //==============================================================================
    class AndroidBluetoothMidiDevicesListBox : public ListBox, private ListBoxModel, private Timer
    {
    public:
        //==============================================================================
    ...
    private:
        ...
        void paintListBoxItem(int rowNumber, Graphics& g, int width, int height, bool) override
        {
            if (isPositiveAndBelow(rowNumber, devices.size()))
            {
                ...
                g.drawText(statusString,
                           Rectangle<float>(deviceNameWidth + xmargin, ymargin, width - deviceNameWidth - (2.0f * xmargin),
                                            height - (2.0f * ymargin)),
        ...
        {
            if (deviceStatus == AndroidBluetoothMidiDevice::offline) return Colours::grey;
        ...
        {
            if (deviceStatus == AndroidBluetoothMidiDevice::offline || deviceStatus == AndroidBluetoothMidiDevice::connecting
                || deviceStatus == AndroidBluetoothMidiDevice::disconnecting)
                return Colours::grey;

            if (deviceStatus == AndroidBluetoothMidiDevice::connected) return Colours::green;
        ...
        //==============================================================================
        struct PairDeviceThread : public Thread, private AsyncUpdater
        {
            PairDeviceThread(const String& bluetoothAddressOfDeviceToPair, AndroidBluetoothMidiDevicesListBox& ownerListBox)
            ...
            {
                if (owner != nullptr) owner->pairDeviceThreadFinished();
        ...
        void updateDeviceList()
        {
            ...
            for (String* address = bluetoothAddresses.begin(); address != bluetoothAddresses.end(); ++address)
            {
    ...
    {
    public:
        BluetoothMidiSelectorOverlay(ModalComponentManager::Callback* exitCallbackToUse, const Rectangle<int>& boundsToUse)
            : bounds(boundsToUse)
        {
        ...
        void paint(Graphics& g) override
        {
            ...
            g.setFont(16);
            g.drawText("Bluetooth MIDI Devices", overlayBounds.removeFromTop(20).reduced(3, 3), Justification::topLeft, true);

            overlayBounds.removeFromTop(2);

            g.setFont(12);
            g.drawText("tap to connect/disconnect", overlayBounds.removeFromTop(18).reduced(3, 3), Justification::topLeft,
                       true);
        }
    ...
    //==============================================================================
    bool BluetoothMidiDevicePairingDialogue::open(ModalComponentManager::Callback* exitCallbackPtr, Rectangle<int>* btBounds)
    {
