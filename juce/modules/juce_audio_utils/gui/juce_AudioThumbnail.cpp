...
namespace juce
{
    struct AudioThumbnail::MinMaxValue
    {
        ...
        inline int8 getMinValue() const noexcept
        {
            return values[0];
        }
        inline int8 getMaxValue() const noexcept
        {
            return values[1];
        }
...
#if JUCE_ANDROID
            Range<float> limitedRange(jlimit(-1.0f, 1.0f, newRange.getStart()), jlimit(-1.0f, 1.0f, newRange.getEnd()));
        ...
        {
            return jmax(std::abs((int)values[0]), std::abs((int)values[1]));
        }
    ...
    public:
        ...
        int useTimeSlice() override
        {
...

            {
                ...
                if (reader != nullptr)
                {
                    if (!readNextBlock()) return 0;

                    justFinished = true;
                }
            }

            if (justFinished) owner.cache.storeThumb(owner, hashCode);
        ...
        bool readNextBlock()
        {
            ...
            if (!isFullyLoaded())
            {
                ...
                if (numToDo > 0)
                {
                    ...
                    for (int i = 0; i < (int)numChannels; ++i) levels[i] = levelData + i * numThumbSamps;
                    ...
                    for (int i = 0; i < numThumbSamps; ++i)
                    {
                        ...
                        for (int j = 0; j < (int)numChannels; ++j) levels[j][i].setFloat(levelsRead[j]);
                    }
    ...
    public:
        ...
        void write(const MinMaxValue* values, int startIndex, int numValues)
        {
            ...
            if (startIndex + numValues > data.size()) ensureSize(startIndex + numValues);

            auto* dest = getData(startIndex);

            for (int i = 0; i < numValues; ++i) dest[i] = values[i];
        }
    ...
    private:
        ...
        void ensureSize(int thumbSamples)
        {
            ...
            if (extraNeeded > 0) data.insertMultiple(-1, MinMaxValue(), extraNeeded);
        }
    };
    ...
    public:
        ...
        void drawChannel(Graphics& g, const Rectangle<int>& area, const double startTime, const double endTime,
                         const int channelNum, const float verticalZoomFactor, const double rate, const int numChans,
                         const int sampsPerThumbSample, LevelDataSource* levelData, const OwnedArray<ThumbData>& chans)
        {
            if (refillCache(area.getWidth(), startTime, endTime, rate, numChans, sampsPerThumbSample, levelData, chans)
    ...
    private:
                         ...
                         LevelDataSource* levelData, const OwnedArray<ThumbData>& chans)
        {
            ...
            if (numSamples == numSamplesCached && numChannelsCached == numChans && startTime == cachedStart
                && timePerPixel == cachedTimePerPixel && !cacheNeedsRefilling)
            {
            ...
            if (timePerPixel * rate <= sampsPerThumbSample && levelData != nullptr)
            {
                ...
                for (i = 0; i < numSamples; ++i)
                {
                    ...
                    if (sample >= 0)
                    {
                        ...
                        {
                            for (int chan = 0; chan < numChannelsCached; ++chan) *getData(chan, i) = MinMaxValue();
                        }
                        else
                        {
        ...
        void ensureSize(const int numSamples)
        {
            ...
            if (data.size() < itemsRequired) data.insertMultiple(-1, MinMaxValue(), itemsRequired - data.size());
        }
    };

    //==============================================================================
    AudioThumbnail::AudioThumbnail(const int originalSamplesPerThumbnailSample, AudioFormatManager& formatManager,
    ...
    {
        while (channels.size() < numChannels) channels.add(new ThumbData(length));
    }
    ...
    bool AudioThumbnail::loadFrom(InputStream& rawInput)
    {
        ...
        for (int i = 0; i < numThumbnailSamples; ++i)
            for (int chan = 0; chan < numChannels; ++chan) channels.getUnchecked(chan)->getData(i)->read(input);
    ...
    void AudioThumbnail::saveTo(OutputStream& output) const
    {
        ...
        for (int i = 0; i < numThumbnailSamples; ++i)
            for (int chan = 0; chan < numChannels; ++chan) channels.getUnchecked(chan)->getData(i)->write(output);
    }
    ...
    void AudioThumbnail::setReader(AudioFormatReader* newReader, int64 hash)
    {
        ...
        if (newReader != nullptr) setDataSource(new LevelDataSource(*this, newReader, hash));
    }
    ...
    void AudioThumbnail::addBlock(int64 startSample, const AudioBuffer<float>& incoming, int startOffsetInBuffer,
                                  int numSamples)
    {
        jassert(startSample >= 0 && startOffsetInBuffer >= 0 && startOffsetInBuffer + numSamples <= incoming.getNumSamples());
        ...
        if (numToDo > 0)
        {
            ...
            for (int chan = 0; chan < numChans; ++chan)
            {
                ...
                for (int i = 0; i < numToDo; ++i)
                {
                    ...
                    dest[i].setFloat(FloatVectorOperations::findMinAndMax(sourceData + start,
    ...
    void AudioThumbnail::setLevels(const MinMaxValue* const* values, int thumbIndex, int numChans, int numValues)
    {
        ...
        if (numSamplesFinished >= start && end > numSamplesFinished) numSamplesFinished = end;
    ...
    float AudioThumbnail::getApproximatePeak() const
    {
        ...
        for (auto* c : channels) peak = jmax(peak, c->getPeak());

        return jlimit(0, 127, peak) / 127.0f;
    }

    void AudioThumbnail::getApproximateMinMax(double startTime, double endTime, int channelIndex, float& minValue,
    ...
    void AudioThumbnail::drawChannel(Graphics& g, const Rectangle<int>& area, double startTime, double endTime,
                                     int channelNum, float verticalZoomFactor)
    {
        const ScopedLock sl(lock);

        window->drawChannel(g, area, startTime, endTime, channelNum, verticalZoomFactor, sampleRate, numChannels,
                            samplesPerThumbSample, source.get(), channels);
    }

    void AudioThumbnail::drawChannels(Graphics& g, const Rectangle<int>& area, double startTimeSeconds, double endTimeSeconds,
                                      float verticalZoomFactor)
    {
        for (int i = 0; i < numChannels; ++i)
        {
            ...
            drawChannel(g, {area.getX(), area.getY() + y1, area.getWidth(), y2 - y1}, startTimeSeconds, endTimeSeconds, i,
                        verticalZoomFactor);
        }
    }
