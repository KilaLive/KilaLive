...
namespace juce
{
    struct AudioVisualiserComponent::ChannelInfo
    {
        ...
        void clear() noexcept
        {
            for (int i = 0; i < levels.size(); ++i) levels.getReference(i) = Range<float>();
        ...
        {
            for (int i = 0; i < num; ++i) pushSample(inputSamples[i]);
        }
    ...
    void AudioVisualiserComponent::setNumChannels(const int numChannels)
    {
        ...
        for (int i = 0; i < numChannels; ++i) channels.add(new ChannelInfo(*this, numSamples));
    }
    ...
    void AudioVisualiserComponent::setBufferSize(int newNumSamples)
    {
        ...
        for (int i = 0; i < channels.size(); ++i) channels.getUnchecked(i)->setBufferSize(newNumSamples);
    }

    void AudioVisualiserComponent::clear()
    {
        for (int i = 0; i < channels.size(); ++i) channels.getUnchecked(i)->clear();
    }
    ...
    void AudioVisualiserComponent::pushBuffer(const float** d, int numChannels, int num)
    {
        ...
        for (int i = 0; i < numChannels; ++i) channels.getUnchecked(i)->pushSamples(d[i], num);
    }

    void AudioVisualiserComponent::pushBuffer(const AudioBuffer<float>& buffer)
    {
        pushBuffer(buffer.getArrayOfReadPointers(), buffer.getNumChannels(), buffer.getNumSamples());
    }
    ...
    void AudioVisualiserComponent::pushSample(const float* d, int numChannels)
    {
        ...
        for (int i = 0; i < numChannels; ++i) channels.getUnchecked(i)->pushSample(d[i]);
    }
    ...
    void AudioVisualiserComponent::getChannelAsPath(Path& path, const Range<float>* levels, int numLevels, int nextSample)
    {
        ...
        for (int i = numLevels; --i >= 0;) path.lineTo((float)i, -(levels[(nextSample + i) % numLevels].getStart()));

        path.closeSubPath();
    }

    void AudioVisualiserComponent::paintChannel(Graphics& g, Rectangle<float> area, const Range<float>* levels, int numLevels,
                                                int nextSample)
    {
        Path p;
        ...
        g.fillPath(
            p, AffineTransform::fromTargetPoints(0.0f, -1.0f, area.getX(), area.getY(), 0.0f, 1.0f, area.getX(),
