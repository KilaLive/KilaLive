...
namespace juce
{
    struct SimpleDeviceManagerInputLevelMeter : public Component, public Timer
    {
    ...
    //==============================================================================
    class AudioDeviceSelectorComponent::MidiInputSelectorComponentListBox : public ListBox, private ListBoxModel
    {
    public:
        ...
        void paintListBoxItem(int row, Graphics& g, int width, int height, bool rowIsSelected) override
        {
            ...
            {
                if (rowIsSelected) g.fillAll(findColour(TextEditor::highlightColourId).withMultipliedAlpha(0.3f));
                ...
                getLookAndFeel().drawTickBox(g, *this, x - tickW, (height - tickW) / 2, tickW, tickW, enabled, true, true,
                                             false);
        ...
        void listBoxItemClicked(int row, const MouseEvent& e) override
        {
            ...
            if (e.x < getTickX()) flipEnablement(row);
        }
    ...
    //==============================================================================
    class AudioDeviceSettingsPanel : public Component, private ChangeListener
    {
    public:
        ...
        void updateConfig(bool updateOutputDevice, bool updateInputDevice, bool updateSampleRate, bool updateBufferSize)
        {
            ...
            if (updateOutputDevice || updateInputDevice)
            {
                ...
                if (!type.hasSeparateInputsAndOutputs()) config.inputDeviceName = config.outputDeviceName;
            ...
            if (error.isNotEmpty())
                AlertWindow::showMessageBoxAsync(AlertWindow::WarningIcon, TRANS("Error when trying to open audio device!"),
                                                 error);
        }
        ...
        void updateAllControls()
        {
            ...
            else
            {
                ...
                if (outputDeviceDropDown != nullptr) outputDeviceDropDown->setSelectedId(-1, dontSendNotification);

                if (inputDeviceDropDown != nullptr) inputDeviceDropDown->setSelectedId(-1, dontSendNotification);
            }
    ...
    private:
        ...
        ScopedPointer<Label>    outputDeviceLabel, inputDeviceLabel, sampleRateLabel, bufferSizeLabel, inputChanLabel,
            outputChanLabel;
        ...
        void showCorrectDeviceName(ComboBox* box, bool isInput)
        {
            if (box != nullptr)
            {
                ...
                if (testButton != nullptr && !isInput) testButton->setEnabled(index >= 0);
            }
        }
        ...
        void addNamesToDeviceBox(ComboBox& combo, bool isInputs)
        {
            ...
            for (int i = 0; i < devs.size(); ++i) combo.addItem(devs[i], i + 1);
        ...
        int getLowestY() const
        {
            ...
            for (auto* c : getChildren()) y = jmax(y, c->getBottom());
        ...
        void updateControlPanelButton()
        {
            ...
            {
                showUIButton.reset(new TextButton(TRANS("Control Panel"), TRANS("Opens the device's own control panel")));
        ...
        void updateResetButton()
        {
            if (auto* currentDevice = setup.manager->getCurrentAudioDevice())
            {
                if (currentDevice->hasControlPanel())
                {
                    if (resetDeviceButton == nullptr)
                    {
                        resetDeviceButton.reset(new TextButton(
                            TRANS("Reset Device"), TRANS("Resets the audio interface - sometimes needed after changing a "
                                                         "device's properties in its custom control panel")));
            ...
            if (setup.maxNumOutputChannels > 0 || !type.hasSeparateInputsAndOutputs())
            {
                if (outputDeviceDropDown == nullptr)
                {
                    ...
                    outputDeviceLabel.reset(
                        new Label({}, type.hasSeparateInputsAndOutputs() ? TRANS("Output:") : TRANS("Device:")));
        ...
        void updateBufferSizeComboBox(AudioIODevice* currentDevice)
        {
            ...
            if (currentRate == 0) currentRate = 48000.0;
    ...
    public:
        //==============================================================================
        class ChannelSelectorListBox : public ListBox, private ListBoxModel
        {
        public:
            enum BoxType
            {
        ...
        private:
            ...
            static String getNameForChannelPair(const String& name1, const String& name2)
            {
                ...
                for (int j = 0; j < name1.length(); ++j)
                    if (name1.substring(0, j).equalsIgnoreCase(name2.substring(0, j))) commonBit = name1.substring(0, j);
            ...
            void flipEnablement(int row)
            {
                ...
                if (isPositiveAndBelow(row, items.size()))
                {
                    ...
                    if (setup.useStereoPairs)
                    {
                        ...
                        for (int i = 0; i < 256; i += 2) bits.setBit(i / 2, original[i] || original[i + 1]);
                        ...
                        for (int i = 0; i < 256; ++i) original.setBit(i, bits[i / 2]);
                    }
                    else
                    {
                        if (type == audioInputType)
            ...
            static void flipBit(BigInteger& chans, int index, int minNumber, int maxNumber)
            {
                ...
                if (chans[index])
                {
                    if (numActive > minNumber) chans.setBit(index, false);
                }
                else
                {
    ...
    //==============================================================================
    AudioDeviceSelectorComponent::AudioDeviceSelectorComponent(AudioDeviceManager& dm, int minInputChannelsToUse,
                                                               int maxInputChannelsToUse, int minOutputChannelsToUse,
                                                               int maxOutputChannelsToUse, bool showMidiInputOptions,
        ...
        , hideAdvancedOptionsWithButton(hideAdvancedOptionsWithButtonToUse)
    {
        ...
        if (types.size() > 1)
        {
            ...
            for (int i = 0; i < types.size(); ++i) deviceTypeDropDown->addItem(types.getUnchecked(i)->getTypeName(), i + 1);
        ...
        if (showMidiInputOptions)
        {
            midiInputsList.reset(
                new MidiInputSelectorComponentListBox(deviceManager, "(" + TRANS("No MIDI inputs available") + ")"));
    ...
    void AudioDeviceSelectorComponent::resized()
    {
        ...
        if (midiOutputSelector != nullptr) midiOutputSelector->setBounds(r.removeFromTop(itemHeight));
    ...
    void AudioDeviceSelectorComponent::updateMidiOutput()
    {
        ...
        if (midiDeviceName == getNoDeviceString()) midiDeviceName = {};
    ...
    void AudioDeviceSelectorComponent::updateAllControls()
    {
        ...
        if (midiOutputSelector != nullptr)
        {
            ...
            for (int i = 0; i < midiOuts.size(); ++i) midiOutputSelector->addItem(midiOuts[i], i + 1);
    ...
    void AudioDeviceSelectorComponent::handleBluetoothButton()
    {
        ...
        if (RuntimePermissions::isGranted(RuntimePermissions::bluetoothMidi)) BluetoothMidiDevicePairingDialogue::open();
    }
