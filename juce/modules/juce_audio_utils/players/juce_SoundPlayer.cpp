...
namespace juce
{
    ...
    {
        AudioSourceOwningTransportSource(PositionableAudioSource* s, double sampleRate)
            : source(s)
        {
    ...
    struct AutoRemovingTransportSource : public AudioTransportSource, private Timer
    {
        ...
        {
            if (!transportSource->isPlaying()) mixer.removeInputSource(this);
        }
    ...
    public:
        ...
        void setNextReadPosition(int64 newPosition) override
        {
            ...
            if (looping) newPosition = newPosition % static_cast<int64>(buffer->getNumSamples());
        ...
        void getNextAudioBlock(const AudioSourceChannelInfo& bufferToFill) override
        {
            ...
            if (samplesToCopy > 0)
            {
                ...
                if (!playAcrossAllChannels) maxOutChannels = jmin(maxOutChannels, maxInChannels);

                for (int i = 0; i < maxOutChannels; ++i)
                    bufferToFill.buffer->copyFrom(i, bufferToFill.startSample, *buffer, i % maxInChannels, position,
                                                  samplesToCopy);
            }

            position += samplesNeeded;

            if (looping) position %= bufferSize;
        }
    ...
    {
        if (file.existsAsFile()) play(formatManager.createReaderFor(file), true);
    }
    ...
    {
        if (reader != nullptr) play(new AudioFormatReaderSource(reader, deleteWhenFinished), true, reader->sampleRate);
    }

    void SoundPlayer::play(AudioBuffer<float>* buffer, bool deleteWhenFinished, bool playOnAllOutputChannels)
    {
        if (buffer != nullptr) play(new AudioBufferSource(buffer, deleteWhenFinished, playOnAllOutputChannels), true);
    }
    ...
    void SoundPlayer::play(PositionableAudioSource* audioSource, bool deleteWhenFinished, double fileSampleRate)
    {
        ...
        {
            if (deleteWhenFinished) delete audioSource;
        }
    }
    ...
    //==============================================================================
    void SoundPlayer::audioDeviceIOCallback(const float** inputChannelData, int numInputChannels, float** outputChannelData,
