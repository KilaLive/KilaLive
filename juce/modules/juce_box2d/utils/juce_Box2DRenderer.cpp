...
namespace juce
{
    ...
    void Box2DRenderer::render(Graphics& g, b2World& world, float left, float top, float right, float bottom,
                               const Rectangle<float>& target)
    {
        graphics = &g;

        g.addTransform(AffineTransform::fromTargetPoints(left, top, target.getX(), target.getY(), right, top,
                                                         target.getRight(), target.getY(), left, bottom, target.getX(),
                                                         target.getBottom()));
    ...
    static void createPath(Path& p, const b2Vec2* vertices, int32 vertexCount)
    {
        ...
        for (int i = 1; i < vertexCount; ++i) p.lineTo(vertices[i].x, vertices[i].y);
    ...
    {
        graphics->setColour(getColour(color));
        graphics->drawEllipse(center.x - radius, center.y - radius, radius * 2.0f, radius * 2.0f, getLineThickness());
    }
    ...
    void Box2DRenderer::DrawSolidCircle(const b2Vec2& center, float32 radius, const b2Vec2& /*axis*/, const b2Color& colour)
    {
        ...
        graphics->fillEllipse(center.x - radius, center.y - radius, radius * 2.0f, radius * 2.0f);
    }
