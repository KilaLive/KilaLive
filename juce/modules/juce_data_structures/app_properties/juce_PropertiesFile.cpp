...
namespace juce
{
    ...
    File PropertiesFile::Options::getDefaultFile() const
    {
        ...
        if (folderName.isNotEmpty()) dir = dir.getChildFile(folderName);

#elif JUCE_LINUX || JUCE_ANDROID
        auto dir = File(commonToAllUsers ? "/var" : "~")
                       .getChildFile(folderName.isNotEmpty() ? folderName : ("." + applicationName));
        ...
        dir = dir.getChildFile(folderName.isNotEmpty() ? folderName : applicationName);
#endif

        return (filenameSuffix.startsWithChar(L'.') ? dir.getChildFile(applicationName).withFileExtension(filenameSuffix)
    ...
    bool PropertiesFile::save()
    {
        ...
        if (options.doNotSave || file == File() || file.isDirectory() || !file.getParentDirectory().createDirectory())
            return false;

        if (options.storageFormat == storeAsXML) return saveAsXml();
        ...
        if (doc != nullptr && doc->hasTagName(PropertyFileConstants::fileTag))
        {
            ...
            if (doc != nullptr)
            {
                forEachXmlChildElementWithTagName(*doc, e, PropertyFileConstants::valueTag)
                {
                    ...
                    if (name.isNotEmpty())
                        getAllProperties().set(name, e->getFirstChildElement() != nullptr
    ...
    bool PropertiesFile::loadAsBinary()
    {
        ...
        if (fileStream.openedOk())
        {
            ...
            if (magicNumber == PropertyFileConstants::magicNumber) return loadAsBinary(fileStream);
        }
    ...
    bool PropertiesFile::saveAsBinary()
    {
...

        {
            ...
            if (options.storageFormat == storeAsCompressedBinary)
            {
                ...
                if (!writeToStream(zipped)) return false;
            }
            else
            {
                ...
                if (!writeToStream(out)) return false;
            }
        }

        if (!tempFile.overwriteTargetFileWithTemporary()) return false;
    ...
    bool PropertiesFile::writeToStream(OutputStream& out)
    {
        ...
        if (!out.writeInt(numProperties)) return false;
