...
namespace juce
{
    struct UndoManager::ActionSet
    {
        ...
        bool undo() const
        {
            for (int i = actions.size(); --i >= 0;)
                if (!actions.getUnchecked(i)->undo()) return false;
        ...
        int getTotalSize() const
        {
            ...
            for (auto* a : actions) total += a->getSizeInUnits();
    ...
    bool UndoManager::perform(UndoableAction* const newAction, const String& actionName)
    {
        ...
        {
            if (actionName.isNotEmpty()) setCurrentTransactionName(actionName);
    ...
    {
        while (nextIndex > 0 && totalUnitsStored > maxNumUnitsToKeep && transactions.size() > minimumTransactionsToKeep)
        {
    ...
    {
        if (auto* action = getCurrentSet()) return action->name;
    ...
    {
        if (auto* s = getCurrentSet()) return s->name;
    ...
    {
        if (auto* s = getNextSet()) return s->name;
    ...
    {
        if (auto* s = getCurrentSet()) return s->time;
    ...
    {
        if (auto* s = getNextSet()) return s->time;
    ...
    void UndoManager::getActionsInCurrentTransaction(Array<const UndoableAction*>& actionsFound) const
    {
        if (!newTransaction)
            if (auto* s = getCurrentSet())
                for (auto* a : s->actions) actionsFound.add(a);
    }

    int UndoManager::getNumActionsInCurrentTransaction() const
    {
        if (!newTransaction)
            if (auto* s = getCurrentSet()) return s->actions.size();
