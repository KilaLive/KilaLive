...
namespace juce
{
    ...
    public:
        ...
        void callListeners(ValueTree::Listener* listenerToExclude, Function fn) const
        {
            ...
            else if (numListeners > 0)
            {
                ...
                for (int i = 0; i < numListeners; ++i)
                {
                    ...
                    if (i == 0 || valueTreesWithListeners.contains(v)) v->listeners.callExcluding(listenerToExclude, fn);
                }
            }
        }
        ...
        {
            for (auto* t = this; t != nullptr; t = t->parent) t->callListeners(listenerToExclude, fn);
        }
        ...
        {
            ValueTree tree(this);
            callListenersForAllParents(nullptr,
                                       [=, &tree, &child](Listener& l) { l.valueTreeChildRemoved(tree, child, index); });
        }
        ...
        void sendChildOrderChangedMessage(int oldIndex, int newIndex)
        {
            ValueTree tree(this);
            callListenersForAllParents(nullptr,
                                       [=, &tree](Listener& l) { l.valueTreeChildOrderChanged(tree, oldIndex, newIndex); });
        }
        ...
        void sendParentChangeMessage()
        {
            ...
            for (int j = children.size(); --j >= 0;)
                if (auto* child = children.getObjectPointer(j)) child->sendParentChangeMessage();
                         ...
                         ValueTree::Listener* listenerToExclude = nullptr)
        {
            ...
            {
                if (properties.set(name, newValue)) sendPropertyChangeMessage(name, listenerToExclude);
            }
            else
            {
                if (auto* existingValue = properties.getVarPointer(name))
                {
                    if (*existingValue != newValue)
                        undoManager->perform(
                            new SetPropertyAction(this, name, newValue, *existingValue, false, false, listenerToExclude));
                }
                else
                {
        ...
        void removeProperty(const Identifier& name, UndoManager* undoManager)
        {
            if (undoManager == nullptr)
            {
                if (properties.remove(name)) sendPropertyChangeMessage(name);
            }
            else
            {
        ...
        void removeAllProperties(UndoManager* undoManager)
        {
            ...
            else
            {
                for (int i = properties.size(); --i >= 0;)
                    undoManager->perform(
                        new SetPropertyAction(this, properties.getName(i), {}, properties.getValueAt(i), false, true));
            }
        }
        ...
        {
            for (int i = properties.size(); --i >= 0;)
                if (!source.properties.contains(properties.getName(i))) removeProperty(properties.getName(i), undoManager);
        ...
        {
            for (auto* s : children)
                if (s->type == typeToMatch) return ValueTree(s);
        ...
        {
            for (auto* s : children)
                if (s->properties[propertyName] == propertyValue) return ValueTree(s);
        ...
        void addChild(SharedObject* child, int index, UndoManager* undoManager)
        {
            if (child != nullptr && child->parent != this)
            {
                if (child != this && !isAChildOf(child))
                {
                    ...
                    {
                        if (!isPositiveAndBelow(index, children.size())) index = children.size();
        ...
        {
            while (children.size() > 0) removeChild(children.size() - 1, undoManager);
        }
        ...
        void moveChild(int currentIndex, int newIndex, UndoManager* undoManager)
        {
            ...
            if (currentIndex != newIndex && isPositiveAndBelow(currentIndex, children.size()))
            {
                if (undoManager == nullptr)
                {
                ...
                {
                    if (!isPositiveAndBelow(newIndex, children.size())) newIndex = children.size() - 1;
        ...
        {
            if (type != other.type || properties.size() != other.properties.size() || children.size() != other.children.size()
                || properties != other.properties)
                return false;
        ...
        void writeToStream(OutputStream& output) const
        {
            ...
            for (int i = 0; i < children.size(); ++i) writeObjectToStream(output, children.getObjectPointerUnchecked(i));
        }
        ...
        {
            SetPropertyAction(SharedObject* so, const Identifier& propertyName, const var& newVal, const var& oldVal,
                              bool isAdding, bool isDeleting, ValueTree::Listener* listenerToExclude = nullptr)
                : target(so)
                , name(propertyName)
                , newValue(newVal)
                , oldValue(oldVal)
                , isAddingNewProperty(isAdding)
                , isDeletingProperty(isDeleting)
    ...
    ValueTree::ValueTree(const Identifier& type, std::initializer_list<std::pair<Identifier, var>> properties,
                         std::initializer_list<ValueTree> subTrees)
        : ValueTree(type)
    {
        for (auto& prop : properties) setProperty(prop.first, prop.second, nullptr);

        for (auto& tree : subTrees) addChild(tree, -1, nullptr);
    }
#endif

    ValueTree::ValueTree(SharedObject* so) noexcept
        : object(so)
    {
    }

    ValueTree::ValueTree(const ValueTree& other) noexcept
        : object(other.object)
    {
    }
    ...
    ValueTree& ValueTree::operator=(const ValueTree& other)
    {
        if (object != other.object)
        {
            ...
            {
                if (object != nullptr) object->valueTreesWithListeners.removeValue(this);

                if (other.object != nullptr) other.object->valueTreesWithListeners.add(this);
    ...
    ValueTree::~ValueTree()
    {
        if (listeners.size() > 0 && object != nullptr) object->valueTreesWithListeners.removeValue(this);
    }
    ...
    {
        return ValueTree(object != nullptr ? object->parent : static_cast<SharedObject*>(nullptr));
    }

    ValueTree ValueTree::getRoot() const noexcept
    {
        return ValueTree(object != nullptr ? object->getRoot() : static_cast<SharedObject*>(nullptr));
    }
    ...
    {
        return object == nullptr ? defaultReturnValue : object->properties.getWithDefault(name, defaultReturnValue);
    }
                                                       ...
                                                       const var& newValue, UndoManager* undoManager)
    {
        ...
        if (object != nullptr) object->setProperty(name, newValue, undoManager, listenerToExclude);
    ...
    {
        if (object != nullptr) object->removeProperty(name, undoManager);
    }

    void ValueTree::removeAllProperties(UndoManager* undoManager)
    {
        if (object != nullptr) object->removeAllProperties(undoManager);
    }
    ...
    //==============================================================================
    struct ValueTreePropertyValueSource : public Value::ValueSource, private ValueTree::Listener
    {
        ValueTreePropertyValueSource(const ValueTree& vt, const Identifier& prop, UndoManager* um, bool sync)
            : tree(vt)
            , property(prop)
            , undoManager(um)
            , updateSynchronously(sync)
        {
    ...
    private:
        ...
        {
            if (tree == changedTree && property == changedProperty) sendChangeMessage(updateSynchronously);
        }
    ...
    {
        return ValueTree(object != nullptr ? object->children.getObjectPointer(index) : static_cast<SharedObject*>(nullptr));
    }
    ...
    void ValueTree::addChild(const ValueTree& child, int index, UndoManager* undoManager)
    {
        ...
        if (object != nullptr) object->addChild(child.object, index, undoManager);
    }
    ...
    {
        if (object != nullptr) object->removeChild(childIndex, undoManager);
    }

    void ValueTree::removeChild(const ValueTree& child, UndoManager* undoManager)
    {
        if (object != nullptr) object->removeChild(object->children.indexOf(child.object), undoManager);
    }

    void ValueTree::removeAllChildren(UndoManager* undoManager)
    {
        if (object != nullptr) object->removeAllChildren(undoManager);
    }

    void ValueTree::moveChild(int currentIndex, int newIndex, UndoManager* undoManager)
    {
        if (object != nullptr) object->moveChild(currentIndex, newIndex, undoManager);
    }
    ...
    void ValueTree::createListOfChildren(OwnedArray<ValueTree>& list) const
    {
        ...
        for (auto* o : object->children) list.add(new ValueTree(o));
    }
    ...
    void ValueTree::addListener(Listener* listener)
    {
        if (listener != nullptr)
        {
            if (listeners.isEmpty() && object != nullptr) object->valueTreesWithListeners.add(this);
    ...
    void ValueTree::removeListener(Listener* listener)
    {
        ...
        if (listeners.isEmpty() && object != nullptr) object->valueTreesWithListeners.removeValue(this);
    }

    void ValueTree::sendPropertyChangeMessage(const Identifier& property)
    {
        if (object != nullptr) object->sendPropertyChangeMessage(property);
    }
    ...
    ValueTree ValueTree::fromXml(const XmlElement& xml)
    {
        if (!xml.isTextElement())
        {
            ...
            forEachXmlChildElement(xml, e) v.appendChild(fromXml(*e), nullptr);
    ...
    String ValueTree::toXmlString() const
    {
        ...
        if (xml != nullptr) return xml->createDocument({});
...
//==============================================================================
    ...
    public:
        ...
        static String createRandomIdentifier(Random& r)
        {
            ...
            if (!XmlElement::isValidXmlName(result)) result = createRandomIdentifier(r);
        ...
        static ValueTree createRandomTree(UndoManager* undoManager, int depth, Random& r)
        {
            ...
            for (int i = r.nextInt(10); --i >= 0;)
            {
                ...
                case 2:
                    if (depth < 5)
                        v.addChild(createRandomTree(undoManager, depth + 1, r), r.nextInt(v.getNumChildren() + 1),
                                   undoManager);
                    break;
                case 3:
