...
namespace juce
{
        ...
        if (numListeners > 0)
        {
            if (synchronous)
            {
                ...
                for (int i = numListeners; --i >= 0;)
                    if (Value* const v = valuesWithListeners[i]) v->callListeners();
            }
            else
            {
                triggerAsyncUpdate();
    ...
    void Value::addListener(Value::Listener* listener)
    {
        if (listener != nullptr)
        {
            if (listeners.size() == 0) value->valuesWithListeners.add(this);
    ...
    void Value::removeListener(Value::Listener* listener)
    {
        ...
        if (listeners.size() == 0) value->valuesWithListeners.removeValue(this);
    }
