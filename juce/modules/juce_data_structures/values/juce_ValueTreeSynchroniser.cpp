...
namespace juce
{
    namespace ValueTreeSynchroniserHelpers
    {
        ...
        static void writeHeader(ValueTreeSynchroniser& target, MemoryOutputStream& stream, ChangeType type, ValueTree v)
        {
            ...
            for (int i = path.size(); --i >= 0;) stream.writeCompressedInt(path.getUnchecked(i));
        }
        ...
        static ValueTree readSubTreeLocation(MemoryInputStream& input, ValueTree v)
        {
            ...
            for (int i = numLevels; --i >= 0;)
            {
                ...
                if (!isPositiveAndBelow(index, v.getNumChildren())) return {};
    ...
    bool ValueTreeSynchroniser::applyChange(ValueTree& root, const void* data, size_t dataSize, UndoManager* undoManager)
    {
        ...
        case ValueTreeSynchroniserHelpers::childMoved:
        {
            ...
            if (isPositiveAndBelow(oldIndex, v.getNumChildren()) && isPositiveAndBelow(newIndex, v.getNumChildren()))
            {
