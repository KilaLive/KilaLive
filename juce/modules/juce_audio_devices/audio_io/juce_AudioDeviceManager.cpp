...
namespace juce
{
    ...
    {
        return outputDeviceName == other.outputDeviceName && inputDeviceName == other.inputDeviceName
               && sampleRate == other.sampleRate && bufferSize == other.bufferSize && inputChannels == other.inputChannels
               && useDefaultInputChannels == other.useDefaultInputChannels && outputChannels == other.outputChannels
    ...
    void AudioDeviceManager::createDeviceTypesIfNeeded()
    {
        if (availableDeviceTypes.size() == 0)
        {
            ...
            for (auto* t : types) addAudioDeviceType(t);

            types.clear(false);

            if (auto* first = availableDeviceTypes.getFirst()) currentDeviceType = first->getTypeName();
        }
    }
    ...
    {
        if (device != nullptr) list.add(device);
    }
    ...
    static bool deviceListContains(AudioIODeviceType* type, bool isInput, const String& name)
    {
        ...
        for (int i = devices.size(); --i >= 0;)
            if (devices[i].trim().equalsIgnoreCase(name.trim())) return true;

        return false;
    }

    //==============================================================================
    String AudioDeviceManager::initialise(const int numInputChannelsNeeded, const int numOutputChannelsNeeded,
                                          ...
                                          const AudioDeviceSetup* preferredSetupOptions)
    {
        ...
        if (xml != nullptr && xml->hasTagName("DEVICESETUP"))
            return initialiseFromXML(*xml, selectDefaultDeviceOnFailure, preferredDefaultDeviceName, preferredSetupOptions);
    ...
    String AudioDeviceManager::initialiseFromXML(const XmlElement& xml, const bool selectDefaultDeviceOnFailure,
                                                 const String&           preferredDefaultDeviceName,
                                                 const AudioDeviceSetup* preferredSetupOptions)
    {
        ...
        if (preferredSetupOptions != nullptr) setup = *preferredSetupOptions;
        ...
        forEachXmlChildElementWithTagName(xml, c, "MIDIINPUT") midiInsFromXml.add(c->getStringAttribute("name"));

        for (auto& m : MidiInput::getDevices()) setMidiInputEnabled(m, midiInsFromXml.contains(m));
    ...
    String AudioDeviceManager::initialiseWithDefaultDevices(int numInputChannelsNeeded, int numOutputChannelsNeeded)
    {
    ...
    void AudioDeviceManager::scanDevicesIfNeeded()
    {
        if (listNeedsScanning)
        {
            ...
            for (int i = availableDeviceTypes.size(); --i >= 0;) availableDeviceTypes.getUnchecked(i)->scanForDevices();
        }
    }
    ...
    AudioIODeviceType* AudioDeviceManager::findType(const String& typeName)
    {
        ...
        for (int i = availableDeviceTypes.size(); --i >= 0;)
            if (availableDeviceTypes.getUnchecked(i)->getTypeName() == typeName) return availableDeviceTypes.getUnchecked(i);
    ...
    void AudioDeviceManager::setCurrentAudioDeviceType(const String& type, const bool treatAsChosenDevice)
    {
        ...
        {
            if (availableDeviceTypes.getUnchecked(i)->getTypeName() == type && currentDeviceType != type)
            {
                if (currentAudioDevice != nullptr)
                {
                    closeAudioDevice();
    ...
    String AudioDeviceManager::setAudioDeviceSetup(const AudioDeviceSetup& newSetup, const bool treatAsChosenDevice)
    {
        jassert(&newSetup != &currentSetup);  // this will have no effect

        if (newSetup == currentSetup && currentAudioDevice != nullptr) return {};
        ...
        if (type == nullptr || (newInputDeviceName.isEmpty() && newOutputDeviceName.isEmpty()))
        {
            ...
            if (treatAsChosenDevice) updateXml();

            return {};
        }

        if (currentSetup.inputDeviceName != newInputDeviceName || currentSetup.outputDeviceName != newOutputDeviceName
    ...
    double AudioDeviceManager::chooseBestSampleRate(double rate) const
    {
        ...
        if (rate > 0 && rates.contains(rate)) return rate;
        ...
        for (int i = rates.size(); --i >= 0;)
        {
            ...
            if (sr >= 44100.0 && (lowestAbove44 < 1.0 || sr < lowestAbove44)) lowestAbove44 = sr;
        }

        if (lowestAbove44 > 0.0) return lowestAbove44;
    ...
    int AudioDeviceManager::chooseBestBufferSize(int bufferSize) const
    {
        ...
        if (bufferSize > 0 && currentAudioDevice->getAvailableBufferSizes().contains(bufferSize)) return bufferSize;
    ...
    {
        if (currentAudioDevice != nullptr) currentAudioDevice->stop();
    ...
    void AudioDeviceManager::restartLastAudioDevice()
    {
        if (currentAudioDevice == nullptr)
        {
            if (currentSetup.inputDeviceName.isEmpty() && currentSetup.outputDeviceName.isEmpty())
            {
    ...
    void AudioDeviceManager::updateXml()
    {
        ...
        if (currentAudioDevice != nullptr)
        {
            ...
            if (currentAudioDevice->getDefaultBufferSize() != currentAudioDevice->getCurrentBufferSizeSamples())
                lastExplicitSettings->setAttribute("audioDeviceBufferSize",
        ...
        for (int i = 0; i < enabledMidiInputs.size(); ++i)
            lastExplicitSettings->createNewChildElement("MIDIINPUT")->setAttribute("name", enabledMidiInputs[i]->getName());
        ...
        if (midiInsFromXml.size() > 0)
        {
            ...
            for (int i = 0; i < midiInsFromXml.size(); ++i)
                if (!availableMidiDevices.contains(midiInsFromXml[i], true))
                    lastExplicitSettings->createNewChildElement("MIDIINPUT")->setAttribute("name", midiInsFromXml[i]);
        }
    ...
    void AudioDeviceManager::removeAudioCallback(AudioIODeviceCallback* callbackToRemove)
    {
        if (callbackToRemove != nullptr)
        {
            ...
            if (needsDeinitialising) callbackToRemove->audioDeviceStopped();
        }
    }

    void AudioDeviceManager::audioDeviceIOCallbackInt(const float** inputChannelData, int numInputChannels,
                                                      float** outputChannelData, int numOutputChannels, int numSamples)
    {
        ...
        if (callbacks.size() > 0)
        {
            ...
            callbacks.getUnchecked(0)->audioDeviceIOCallback(inputChannelData, numInputChannels, outputChannelData,
            ...
            {
                callbacks.getUnchecked(i)->audioDeviceIOCallback(inputChannelData, numInputChannels, tempChans,
                                                                 numOutputChannels, numSamples);
                ...
                for (int chan = 0; chan < numOutputChannels; ++chan)
                {
                    if (auto* src = tempChans[chan])
                        if (auto* dst = outputChannelData[chan])
                            for (int j = 0; j < numSamples; ++j) dst[j] += src[j];
                }
            }
            ...
            if (msTaken > msPerBlock) xruns++;
        }
        else
        {
            for (int i = 0; i < numOutputChannels; ++i) zeromem(outputChannelData[i], sizeof(float) * (size_t)numSamples);
        }
        ...
        if (testSound != nullptr)
        {
            ...
            for (int i = 0; i < numOutputChannels; ++i)
                for (int j = 0; j < numSamps; ++j) outputChannelData[i][j] += src[j];

            testSoundPosition += numSamps;

            if (testSoundPosition >= testSound->getNumSamples()) testSound.reset();
        }
    }
    ...
    void AudioDeviceManager::audioDeviceAboutToStartInt(AudioIODevice* const device)
    {
...

        {
            ...
            for (int i = callbacks.size(); --i >= 0;) callbacks.getUnchecked(i)->audioDeviceAboutToStart(device);
        }
    ...
    void AudioDeviceManager::audioDeviceStoppedInt()
    {
        ...
        for (int i = callbacks.size(); --i >= 0;) callbacks.getUnchecked(i)->audioDeviceStopped();
    }
    ...
    void AudioDeviceManager::audioDeviceErrorInt(const String& message)
    {
        ...
        for (int i = callbacks.size(); --i >= 0;) callbacks.getUnchecked(i)->audioDeviceError(message);
    }
        ...
        if (enabled != isMidiInputEnabled(name))
        {
            ...
            else
            {
                for (int i = enabledMidiInputs.size(); --i >= 0;)
                    if (enabledMidiInputs[i]->getName() == name) enabledMidiInputs.remove(i);
            }
    ...
    bool AudioDeviceManager::isMidiInputEnabled(const String& name) const
    {
        for (auto* mi : enabledMidiInputs)
            if (mi->getName() == name) return true;
    ...
    void AudioDeviceManager::setDefaultMidiOutput(const String& deviceName)
    {
        if (defaultMidiOutputName != deviceName)
        {
            ...
            if (currentAudioDevice != nullptr)
                for (int i = oldCallbacks.size(); --i >= 0;) oldCallbacks.getUnchecked(i)->audioDeviceStopped();
            ...
            if (currentAudioDevice != nullptr)
                for (auto* c : oldCallbacks) c->audioDeviceAboutToStart(currentAudioDevice.get());
    ...
    void AudioDeviceManager::LevelMeter::updateLevel(const float* const* channelData, int numChannels,
                                                     int numSamples) noexcept
    {
        ...
        if (enabled.get() != 0 && numChannels > 0)
        {
            for (int j = 0; j < numSamples; ++j)
            {
                ...
                for (int i = 0; i < numChannels; ++i) s += std::abs(channelData[i][j]);
    ...
    void AudioDeviceManager::playTestSound()
    {
        ...
        if (currentAudioDevice != nullptr)
        {
            ...
            for (int i = 0; i < soundLength; ++i) newSound->setSample(0, i, amplitude * (float)std::sin(i * phasePerSample));
    ...
    double AudioDeviceManager::getCurrentInputLevel() const noexcept
    {
        return inputLevelMeter.getCurrentLevel();
    }
    double AudioDeviceManager::getCurrentOutputLevel() const noexcept
    {
        return outputLevelMeter.getCurrentLevel();
    }

    void AudioDeviceManager::enableInputLevelMeasurement(bool enable) noexcept
    {
        inputLevelMeter.setEnabled(enable);
    }
    void AudioDeviceManager::enableOutputLevelMeasurement(bool enable) noexcept
    {
        outputLevelMeter.setEnabled(enable);
    }
