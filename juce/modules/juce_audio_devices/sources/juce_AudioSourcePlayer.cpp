...
namespace juce
{
    ...
    void AudioSourcePlayer::setSource(AudioSource* newSource)
    {
        if (source != newSource)
        {
            ...
            if (newSource != nullptr && bufferSize > 0 && sampleRate > 0) newSource->prepareToPlay(bufferSize, sampleRate);
            ...
            if (oldSource != nullptr) oldSource->releaseResources();
        }
    }
    ...
    void AudioSourcePlayer::audioDeviceIOCallback(const float** inputChannelData, int totalNumInputChannels,
                                                  float** outputChannelData, int totalNumOutputChannels, int numSamples)
    {
        ...
        if (source != nullptr)
        {
            ...
            for (int i = 0; i < totalNumInputChannels; ++i)
            {
                if (inputChannelData[i] != nullptr)
                {
                    ...
                    if (numInputs >= numElementsInArray(inputChans)) break;
                }
            }
            ...
            for (int i = 0; i < totalNumOutputChannels; ++i)
            {
                if (outputChannelData[i] != nullptr)
                {
                    ...
                    if (numOutputs >= numElementsInArray(outputChans)) break;
                }
            }
        ...
        {
            for (int i = 0; i < totalNumOutputChannels; ++i)
                if (outputChannelData[i] != nullptr) zeromem(outputChannelData[i], sizeof(float) * (size_t)numSamples);
        }
    }
    ...
    void AudioSourcePlayer::prepareToPlay(double newSampleRate, int newBufferSize)
    {
        ...
        if (source != nullptr) source->prepareToPlay(bufferSize, sampleRate);
    }
