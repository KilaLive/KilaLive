...
namespace juce
{
    ...
    void AudioTransportSource::setSource(PositionableAudioSource* const newSource, int readAheadSize,
                                         TimeSliceThread* readAheadThread, double sourceSampleRateToCorrectFor,
                                         int maxNumChannels)
    {
        if (source == newSource)
        {
        ...
        if (oldMasterSource != nullptr) oldMasterSource->releaseResources();
    }
    ...
    void AudioTransportSource::stop()
    {
        if (playing)
        {
            ...
            int n = 500;
            while (--n >= 0 && !stopped) Thread::sleep(2);
    ...
    {
        if (sampleRate > 0.0) setNextReadPosition((int64)(newPosition * sampleRate));
    }

    double AudioTransportSource::getCurrentPosition() const
    {
        if (sampleRate > 0.0) return (double)getNextReadPosition() / sampleRate;
    ...
    {
        if (sampleRate > 0.0) return (double)getTotalLength() / sampleRate;
    ...
    void AudioTransportSource::setNextReadPosition(int64 newPosition)
    {
        if (positionableSource != nullptr)
        {
            ...
            if (resamplerSource != nullptr) resamplerSource->flushBuffers();
    ...
    void AudioTransportSource::prepareToPlay(int samplesPerBlockExpected, double newSampleRate)
    {
        ...
        if (masterSource != nullptr) masterSource->prepareToPlay(samplesPerBlockExpected, sampleRate);
    ...
    void AudioTransportSource::releaseMasterResources()
    {
        ...
        if (masterSource != nullptr) masterSource->releaseResources();
    ...
    void AudioTransportSource::getNextAudioBlock(const AudioSourceChannelInfo& info)
    {
        ...
        if (masterSource != nullptr && !stopped)
        {
            ...
            if (!playing)
            {
                ...
                if (info.numSamples > 256) info.buffer->clear(info.startSample + 256, info.numSamples - 256);
            }
