...
#define SL_ANDROID_RECORDING_PRESET_UNPROCESSED ((SLuint32)0x00000005)
    ...
    public:
        ...
        SlObjectRef(SlObjectRef&& obj) noexcept
            : cb(static_cast<ReferenceCountedObjectPtr<ControlBlock>&&>(obj.cb))
        {
            obj.cb = nullptr;
        }
        ...
        {
            cb   = static_cast<ReferenceCountedObjectPtr<ControlBlock>&&>(r.cb);
            r.cb = nullptr;
            return *this;
        }
        ...
        //==============================================================================
        const SLObjectItf_* const operator*() noexcept
        {
            return *cb->ptr.get();
        }
    ...
    private:
        ...
        {
            ScopedPointer<const SLObjectItf_* const> ptr;
            ControlBlock() {}
    ...
    public:
        ...
        SlRef(SlRef&& r) noexcept
            : SlObjectRef(static_cast<SlRef&&>(r))
            , type(r.type)
        {
            r.type = nullptr;
        }
        ...
        {
            SlObjectRef::operator=(static_cast<SlObjectRef&&>(r));
            type                 = r.type;
            r.type               = nullptr;
            return *this;
        }
        ...
        //==============================================================================
        static SlRef cast(SlObjectRef& base)
        {
            return SlRef(base);
        }
        static SlRef cast(SlObjectRef&& base)
        {
            return SlRef(static_cast<SlObjectRef&&>(base));
        }
    ...
    private:
            ...
            : SlObjectRef(base)
        {
            ...
            if (type == nullptr || err != SL_RESULT_SUCCESS) *this = nullptr;
        }

        SlRef(SlObjectRef&& base)
            : SlObjectRef(static_cast<SlObjectRef&&>(base))
        {
            SLObjectItf obj                    = SlObjectRef::operator->();
            SLresult                       err = (*obj)->GetInterface(obj, &IntfIID<T>::iid, &type);
            base                               = nullptr;
    ...
    struct ContainerDeletePolicy<const SLObjectItf_* const>
    {
        ...
        {
            if (object != nullptr) (*object)->Destroy(object);
        }
    };
        ...
        static void convertFromOpenSL(const int16* srcInterleaved, AudioBuffer<float>& audioBuffer)
        {
            for (int i = 0; i < audioBuffer.getNumChannels(); ++i)
            {
                typedef AudioData::Pointer<AudioData::Float32, AudioData::NativeEndian, AudioData::NonInterleaved,
                                           AudioData::NonConst>
                    DstSampleType;
                typedef AudioData::Pointer<AudioData::Int16, AudioData::LittleEndian, AudioData::Interleaved,
                                           AudioData::Const>
                    SrcSampleType;
    ...
    struct BufferHelpers<float>
    {
        ...
        {
            if (audioBuffer.getNumChannels() == 1) audioBuffer.setDataToReferTo(&native, 1, audioBuffer.getNumSamples());
        }
        ...
        static void convertFromOpenSL(const float* srcInterleaved, AudioBuffer<float>& audioBuffer)
        {
            ...
            for (int i = 0; i < audioBuffer.getNumChannels(); ++i)
            {
                typedef AudioData::Pointer<AudioData::Float32, AudioData::NativeEndian, AudioData::NonInterleaved,
                                           AudioData::NonConst>
                    DstSampleType;
                typedef AudioData::Pointer<AudioData::Float32, AudioData::LittleEndian, AudioData::Interleaved,
                                           AudioData::Const>
                    SrcSampleType;
    ...
    public:
        ...
        struct OpenSLQueueRunner
        {
            ...
            bool init()
            {
                ...
                const bool supportsJavaProxy =
                    (getEnv()->GetStaticIntField(AndroidBuildVersion, AndroidBuildVersion.SDK_INT) >= 24);
                ...
                if (supportsJavaProxy)
                {
                    ...
                    if (config != nullptr)
                    {
                        ...
                        auto    status =
                            (*config)->AcquireJavaProxy(config, /*SL_ANDROID_JAVA_PROXY_ROUTING*/ 1, &audioRoutingJni);

                        if (status == SL_RESULT_SUCCESS && audioRoutingJni != 0) javaProxy = GlobalRef(audioRoutingJni);
                    }
                }
            ...
            void clear()
            {
                ...
                zeromem(nativeBuffer.get(),
                        static_cast<size_t>(owner.bufferSize * numChannels * owner.numBuffers) * sizeof(T));
                scratchBuffer.clear();
                (*queue)->Clear(queue);
            }
        ...
        struct OpenSLQueueRunnerPlayer : OpenSLQueueRunner<T, OpenSLQueueRunnerPlayer<T>, SLPlayItf_>
        {
            ...
            {
                SLDataLocator_AndroidSimpleBufferQueue queueLocator = {SL_DATALOCATOR_ANDROIDSIMPLEBUFFERQUEUE,
                ...
                SLresult status =
                    (*Base::owner.engine)
                        ->CreateAudioPlayer(Base::owner.engine, &obj, &source, &sink, 2, queueInterfaces, interfaceRequired);
                if (status != SL_RESULT_SUCCESS || obj == nullptr || (*obj)->Realize(obj, 0) != SL_RESULT_SUCCESS)
                {
                    if (obj != nullptr) (*obj)->Destroy(obj);
        ...
        struct OpenSLQueueRunnerRecorder : OpenSLQueueRunner<T, OpenSLQueueRunnerRecorder<T>, SLRecordItf_>
        {
            ...
            {
                SLDataLocator_IODevice                 ioDeviceLocator = {SL_DATALOCATOR_IODEVICE, SL_IODEVICE_AUDIOINPUT,
                ...
                SLresult status = (*Base::owner.engine)
                                      ->CreateAudioRecorder(Base::owner.engine, &obj, &source, &sink, 2, queueInterfaces,
                                                            interfaceRequired);
                if (status != SL_RESULT_SUCCESS || obj == nullptr || (*obj)->Realize(obj, 0) != SL_RESULT_SUCCESS)
                {
                    if (obj != nullptr) (*obj)->Destroy(obj);
            ...
            bool setAudioPreprocessingEnabled(bool shouldEnable)
            {
                ...
                {
                    const bool supportsUnprocessed =
                        (getEnv()->GetStaticIntField(AndroidBuildVersion, AndroidBuildVersion.SDK_INT) >= 25);
                    ...
                    SLresult status = (*Base::config)
                                          ->SetConfiguration(Base::config, SL_ANDROID_KEY_RECORDING_PRESET,
            ...
            void setState(bool running)
            {
                (*Base::runner)->SetRecordState(Base::runner, running ? SL_RECORDSTATE_RECORDING : SL_RECORDSTATE_STOPPED);
            }
        };
        ...
        {
        public:
            OpenSLSession(DynamicLibrary& slLibraryToUse, int numInputChannels, int numOutputChannels, double samleRateToUse,
            ...
            {
                return (engine != nullptr && (outputChannels == 0 || (outputMix != nullptr)));
            }
            virtual void start()
            {
                stop();
            ...
            void setCallback(AudioIODeviceCallback* callbackToUse)
            {
                ...
                while (true)
                {
                    ...
                    if (old == callbackToUse) break;

                    if (callback.compareAndSetBool(callbackToUse, old)) break;
            ...
            static OpenSLSession* create(DynamicLibrary& slLibrary, int numInputChannels, int numOutputChannels,
                                         double samleRateToUse, int bufferSizeToUse, int numBuffersToUse);

            //==============================================================================
            typedef SLresult (*CreateEngineFunc)(SLObjectItf*, SLuint32, const SLEngineOption*, SLuint32,
        ...
        {
        public:
            OpenSLSessionT(DynamicLibrary& slLibraryToUse, int numInputChannels, int numOutputChannels, double samleRateToUse,
                           int bufferSizeToUse, int numBuffersToUse)
                : OpenSLSession(slLibraryToUse, numInputChannels, numOutputChannels, samleRateToUse, bufferSizeToUse,
                                numBuffersToUse)
            {
                jassert(numInputChannels > 0 || numOutputChannels > 0);
                ...
                if (OpenSLSession::openedOK())
                {
                    ...
                    if (outputChannels > 0)
                    {
                        ...
                        const bool supportsUnderrunCount =
                            (getEnv()->GetStaticIntField(AndroidBuildVersion, AndroidBuildVersion.SDK_INT) >= 24);
                        getUnderrunCount =
                            supportsUnderrunCount ? getEnv()->GetMethodID(AudioTrack, "getUnderrunCount", "()I") : 0;
                    }
                }
            }
            ...
            void start() override
            {
                ...
                if (inputChannels > 0) recorder->clear();

                if (outputChannels > 0) player->clear();

                // first enqueue all buffers
                for (int i = 0; i < numBuffers; ++i) doSomeWorkOnAudioThread();

                if (inputChannels > 0) recorder->setState(true);

                if (outputChannels > 0) player->setState(true);
            }
            ...
            void stop() override
            {
                ...
                while (!guard.compareAndSetBool(1, 0)) Thread::sleep(1);

                if (inputChannels > 0) recorder->setState(false);

                if (outputChannels > 0) player->setState(false);
            ...
            bool setAudioPreprocessingEnabled(bool shouldEnable) override
            {
                if (shouldEnable != audioProcessingEnabled)
                {
                    ...
                    if (recorder != nullptr) return recorder->setAudioPreprocessingEnabled(audioProcessingEnabled);
                }
            ...
            void doSomeWorkOnAudioThread()
            {
                ...
                if (guard.compareAndSetBool(1, 0))
                {
                        ...
                        && (outputChannels == 0 || player->isBufferAvailable()))
                    {
                        ...
                        if (recorderBuffer != nullptr) recorder->enqueueBuffer();
        ...
        Array<double> getAvailableSampleRates() override
        {
            ...
            if (native != 0.0 && !retval.contains(native)) retval.add(native);
        ...
        Array<int> getAvailableBufferSizes() override
        {
            ...
            Array<int> retval;
            for (int i = minBuffersToQueue; i <= maxBuffersToQueue; ++i) retval.add(i * nativeBufferSize);

            return retval;
        }

        String open(const BigInteger& inputChannels, const BigInteger& outputChannels, double requestedSampleRate,
                    int bufferSize) override
        {
            ...
            session = OpenSLSession::create(slLibrary, numInputChannels, numOutputChannels, sampleRate, actualBufferSize,
            ...
            else
            {
                    ...
                    && RuntimePermissions::isGranted(RuntimePermissions::recordAudio))
                {
                    ...
                    session = OpenSLSession::create(slLibrary, numInputChannels, numOutputChannels, sampleRate,
                                                    actualBufferSize, audioBuffersToEnqueue);
                }
            }

            DBG("OpenSL: numInputChannels = " << numInputChannels << ", numOutputChannels = " << numOutputChannels
                                              << ", nativeBufferSize = " << getNativeBufferSize() << ", nativeSampleRate = "
                                              << getNativeSampleRate() << ", actualBufferSize = " << actualBufferSize
                                              << ", audioBuffersToEnqueue = " << audioBuffersToEnqueue
                                              << ", sampleRate = " << sampleRate << ", supportsFloatingPoint = "
                                              << (session != nullptr && session->supportsFloatingPoint() ? "true" : "false"));

            if (session == nullptr) lastError = "Unknown error initializing opensl session";
        ...
        bool setAudioPreprocessingEnabled(bool shouldAudioProcessingBeEnabled) override
        {
            ...
            if (session != nullptr) session->setAudioPreprocessingEnabled(audioProcessingEnabled);
    ...
    private:
        ...
        static int buffersToQueueForBufferDuration(int bufferDurationInMs, double sampleRate) noexcept
        {
            ...
            auto maxNumBuffers   = static_cast<int>(
                std::ceil(static_cast<double>(maxBufferFrames) / static_cast<double>(getNativeBufferSize())));
        ...
        static String audioManagerGetProperty(const String& property)
        {
            ...
            const LocalRef<jstring> text(
                (jstring)android.activity.callObjectMethod(JuceAppActivity.audioManagerGetProperty, jProperty.get()));
            if (text.get() != 0) return juceString(text);
        ...
        {
            return ((requestedBufferSize % getNativeBufferSize()) == 0) && (requestedSampleRate == getNativeSampleRate())
                   && isProAudioDevice();
        }
                                                                                   ...
                                                                                   int numBuffersToUse)
    {
        ...
        if (sdkVersion >= 21)
        {
            ...
            if (retval != nullptr && (!retval->openedOK())) retval = nullptr;
        }
    ...
    public:
        ...
        AudioIODevice* createDevice(const String& outputDeviceName, const String& inputDeviceName) override
        {
            ...
            if (outputDeviceName.isNotEmpty() || inputDeviceName.isNotEmpty())
                dev = new OpenSLAudioIODevice(outputDeviceName.isNotEmpty() ? outputDeviceName : inputDeviceName);
    ...
    public:
        ...
        SLRealtimeThread()
        {
            if (auto createEngine =
                    (OpenSLAudioIODevice::OpenSLSession::CreateEngineFunc)slLibrary.getFunction("slCreateEngine"))
            {
                SLObjectItf obj = nullptr;
                auto        err = createEngine(&obj, 0, nullptr, 0, nullptr, nullptr);

                if (err != SL_RESULT_SUCCESS || obj == nullptr) return;
