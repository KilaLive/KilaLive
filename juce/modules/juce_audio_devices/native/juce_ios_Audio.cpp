...
#define JUCE_NSERROR_CHECK(X)                                                                                                \
    ...
    //==============================================================================
    class iOSAudioIODeviceType : public AudioIODeviceType, public AsyncUpdater
    {
    public:
    ...
    //==============================================================================
    struct iOSAudioIODevice::Pimpl : public AudioPlayHead, public AsyncUpdater
    {
        ...
        static void setAudioSessionCategory(NSString* category)
        {
            ...
            JUCE_NSERROR_CHECK([[AVAudioSession sharedInstance] setCategory:category withOptions:options error:&error]);
        }

        static void setAudioSessionActive(bool enabled)
        {
            JUCE_NSERROR_CHECK([[AVAudioSession sharedInstance] setActive:enabled error:&error]);
        }
        ...
        int tryBufferSize(const double currentSampleRate, const int newBufferSize)
        {
            NSTimeInterval bufferDuration =
                currentSampleRate > 0 ? (NSTimeInterval)((newBufferSize + 1) / currentSampleRate) : 0.0;
        ...
        void updateAvailableBufferSizes()
        {
...
#if JUCE_IOS_AUDIO_LOGGING
            {
                ...
                for (auto size : availableBufferSizes) info << " " << size;
        ...
        void updateAvailableSampleRates()
        {
            ...
            AudioUnitRemovePropertyListenerWithUserData(audioUnit, kAudioUnitProperty_StreamFormat,
...
#if JUCE_IOS_AUDIO_LOGGING
            {
                ...
                for (auto rate : availableSampleRates) info << " " << rate;
        ...
        {
            if (!hardwareInfoNeedsUpdating.compareAndSetBool(false, true)) return;
        ...
        String open(const BigInteger& inputChannelsWanted, const BigInteger& outputChannelsWanted, double sampleRateWanted,
                    int bufferSizeWanted)
        {
            ...
            JUCE_IOS_AUDIO_LOG("Opening audio device:"
                               ...
                               << ", targetSampleRate: " << targetSampleRate << ", targetBufferSize: " << targetBufferSize);

            channelData.reconfigure(requestedInputChannels, requestedOutputChannels);

            setAudioSessionCategory(channelData.areInputChannelsAvailable() ? AVAudioSessionCategoryPlayAndRecord
            ...
            if (callback != nullptr) callback->audioDeviceAboutToStart(&owner);
        ...
        void start(AudioIODeviceCallback* newCallback)
        {
            ...
            {
                if (newCallback != nullptr) newCallback->audioDeviceAboutToStart(&owner);
        ...
        bool setAudioPreprocessingEnabled(bool enable)
        {
            ...
            NSString* mode = (enable ? AVAudioSessionModeDefault : AVAudioSessionModeMeasurement);
        ...
        void transportPlay(bool shouldSartPlaying) override
        {
            ...
            OSStatus err = callbackInfo.transportStateProc2(callbackInfo.hostUserData, &hostIsPlaying, nullptr, nullptr,
                                                            nullptr, nullptr, nullptr, nullptr);

            ignoreUnused(err);
            jassert(err == noErr);

            if (hostIsPlaying != shouldSartPlaying) handleAudioTransportEvent(kAudioUnitRemoteControlEvent_TogglePlayPause);
        }
        ...
        void transportRecord(bool shouldStartRecording) override
        {
            ...
            OSStatus err = callbackInfo.transportStateProc2(callbackInfo.hostUserData, nullptr, &hostIsRecording, nullptr,
                                                            nullptr, nullptr, nullptr, nullptr);
            ignoreUnused(err);
            jassert(err == noErr);

            if (hostIsRecording != shouldStartRecording) handleAudioTransportEvent(kAudioUnitRemoteControlEvent_ToggleRecord);
        }

        void transportRewind() override
        {
            if (canControlTransport()) handleAudioTransportEvent(kAudioUnitRemoteControlEvent_Rewind);
        }

        bool getCurrentPosition(CurrentPositionInfo& result) override
        {
            if (!canControlTransport()) return false;
            ...
            if (callbackInfo.hostUserData == nullptr) return false;
            ...
            OSStatus err = callbackInfo.transportStateProc2(callbackInfo.hostUserData, &hostIsPlaying, &hostIsRecording,
                                                            nullptr, &hostCurrentSampleInTimeLine, &hostIsCycling,
                                                            &hostCycleStartBeat, &hostCycleEndBeat);
            if (err == kAUGraphErr_CannotDoInCurrentContext) return false;
            ...
            Float64 hostBeat  = 0;
            Float64 hostTempo = 0;
            err               = callbackInfo.beatAndTempoProc(callbackInfo.hostUserData, &hostBeat, &hostTempo);
            jassert(err == noErr);
            ...
            err = callbackInfo.musicalTimeLocationProc(callbackInfo.hostUserData, nullptr, &hostTimeSigNumerator,
        ...
        Image getIcon(int size)
        {
            if (interAppAudioConnected)
            {
                ...
                if (hostUIImage != nullptr) return juce_createImageFromUIImage(hostUIImage);
            }
            return Image();
        }
#endif
        ...
        {
            if (!interAppAudioConnected) return;

            CFURLRef hostUrl;
            UInt32   dataSize = sizeof(hostUrl);
            OSStatus err =
                AudioUnitGetProperty(audioUnit, kAudioUnitProperty_PeerURL, kAudioUnitScope_Global, 0, &hostUrl, &dataSize);
            if (err == noErr)
            {
#if (!defined __IPHONE_OS_VERSION_MIN_REQUIRED) || (!defined __IPHONE_10_0)                                                  \
    || (__IPHONE_OS_VERSION_MIN_REQUIRED < __IPHONE_10_0)
        ...
        void invokeAudioDeviceErrorCallback(const String& reason)
        {
            ...
            if (callback != nullptr) callback->audioDeviceError(reason);
        }
        ...
        void handleStatusChange(bool enabled, const char* reason)
        {
            ...
            if (!enabled) invokeAudioDeviceErrorCallback(reason);
        }
        ...
        void handleRouteChange(AVAudioSessionRouteChangeReason reason)
        {
            ...
            if (isRunning) invokeAudioDeviceErrorCallback(reasonString);
        ...
        void handleAudioUnitPropertyChange(AudioUnit, AudioUnitPropertyID propertyID, AudioUnitScope scope,
                                           AudioUnitElement element)
        {
            ignoreUnused(scope);
            ignoreUnused(element);
            JUCE_IOS_AUDIO_LOG("handleAudioUnitPropertyChange: propertyID: "
                               << String(propertyID) << " scope: " << String(scope) << " element: " << String(element));
        ...
        void handleInterAppAudioConnectionChange()
        {
            ...
            OSStatus err = AudioUnitGetProperty(audioUnit, kAudioUnitProperty_IsInterAppConnected, kAudioUnitScope_Global, 0,
                                                &connected, &dataSize);
            ignoreUnused(err);
            jassert(err == noErr);

            JUCE_IOS_AUDIO_LOG("handleInterAppAudioConnectionChange: " << (connected ? "connected" : "disconnected"));
        ...
        //==============================================================================
        OSStatus process(AudioUnitRenderActionFlags* flags, const AudioTimeStamp* time, const UInt32 numFrames,
                         AudioBufferList* data)
        {
            ...
            if (useInput) err = AudioUnitRender(audioUnit, flags, time, 1, numFrames, data);

            const int numChannels = jmax(channelData.inputs->numHardwareChannels, channelData.outputs->numHardwareChannels);
            ...
            {
                if ((int)numFrames > channelData.getFloatBufferSize()) channelData.setFloatBufferSize((int)numFrames);
                ...
                if (useInput)
                {
                    ...
                    for (UInt32 i = 0; i < numFrames; ++i)
                    {
                        for (int channel = 0; channel < channelData.inputs->numActiveChannels; ++channel)
                            inputData[channel][i] =
                                *(inputShortData + channelData.inputs->activeChannelIndicies[channel]) * (1.0f / 32768.0f);
                ...
                callback->audioDeviceIOCallback((const float**)inputData, channelData.inputs->numActiveChannels, outputData,
                                                channelData.outputs->numActiveChannels, (int)numFrames);
                ...
                if (channelData.outputs->numActiveChannels > 0)
                {
                    for (UInt32 i = 0; i < numFrames; ++i)
                    {
                        for (int channel = 0; channel < channelData.outputs->numActiveChannels; ++channel)
                            *(outputShortData + channelData.outputs->activeChannelIndicies[channel]) =
                                (short)(outputData[channel][i] * 32767.0f);
        ...
        void recordXruns(const AudioTimeStamp* time, UInt32 numFrames)
        {
            if (time != nullptr && (time->mFlags & kAudioTimeStampSampleTimeValid) != 0)
            {
                if (!firstHostTime)
                {
                    if ((time->mSampleTime - lastSampleTime) != lastNumFrames) xrun++;
                }
                else
                    firstHostTime = false;
        ...
        bool createAudioUnit()
        {
            ...
            OSStatus err = AudioOutputUnitPublish(&appDesc, CFSTR(JucePlugin_IAAName), JucePlugin_VersionCode, audioUnit);
            ...
            err = AudioUnitAddPropertyListener(audioUnit, kAudioUnitProperty_IsInterAppConnected,
                                               dispatchAudioUnitPropertyChange, this);
            jassert(err == noErr);
#endif
            ...
            {
                const UInt32 one = 1;
                AudioUnitSetProperty(audioUnit, kAudioOutputUnitProperty_EnableIO, kAudioUnitScope_Input, 1, &one,
                                     sizeof(one));
            }

            {
                ...
                AudioUnitSetProperty(audioUnit, kAudioUnitProperty_SetRenderCallback, kAudioUnitScope_Input, 0, &inputProc,
                                     sizeof(inputProc));
            }

            {
                ...
                format.mChannelsPerFrame =
                    (UInt32)jmax(channelData.inputs->numHardwareChannels, channelData.outputs->numHardwareChannels);
                format.mBytesPerFrame = format.mBytesPerPacket = format.mChannelsPerFrame * sizeof(short);

                AudioUnitSetProperty(audioUnit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Input, 0, &format,
                                     sizeof(format));
                AudioUnitSetProperty(audioUnit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Output, 1, &format,
                                     sizeof(format));
            }
...

            {
                ...
                if (AudioUnitGetProperty(audioUnit, kAudioUnitProperty_MaximumFramesPerSlice, kAudioUnitScope_Global, 0,
                                         &framesPerSlice, &dataSize)
                        == noErr
                    && dataSize == sizeof(framesPerSlice) && static_cast<int>(framesPerSlice) != bufferSize)
                {
        ...
        void fillHostCallbackInfo(HostCallbackInfo& callbackInfo)
        {
            ...
            OSStatus err      = AudioUnitGetProperty(audioUnit, kAudioUnitProperty_HostCallbacks, kAudioUnitScope_Global, 0,
        ...
        static void fixAudioRouteIfSetToReceiver()
        {
            ...
            for (AVAudioSessionPortDescription* port in route.outputs)
            {
                ...
                {
                    JUCE_NSERROR_CHECK([session overrideOutputAudioPort:AVAudioSessionPortOverrideSpeaker error:&error]);
                    setAudioSessionActive(true);
                }
            }
        }
        ...
        void handleStreamFormatChange()
        {
            ...
            AudioUnitGetProperty(audioUnit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Output, 0, &desc, &dataSize);
        ...
        {
            if (messageCollector != nullptr) messageCollector->addMessageToQueue(msg);
        }
        ...
        static void midiEventCallback(void* client, UInt32 status, UInt32 data1, UInt32 data2, UInt32)
        {
            return static_cast<Pimpl*>(client)->handleMidiMessage(
                MidiMessage((int)status, (int)data1, (int)data2, Time::getMillisecondCounter() / 1000.0));
        }
        ...
        struct IOChannelData
        {
            ...
            public:
                    ...
                    , activeChannelIndicies(getActiveChannelIndicies(activeChannels))
                {
#if JUCE_IOS_AUDIO_LOGGING
                    {
                        ...
                        info << "Number of hardware channels: " << numHardwareChannels << ", Hardware channel names:";

                        for (auto& name : hardwareChannelNames) info << " \"" << name << "\"";

                        info << ", Are channels available: " << (areChannelsAccessible ? "yes" : "no")
                             << ", Active channel indices:";

                        for (auto i : activeChannelIndicies) info << " " << i;
            ...
            private:
                static StringArray getHardwareChannelNames(const bool isInput)
                {
                    ...
                    if (result.isEmpty()) return {"Left", "Right"};
                ...
                {
                    requiredChannels.setRange(numHardwareChannelsAvailable, requiredChannels.getHighestBit() + 1, false);
            ...
            void reconfigure(const BigInteger requiredInputChannels, const BigInteger requiredOutputChannels)
            {
                inputs  = new IOChannelConfig(true, requiredInputChannels);
                outputs = new IOChannelConfig(false, requiredOutputChannels);

                audioData.setSize(inputs->numActiveChannels + outputs->numActiveChannels, audioData.getNumSamples());
            }
    ...
    //==============================================================================
    String iOSAudioIODevice::open(const BigInteger& inChans, const BigInteger& outChans, double requestedSampleRate,
    ...
    {
        return pimpl->channelData.inputs->areChannelsAccessible ? pimpl->channelData.inputs->hardwareChannelNames
                                                                : StringArray();
    }
    ...
    int iOSAudioIODevice::getXRunCount() const noexcept
    {
        return pimpl->xrun;
    }

    void iOSAudioIODevice::setMidiMessageCollector(MidiMessageCollector* collector)
    {
    ...
    int iOSAudioIODeviceType::getDefaultDeviceIndex(bool) const
    {
        return 0;
    }
    ...
    {
        for (auto device : activeDevices) device->handleStatusChange(enabled, reason);
    }

    void AudioSessionHolder::handleRouteChange(AVAudioSessionRouteChangeReason reason)
    {
        for (auto device : activeDevices) device->handleRouteChange(reason);

        for (auto deviceType : activeDeviceTypes) deviceType->handleRouteChange(reason);
    }
