...
#undef JUCE_IUNKNOWNCLASS
    ...
    namespace WasapiClasses
    {
        ...
        EDataFlow getDataFlow(const ComSmartPtr<IMMDevice>& device)
        {
            ...
            if (check(device.QueryInterface(endPoint))) (void)check(endPoint->GetDataFlow(&flow));
        ...
        {
            memcpy(&dest, src,
                   src->wFormatTag == WAVE_FORMAT_EXTENSIBLE ? sizeof(WAVEFORMATEXTENSIBLE) : sizeof(WAVEFORMATEX));
        }
        ...
        public:
                ...
                , shouldClose(false)
            {
                ...
                if (tempClient == nullptr) return;

                REFERENCE_TIME defaultPeriod, minPeriod;
                if (!check(tempClient->GetDevicePeriod(&defaultPeriod, &minPeriod))) return;

                WAVEFORMATEX* mixFormat = nullptr;
                if (!check(tempClient->GetMixFormat(&mixFormat))) return;
                ...
                if (useExclusiveMode && findSupportedFormat(tempClient, defaultSampleRate, format.dwChannelMask, format))
                {
                    // Got a format that is supported by the device so we can ask what sample rates are supported (in whatever
                    // format)
                }
                ...
                {
                    if (rates.contains(ratesToTest[i])) continue;

                    format.Format.nSamplesPerSec = (DWORD)ratesToTest[i];
                    format.Format.nAvgBytesPerSec =
                        (DWORD)(format.Format.nSamplesPerSec * format.Format.nChannels * format.Format.wBitsPerSample / 8);

                    if (SUCCEEDED(tempClient->IsFormatSupported(
                            useExclusiveMode ? AUDCLNT_SHAREMODE_EXCLUSIVE : AUDCLNT_SHAREMODE_SHARED, (WAVEFORMATEX*)&format,
                            0)))
                        if (!rates.contains(ratesToTest[i])) rates.addUsingDefaultSort(ratesToTest[i]);
                }
            }
            ...
            bool openClient(const double newSampleRate, const BigInteger& newChannels, const int bufferSizeSamples)
            {
                ...
                if (client != nullptr && tryInitialisingWithBufferSize(bufferSizeSamples))
                {
                    ...
                    for (int i = 0; i <= channels.getHighestBit(); ++i)
                        if (channels[i]) channelMaps.add(i);

                    REFERENCE_TIME latency;
                    if (check(client->GetStreamLatency(&latency))) latencySamples = refTimeToSamples(latency, sampleRate);
        ...
        private:
            ...
            public:
                ...
                {
                    if (state == AudioSessionStateInactive || state == AudioSessionStateExpired) owner.deviceBecameInactive();
                ...
                JUCE_COMRESULT OnSessionDisconnected(AudioSessionDisconnectReason reason)
                {
                    ...
                    if (reason == DisconnectReasonFormatChanged) owner.deviceSampleRateChanged();
            ...
            {
                deleteSessionEventCallback();
                client->GetService(__uuidof(IAudioSessionControl), (void**)audioSessionControl.resetAndGetPointerAddress());
            ...
            ComSmartPtr<IAudioClient> createClient()
            {
                ...
                if (device != nullptr)
                    logFailure(device->Activate(__uuidof(IAudioClient), CLSCTX_INPROC_SERVER, nullptr,
            ...
            bool findSupportedFormat(IAudioClient* clientToUse, double newSampleRate, DWORD newMixFormatChannelMask,
                                     WAVEFORMATEXTENSIBLE& format) const
            {
                static const AudioSampleFormat formats[] = {{true, 32, 4},  {false, 32, 4}, {false, 24, 4}, {false, 24, 3},
                                                            {false, 20, 4}, {false, 20, 3}, {false, 16, 2}};

                for (int i = 0; i < numElementsInArray(formats); ++i)
                    if (tryFormat(formats[i], clientToUse, newSampleRate, newMixFormatChannelMask, format)) return true;
            ...
            bool tryInitialisingWithBufferSize(const int bufferSizeSamples)
            {
                ...
                if (findSupportedFormat(client, sampleRate, mixFormatChannelMask, format))
                {
                    ...
                    for (;;)
                    {
                        GUID    session;
                        HRESULT hr =
                            client->Initialize(useExclusiveMode ? AUDCLNT_SHAREMODE_EXCLUSIVE : AUDCLNT_SHAREMODE_SHARED,
                                               0x40000 /*AUDCLNT_STREAMFLAGS_EVENTCALLBACK*/, defaultPeriod,
                                               useExclusiveMode ? defaultPeriod : 0, (WAVEFORMATEX*)&format, &session);
                        ...
                        // http://msdn.microsoft.com/en-us/library/windows/desktop/dd370875(v=vs.85).aspx (see Remarks)
                        if (hr != MAKE_HRESULT(1, 0x889, 0x19))  // AUDCLNT_E_BUFFER_SIZE_NOT_ALIGNED
                            break;

                        UINT32 numFrames = 0;
                        if (!check(client->GetBufferSize(&numFrames))) break;
        ...
        public:
            ...
            {
                typedef AudioData::Pointer<AudioData::Float32, AudioData::NativeEndian, AudioData::NonInterleaved,
                                           AudioData::NonConst>
                    NativeType;
                converter = new AudioData::ConverterInstance<
                    AudioData::Pointer<SourceType, AudioData::LittleEndian, AudioData::Interleaved, AudioData::Const>,
            ...
            bool start(const int userBufferSize)
            {
                ...
                if (!check(client->Start())) return false;
            ...
            {
                return reservoirWritePos - reservoirReadPos;
            }
            ...
            void handleDeviceBuffer()
            {
                ...
                while (check(captureClient->GetBuffer(&inputData, &numSamplesAvailable, &flags, nullptr, nullptr))
                       && numSamplesAvailable > 0)
                {
                    if ((flags & AUDCLNT_BUFFERFLAGS_DATA_DISCONTINUITY) != 0) xruns++;
                    ...
                    if (getNumSamplesInReservoir() > reservoirSize) reservoirReadPos = reservoirWritePos - reservoirSize;
            ...
            {
                if ((numChannels <= 0 && bufferSize == 0) || reservoir.getSize() == 0) return;

                int offset = jmax(0, bufferSize - getNumSamplesInReservoir());

                if (offset > 0)
                {
                    for (int i = 0; i < numDestBuffers; ++i) zeromem(destBuffers[i], offset * sizeof(float));
                ...
                while (bufferSize > 0)
                {
                    ...
                    if (samplesToDo <= 0) break;

                    const int reservoirOffset = localRead * bytesPerFrame;

                    for (int i = 0; i < numDestBuffers; ++i)
                        converter->convertSamples(destBuffers[i] + offset, 0,
                                                  addBytesToPointer(reservoir.getData(), reservoirOffset),
                                                  channelMaps.getUnchecked(i), samplesToDo);
        ...
        public:
            ...
            void updateFormatWithType(DestType*)
            {
                ...
                converter = new AudioData::ConverterInstance<
                    NativeType,
                    AudioData::Pointer<DestType, AudioData::LittleEndian, AudioData::Interleaved, AudioData::NonConst>>(
                    1, actualNumChannels);
            }
            ...
            int getNumSamplesAvailableToCopy() const
            {
                ...
                {
                    UINT32 padding = 0;
                    if (check(client->GetCurrentPadding(&padding))) return actualBufferSize - (int)padding;
                }
                             ...
                             WASAPIInputDevice* inputDevice, Thread& thread)
            {
                ...
                while (bufferSize > 0)
                {
                    ...
                    if (samplesToDo == 0)
                    {
                        ...
                        if (!thread.threadShouldExit() && WaitForSingleObject(clientEvent, 1000) == WAIT_OBJECT_0) continue;

                        break;
                    }

                    if (useExclusiveMode && WaitForSingleObject(clientEvent, 1000) == WAIT_TIMEOUT) break;

                    uint8* outputData = nullptr;
                    if (check(renderClient->GetBuffer((UINT32)samplesToDo, &outputData)))
                    {
                        for (int i = 0; i < numSrcBuffers; ++i)
                            converter->convertSamples(outputData, channelMaps.getUnchecked(i), srcBuffers[i] + offset, 0,
                                                      samplesToDo);
        ...
        //==============================================================================
        class WASAPIAudioIODevice : public AudioIODevice, public Thread, private AsyncUpdater
        {
        public:
            WASAPIAudioIODevice(const String& deviceName, const String& typeName, const String& outputDeviceID,
                                const String& inputDeviceID, const bool exclusiveMode)
            ...
            bool initialise()
            {
                ...
                if (createDevices())
                {
                    ...
                    if (minBufferSize != defaultBufferSize) bufferSizes.addUsingDefaultSort(minBufferSize);

                    int n = 64;
                    for (int i = 0; i < 40; ++i)
                    {
                        if (n >= minBufferSize && n <= 2048 && !bufferSizes.contains(n)) bufferSizes.addUsingDefaultSort(n);
            ...
            StringArray getOutputChannelNames() override
            {
                ...
                if (outputDevice != nullptr)
                    for (int i = 1; i <= outputDevice->actualNumChannels; ++i) outChannels.add("Output channel " + String(i));
            ...
            StringArray getInputChannelNames() override
            {
                ...
                if (inputDevice != nullptr)
                    for (int i = 1; i <= inputDevice->actualNumChannels; ++i) inChannels.add("Input channel " + String(i));
                        ...
                        int bufferSizeSamples) override
            {
                ...
                currentBufferSizeSamples =
                    bufferSizeSamples <= 0 ? defaultBufferSize : jmax(bufferSizeSamples, minBufferSize);
                ...
                if (useExclusiveMode)
                {
                    ...
                    currentBufferSizeSamples =
                        outputDevice != nullptr ? outputDevice->actualBufferSize : inputDevice->actualBufferSize;
                }
            ...
            void stop() override
            {
                if (isStarted)
                {
                    ...
                    if (callbackLocal != nullptr) callbackLocal->audioDeviceStopped();
                }
            }
            ...
            void setMMThreadPriority()
            {
                ...
                JUCE_LOAD_WINAPI_FUNCTION(dll, AvSetMmThreadCharacteristicsW, avSetMmThreadCharacteristics, HANDLE,
                                          (LPCWSTR, LPDWORD))
                ...
                if (avSetMmThreadCharacteristics != 0 && avSetMmThreadPriority != 0)
                {
                    ...
                    if (h != 0) avSetMmThreadPriority(h, AVRT_PRIORITY_NORMAL);
                }
            }
            ...
            void run() override
            {
                ...
                {
                    if (outputDevice != nullptr && outputDevice->shouldClose) deviceBecameInactive = true;

                    if (inputDevice != nullptr && !deviceBecameInactive)
                    {
                        if (inputDevice->shouldClose) deviceBecameInactive = true;

                        if (outputDevice == nullptr)
                        {
                            if (WaitForSingleObject(inputDevice->clientEvent, 1000) == WAIT_TIMEOUT) break;

                            inputDevice->handleDeviceBuffer();

                            if (inputDevice->getNumSamplesInReservoir() < bufferSize) continue;
                        }
                        else
                        {
                    ...
                    if (outputDevice != nullptr && !deviceBecameInactive)
                    {
                        ...
                        outputDevice->copyBuffers(const_cast<const float**>(outputBuffers), numOutputBuffers, bufferSize,
                                                  inputDevice, *this);
        ...
        private:
            ...
            bool createDevices()
            {
                ...
                if (!check(enumerator.CoCreateInstance(__uuidof(MMDeviceEnumerator)))) return false;

                ComSmartPtr<IMMDeviceCollection> deviceCollection;
                if (!check(enumerator->EnumAudioEndpoints(eAll, DEVICE_STATE_ACTIVE,
                                                          deviceCollection.resetAndGetPointerAddress())))
                    return false;

                UINT32 numDevices = 0;
                if (!check(deviceCollection->GetCount(&numDevices))) return false;

                for (UINT32 i = 0; i < numDevices; ++i)
                {
                    ComSmartPtr<IMMDevice> device;
                    if (!check(deviceCollection->Item(i, device.resetAndGetPointerAddress()))) continue;
            ...
            void handleAsyncUpdate() override
            {
                ...
                if (!deviceBecameInactive)
                {
                    ...
                    open(lastKnownInputChannels, lastKnownOutputChannels, getChangedSampleRate(), currentBufferSizeSamples);
            ...
            {
                if (outputDevice != nullptr && sampleRateChangedByOutput) return outputDevice->defaultSampleRate;

                if (inputDevice != nullptr && !sampleRateChangedByOutput) return inputDevice->defaultSampleRate;
        ...
        public:
            ...
            {
                if (notifyClient != nullptr) enumerator->UnregisterEndpointNotificationCallback(notifyClient);
            }
            ...
            void scanForDevices()
            {
                ...
                scan(outputDeviceNames, inputDeviceNames, outputDeviceIds, inputDeviceIds);
            }
            ...
            int getIndexOfDevice(AudioIODevice* device, bool asInput) const
            {
                ...
                if (WASAPIAudioIODevice* const d = dynamic_cast<WASAPIAudioIODevice*>(device))
                    return asInput ? inputDeviceIds.indexOf(d->inputDeviceId) : outputDeviceIds.indexOf(d->outputDeviceId);
            ...
            AudioIODevice* createDevice(const String& outputDeviceName, const String& inputDeviceName)
            {
                ...
                {
                    device = new WASAPIAudioIODevice(outputDeviceName.isNotEmpty() ? outputDeviceName : inputDeviceName,
                                                     getTypeName(), outputDeviceIds[outputIndex], inputDeviceIds[inputIndex],
                                                     exclusiveMode);

                    if (!device->initialise()) device = nullptr;
                }
        ...
        private:
            ...
            public:
                ChangeNotificationClient(WASAPIAudioIODeviceType& d)
                    : ComBaseClassHelper<IMMNotificationClient>(0)
                    , device(d)
                {
                }
            ...
            static String getDefaultEndpoint(IMMDeviceEnumerator* const enumerator, const bool forCapture)
            {
                ...
                if (check(enumerator->GetDefaultAudioEndpoint(forCapture ? eCapture : eRender, eMultimedia, &dev)))
                {
                    WCHAR* deviceId = nullptr;
            ...
            //==============================================================================
            void scan(StringArray& outDeviceNames, StringArray& inDeviceNames, StringArray& outDeviceIds,
                      StringArray& inDeviceIds)
            {
                if (enumerator == nullptr)
                {
                    if (!check(enumerator.CoCreateInstance(__uuidof(MMDeviceEnumerator)))) return;
                ...
                for (UINT32 i = 0; i < numDevices; ++i)
                {
                    ...
                    DWORD state = 0;
                    if (!(check(device->GetState(&state)) && state == DEVICE_STATE_ACTIVE)) continue;
...

                    {
                        ...
                        if (!check(device->OpenPropertyStore(STGM_READ, properties.resetAndGetPointerAddress()))) continue;
                        ...
                        if (check(properties->GetValue(PKEY_Device_FriendlyName, &value))) name = value.pwszVal;
            ...
            void systemDeviceChanged() override
            {
                ...
                if (newOutNames != outputDeviceNames || newInNames != inputDeviceNames || newOutIds != outputDeviceIds
        ...
        struct MMDeviceMasterVolume
        {
            ...
            {
                float vol = 0.0f;
                if (endpointVolume != nullptr) check(endpointVolume->GetMasterVolumeLevelScalar(&vol));
            ...
            {
                BOOL mute = 0;
                return endpointVolume != nullptr && check(endpointVolume->GetMute(&mute)) && mute != 0;
            }

            bool setMuted(bool shouldMute) const
            {
                return endpointVolume != nullptr && check(endpointVolume->SetMute(shouldMute, nullptr));
            }
    ...
    AudioIODeviceType* AudioIODeviceType::createAudioIODeviceType_WASAPI(bool exclusiveMode)
    {
        ...
        if (exclusiveMode) return nullptr;
#endif
