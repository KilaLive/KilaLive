...
#define DSOUND_FUNCTION_LOAD(functionName)                                                                                   \
    ...
    public:
        ...
        String open()
        {
            JUCE_DS_LOG("opening output: " + name + "  rate=" + String(sampleRate) + " bits=" + String(bitDepth)
            ...
            if (dsDirectSoundCreate != nullptr) hr = dsDirectSoundCreate(&guid, &pDirectSound, nullptr);
                    ...
                    if (SUCCEEDED(hr))
                    {
                        ...
                        if (SUCCEEDED(hr))
                        {
                            ...
                            if (SUCCEEDED(hr))
                            {
                                ...
                                hr = pOutputBuffer->Lock(0, (DWORD)totalBytesPerBuffer, (LPVOID*)&pDSBuffData, &dwDataLen, 0,
                                                         0, 0);
                                JUCE_DS_LOG_ERROR(hr);
        ...
        bool service()
        {
            if (pOutputBuffer == 0) return true;
            ...
            if (playWriteGap < 0) playWriteGap += totalBytesPerBuffer;

            int bytesEmpty = (int)(playCursor - writeOffset);
            if (bytesEmpty < 0) bytesEmpty += totalBytesPerBuffer;
            ...
            if (bytesEmpty >= bytesPerBuffer)
            {
                ...
                HRESULT hr = pOutputBuffer->Lock(writeOffset, (DWORD)bytesPerBuffer, (void**)&buf1, &dwSize1, (void**)&buf2,
                                                 &dwSize2, 0);
                ...
                if (hr == MAKE_HRESULT(1, 0x878, 150))  // DSERR_BUFFERLOST
                {
                    ...
                    hr = pOutputBuffer->Lock(writeOffset, (DWORD)bytesPerBuffer, (void**)&buf1, &dwSize1, (void**)&buf2,
                                             &dwSize2, 0);
                }
    ...
    public:
        ...
        String open()
        {
            JUCE_DS_LOG("opening input: " + name + "  rate=" + String(sampleRate) + " bits=" + String(bitDepth)
                        + " buf=" + String(bufferSizeSamples));
        ...
        bool service()
        {
            ...
            if (bytesFilled < 0) bytesFilled += totalBytesPerBuffer;
            ...
            if (bytesFilled >= bytesPerBuffer)
            {
                ...
                hr = pInputBuffer->Lock((DWORD)readOffset, (DWORD)bytesPerBuffer, (void**)&buf1, &dwsize1, (void**)&buf2,
                                        &dwsize2, 0);
    ...
    //==============================================================================
    class DSoundAudioIODevice : public AudioIODevice, public Thread
    {
    public:
        ...
        String open(const BigInteger& inputChannels, const BigInteger& outputChannels, double newSampleRate,
        ...
        int getCurrentBitDepth() override
        {
            ...
            for (int i = inChans.size(); --i >= 0;) bits = jmin(bits, inChans[i]->bitDepth);

            for (int i = outChans.size(); --i >= 0;) bits = jmin(bits, outChans[i]->bitDepth);
    ...
    private:
        ...
        String openDevice(const BigInteger& inputChannels, const BigInteger& outputChannels, double sampleRate_,
        ...
        void resync()
        {
            if (!threadShouldExit())
            {
                ...
                for (int i = 0; i < outChans.size(); ++i) outChans.getUnchecked(i)->synchronisePosition();

                for (int i = 0; i < inChans.size(); ++i) inChans.getUnchecked(i)->synchronisePosition();
            }
        }
    ...
    struct DSoundDeviceList
    {
        ...
        {
            return outputDeviceNames != other.outputDeviceNames || inputDeviceNames != other.inputDeviceNames
                   || outputGuids != other.outputGuids || inputGuids != other.inputGuids;
        }
    ...
    private:
        static BOOL enumProc(LPGUID lpGUID, String desc, StringArray& names, Array<GUID>& guids)
        {
            ...
            if (desc.isNotEmpty())
            {
                ...
                int n = 2;
                while (names.contains(desc)) desc = origDesc + " (" + String(n++) + ")";
    ...
    //==============================================================================
    String DSoundAudioIODevice::openDevice(const BigInteger& inputChannels, const BigInteger& outputChannels,
                                           double sampleRate_, int bufferSizeSamples_)
    {
        ...
        if (bufferSizeSamples_ <= 0) bufferSizeSamples_ = 960;  // use as a default size if none is set.
        ...
        enabledInputs.setRange(inChannels.size(), enabledInputs.getHighestBit() + 1 - inChannels.size(), false);
    ...
    public:
        ...
        StringArray getDeviceNames(bool wantInputNames) const
        {
            ...
            return wantInputNames ? deviceList.inputDeviceNames : deviceList.outputDeviceNames;
        }
        ...
        AudioIODevice* createDevice(const String& outputDeviceName, const String& inputDeviceName)
        {
            ...
            if (outputIndex >= 0 || inputIndex >= 0)
                return new DSoundAudioIODevice(outputDeviceName.isNotEmpty() ? outputDeviceName : inputDeviceName,
