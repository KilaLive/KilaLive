...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD)                                                          \
    ...
    METHOD(openMidiInputPortWithJuceIndex, "openMidiInputPortWithJuceIndex",                                                 \
           "(IJ)L" JUCE_ANDROID_ACTIVITY_CLASSPATH "$JuceMidiPort;")                                                         \
    METHOD(openMidiOutputPortWithJuceIndex, "openMidiOutputPortWithJuceIndex",                                               \
           "(I)L" JUCE_ANDROID_ACTIVITY_CLASSPATH "$JuceMidiPort;")                                                          \
...
#undef JNI_CLASS_MEMBERS
    ...
    {
    public:
        AndroidMidiInput(MidiInput* midiInput, int portIdx, juce::MidiInputCallback* midiInputCallback, jobject deviceManager)
            : juceMidiInput(midiInput)
            , callback(midiInputCallback)
            , midiConcatenator(2048)
            , javaMidiDevice(getEnv()->CallObjectMethod(deviceManager, MidiDeviceManager.openMidiInputPortWithJuceIndex,
                                                        (jint)portIdx, (jlong)this))
        {
        }
        ...
        void start()
        {
            if (jobject d = javaMidiDevice.get()) getEnv()->CallVoidMethod(d, JuceMidiPort.start);
        }

        void stop()
        {
            if (jobject d = javaMidiDevice.get()) getEnv()->CallVoidMethod(d, JuceMidiPort.stop);
        ...
        void receive(jbyteArray byteArray, jlong offset, jint len, jlong timestamp)
        {
            ...
            midiConcatenator.pushMidiData(buffer.get(), len, static_cast<double>(timestamp) * 1.0e-9, juceMidiInput,
                                          *callback);
    ...
    public:
        ...
        {
            if (jobject d = javaMidiDevice.get()) getEnv()->CallVoidMethod(d, JuceMidiPort.sendMidi, byteArray, offset, len);
        }
    ...
    JUCE_JNI_CALLBACK(JUCE_JOIN_MACRO(JUCE_ANDROID_ACTIVITY_CLASSNAME, _00024JuceMidiInputPort), handleReceive, void,
                      (JNIEnv * env, jobject, jlong host, jbyteArray byteArray, jint offset, jint count, jlong timestamp))
    {
    ...
    public:
        ...
        String getInputPortNameForJuceIndex(int idx)
        {
            ...
            {
                LocalRef<jstring> string(
                    (jstring)getEnv()->CallObjectMethod(dm, MidiDeviceManager.getInputPortNameForJuceIndex, idx));
        ...
        String getOutputPortNameForJuceIndex(int idx)
        {
            ...
            {
                LocalRef<jstring> string(
                    (jstring)getEnv()->CallObjectMethod(dm, MidiDeviceManager.getOutputPortNameForJuceIndex, idx));
                return juceString(string);
            }
        ...
        AndroidMidiInput* openMidiInputPortWithIndex(int idx, MidiInput* juceMidiInput, juce::MidiInputCallback* callback)
        {
            if (jobject dm = deviceManager.get())
            {
                ...
                if (androidMidiInput->isOpen()) return androidMidiInput.release();
            }
        ...
        {
            if (jobject dm = deviceManager.get())
                if (jobject javaMidiPort =
                        getEnv()->CallObjectMethod(dm, MidiDeviceManager.openMidiOutputPortWithJuceIndex, (jint)idx))
    ...
    MidiInput* MidiInput::openDevice(int index, juce::MidiInputCallback* callback)
    {
        ...
        return midiInput->internal != nullptr ? midiInput.release() : nullptr;
    }

    void MidiInput::start()
    {
        if (AndroidMidiInput* mi = reinterpret_cast<AndroidMidiInput*>(internal)) mi->start();
    }

    void MidiInput::stop()
    {
        if (AndroidMidiInput* mi = reinterpret_cast<AndroidMidiInput*>(internal)) mi->stop();
    }
