...
#define JUCE_ASIO_LOG_ERROR(msg, errNum)                                                                                     \
    ...
    struct ASIOSampleFormat
    {
        ...
        {
            if (dst != nullptr) zeromem(dst, numSamps * byteStride);
        }
    ...
    private:
        static void convertInt16ToFloat(const char* src, float* dest, const int srcStrideBytes, int numSamples,
        ...
        static void convertFloatToInt16(const float* src, char* dest, const int dstStrideBytes, int numSamples,
                                        const bool littleEndian) noexcept
        {
            ...
            if (littleEndian)
            {
                ...
                {
                    *(uint16*)dest =
                        ByteOrder::swapIfBigEndian((uint16)(short)roundToInt(jlimit(-maxVal, maxVal, maxVal * *src++)));
                    dest += dstStrideBytes;
                }
            }
            else
            {
                ...
                {
                    *(uint16*)dest =
                        ByteOrder::swapIfLittleEndian((uint16)(short)roundToInt(jlimit(-maxVal, maxVal, maxVal * *src++)));
                    dest += dstStrideBytes;
                }
            }
        }

        static void convertInt24ToFloat(const char* src, float* dest, const int srcStrideBytes, int numSamples,
                                        const bool littleEndian) noexcept
        {
        ...
        static void convertFloatToInt24(const float* src, char* dest, const int dstStrideBytes, int numSamples,
                                        const bool littleEndian) noexcept
        {
        ...
        static void convertInt32ToFloat(const char* src, float* dest, const int srcStrideBytes, int numSamples,
                                        const bool littleEndian) noexcept
        {
        ...
        static void convertFloatToInt32(const float* src, char* dest, const int dstStrideBytes, int numSamples,
                                        const bool littleEndian) noexcept
        {
            ...
            else
            {
                ...
                {
                    *(uint32*)dest =
                        ByteOrder::swapIfLittleEndian((uint32)roundToInt(jlimit(-maxVal, maxVal, maxVal * *src++)));
                    dest += dstStrideBytes;
                }
            }
        }
    ...
    //==============================================================================
    class ASIOAudioIODevice : public AudioIODevice, private Timer
    {
    public:
        ...
        ~ASIOAudioIODevice()
        {
            for (int i = 0; i < numElementsInArray(currentASIODev); ++i)
                if (currentASIODev[i] == this) currentASIODev[i] = nullptr;
...
#if JUCE_ASIO_DEBUGGING
                StringArray s;
                for (int i = 0; i < sampleRates.size(); ++i) s.add(String(sampleRates.getUnchecked(i)));
        ...
        String open(const BigInteger& inputChannels, const BigInteger& outputChannels, double sr,
                    int bufferSizeSamples) override
        {
            ...
            if (bufferSizeSamples < 8 || bufferSizeSamples > 16384) shouldUsePreferredSize = true;

            if (asioObject == nullptr)
            {
                const String openingError(openDevice());

                if (asioObject == nullptr) return openingError;
            }
            ...
            if (sampleRate == 0 || (sampleRates.size() > 0 && !sampleRates.contains(sampleRate))) sampleRate = sampleRates[0];
            ...
            if (asioObject->future(kAsioCanReportOverload, nullptr) != ASE_OK) xruns = -1;
            ...
            if (needToReset)
            {
                ...
                if (initError.isNotEmpty()) JUCE_ASIO_LOG("ASIOInit: " + initError);
            ...
            if (err == ASE_OK)
            {
                ...
                for (int i = types.size(); --i >= 0;) JUCE_ASIO_LOG("channel format: " + String(types[i]));
                ...
                {
                    int count = 300;
                    while (--count > 0 && !calledback) Thread::sleep(10);
    ...
    private:
        ...
        {
            if (CharPointer_UTF8::isValidString(text, length)) return String::fromUTF8(text, length);
        ...
        {
            if (asioObject != nullptr && asioObject->getChannels(&totalNumInputChans, &totalNumOutputChans) == ASE_OK)
            {
                inputChannelNames.clear();
                outputChannelNames.clear();

                for (int i = 0; i < totalNumInputChans; ++i) inputChannelNames.add(getChannelName(i, true));

                for (int i = 0; i < totalNumOutputChans; ++i) outputChannelNames.add(getChannelName(i, false));
        ...
        int readBufferSizes(int bufferSizeSamples)
        {
            ...
            if (asioObject->getBufferSize(&minBufferSize, &maxBufferSize, &newPreferredSize, &bufferGranularity) == ASE_OK)
            {
                ...
                if (bufferSizeSamples < minBufferSize || bufferSizeSamples > maxBufferSize) shouldUsePreferredSize = true;
        ...
        void addBufferSizes(long minSize, long maxSize, long preferredSize, long granularity)
        {
            ...
            JUCE_ASIO_LOG(String((int)minSize) + "->" + String((int)maxSize) + ", " + String((int)preferredSize) + ", "
                          + String((int)granularity));

            if (granularity >= 0)
            {
                granularity = jmax(16, (int)granularity);

                for (int i = jmax((int)(minSize + 15) & ~15, (int)granularity); i < jmin(6400, (int)maxSize);
                     i += granularity)
            ...
            else if (granularity < 0)
            {
                for (int i = 0; i < 18; ++i)
                {
                    ...
                    if (s >= minSize && s <= maxSize) bufferSizes.add(s);
                }
            }
        ...
        void setSampleRate(double newRate)
        {
            if (currentSampleRate != newRate)
            {
                ...
                if (err == 0) currentSampleRate = newRate;
        ...
        void updateClockSources()
        {
            ...
            {
                if (numClockSources == 0) JUCE_ASIO_LOG("no clock sources!");
            }
        }
        ...
        bool loadDriver()
        {
            ...
            if (crashed) JUCE_ASIO_LOG("** Driver crashed while being opened");
        ...
        String initDriver()
        {
            if (asioObject == nullptr) return "No Driver";
            ...
            if ((!initOk) || getName().containsIgnoreCase("denon dj")) driverError = getLastDriverError();

            if ((!initOk) && driverError.isEmpty()) driverError = "Driver failed to initialise";
        ...
        String openDevice()
        {
            ...
            if (getName().isEmpty()) return error;
                ...
                if ((error = initDriver()).isEmpty())
                {
                        ...
                        && (err = asioObject->getChannels(&totalNumInputChans, &totalNumOutputChans)) == 0)
                    {
                        ...
                        if ((err = refreshBufferSizes()) == 0)
                        {
                            ...
                            if (postOutput) JUCE_ASIO_LOG("outputReady true");
        ...
        void JUCE_ASIOCALLBACK callback(const long index)
        {
            ...
            {
                if (postOutput && (asioObject != nullptr)) asioObject->outputReady();
            }
        ...
        void processBuffer()
        {
            ...
            if (bi >= 0)
            {
                ...
                if (currentCallback != nullptr)
                {
                    ...
                    currentCallback->audioDeviceIOCallback(const_cast<const float**>(inBuffers.getData()),
                                                           numActiveInputChans, outBuffers, numActiveOutputChans, samps);
            ...
            if (postOutput) asioObject->outputReady();
        }
        ...
        struct ASIOCallbackFunctions
        {
            ...
            {
                if (currentASIODev[deviceIndex] != nullptr) currentASIODev[deviceIndex]->callback(index);
            ...
            {
                if (currentASIODev[deviceIndex] != nullptr) currentASIODev[deviceIndex]->resetRequest();
            }
    ...
    public:
        ...
        void scanForDevices()
        {
            ...
            if (RegOpenKey(HKEY_LOCAL_MACHINE, _T("software\\asio"), &hk) == ERROR_SUCCESS)
            {
                ...
                while (RegEnumKey(hk, index++, name, numElementsInArray(name)) == ERROR_SUCCESS) addDriverInfo(name, hk);
        ...
        int getDefaultDeviceIndex(bool) const
        {
            ...
            for (int i = deviceNames.size(); --i >= 0;)
                if (deviceNames[i].containsIgnoreCase("asio4all")) return i;  // asio4all is a safe choice for a default..
        ...
        {
            for (int i = 0; i < numElementsInArray(currentASIODev); ++i)
                if (currentASIODev[i] == 0) return i;
        ...
        AudioIODevice* createDevice(const String& outputDeviceName, const String& inputDeviceName)
        {
            ...
            if (index >= 0)
            {
                ...
                if (freeSlot >= 0) return new ASIOAudioIODevice(this, deviceName, classIds.getReference(index), freeSlot);
            }
                        ...
                        if (RegOpenKeyEx(hk, name, 0, KEY_READ, &subKey) == ERROR_SUCCESS)
                        {
                            if (RegOpenKeyEx(subKey, _T("InprocServer32"), 0, KEY_READ, &pathKey) == ERROR_SUCCESS)
                            {
                                ...
                                if (RegQueryValueEx(pathKey, 0, 0, &dtype, (LPBYTE)pathName, &dsize) == ERROR_SUCCESS)
                                    // In older code, this used to check for the existence of the file, but there are
                                    // situations where our process doesn't have access to it, but where the driver still
                                    // loads ok..
                                    ok = (pathName[0] != 0);
    ...
    {
        if (type != nullptr) type->sendDeviceChangeToListeners();
    }
