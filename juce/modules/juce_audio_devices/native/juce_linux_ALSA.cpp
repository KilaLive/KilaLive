...
#define JUCE_CHECKED_RESULT(x) (logErrorMessage(x, __LINE__))
        ...
        {
            if (err < 0) JUCE_ALSA_LOG("Error: line " << lineNum << ": code " << err << " (" << snd_strerror(err) << ")");
...
#define JUCE_ALSA_FAILED(x) failed(x)
        ...
        static void getDeviceProperties(const String& deviceID, unsigned int& minChansOut, unsigned int& maxChansOut,
                                        unsigned int& minChansIn, unsigned int& maxChansIn, Array<double>& rates,
                                        bool testOutput, bool testInput)
        {
            ...
            if (testInput)
            {
                ...
                if (JUCE_CHECKED_RESULT(snd_pcm_open(&pcmHandle, deviceID.toUTF8(), SND_PCM_STREAM_CAPTURE, SND_PCM_NONBLOCK)
                                        >= 0))
                {
                    getDeviceNumChannels(pcmHandle, &minChansIn, &maxChansIn);

                    if (rates.size() == 0) getDeviceSampleRates(pcmHandle, rates);
        ...
        static void ensureMinimumNumBitsSet(BigInteger& chans, int minNumChans)
        {
            ...
            while (chans.countNumberOfSetBits() < minNumChans) chans.setBit(i++);
        }
        ...
        public:
                ...
                , isInterleaved(true)
            {
                ...
                int err = snd_pcm_open(&handle, deviceID.toUTF8(),
                                       forInput ? SND_PCM_STREAM_CAPTURE : SND_PCM_STREAM_PLAYBACK, SND_PCM_ASYNC);
                if (err < 0)
                {
                    if (-err == EBUSY)
            ...
            bool setParameters(unsigned int sampleRate, int numChannels, int bufferSize)
            {
                ...
                JUCE_ALSA_LOG("ALSADevice::setParameters(" << deviceID << ", " << (int)sampleRate << ", " << numChannels
                                                           << ", " << bufferSize << ")");
                ...
                if (snd_pcm_hw_params_set_access(handle, hwParams, SND_PCM_ACCESS_RW_INTERLEAVED)
                ...
                for (int i = 0; i < numElementsInArray(formatsToTry); i += 2)
                {
                    if (snd_pcm_hw_params_set_format(handle, hwParams, (_snd_pcm_format)formatsToTry[i]) >= 0)
                    {
                        ...
                        converter =
                            createConverter(isInput, bitDepth, (type & isFloatBit) != 0, (type & isLittleEndianBit) != 0,
                                            (type & onlyUseLower24Bits) != 0, numChannels, isInterleaved);
                        break;
                    }
                }
            ...
            bool writeToOutputDevice(AudioBuffer<float>& outputChannelBuffer, const int numSamples)
            {
                ...
                {
                    for (int i = 0; i < numChannelsRunning; ++i) converter->convertSamples(data[i], data[i], numSamples);
                ...
                if (numDone < 0)
                {
                    if (numDone == -(EPIPE)) underrunCount++;

                    if (JUCE_ALSA_FAILED(snd_pcm_recover(handle, (int)numDone, 1 /* silent */))) return false;
                }
            ...
            bool readFromInputDevice(AudioBuffer<float>& inputChannelBuffer, const int numSamples)
            {
                ...
                if (isInterleaved)
                {
                    ...
                    if (num < 0)
                    {
                        if (num == -(EPIPE)) overrunCount++;

                        if (JUCE_ALSA_FAILED(snd_pcm_recover(handle, (int)num, 1 /* silent */))) return false;
                    }
        ...
        private:
            ...
            {
                static AudioData::Converter* createConverter(const bool forInput, const bool isLittleEndian,
                                                             const int numInterleavedChannels, bool interleaved)
                {
                    if (interleaved) return create<AudioData::Interleaved>(forInput, isLittleEndian, numInterleavedChannels);
            ...
            private:
                ...
                static AudioData::Converter* create(const bool forInput, const bool isLittleEndian,
                                                    const int numInterleavedChannels)
                {
                    if (forInput)
                    {
                        ...
                        if (isLittleEndian)
                            return new AudioData::ConverterInstance<
                                AudioData::Pointer<SampleType, AudioData::LittleEndian, InterleavedType, AudioData::Const>,
                                DestType>(numInterleavedChannels, 1);

                        return new AudioData::ConverterInstance<
                            AudioData::Pointer<SampleType, AudioData::BigEndian, InterleavedType, AudioData::Const>,
                            DestType>(numInterleavedChannels, 1);
                    }
                    ...
                    if (isLittleEndian)
                        return new AudioData::ConverterInstance<
                            SourceType,
                            AudioData::Pointer<SampleType, AudioData::LittleEndian, InterleavedType, AudioData::NonConst>>(
                            1, numInterleavedChannels);

                    return new AudioData::ConverterInstance<
                        SourceType,
                        AudioData::Pointer<SampleType, AudioData::BigEndian, InterleavedType, AudioData::NonConst>>(
                        1, numInterleavedChannels);
                }
            };

            static AudioData::Converter* createConverter(bool forInput, int bitDepth, bool isFloat, bool isLittleEndian,
                                                         bool useOnlyLower24Bits, int numInterleavedChannels,
                                                         bool interleaved)
            {
                JUCE_ALSA_LOG("format: bitDepth=" << bitDepth << ", isFloat=" << (int)isFloat << ", isLittleEndian="
                                                  << (int)isLittleEndian << ", numChannels=" << numInterleavedChannels);

                if (isFloat)
                    return ConverterHelper<AudioData::Float32>::createConverter(forInput, isLittleEndian,
                                                                                numInterleavedChannels, interleaved);
                if (bitDepth == 16)
                    return ConverterHelper<AudioData::Int16>::createConverter(forInput, isLittleEndian,
                                                                              numInterleavedChannels, interleaved);
                if (bitDepth == 24)
                    return ConverterHelper<AudioData::Int24>::createConverter(forInput, isLittleEndian,
                                                                              numInterleavedChannels, interleaved);

                jassert(bitDepth == 32);

                if (useOnlyLower24Bits)
                    return ConverterHelper<AudioData::Int24in32>::createConverter(forInput, isLittleEndian,
                                                                                  numInterleavedChannels, interleaved);
        ...
        public:
            ...
            void open(BigInteger inputChannels, BigInteger outputChannels, const double newSampleRate,
                      const int newBufferSize)
            {
                ...
                if (outputChannelDataForCallback.size() > 0 && outputId.isNotEmpty())
                {
                    ...
                    if (!outputDevice->setParameters(
                            ...
                            jlimit((int)minChansOut, (int)maxChansOut, currentOutputChans.getHighestBit() + 1), bufferSize))
                    {
                        error        = outputDevice->error;
                        outputDevice = nullptr;
                        return;
                ...
                if (inputChannelDataForCallback.size() > 0 && inputId.isNotEmpty())
                {
                    ...
                    if (!inputDevice->setParameters(
                            (unsigned int)sampleRate,
                            jlimit((int)minChansIn, (int)maxChansIn, currentInputChans.getHighestBit() + 1), bufferSize))
                    {
                        error       = inputDevice->error;
                        inputDevice = nullptr;
                        return;
                ...
                if (inputDevice != nullptr && JUCE_ALSA_FAILED(snd_pcm_prepare(inputDevice->handle))) return;

                if (outputDevice != nullptr && JUCE_ALSA_FAILED(snd_pcm_prepare(outputDevice->handle))) return;
                ...
                while (!threadShouldExit())
                {
                    if (inputDevice != nullptr && inputDevice->handle != nullptr)
                    {
                        if (outputDevice == nullptr || outputDevice->handle == nullptr)
                        {
                            ...
                            if (avail < 0) JUCE_ALSA_FAILED(snd_pcm_recover(inputDevice->handle, (int)avail, 0));
                        }
                    ...
                    if (outputDevice != nullptr && outputDevice->handle != nullptr)
                    {
                        ...
                        if (avail < 0) JUCE_ALSA_FAILED(snd_pcm_recover(outputDevice->handle, (int)avail, 0));
            ...
            {
                if (outputDevice != nullptr) return outputDevice->bitDepth;

                if (inputDevice != nullptr) return inputDevice->bitDepth;
            ...
            int getXRunCount() const noexcept
            {
                ...
                if (outputDevice != nullptr) result += outputDevice->underrunCount;

                if (inputDevice != nullptr) result += inputDevice->overrunCount;
        ...
        private:
            ...
            void initialiseRatesAndChannels()
            {
                ...
                for (unsigned int i = 0; i < maxChansOut; ++i) channelNamesOut.add("channel " + String((int)i + 1));

                for (unsigned int i = 0; i < maxChansIn; ++i) channelNamesIn.add("channel " + String((int)i + 1));
            }
        ...
        {
        public:
            ALSAAudioIODevice(const String& deviceName, const String& deviceTypeName, const String& inputDeviceID,
            ...
            void stop() override
            {
                ...
                if (oldCallback != nullptr) oldCallback->audioDeviceStopped();
            }
        ...
        public:
            ...
            int getIndexOfDevice(AudioIODevice* device, bool asInput) const
            {
                ...
                if (ALSAAudioIODevice* d = dynamic_cast<ALSAAudioIODevice*>(device))
                    return asInput ? inputIds.indexOf(d->inputId) : outputIds.indexOf(d->outputId);
        ...
        private:
            ...
            bool testDevice(const String& id, const String& outputName, const String& inputName)
            {
                ...
                {
                    JUCE_ALSA_LOG("testDevice: '" << id.toUTF8().getAddress() << "' -> isInput: " << (int)isInput
                ...
                while (outputIds.size() + inputIds.size() <= 64)
                {
                    ...
                    if (JUCE_CHECKED_RESULT(snd_ctl_open(&handle, ("hw:" + String(cardNum)).toUTF8(), SND_CTL_NONBLOCK)) >= 0)
                    {
                        if (JUCE_CHECKED_RESULT(snd_ctl_card_info(handle, info)) >= 0)
                        {
                            ...
                            if (cardId.removeCharacters("0123456789").isEmpty()) cardId = String(cardNum);
                            ...
                            for (;;)
                            {
                                if (snd_ctl_pcm_next_device(handle, &device) < 0 || device < 0) break;
                                ...
                                for (unsigned int subDevice = 0, nbSubDevice = 1; subDevice < nbSubDevice; ++subDevice)
                                {
                                    ...
                                    if (nbSubDevice == 1) nbSubDevice = snd_pcm_info_get_subdevices_count(pcmInfo);
                                    ...
                                    JUCE_ALSA_LOG("Soundcard ID: " << id << ", name: '" << name << ", isInput:"
            ...
            void enumerateAlsaPCMDevices()
            {
                ...
                if (JUCE_CHECKED_RESULT(snd_device_name_hint(-1, "pcm", &hints)) == 0)
                {
                    for (char** h = (char**)hints; *h; ++h)
                    {
                        ...
                        if (id.isEmpty() || id.startsWith("default:") || id.startsWith("sysdefault:")
                ...
                if (!outputIds.contains("default")) testDevice("default", "Default ALSA Output", "Default ALSA Input");

                // same for the pulseaudio plugin
                if (!outputIds.contains("pulse")) testDevice("pulse", "Pulseaudio output", "Pulseaudio input");
