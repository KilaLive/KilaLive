...
#define CHECK_ERROR(a) CoreMidiHelpers::checkError(a, __LINE__)
        ...
        struct ScopedCFString
        {
            ...
            ~ScopedCFString() noexcept
            {
                if (cfString != nullptr) CFRelease(cfString);
            }
        ...
        static String getEndpointName(MIDIEndpointRef endpoint, bool isExternal)
        {
            ...
            if (result.isEmpty()) result = getMidiObjectName(entity);  // endpoint name is empty - try the entity
        ...
        static void setUniqueIdForMidiPort(MIDIObjectRef device, const String& portName, bool isInput)
        {
            ...
            CFURLRef bundleURL = CFURLCreateWithFileSystemPath(kCFAllocatorDefault, appBundle.getFullPathName().toCFString(),
            ...
            if (bundleURL != nullptr)
            {
                ...
                {
                    if (auto bundleId = CFBundleGetIdentifier(bundleRef)) portUniqueId = String::fromCFString(bundleId);
        ...
        static StringArray findDevices(const bool forInput)
        {
            ...
            const ItemCount num = forInput ? MIDIGetNumberOfSources() : MIDIGetNumberOfDestinations();
            StringArray     s;

            for (ItemCount i = 0; i < num; ++i)
            {
                MIDIEndpointRef dest = forInput ? MIDIGetSource(i) : MIDIGetDestination(i);
                String          name;
        ...
        {
            if (auto* app = JUCEApplicationBase::getInstance()) return app->getApplicationName();
        ...
        public:
            ...
            ~MidiPortAndEndpoint() noexcept
            {
                ...
                if (port == 0
                    && endPoint != 0)  // if port == nullptr, it means we created the endpoint, so it's safe to delete it
        ...
        public:
            ...
            void handlePackets(const MIDIPacketList* const pktlist)
            {
                ...
                if (activeCallbacks.contains(this) && active)
                {
                    ...
                    {
                        concatenator.pushMidiData(packet->data, (int)packet->length, time, input, callback);
