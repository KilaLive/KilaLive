...
namespace juce
{
    ...
    private:
        struct WindowsInputWrapper : public InputWrapper
        {
            ...
            {
                MidiInCollector(WindowsMidiService& s, MidiInput* const inputDevice, MidiInputCallback& cb)
                    : midiService(s)
                    , input(inputDevice)
                    , callback(cb)
                {
                ...
                ~MidiInCollector()
                {
                    ...
                    if (deviceHandle != 0)
                    {
                        ...
                        {
                            if (midiInClose(deviceHandle) == MMSYSERR_NOERROR) break;
                ...
                void handleSysEx(MIDIHDR* const hdr, const uint32 timeStamp)
                {
                    ...
                    {
                        concatenator.pushMidiData(hdr->lpData, (int)hdr->dwBytesRecorded, convertTimeStamp(timeStamp), input,
                                                  callback);
                        writeFinishedBlocks();
                    }
                }
                ...
                static void CALLBACK midiInCallback(HMIDIIN, UINT uMsg, DWORD_PTR dwInstance, DWORD_PTR midiMessage,
                                                    DWORD_PTR timeStamp)
                {
            ...
            private:
                ...
                struct MidiHeader
                {
                    ...
                    {
                        if ((hdr.dwFlags & WHDR_DONE) != 0) write(device);
                    }
                ...
                {
                    for (int i = 0; i < (int)numHeaders; ++i) headers[i].writeIfFinished(deviceHandle);
                }

                void unprepareAllHeaders()
                {
                    for (int i = 0; i < (int)numHeaders; ++i) headers[i].unprepare(deviceHandle);
                }
            ...
            //==============================================================================
            WindowsInputWrapper(WindowsMidiService& parentService, MidiInput* const input, const int index,
                                MidiInputCallback* const callback)
            {
                ...
                HMIDIIN  h;
                MMRESULT err = midiInOpen(&h, deviceId, (DWORD_PTR)&MidiInCollector::midiInCallback,
                                          (DWORD_PTR)(MidiInCollector*)collector.get(), CALLBACK_FUNCTION);

                if (err != MMSYSERR_NOERROR) throw std::runtime_error("Failed to create Windows input device wrapper");
        ...
        struct WindowsOutputWrapper : public OutputWrapper
        {
            ...
            WindowsOutputWrapper(WindowsMidiService& p, int index)
                : parent(p)
            {
                auto names    = getDevices();
                UINT deviceId = MIDI_MAPPER;
                ...
                if (deviceId == MIDI_MAPPER)
                {
                    ...
                    for (int i = 0; i < names.size(); ++i)
                        if (names[i].containsIgnoreCase("microsoft")) deviceId = (UINT)i;
                }
            ...
            void sendMessageNow(const MidiMessage& message) override
            {
                if (message.getRawDataSize() > 3 || message.isSysEx())
                {
                    ...
                    if (midiOutPrepareHeader(han->handle, &h, sizeof(MIDIHDR)) == MMSYSERR_NOERROR)
                    {
                        ...
                        if (res == MMSYSERR_NOERROR)
                        {
                            while ((h.dwFlags & MHDR_DONE) == 0) Sleep(1);
                ...
                else
                {
                    for (int i = 0; i < 50; ++i)
                    {
                        if (midiOutShortMsg(han->handle, *(unsigned int*)message.getRawData()) != MIDIERR_NOTREADY) break;
            ...
            static Array<MIDIOUTCAPS> getDeviceCaps()
            {
                ...
                for (UINT i = 0; i < num; ++i)
                {
                    ...
                    if (midiOutGetDevCaps(i, &mc, sizeof(mc)) == MMSYSERR_NOERROR) devices.add(mc);
                }
            ...
            static StringArray getDevices()
            {
                ...
                for (auto& mc : getDeviceCaps()) s.add(String(mc.szPname, (size_t)numElementsInArray(mc.szPname)));
            ...
            static int getDefaultDeviceIndex()
            {
                ...
                {
                    if ((mc.wTechnology & MOD_MAPPER) != 0) return n;
    ...
    public:
        ...
        {
            return isInput ? WindowsInputWrapper::getDevices() : WindowsOutputWrapper::getDevices();
        }

        int getDefaultDeviceIndex(bool isInput) override
        {
            return isInput ? WindowsInputWrapper::getDefaultDeviceIndex() : WindowsOutputWrapper::getDefaultDeviceIndex();
        }
...
//==============================================================================
    ...
    private:
        ...
        struct MidiIODeviceWatcher
        {
            ...
            bool start()
            {
                ...
                auto deviceInformationFactory = WinRTWrapper::getInstance()->getWRLFactory<IDeviceInformationStatics>(
                    &RuntimeClass_Windows_Devices_Enumeration_DeviceInformation[0]);
                if (deviceInformationFactory == nullptr) return false;
                ...
                public:
                    ...
                    void run() override
                    {
                        ...
                        parent.watcher->add_Removed(Callback<ITypedEventHandler<DeviceWatcher*, DeviceInformationUpdate*>>(
                                                        [parentPtr](IDeviceWatcher*, IDeviceInformationUpdate* info) {
                                                            return parentPtr->removeDevice(info);
                                                        })
                                                        .Get(),
                                                    &parent.deviceRemovedToken);

                        EventRegistrationToken deviceEnumerationCompletedToken{0};
                        parent.watcher->add_EnumerationCompleted(Callback<ITypedEventHandler<DeviceWatcher*, IInspectable*>>(
                                                                     [this](IDeviceWatcher*, IInspectable*) {
            ...
            bool stop()
            {
                if (watcher == nullptr) return true;
            ...
            StringArray getDevices()
            {
                ...
                StringArray result;
                for (auto info : lastQueriedConnectedDevices.get()) result.add(info.name);
            ...
            int getDefaultDeviceIndex()
            {
                ...
                for (int i = 0; i < lastDevices.size(); ++i)
                    if (lastDevices[i].isDefault) return i;
        ...
        {
        public:
            OpenMidiPortThread(String threadName, String midiDeviceId, ComSmartPtr<COMFactoryType>& comFactory,
            ...
            void run() override
            {
                ...
                hr = asyncOp->put_Completed(Callback<IAsyncOperationCompletedHandler<COMType*>>(
                                                [this](IAsyncOperation<COMType*>* asyncOpPtr, AsyncStatus) {
                                                    if (asyncOpPtr == nullptr) return E_ABORT;
        ...
        {
            WinRTInputWrapper(WinRTMidiService& service, MidiInput* const input, const int index, MidiInputCallback& cb)
                : inputDevice(input)
                , callback(cb)
                , concatenator(4096)
            {
                ...
                if (deviceName.isEmpty()) throw std::runtime_error("Invalid device index");

                const auto deviceID = service.inputDeviceWatcher->getDeviceID(deviceName);
                if (deviceID.isEmpty()) throw std::runtime_error("Device unavailable");

                OpenMidiPortThread<IMidiInPortStatics, IMidiInPort, MidiInPort> portThread(
                    "Open WinRT MIDI input port", deviceID, service.midiInFactory, midiInPort);
                portThread.startThread();
                portThread.waitForThreadToExit(-1);
                if (midiInPort == nullptr) throw std::runtime_error("Timed out waiting for midi input port creation");
                ...
                HRESULT hr = midiInPort->add_MessageReceived(
                    Callback<ITypedEventHandler<MidiInPort*, MidiMessageReceivedEventArgs*>>(
                        ...
                        .Get(),
                    &midiInMessageToken);
                if (FAILED(hr)) throw std::runtime_error("Failed to set midi input callback");
            }

            ~WinRTInputWrapper()
            {
                if (midiInMessageToken.value != 0) midiInPort->remove_MessageReceived(midiInMessageToken);
        ...
        struct WinRTOutputWrapper : public OutputWrapper
        {
            WinRTOutputWrapper(WinRTMidiService& service, const int index)
            {
                ...
                OpenMidiPortThread<IMidiOutPortStatics, IMidiOutPort, IMidiOutPort> portThread(
                    "Open WinRT MIDI output port", deviceID, service.midiOutFactory, midiOutPort);
                portThread.startThread();
                portThread.waitForThreadToExit(-1);
                if (midiOutPort == nullptr) throw std::runtime_error("Timed out waiting for midi output port creation");

                auto bufferFactory = WinRTWrapper::getInstance()->getWRLFactory<IBufferFactory>(
                    &RuntimeClass_Windows_Storage_Streams_Buffer[0]);
                if (bufferFactory == nullptr) throw std::runtime_error("Failed to create output buffer factory");

                HRESULT hr = bufferFactory->Create(static_cast<UINT32>(65536), buffer.resetAndGetPointerAddress());
                if (FAILED(hr)) throw std::runtime_error("Failed to create output buffer");

                hr = buffer->QueryInterface(bufferByteAccess.resetAndGetPointerAddress());
                if (FAILED(hr)) throw std::runtime_error("Failed to get buffer byte access");

                hr = bufferByteAccess->Buffer(&bufferData);
                if (FAILED(hr)) throw std::runtime_error("Failed to get buffer data pointer");
            }
    ...
    public:
        WinRTMidiService()
        {
            ...
            midiInFactory = WinRTWrapper::getInstance()->getWRLFactory<IMidiInPortStatics>(
                &RuntimeClass_Windows_Devices_Midi_MidiInPort[0]);
            if (midiInFactory == nullptr) throw std::runtime_error("Failed to create midi in factory");

            midiOutFactory = WinRTWrapper::getInstance()->getWRLFactory<IMidiOutPortStatics>(
                &RuntimeClass_Windows_Devices_Midi_MidiOutPort[0]);
            if (midiOutFactory == nullptr) throw std::runtime_error("Failed to create midi out factory");

            inputDeviceWatcher = new MidiIODeviceWatcher<IMidiInPortStatics>(midiInFactory);
            if (!inputDeviceWatcher->start()) throw std::runtime_error("Failed to start midi input device watcher");

            outputDeviceWatcher = new MidiIODeviceWatcher<IMidiOutPortStatics>(midiOutFactory);
            if (!outputDeviceWatcher->start()) throw std::runtime_error("Failed to start midi output device watcher");
        }
        ...
        StringArray getDevices(bool isInput) override
        {
            return isInput ? inputDeviceWatcher->getDevices() : outputDeviceWatcher->getDevices();
        }

        int getDefaultDeviceIndex(bool isInput) override
        {
            return isInput ? inputDeviceWatcher->getDefaultDeviceIndex() : outputDeviceWatcher->getDefaultDeviceIndex();
        }
    ...
    {
        static_cast<MidiServiceType::InputWrapper*>(internal)->start();
    }
    void MidiInput::stop()
    {
        static_cast<MidiServiceType::InputWrapper*>(internal)->stop();
    }
