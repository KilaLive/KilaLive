...
namespace juce
{
    ...
    {
        if (juce_libjackHandle == nullptr) return nullptr;
...
#define JUCE_DECL_VOID_JACK_FUNCTION(fn_name, argument_types, arguments)                                                     \
    ...
    JUCE_DECL_VOID_JACK_FUNCTION(jack_on_shutdown, (jack_client_t * client, void (*function)(void* arg), void* arg),
                                 (client, function, arg));
    JUCE_DECL_JACK_FUNCTION(void*, jack_port_get_buffer, (jack_port_t * port, jack_nframes_t nframes), (port, nframes));
    JUCE_DECL_JACK_FUNCTION(jack_nframes_t, jack_port_get_total_latency, (jack_client_t * client, jack_port_t* port),
                            (client, port));
    JUCE_DECL_JACK_FUNCTION(jack_port_t*, jack_port_register,
                            (jack_client_t * client, const char* port_name, const char* port_type, unsigned long flags,
    ...
    JUCE_DECL_JACK_FUNCTION(jack_port_t*, jack_port_by_id, (jack_client_t * client, jack_port_id_t port_id),
                            (client, port_id));
    ...
    JUCE_DECL_JACK_FUNCTION(int, jack_set_xrun_callback, (jack_client_t * client, JackXRunCallback xrun_callback, void* arg),
...
#define JACK_LOGGING_ENABLED 1
    ...
    namespace
    {
        ...
        {
            if (status & JackServerFailed || status & JackServerError) return "Unable to connect to JACK server";
...
#define JUCE_JACK_CLIENT_NAME "JUCEJack"
    ...
    struct JackPortIterator
    {
        ...
        {
            if (client != nullptr)
                ports = juce::jack_get_ports(client, nullptr, nullptr, forInput ? JackPortIsOutput : JackPortIsInput);
        ...
        bool next()
        {
            if (ports == nullptr || ports[index + 1] == nullptr) return false;
    ...
    {
    public:
        JackAudioIODevice(const String& deviceName, const String& inId, const String& outId)
            ...
            , totalNumberOfOutputChannels(0)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < inputChannels.size(); ++i)
                {
                    ...
                    inputPorts.add(
                        juce::jack_port_register(client, inputName.toUTF8(), JACK_DEFAULT_AUDIO_TYPE, JackPortIsInput, 0));
                }
                ...
                for (int i = 0; i < outputChannels.size(); ++i)
                {
                    ...
                    outputPorts.add(
                        juce::jack_port_register(client, outputName.toUTF8(), JACK_DEFAULT_AUDIO_TYPE, JackPortIsOutput, 0));
                }
        ...
        StringArray getChannelNames(bool forInput) const
        {
            ...
            for (JackPortIterator i(client, forInput); i.next();)
                if (i.clientName == getName()) names.add(i.name.fromFirstOccurrenceOf(":", false, false));
        ...
        Array<double> getAvailableSampleRates() override
        {
            ...
            if (client != nullptr) rates.add(juce::jack_get_sample_rate(client));
        ...
        Array<int> getAvailableBufferSizes() override
        {
            ...
            if (client != nullptr) sizes.add(juce::jack_get_buffer_size(client));
                    ...
                    int /* bufferSizeSamples */) override
        {
            ...
            if (!inputChannels.isZero())
            {
                for (JackPortIterator i(client, true); i.next();)
                {
                    ...
                    {
                        int error = juce::jack_connect(client, i.ports[i.index],
                                                       juce::jack_port_name((jack_port_t*)inputPorts[i.index]));
                        if (error != 0)
                            JUCE_JACK_LOG("Cannot connect input port " + String(i.index) + " (" + i.name + "), error "
                                          + String(error));
                    }
                }
            }
            ...
            if (!outputChannels.isZero())
            {
                for (JackPortIterator i(client, false); i.next();)
                {
                    ...
                    {
                        int error = juce::jack_connect(client, juce::jack_port_name((jack_port_t*)outputPorts[i.index]),
                                                       i.ports[i.index]);
                        if (error != 0)
                            JUCE_JACK_LOG("Cannot connect output port " + String(i.index) + " (" + i.name + "), error "
                                          + String(error));
                    }
                }
            }
    ...
    private:
        void process(const int numSamples)
        {
            ...
            for (int i = 0; i < totalNumberOfInputChannels; ++i)
            {
                if (activeInputChannels[i])
                    if (jack_default_audio_sample_t* in = (jack_default_audio_sample_t*)juce::jack_port_get_buffer(
            ...
            for (int i = 0; i < totalNumberOfOutputChannels; ++i)
            {
                if (activeOutputChannels[i])
                    if (jack_default_audio_sample_t* out = (jack_default_audio_sample_t*)juce::jack_port_get_buffer(
            ...
            if (callback != nullptr)
            {
                if ((numActiveInChans + numActiveOutChans) > 0)
                    callback->audioDeviceIOCallback(const_cast<const float**>(inChans.getData()), numActiveInChans, outChans,
                                                    numActiveOutChans, numSamples);
            }
            else
            {
                for (int i = 0; i < numActiveOutChans; ++i) zeromem(outChans[i], sizeof(float) * numSamples);
            }
        }

        static int processCallback(jack_nframes_t nframes, void* callbackArgument)
        {
            if (callbackArgument != nullptr) ((JackAudioIODevice*)callbackArgument)->process(nframes);
        ...
        {
            if (callbackArgument != nullptr) ((JackAudioIODevice*)callbackArgument)->xruns++;
        ...
        void updateActivePorts()
        {
            ...
            for (int i = 0; i < outputPorts.size(); ++i)
                if (juce::jack_port_connected((jack_port_t*)outputPorts.getUnchecked(i))) newOutputChannels.setBit(i);

            for (int i = 0; i < inputPorts.size(); ++i)
                if (juce::jack_port_connected((jack_port_t*)inputPorts.getUnchecked(i))) newInputChannels.setBit(i);

            if (newOutputChannels != activeOutputChannels || newInputChannels != activeInputChannels)
            {
                AudioIODeviceCallback* const oldCallback = callback;
        ...
        {
            if (JackAudioIODevice* device = static_cast<JackAudioIODevice*>(arg)) device->updateActivePorts();
        }
    ...
    public:
        ...
        AudioIODevice* createDevice(const String& outputDeviceName, const String& inputDeviceName)
        {
            ...
            if (inputIndex >= 0 || outputIndex >= 0)
                return new JackAudioIODevice(outputIndex >= 0 ? outputDeviceName : inputDeviceName, inputIds[inputIndex],
    ...
    void JackAudioIODevice::sendDeviceChangedCallback()
    {
        for (int i = activeDeviceTypes.size(); --i >= 0;)
            if (JackAudioIODeviceType* d = activeDeviceTypes[i]) d->portConnectionChange();
    }
