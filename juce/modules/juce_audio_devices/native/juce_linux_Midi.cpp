...
#define JUCE_ALSA_MIDI_NAME JUCEApplicationBase::getInstance()->getApplicationName().toUTF8()
    ...
    namespace
    {
        ...
        public:
            ...
            {
                if (instance == nullptr) instance = new AlsaClient();
            ...
            {
                if (inputThread == nullptr) inputThread = new MidiInputThread(*this);

                if (++activeCallbacks - 1 == 0) inputThread->startThread();
            }

            void unregisterCallback()
            {
                jassert(activeCallbacks.get() > 0);
                if (--activeCallbacks == 0 && inputThread->isThreadRunning()) inputThread->signalThreadShouldExit();
            }
            ...
            void handlePartialSysexMessage(snd_seq_event* event, const uint8* messageData, int numBytesSoFar,
                                           double timeStamp)
            {
                if (event->dest.port < ports.size() && ports[event->dest.port]->callbackEnabled)
        ...
        private:
            ...
            ~AlsaClient()
            {
                ...
                if (handle != nullptr) snd_seq_close(handle);

                jassert(activeCallbacks.get() == 0);

                if (inputThread) inputThread->stopThread(3000);
            }
            ...
            public:
                ...
                void run() override
                {
                    ...
                    if (snd_midi_event_new(maxEventSize, &midiParser) >= 0)
                    {
                        ...
                        {
                            if (poll(pfd, (nfds_t)numPfds, 100)
                                > 0)  // there was a "500" here which is a bit long when we exit the program and have to wait
                                      // for a timeout on this poll call
                            {
                                ...
                                do
                                {
                                    ...
                                    if (snd_seq_event_input(seqHandle, &inputEvent) >= 0)
                                    {
                                        ...
                                        const long numBytes =
                                            snd_midi_event_decode(midiParser, buffer, maxEventSize, inputEvent);

                                        snd_midi_event_reset_decode(midiParser);

                                        concatenator.pushMidiData(buffer, (int)numBytes,
                                                                  Time::getMillisecondCounter() * 0.001, inputEvent, client);
        ...
        //==============================================================================
        static AlsaClient::Port* iterateMidiClient(const AlsaClient::Ptr& client, snd_seq_client_info_t* clientInfo,
                                                   const bool forInput, StringArray& deviceNamesFound,
        ...
        static AlsaClient::Port* iterateMidiDevices(const bool forInput, StringArray& deviceNamesFound,
                                                    const int deviceIndexToOpen)
        {
            AlsaClient::Port*     port = nullptr;
                ...
                if (snd_seq_system_info(seqHandle, systemInfo) == 0)
                {
                    ...
                    while (--numClients >= 0)
                    {
                        if (snd_seq_query_next_client(seqHandle, clientInfo) == 0)
                        {
                            ...
                            {
                                port = iterateMidiClient(client, clientInfo, forInput, deviceNamesFound, deviceIndexToOpen);
                                if (port) break;
                            }
                        }
                    }
