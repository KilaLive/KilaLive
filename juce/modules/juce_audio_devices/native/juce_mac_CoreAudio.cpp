...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    ...
    struct SystemVol
    {
            ...
            : outputDeviceID(kAudioObjectUnknown)
        {
            ...
            if (AudioHardwareServiceHasProperty(kAudioObjectSystemObject, &addr))
            {
                ...
                OSStatus status       = AudioHardwareServiceGetPropertyData(kAudioObjectSystemObject, &addr, 0, nullptr,
                                                                      &deviceIDSize, &outputDeviceID);
                ...
                if (status == noErr)
                {
                    ...
                    if (!AudioHardwareServiceHasProperty(outputDeviceID, &addr)) outputDeviceID = kAudioObjectUnknown;
                }
            }
        }
        ...
        float getGain() const noexcept
        {
            ...
            if (outputDeviceID != kAudioObjectUnknown)
            {
                ...
                AudioHardwareServiceGetPropertyData(outputDeviceID, &addr, 0, nullptr, &size, &gain);
            }
        ...
        bool setGain(float gain) const noexcept
        {
            if (outputDeviceID != kAudioObjectUnknown && canSetVolume())
            {
                ...
                return AudioHardwareServiceSetPropertyData(outputDeviceID, &addr, 0, nullptr, size, &newVolume) == noErr;
            }
        ...
        bool isMuted() const noexcept
        {
            ...
            if (outputDeviceID != kAudioObjectUnknown)
            {
                ...
                AudioHardwareServiceGetPropertyData(outputDeviceID, &addr, 0, nullptr, &size, &muted);
            }
        ...
        bool setMuted(bool mute) const noexcept
        {
            if (outputDeviceID != kAudioObjectUnknown && canSetVolume())
            {
                ...
                return AudioHardwareServiceSetPropertyData(outputDeviceID, &addr, 0, nullptr, size, &newMute) == noErr;
            }
    ...
    private:
        ...
        bool canSetVolume() const noexcept
        {
            ...
            return AudioHardwareServiceIsPropertySettable(outputDeviceID, &addr, &isSettable) == noErr && isSettable;
        }
    };
...
#define JUCE_SYSTEMAUDIOVOL_IMPLEMENTED 1
    ...
    struct CoreAudioClasses
    {
        ...
        public:
                    ...
                    if (OK(AudioObjectGetPropertyData(deviceID, &pa, 0, nullptr, &size, bufList)))
                    {
                        ...
                        for (int i = 0; i < numStreams; ++i)
                        {
                            ...
                            for (unsigned int j = 0; j < b.mNumberChannels; ++j)
                            {
                                ...
                                if (name.isEmpty()) name << (input ? "Input " : "Output ") << (chanNum + 1);
            ...
            Array<double> getSampleRatesFromDevice() const
            {
                ...
                if (OK(AudioObjectGetPropertyDataSize(deviceID, &pa, 0, nullptr, &size)))
                {
                    ...
                    {
                        static const double possibleRates[] = {44100.0,  48000.0,  88200.0, 96000.0,
            ...
            int getBitDepthFromDevice(AudioObjectPropertyScope scope) const
            {
                ...
                if (OK(AudioObjectGetPropertyData(deviceID, &pa, 0, nullptr, &size, &asbd))) return (int)asbd.mBitsPerChannel;
            ...
            bool isDeviceAlive() const
            {
                ...
                return deviceID != 0 && OK(AudioObjectGetPropertyData(deviceID, &pa, 0, nullptr, &size, &isAlive))
                       && isAlive != 0;
            }
            ...
            StringArray getDeviceDetails()
            {
                ...
                for (auto& s : sampleRates) availableSampleRates << " " << s;
                ...
                for (auto& b : bufferSizes) availableBufferSizes << " " << b;
            ...
            //==============================================================================
            String reopen(const BigInteger& inputChannels, const BigInteger& outputChannels, double newSampleRate,
                          int bufferSizeSamples)
            {
                ...
                activeInputChans.setRange(inChanNames.size(), activeInputChans.getHighestBit() + 1 - inChanNames.size(),
                                          false);

                activeOutputChans = outputChannels;
                activeOutputChans.setRange(outChanNames.size(), activeOutputChans.getHighestBit() + 1 - outChanNames.size(),
                                           false);
            ...
            void audioCallback(const AudioBufferList* inInputData, AudioBufferList* outOutputData)
            {
                ...
                if (callback != nullptr)
                {
                    ...
                    callback->audioDeviceIOCallback(const_cast<const float**>(tempInputBuffers.get()), numInputChans,
                                                    tempOutputBuffers, numOutputChans, bufferSize);
        ...
        private:
            ...
            //==============================================================================
            static OSStatus audioIOProc(AudioDeviceID /*inDevice*/, const AudioTimeStamp* /*inNow*/,
                                        const AudioBufferList* inInputData, const AudioTimeStamp* /*inInputTime*/,
                                        AudioBufferList* outOutputData, const AudioTimeStamp* /*inOutputTime*/, void* device)
            {
            ...
            static int getAllDataSourcesForDevice(AudioDeviceID deviceID, HeapBlock<OSType>& types)
            {
                ...
                if (deviceID != 0 && AudioObjectGetPropertyDataSize(deviceID, &pa, 0, nullptr, &size) == noErr)
                {
                    types.calloc(size, 1);
        ...
        //==============================================================================
        class CoreAudioIODevice : public AudioIODevice, private Timer
        {
        public:
            CoreAudioIODevice(CoreAudioIODeviceType& dt, const String& deviceName, AudioDeviceID inputDeviceId,
                              const int inputIndex_, AudioDeviceID outputDeviceId, const int outputIndex_)
            ...
            {
                if (internal->callback == nullptr) isStarted = false;
                ...
                {
                    {
                        ...
                        if (isStarted)
                        {
                            if (internal->callback != nullptr) previousCallback = internal->callback;
        ...
        private:
            ...
            static OSStatus hardwareListenerProc(AudioDeviceID /*inDevice*/, UInt32 /*inLine*/,
                                                 const AudioObjectPropertyAddress* pa, void* inClientData)
            {
                switch (pa->mSelector)
                {
        ...
        //==============================================================================
        class AudioIODeviceCombiner : public AudioIODevice, private Thread, private Timer
        {
        public:
            ...
            void addDevice(CoreAudioIODevice* device, bool useInputs, bool useOutputs)
            {
                ...
                if (currentSampleRate == 0) currentSampleRate = device->getCurrentSampleRate();

                if (currentBufferSize == 0) currentBufferSize = device->getCurrentBufferSizeSamples();
            }
            ...
            Array<AudioIODevice*> getDevices() const
            {
                ...
                for (auto* d : devices) devs.add(d->device.get());
            ...
            StringArray getOutputChannelNames() override
            {
                ...
                for (auto* d : devices) names.addArray(d->getOutputChannelNames());
            ...
            StringArray getInputChannelNames() override
            {
                ...
                for (auto* d : devices) names.addArray(d->getInputChannelNames());
            ...
            int getCurrentBitDepth() override
            {
                ...
                for (auto* d : devices) depth = jmin(depth, d->device->getCurrentBitDepth());
            ...
            int getDefaultBufferSize() override
            {
                ...
                for (auto* d : devices) size = jmax(size, d->device->getDefaultBufferSize());
                        ...
                        int bufferSize) override
            {
                ...
                if (sampleRate <= 0)
                {
                    ...
                    for (int i = 0; i < rates.size() && sampleRate < 44100.0; ++i) sampleRate = rates.getUnchecked(i);
                }
                ...
                for (auto* d : devices)
                {
                    ...
                    String err = d->open(ins, outs, sampleRate, bufferSize, chanIndex, fifoSize);
            ...
            int getOutputLatencyInSamples() override
            {
                ...
                for (auto* d : devices) lat = jmax(lat, d->device->getOutputLatencyInSamples());
            ...
            int getInputLatencyInSamples() override
            {
                ...
                for (auto* d : devices) lat = jmax(lat, d->device->getInputLatencyInSamples());
        ...
        private:
            ...
            void run() override
            {
                ...
                while (!threadShouldExit())
                {
                    ...
                    {
                        for (int i = 0; i < numOutputChans; ++i) FloatVectorOperations::clear(outputChans[i], numSamples);
            ...
            void shutdown(const String& error)
            {
                ...
                for (auto* d : devices) d->device->stop();
            ...
            {
                for (auto* d : devices) d->done = (d->numInputChans == 0);
                ...
                for (auto* d : devices)
                    if (!d->done)
                        for (int i = 0; i < d->numInputChans; ++i) buffer.clear(d->inputIndex + i, 0, numSamples);
            }

            void pushOutputData(AudioBuffer<float>& buffer, const int numSamples, const int blockSizeMs)
            {
                for (auto* d : devices) d->done = (d->numOutputChans == 0);
                ...
                for (int tries = 5;;)
                {
                    ...
                    if ((!anyRemaining) || --tries == 0) return;
            ...
            void handleAudioDeviceAboutToStart(AudioIODevice* device)
            {
                ...
                if (anySampleRateChanges) owner.audioDeviceListChanged();
            ...
            struct DeviceWrapper : private AudioIODeviceCallback
            {
                ...
                String open(const BigInteger& inputChannels, const BigInteger& outputChannels, double sampleRate,
                            int bufferSize, int channelIndex, int fifoSize)
                {
                    ...
                    auto err = device->open(useInputs ? inputChannels : BigInteger(),
                                            useOutputs ? outputChannels : BigInteger(), sampleRate, bufferSize);
                                           ...
                                           int numOutputChannels, int numSamples) override
                {
                    if (numInputChannels > 0)
                    {
                        ...
                        if (numSamples > totalSize)
                        {
                            ...
                            for (int i = 0; i < numInputChans; ++i)
                                FloatVectorOperations::clear(owner.fifoWritePointers[inputIndex + i] + totalSize,
                                                             samplesRemaining);
        ...
        //==============================================================================
        class CoreAudioIODeviceType : public AudioIODeviceType, private AsyncUpdater
        {
        public:
                ...
                if (AudioObjectGetPropertyData(kAudioObjectSystemObject, &pa, 0, nullptr, &size, &deviceID) == noErr)
                {
                    if (forInput)
                    {
                        for (int i = inputIds.size(); --i >= 0;)
                            if (inputIds[i] == deviceID) return i;
                    }
                    else
                    {
                        for (int i = outputIds.size(); --i >= 0;)
                            if (outputIds[i] == deviceID) return i;
                    }
                }
            ...
            int getIndexOfDevice(AudioIODevice* device, bool asInput) const override
            {
                ...
                if (auto* d = dynamic_cast<CoreAudioIODevice*>(device)) return asInput ? d->inputIndex : d->outputIndex;
            ...
            static int getNumChannels(AudioDeviceID deviceID, bool input)
            {
                ...
                if (AudioObjectGetPropertyDataSize(deviceID, &pa, 0, nullptr, &size) == noErr)
                {
                    ...
                    if (AudioObjectGetPropertyData(deviceID, &pa, 0, nullptr, &size, bufList) == noErr)
                    {
                        ...
                        for (int i = 0; i < numStreams; ++i) total += bufList->mBuffers[i].mNumberChannels;
                    }
                }
