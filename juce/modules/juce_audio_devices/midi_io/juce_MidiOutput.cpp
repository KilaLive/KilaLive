...
namespace juce
{
    ...
    void MidiOutput::sendBlockOfMessagesNow(const MidiBuffer& buffer)
    {
        ...
        while (i.getNextEvent(message, samplePosition)) sendMessageNow(message);
    }

    void MidiOutput::sendBlockOfMessages(const MidiBuffer& buffer, const double millisecondCounterToStartAt,
                                         double samplesPerSecondForBuffer)
    {
        ...
        while (i.getNextEvent(data, len, time))
        {
            ...
            else
            {
                ...
                while (mm->next != nullptr && mm->next->message.getTimeStamp() <= eventTime) mm = mm->next;
    ...
    void MidiOutput::run()
    {
        while (!threadShouldExit())
        {
            ...
            if (message != nullptr)
            {
                ...
                if (eventTime > now - 200) sendMessageNow(message->message);
            }
            else
            {
