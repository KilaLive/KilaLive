...
#define JUCE_INCREMENT_DEST dest += (16 / sizeof(*dest));
        ...
        struct BasicOps32
        {
            ...
            static forcedinline ParallelType add(ParallelType a, ParallelType b) noexcept
            {
                return _mm_add_ps(a, b);
            }
            static forcedinline ParallelType sub(ParallelType a, ParallelType b) noexcept
            {
                return _mm_sub_ps(a, b);
            }
            static forcedinline ParallelType mul(ParallelType a, ParallelType b) noexcept
            {
                return _mm_mul_ps(a, b);
            }
            static forcedinline ParallelType max(ParallelType a, ParallelType b) noexcept
            {
                return _mm_max_ps(a, b);
            }
            static forcedinline ParallelType min(ParallelType a, ParallelType b) noexcept
            {
...
#define JUCE_PERFORM_VEC_OP_SRC1_SRC2_DEST_DEST(normalOp, vecOp, locals, increment, setupOp)                                 \
    ...
    {                                                                                                                        \
        ...
        {                                                                                                                    \
            if (FloatVectorHelpers::isAligned(src2))                                                                         \
                JUCE_VEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD(vecOp, Mode::loadA, Mode::loadA, Mode::loadA, Mode::storeA, locals, \
                                                         increment)                                                          \
            else                                                                                                             \
                JUCE_VEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD(vecOp, Mode::loadA, Mode::loadU, Mode::loadA, Mode::storeA, locals, \
                                                         increment)                                                          \
        }                                                                                                                    \
        else                                                                                                                 \
        {                                                                                                                    \
            if (FloatVectorHelpers::isAligned(src2))                                                                         \
                JUCE_VEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD(vecOp, Mode::loadU, Mode::loadA, Mode::loadA, Mode::storeA, locals, \
                                                         increment)                                                          \
            else                                                                                                             \
                JUCE_VEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD(vecOp, Mode::loadU, Mode::loadU, Mode::loadA, Mode::storeA, locals, \
                                                         increment)                                                          \
        }                                                                                                                    \
    }                                                                                                                        \
    else                                                                                                                     \
    {                                                                                                                        \
        ...
        {                                                                                                                    \
            if (FloatVectorHelpers::isAligned(src2))                                                                         \
                JUCE_VEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD(vecOp, Mode::loadA, Mode::loadA, Mode::loadU, Mode::storeU, locals, \
                                                         increment)                                                          \
            else                                                                                                             \
                JUCE_VEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD(vecOp, Mode::loadA, Mode::loadU, Mode::loadU, Mode::storeU, locals, \
                                                         increment)                                                          \
        }                                                                                                                    \
        else                                                                                                                 \
        {                                                                                                                    \
            if (FloatVectorHelpers::isAligned(src2))                                                                         \
                JUCE_VEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD(vecOp, Mode::loadU, Mode::loadA, Mode::loadU, Mode::storeU, locals, \
                                                         increment)                                                          \
            else                                                                                                             \
                JUCE_VEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD(vecOp, Mode::loadU, Mode::loadU, Mode::loadU, Mode::storeU, locals, \
                                                         increment)                                                          \
        }                                                                                                                    \
    }                                                                                                                        \
    JUCE_FINISH_VEC_OP(normalOp)
...
#define JUCE_PERFORM_VEC_OP_DEST(normalOp, vecOp, locals, setupOp)                                                           \
    JUCE_BEGIN_VEC_OP                                                                                                        \
    setupOp JUCE_VEC_LOOP(vecOp, dummy, Mode::loadU, Mode::storeU, locals, JUCE_INCREMENT_DEST) JUCE_FINISH_VEC_OP(normalOp)
...
#define JUCE_PERFORM_VEC_OP_SRC1_SRC2_DEST_DEST(normalOp, vecOp, locals, increment, setupOp)                                 \
    JUCE_BEGIN_VEC_OP                                                                                                        \
    setupOp JUCE_VEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD(vecOp, Mode::loadU, Mode::loadU, Mode::loadU, Mode::storeU, locals,     \
...
#define JUCE_LOAD_SRC_DEST(srcLoad, dstLoad) const Mode::ParallelType d = dstLoad(dest), s = srcLoad(src);
        ...
        struct MinMax
        {
            ...
            static Type findMinOrMax(const Type* src, int num, const bool isMinimum) noexcept
            {
                ...
                if (numLongOps > 1)
                {
                    ...
                    for (int i = 0; i < num; ++i) result = isMinimum ? jmin(result, src[i]) : jmax(result, src[i]);
            ...
            static Range<Type> findMinAndMax(const Type* src, int num) noexcept
            {
                ...
                if (numLongOps > 1)
                {
                    ...
                    for (int i = 0; i < num; ++i) result = result.getUnionWith(src[i]);
    ...
    void JUCE_CALLTYPE FloatVectorOperations::copyWithMultiply(float* dest, const float* src, float multiplier,
                                                               int num) noexcept
    {
#if JUCE_USE_VDSP_FRAMEWORK
        vDSP_vsmul(src, 1, &multiplier, dest, 1, (vDSP_Length)num);
#else
        JUCE_PERFORM_VEC_OP_SRC_DEST(dest[i] = src[i] * multiplier, Mode::mul(mult, s), JUCE_LOAD_SRC,
                                     JUCE_INCREMENT_SRC_DEST, const Mode::ParallelType mult = Mode::load1(multiplier);)
#endif
    }

    void JUCE_CALLTYPE FloatVectorOperations::copyWithMultiply(double* dest, const double* src, double multiplier,
                                                               int num) noexcept
    {
#if JUCE_USE_VDSP_FRAMEWORK
...
#else
        JUCE_PERFORM_VEC_OP_SRC_DEST(dest[i] = src[i] + amount, Mode::add(am, s), JUCE_LOAD_SRC, JUCE_INCREMENT_SRC_DEST,
...
#else
        JUCE_PERFORM_VEC_OP_SRC1_SRC2_DEST(dest[i] = src1[i] + src2[i], Mode::add(s1, s2), JUCE_LOAD_SRC1_SRC2,
...
#else
        JUCE_PERFORM_VEC_OP_SRC1_SRC2_DEST(dest[i] = src1[i] - src2[i], Mode::sub(s1, s2), JUCE_LOAD_SRC1_SRC2,
                                           JUCE_INCREMENT_SRC1_SRC2_DEST, )
#endif
    }
    ...
    void JUCE_CALLTYPE FloatVectorOperations::addWithMultiply(float* dest, const float* src, float multiplier,
                                                              int num) noexcept
    {
#if JUCE_USE_VDSP_FRAMEWORK
        vDSP_vsma(src, 1, &multiplier, dest, 1, dest, 1, (vDSP_Length)num);
#else
        JUCE_PERFORM_VEC_OP_SRC_DEST(dest[i] += src[i] * multiplier, Mode::add(d, Mode::mul(mult, s)), JUCE_LOAD_SRC_DEST,
                                     JUCE_INCREMENT_SRC_DEST, const Mode::ParallelType mult = Mode::load1(multiplier);)
#endif
    }

    void JUCE_CALLTYPE FloatVectorOperations::addWithMultiply(double* dest, const double* src, double multiplier,
                                                              int num) noexcept
    {
#if JUCE_USE_VDSP_FRAMEWORK
    ...
    void JUCE_CALLTYPE FloatVectorOperations::addWithMultiply(float* dest, const float* src1, const float* src2,
                                                              int num) noexcept
    {
#if JUCE_USE_VDSP_FRAMEWORK
    ...
    void JUCE_CALLTYPE FloatVectorOperations::addWithMultiply(double* dest, const double* src1, const double* src2,
                                                              int num) noexcept
    {
#if JUCE_USE_VDSP_FRAMEWORK
    ...
    void JUCE_CALLTYPE FloatVectorOperations::subtractWithMultiply(float* dest, const float* src, float multiplier,
                                                                   int num) noexcept
    {
        JUCE_PERFORM_VEC_OP_SRC_DEST(dest[i] -= src[i] * multiplier, Mode::sub(d, Mode::mul(mult, s)), JUCE_LOAD_SRC_DEST,
                                     JUCE_INCREMENT_SRC_DEST, const Mode::ParallelType mult = Mode::load1(multiplier);)
    }

    void JUCE_CALLTYPE FloatVectorOperations::subtractWithMultiply(double* dest, const double* src, double multiplier,
                                                                   int num) noexcept
    {
        JUCE_PERFORM_VEC_OP_SRC_DEST(dest[i] -= src[i] * multiplier, Mode::sub(d, Mode::mul(mult, s)), JUCE_LOAD_SRC_DEST,
                                     JUCE_INCREMENT_SRC_DEST, const Mode::ParallelType mult = Mode::load1(multiplier);)
    ...
    void JUCE_CALLTYPE FloatVectorOperations::subtractWithMultiply(float* dest, const float* src1, const float* src2,
                                                                   int num) noexcept
    {
    ...
    void JUCE_CALLTYPE FloatVectorOperations::subtractWithMultiply(double* dest, const double* src1, const double* src2,
                                                                   int num) noexcept
    {
        JUCE_PERFORM_VEC_OP_SRC1_SRC2_DEST_DEST(dest[i] -= src1[i] * src2[i], Mode::sub(d, Mode::mul(s1, s2)),
                                                JUCE_LOAD_SRC1_SRC2_DEST, JUCE_INCREMENT_SRC1_SRC2_DEST, )
...
#else
        JUCE_PERFORM_VEC_OP_SRC1_SRC2_DEST(dest[i] = src1[i] * src2[i], Mode::mul(s1, s2), JUCE_LOAD_SRC1_SRC2,
                                           JUCE_INCREMENT_SRC1_SRC2_DEST, )
#endif
    }
    ...
    void FloatVectorOperations::abs(float* dest, const float* src, int num) noexcept
    {
        ...
        JUCE_PERFORM_VEC_OP_SRC_DEST(dest[i] = std::abs(src[i]), Mode::bit_and(s, mask), JUCE_LOAD_SRC,
                                     JUCE_INCREMENT_SRC_DEST, const Mode::ParallelType mask = Mode::load1(signMask.f);)
    ...
    void FloatVectorOperations::abs(double* dest, const double* src, int num) noexcept
    {
        ...
        JUCE_PERFORM_VEC_OP_SRC_DEST(dest[i] = std::abs(src[i]), Mode::bit_and(s, mask), JUCE_LOAD_SRC,
                                     JUCE_INCREMENT_SRC_DEST, const Mode::ParallelType mask = Mode::load1(signMask.d);)

        ignoreUnused(signMask);
#endif
    }

    void JUCE_CALLTYPE FloatVectorOperations::convertFixedToFloat(float* dest, const int* src, float multiplier,
                                                                  int num) noexcept
    {
#if JUCE_USE_ARM_NEON
        JUCE_PERFORM_VEC_OP_SRC_DEST(dest[i] = src[i] * multiplier, vmulq_n_f32(vcvtq_f32_s32(vld1q_s32(src)), multiplier),
                                     JUCE_LOAD_NONE, JUCE_INCREMENT_SRC_DEST, )
#else
        JUCE_PERFORM_VEC_OP_SRC_DEST(dest[i] = (float)src[i] * multiplier,
                                     Mode::mul(mult, _mm_cvtepi32_ps(_mm_loadu_si128((const __m128i*)src))), JUCE_LOAD_NONE,
                                     JUCE_INCREMENT_SRC_DEST, const Mode::ParallelType mult = Mode::load1(multiplier);)
#endif
    }
    ...
    {
        JUCE_PERFORM_VEC_OP_SRC_DEST(dest[i] = jmin(src[i], comp), Mode::min(s, cmp), JUCE_LOAD_SRC, JUCE_INCREMENT_SRC_DEST,
...
#else
        JUCE_PERFORM_VEC_OP_SRC1_SRC2_DEST(dest[i] = jmin(src1[i], src2[i]), Mode::min(s1, s2), JUCE_LOAD_SRC1_SRC2,
                                           JUCE_INCREMENT_SRC1_SRC2_DEST, )
#endif
    }
    ...
    {
        JUCE_PERFORM_VEC_OP_SRC_DEST(dest[i] = jmax(src[i], comp), Mode::max(s, cmp), JUCE_LOAD_SRC, JUCE_INCREMENT_SRC_DEST,
                                     const Mode::ParallelType cmp = Mode::load1(comp);)
    }
...
#else
        JUCE_PERFORM_VEC_OP_SRC1_SRC2_DEST(dest[i] = jmax(src1[i], src2[i]), Mode::max(s1, s2), JUCE_LOAD_SRC1_SRC2,
                                           JUCE_INCREMENT_SRC1_SRC2_DEST, )
#endif
    }
...
#else
        JUCE_PERFORM_VEC_OP_SRC_DEST(dest[i] = jmax(jmin(src[i], high), low), Mode::max(Mode::min(s, hi), lo), JUCE_LOAD_SRC,
                                     JUCE_INCREMENT_SRC_DEST, const Mode::ParallelType lo = Mode::load1(low);
...
//==============================================================================
    ...
    public:
        FloatVectorOperationsTests()
            : UnitTest("FloatVectorOperations", "Audio")
        {
        }
        ...
        struct TestRunner
        {
            ...
            {
                while (--num >= 0)
                    if (!valuesMatch(*d1++, *d2++)) return false;
