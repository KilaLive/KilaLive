...
namespace juce
{
    ...
    */
    ...
    public:
        ...
        */
            ...
            , allocatedBytes(other.allocatedBytes)
        {
            ...
            else
            {
                ...
                {
                    for (int i = 0; i < numChannels; ++i) FloatVectorOperations::copy(channels[i], other.channels[i], size);
                }
            }
        }
        ...
        */
            ...
            , isClear(other.isClear)
        {
            if (numChannels < (int)numElementsInArray(preallocatedChannelSpace))
            {
                ...
                for (int i = 0; i < numChannels; ++i) preallocatedChannelSpace[i] = other.channels[i];
            }
            else
            {
        ...
        */
        void setSize(int newNumChannels, int newNumSamples, bool keepExistingContent = false, bool clearExtraSpace = false,
        ...
        */
        ...
        void makeCopyOf(const AudioBuffer<OtherType>& other, bool avoidReallocating = false)
        {
            ...
            else
            {
                ...
                for (int chan = 0; chan < numChannels; ++chan)
                {
                    ...
                    for (int i = 0; i < size; ++i) dest[i] = static_cast<Type>(src[i]);
                }
            }
        }
        ...
        void clear() noexcept
        {
            if (!isClear)
            {
                for (int i = 0; i < numChannels; ++i) FloatVectorOperations::clear(channels[i], size);
        ...
        */
        void clear(int startSample, int numSamples) noexcept
        {
            ...
            if (!isClear)
            {
                if (startSample == 0 && numSamples == size) isClear = true;

                for (int i = 0; i < numChannels; ++i) FloatVectorOperations::clear(channels[i] + startSample, numSamples);
            }
        }
        ...
        */
        ...
        {
            for (int i = 0; i < numChannels; ++i) applyGain(i, startSample, numSamples, gain);
        }
        ...
        */
        ...
        {
            for (int i = 0; i < numChannels; ++i) applyGainRamp(i, startSample, numSamples, startGain, endGain);
        }
        ...
        /** Adds samples from another buffer to this one.
            ...
            @see copyFrom
        */
        void addFrom(int destChannel, int destStartSample, const AudioBuffer& source, int sourceChannel,
                     int sourceStartSample, int numSamples, Type gainToApplyToSource = Type(1)) noexcept
        {
        ...
        /** Adds samples from an array of floats to one of the channels.
            ...
            @see copyFrom
        */
        void addFrom(int destChannel, int destStartSample, const Type* source, int numSamples,
        ...
        */
        void addFromWithRamp(int destChannel, int destStartSample, const Type* source, int numSamples, Type startGain,
        ...
        */
        void copyFrom(int destChannel, int destStartSample, const AudioBuffer& source, int sourceChannel,
                      int sourceStartSample, int numSamples) noexcept
        {
            jassert(&source != this || sourceChannel != destChannel);
            jassert(isPositiveAndBelow(destChannel, numChannels));
            ...
            if (numSamples > 0)
            {
                if (source.isClear)
                {
                    if (!isClear) FloatVectorOperations::clear(channels[destChannel] + destStartSample, numSamples);
                }
                else
                {
                    isClear = false;
        ...
        */
        ...
        /** Copies samples from an array of floats into one of the channels, applying a gain ramp.
            ...
            @see addFrom
        */
        void copyFromWithRamp(int destChannel, int destStartSample, const Type* source, int numSamples, Type startGain,
                              Type endGain) noexcept
        {
            if (startGain == endGain)
            {
        ...
        */
        Range<Type> findMinMax(int channel, int startSample, int numSamples) const noexcept
        {
            ...
            if (isClear) return {Type(0), Type(0)};
        ...
        Type getMagnitude(int startSample, int numSamples) const noexcept
        {
            ...
            if (!isClear)
                for (int i = 0; i < numChannels; ++i) mag = jmax(mag, getMagnitude(i, startSample, numSamples));
        ...
        void reverse(int channel, int startSample, int numSamples) const noexcept
        {
            ...
            if (!isClear) std::reverse(channels[channel] + startSample, channels[channel] + startSample + numSamples);
        }

        /** Reverses a part of the buffer. */
        void reverse(int startSample, int numSamples) const noexcept
        {
            for (int i = 0; i < numChannels; ++i) reverse(i, startSample, numSamples);
        }
