...
namespace juce
{
    ...
    void MPESynthesiser::noteAdded(MPENote newNote)
    {
        ...
        if (auto* voice = findFreeVoice(newNote, shouldStealVoices)) startVoice(voice, newNote);
    }
    ...
    void MPESynthesiser::noteReleased(MPENote finishedNote)
    {
        ...
        for (int i = voices.size(); --i >= 0;)
        {
            ...
            if (voice->isCurrentlyPlayingNote(finishedNote)) stopVoice(voice, finishedNote, true);
        }
    }
    ...
    void MPESynthesiser::setCurrentPlaybackSampleRate(const double newRate)
    {
        ...
        for (int i = voices.size(); --i >= 0;) voices.getUnchecked(i)->setCurrentSampleRate(newRate);
    }
    ...
    MPESynthesiserVoice* MPESynthesiser::findFreeVoice(MPENote noteToFindVoiceFor, bool stealIfNoneAvailable) const
    {
        ...
        if (stealIfNoneAvailable) return findVoiceToSteal(noteToFindVoiceFor);
    ...
    MPESynthesiserVoice* MPESynthesiser::findVoiceToSteal(MPENote noteToStealVoiceFor) const
    {
        ...
        for (auto* voice : voices)
        {
            ...
            struct Sorter
            {
                bool operator()(const MPESynthesiserVoice* a, const MPESynthesiserVoice* b) const noexcept
                {
            ...
            if (!voice->isPlayingButReleased())  // Don't protect released notes
            {
                ...
                if (low == nullptr || noteNumber < low->getCurrentlyPlayingNote().initialNote) low = voice;

                if (top == nullptr || noteNumber > top->getCurrentlyPlayingNote().initialNote) top = voice;
            }
        }
        ...
        if (noteToStealVoiceFor.isValid())
            for (auto* voice : usableVoices)
                if (voice->getCurrentlyPlayingNote().initialNote == noteToStealVoiceFor.initialNote) return voice;
    ...
    void MPESynthesiser::turnOffAllVoices(bool allowTailOff)
    {
        ...
        for (auto* voice : voices) voice->noteStopped(allowTailOff);
    ...
    void MPESynthesiser::renderNextSubBlock(AudioBuffer<float>& buffer, int startSample, int numSamples)
    {
        for (auto* voice : voices)
        {
            if (voice->isActive()) voice->renderNextBlock(buffer, startSample, numSamples);
        }
    }
