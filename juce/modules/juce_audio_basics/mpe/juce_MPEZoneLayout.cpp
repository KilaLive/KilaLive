...
namespace juce
{
    ...
    bool MPEZoneLayout::addZone(MPEZone newZone)
    {
        ...
        for (int i = zones.size(); --i >= 0;)
        {
            ...
            {
                if (!zone.truncateToFit(newZone)) zones.removeRange(i, 1);
    ...
    {
        if (zones.size() < index) return nullptr;
    ...
    void MPEZoneLayout::processNextMidiEvent(const MidiMessage& message)
    {
        ...
        if (rpnDetector.parseControllerMessage(message.getChannel(), message.getControllerNumber(),
                                               message.getControllerValue(), rpn))
        {
    ...
    void MPEZoneLayout::processNextMidiBuffer(const MidiBuffer& buffer)
    {
        ...
        while (iter.getNextEvent(message, samplePosition)) processNextMidiEvent(message);
    }
    ...
    {
        for (auto& zone : zones)
            if (zone.isUsingChannel(channel)) return &zone;
    ...
    {
        for (auto& zone : zones)
            if (zone.getMasterChannel() == channel) return &zone;
    ...
    {
        for (auto& zone : zones)
            if (zone.getFirstNoteChannel() == channel) return &zone;
    ...
    {
        for (auto& zone : zones)
            if (zone.isUsingChannelAsNoteChannel(channel)) return &zone;
...
//==============================================================================
    ...
    public:
        MPEZoneLayoutTests()
            : UnitTest("MPEZoneLayout class", "MIDI/MPE")
        {
        }
