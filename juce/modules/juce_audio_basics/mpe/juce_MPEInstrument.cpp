...
namespace juce
{
    ...
    void MPEInstrument::processNextMidiEvent(const MidiMessage& message)
    {
        ...
        if (message.isNoteOn(true))
            ...
            processMidiAllNotesOffMessage(message);
    ...
    void MPEInstrument::processMidiNoteOnMessage(const MidiMessage& message)
    {
        ...
        {
            noteOff(message.getChannel(), message.getNoteNumber(), MPEValue::from7BitInt(64));
        }
        else
        {
            noteOn(message.getChannel(), message.getNoteNumber(), MPEValue::from7BitInt(message.getVelocity()));
        }
    }
    ...
    {
        noteOff(message.getChannel(), message.getNoteNumber(), MPEValue::from7BitInt(message.getVelocity()));
    }

    //==============================================================================
    void MPEInstrument::processMidiPitchWheelMessage(const MidiMessage& message)
    {
        pitchbend(message.getChannel(), MPEValue::from14BitInt(message.getPitchWheelValue()));
    }

    //==============================================================================
    void MPEInstrument::processMidiChannelPressureMessage(const MidiMessage& message)
    {
        pressure(message.getChannel(), MPEValue::from7BitInt(message.getChannelPressureValue()));
    }
    ...
    void MPEInstrument::handlePressureMSB(int midiChannel, int value) noexcept
    {
        ...
        pressure(midiChannel,
                 lsb == noLSBValueReceived ? MPEValue::from7BitInt(value) : MPEValue::from14BitInt(lsb + (value << 7)));
    }
    ...
    //==============================================================================
    void MPEInstrument::noteOn(int midiChannel, int midiNoteNumber, MPEValue midiNoteOnVelocity)
    {
        if (!isNoteChannel(midiChannel)) return;
    ...
    //==============================================================================
    void MPEInstrument::noteOff(int midiChannel, int midiNoteNumber, MPEValue midiNoteOffVelocity)
    {
        if (notes.isEmpty() || !isNoteChannel(midiChannel)) return;
        ...
        else if (isNoteChannel(midiChannel))
        {
            if (dimension.trackingMode == allNotesOnChannel)
            {
                for (auto i = notes.size(); --i >= 0;)
                {
                    ...
                    if (note.midiChannel == midiChannel) updateDimensionForNote(note, dimension, value);
                }
            }
            else
            {
    ...
    void MPEInstrument::updateDimensionMaster(const MPEZone& zone, MPEDimension& dimension, MPEValue value)
    {
        ...
        for (auto i = notes.size(); --i >= 0;)
        {
            ...
            if (!channels.contains(note.midiChannel)) continue;
    ...
    void MPEInstrument::updateDimensionForNote(MPENote& note, MPEDimension& dimension, MPEValue value)
    {
        if (dimension.getValue(note) != value)
        {
            ...
            if (&dimension == &pitchbendDimension) updateNoteTotalPitchbend(note);
        ...
        else
        {
            if (auto* zone = zoneLayout.getZoneByNoteChannel(note.midiChannel))
            {
                ...
                auto masterPitchbendInSemitones =
                    pitchbendDimension.lastValueReceivedOnChannel[zone->getMasterChannel() - 1].asSignedFloat()
    ...
    void MPEInstrument::handleSustainOrSostenuto(int midiChannel, bool isDown, bool isSostenuto)
    {
        ...
        if (legacyMode.isEnabled ? (!legacyMode.channelRange.contains(midiChannel)) : (affectedZone == nullptr)) return;
    ...
    {
        if (legacyMode.isEnabled) return legacyMode.channelRange.contains(midiChannel);
    ...
    {
        if (legacyMode.isEnabled) return false;
    ...
    {
        if (auto* note = getNotePtr(midiChannel, midiNoteNumber)) return *note;
    ...
    {
        if (auto* note = getLastNotePlayedPtr(midiChannel)) return *note;
    ...
    MPENote MPEInstrument::getMostRecentNoteOtherThan(MPENote otherThanThisNote) const noexcept
    {
        for (auto i = notes.size(); --i >= 0;)
        {
            ...
            if (note != otherThanThisNote) return note;
        }
    ...
    const MPENote* MPEInstrument::getNotePtr(int midiChannel, int midiNoteNumber) const noexcept
    {
        for (int i = 0; i < notes.size(); ++i)
        {
            ...
            if (note.midiChannel == midiChannel && note.initialNote == midiNoteNumber) return &note;
        }
...
//==============================================================================
    ...
    public:
        ...
        void runTest() override
        {
            ...
            {
                MPENote testNote(3, 60, MPEValue::centreValue(), MPEValue::centreValue(), MPEValue::centreValue(),
            ...
            }
            {
                ...
                while (iter.getNextEvent(message, samplePosition)) test.processNextMidiEvent(message);
...
public:
    ...
    int noteOnCallCounter, noteOffCallCounter, pitchbendCallCounter, pressureCallCounter, timbreCallCounter,
        sustainPedalCallCounter, sostenutoPedalCallCounter, noteAddedCallCounter, notePressureChangedCallCounter,
        notePitchbendChangedCallCounter, noteTimbreChangedCallCounter, noteKeyStateChangedCallCounter,
...
//==============================================================================
void expectNote(MPENote noteToTest, int noteOnVelocity7Bit, int pressure7Bit, int pitchbend14Bit, int timbre7Bit,
...
void expectHasFinishedNote(const UnitTestInstrument& test, int channel, int noteNumber, int noteOffVelocity7Bit)
{
