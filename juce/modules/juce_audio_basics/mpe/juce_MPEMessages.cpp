...
namespace juce
{
    ...
    {
        auto buffer = MidiRPNGenerator::generate(zone.getFirstNoteChannel(), zoneLayoutMessagesRpnNumber,
                                                 zone.getNumNoteChannels(), false, false);
    ...
    {
        return MidiRPNGenerator::generate(zone.getFirstNoteChannel(), 0, zone.getPerNotePitchbendRange(), false, false);
    }

    MidiBuffer MPEMessages::masterPitchbendRange(MPEZone zone)
    {
        return MidiRPNGenerator::generate(zone.getMasterChannel(), 0, zone.getMasterPitchbendRange(), false, false);
    }
    ...
    MidiBuffer MPEMessages::setZoneLayout(const MPEZoneLayout& layout)
    {
        ...
        for (int i = 0; i < layout.getNumZones(); ++i) buffer.addEvents(addZone(*layout.getZoneByIndex(i)), 0, -1, 0);
...
//==============================================================================
    ...
    public:
        MPEMessagesTests()
            : UnitTest("MPEMessages class", "MIDI/MPE")
        {
        }
    ...
    private:
        ...
        void extractRawBinaryData(const MidiBuffer& midiBuffer, const uint8* bufferToCopyTo, std::size_t maxBytes)
        {
            ...
            while (iter.getNextEvent(midiMessage, samplePosition))
            {
                ...
                if (pos + dataSize > maxBytes) return;
