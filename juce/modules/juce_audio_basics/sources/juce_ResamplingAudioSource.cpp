...
namespace juce
{
    ResamplingAudioSource::ResamplingAudioSource(AudioSource* const inputSource, const bool deleteInputWhenDeleted,
                                                 const int channels)
    ...
    void ResamplingAudioSource::getNextAudioBlock(const AudioSourceChannelInfo& info)
    {
        ...
        for (int m = info.numSamples; --m >= 0;)
        {
            ...
            for (int channel = 0; channel < channelsToProcess; ++channel)
                *destBuffers[channel]++ =
                    srcBuffers[channel][bufferPos] + alpha * (srcBuffers[channel][nextPos] - srcBuffers[channel][bufferPos]);
        ...
        {
            // if the filter's not currently being applied, keep it stoked with the last couple of samples to avoid
            // discontinuities
            for (int i = channelsToProcess; --i >= 0;)
            {
    ...
    {
        if (filterStates != nullptr) filterStates.clear((size_t)numChannels);
    }
    ...
    void ResamplingAudioSource::applyFilter(float* samples, int num, FilterState& fs)
    {
        while (--num >= 0)
        {
            ...
            double out = coefficients[0] * in + coefficients[1] * fs.x1 + coefficients[2] * fs.x2 - coefficients[4] * fs.y1
