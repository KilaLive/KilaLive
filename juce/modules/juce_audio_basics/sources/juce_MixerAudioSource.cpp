...
namespace juce
{
    ...
    void MixerAudioSource::addInputSource(AudioSource* input, const bool deleteWhenRemoved)
    {
        if (input != nullptr && !inputs.contains(input))
        {
            ...
            if (localRate > 0.0) input->prepareToPlay(localBufferSize, localRate);
    ...
    void MixerAudioSource::removeInputSource(AudioSource* const input)
    {
        if (input != nullptr)
        {
...

            {
                ...
                if (inputsToDelete[index]) toDelete.reset(input);
    ...
    void MixerAudioSource::removeAllInputs()
    {
...

        {
            ...
            for (int i = inputs.size(); --i >= 0;)
                if (inputsToDelete[i]) toDelete.add(inputs.getUnchecked(i));

            inputs.clear();
        }

        for (int i = toDelete.size(); --i >= 0;) toDelete.getUnchecked(i)->releaseResources();
    }
    ...
    void MixerAudioSource::prepareToPlay(int samplesPerBlockExpected, double sampleRate)
    {
        ...
        for (int i = inputs.size(); --i >= 0;) inputs.getUnchecked(i)->prepareToPlay(samplesPerBlockExpected, sampleRate);
    }
    ...
    void MixerAudioSource::releaseResources()
    {
        ...
        for (int i = inputs.size(); --i >= 0;) inputs.getUnchecked(i)->releaseResources();
    ...
    void MixerAudioSource::getNextAudioBlock(const AudioSourceChannelInfo& info)
    {
        ...
        if (inputs.size() > 0)
        {
            ...
            if (inputs.size() > 1)
            {
                tempBuffer.setSize(jmax(1, info.buffer->getNumChannels()), info.buffer->getNumSamples());
