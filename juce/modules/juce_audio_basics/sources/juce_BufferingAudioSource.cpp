...
namespace juce
{
    BufferingAudioSource::BufferingAudioSource(PositionableAudioSource* s, TimeSliceThread& thread,
                                               const bool deleteSourceWhenDeleted, const int bufferSizeSamples,
                                               const int numChannels, bool prefillBufferOnPrepareToPlay)
    ...
    void BufferingAudioSource::prepareToPlay(int samplesPerBlockExpected, double newSampleRate)
    {
        ...
        if (newSampleRate != sampleRate || bufferSizeNeeded != buffer.getNumSamples() || !isPrepared)
        {
    ...
    void BufferingAudioSource::releaseResources()
    {
        ...
        if (source != this) source->releaseResources();
    }
    ...
    void BufferingAudioSource::getNextAudioBlock(const AudioSourceChannelInfo& info)
    {
        ...
        {
            if (validStart > 0) info.buffer->clear(info.startSample, validStart);  // partial cache miss at start
            ...
            if (validStart < validEnd)
            {
                for (int chan = jmin(numberOfChannels, info.buffer->getNumChannels()); --chan >= 0;)
                {
                    ...
                    {
                        info.buffer->copyFrom(chan, info.startSample + validStart, buffer, chan, startBufferIndex,
                                              validEnd - validStart);
                    }
                    else
                    {
                        ...
                        info.buffer->copyFrom(chan, info.startSample + validStart + initialSize, buffer, chan, 0,
    ...
    {
        if (!source || source->getTotalLength() <= 0) return false;

        if (nextPlayPos + info.numSamples < 0) return true;

        if (!isLooping() && nextPlayPos > getTotalLength()) return true;

        uint32       now       = Time::getMillisecondCounter();
        const uint32 startTime = now;

        uint32 elapsed = (now >= startTime ? now - startTime : (std::numeric_limits<uint32>::max() - startTime) + now);
        ...
        {
            {
                ...
                const int validEnd =
                    static_cast<int>(jlimit(bufferValidStart, bufferValidEnd, nextPlayPos + info.numSamples) - nextPlayPos);

                if (validStart <= 0 && validStart < validEnd && validEnd >= info.numSamples) return true;
            }

            if (elapsed < timeout && (!bufferReadyEvent.wait(static_cast<int>(timeout - elapsed)))) return false;

            now     = Time::getMillisecondCounter();
            elapsed = (now >= startTime ? now - startTime : (std::numeric_limits<uint32>::max() - startTime) + now);
        }
    ...
    int64 BufferingAudioSource::getNextReadPosition() const
    {
        ...
        return (source->isLooping() && nextPlayPos > 0) ? nextPlayPos % source->getTotalLength() : nextPlayPos;
    }
    ...
    bool BufferingAudioSource::readNextBufferChunk()
    {
...

        {
            ...
            else if (std::abs((int)(newBVS - bufferValidStart)) > 512 || std::abs((int)(newBVE - bufferValidEnd)) > 512)
            {
        ...
        if (sectionToReadStart == sectionToReadEnd) return false;
        ...
        else
        {
            ...
            readBufferSection(sectionToReadStart + initialSize, (int)(sectionToReadEnd - sectionToReadStart) - initialSize,
                              0);
        }
    ...
    {
        if (source->getNextReadPosition() != start) source->setNextReadPosition(start);
