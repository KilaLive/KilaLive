...
namespace juce
{
    ...
    {
        if (phasePerSample == 0.0) phasePerSample = MathConstants<double>::twoPi / (sampleRate / frequency);
        ...
        for (int i = 0; i < info.numSamples; ++i)
        {
            ...
            for (int j = info.buffer->getNumChannels(); --j >= 0;) info.buffer->setSample(j, info.startSample + i, sample);
        }
    }
