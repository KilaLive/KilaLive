...
namespace juce
{
        ...
        : isLooping(shouldLoop)
    {
        if (copyMemory)
            ...
            buffer.setDataToReferTo(bufferToUse.getArrayOfWritePointers(), bufferToUse.getNumChannels(),
    ...
    void MemoryAudioSource::getNextAudioBlock(const AudioSourceChannelInfo& bufferToFill)
    {
        ...
        for (auto i = position; (i < n || isLooping) && (pos < m); i += max)
        {
            ...
            int ch = 0;
            for (; ch < channels; ++ch) dst.copyFrom(ch, bufferToFill.startSample + pos, buffer, ch, i % n, max);

            for (; ch < dst.getNumChannels(); ++ch) dst.clear(ch, bufferToFill.startSample + pos, max);

            pos += max;
        }

        if (pos < m) dst.clear(bufferToFill.startSample + pos, m - pos);
    }
