...
namespace juce
{
    namespace MidiBufferHelpers
    {
        ...
        {
            while (d < endData && getEventTime(d) <= samplePosition) d += getEventTotalSize(d);
    ...
    void MidiBuffer::addEvent(const void* const newData, const int maxBytes, const int sampleNumber)
    {
        ...
        if (numBytes > 0)
        {
            ...
            const int    offset =
                (int)(MidiBufferHelpers::findEventAfter(data.begin(), data.end(), sampleNumber) - data.begin());
    ...
    void MidiBuffer::addEvents(const MidiBuffer& otherBuffer, const int startSample, const int numSamples,
                               const int sampleDeltaToAdd)
    {
        ...
        while (i.getNextEvent(eventData, eventSize, position) && (position < startSample + numSamples || numSamples < 0))
        {
    ...
    int MidiBuffer::getNumEvents() const noexcept
    {
        ...
        for (const uint8* d = data.begin(); d < end; ++n) d += MidiBufferHelpers::getEventTotalSize(d);
    ...
    int MidiBuffer::getLastEventTime() const noexcept
    {
        ...
        for (const uint8* d = data.begin();;)
        {
            ...
            if (nextOne >= endData) return MidiBufferHelpers::getEventTime(d);
    ...
    {
        if (data >= buffer.data.end()) return false;
