...
namespace juce
{
    ...
    MidiMessageSequence::MidiEventHolder::MidiEventHolder(MidiMessage&& mm)
        : message(static_cast<MidiMessage&&>(mm))
    {
    }
    ...
    MidiMessageSequence::MidiEventHolder** MidiMessageSequence::begin() const noexcept
    {
        return list.begin();
    }
    MidiMessageSequence::MidiEventHolder** MidiMessageSequence::end() const noexcept
    {
        return list.end();
    }
    ...
    double MidiMessageSequence::getTimeOfMatchingKeyUp(int index) const noexcept
    {
        if (auto* meh = list[index])
            if (meh->noteOffObject != nullptr) return meh->noteOffObject->message.getTimeStamp();
    ...
    {
        if (auto* meh = list[index]) return list.indexOf(meh->noteOffObject);
    ...
    int MidiMessageSequence::getNextIndexAtTime(double timeStamp) const noexcept
    {
        ...
        for (i = 0; i < numEvents; ++i)
            if (list.getUnchecked(i)->message.getTimeStamp() >= timeStamp) break;
    ...
    {
        if (auto* meh = list[index]) return meh->message.getTimeStamp();
    ...
    MidiMessageSequence::MidiEventHolder* MidiMessageSequence::addEvent(MidiEventHolder* newEvent, double timeAdjustment)
    {
        ...
        int i;
        for (i = list.size(); --i >= 0;)
            if (list.getUnchecked(i)->message.getTimeStamp() <= time) break;
    ...
    void MidiMessageSequence::deleteEvent(int index, bool deleteMatchingNoteUp)
    {
        ...
        {
            if (deleteMatchingNoteUp) deleteEvent(getIndexOfMatchingKeyUp(index), false);
    ...
    void MidiMessageSequence::addSequence(const MidiMessageSequence& other, double timeAdjustment, double firstAllowableTime,
    ...
    {
        if (delta != 0)
            for (auto* m : list) m->message.addToTimeStamp(delta);
    }

    //==============================================================================
    void MidiMessageSequence::extractMidiChannelMessages(const int channelNumberToExtract, MidiMessageSequence& destSequence,
                                                         const bool alsoIncludeMetaEvents) const
    {
        for (auto* meh : list)
            if (meh->message.isForChannel(channelNumberToExtract) || (alsoIncludeMetaEvents && meh->message.isMetaEvent()))
    ...
    {
        for (auto* meh : list)
            if (meh->message.isSysEx()) destSequence.addEvent(meh->message);
    }

    void MidiMessageSequence::deleteMidiChannelMessages(const int channelNumberToRemove)
    {
        for (int i = list.size(); --i >= 0;)
            if (list.getUnchecked(i)->message.isForChannel(channelNumberToRemove)) list.remove(i);
    }

    void MidiMessageSequence::deleteSysExMessages()
    {
        for (int i = list.size(); --i >= 0;)
            if (list.getUnchecked(i)->message.isSysEx()) list.remove(i);
    }
    ...
    struct MidiMessageSequenceTest : public juce::UnitTest
    {
        MidiMessageSequenceTest()
            : juce::UnitTest("MidiMessageSequence")
        {
        }
