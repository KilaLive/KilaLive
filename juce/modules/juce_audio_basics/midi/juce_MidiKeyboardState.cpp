...
namespace juce
{
    ...
    bool MidiKeyboardState::isNoteOn(const int midiChannel, const int n) const noexcept
    {
        ...
        return isPositiveAndBelow(n, 128) && (noteStates[n] & (1 << (midiChannel - 1))) != 0;
    }
    ...
    void MidiKeyboardState::allNotesOff(const int midiChannel)
    {
        ...
        if (midiChannel <= 0)
        {
            for (int i = 1; i <= 16; ++i) allNotesOff(i);
        }
        else
        {
            for (int i = 0; i < 128; ++i) noteOff(midiChannel, i, 0.0f);
        }
    }
    ...
    void MidiKeyboardState::processNextMidiEvent(const MidiMessage& message)
    {
        ...
        {
            for (int i = 0; i < 128; ++i) noteOffInternal(message.getChannel(), i, 0.0f);
        }
    }

    void MidiKeyboardState::processNextMidiBuffer(MidiBuffer& buffer, const int startSample, const int numSamples,
