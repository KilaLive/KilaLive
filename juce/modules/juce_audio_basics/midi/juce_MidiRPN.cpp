...
namespace juce
{
    ...
    bool MidiRPNDetector::parseControllerMessage(int midiChannel, int controllerNumber, int controllerValue,
    ...
    bool MidiRPNDetector::ChannelState::handleController(int channel, int controllerNumber, int value,
                                                         MidiRPNMessage& result) noexcept
    {
    ...
    {
        return generate(message.channel, message.parameterNumber, message.value, message.isNRPN, message.is14BitValue);
    }

    MidiBuffer MidiRPNGenerator::generate(int midiChannel, int parameterNumber, int value, bool isNRPN, bool use14BitValue)
    {
        ...
        if (use14BitValue) buffer.addEvent(MidiMessage(channelByte, 0x26, valueLSB), 0);
...
//==============================================================================
    ...
    public:
        MidiRPNDetectorTests()
            : UnitTest("MidiRPNDetector class", "MIDI/MPE")
        {
        }
    ...
    public:
        MidiRPNGeneratorTests()
            : UnitTest("MidiRPNGenerator class", "MIDI/MPE")
        {
        }
    ...
    private:
        //==============================================================================
        void expectContainsRPN(const MidiBuffer& midiBuffer, int channel, int parameterNumber, int value, bool isNRPN,
                               bool is14BitValue)
        {
        ...
        void expectContainsRPN(const MidiBuffer& midiBuffer, MidiRPNMessage expected)
        {
            ...
            {
                if (detector.parseControllerMessage(midiMessage.getChannel(), midiMessage.getControllerNumber(),
                                                    midiMessage.getControllerValue(), result))
                    break;
            }
