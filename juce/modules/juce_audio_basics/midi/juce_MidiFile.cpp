...
namespace juce
{
    namespace MidiFileHelpers
    {
        ...
        {
            if (timeFormat < 0) return time / (-(timeFormat >> 8) * (timeFormat & 0xff));
            ...
            for (int i = 0; i < numEvents; ++i)
            {
                ...
                if (m.isTempoMetaEvent()) secsPerTick = tickLen * m.getTempoSecondsPerQuarterNote();

                while (i + 1 < numEvents)
                {
                    auto& m2 = tempoEvents.getEventPointer(i + 1)->message;

                    if (m2.getTimeStamp() != eventTime) break;

                    if (m2.isTempoMetaEvent()) secsPerTick = tickLen * m2.getTempoSecondsPerQuarterNote();
        ...
        static void findAllMatchingEvents(const OwnedArray<MidiMessageSequence>& tracks, MidiMessageSequence& results,
                                          MethodType method)
        {
            for (auto* track : tracks)
            {
                ...
                for (int j = 0; j < numEvents; ++j)
                {
                    ...
                    if ((m.*method)()) results.addEvent(m);
                }
            }
        }
    }  // namespace MidiFileHelpers
    ...
    double MidiFile::getLastTimestamp() const
    {
        ...
        for (auto* ms : tracks) t = jmax(t, ms->getEndTime());
        ...
        if (sourceStream.readIntoMemoryBlock(data, maxSensibleMidiFileSize))
        {
            ...
            if (size > 16 && MidiFileHelpers::parseMidiHeader(d, timeFormat, fileType, expectedTracks))
            {
                ...
                while (size > 0 && track < expectedTracks)
                {
                    ...
                    if (chunkType == (int)ByteOrder::bigEndianInt("MTrk")) readNextTrack(d, chunkSize);
    ...
    void MidiFile::readNextTrack(const uint8* data, int size)
    {
        ...
        while (size > 0)
        {
            ...
            if ((firstByte & 0xf0) != 0xf0) lastStatusByte = firstByte;
        }
        ...
        std::stable_sort(result.list.begin(), result.list.end(),
                         [](const MidiMessageSequence::MidiEventHolder* a, const MidiMessageSequence::MidiEventHolder* b) {
    ...
    bool MidiFile::writeTo(OutputStream& out, int midiFileType)
    {
        ...
        for (auto* ms : tracks)
            if (!writeTrack(out, *ms)) return false;
    ...
    bool MidiFile::writeTrack(OutputStream& mainOut, const MidiMessageSequence& ms)
    {
        ...
        for (int i = 0; i < ms.getNumEvents(); ++i)
        {
            ...
            if (mm.isEndOfTrackMetaEvent()) endOfTrackEventWritten = true;
            ...
            if (statusByte == lastStatusByte && (statusByte & 0xf0) != 0xf0 && dataSize > 1 && i > 0)
            {
                ++data;
                --dataSize;
            }
