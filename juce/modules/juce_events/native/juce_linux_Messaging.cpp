...
namespace juce
{
    ...
    public:
        InternalMessageQueue()
        {
            ...
            readCallback[INTERNAL_QUEUE_FD] =
                new LinuxEventLoop::CallbackFunction<decltype(internalQueueCb)>(internalQueueCb);
        }
        ...
        bool dispatchNextEvent() noexcept
        {
            for (int counter = 0; counter < fdCount; counter++)
            {
                ...
                if (readCallback[i] != nullptr && readCallback[i]->active)
                    if ((*readCallback[i])(pfds[i].fd)) return true;
            }
    ...
    namespace LinuxErrorHandling
    {
        ...
        {
            if (sig == SIGINT) keyboardBreakOccurred = true;
        }
    ...
    {
        if (JUCEApplicationBase::isStandaloneApp()) LinuxErrorHandling::installKeyboardBreakHandler();
    ...
    bool MessageManager::dispatchNextMessageOnSystemQueue(bool returnIfNoPendingMessages)
    {
        for (;;)
        {
            if (LinuxErrorHandling::keyboardBreakOccurred) JUCEApplicationBase::getInstance()->quit();

            if (auto* queue = InternalMessageQueue::getInstanceWithoutCreating())
            {
                if (queue->dispatchNextEvent()) break;

                if (returnIfNoPendingMessages) return false;
    ...
    {
        if (auto* queue = InternalMessageQueue::getInstanceWithoutCreating()) queue->setWindowSystemFd(fd, readCallback);
    }

    void LinuxEventLoop::removeWindowSystemFd() noexcept
    {
        if (auto* queue = InternalMessageQueue::getInstanceWithoutCreating()) queue->removeWindowSystemFd();
    }
