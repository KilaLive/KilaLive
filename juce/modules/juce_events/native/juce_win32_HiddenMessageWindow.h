...
namespace juce
{
    ...
    public:
        HiddenMessageWindow(const TCHAR* const messageWindowName, WNDPROC wndProc)
        {
            ...
            hwnd = CreateWindow(getClassNameFromAtom(), messageWindowName, 0, 0, 0, 0, 0, 0, 0, moduleHandle, 0);
            jassert(hwnd != 0);
        }
    ...
    private:
        ...
        static LRESULT CALLBACK deviceChangeEventCallback(HWND h, const UINT message, const WPARAM wParam,
                                                          const LPARAM lParam)
        {
            ...
            {
                ((DeviceChangeDetector*)GetWindowLongPtr(h, GWLP_USERDATA))->triggerAsyncDeviceChangeCallback();
            }
