...
namespace juce
{
    ...
    {
        const unsigned int customMessageID             = WM_USER + 3000;
        ...
        BOOL CALLBACK broadcastEnumWindowProc(HWND hwnd, LPARAM lParam)
        {
            if (hwnd != juce_messageWindowHandle)
            {
                ...
                if (String(windowName) == messageWindowName) reinterpret_cast<Array<HWND>*>(lParam)->add(hwnd);
            }
        ...
        LRESULT CALLBACK messageWndProc(HWND h, const UINT message, const WPARAM wParam, const LPARAM lParam) noexcept
        {
            if (h == juce_messageWindowHandle)
            {
                ...
                {
                    if (settingChangeCallback != nullptr) settingChangeCallback();
                }
            }
    ...
    bool MessageManager::dispatchNextMessageOnSystemQueue(const bool returnIfNoPendingMessages)
    {
        ...
        if (returnIfNoPendingMessages && !PeekMessage(&m, (HWND)0, 0, 0, PM_NOREMOVE)) return false;

        if (GetMessage(&m, (HWND)0, 0, 0) >= 0)
        {
#if JUCE_MODULE_AVAILABLE_juce_gui_extra
            if (juce_offerEventToActiveXControl(m) != S_FALSE) return true;
#endif
            ...
            else if (isEventBlockedByModalComps == nullptr || !isEventBlockedByModalComps(m))
            {
                    ...
                    && !JuceWindowIdentifier::isJUCEWindow(m.hwnd))
                {
                    ...
                    if (currentFocus == 0 || JuceWindowIdentifier::isJUCEWindow(currentFocus)) SetFocus(m.hwnd);
                }
    ...
    void MessageManager::broadcastMessage(const String& value)
    {
        ...
        for (int i = windows.size(); --i >= 0;)
        {
            ...
            DWORD_PTR result;
            SendMessageTimeout(windows.getUnchecked(i), WM_COPYDATA, (WPARAM)juce_messageWindowHandle, (LPARAM)&data,
    ...
    struct MountedVolumeListChangeDetector::Pimpl : private DeviceChangeDetector
    {
        Pimpl(MountedVolumeListChangeDetector& d)
            : DeviceChangeDetector(L"MountedVolumeList")
            , owner(d)
        {
