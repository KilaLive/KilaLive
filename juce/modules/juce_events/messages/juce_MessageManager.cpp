...
namespace juce
{
    ...
    {
        if (JUCEApplicationBase::isStandaloneApp()) Thread::setCurrentThreadName("JUCE Message Thread");
    }
...
//==============================================================================
    ...
    bool MessageManager::runDispatchLoopUntil(int millisecondsToRunFor)
    {
        ...
        while (quitMessageReceived.get() == 0)
        {
            JUCE_TRY
            {
                if (!dispatchNextMessageOnSystemQueue(millisecondsToRunFor >= 0)) Thread::sleep(1);
            }
            JUCE_CATCH_EXCEPTION

            if (millisecondsToRunFor >= 0 && Time::currentTimeMillis() >= endTime) break;
        }
    ...
    public:
        ...
        {
            if (auto* mm = MessageManager::instance) mm->quitMessageReceived = true;
        }
    ...
    void MessageManager::runDispatchLoop()
    {
        ...
        while (quitMessageReceived.get() == 0)
        {
            JUCE_TRY
            {
                if (!dispatchNextMessageOnSystemQueue(false)) Thread::sleep(1);
            }
            JUCE_CATCH_EXCEPTION
        }
    }
    ...
    {
        if (isThisTheMessageThread()) return func(parameter);
    ...
    {
        if (broadcaster != nullptr) broadcaster->sendActionMessage(value);
    }

    void MessageManager::registerBroadcastListener(ActionListener* const listener)
    {
        if (broadcaster == nullptr) broadcaster.reset(new ActionBroadcaster());
    ...
    {
        if (broadcaster != nullptr) broadcaster->removeActionListener(listener);
    }
    ...
    */
        ...
        {
            {
                ...
                if (auto* o = owner.get()) o->messageCallback();
            }
    ...
    bool MessageManager::Lock::tryAcquire(bool lockIsMandatory) const noexcept
    {
        ...
        if (mm->currentThreadHasLockedMessageManager()) return true;
        ...
        do
        {
            while (abortWait.get() == 0) lockedEvent.wait(-1);
    ...
    void MessageManager::Lock::exit() const noexcept
    {
        if (lockGained.compareAndSetBool(false, true))
        {
            ...
            if (mm != nullptr) mm->threadWithLock = 0;
    ...
    bool MessageManagerLock::attemptLock(Thread* threadToCheck, ThreadPoolJob* jobToCheck)
    {
        ...
        if (threadToCheck != nullptr) threadToCheck->addListener(this);

        if (jobToCheck != nullptr) jobToCheck->addListener(this);
        ...
        if (threadToCheck != nullptr)
        {
            ...
            if (threadToCheck->threadShouldExit()) return false;
        }
        ...
        if (jobToCheck != nullptr)
        {
            ...
            if (jobToCheck->shouldExit()) return false;
        }
