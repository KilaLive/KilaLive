...
#else
#define START_JUCE_APPLICATION(AppClass)                                                                                     \
    static_assert(false, "You are trying to use START_JUCE_APPLICATION in an audio plug-in. Define "                         \
                         "JUCE_USE_CUSTOM_PLUGIN_STANDALONE_APP=1 if you want to use a custom standalone target app.");
#if JUCE_IOS
#define START_JUCE_APPLICATION_WITH_CUSTOM_DELEGATE(AppClass, DelegateClass)                                                 \
    static_assert(false, "You are trying to use START_JUCE_APPLICATION in an audio plug-in. Define "                         \
                         "JUCE_USE_CUSTOM_PLUGIN_STANDALONE_APP=1 if you want to use a custom standalone target app.");
...
#define START_JUCE_APPLICATION(AppClass)                                                                                     \
    ...
    /**
       ...
       {
           self = [super init];
           juceDelegate = reinterpret_cast<NSObject<UIApplicationDelegate>*> ([[NSClassFromString (@"JuceAppStartupDelegate")
