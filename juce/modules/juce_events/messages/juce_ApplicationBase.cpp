...
namespace juce
{
    ...
    void JUCEApplicationBase::sendUnhandledException(const std::exception* const e, const char* const sourceFile,
...
#define JUCE_HANDLE_MULTIPLE_INSTANCES 1
    ...
    struct JUCEApplicationBase::MultipleInstanceHandler : public ActionListener
    {
        ...
        {
            if (appLock.enter(0)) return false;
        ...
        void actionListenerCallback(const String& message) override
        {
            if (auto* app = JUCEApplicationBase::getInstance())
            {
                ...
                if (message.startsWith(appName + "/")) app->anotherInstanceStarted(message.substring(appName.length() + 1));
            }
        }
...
//==============================================================================
    ...
    {
        return CharacterFunctions::findEndOfToken(CharPointer_UTF16(GetCommandLineW()), CharPointer_UTF16(L" "),
                                                  CharPointer_UTF16(L"\""))
            .findEndOfWhitespace();
    }
...
#if JUCE_LINUX && JUCE_MODULE_AVAILABLE_juce_gui_extra && (!defined(JUCE_WEB_BROWSER) || JUCE_WEB_BROWSER)
            if (argc >= 2 && String(argv[1]) == "--juce-gtkwebkitfork-child") return juce_gtkWebkitMain(argc, argv);
#endif
    ...
    int JUCEApplicationBase::main()
    {
        ...
        if (!app->initialiseApp()) return app->shutdownApp();
    ...
    bool JUCEApplicationBase::initialiseApp()
    {
        ...
        if (MessageManager::getInstance()->hasStopMessageBeenSent()) return false;

#if JUCE_HANDLE_MULTIPLE_INSTANCES
        if (auto* mih = multipleInstanceHandler.get()) MessageManager::getInstance()->registerBroadcastListener(mih);
#endif
...
#if JUCE_HANDLE_MULTIPLE_INSTANCES
        if (auto* mih = multipleInstanceHandler.get()) MessageManager::getInstance()->deregisterBroadcastListener(mih);
#endif
