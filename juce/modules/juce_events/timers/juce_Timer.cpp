...
namespace juce
{
    class Timer::TimerThread : private Thread, private DeletedAtShutdown, private AsyncUpdater
    {
    public:
        ...
        void run() override
        {
            ...
            while (!threadShouldExit())
            {
                ...
                auto elapsed =
                    (int)(now >= lastTime ? (now - lastTime) : (std::numeric_limits<uint32>::max() - (lastTime - now)));
                lastTime = now;
        ...
        void callTimers()
        {
            ...
            while (!timers.empty())
            {
                ...
                if (Time::getMillisecondCounter() > timeout) break;
            }
        ...
        {
            if (instance == nullptr) instance = new TimerThread();
        ...
        {
            if (instance != nullptr) instance->removeTimer(tim);
        }

        static inline void resetCounter(Timer* tim) noexcept
        {
            if (instance != nullptr) instance->resetTimerCounter(tim);
        }
    ...
    private:
        ...
        struct CallTimersMessage : public MessageManager::MessageBase
        {
            ...
            {
                if (instance != nullptr) instance->callTimers();
            }
        };
        ...
        void addTimer(Timer* t)
        {
            ...
            jassert(std::find_if(timers.begin(), timers.end(), [t](TimerCountdown i) { return i.timer == t; })
                    == timers.end());
        ...
        void shuffleTimerBackInQueue(size_t pos)
        {
            ...
            if (pos < numTimers - 1)
            {
                ...
                for (;;)
                {
                    ...
                    if (next == numTimers || timers[next].countdownMs >= t.countdownMs) break;
        ...
        void shuffleTimerForwardInQueue(size_t pos)
        {
            if (pos > 0)
            {
                ...
                while (pos > 0)
                {
                    ...
                    if (prev.countdownMs <= t.countdownMs) break;
        ...
        int getTimeUntilFirstTimer(int numMillisecsElapsed)
        {
            ...
            for (auto& t : timers) t.countdownMs -= numMillisecsElapsed;
