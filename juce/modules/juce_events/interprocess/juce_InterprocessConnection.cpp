...
namespace juce
{
    ...
    //==============================================================================
    bool InterprocessConnection::connectToSocket(const String& hostName, const int portNumber, const int timeOutMillisecs)
    {
        disconnect();
    ...
    {
        {
            ...
            if (pipe == nullptr && socket == nullptr) return {};

            if (socket != nullptr && !socket->isLocal()) return socket->getHostName();
        }
    ...
    int InterprocessConnection::writeData(void* data, int dataSize)
    {
        ...
        if (socket != nullptr) return socket->write(data, dataSize);

        if (pipe != nullptr) return pipe->write(data, dataSize, pipeReceiveMessageTimeout);
    ...
    struct DataDeliveryMessage : public Message
    {
        ...
        void messageCallback() override
        {
            if (auto* ipc = owner.get()) ipc->messageReceived(data);
        }
    ...
    bool InterprocessConnection::readNextMessageInt()
    {
        ...
        if (bytes == sizeof(messageHeader) && ByteOrder::swapIfBigEndian(messageHeader[0]) == magicMessageHeader)
        {
            ...
            if (bytesInMessage > 0)
            {
                ...
                while (bytesInMessage > 0)
                {
                    ...
                    const int bytesIn =
                        socket != nullptr ? socket->read(data, numThisTime, true) : pipe->read(data, numThisTime, -1);
                ...
                if (bytesRead >= 0) deliverDataInt(messageData);
            }
        }
        else if (bytes < 0)
        {
            if (socket != nullptr) deletePipeAndSocket();
    ...
    void InterprocessConnection::runThread()
    {
        while (!thread->threadShouldExit())
        {
            ...
            if (thread->threadShouldExit() || !readNextMessageInt()) break;
        }
    }
