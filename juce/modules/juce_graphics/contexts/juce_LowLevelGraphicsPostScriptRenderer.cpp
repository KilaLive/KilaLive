...
#define notPossibleInPostscriptAssert
    ...
    LowLevelGraphicsPostScriptRenderer::LowLevelGraphicsPostScriptRenderer(OutputStream& resultingPostScript,
                                                                           const String& documentTitle, const int totalWidth_,
    ...
    {
        return stateStack.getLast()->clip.intersectsRectangle(
            r.translated(stateStack.getLast()->xOffset, stateStack.getLast()->yOffset));
    }
    ...
    void LowLevelGraphicsPostScriptRenderer::restoreState()
    {
        ...
        if (stateStack.size() > 0) stateStack.removeLast();
    }
    ...
    void LowLevelGraphicsPostScriptRenderer::writeColour(Colour colour)
    {
        ...
        if (lastColour != c)
        {
            ...
            out << String(c.getFloatRed(), 3) << ' ' << String(c.getFloatGreen(), 3) << ' ' << String(c.getFloatBlue(), 3)
                << " c\n";
        }
    }
    ...
    {
        out << "[ " << trans.mat00 << ' ' << trans.mat10 << ' ' << trans.mat01 << ' ' << trans.mat11 << ' ' << trans.mat02
            << ' ' << trans.mat12 << " ] concat ";
    }
    ...
    void LowLevelGraphicsPostScriptRenderer::fillRect(const Rectangle<float>& r)
    {
        if (stateStack.getLast()->fillType.isColour())
        {
            ...
            Rectangle<float> r2(r.translated((float)stateStack.getLast()->xOffset, (float)stateStack.getLast()->yOffset));
    ...
    void LowLevelGraphicsPostScriptRenderer::fillPath(const Path& path, const AffineTransform& t)
    {
        ...
        else if (stateStack.getLast()->fillType.isGradient())
        {
            ...
            notPossibleInPostscriptAssert;  // you can disable this warning by setting the
                                            // WARN_ABOUT_NON_POSTSCRIPT_OPERATIONS flag at the top of this file
            ...
            out << bounds.getX() << ' ' << -bounds.getBottom() << ' ' << bounds.getWidth() << ' ' << bounds.getHeight()
                << " rectfill\n";
    ...
    //==============================================================================
    void LowLevelGraphicsPostScriptRenderer::writeImage(const Image& im, const int sx, const int sy, const int maxW,
    ...
    void LowLevelGraphicsPostScriptRenderer::drawGlyph(int glyphNumber, const AffineTransform& transform)
    {
        ...
        fillPath(
            p, AffineTransform::scale(font.getHeight() * font.getHorizontalScale(), font.getHeight()).followedBy(transform));
    }
