...
#pragma optimize("t", on)
    ...
    //==============================================================================
    PathFlatteningIterator::PathFlatteningIterator(const Path& pathToUse, const AffineTransform& t, float tolerance)
        : x2(0)
        , y2(0)
        , closesSubPath(false)
        , subPathIndex(-1)
    ...
    {
        return stackPos == stackBase.get() && (source == path.data.end() || isMarker(*source, Path::moveMarker));
    }
    ...
    bool PathFlatteningIterator::next()
    {
        ...
        for (;;)
        {
            ...
            {
                if (source == path.data.end()) return false;
                ...
                if (!isMarker(type, Path::closeSubPathMarker))
                {
                    ...
                    if (isMarker(type, Path::quadMarker))
                    {
                        ...
                        if (!isIdentityTransform) transform.transformPoints(x2, y2, x3, y3);
                    }
                    else if (isMarker(type, Path::cubicMarker))
                    {
                        ...
                        if (!isIdentityTransform) transform.transformPoints(x2, y2, x3, y3, x4, y4);
                    }
                    else
                    {
                        if (!isIdentityTransform) transform.transformPoint(x2, y2);
                    }
                }
            }
            else
            ...
            if (isMarker(type, Path::lineMarker))
            {
                ...
                closesSubPath = stackPos == stackBase && source != path.data.end() && *source == Path::closeSubPathMarker
                                && x2 == subPathCloseX && y2 == subPathCloseY;
