...
namespace juce
{
    ...
    */
    ...
    public:
        ...
        */
        Point<ValueType> getPointAlongLine(ValueType distanceFromStart, ValueType perpendicularDistance) const noexcept
        {
            ...
            auto length = juce_hypot((double)delta.x, (double)delta.y);
            if (length <= 0) return start;

            return {
                start.x + static_cast<ValueType>((delta.x * distanceFromStart - delta.y * perpendicularDistance) / length),
        ...
        */
        Point<ValueType> getPointAlongLineProportionally(typename Point<ValueType>::FloatType proportionOfLength) const
            noexcept
        {
        ...
        */
        ValueType getDistanceFromPoint(Point<ValueType> targetPoint, Point<ValueType>& pointOnLine) const noexcept
        {
            ...
            if (length > 0)
            {
                auto prop = ((targetPoint.x - start.x) * delta.x + (targetPoint.y - start.y) * delta.y) / (double)length;
            ...
            return length <= 0 ? 0
                               : jlimit(ValueType(), static_cast<ValueType>(1),
                                        static_cast<ValueType>(
                                            (((point.x - start.x) * delta.x + (point.y - start.y) * delta.y) / length)));
        }
        ...
        */
        ...
        {
            return start.x != end.x && point.y < ((end.y - start.y) * (point.x - start.x)) / (end.x - start.x) + start.y;
        }
    ...
    private:
        ...
        static bool findIntersection(const Point<ValueType> p1, const Point<ValueType> p2, const Point<ValueType> p3,
                                     const Point<ValueType> p4, Point<ValueType>& intersection) noexcept
        {
            if (p2 == p3)
            {
                intersection = p2;
            ...
            if (!isZeroToOne(along1)) return false;
