    ...
    public:
        ...
        bool clipTo(const RectangleList<OtherValueType>& other)
        {
            ...
            for (auto& rect : rects)
            {
                for (auto& r : other)
                {
                    ...
                    if (rect.intersectRectangle(clipped)) result.rects.add(clipped);
                }
            }
        ...
        */
        bool getIntersectionWith(RectangleType rect, RectangleList& destRegion) const
        {
            ...
            if (!rect.isEmpty())
                for (auto& r : rects)
                    if (rect.intersectRectangle(r)) destRegion.rects.add(r);
        ...
        */
        ...
        {
            for (auto& r : rects)
                if (r.intersects(rectangleToCheck)) return true;
        ...
        */
        ...
        {
            for (auto& r : rects)
                if (other.intersectsRectangle(r)) return true;
        ...
        */
        ...
        {
            for (auto& r : rects) r *= scaleFactor;
        }
        ...
        */
        ...
        {
            for (auto& r : rects) r = r.transformedBy(transform);
        }
        ...
        Path toPath() const
        {
            ...
            for (auto& r : rects) p.addRectangle(r);
