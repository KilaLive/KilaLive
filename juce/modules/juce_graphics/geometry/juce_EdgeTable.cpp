...
namespace juce
{
    ...
    EdgeTable::EdgeTable(Rectangle<int> area, const Path& path, const AffineTransform& transform)
        ...
        maxEdgesPerLine(jmax(juce_edgeTableDefaultEdgesPerLine / 2, 4 * (int)std::sqrt(path.data.size())))
        , lineStrideElements(maxEdgesPerLine * 2 + 1)
    {
        ...
        while (iter.next())
        {
            ...
            if (y1 != y2)
            {
                ...
                if (y2 > heightLimit) y2 = heightLimit;
        ...
        , needToCheckEmptiness(true)
    {
        ...
        for (auto& r : rectanglesToAdd)
        {
            ...
            for (int j = r.getHeight(); --j >= 0;) addEdgePointPair(x1, x2, y++, 255);
        }
        ...
        , lineStrideElements(rectanglesToAdd.getNumRectangles() * 4 + 1)
    {
        ...
        for (auto& r : rectanglesToAdd)
        {
            ...
            else
            {
                ...
                while (y < lastLine) addEdgePointPair(x1, x2, y++, 255);
    ...
    EdgeTable::EdgeTable(Rectangle<float> rectangleToAdd)
        : bounds((int)std::floor(rectangleToAdd.getX()), roundToInt(rectangleToAdd.getY() * 256.0f) >> 8,
                 2 + (int)rectangleToAdd.getWidth(), 2 + (int)rectangleToAdd.getHeight())
        , maxEdgesPerLine(juce_edgeTableDefaultEdgesPerLine)
        , lineStrideElements((juce_edgeTableDefaultEdgesPerLine * 2) + 1)
    {
                ...
                while (src < itemsEnd)
                {
                    ...
                    if (corrected >> 8)
                    {
                        ...
                        else
                        {
                            ...
                            if (corrected >> 8) corrected = 511 - corrected;
                        }
                    }
    ...
    void EdgeTable::optimiseTable()
    {
        ...
        for (int i = bounds.getHeight(); --i >= 0;) maxLineElements = jmax(maxLineElements, table[i * lineStrideElements]);
    ...
    void EdgeTable::clipEdgeTableLineToRange(int* dest, const int x1, const int x2) noexcept
    {
        ...
        if (x1 > dest[1])
        {
            while (lastItem[0] > x1) lastItem -= 2;
    ...
    void EdgeTable::clipToRectangle(Rectangle<int> r)
    {
        ...
        else
        {
            ...
            if (bottom < bounds.getHeight()) bounds.setHeight(bottom);

            for (int i = 0; i < top; ++i) table[lineStrideElements * i] = 0;
            ...
            if (clipped.getX() > bounds.getX() || clipped.getRight() < bounds.getRight())
            {
                ...
                {
                    if (line[0] != 0) clipEdgeTableLineToRange(line, x1, x2);
    ...
    void EdgeTable::excludeRectangle(Rectangle<int> r)
    {
        ...
        if (!clipped.isEmpty())
        {
            ...
            const int rectLine[] = {4, std::numeric_limits<int>::min(), 255, clipped.getX() << 8,
                                    0, clipped.getRight() << 8,         255, std::numeric_limits<int>::max(),
                                    0};

            for (int i = top; i < bottom; ++i) intersectWithEdgeTableLine(i, rectLine);
    ...
    void EdgeTable::clipToEdgeTable(const EdgeTable& other)
    {
        ...
        else
        {
            ...
            if (clipped.getRight() < bounds.getRight()) bounds.setRight(clipped.getRight());
    ...
    void EdgeTable::clipLineToMask(int x, int y, const uint8* mask, int maskStride, int numPixels)
    {
        ...
        if (y < 0 || y >= bounds.getHeight()) return;
