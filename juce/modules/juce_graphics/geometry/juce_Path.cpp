...
#define JUCE_CHECK_COORDS_ARE_VALID(x, y) jassert(x == x && y == y);
    ...
    bool Path::isEmpty() const noexcept
    {
        for (auto i = data.begin(), e = data.end(); i != e; ++i)
        {
            ...
            else if (isMarker(type, lineMarker) || isMarker(type, quadMarker) || isMarker(type, cubicMarker))
            {
                return false;
            }
        }
    ...
    void Path::lineTo(const float x, const float y)
    {
        ...
        if (data.isEmpty()) startNewSubPath(0, 0);
    ...
    void Path::quadraticTo(const float x1, const float y1, const float x2, const float y2)
    {
    ...
    void Path::cubicTo(const float x1, const float y1, const float x2, const float y2, const float x3, const float y3)
    {
        JUCE_CHECK_COORDS_ARE_VALID(x1, y1)
        JUCE_CHECK_COORDS_ARE_VALID(x2, y2)
    ...
    {
        if (!(data.isEmpty() || isMarker(data.getLast(), closeSubPathMarker))) data.add(closeSubPathMarker);
    }
    ...
    void Path::addRectangle(float x, float y, float w, float h)
    {
        ...
        data.add(moveMarker, x1, y2, lineMarker, x1, y1, lineMarker, x2, y1, lineMarker, x2, y2, closeSubPathMarker);
    }
    ...
    void Path::addRoundedRectangle(const float x, const float y, const float w, const float h, float csx, float csy,
    ...
    void Path::addArc(float x, float y, float w, float h, float fromRadians, float toRadians, bool startAsNewSubPath)
    {
        auto radiusX = w / 2.0f;
        auto radiusY = h / 2.0f;
    ...
    void Path::addCentredArc(float centreX, float centreY, float radiusX, float radiusY, float rotationOfEllipse,
                             float fromRadians, float toRadians, bool startAsNewSubPath)
    {
        if (radiusX > 0.0f && radiusY > 0.0f)
        {
            ...
            {
                if (startAsNewSubPath) angle += PathHelpers::ellipseAngularIncrement;
    ...
    void Path::addPieSegment(float x, float y, float width, float height, float fromRadians, float toRadians,
    ...
    void Path::addPieSegment(Rectangle<float> segmentBounds, float fromRadians, float toRadians,
                             float innerCircleProportionalSize)
    {
        addPieSegment(segmentBounds.getX(), segmentBounds.getY(), segmentBounds.getWidth(), segmentBounds.getHeight(),
    ...
    void Path::addArrow(Line<float> line, float lineThickness, float arrowheadWidth, float arrowheadLength)
    {
        auto reversed = line.reversed();
        lineThickness *= 0.5f;
        arrowheadWidth *= 0.5f;
    ...
    void Path::addBubble(Rectangle<float> bodyArea, Rectangle<float> maximumArea, Point<float> arrowTip, float cornerSize,
                         float arrowBaseWidth)
    {
        ...
        if (Rectangle<float>(targetLimit.getX(), maximumArea.getY(), targetLimit.getWidth(),
                             bodyArea.getY() - maximumArea.getY())
                .contains(arrowTip))
        {
        ...
        addArc(bodyArea.getRight() - cornerSizeW2, bodyArea.getY(), cornerSizeW2, cornerSizeH2, 0,
               MathConstants<float>::halfPi);

        if (Rectangle<float>(bodyArea.getRight(), targetLimit.getY(), maximumArea.getRight() - bodyArea.getRight(),
                             targetLimit.getHeight())
                .contains(arrowTip))
        {
        ...
        addArc(bodyArea.getRight() - cornerSizeW2, bodyArea.getBottom() - cornerSizeH2, cornerSizeW2, cornerSizeH2,
               MathConstants<float>::halfPi, MathConstants<float>::pi);

        if (Rectangle<float>(targetLimit.getX(), bodyArea.getBottom(), targetLimit.getWidth(),
                             maximumArea.getBottom() - bodyArea.getBottom())
                .contains(arrowTip))
        {
        ...
        addArc(bodyArea.getX(), bodyArea.getBottom() - cornerSizeH2, cornerSizeW2, cornerSizeH2, MathConstants<float>::pi,
               MathConstants<float>::pi * 1.5f);

        if (Rectangle<float>(maximumArea.getX(), targetLimit.getY(), bodyArea.getX() - maximumArea.getX(),
                             targetLimit.getHeight())
                .contains(arrowTip))
        {
        ...
        addArc(bodyArea.getX(), bodyArea.getY(), cornerSizeW2, cornerSizeH2, MathConstants<float>::pi * 1.5f,
               MathConstants<float>::twoPi - 0.05f);
    ...
    {
        return getTransformToScaleToFit(area.getX(), area.getY(), area.getWidth(), area.getHeight(), preserveProportions,
                                        justification);
    }

    AffineTransform Path::getTransformToScaleToFit(float x, float y, float w, float h, bool preserveProportions,
                                                   Justification justification) const
    {
        ...
        {
            if (w <= 0 || h <= 0 || boundsRect.isEmpty()) return AffineTransform();
            ...
            return AffineTransform::translation(boundsRect.getWidth() * -0.5f - boundsRect.getX(),
                ...
                .scaled(newW / boundsRect.getWidth(), newH / boundsRect.getHeight())
                .translated(newXCentre, newYCentre);
        }
        else
        {
            return AffineTransform::translation(-boundsRect.getX(), -boundsRect.getY())
                .scaled(w / boundsRect.getWidth(), h / boundsRect.getHeight())
                .translated(x, y);
        }
    }
    ...
    {
        if (x <= bounds.pathXMin || x >= bounds.pathXMax || y <= bounds.pathYMin || y >= bounds.pathYMax) return false;
    ...
    bool Path::intersectsLine(Line<float> line, float tolerance)
    {
        ...
        while (i.next())
            if (line.intersects(Line<float>(i.x1, i.y1, i.x2, i.y2), intersection)) return true;
    ...
    Line<float> Path::getClippedLine(Line<float> line, bool keepSectionOutsidePath) const
    {
        ...
        {
            if (keepSectionOutsidePath == startInside) result = Line<float>();
        }
        else
        {
    ...
    float Path::getLength(const AffineTransform& transform, float tolerance) const
    {
        ...
        while (i.next()) length += Line<float>(i.x1, i.y1, i.x2, i.y2).getLength();

        return length;
    }

    Point<float> Path::getPointAlongPath(float distanceFromStart, const AffineTransform& transform, float tolerance) const
    {
        PathFlatteningIterator i(*this, transform, tolerance);
        ...
        while (i.next())
        {
            ...
            if (distanceFromStart <= lineLength) return line.getPointAlongLine(distanceFromStart);
    ...
    float Path::getNearestPoint(Point<float> targetPoint, Point<float>& pointOnPath, const AffineTransform& transform,
    ...
    {
        if (cornerRadius <= 0.01f) return *this;
            ...
            else if (isMarker(type, lineMarker) || isMarker(type, closeSubPathMarker))
            {
                ...
                if (lastWasLine)
                {
                    ...
                    if (len2 > 0)
                    {
                        ...
                        p.quadraticTo(joinX, joinY, (float)(joinX + (endX - joinX) * propNeeded),
    ...
    String Path::toString() const
    {
        ...
        for (int i = 0; i < data.size();)
        {
            ...
            while (--numCoords >= 0 && i < data.size())
            {
                ...
                while (coord.endsWithChar('0') && coord != "0") coord = coord.dropLastCharacters(1);

                if (coord.endsWithChar('.')) coord = coord.dropLastCharacters(1);
    ...
    void Path::restoreFromString(StringRef stringVersion)
    {
        ...
        for (;;)
        {
            ...
            for (int i = startNum; i < numValues; ++i) values[i] = PathHelpers::nextToken(t).getFloatValue();
