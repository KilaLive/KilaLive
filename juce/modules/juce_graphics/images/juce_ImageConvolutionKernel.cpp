...
namespace juce
{
    ...
    {
        if (isPositiveAndBelow(x, size) && isPositiveAndBelow(y, size)) return values[x + y * size];
    ...
    {
        for (int i = size * size; --i >= 0;) values[i] = 0;
    }
    ...
    void ImageConvolutionKernel::setOverallSum(const float desiredTotalSum)
    {
        ...
        for (int i = size * size; --i >= 0;) currentTotal += values[i];
    ...
    {
        for (int i = size * size; --i >= 0;) values[i] *= multiplier;
    }
    ...
    //==============================================================================
    void ImageConvolutionKernel::applyToImage(Image& destImage, const Image& sourceImage,
                                              const Rectangle<int>& destinationArea) const
    {
        ...
        {
            if (sourceImage.getWidth() != destImage.getWidth() || sourceImage.getHeight() != destImage.getHeight()
