...
namespace juce
{
    struct ImageCache::Pimpl : private Timer, private DeletedAtShutdown
    {
        Pimpl() {}
        ~Pimpl()
        {
        ...
        void addImageToCache(const Image& image, const int64 hashCode)
        {
            if (image.isValid())
            {
                if (!isTimerRunning()) startTimer(2000);
        ...
        void timerCallback() override
        {
            ...
            for (int i = images.size(); --i >= 0;)
            {
                ...
                {
                    if (now > item.lastUseTime + cacheTimeout || now < item.lastUseTime - 1000) images.remove(i);
                }
                else
                {
        ...
        void releaseUnusedImages()
        {
            ...
            for (int i = images.size(); --i >= 0;)
                if (images.getReference(i).image.getReferenceCount() <= 1) images.remove(i);
        }
