...
namespace juce
{
    ...
    TextLayout::Line::Line(Range<int> range, Point<float> o, float asc, float desc, float lead, int numRunsToPreallocate)
        : stringRange(range)
        , lineOrigin(o)
        , ascent(asc)
        , descent(desc)
    ...
    void TextLayout::draw(Graphics& g, Rectangle<float> area) const
    {
        ...
        for (auto* line : lines)
        {
            ...
            if (lineRangeY.getEnd() < clipTop) continue;

            if (lineRangeY.getStart() > clipBottom) break;
    ...
    void TextLayout::createLayout(const AttributedString& text, float maxWidth, float maxHeight)
    {
        ...
        if (!createNativeLayout(text)) createStandardLayout(text);
    ...
    void TextLayout::createLayoutWithBalancedLineLengths(const AttributedString& text, float maxWidth, float maxHeight)
    {
        ...
        if (bestWidth != maxWidth) createLayout(text, bestWidth, maxHeight);
    }
    ...
    namespace TextLayoutHelpers
    {
                ...
                for (int i = 0; i < tokens.size(); ++i)
                {
                    ...
                    if (newGlyphs.size() > 0)
                    {
                        ...
                        for (int j = 0; j < newGlyphs.size(); ++j)
                        {
                            ...
                            currentRun->glyphs.add(TextLayout::Glyph(newGlyphs.getUnchecked(j), glyphOffset.translated(x, 0),
                    ...
                    if (auto* nextToken = tokens[i + 1])
                    {
                        ...
                        if (t.line != nextToken->line)
                        {
                            ...
                            if (!needToSetLineOrigin) layout.addLine(currentLine.release());
        ...
        private:
            ...
            void appendText(const String& stringText, const Font& font, Colour colour)
            {
                ...
                for (;;)
                {
                    ...
                    if (charType == 0 || charType != lastCharType)
                    {
                        if (currentString.isNotEmpty())
                            tokens.add(new Token(currentString, font, colour, lastCharType == 2 || lastCharType == 0));
                ...
                if (currentString.isNotEmpty()) tokens.add(new Token(currentString, font, colour, lastCharType == 2));
            }
            ...
            void addTextRuns(const AttributedString& text)
            {
                ...
                for (int i = 0; i < numAttributes; ++i)
                {
                    ...
                    appendText(text.getText().substring(attr.range.getStart(), attr.range.getEnd()), attr.font, attr.colour);
                }
            }
            ...
            static String getTrimmedEndIfNotAllWhitespace(const String& s)
            {
                ...
                if (trimmed.isEmpty() && s.isNotEmpty()) trimmed = s.replaceCharacters("\r\n\t", "   ");
    ...
    void TextLayout::recalculateSize()
    {
        if (!lines.isEmpty())
        {
            ...
            for (auto* line : lines) bounds = bounds.getUnion(line->getLineBounds());

            for (auto* line : lines) line->lineOrigin.x -= bounds.getX();
