...
namespace juce
{
    struct FontStyleHelpers
    {
        ...
        {
            return getStyleName((styleFlags & Font::bold) != 0, (styleFlags & Font::italic) != 0);
        }
        ...
        {
            return style.containsWholeWordIgnoreCase("Italic") || style.containsWholeWordIgnoreCase("Oblique");
        }

        static bool isPlaceholderFamilyName(const String& family)
        {
            return family == Font::getDefaultSansSerifFontName() || family == Font::getDefaultSerifFontName()
        ...
        static String getConcreteFamilyName(const Font& font)
        {
            ...
            return isPlaceholderFamilyName(family) ? getConcreteFamilyNameFromPlaceholder(family) : family;
        }
    };
    ...
    EdgeTable* Typeface::getEdgeTableForGlyph(int glyphNumber, const AffineTransform& transform, float fontHeight)
    {
        ...
        if (getOutlineForGlyph(glyphNumber, path) && !path.isEmpty())
        {
            ...
            return new EdgeTable(path.getBoundsTransformed(transform).getSmallestIntegerContainer().expanded(1, 0), path,
                                 transform);
        }
    ...
    struct Typeface::HintingParams
    {
        ...
        void applyVerticalHintingTransform(float fontSize, Path& path)
        {
            ...
            if (bottom < top + 3.0f / fontSize) return;
            ...
            for (Path::Iterator i(path); i.next();)
            {
                ...
                case Path::Iterator::quadraticTo:
                    result.quadraticTo(i.x1, cachedScale.apply(i.y1), i.x2, cachedScale.apply(i.y2));
                    break;
                case Path::Iterator::cubicTo:
                    result.cubicTo(i.x1, cachedScale.apply(i.y1), i.x2, cachedScale.apply(i.y2), i.x3,
                                   cachedScale.apply(i.y3));
                    break;
    ...
    private:
        struct Scaling
        {
                ...
                : middle(m)
            {
                ...
                const float newM =
                    std::floor(fontSize * m + 0.3f) / fontSize;  // this is slightly biased so that lower-case letters
            ...
            {
                return y < middle ? (y * upperScale + upperOffset) : (y * lowerScale + lowerOffset);
            }
        ...
        static float getAverageY(const Font& font, const char* chars, bool getTop)
        {
            ...
            for (auto& glyph : ga)
            {
                ...
                if (!p.isEmpty()) yValues.add(getTop ? bounds.getY() : bounds.getBottom());
            }
    ...
    void Typeface::applyVerticalHintingTransform(float fontSize, Path& path)
    {
        if (fontSize > 3.0f && fontSize < 25.0f)
        {
            ...
            if (hintingParams == nullptr) hintingParams.reset(new HintingParams(*this));
