...
namespace juce
{
    ...
    float Font::getDefaultMinimumHorizontalScaleFactor() noexcept
    {
        return FontValues::minimumHorizontalScale;
    }
    void Font::setDefaultMinimumHorizontalScaleFactor(float newValue) noexcept
    {
    ...
    public:
        ...
        Typeface::Ptr findTypefaceFor(const Font& font)
        {
            ...
            for (int i = faces.size(); --i >= 0;)
            {
                ...
                if (face.typefaceName == faceName && face.typefaceStyle == faceStyle && face.typeface != nullptr
            ...
            if (defaultFace == nullptr && font == Font()) defaultFace = face.typeface;
    ...
    void Typeface::clearTypefaceCache()
    {
        ...
        if (clearOpenGLGlyphCache != nullptr) clearOpenGLGlyphCache();
    }
    ...
    public:
        ...
        {
            if (styleFlags == plain) typeface = TypefaceCache::getInstance()->defaultFace;
        }
        ...
        SharedFontInternal(const String& name, int styleFlags, float fontHeight) noexcept
            ...
            , underline((styleFlags & underlined) != 0)
        {
            if (styleFlags == plain && typefaceName.isEmpty()) typeface = TypefaceCache::getInstance()->defaultFace;
        }
        ...
        SharedFontInternal(const String& name, const String& style, float fontHeight) noexcept
            ...
            , height(fontHeight)
        {
            if (typefaceName.isEmpty()) typefaceName = Font::getDefaultSansSerifFontName();
        }
        ...
        {
            return height == other.height && underline == other.underline && horizontalScale == other.horizontalScale
                   && kerning == other.kerning && typefaceName == other.typefaceName && typefaceStyle == other.typefaceStyle;
        }
    ...
    {
        return font == other.font || *font == *other.font;
    }
    ...
    {
        if (font->getReferenceCount() > 1) font = new SharedFontInternal(*font);
    }

    void Font::checkTypefaceSuitability()
    {
        if (font->typeface != nullptr && !font->typeface->isSuitableForFont(*this)) font->typeface = nullptr;
    }

    //==============================================================================
    struct FontPlaceholderNames
    {
        String sans{"<Sans-Serif>"}, serif{"<Serif>"}, mono{"<Monospaced>"}, regular{"<Regular>"};
    };
    ...
    void Font::setSizeAndStyle(float newHeight, const int newStyleFlags, const float newHorizontalScale,
                               const float newKerningAmount)
    {
        newHeight = FontValues::limitFontHeight(newHeight);

        if (font->height != newHeight || font->horizontalScale != newHorizontalScale || font->kerning != newKerningAmount)
        {
            dupeInternalIfShared();
            font->height          = newHeight;
    ...
    void Font::setSizeAndStyle(float newHeight, const String& newStyle, const float newHorizontalScale,
                               const float newKerningAmount)
    {
        newHeight = FontValues::limitFontHeight(newHeight);
    ...
    void Font::setBold(const bool shouldBeBold)
    {
        ...
        setStyleFlags(shouldBeBold ? (flags | bold) : (flags & ~bold));
    }

    void Font::setItalic(const bool shouldBeItalic)
    {
        auto flags = getStyleFlags();
        setStyleFlags(shouldBeItalic ? (flags | italic) : (flags & ~italic));
    }
    ...
    {
        if (font->ascent == 0.0f) font->ascent = getTypeface()->getAscent();
    ...
    float Font::getStringWidthFloat(const String& text) const
    {
        ...
        if (font->kerning != 0.0f) w += font->kerning * text.length();
    ...
    void Font::getGlyphPositions(const String& text, Array<int>& glyphs, Array<float>& xOffsets) const
    {
        ...
        if (auto num = xOffsets.size())
        {
            ...
            {
                for (int i = 0; i < num; ++i) x[i] = (x[i] + i * font->kerning) * scale;
            }
            else
            {
                for (int i = 0; i < num; ++i) x[i] *= scale;
            }
        }
    }
    ...
    void Font::findFonts(Array<Font>& destArray)
    {
        for (auto& name : findAllTypefaceNames())
        {
            ...
            if (!styles.contains(style, true)) style = styles[0];
    ...
    String Font::toString() const
    {
        ...
        if (getTypefaceName() != getDefaultSansSerifFontName()) s << getTypefaceName() << "; ";

        s << String(getHeight(), 1);

        if (getTypefaceStyle() != getDefaultStyle()) s << ' ' << getTypefaceStyle();
    ...
    Font Font::fromString(const String& fontDescription)
    {
        ...
        if (separator > 0) name = fontDescription.substring(0, separator).trim();

        if (name.isEmpty()) name = getDefaultSansSerifFontName();
