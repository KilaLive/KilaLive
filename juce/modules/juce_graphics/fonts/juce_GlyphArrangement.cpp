...
namespace juce
{
    ...
    PositionedGlyph::PositionedGlyph(const Font& font_, juce_wchar character_, int glyphNumber, float anchorX,
                                     float baselineY, float width, bool whitespace_)
        : font(font_)
    ...
    {
        if (!isWhitespace()) drawGlyphWithFont(g, glyph, font, AffineTransform::translation(x, y));
    }

    void PositionedGlyph::draw(Graphics& g, AffineTransform transform) const
    {
        if (!isWhitespace()) drawGlyphWithFont(g, glyph, font, AffineTransform::translation(x, y).followedBy(transform));
    }
    ...
    void PositionedGlyph::createPath(Path& path) const
    {
        if (!isWhitespace())
        {
            if (auto* t = font.getTypeface())
            {
                ...
                path.addPath(
                    p,
                    AffineTransform::scale(font.getHeight() * font.getHorizontalScale(), font.getHeight()).translated(x, y));
            }
        }
    }
    ...
    void GlyphArrangement::addCurtailedLineOfText(const Font& font, const String& text, float xOffset, float yOffset,
                                                  float maxWidthPixels, bool useEllipsis)
    {
        if (text.isNotEmpty())
        {
            ...
            for (int i = 0; i < textLen; ++i)
            {
                ...
                glyphs.add(PositionedGlyph(font, t.getAndAdvance(), newGlyphs.getUnchecked(i), xOffset + thisX, yOffset,
    ...
    int GlyphArrangement::insertEllipsis(const Font& font, float maxXPos, int startIndex, int endIndex)
    {
        ...
        if (!glyphs.isEmpty())
        {
            ...
            for (int i = 3; --i >= 0;)
            {
                glyphs.insert(endIndex++, PositionedGlyph(font, '.', dotGlyphs.getFirst(), xOffset, yOffset, dx, false));
                --numDeleted;
                xOffset += dx;
    ...
    void GlyphArrangement::addJustifiedText(const Font& font, const String& text, float x, float y, float maxLineWidth,
                                            Justification horizontalLayout)
    {
        ...
        while (lineStartIndex < glyphs.size())
        {
            ...
            if (glyphs.getReference(i).getCharacter() != '\n' && glyphs.getReference(i).getCharacter() != '\r') ++i;
            ...
            while (i < glyphs.size())
            {
                ...
                if (c == '\r' || c == '\n')
                {
                    ...
                    if (c == '\r' && i < glyphs.size() && glyphs.getReference(i).getCharacter() == '\n') ++i;
                ...
                {
                    if (lastWordBreakIndex >= 0) i = lastWordBreakIndex;
            ...
            moveRangeOfGlyphs(lineStartIndex, i - lineStartIndex, x + deltaX - currentLineStartX, y - originalY);
    ...
    void GlyphArrangement::addFittedText(const Font& f, const String& text, float x, float y, float width, float height,
                                         Justification layout, int maximumLines, float minimumHorizontalScale)
    {
        if (minimumHorizontalScale == 0.0f) minimumHorizontalScale = Font::getDefaultMinimumHorizontalScaleFactor();
        ...
        else
        {
            ...
            {
                auto lineWidth =
                    glyphs.getReference(glyphs.size() - 1).getRight() - glyphs.getReference(startIndex).getLeft();

                if (lineWidth > 0)
                {
                    if (lineWidth * minimumHorizontalScale < width)
                    {
                        if (lineWidth > width) stretchRangeOfGlyphs(startIndex, numGlyphs, width / lineWidth);

                        justifyGlyphs(startIndex, numGlyphs, x, y, width, height, layout);
                    }
                    else if (maximumLines <= 1)
                    {
                        fitLineIntoSpace(startIndex, numGlyphs, x, y, width, height, f, layout, minimumHorizontalScale);
                    }
                    else
                    {
                        splitLines(trimmed, f, startIndex, x, y, width, height, maximumLines, lineWidth, layout,
    ...
    void GlyphArrangement::moveRangeOfGlyphs(int startIndex, int num, const float dx, const float dy)
    {
        ...
        if (dx != 0.0f || dy != 0.0f)
        {
            if (num < 0 || startIndex + num > glyphs.size()) num = glyphs.size() - startIndex;

            while (--num >= 0) glyphs.getReference(startIndex++).moveBy(dx, dy);
        }
    }

    void GlyphArrangement::addLinesWithLineBreaks(const String& text, const Font& f, float x, float y, float width,
                                                  float height, Justification layout)
    {
        GlyphArrangement ga;
    ...
    Rectangle<float> GlyphArrangement::getBoundingBox(int startIndex, int num, bool includeWhitespace) const
    {
        ...
        while (--num >= 0)
        {
            ...
            if (includeWhitespace || !pg.isWhitespace()) result = result.getUnion(pg.getBounds());
        }

        return result;
    }

    void GlyphArrangement::justifyGlyphs(int startIndex, int num, float x, float y, float width, float height,
                                         Justification justification)
    {
        ...
        if (glyphs.size() > 0 && num > 0)
        {
            auto bb = getBoundingBox(
                startIndex, num,
                !justification.testFlags(Justification::horizontallyJustified | Justification::horizontallyCentred));
            ...
            if (justification.testFlags(Justification::horizontallyJustified))
                ...
                deltaX += (width - bb.getWidth()) * 0.5f - bb.getX();
            else if (justification.testFlags(Justification::right))
                deltaX += width - bb.getRight();
            else
                deltaX -= bb.getX();
            ...
            if (justification.testFlags(Justification::top))
                ...
                deltaY += height - bb.getBottom();
            else
                deltaY += (height - bb.getHeight()) * 0.5f - bb.getY();
            ...
            if (justification.testFlags(Justification::horizontallyJustified))
            {
                ...
                if (i > lineStart) spreadOutLine(startIndex + lineStart, i - lineStart, width);
            }
        }
    }
    ...
    {
        if (start + num < glyphs.size() && glyphs.getReference(start + num - 1).getCharacter() != '\r'
            && glyphs.getReference(start + num - 1).getCharacter() != '\n')
        {
            ...
            if (numSpaces > 0)
            {
                ...
                for (int i = 0; i < num; ++i)
                {
                    ...
                    if (glyphs.getReference(start + i).isWhitespace()) deltaX += extraPaddingBetweenWords;
                }
            }
        }
    }
    ...
    void GlyphArrangement::splitLines(const String& text, Font font, int startIndex, float x, float y, float width,
                                      float height, int maximumLines, float lineWidth, Justification layout,
                                      float minimumHorizontalScale)
    {
        ...
        while (numLines < maximumLines)
        {
            ...
            if (newFontHeight < font.getHeight())
            {
                ...
                lineWidth = glyphs.getReference(glyphs.size() - 1).getRight() - glyphs.getReference(startIndex).getLeft();
            }
            ...
            if (numLines > (lineWidth + lineLengthUnevennessAllowance) / width || newFontHeight < 8.0f) break;
        }
        ...
        int  lineIndex    = 0;
        auto lineY        = y;
        auto widthPerLine = jmin(width / minimumHorizontalScale, lineWidth / numLines);
        ...
        while (lineY < y + height)
        {
            ...
            if (lineIndex++ >= numLines - 1 || lineBottomY >= y + height)
            {
                widthPerLine = width;
            ...
            else
            {
                ...
                while (wsStart > 0 && glyphs.getReference(wsStart - 1).isWhitespace()) --wsStart;

                while (wsEnd < glyphs.size() && glyphs.getReference(wsEnd).isWhitespace()) ++wsEnd;
            ...
            endIndex -=
                fitLineIntoSpace(startIndex, endIndex - startIndex, x, lineY, width, font.getHeight(), font,
                                 layout.getOnlyHorizontalFlags() | Justification::verticallyCentred, minimumHorizontalScale);

            startIndex = endIndex;
            lineY      = lineBottomY;

            if (startIndex >= glyphs.size()) break;
        }

        justifyGlyphs(originalStartIndex, glyphs.size() - originalStartIndex, x, y, width, height,
                      layout.getFlags() & ~Justification::horizontallyJustified);
    }

    //==============================================================================
    void GlyphArrangement::drawGlyphUnderline(const Graphics& g, const PositionedGlyph& pg, int i,
                                              AffineTransform transform) const
    {
        auto lineThickness = (pg.font.getDescent()) * 0.3f;
        auto nextX         = pg.x + pg.w;

        if (i < glyphs.size() - 1 && glyphs.getReference(i + 1).y == pg.y) nextX = glyphs.getReference(i + 1).x;
    ...
    void GlyphArrangement::draw(const Graphics& g, AffineTransform transform) const
    {
        ...
        for (int i = 0; i < glyphs.size(); ++i)
        {
            ...
            if (pg.font.isUnderlined()) drawGlyphUnderline(g, pg, i, transform);
        ...
        if (needToRestore) context.restoreState();
    }
    ...
    {
        for (int i = 0; i < glyphs.size(); ++i)
            if (glyphs.getReference(i).hitTest(x, y)) return i;
