...
namespace juce
{
        ...
        float getHorizontalSpacing(juce_wchar subsequentCharacter) const noexcept
        {
            if (subsequentCharacter != 0)
                for (auto& kp : kerningPairs)
                    if (kp.character2 == subsequentCharacter) return width + kp.kerningAmount;
    ...
    void CustomTypeface::setCharacteristics(const String& newName, float newAscent, bool isBold, bool isItalic,
                                            juce_wchar newDefaultCharacter) noexcept
    {
        name             = newName;
    ...
    void CustomTypeface::setCharacteristics(const String& newName, const String& newStyle, float newAscent,
                                            juce_wchar newDefaultCharacter) noexcept
    {
        name             = newName;
        style            = newStyle;
    ...
    CustomTypeface::GlyphInfo* CustomTypeface::findGlyph(juce_wchar character, bool loadIfNeeded) noexcept
    {
        ...
        for (auto* g : glyphs)
            if (g->character == character) return g;

        if (loadIfNeeded && loadGlyphIfPossible(character)) return findGlyph(character, false);
    ...
    void CustomTypeface::addGlyphsFromOtherTypeface(Typeface& typefaceToCopy, juce_wchar characterStartIndex,
                                                    int numCharacters) noexcept
    {
        ...
        for (int i = 0; i < numCharacters; ++i)
        {
            ...
            if (glyphIndex >= 0 && glyphIndexes.size() > 0)
            {
                ...
                for (int j = glyphs.size() - 1; --j >= 0;)
                {
                    ...
                    offsets.clearQuick();
                    typefaceToCopy.getGlyphPositions(String::charToString(c) + String::charToString(char2), glyphIndexes,
                                                     offsets);

                    if (offsets.size() > 1) addKerningPair(c, char2, offsets[1] - glyphWidth);
                }
            }
        }
    }
        ...
        for (auto t = text.getCharPointer(); !t.isEmpty();)
        {
            ...
            else
            {
                if (auto fallbackTypeface = Typeface::getFallbackTypeface())
                    if (fallbackTypeface != this) x += fallbackTypeface->getStringWidth(String::charToString(c));
            }
        }
    ...
    bool CustomTypeface::getOutlineForGlyph(int glyphNumber, Path& path)
    {
        ...
        if (auto fallbackTypeface = getFallbackTypeface())
            if (fallbackTypeface != this) return fallbackTypeface->getOutlineForGlyph(glyphNumber, path);
