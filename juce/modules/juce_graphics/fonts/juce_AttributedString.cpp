...
namespace juce
{
    namespace
    {
        ...
        void appendRange(Array<AttributedString::Attribute>& atts, int length, const Font* f, const Colour* c)
        {
            if (atts.size() == 0)
            {
                atts.add(AttributedString::Attribute(Range<int>(0, length), f != nullptr ? *f : Font(),
        ...
        void applyFontAndColour(Array<AttributedString::Attribute>& atts, Range<int> range, const Font* f, const Colour* c)
        {
            ...
            for (int i = 0; i < atts.size(); ++i)
            {
                ...
                {
                    if (range.getEnd() <= att.range.getStart()) break;
        ...
        void truncate(Array<AttributedString::Attribute>& atts, int newLength)
        {
            ...
            for (int i = atts.size(); --i >= 0;)
                if (atts.getReference(i).range.getStart() >= newLength) atts.remove(i);
        }
    }  // namespace
    ...
    void AttributedString::append(const AttributedString& other)
    {
        ...
        for (int i = originalNumAtts; i < attributes.size(); ++i) attributes.getReference(i).range += originalLength;
