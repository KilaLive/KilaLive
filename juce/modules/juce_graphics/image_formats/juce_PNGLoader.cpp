...
#pragma warning(disable : 4611)  // (warning about setjmp)
        ...
        static bool readImageData(png_structp pngReadStruct, png_infop pngInfoStruct, jmp_buf& errorJumpBuf,
                                  png_bytepp rows) noexcept
        {
            if (setjmp(errorJumpBuf) == 0)
            {
                if (png_get_valid(pngReadStruct, pngInfoStruct, PNG_INFO_tRNS)) png_set_expand(pngReadStruct);
...
#pragma warning(pop)
        ...
        static Image readImage(InputStream& in, png_structp pngReadStruct, png_infop pngInfoStruct)
        {
            ...
            if (readHeader(in, pngReadStruct, pngInfoStruct, errorJumpBuf, width, height, bitDepth, colorType, interlaceType))
            {
                ...
                for (size_t y = 0; y < height; ++y) rows[y] = (png_bytep)(tempBuffer + lineStride * y);
                ...
                if (readImageData(pngReadStruct, pngInfoStruct, errorJumpBuf, rows))
                    return createImageFromData((colorType & PNG_COLOR_MASK_ALPHA) != 0 || num_trans != 0, (int)width,
                                               (int)height, rows);
            }
    ...
    bool PNGImageFormat::canUnderstand(InputStream& in)
    {
        ...
        return in.read(header, bytesNeeded) == bytesNeeded && header[1] == 'P' && header[2] == 'N' && header[3] == 'G';
    }
    ...
    bool PNGImageFormat::writeImageToStream(const Image& image, OutputStream& out)
    {
        ...
        if (pngWriteStruct == nullptr) return false;
        ...
        png_set_IHDR(pngWriteStruct, pngInfoStruct, (png_uint_32)width, (png_uint_32)height, 8,
                     image.hasAlphaChannel() ? PNG_COLOR_TYPE_RGB_ALPHA : PNG_COLOR_TYPE_RGB, PNG_INTERLACE_NONE,
