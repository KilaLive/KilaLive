...
#pragma warning(pop)
    ...
    bool JPEGImageFormat::canUnderstand(InputStream& in)
    {
        ...
        if (in.read(header, bytesNeeded) == bytesNeeded && header[0] == 0xff && header[1] == 0xd8 && header[2] == 0xff)
            return true;

#if JUCE_USING_COREIMAGE_LOADER
        return header[20] == 'j' && header[21] == 'p' && header[22] == '2' && header[23] == ' ';
#endif
        ...
        if (mb.getDataSize() > 16)
        {
            ...
            if (!hasFailed)
            {
                ...
                if (!hasFailed)
                {
                    ...
                    JSAMPARRAY buffer = (*jpegDecompStruct.mem->alloc_sarray)((j_common_ptr)&jpegDecompStruct, JPOOL_IMAGE,
                    ...
                    if (jpeg_start_decompress(&jpegDecompStruct) && !hasFailed)
                    {
                        ...
                        const bool hasAlphaChan =
                            image.hasAlphaChannel();  // (the native image creator may not give back what we expect)
                        ...
                        if (!hasFailed) jpeg_finish_decompress(&jpegDecompStruct);
    ...
    bool JPEGImageFormat::writeImageToStream(const Image& image, OutputStream& out)
    {
        ...
        JSAMPARRAY buffer =
            (*jpegCompStruct.mem->alloc_sarray)((j_common_ptr)&jpegCompStruct, JPOOL_IMAGE, (JDIMENSION)strideBytes, 1);
