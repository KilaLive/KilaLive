...
namespace juce
{
    ...
    public:
            ...
            , endCode(0)
        {
            ...
            if (!getSizeFromHeader(imageWidth, imageHeight)) return;

            uint8 buf[16];
            if (in.read(buf, 3) != 3) return;

            int numColours  = 2 << (buf[0] & 7);
            int transparent = -1;

            if ((buf[0] & 0x80) != 0) readPalette(numColours);
            ...
            for (;;)
            {
                ...
                if (buf[0] == '!')
                {
                    if (readExtension(transparent)) continue;
                ...
                if (input.read(buf, 9) == 9)
                {
                    ...
                    if ((buf[8] & 0x80) != 0)
                        if (!readPalette(numColours)) break;

                    image = Image(transparent >= 0 ? Image::ARGB : Image::RGB, imageWidth, imageHeight, transparent >= 0);
    ...
    private:
        ...
        bool getSizeFromHeader(int& w, int& h)
        {
            ...
            if (input.read(b, 6) == 6 && (strncmp("GIF87a", b, 6) == 0 || strncmp("GIF89a", b, 6) == 0))
            {
                if (input.read(b, 4) == 4)
                {
        ...
        int readDataBlock(uint8* const dest)
        {
            ...
            if (input.read(&n, 1) == 1)
            {
                ...
                if (dataBlockIsZero || (input.read(dest, n) == n)) return n;
            }
        ...
        int readLZWByte()
        {
            if (fresh)
            {
                ...
                for (;;)
                {
                    ...
                    if (firstcode != clearCode) return firstcode;
                }
            }
        ...
        {
            uint8 c;
            if (input.read(&c, 1) != 1) return false;

            initialise(c);

            if (transparent >= 0) palette[transparent].setARGB(0, 0, 0, 0);
    ...
    bool GIFImageFormat::canUnderstand(InputStream& in)
    {
        ...
        return (in.read(header, sizeof(header)) == sizeof(header)) && header[0] == 'G' && header[1] == 'I'
               && header[2] == 'F';
    }
