...
namespace juce
{
    ...
    {
        static const char* pathsToSearch[] = {"/etc/fonts/fonts.conf", "/usr/share/fonts/fonts.conf"};

        for (auto* path : pathsToSearch)
            if (auto* xml = XmlDocument::parse(File(path))) return xml;
    ...
    StringArray FTTypefaceList::getDefaultFontDirectories()
    {
                ...
                forEachXmlChildElementWithTagName(*fontsInfo, e, "dir")
                {
                    ...
                    if (fontPath.isNotEmpty())
                    {
                        if (e->getStringAttribute("prefix") == "xdg")
                        {
                            ...
                            if (xdgDataHome.trimStart().isEmpty()) xdgDataHome = "~/.local/share";
        ...
        if (fontDirs.isEmpty()) fontDirs.add("/usr/X11R6/lib/X11/fonts");
    ...
    private:
        static String pickBestFont(const StringArray& names, const char* const* choicesArray)
        {
            ...
            for (auto& choice : choices)
                if (names.contains(choice, true)) return choice;

            for (auto& choice : choices)
                for (auto& name : names)
                    if (name.startsWithIgnoreCase(choice)) return name;

            for (auto& choice : choices)
                for (auto& name : names)
                    if (name.containsIgnoreCase(choice)) return name;
        ...
        static String getDefaultSansSerifFontName()
        {
            ...
            static const char* targets[] = {
                "Verdana", "Bitstream Vera Sans", "Luxi Sans", "Liberation Sans", "DejaVu Sans", "Sans", nullptr};
        ...
        static String getDefaultSerifFontName()
        {
            ...
            static const char* targets[] = {"Bitstream Vera Serif", "Times", "Nimbus Roman", "Liberation Serif",
        ...
        static String getDefaultMonospacedFontName()
        {
            ...
            static const char* targets[] = {"DejaVu Sans Mono",
