...
#pragma warning(disable : 4127)  // "expression is constant" warning
    ...
    namespace RenderingHelpers
    {
        ...
        public:
            ...
            {
                return isOnlyTranslated ? AffineTransform::translation(offset) : complexTransform;
            }

            AffineTransform getTransformWith(const AffineTransform& userTransform) const noexcept
            {
                return isOnlyTranslated ? userTransform.translated(offset) : userTransform.followedBy(complexTransform);
            }
            ...
            void setOrigin(Point<int> delta) noexcept
            {
                if (isOnlyTranslated)
                    ...
                    complexTransform = AffineTransform::translation(delta).followedBy(complexTransform);
            }
            ...
            void addTransform(const AffineTransform& t) noexcept
            {
                ...
                isRotated = (complexTransform.mat01 != 0.0f || complexTransform.mat10 != 0.0f || complexTransform.mat00 < 0
            ...
            {
                return isOnlyTranslated ? r - offset : r.transformedBy(complexTransform.inverted());
            }
        ...
        private:
            ...
            {
                for (auto* g : glyphs)
                    if (g->glyph == glyphNumber && g->font == font) return g;
            ...
            CachedGlyphType* getGlyphForReuse()
            {
                ...
                {
                    if (misses.get() * 2 > hits.get()) addNewGlyphSlots(32);

                    hits   = 0;
                    misses = 0;
                }

                if (auto* g = findLeastRecentlyUsedGlyph()) return g;
            ...
            void addNewGlyphSlots(int num)
            {
                ...
                while (--num >= 0) glyphs.add(new CachedGlyphType());
            }
            ...
            CachedGlyphType* findLeastRecentlyUsedGlyph() const noexcept
            {
                ...
                for (auto* g : glyphs)
                {
                    if (g->lastAccessCount <= oldestCounter && g->getReferenceCount() == 1)
                    {
        ...
        public:
            ...
            {
                if (snapToIntegerCoordinate) pos.x = std::floor(pos.x + 0.5f);

                if (edgeTable != nullptr) state.fillEdgeTable(*edgeTable, pos.x, roundToInt(pos.y));
            }
            ...
            void generate(const Font& newFont, int glyphNumber)
            {
                ...
                edgeTable.reset(typeface->getEdgeTableForGlyph(
                    glyphNumber, AffineTransform::scale(fontHeight * font.getHorizontalScale(), fontHeight), fontHeight));
            }
        ...
        */
        ...
        namespace GradientPixelIterators
        {
            ...
            struct Linear
            {
                Linear(const ColourGradient& gradient, const AffineTransform& transform, const PixelARGB* colours,
                       int numColours)
                    : lookupTable(colours)
                ...
                {
                    return vertical ? linePix : lookupTable[jlimit(0, numEntries, (x * scale - start) >> (int)numScaleBits)];
                }
            ...
            {
                TransformedRadial(const ColourGradient& gradient, const AffineTransform& transform, const PixelARGB* colours,
                                  int numColours)
                ...
                inline PixelARGB getPixel(int px) const noexcept
                {
                    ...
                    if (x >= maxDist) return lookupTable[numEntries];
    ...
    {                                                                                                                        \
        ...
        namespace EdgeTableFillers
        {
            ...
            private:
                ...
                inline void blendLine(PixelType* dest, PixelARGB colour, int width) const
                    noexcept {JUCE_PERFORM_PIXEL_OP_LOOP(blend(colour))}
                ...
                forcedinline void replaceLine(PixelARGB* dest, const PixelARGB colour, int width) const
                    noexcept {JUCE_PERFORM_PIXEL_OP_LOOP(set(colour))}
            ...
            struct ImageFill
            {
                ...
                forcedinline void handleEdgeTablePixel(int x, int alphaLevel) const noexcept
                {
                    ...
                    getDestPixel(x)->blend(*getSrcPixel(repeatPattern ? ((x - xOffset) % srcData.width) : (x - xOffset)),
                                           (uint32)alphaLevel);
                }
                ...
                void clipEdgeTableLine(EdgeTable& et, int x, int y, int width)
                {
                    ...
                    if (sizeof(SrcPixelType) == sizeof(PixelARGB)) mask += PixelARGB::indexA;
            ...
            private:
                ...
                forcedinline void copyRow(DestPixelType* dest, SrcPixelType const* src, int width) const noexcept
                {
                    ...
                    if (destStride == srcStride && srcData.pixelFormat == Image::RGB && destData.pixelFormat == Image::RGB)
                    {
            ...
            struct TransformedImageFill
            {
                TransformedImageFill(const Image::BitmapData& dest, const Image::BitmapData& src,
                    ...
                    : interpolator(transform, q != Graphics::lowResamplingQuality ? 0.5f : 0.0f,
                ...
                void clipEdgeTableLine(EdgeTable& et, int x, int y, int width)
                {
                    ...
                    et.clipLineToMask(x, y, reinterpret_cast<uint8*>(scratchBuffer.get()) + SrcPixelType::indexA,
            ...
            private:
                        ...
                        if (quality != Graphics::lowResamplingQuality)
                        {
                            if (isPositiveAndBelow(loResX, maxX))
                            {
                                if (isPositiveAndBelow(loResY, maxY))
                                {
                                    ...
                                    render4PixelAverage(dest, this->srcData.getPixelPointer(loResX, loResY), hiResX & 255,
                                                        hiResY & 255);
                                    ++dest;
                                    continue;
                                }
                ...
                void render4PixelAverage(PixelARGB* dest, const uint8* src, int subPixelX, int subPixelY) noexcept
                {
                    ...
                    dest->setARGB((uint8)(c[PixelARGB::indexA] >> 16), (uint8)(c[PixelARGB::indexR] >> 16),
                                  (uint8)(c[PixelARGB::indexG] >> 16), (uint8)(c[PixelARGB::indexB] >> 16));
                }
                ...
                void render2PixelAverageX(PixelARGB* dest, const uint8* src, uint32 subPixelX) noexcept
                {
                    ...
                    dest->setARGB((uint8)(c[PixelARGB::indexA] >> 8), (uint8)(c[PixelARGB::indexR] >> 8),
                                  (uint8)(c[PixelARGB::indexG] >> 8), (uint8)(c[PixelARGB::indexB] >> 8));
                }
                ...
                void render4PixelAverage(PixelRGB* dest, const uint8* src, uint32 subPixelX, uint32 subPixelY) noexcept
                {
                    ...
                    dest->setARGB((uint8)255, (uint8)(c[PixelRGB::indexR] >> 16), (uint8)(c[PixelRGB::indexG] >> 16),
                ...
                void render2PixelAverageX(PixelRGB* dest, const uint8* src, uint32 subPixelX) noexcept
                {
                    ...
                    dest->setARGB((uint8)255, (uint8)(c[PixelRGB::indexR] >> 8), (uint8)(c[PixelRGB::indexG] >> 8),
                ...
                {
                    TransformedImageSpanInterpolator(const AffineTransform& transform, float offsetFloat,
                                                     int offsetInt) noexcept
                        : inverseTransform(transform.inverted())
                        , pixelOffset(offsetFloat)
                        , pixelOffsetInt(offsetInt)
                    {
                    }
            ...
            void renderImageTransformed(Iterator& iter, const Image::BitmapData& destData, const Image::BitmapData& srcData,
                                        int alpha, const AffineTransform& transform, Graphics::ResamplingQuality quality,
                                        bool tiledFill)
            {
                ...
                case Image::ARGB:
                    ...
                    default:
                        ...
                        {
                            TransformedImageFill<PixelARGB, PixelAlpha, false> r(destData, srcData, transform, alpha,
                                                                                 quality);
                            iter.iterate(r);
                        }
                        break;
                ...
                default:
                    ...
                    case Image::ARGB:
                        ...
                        {
                            TransformedImageFill<PixelAlpha, PixelARGB, false> r(destData, srcData, transform, alpha,
                                                                                 quality);
                            iter.iterate(r);
                        }
                        break;
                    ...
                    default:
                        if (tiledFill)
                        {
                            TransformedImageFill<PixelAlpha, PixelAlpha, true> r(destData, srcData, transform, alpha,
                                                                                 quality);
                            iter.iterate(r);
                        }
                        else
                        {
                            TransformedImageFill<PixelAlpha, PixelAlpha, false> r(destData, srcData, transform, alpha,
                                                                                  quality);
                            iter.iterate(r);
                        }
                        break;
            ...
            template <class Iterator>
            void renderImageUntransformed(Iterator& iter, const Image::BitmapData& destData, const Image::BitmapData& srcData,
                                          int alpha, int x, int y, bool tiledFill)
            {
                switch (destData.pixelFormat)
                {
                case Image::ARGB:
            ...
            void renderSolidFill(Iterator& iter, const Image::BitmapData& destData, PixelARGB fillColour,
            ...
            template <class Iterator, class DestPixelType>
            void renderGradient(Iterator& iter, const Image::BitmapData& destData, const ColourGradient& g,
                                const AffineTransform& transform, const PixelARGB* lookupTable, int numLookupEntries,
                                bool isIdentity, DestPixelType*)
            {
                if (g.isRadial)
                {
                    if (isIdentity)
                    {
                        EdgeTableFillers::Gradient<DestPixelType, GradientPixelIterators::Radial> renderer(
                            destData, g, transform, lookupTable, numLookupEntries);
                        iter.iterate(renderer);
                    }
                    else
                    {
                        EdgeTableFillers::Gradient<DestPixelType, GradientPixelIterators::TransformedRadial> renderer(
                            destData, g, transform, lookupTable, numLookupEntries);
                        iter.iterate(renderer);
                    }
                }
                ...
                {
                    EdgeTableFillers::Gradient<DestPixelType, GradientPixelIterators::Linear> renderer(
                        destData, g, transform, lookupTable, numLookupEntries);
                    iter.iterate(renderer);
                }
            }
        ...
        struct ClipRegions
        {
            struct Base : public SingleThreadedReferenceCountedObject
            {
                ...
                virtual void fillRectWithColour(SavedStateType&, Rectangle<int>, PixelARGB colour,
                                                bool replaceContents) const                                   = 0;
                ...
                virtual void fillAllWithGradient(SavedStateType&, ColourGradient&, const AffineTransform&,
                                                 bool isIdentity) const                                       = 0;
                virtual void renderImageTransformed(SavedStateType&, const Image&, int alpha, const AffineTransform&,
                                                    Graphics::ResamplingQuality, bool tiledFill) const        = 0;
                virtual void renderImageUntransformed(SavedStateType&, const Image&, int alpha, int x, int y,
                                                      bool tiledFill) const                                   = 0;
            };
            ...
            struct EdgeTableRegion : public Base
            {
                ...
                Ptr clipToRectangleList(const RectangleList<int>& r) override
                {
                    ...
                    if (inverse.subtract(r))
                        for (auto& i : inverse) edgeTable.excludeRectangle(i);
                ...
                Ptr clipToImageAlpha(const Image& image, const AffineTransform& transform,
                                     Graphics::ResamplingQuality quality) override
                {
                    ...
                    if (transform.isSingularity()) return Ptr();
                ...
                void fillRectWithColour(SavedStateType& state, Rectangle<int> area, PixelARGB colour,
                                        bool replaceContents) const override
                {
                ...
                void fillAllWithGradient(SavedStateType& state, ColourGradient& gradient, const AffineTransform& transform,
                                         bool isIdentity) const override
                {
                    state.fillWithGradient(edgeTable, gradient, transform, isIdentity);
                }

                void renderImageTransformed(SavedStateType& state, const Image& src, int alpha,
                                            const AffineTransform& transform, Graphics::ResamplingQuality quality,
                                            bool tiledFill) const override
                {
                    state.renderImageTransformed(edgeTable, src, alpha, transform, quality, tiledFill);
                }

                void renderImageUntransformed(SavedStateType& state, const Image& src, int alpha, int x, int y,
                                              bool tiledFill) const override
                {
            ...
            private:
                ...
                void transformedClipImage(const Image::BitmapData& srcData, const AffineTransform& transform,
                                          Graphics::ResamplingQuality quality, const SrcPixelType*)
                {
                    EdgeTableFillers::TransformedImageFill<SrcPixelType, SrcPixelType, false> renderer(
                        srcData, srcData, transform, 255, quality);

                    for (int y = 0; y < edgeTable.getMaximumBounds().getHeight(); ++y)
                        renderer.clipEdgeTableLine(edgeTable, edgeTable.getMaximumBounds().getX(),
                                                   y + edgeTable.getMaximumBounds().getY(),
                ...
                void straightClipImage(const Image::BitmapData& srcData, int imageX, int imageY, const SrcPixelType*)
                {
                    ...
                    EdgeTableFillers::ImageFill<SrcPixelType, SrcPixelType, false> renderer(srcData, srcData, 255, imageX,
                                                                                            imageY);
                ...
                public:
                    ...
                    void iterate(Renderer& r) const noexcept
                    {
                        ...
                        for (auto& i : clip)
                        {
                            ...
                            if (f.totalBottom > clipTop && f.totalTop < clipBottom && f.totalRight > clipLeft
                                && f.totalLeft < clipRight)
                            {
                                if (f.isOnePixelWide())
                                {
                                    ...
                                    if (h > 0) r.handleEdgeTableRectangleFull(f.left, y1, 1, h);
                                ...
                                else
                                {
                                    ...
                                    if (h > 0)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if (clippedWidth > 0)
                                                r.handleEdgeTableRectangleFull(clippedLeft, y1, clippedWidth, h);
        ...
        public:
            ...
            SavedStateBase(const RectangleList<int>& clipList, Point<int> origin)
                : clip(new RectangleListRegionType(clipList))
                , transform(origin)
                , interpolationQuality(Graphics::mediumResamplingQuality)
                , transparencyLayerAlpha(1.0f)
            {
            ...
            bool clipToRectangleList(const RectangleList<int>& r)
            {
                if (clip != nullptr)
                {
                    ...
                    else if (!transform.isRotated)
                    {
                        ...
                        for (auto& i : r) scaledList.add(transform.transformed(i));
            ...
            bool clipRegionIntersects(Rectangle<int> r) const
            {
                if (clip != nullptr)
                {
                    if (transform.isOnlyTranslated) return clip->clipRegionIntersects(transform.translated(r));
            ...
            {
                return clip != nullptr ? transform.deviceSpaceToUserSpace(clip->getClipBounds()) : Rectangle<int>();
            }
            ...
            void fillTargetRect(Rectangle<int> r, bool replaceContents)
            {
                ...
                else
                {
                    ...
                    if (!clipped.isEmpty()) fillShape(new RectangleListRegionType(clipped), false);
                }
            }
            ...
            void fillTargetRect(Rectangle<float> r)
            {
                ...
                else
                {
                    ...
                    if (!clipped.isEmpty()) fillShape(new EdgeTableRegionType(clipped), false);
                }
            }
            ...
            void fillRectList(const RectangleList<float>& list)
            {
                if (clip != nullptr)
                {
                    if (list.getNumRectangles() == 1) return fillRect(*list.begin());
            ...
            void fillEdgeTable(const EdgeTable& edgeTable, float x, int y)
            {
                if (clip != nullptr)
                {
                    ...
                    if (fillType.isColour())
                    {
                        ...
                        if (brightness > 0.0f) edgeTableClip->edgeTable.multiplyLevels(1.0f + 1.6f * brightness);
                    }
            ...
            {
                if (clip != nullptr && !fillType.colour.isTransparent()) renderImage(sourceImage, trans, nullptr);
            }

            static bool isOnlyTranslationAllowingError(const AffineTransform& t, float tolerence) noexcept
            {
                return std::abs(t.mat01) < tolerence && std::abs(t.mat10) < tolerence && std::abs(t.mat00 - 1.0f) < tolerence
                             ...
                             const BaseRegionType* tiledFillClipRegion)
            {
                ...
                if (!t.isSingularity())
                {
                    if (tiledFillClipRegion != nullptr)
                    {
                        tiledFillClipRegion->renderImageTransformed(getThis(), sourceImage, alpha, t, interpolationQuality,
                                                                    true);
                    }
                    else
                    {
            ...
            {
                if (clip->getReferenceCount() > 1) clip = clip->clone();
            }
        ...
        public:
                ...
                if (clip != nullptr)
                {
                    if (trans.isOnlyTranslation() && !transform.isRotated)
                    {
                        ...
                        else
                        {
                            ...
                            if (std::abs(xScale - 1.0f) > 0.01f) f.setHorizontalScale(xScale);
                    ...
                    else
                    {
                        ...
                        auto t = transform.getTransformWith(
                            AffineTransform::scale(fontHeight * font.getHorizontalScale(), fontHeight).followedBy(trans));

                        ScopedPointer<EdgeTable> et(font.getTypeface()->getEdgeTableForGlyph(glyphNumber, t, fontHeight));

                        if (et != nullptr) fillShape(new EdgeTableRegionType(*et), false);
                    }
                }
            }
            ...
            void renderImageTransformed(IteratorType& iter, const Image& src, int alpha, const AffineTransform& trans,
                                        Graphics::ResamplingQuality quality, bool tiledFill) const
            {
            ...
            template <typename IteratorType>
            void fillWithGradient(IteratorType& iter, ColourGradient& gradient, const AffineTransform& trans,
                                  bool isIdentity) const
            {
                ...
                {
                case Image::ARGB:
                    EdgeTableFillers::renderGradient(iter, destData, gradient, trans, lookupTable, numLookupEntries,
                                                     isIdentity, (PixelARGB*)0);
                    break;
                case Image::RGB:
                    EdgeTableFillers::renderGradient(iter, destData, gradient, trans, lookupTable, numLookupEntries,
                                                     isIdentity, (PixelRGB*)0);
                    break;
                default:
                    EdgeTableFillers::renderGradient(iter, destData, gradient, trans, lookupTable, numLookupEntries,
                                                     isIdentity, (PixelAlpha*)0);
                    break;
                }
            }
