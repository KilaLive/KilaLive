...
namespace juce
{
    ...
    namespace
    {
        ...
        inline Point<float> convertPoint(D2D1_POINT_2F p) noexcept
        {
            return Point<float>((float)p.x, (float)p.y);
        }
    }  // namespace
    ...
    public:
        Direct2DFactories()
        {
            ...
            {
                JUCE_LOAD_WINAPI_FUNCTION(direct2dDll, D2D1CreateFactory, d2d1CreateFactory, HRESULT,
            ...
            {
                JUCE_LOAD_WINAPI_FUNCTION(directWriteDll, DWriteCreateFactory, dWriteCreateFactory, HRESULT,
                ...
                {
                    D2D1_RENDER_TARGET_PROPERTIES d2dRTProp =
                        D2D1::RenderTargetProperties(D2D1_RENDER_TARGET_TYPE_SOFTWARE,
                                                     D2D1::PixelFormat(DXGI_FORMAT_B8G8R8A8_UNORM, D2D1_ALPHA_MODE_IGNORE), 0,
                                                     0, D2D1_RENDER_TARGET_USAGE_GDI_COMPATIBLE, D2D1_FEATURE_LEVEL_DEFAULT);
    ...
    public:
            ...
            , ascent(0.0f)
        {
            ...
            // Get a specific font in the font family using typeface style
            {
                ...
                for (int i = (int)dwFontFamily->GetFontCount(); --i >= 0;)
                {
                    ...
                    if (font.getTypefaceStyle() == getLocalisedName(faceNames)) break;
                }
        ...
        float getStringWidth(const String& text)
        {
            ...
            float x = 0;
            for (size_t i = 0; i < len; ++i) x += (float)dwGlyphMetrics[i].advanceWidth / designUnitsPerEm;
        ...
        bool getOutlineForGlyph(int glyphNumber, Path& path)
        {
            ...
            if (!pathTransform.isIdentity()) path.applyTransform(pathTransform);
        ...
        float getUnitsToHeightScaleFactor() const noexcept
        {
            return unitsToHeightScaleFactor;
        }
    ...
    private:
        ...
        {
            PathGeometrySink()
                : ComBaseClassHelper<IDWriteGeometrySink>(0)
            {
            }
            ...
            void __stdcall AddBeziers(const D2D1_BEZIER_SEGMENT* beziers, UINT beziersCount) override
            {
                for (UINT i = 0; i < beziersCount; ++i)
                    path.cubicTo(convertPoint(beziers[i].point1), convertPoint(beziers[i].point2),
                                 convertPoint(beziers[i].point3));
            }

            void __stdcall AddLines(const D2D1_POINT_2F* points, UINT pointsCount) override
            {
                for (UINT i = 0; i < pointsCount; ++i) path.lineTo(convertPoint(points[i]));
            }
