...
namespace juce
{
    ...
    */
    namespace TTFNameExtractor
    {
        ...
        static String parseNameRecord(MemoryInputStream& input, const NameRecord& nameRecord, const int64 directoryOffset,
                                      const int64 offsetOfStringStorage)
        {
            String      result;
            const int64 oldPos = input.getPosition();
            input.setPosition(directoryOffset + offsetOfStringStorage
                              + ByteOrder::swapIfLittleEndian(nameRecord.offsetFromStorageArea));
            ...
            if (platformID == 0 || platformID == 3)
            {
                ...
                for (int i = 0; i < numChars; ++i) buffer[i] = ByteOrder::swapIfLittleEndian(buffer[i]);
        ...
        static String parseNameTable(MemoryInputStream& input, int64 directoryOffset)
        {
            ...
            for (int i = 0; i < (int)ByteOrder::swapIfLittleEndian(namingTable.numberOfNameRecords); ++i)
            {
                ...
                {
                    const String result(
                        parseNameRecord(input, nameRecord, directoryOffset,
    ...
    StringArray Font::findAllTypefaceNames()
    {
        ...
        if (factories->systemFonts != nullptr)
        {
            ...
            for (uint32 i = 0; i < fontFamilyCount; ++i)
            {
                ...
                if (SUCCEEDED(hr)) results.addIfNotAlreadyThere(getFontFamilyName(fontFamily));
            }
        }
        else
#endif
        {
...

            {
                ...
                EnumFontFamiliesEx(dc, &lf, (FONTENUMPROCW)&FontEnumerators::fontEnum1, (LPARAM)&results, 0);
            }
    ...
    struct DefaultFontNames
    {
        DefaultFontNames()
        {
            ...
            else
            {
                ...
                defaultSerif    = L"宋体";
                defaultFixed    = L"宋体";
                defaultFallback = L"宋体";  // (contains plenty of unicode characters)
            }
        }
    ...
    Typeface::Ptr Font::getDefaultTypefaceForFont(const Font& font)
    {
        ...
        if (faceName == getDefaultSansSerifFontName())
            newFont.setTypefaceName(defaultNames.defaultSans);
        else if (faceName == getDefaultSerifFontName())
            newFont.setTypefaceName(defaultNames.defaultSerif);
        else if (faceName == getDefaultMonospacedFontName())
            newFont.setTypefaceName(defaultNames.defaultFixed);

        if (font.getTypefaceStyle() == getDefaultStyle()) newFont.setTypefaceStyle("Regular");
    ...
    public:
            ...
            , defaultGlyph(-1)
        {
            ...
            memoryFont         = AddFontMemResourceEx(const_cast<void*>(data), (DWORD)dataSize, nullptr, &numInstalled);
        ...
        {
            SelectObject(
                dc, previousFontH);  // Replacing the previous font before deleting the DC avoids a warning in BoundsChecker
            DeleteDC(dc);

            if (fontH != 0) DeleteObject(fontH);

            if (memoryFont != 0) RemoveFontMemResourceEx(memoryFont);
        }
        ...
        float getStringWidth(const String& text)
        {
            ...
            if (GetGlyphIndices(dc, utf16, (int)numChars, reinterpret_cast<WORD*>(results.getData()),
                ...
                != GDI_ERROR)
            {
                for (size_t i = 0; i < numChars; ++i) x += getKerning(dc, results[i], results[i + 1]);
            }
        ...
        {
            if (glyphNumber < 0) glyphNumber = defaultGlyph;

            GLYPHMETRICS gm;
            // (although GetGlyphOutline returns a DWORD, it may be -1 on failure, so treat it as signed int..)
            const int bufSize =
                (int)GetGlyphOutline(dc, (UINT)glyphNumber, GGO_NATIVE | GGO_GLYPH_INDEX, &gm, 0, 0, &identityMatrix);

            if (bufSize > 0)
            {
                HeapBlock<char> data(bufSize);
                GetGlyphOutline(dc, (UINT)glyphNumber, GGO_NATIVE | GGO_GLYPH_INDEX, &gm, bufSize, data, &identityMatrix);
                ...
                {
                    glyphPath.startNewSubPath(scaleX * pheader->pfxStart.x.value, scaleY * pheader->pfxStart.y.value);
                    ...
                    while ((const char*)curve < curveEnd)
                    {
                        if (curve->wType == TT_PRIM_LINE)
                        {
                            for (int i = 0; i < curve->cpfx; ++i)
                                glyphPath.lineTo(scaleX * curve->apfx[i].x.value, scaleY * curve->apfx[i].y.value);
                        }
    ...
    private:
        ...
        static int getGlyphForChar(HDC dc, juce_wchar character)
        {
            ...
            if (GetGlyphIndices(dc, charToTest, 1, &index, GGI_MARK_NONEXISTING_GLYPHS) == GDI_ERROR || index == 0xffff)
                return -1;
    ...
    Typeface::Ptr Typeface::createSystemTypefaceFor(const Font& font)
    {
        ...
        if (factories->systemFonts != nullptr)
        {
            ...
            if (wtf->loadedOk()) return wtf.release();
        }
#endif
