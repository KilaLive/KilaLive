...
namespace juce
{
    ...
    namespace DirectWriteTypeLayout
    {
        ...
        public:
            ...
            {
                if (refId == __uuidof(IDWritePixelSnapping)) return castToType<IDWritePixelSnapping>(result);
            ...
            JUCE_COMRESULT DrawGlyphRun(void* clientDrawingContext, FLOAT baselineOriginX, FLOAT baselineOriginY,
                                        DWRITE_MEASURING_MODE, DWRITE_GLYPH_RUN const* glyphRun,
                                        DWRITE_GLYPH_RUN_DESCRIPTION const* runDescription,
                                        IUnknown*                           clientDrawingEffect) override
            {
                ...
                TextLayout::Run* const glyphRunLayout = new TextLayout::Run(
                    Range<int>(runDescription->textPosition, runDescription->textPosition + runDescription->stringLength),
                    glyphRun->glyphCount);
                ...
                const float extraKerning = glyphRunLayout->font.getExtraKerningFactor() * glyphRunLayout->font.getHeight();
                ...
                for (UINT32 i = 0; i < glyphRun->glyphCount; ++i)
                {
                    ...
                    glyphRunLayout->glyphs.add(TextLayout::Glyph(glyphRun->glyphIndices[i],
                                                                 Point<float>(x, baselineOriginY - lineOrigin.y), advance));
        ...
        private:
            ...
            {
                if (d2dBrush == nullptr) return Colours::black;
        ...
        static float getFontHeightToEmSizeFactor(IDWriteFont& dwFont)
        {
            ...
            if (dwFontFace == nullptr) return 1.0f;
        ...
        void addAttributedRange(const AttributedString::Attribute& attr, IDWriteTextLayout& textLayout, const int textLen,
                                ID2D1RenderTarget& renderTarget, IDWriteFontCollection& fontCollection)
        {
...

            {
                ...
                for (int i = fontFacesCount; --i >= 0;)
                {
                    ...
                    if (attr.font.getTypefaceStyle() == getFontFaceName(dwFont)) break;
                }
...

            {
                ...
                renderTarget.CreateSolidColorBrush(
                    D2D1::ColorF(col.getFloatRed(), col.getFloatGreen(), col.getFloatBlue(), col.getFloatAlpha()),
                         ...
                         IDWriteFontCollection& fontCollection, ComSmartPtr<IDWriteTextLayout>& textLayout)
        {
            ...
            ComSmartPtr<IDWriteFont> dwFont;
            hr = dwFontFamily->GetFirstMatchingFont(DWRITE_FONT_WEIGHT_NORMAL, DWRITE_FONT_STRETCH_NORMAL,
                                                    DWRITE_FONT_STYLE_NORMAL, dwFont.resetAndGetPointerAddress());
            jassert(dwFont != nullptr);
            ...
            hr = directWriteFactory.CreateTextFormat(defaultFont.getTypefaceName().toWideCharPointer(), &fontCollection,
                                                     ...
                                                     defaultFont.getHeight() * defaultFontHeightToEmSizeFactor, L"en-us",
            ...
            hr = directWriteFactory.CreateTextLayout(text.getText().toWideCharPointer(), textLen, dwTextFormat, maxWidth,
                                                     maxHeight, textLayout.resetAndGetPointerAddress());

            if (FAILED(hr) || textLayout == nullptr) return false;
        ...
        void createLayout(TextLayout& layout, const AttributedString& text, IDWriteFactory& directWriteFactory,
                          IDWriteFontCollection& fontCollection, ID2D1DCRenderTarget& renderTarget)
        {
            ComSmartPtr<IDWriteTextLayout> dwTextLayout;

            if (!setupLayout(text, layout.getWidth(), layout.getHeight(), renderTarget, directWriteFactory, fontCollection,
                             dwTextLayout))
                return;
            ...
            {
                ComSmartPtr<CustomDirectWriteTextRenderer> textRenderer(
                    new CustomDirectWriteTextRenderer(fontCollection, text));
                              ...
                              IDWriteFactory& directWriteFactory, IDWriteFontCollection& fontCollection)
        {
            ...
            if (setupLayout(text, area.getWidth(), area.getHeight(), renderTarget, directWriteFactory, fontCollection,
                            dwTextLayout))
            {
                ComSmartPtr<ID2D1SolidColorBrush> d2dBrush;
                ...
                renderTarget.DrawTextLayout(D2D1::Point2F((float)area.getX(), (float)area.getY()), dwTextLayout, d2dBrush,
    ...
    static bool canAllTypefacesBeUsedInLayout(const AttributedString& text)
    {
        ...
        for (int i = 0; i < numCharacterAttributes; ++i)
            if (dynamic_cast<WindowsDirectWriteTypeface*>(text.getAttribute(i).font.getTypeface()) == nullptr) return false;
    ...
    {
#if JUCE_USE_DIRECTWRITE
        if (!canAllTypefacesBeUsedInLayout(text)) return false;

        SharedResourcePointer<Direct2DFactories> factories;

        if (factories->d2dFactory != nullptr && factories->systemFonts != nullptr)
        {
            DirectWriteTypeLayout::createLayout(*this, text, *factories->directWriteFactory, *factories->systemFonts,
