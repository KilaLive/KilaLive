...
#undef JNI_CLASS_MEMBERS
    ...
    public:
            ...
            , heightToPointsFactor(1.0f)
        {
            ...
            typeface = GlobalRef(
                android.activity.callObjectMethod(JuceAppActivity.getTypeFaceFromAsset, javaString("fonts/" + name).get()));
            ...
            if (typeface.get() == nullptr)
            {
                ...
                if (!fontFile.exists()) fontFile = findFontFile(name, isBold, isItalic);
        ...
        float getStringWidth(const String& text) override
        {
            ...
            float x = 0;
            for (int i = 0; i < numDone; ++i) x += localWidths[i];
            ...
            for (int i = 0; i < numDone; ++i)
            {
                ...
                if (local == 0.0f && glyphs.size() > 0 && getStringWidth(String(ch)) > 0.0f)
                {
                    ...
                    if (glyphNumber < std::numeric_limits<jchar>::max())
                    {
                        ...
                        const unsigned int thisGlyph = static_cast<unsigned int>(ch) & ((1U << (sizeof(jchar) * 8U)) - 1U);
        ...
        EdgeTable* getEdgeTableForGlyph(int glyphNumber, const AffineTransform& t, float /*fontHeight*/) override
        {
            ...
            jchar ch2 =
                (static_cast<unsigned int>(glyphNumber) >> (sizeof(jchar) * 8U)) & ((1U << (sizeof(jchar) * 8U)) - 1U);
#else
            ...
            jintArray maskData = (jintArray)android.activity.callObjectMethod(JuceAppActivity.renderGlyph, ch1, ch2,
                                                                              paint.get(), matrix, rect.get());
    ...
    private:
        static File findFontFile(const String& family, const bool bold, const bool italic)
        {
            File file;
            ...
            if (!file.exists()) file = getFontFile(family, String());
        ...
        static File getFontFile(const String& family, const String& style)
        {
            ...
            if (style.isNotEmpty()) path << '-' << style;
