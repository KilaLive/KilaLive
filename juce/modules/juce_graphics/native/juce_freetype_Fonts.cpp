...
namespace juce
{
    ...
    struct FTFaceWrapper : public ReferenceCountedObject
    {
        FTFaceWrapper(const FTLibWrapper::Ptr& ftLib, const File& file, int faceIndex)
            ...
            , library(ftLib)
        {
            if (FT_New_Face(ftLib->library, file.getFullPathName().toUTF8(), faceIndex, &face) != 0) face = 0;
        }

        FTFaceWrapper(const FTLibWrapper::Ptr& ftLib, const void* data, size_t dataSize, int faceIndex)
            : face(0)
            , library(ftLib)
            , savedFaceData(data, dataSize)
        {
            if (FT_New_Memory_Face(ftLib->library, (const FT_Byte*)savedFaceData.getData(), (FT_Long)savedFaceData.getSize(),
                                   faceIndex, &face)
                != 0)
                face = 0;
        }
    ...
    public:
        ...
        FTFaceWrapper::Ptr createFace(const String& fontName, const String& fontStyle)
        {
            ...
            if (ftFace != nullptr) return createFace(ftFace->file, ftFace->faceIndex);
        ...
        StringArray findAllFamilyNames() const
        {
            ...
            for (int i = 0; i < faces.size(); ++i) s.addIfNotAlreadyThere(faces.getUnchecked(i)->family);
        ...
        static int indexOfRegularStyle(const StringArray& styles)
        {
            ...
            if (i < 0)
                for (i = 0; i < styles.size(); ++i)
                    if (!(styles[i].containsIgnoreCase("Bold") || styles[i].containsIgnoreCase("Italic"))) break;
        ...
        StringArray findAllTypefaceStyles(const String& family) const
        {
            ...
            for (int i = 0; i < faces.size(); ++i)
            {
                ...
                if (face->family == family) s.addIfNotAlreadyThere(face->style);
            }

            // try to get a regular style to be first in the list
            const int regular = indexOfRegularStyle(s);
            if (regular > 0) s.strings.swap(0, regular);
        ...
        void scanFontPaths(const StringArray& paths)
        {
            ...
            {
                DirectoryIterator iter(File::getCurrentWorkingDirectory().getChildFile(paths[i]), true);

                while (iter.next())
                    if (iter.getFile().hasFileExtension("ttf;pfb;pcf;otf")) scanFont(iter.getFile());
            }
        }
        ...
        {
            for (int i = 0; i < faces.size(); ++i)
                if (faces.getUnchecked(i)->isMonospaced) monoSpaced.addIfNotAlreadyThere(faces.getUnchecked(i)->family);
        }

        void getSerifNames(StringArray& serif) const
        {
            for (int i = 0; i < faces.size(); ++i)
                if (!faces.getUnchecked(i)->isSansSerif) serif.addIfNotAlreadyThere(faces.getUnchecked(i)->family);
        }

        void getSansSerifNames(StringArray& sansSerif) const
        {
            for (int i = 0; i < faces.size(); ++i)
                if (faces.getUnchecked(i)->isSansSerif) sansSerif.addIfNotAlreadyThere(faces.getUnchecked(i)->family);
        }
    ...
    private:
        ...
        void scanFont(const File& file)
        {
            ...
            do
            {
                ...
                if (face.face != 0)
                {
                    if (faceIndex == 0) numFaces = (int)face.face->num_faces;
        ...
        const KnownTypeface* matchTypeface(const String& familyName, const String& style) const noexcept
        {
            for (int i = 0; i < faces.size(); ++i)
            {
                ...
                if (face->family == familyName && (face->style.equalsIgnoreCase(style) || style.isEmpty())) return face;
            }
        ...
        static bool isFaceSansSerif(const String& family)
        {
            ...
            for (int i = 0; i < numElementsInArray(sansNames); ++i)
                if (family.containsIgnoreCase(sansNames[i])) return true;
    ...
    public:
        FreeTypeTypeface(const Font& font)
            : faceWrapper(FTTypefaceList::getInstance()->createFace(font.getTypefaceName(), font.getTypefaceStyle()))
        {
            if (faceWrapper != nullptr) initialiseCharacteristics(font.getTypefaceName(), font.getTypefaceStyle());
        }
            ...
            : faceWrapper(FTTypefaceList::getInstance()->createFace(data, dataSize, 0))
        {
            if (faceWrapper != nullptr)
                initialiseCharacteristics(faceWrapper->face->family_name, faceWrapper->face->style_name);
        }
        ...
        void initialiseCharacteristics(const String& fontName, const String& fontStyle)
        {
            setCharacteristics(
                fontName, fontStyle,
                faceWrapper->face->ascender / (float)(faceWrapper->face->ascender - faceWrapper->face->descender), L' ');
        }
        ...
        bool loadGlyphIfPossible(const juce_wchar character)
        {
            if (faceWrapper != nullptr)
            {
                ...
                if (FT_Load_Glyph(face, glyphIndex,
                                  FT_LOAD_NO_SCALE | FT_LOAD_NO_BITMAP | FT_LOAD_IGNORE_TRANSFORM | FT_LOAD_NO_HINTING)
                        == 0
                    && face->glyph->format == ft_glyph_format_outline)
                {
                    ...
                    if (getGlyphShape(destShape, face->glyph->outline, scale))
                    {
                        ...
                        if ((face->face_flags & FT_FACE_FLAG_KERNING) != 0) addKerning(face, (uint32)character, glyphIndex);
    ...
    private:
        ...
        bool getGlyphShape(Path& destShape, const FT_Outline& outline, const float scaleX)
        {
            ...
            for (int c = 0; c < outline.n_contours; ++c)
            {
                ...
                for (int p = startPoint; p <= endPoint; ++p)
                {
                    ...
                    {
                        if (p != startPoint) destShape.lineTo(x, y);
                    }
                    ...
                    else if (FT_CURVE_TAG(tags[p]) == FT_Curve_Tag_Cubic)
                    {
                        ...
                        if (p >= endPoint || FT_CURVE_TAG(tags[next1]) != FT_Curve_Tag_Cubic
        ...
        void addKerning(FT_Face face, const uint32 character, const uint32 glyphIndex)
        {
            ...
            while (rightGlyphIndex != 0)
            {
                ...
                if (FT_Get_Kerning(face, glyphIndex, rightGlyphIndex, ft_kerning_unscaled, &kerning) == 0 && kerning.x != 0)
