...
namespace juce
{
    ...
    static Image getIconFromIcnsFile(const File& icnsFile, const int size)
    {
        ...
        if (stream.read(headerSection, numHeaderSectionBytes) != numHeaderSectionBytes || headerSection[0] != 'i'
            || headerSection[1] != 'c' || headerSection[2] != 'n' || headerSection[3] != 's')
            return {};

        if (stream.read(headerSection, numHeaderSectionBytes) != numHeaderSectionBytes) return {};
        ...
        while (stream.getPosition() < dataSize)
        {
            ...
            if (!stream.setPosition(sectionStart + 4)) break;

            if (stream.read(headerSection, numHeaderSectionBytes) != numHeaderSectionBytes) break;
        ...
        return maxWidthIndex == -1 ? juce::Image()
                                   : images.getReference(maxWidthIndex)
                                         .rescaled(size, size, Graphics::ResamplingQuality::highResamplingQuality);
    }
    ...
    Image JUCE_API getIconFromApplication(const String& applicationPath, const int size)
    {
        ...
        if (CFStringRef pathCFString =
                CFStringCreateWithCString(kCFAllocatorDefault, applicationPath.toRawUTF8(), kCFStringEncodingUTF8))
        {
                ...
                if (CFBundleRef appBundle = CFBundleCreate(kCFAllocatorDefault, url))
                {
                    if (CFTypeRef infoValue = CFBundleGetValueForInfoDictionaryKey(appBundle, CFSTR("CFBundleIconFile")))
                    {
                        if (CFGetTypeID(infoValue) == CFStringGetTypeID())
                        {
                            ...
                            CFStringRef resourceURLSuffix =
                                CFStringHasSuffix(iconFilename, CFSTR(".icns")) ? nullptr : CFSTR("icns");
                            if (CFURLRef iconURL =
                                    CFBundleCopyResourceURL(appBundle, iconFilename, resourceURLSuffix, nullptr))
                            {
