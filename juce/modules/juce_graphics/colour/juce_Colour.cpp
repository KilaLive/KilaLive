...
namespace juce
{
    namespace ColourHelpers
    {
        ...
        struct HSB
        {
            ...
            static PixelARGB toRGB(float h, float s, float v, const uint8 alpha) noexcept
            {
                ...
                if (s <= 0) return PixelARGB(alpha, intV, intV, intV);
        ...
        struct YIQ
        {
            ...
            {
                return Colour::fromFloatRGBA(y + 0.9563f * i + 0.6210f * q, y - 0.2721f * i - 0.6474f * q,
                                             y - 1.1070f * i + 1.7046f * q, alpha);
            }
    ...
    {
        return Colour(ColourHelpers::floatToUInt8(red), ColourHelpers::floatToUInt8(green), ColourHelpers::floatToUInt8(blue),
                      alpha);
    }
    ...
    Colour Colour::overlaidWith(Colour src) const noexcept
    {
        ...
        return Colour((uint8)(src.getRed() + ((((int)getRed() - src.getRed()) * da) >> 8)),
                      ...
                      (uint8)(src.getBlue() + ((((int)getBlue() - src.getBlue()) * da) >> 8)), (uint8)resA);
    }

    Colour Colour::interpolatedWith(Colour other, float proportionOfOther) const noexcept
    {
        if (proportionOfOther <= 0) return *this;

        if (proportionOfOther >= 1.0f) return other;
    ...
    {
        return std::sqrt(0.241f * square(getFloatRed()) + 0.691f * square(getFloatGreen()) + 0.068f * square(getFloatBlue()));
    }
    ...
    Colour Colour::brighter(float amount) const noexcept
    {
        ...
        return Colour((uint8)(255 - (amount * (255 - getRed()))), (uint8)(255 - (amount * (255 - getGreen()))),
                      (uint8)(255 - (amount * (255 - getBlue()))), getAlpha());
    }
    ...
    Colour Colour::darker(float amount) const noexcept
    {
        ...
        return Colour((uint8)(amount * getRed()), (uint8)(amount * getGreen()), (uint8)(amount * getBlue()), getAlpha());
    }
    ...
    Colour Colour::contrasting(Colour target, float minContrast) const noexcept
    {
        ...
        if (std::abs(bg.y - fg.y) >= minContrast) return target;
    ...
    Colour Colour::contrasting(Colour colour1, Colour colour2) noexcept
    {
        ...
        return colour1.overlaidWith(colour2.withMultipliedAlpha(0.5f)).withBrightness(best);
    }
