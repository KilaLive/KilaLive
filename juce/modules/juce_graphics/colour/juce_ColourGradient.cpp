...
#define JUCE_COLOURGRADIENT_CHECK_COORDS_INITIALISED
    ...
    ColourGradient::ColourGradient(Colour colour1, float x1, float y1, Colour colour2, float x2, float y2, bool radial)
        : ColourGradient(colour1, Point<float>(x1, y1), colour2, Point<float>(x2, y2), radial)
    {
    }

    ColourGradient::ColourGradient(Colour colour1, Point<float> p1, Colour colour2, Point<float> p2, bool radial)
        : point1(p1)
        , point2(p2)
        , isRadial(radial)
    {
        colours.add(ColourPoint{0.0, colour1}, ColourPoint{1.0, colour2});
    }
    ...
    {
        return point1 == other.point1 && point2 == other.point2 && isRadial == other.isRadial && colours == other.colours;
    }
    ...
    int ColourGradient::addColour(const double proportionAlongGradient, Colour colour)
    {
        ...
        for (i = 0; i < colours.size(); ++i)
            if (colours.getReference(i).position > pos) break;
    ...
    {
        for (auto& c : colours) c.colour = c.colour.withMultipliedAlpha(multiplier);
    }
    ...
    {
        if (isPositiveAndBelow(index, colours.size())) return colours.getReference(index).position;
    ...
    {
        if (isPositiveAndBelow(index, colours.size())) return colours.getReference(index).colour;
    ...
    {
        if (isPositiveAndBelow(index, colours.size())) colours.getReference(index).colour = newColour;
    }
    ...
    Colour ColourGradient::getColourAtPosition(double position) const noexcept
    {
        ...
        if (position <= 0 || colours.size() <= 1) return colours.getReference(0).colour;

        int i = colours.size() - 1;
        while (position < colours.getReference(i).position) --i;

        auto& p1 = colours.getReference(i);

        if (i >= colours.size() - 1) return p1.colour;
    ...
    int ColourGradient::createLookupTable(const AffineTransform& transform, HeapBlock<PixelARGB>& lookupTable) const
    {
        ...
        auto numEntries = jlimit(1, jmax(1, (colours.size() - 1) << 8),
                                 3 * (int)point1.transformedBy(transform).getDistanceFrom(point2.transformedBy(transform)));
    ...
    {
        for (auto& c : colours)
            if (!c.colour.isOpaque()) return false;
    ...
    {
        for (auto& c : colours)
            if (!c.colour.isTransparent()) return false;
