        ...
        */
        struct ConvolutionEngine
        {
            ...
            void reset()
            {
                ...
                for (auto i = 0; i < buffersInputSegments.size(); ++i) buffersInputSegments.getReference(i).clear();
            ...
            void processSamples(const float* input, float* output, size_t numSamples)
            {
                ...
                while (numSamplesProcessed < numSamples)
                {
                    ...
                    FloatVectorOperations::copy(inputData + inputDataPos, input + numSamplesProcessed,
                                                static_cast<int>(numSamplesToProcess));

                    auto* inputSegmentData =
                        buffersInputSegments.getReference(static_cast<int>(currentSegment)).getWritePointer(0);
                    ...
                    if (inputDataWasEmpty)
                    {
                        ...
                        for (size_t i = 1; i < numSegments; ++i)
                        {
                            ...
                            if (index >= numInputSegments) index -= numInputSegments;

                            convolutionProcessingAndAccumulate(
                                buffersInputSegments.getReference(static_cast<int>(index)).getWritePointer(0),
                                buffersImpulseSegments.getReference(static_cast<int>(i)).getWritePointer(0), outputTempData);
                        }
                    }
                    ...
                    convolutionProcessingAndAccumulate(
                        buffersInputSegments.getReference(static_cast<int>(currentSegment)).getWritePointer(0),
                        buffersImpulseSegments.getReference(0).getWritePointer(0), outputData);
                    ...
                    if (inputDataPos == blockSize)
                    {
                        ...
                        FloatVectorOperations::add(&(outputData[blockSize]), &(overlapData[blockSize]),
            ...
            void prepareForConvolution(float* samples) noexcept
            {
                ...
                for (size_t i = 0; i < FFTSizeDiv2; i++) samples[i] = samples[2 * i];

                samples[FFTSizeDiv2] = 0;

                for (size_t i = 1; i < FFTSizeDiv2; i++) samples[i + FFTSizeDiv2] = -samples[2 * (FFTSize - i) + 1];
            }
            ...
            void convolutionProcessingAndAccumulate(const float* input, const float* impulse, float* output)
            {
                ...
                FloatVectorOperations::subtractWithMultiply(output, &(input[FFTSizeDiv2]), &(impulse[FFTSizeDiv2]),
                                                            static_cast<int>(FFTSizeDiv2));

                FloatVectorOperations::addWithMultiply(&(output[FFTSizeDiv2]), input, &(impulse[FFTSizeDiv2]),
                                                       static_cast<int>(FFTSizeDiv2));
                FloatVectorOperations::addWithMultiply(&(output[FFTSizeDiv2]), &(input[FFTSizeDiv2]), impulse,
                                                       static_cast<int>(FFTSizeDiv2));
        ...
        */
        struct Convolution::Pimpl : private Thread
        {
                ...
                , abstractFifo(fifoSize)
            {
                ...
                for (auto i = 0; i < 4; ++i) engines.add(new ConvolutionEngine());
            ...
            */
            void processFifo()
            {
                if (getNumRemainingEntries() == 0 || isThreadRunning() || mustInterpolate) return;
                ...
                for (auto n = 0; n < numRequests; ++n)
                {
                    ...
                    case ChangeRequest::changeSampleRate:
                    {
                        ...
                        if (currentInfo.sampleRate != newSampleRate) changeLevel = 3;
                    ...
                    case ChangeRequest::changeMaximumBufferSize:
                    {
                        ...
                        if (currentInfo.maximumBufferSize != (size_t)newMaximumBufferSize) changeLevel = 3;
                    ...
                    case ChangeRequest::changeSource:
                    {
                        ...
                        if (currentInfo.sourceType != newSourceType) changeLevel = jmax(2, changeLevel);
                        ...
                        else if (newSourceType == SourceType::sourceAudioFile)
                        {
                            ...
                            if (currentInfo.fileImpulseResponse != newFile) changeLevel = jmax(2, changeLevel);
                    ...
                    case ChangeRequest::changeImpulseResponseSize:
                    {
                        ...
                        if (currentInfo.wantedSize != newSize) changeLevel = jmax(1, changeLevel);
                    ...
                    case ChangeRequest::changeStereo:
                    {
                        ...
                        if (currentInfo.wantsStereo != newWantsStereo) changeLevel = jmax(0, changeLevel);
                    ...
                    case ChangeRequest::changeTrimming:
                    {
                        ...
                        if (currentInfo.wantsTrimming != newWantsTrimming) changeLevel = jmax(1, changeLevel);
                    ...
                    case ChangeRequest::changeNormalization:
                    {
                        ...
                        if (currentInfo.wantsNormalization != newWantsNormalization) changeLevel = jmax(1, changeLevel);
                ...
                if (currentInfo.sourceType == SourceType::sourceNone)
                {
                    ...
                    if (currentInfo.sampleRate == 0) currentInfo.sampleRate = 44100;

                    if (currentInfo.maximumBufferSize == 0) currentInfo.maximumBufferSize = 128;
            ...
            */
            void copyBufferToTemporaryLocation(dsp::AudioBlock<float> block)
            {
                ...
                for (auto channel = 0; channel < currentInfo.originalNumChannels; ++channel)
                    temporaryBuffer.copyFrom(channel, 0, block.getChannelPointer((size_t)channel),
            ...
            void processSamples(const AudioBlock<float>& input, AudioBlock<float>& output)
            {
                ...
                if (mustInterpolate == false)
                {
                    for (size_t channel = 0; channel < numChannels; ++channel)
                        engines[(int)channel]->processSamples(input.getChannelPointer(channel),
                                                              output.getChannelPointer(channel), numSamples);
                }
                else
                {
                    ...
                    for (size_t channel = 0; channel < numChannels; ++channel)
                    {
                        ...
                        engines[(int)channel]->processSamples(input.getChannelPointer(channel), buffer.getChannelPointer(0),
                                                              numSamples);
        ...
        private:
            ...
            */
            void run() override
            {
                if (changeLevel == 2)
                {
                    ...
                    if (isThreadRunning() && threadShouldExit()) return;
                }
            ...
            void loadImpulseResponse()
            {
                ...
                {
                    if (!(copyAudioStreamInAudioBuffer(
                            new MemoryInputStream(currentInfo.sourceData, (size_t)currentInfo.sourceDataSize, false))))
                        return;
                }
                else if (currentInfo.sourceType == SourceType::sourceAudioFile)
                {
                    if (!(copyAudioStreamInAudioBuffer(new FileInputStream(currentInfo.fileImpulseResponse)))) return;
                }
            ...
            */
            ...
            {
                trimAndResampleImpulseResponse(currentInfo.originalNumChannels, currentInfo.originalSampleRate,
                                               currentInfo.wantsTrimming);
            ...
            */
            bool copyAudioStreamInAudioBuffer(InputStream* stream)
            {
                ...
                if (ScopedPointer<AudioFormatReader> formatReader = manager.createReaderFor(stream))
                {
                    ...
                    formatReader->read(&(impulseResponseOriginal), 0, (int)currentInfo.originalSize, 0, true,
                                       currentInfo.originalNumChannels > 1);
            ...
            */
            ...
            void trimAndResampleImpulseResponse(int numChannels, double srcSampleRate, bool mustTrim)
            {
                ...
                if (mustTrim)
                {
                    ...
                    for (auto channel = 0; channel < numChannels; ++channel)
                    {
                        ...
                        while (localIndexStart < currentInfo.originalSize - 1 && channelData[localIndexStart] <= thresholdTrim
                               && channelData[localIndexStart] >= -thresholdTrim)
                            ++localIndexStart;

                        while (localIndexEnd >= 0 && channelData[localIndexEnd] <= thresholdTrim
                    ...
                    if (indexStart > 0)
                    {
                        for (auto channel = 0; channel < numChannels; ++channel)
                        {
                            ...
                            for (auto i = 0; i < indexEnd - indexStart + 1; ++i) channelData[i] = channelData[i + indexStart];
                ...
                else
                {
                    ...
                    currentInfo.finalSize = jmin(static_cast<int>(currentInfo.wantedSize),
                                                 roundToInt((indexEnd - indexStart + 1) / factorReading));
                ...
                if (numChannels == 1) impulseResponse.copyFrom(1, 0, impulseResponse, 0, 0, (int)currentInfo.finalSize);
            }
            ...
            void normalizeImpulseResponse(float* samples, int numSamples, double factorResampling) const
            {
                ...
                for (auto i = 0; i < numSamples; ++i) magnitude += samples[i] * samples[i];

                auto magnitudeInv = 1.0f / (4.0f * std::sqrt(magnitude)) * 0.5f * static_cast<float>(factorResampling);

                for (auto i = 0; i < numSamples; ++i) samples[i] *= magnitudeInv;
            }
            ...
            */
            ...
            {
                if (currentInfo.maximumBufferSize == 0) return;

                if (changeLevel == 3)
                {
                    for (auto i = 0; i < 2; ++i) engines[i]->initializeConvolutionEngine(currentInfo, i);
            ...
            AudioBuffer<float> temporaryBuffer;  // a temporary buffer that is used when the function
                                                 // copyAndLoadImpulseResponse is called in the main API
            ...
            AudioBuffer<float>
                impulseResponse;  // a buffer with the impulse response trimmed, resampled, resized and normalized

            //==============================================================================
            OwnedArray<ConvolutionEngine> engines;  // the 4 convolution engines being used

            AudioBuffer<float>
                                       interpolationBuffer;  // a buffer to do the interpolation between the convolution engines 0-1 and 2-3
        ...
        void Convolution::loadImpulseResponse(const void* sourceData, size_t sourceDataSize, bool wantsStereo,
                                              bool wantsTrimming, size_t size, bool wantsNormalization)
        {
            ...
            Pimpl::ChangeRequest types[] = {Pimpl::ChangeRequest::changeSource,
                                            ...
                                            Pimpl::ChangeRequest::changeStereo, Pimpl::ChangeRequest::changeTrimming,
            ...
            juce::var parameters[] = {juce::var(sourceParameter), juce::var(static_cast<int64>(wantedSize)),
                                      juce::var(wantsStereo), juce::var(wantsTrimming), juce::var(wantsNormalization)};

            pimpl->addToFifo(types, parameters, 5);
        }

        void Convolution::loadImpulseResponse(const File& fileImpulseResponse, bool wantsStereo, bool wantsTrimming,
                                              size_t size, bool wantsNormalization)
        {
            if (!fileImpulseResponse.existsAsFile()) return;
        ...
        void Convolution::copyAndLoadImpulseResponseFromBuffer(AudioBuffer<float>& buffer, double bufferSampleRate,
                                                               bool wantsStereo, bool wantsTrimming, bool wantsNormalization,
                                                               size_t size)
        {
            copyAndLoadImpulseResponseFromBlock(AudioBlock<float>(buffer), bufferSampleRate, wantsStereo, wantsTrimming,
                                                wantsNormalization, size);
        }
        ...
        void Convolution::prepare(const ProcessSpec& spec)
        {
            ...
            juce::var parameters[] = {juce::var(spec.sampleRate), juce::var(static_cast<int>(spec.maximumBlockSize))};
            ...
            dryBuffer  = AudioBlock<float>(dryBufferStorage, jmin(spec.numChannels, 2u), spec.maximumBlockSize);
        ...
        void Convolution::processSamples(const AudioBlock<float>& input, AudioBlock<float>& output, bool isBypassed) noexcept
        {
            ...
            {
                if (!currentIsBypassed) pimpl->processSamples(input, output);
