...
namespace juce
{
    namespace dsp
    {
        ...
        {
            return std::cos(static_cast<FloatType>(order * i) * MathConstants<FloatType>::pi
        ...
        template <typename FloatType>
        void WindowingFunction<FloatType>::fillWindowingTables(size_t size, WindowingMethod type, bool normalize,
        ...
        template <typename FloatType>
        void WindowingFunction<FloatType>::fillWindowingTables(FloatType* samples, size_t size, WindowingMethod type,
                                                               bool normalize, FloatType beta) noexcept
        {
            ...
            case rectangular:
            {
                for (size_t i = 0; i < size; ++i) samples[i] = static_cast<FloatType>(1);
            }
            break;
                ...
                for (size_t i = 0; i < size; ++i)
                    samples[i] = static_cast<FloatType>(
                        SpecialFunctions::besselI0(
                            beta * std::sqrt(1.0 - std::pow((i - 0.5 * (size - 1.0)) / (0.5 * (size - 1.0)), 2.0)))
                        * factor);
            }
            break;
            ...
            if (normalize)
            {
                ...
                for (size_t i = 0; i < size; ++i) sum += samples[i];
        ...
        {
            FloatVectorOperations::multiply(samples, windowTable.getRawDataPointer(),
