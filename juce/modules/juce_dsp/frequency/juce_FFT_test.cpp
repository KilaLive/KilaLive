        ...
        struct FFTUnitTest : public UnitTest
        {
            ...
            static void fillRandom(Random& random, Complex<float>* buffer, size_t n)
            {
                for (size_t i = 0; i < n; ++i)
                    buffer[i] = Complex<float>((2.0f * random.nextFloat()) - 1.0f, (2.0f * random.nextFloat()) - 1.0f);
            }
            ...
            static Complex<float> freqConvolution(const Complex<float>* in, float freq, size_t n)
            {
                ...
                for (size_t i = 0; i < n; ++i) sum += in[i] * exp(Complex<float>(0, static_cast<float>(i) * freq));
            ...
            {
                auto base_freq =
                    static_cast<float>(((reverse ? 1.0 : -1.0) * MathConstants<double>::twoPi) / static_cast<float>(n));

                for (size_t i = 0; i < n; ++i) out[i] = freqConvolution(in, static_cast<float>(i) * base_freq, n);
            }
            ...
            static void performReferenceFourier(const float* in, Complex<float>* out, size_t n, bool reverse)
            {
                ...
                for (size_t i = 0; i < n; ++i) buffer.getData()[i] = Complex<float>(in[i], 0.0f);
            ...
            {
                for (size_t i = 0; i < n; ++i)
                    if (std::abs(a[i] - b[i]) > 1e-3f) return false;
            ...
            struct FrequencyOnlyTest
            {
                static void run(FFTUnitTest& u)
                {
                    ...
                    for (size_t order = 0; order <= 8; ++order)
                    {
                        ...
                        for (size_t i = 0; i < n; ++i) reference.getData()[i] = std::abs(frequency.getData()[i]);
