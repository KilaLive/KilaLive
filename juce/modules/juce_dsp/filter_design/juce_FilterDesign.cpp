...
namespace juce
{
    namespace dsp
    {
        ...
        typename FIR::Coefficients<FloatType>::Ptr FilterDesign<FloatType>::designFIRLowpassWindowMethod(
            FloatType frequency, double sampleRate, size_t order, WindowingMethod type, FloatType beta)
        {
        ...
        template <typename FloatType>
        typename FIR::Coefficients<FloatType>::Ptr FilterDesign<FloatType>::designFIRLowpassKaiserMethod(
            FloatType frequency, double sampleRate, FloatType normalizedTransitionWidth, FloatType attenuationdB)
        {
            jassert(sampleRate > 0);
            jassert(frequency > 0 && frequency <= sampleRate * 0.5);
            ...
            int order = attenuationdB < -21
                            ? roundToInt(std::ceil((-attenuationdB - 7.95)
                                                   / (2.285 * normalizedTransitionWidth * MathConstants<double>::twoPi)))
        ...
        template <typename FloatType>
        typename FIR::Coefficients<FloatType>::Ptr FilterDesign<FloatType>::designFIRLowpassTransitionMethod(
            FloatType frequency, double sampleRate, size_t order, FloatType normalizedTransitionWidth, FloatType spline)
        {
            jassert(sampleRate > 0);
            jassert(frequency > 0 && frequency <= sampleRate * 0.5);
            jassert(normalizedTransitionWidth > 0 && normalizedTransitionWidth <= 0.5);
            ...
            for (size_t i = 0; i <= order; ++i)
            {
                ...
                else
                {
                    ...
                    c[i]         = static_cast<FloatType>(std::sin(2 * indice * normalizedFrequency) / indice
        ...
        template <typename FloatType>
        typename FIR::Coefficients<FloatType>::Ptr FilterDesign<FloatType>::designFIRLowpassLeastSquaresMethod(
            FloatType frequency, double sampleRate, size_t order, FloatType normalizedTransitionWidth,
            FloatType stopBandWeight)
        {
            ...
            auto wp =
                MathConstants<double>::twoPi * (static_cast<double>(normalizedFrequency - normalizedTransitionWidth / 2.0));
            auto ws =
                MathConstants<double>::twoPi * (static_cast<double>(normalizedFrequency + normalizedTransitionWidth / 2.0));
            ...
            if (N % 2 == 1)
            {
                ...
                auto sinc = [](double x) {
                    return x == 0 ? 1 : std::sin(x * MathConstants<double>::pi) / (MathConstants<double>::pi * x);
                };

                auto factorp = wp / MathConstants<double>::pi;
                auto factors = ws / MathConstants<double>::pi;

                for (size_t i = 0; i <= M; ++i) b(i, 0) = factorp * sinc(factorp * i);
            ...
            else
            {
                ...
                for (size_t i = 0; i < M; ++i) b(i, 0) = factorp * sinc(factorp * (i + 0.5));
        ...
        template <typename FloatType>
        typename FIR::Coefficients<FloatType>::Ptr FilterDesign<FloatType>::designFIRLowpassHalfBandEquirippleMethod(
            FloatType normalizedTransitionWidth, FloatType attenuationdB)
        {
            jassert(normalizedTransitionWidth > 0 && normalizedTransitionWidth <= 0.5);
            ...
            auto n =
                roundToInt(std::ceil((attenuationdB - 18.18840664 * wpT + 33.64775300) / (18.54155181 * wpT - 29.13196871)));
            ...
            for (int i = 0; i < hn.size(); ++i) hh.setUnchecked(i, A * hh[i] + B * hnm[i]);

            auto* result = new typename FIR::Coefficients<FloatType>(static_cast<size_t>(hh.size()));
            auto* c      = result->getRawCoefficients();

            for (int i = 0; i < hh.size(); ++i) c[i] = (float)hh[i];
            ...
            {
                auto w01 =
                    std::sqrt(kp * kp + (1 - kp * kp) * std::pow(std::cos(MathConstants<double>::pi / (2.0 * n + 1.0)), 2.0));
                auto om01 = std::acos(-w01);

                NN = -2.0 * result->getMagnitudeForFrequency(om01 / MathConstants<double>::twoPi, 1.0);
            }

            for (int i = 0; i < hh.size(); ++i) c[i] = static_cast<FloatType>((A * hn[i] + B * hnm[i]) / NN);
        ...
        Array<double> FilterDesign<FloatType>::getPartialImpulseResponseHn(int n, double kp)
        {
            ...
            if (n > 0) alpha.setUnchecked(2 * n - 2, -(2 * n * kp * kp + 1) * alpha[2 * n]);
            ...
            {
                auto c1 =
                    (3 * (n * (n + 2) - k * (k - 2)) + 2 * k - 3 + 2 * (k - 2) * (2 * k - 3) * kp * kp) * alpha[2 * k - 4];
                auto c2 = (3 * (n * (n + 2) - (k - 1) * (k + 1)) + 2 * (2 * k - 1) + 2 * k * (2 * k - 1) * kp * kp)
                          * alpha[2 * k - 2];
            ...
            for (int k = 0; k <= n; ++k) ai.setUnchecked(2 * k + 1, alpha[2 * k] / (2.0 * k + 1.0));
        ...
        template <typename FloatType>
        Array<IIR::Coefficients<FloatType>> FilterDesign<FloatType>::designIIRLowpassHighOrderButterworthMethod(
            FloatType frequency, double sampleRate, FloatType normalizedTransitionWidth, FloatType passbandAttenuationdB,
        ...
        template <typename FloatType>
        Array<IIR::Coefficients<FloatType>> FilterDesign<FloatType>::designIIRLowpassHighOrderChebyshev1Method(
            FloatType frequency, double sampleRate, FloatType normalizedTransitionWidth, FloatType passbandAttenuationdB,
            FloatType stopbandAttenuationdB)
        {
        ...
        template <typename FloatType>
        Array<IIR::Coefficients<FloatType>> FilterDesign<FloatType>::designIIRLowpassHighOrderChebyshev2Method(
            FloatType frequency, double sampleRate, FloatType normalizedTransitionWidth, FloatType passbandAttenuationdB,
            FloatType stopbandAttenuationdB)
        {
        ...
        template <typename FloatType>
        Array<IIR::Coefficients<FloatType>> FilterDesign<FloatType>::designIIRLowpassHighOrderEllipticMethod(
            FloatType frequency, double sampleRate, FloatType normalizedTransitionWidth, FloatType passbandAttenuationdB,
            FloatType stopbandAttenuationdB)
        {
        ...
        template <typename FloatType>
        Array<IIR::Coefficients<FloatType>> FilterDesign<FloatType>::designIIRLowpassHighOrderGeneralMethod(
            int type, FloatType frequency, double sampleRate, FloatType normalizedTransitionWidth,
            FloatType passbandAttenuationdB, FloatType stopbandAttenuationdB)
        {
            jassert(sampleRate > 0);
            jassert(frequency > 0 && frequency <= sampleRate * 0.5);
            jassert(normalizedTransitionWidth > 0 && normalizedTransitionWidth <= 0.5);
            ...
            if (type == 0)
            {
                if (r == 1) pa.add(-omegap * std::pow(epsp, -1.0 / (double)N));
            ...
            else if (type == 1)
            {
                ...
                if (r == 1) pa.add(-omegap * std::sinh(v0 * halfPi));
            ...
            else if (type == 2)
            {
                ...
                if (r == 1) pa.add(-1.0 / (k / omegap * std::sinh(v0 * halfPi)));
            ...
            else
            {
                ...
                if (r == 1) pa.add(omegap * j * SpecialFunctions::sne(j * v0, k));
            ...
            FloatType frequency, double sampleRate, int order)
        {
            ...
            if (order % 2 == 1)
            {
                ...
                for (auto i = 0; i < order / 2; ++i)
                {
                    ...
                    arrayFilters.add(
                        *IIR::Coefficients<FloatType>::makeLowPass(sampleRate, frequency, static_cast<FloatType>(Q)));
                }
            }
            else
            {
        ...
        template <typename FloatType>
        Array<IIR::Coefficients<FloatType>> FilterDesign<FloatType>::designIIRHighpassHighOrderButterworthMethod(
            FloatType frequency, double sampleRate, int order)
        {
            jassert(sampleRate > 0);
            jassert(frequency > 0 && frequency <= sampleRate * 0.5);
            ...
            if (order % 2 == 1)
            {
                ...
                for (auto i = 0; i < order / 2; ++i)
                {
                    auto Q = 1.0 / (2.0 * std::cos((i + 1.0) * MathConstants<double>::pi / order));
                    arrayFilters.add(
                        *IIR::Coefficients<FloatType>::makeHighPass(sampleRate, frequency, static_cast<FloatType>(Q)));
                }
            }
            else
            {
                                                                                ...
                                                                                FloatType stopbandAttenuationdB)
        {
            ...
            for (int i = 1; i <= N; ++i)
            {
                ...
                while (std::abs(delta) > 1e-100)
                {
                    delta = std::pow(-1, m) * std::pow(q, m * m) * std::cos(m * MathConstants<double>::twoPi * i / (double)n);
                    den += delta;
                    ++m;
                }
            ...
            for (int i = 0; i < N; i += 2)
                structure.directPath.add(
                    IIR::Coefficients<FloatType>(static_cast<FloatType>(ai[i]), 0, 1, 1, 0, static_cast<FloatType>(ai[i])));
