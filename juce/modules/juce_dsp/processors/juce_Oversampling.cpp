...
namespace juce
{
    namespace dsp
    {
        ...
        public:
            ...
            virtual void initProcessing(size_t maximumNumberOfSamplesBeforeOversampling)
            {
                buffer.setSize(static_cast<int>(numChannels),
                               static_cast<int>(maximumNumberOfSamplesBeforeOversampling * factor), false, false, true);
            }
        ...
        */
        ...
        public:
            //===============================================================================
            OversamplingDummy(size_t numChannels)
                : OversamplingEngine<SampleType>(numChannels, 1)
            {
            }
            ~OversamplingDummy() {}
            ...
            void processSamplesUp(dsp::AudioBlock<SampleType> &inputBlock) override
            {
                jassert(inputBlock.getNumChannels()
                        <= static_cast<size_t>(OversamplingEngine<SampleType>::buffer.getNumChannels()));
                jassert(inputBlock.getNumSamples() * OversamplingEngine<SampleType>::factor
                        <= static_cast<size_t>(OversamplingEngine<SampleType>::buffer.getNumSamples()));
            ...
            void processSamplesDown(dsp::AudioBlock<SampleType> &outputBlock) override
            {
                jassert(outputBlock.getNumChannels()
                        <= static_cast<size_t>(OversamplingEngine<SampleType>::buffer.getNumChannels()));
                jassert(outputBlock.getNumSamples() * OversamplingEngine<SampleType>::factor
                        <= static_cast<size_t>(OversamplingEngine<SampleType>::buffer.getNumSamples()));
        ...
        */
        ...
        public:
            ...
            Oversampling2TimesEquirippleFIR(size_t numChannels, SampleType normalizedTransitionWidthUp,
                                            SampleType stopbandAttenuationdBUp, SampleType normalizedTransitionWidthDown,
                                            SampleType stopbandAttenuationdBDown)
                : OversamplingEngine<SampleType>(numChannels, 2)
            {
                coefficientsUp = *dsp::FilterDesign<SampleType>::designFIRLowpassHalfBandEquirippleMethod(
                    normalizedTransitionWidthUp, stopbandAttenuationdBUp);
                coefficientsDown = *dsp::FilterDesign<SampleType>::designFIRLowpassHalfBandEquirippleMethod(
            ...
            void processSamplesUp(dsp::AudioBlock<SampleType> &inputBlock) override
            {
                ...
                for (size_t channel = 0; channel < inputBlock.getNumChannels(); channel++)
                {
                    ...
                    for (size_t i = 0; i < numSamples; i++)
                    {
                        ...
                        for (size_t k = 0; k < Ndiv2; k += 2) out += (buf[k] + buf[N - k - 1]) * fir[k];
                        ...
                        // Shift data
                        for (size_t k = 0; k < N - 2; k += 2) buf[k] = buf[k + 2];
                    }
                }
            }
            ...
            void processSamplesDown(dsp::AudioBlock<SampleType> &outputBlock) override
            {
                ...
                for (size_t channel = 0; channel < outputBlock.getNumChannels(); channel++)
                {
                    ...
                    for (size_t i = 0; i < numSamples; i++)
                    {
                        ...
                        // Shift data
                        for (size_t k = 0; k < N - 2; k++) buf[k] = buf[k + 2];
        ...
        */
        ...
        public:
            ...
            Oversampling2TimesPolyphaseIIR(size_t numChannels, SampleType normalizedTransitionWidthUp,
                ...
                : OversamplingEngine<SampleType>(numChannels, 2)
            {
                auto structureUp = dsp::FilterDesign<SampleType>::designIIRLowpassHalfBandPolyphaseAllpassMethod(
                    normalizedTransitionWidthUp, stopbandAttenuationdBUp);
                dsp::IIR::Coefficients<SampleType> coeffsUp = getCoefficients(structureUp);
                latency = static_cast<SampleType>(-(coeffsUp.getPhaseForFrequency(0.0001, 1.0))
                                                  / (0.0001 * MathConstants<double>::twoPi));

                auto structureDown = dsp::FilterDesign<SampleType>::designIIRLowpassHalfBandPolyphaseAllpassMethod(
                    normalizedTransitionWidthDown, stopbandAttenuationdBDown);
                dsp::IIR::Coefficients<SampleType> coeffsDown = getCoefficients(structureDown);
                latency += static_cast<SampleType>(-(coeffsDown.getPhaseForFrequency(0.0001, 1.0))
                                                   / (0.0001 * MathConstants<double>::twoPi));
            ...
            void snapToZero(bool snapUpProcessing)
            {
                if (snapUpProcessing)
                {
                    for (auto channel = 0; channel < OversamplingEngine<SampleType>::buffer.getNumChannels(); channel++)
                    {
                        ...
                        for (auto n = 0; n < numStages; n++) util::snapToZero(lv1[n]);
                    }
                }
                else
                {
        ...
        private:
            ...
            */
            const dsp::IIR::Coefficients<SampleType> getCoefficients(
                typename dsp::FilterDesign<SampleType>::IIRPolyphaseAllpassStructure &structure) const
            {
                ...
                for (auto i = 0; i <= numerator.getOrder(); i++) coeffs.coefficients.add(numerator[i] * inversion);

                for (auto i = 1; i <= denominator.getOrder(); i++) coeffs.coefficients.add(denominator[i] * inversion);
        ...
        //===============================================================================
        template <typename SampleType>
        Oversampling<SampleType>::Oversampling(size_t newNumChannels, size_t newFactor, FilterType newType,
                                               bool newMaxQuality)
        {
            ...
            else if (type == FilterType::filterHalfBandPolyphaseIIR)
            {
                ...
                for (size_t n = 0; n < numStages; n++)
                {
                    ...
                    engines.add(new Oversampling2TimesPolyphaseIIR<SampleType>(numChannels, twUp,
                                                                               gaindBStartUp + gaindBFactorUp * n, twDown,
                                                                               gaindBStartDown + gaindBFactorDown * n));
                }
            }
            else if (type == FilterType::filterHalfBandFIREquiripple)
            {
                ...
                for (size_t n = 0; n < numStages; n++)
                {
                    ...
                    engines.add(new Oversampling2TimesEquirippleFIR<SampleType>(numChannels, twUp,
                                                                                gaindBStartUp + gaindBFactorUp * n, twDown,
                                                                                gaindBStartDown + gaindBFactorDown * n));
                }
            }
        ...
        void Oversampling<SampleType>::reset() noexcept
        {
            ...
            if (isReady)
                for (auto n = 0; n < engines.size(); n++) engines[n]->reset();
        }

        template <typename SampleType>
        typename dsp::AudioBlock<SampleType> Oversampling<SampleType>::processSamplesUp(
            const dsp::AudioBlock<SampleType> &inputBlock) noexcept
        {
            jassert(engines.size() > 0);

            if (!isReady) return dsp::AudioBlock<SampleType>();
        ...
        void Oversampling<SampleType>::processSamplesDown(dsp::AudioBlock<SampleType> &outputBlock) noexcept
        {
            ...
            for (size_t n = 0; n < numStages - 1; n++) currentNumSamples *= engines[static_cast<int>(n)]->getFactor();
