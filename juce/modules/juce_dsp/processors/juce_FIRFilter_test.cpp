    ...
    namespace dsp
    {
        class FIRFilterTest : public UnitTest
        {
            ...
            struct Helpers
            {
                ...
                {
                    for (size_t i = 0; i < n; ++i) buffer[i] = (2.0f * random.nextFloat()) - 1.0f;
                }
            ...
            struct Helpers<SIMDRegister<Type>>
            {
                ...
                {
                    return Helpers<Type>::checkArrayIsSimilar(reinterpret_cast<Type*>(a), reinterpret_cast<Type*>(b),
            ...
            static void reference(const NumericType* firCoefficients, size_t numCoefficients, const SampleType* input,
                                  SampleType* output, size_t n) noexcept
            {
...
#if JUCE_USE_SIMD
                SampleType* buffer = reinterpret_cast<SampleType*>(SIMDRegister<NumericType>::getNextSIMDAlignedPtr(
                ...
                for (size_t i = 0; i < n; ++i)
                {
                    for (size_t j = (numCoefficients - 1); j >= 1; --j) buffer[j] = buffer[j - 1];

                    buffer[0] = input[i];

                    SampleType sum(0);

                    for (size_t j = 0; j < numCoefficients; ++j) sum += buffer[j] * firCoefficients[j];
            ...
            struct SampleBySampleTest
            {
                ...
                static void run(FIR::Filter<FloatType>& filter, FloatType* src, FloatType* dst, size_t n)
                {
                    for (size_t i = 0; i < n; ++i) dst[i] = filter.processSample(src[i]);
                }
            };
            ...
            void runTestForType()
            {
                ...
                for (auto size : {1, 2, 4, 8, 12, 13, 25})
                {
                    ...
                    FIR::Filter<SampleType> filter(
                        new FIR::Coefficients<NumericType>(fir.getChannelPointer(0), static_cast<size_t>(size)));
