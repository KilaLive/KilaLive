        ...
        public:
            ...
            */
            ...
            void initialise(const std::function<NumericType(NumericType)>& function, size_t lookupTableNumPoints = 0)
            {
                ...
                {
                    auto* table = new LookupTableTransform<NumericType>(function, -MathConstants<NumericType>::pi,
            ...
            void reset() noexcept
            {
                ...
                if (sampleRate > 0) frequency.reset(sampleRate, 0.05);
            }
            ...
            void process(const ProcessContext& context) noexcept
            {
                ...
                if (frequency.isSmoothing())
                {
                    ...
                    for (size_t i = 0; i < len; ++i)
                        buffer[i] = phase.advance(baseIncrement * frequency.getNextValue()) - MathConstants<NumericType>::pi;

                    for (size_t ch = 0; ch < numChannels; ++ch)
                    {
                        auto* dst = outBlock.getChannelPointer(ch);

                        for (size_t i = 0; i < len; ++i) dst[i] = generator(buffer[i]);
                    }
                }
                else
                {
                    ...
                    for (size_t ch = 0; ch < numChannels; ++ch)
                    {
                        ...
                        for (size_t i = 0; i < len; ++i) dst[i] = generator(p.advance(freq) - MathConstants<NumericType>::pi);
                    }
