    ...
    namespace dsp
    {
        ...
        */
        ...
        public:
            ...
            void reset() noexcept
            {
                if (sampleRate > 0) gain.reset(sampleRate, rampDurationSeconds);
            }
            ...
            void process(const ProcessContext& context) noexcept
            {
                ...
                if (numChannels == 1)
                {
                    ...
                    for (size_t i = 0; i < len; ++i) dst[i] = src[i] * gain.getNextValue();
                }
                else
                {
                    auto* gains = static_cast<FloatType*>(alloca(sizeof(FloatType) * len));

                    for (size_t i = 0; i < len; ++i) gains[i] = gain.getNextValue();

                    for (size_t chan = 0; chan < numChannels; ++chan)
                        FloatVectorOperations::multiply(outBlock.getChannelPointer(chan), inBlock.getChannelPointer(chan),
