...
namespace juce
{
    namespace dsp
    {
        ...
        IIR::Coefficients<NumericType>::Coefficients()
            : coefficients({NumericType(), NumericType(), NumericType(), NumericType(), NumericType()})
        {
        }

        template <typename NumericType>
        IIR::Coefficients<NumericType>::Coefficients(NumericType b0, NumericType b1, NumericType a0, NumericType a1)
        {
            jassert(a0 != 0);
            ...
            coefficients.add(b0 * a0inv, b1 * a0inv, a1 * a0inv);
        }

        template <typename NumericType>
        IIR::Coefficients<NumericType>::Coefficients(NumericType b0, NumericType b1, NumericType b2, NumericType a0,
                                                     NumericType a1, NumericType a2)
        {
            ...
            coefficients.add(b0 * a0inv, b1 * a0inv, b2 * a0inv, a1 * a0inv, a2 * a0inv);
        }
                                                     ...
                                                     NumericType a0, NumericType a1, NumericType a2, NumericType a3)
        {
            ...
            coefficients.add(b0 * a0inv, b1 * a0inv, b2 * a0inv, b3 * a0inv, a1 * a0inv, a2 * a0inv, a3 * a0inv);
        }

        template <typename NumericType>
        typename IIR::Coefficients<NumericType>::Ptr IIR::Coefficients<NumericType>::makeFirstOrderLowPass(
        ...
        template <typename NumericType>
        typename IIR::Coefficients<NumericType>::Ptr IIR::Coefficients<NumericType>::makeFirstOrderHighPass(
            double sampleRate, NumericType frequency)
        {
            jassert(sampleRate > 0.0);
            jassert(frequency > 0 && frequency <= static_cast<float>(sampleRate * 0.5));
        ...
        template <typename NumericType>
        typename IIR::Coefficients<NumericType>::Ptr IIR::Coefficients<NumericType>::makeFirstOrderAllPass(
            double sampleRate, NumericType frequency)
        {
            jassert(sampleRate > 0.0);
            jassert(frequency > 0 && frequency <= static_cast<float>(sampleRate * 0.5));
                                                                                                  ...
                                                                                                  NumericType Q)
        {
            ...
            return new Coefficients(c1 * n * invQ, 0, -c1 * n * invQ, 1, c1 * 2 * (1 - nSquared),
                                                                                                  ...
                                                                                                  NumericType gainFactor)
        {
            ...
            auto aminus1 = A - 1;
            auto aplus1  = A + 1;
            auto omega   = (2 * MathConstants<NumericType>::pi * jmax(cutOffFrequency, static_cast<NumericType>(2.0)))
            ...
            return new Coefficients(A * (aplus1 - aminus1TimesCoso + beta), A * 2 * (aminus1 - aplus1 * coso),
                                    A * (aplus1 - aminus1TimesCoso - beta), aplus1 + aminus1TimesCoso + beta,
                                    -2 * (aminus1 + aplus1 * coso), aplus1 + aminus1TimesCoso - beta);
        }

        template <typename NumericType>
        typename IIR::Coefficients<NumericType>::Ptr IIR::Coefficients<NumericType>::makeHighShelf(
            double sampleRate, NumericType cutOffFrequency, NumericType Q, NumericType gainFactor)
        {
            jassert(sampleRate > 0);
                                                                                                    ...
                                                                                                    NumericType gainFactor)
        {
            ...
            auto A     = jmax(static_cast<NumericType>(0.0), std::sqrt(gainFactor));
            auto omega = (2 * MathConstants<NumericType>::pi * jmax(frequency, static_cast<NumericType>(2.0)))
                         / static_cast<NumericType>(sampleRate);
        ...
        void IIR::Coefficients<NumericType>::getMagnitudeForFrequencyArray(const double* frequencies, double* magnitudes,
                                                                           size_t numSamples, double sampleRate) const
            noexcept
        {
            constexpr Complex<double> j(0, 1);
            const auto                order = getFilterOrder();
        ...
        template <typename NumericType>
        void IIR::Coefficients<NumericType>::getPhaseForFrequencyArray(double* frequencies, double* phases, size_t numSamples,
