    ...
    namespace dsp
    {
        ...
        */
            ...
            */
            ...
            void process(const ProcessContext& context) noexcept
            {
                ...
                if (numChannels == 1)
                {
                    ...
                    for (size_t i = 0; i < len; ++i) dst[i] = src[i] + bias.getNextValue();
                }
                else
                {
                    auto* biases = static_cast<FloatType*>(alloca(sizeof(FloatType) * len));

                    for (size_t i = 0; i < len; ++i) biases[i] = bias.getNextValue();

                    for (size_t chan = 0; chan < numChannels; ++chan)
                        FloatVectorOperations::add(outBlock.getChannelPointer(chan), inBlock.getChannelPointer(chan), biases,
                                                   static_cast<int>(len));
                }
            }
        ...
        private:
            ...
            {
                if (sampleRate > 0) bias.reset(sampleRate, rampDurationSeconds);
            }
        };
