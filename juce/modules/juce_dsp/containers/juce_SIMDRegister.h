    ...
    namespace dsp
    {
        ...
        */
        ...
        struct SIMDRegister
        {
            ...
            inline SIMDRegister(vSIMDType a) noexcept
                : value(a)
            {
            }

            /** Constructs an object from a scalar type by broadcasting it to all elements. */
            inline SIMDRegister(Type s) noexcept
            {
                *this = s;
            }
            ...
            inline static SIMDRegister JUCE_VECTOR_CALLTYPE expand(ElementType s) noexcept
            {
            ...
            inline static SIMDRegister JUCE_VECTOR_CALLTYPE fromNative(vSIMDType a) noexcept
            {
                return {a};
            }
            ...
            inline SIMDRegister& JUCE_VECTOR_CALLTYPE operator+=(SIMDRegister v) noexcept
            {
            ...
            inline SIMDRegister JUCE_VECTOR_CALLTYPE operator+(SIMDRegister v) const noexcept
            {
            ...
            //==============================================================================
            /** Returns a vector where each element is the sum of the corresponding element in the receiver and the scalar
             * s.*/
            inline SIMDRegister JUCE_VECTOR_CALLTYPE operator+(ElementType s) const noexcept
            {
                return {NativeOps::add(value, CmplxOps::expand(s))};
            }

            /** Returns a vector where each element is the difference of the corresponding element in the receiver and the
             * scalar s.*/
            inline SIMDRegister JUCE_VECTOR_CALLTYPE operator-(ElementType s) const noexcept
            {
            ...
            /** Returns a vector where each element is the product of the corresponding element in the receiver and the scalar
             * s.*/
            inline SIMDRegister JUCE_VECTOR_CALLTYPE operator*(ElementType s) const noexcept
            {
            ...
            inline SIMDRegister JUCE_VECTOR_CALLTYPE operator&(vMaskType v) const noexcept
            {
            ...
            //==============================================================================
            /** Returns a vector where each element is the bit-and'd value of the corresponding element in the receiver and
             * the scalar s.*/
            inline SIMDRegister JUCE_VECTOR_CALLTYPE operator&(MaskType s) const noexcept
            {
                return {NativeOps::bit_and(value, toVecType(s))};
            }

            /** Returns a vector where each element is the bit-or'd value of the corresponding element in the receiver and the
             * scalar s.*/
            inline SIMDRegister JUCE_VECTOR_CALLTYPE operator|(MaskType s) const noexcept
            {
            ...
            /** Returns a vector where each element is the bit-xor'd value of the corresponding element in the receiver and
             * the scalar s.*/
            inline SIMDRegister JUCE_VECTOR_CALLTYPE operator^(MaskType s) const noexcept
            {
            ...
            inline bool JUCE_VECTOR_CALLTYPE operator==(SIMDRegister other) const noexcept
            {
            ...
            static inline vMaskType JUCE_VECTOR_CALLTYPE equal(SIMDRegister a, SIMDRegister b) noexcept
            {
            ...
            /** Returns a SIMDRegister of the corresponding integral type where each element has each bit set
                ...
                The result can then be used in bit operations defined above to avoid branches in vector SIMD code. */
            static inline vMaskType JUCE_VECTOR_CALLTYPE notEqual(SIMDRegister a, SIMDRegister b) noexcept
            {
            ...
            /** Returns a SIMDRegister of the corresponding integral type where each element has each bit set
                ...
                The result can then be used in bit operations defined above to avoid branches in vector SIMD code. */
            static inline vMaskType JUCE_VECTOR_CALLTYPE lessThan(SIMDRegister a, SIMDRegister b) noexcept
            {
            ...
            /** Returns a SIMDRegister of the corresponding integral type where each element has each bit set
                ...
                The result can then be used in bit operations defined above to avoid branches in vector SIMD code. */
            static inline vMaskType JUCE_VECTOR_CALLTYPE lessThanOrEqual(SIMDRegister a, SIMDRegister b) noexcept
            {
            ...
            /** Returns a SIMDRegister of the corresponding integral type where each element has each bit set
                ...
                The result can then be used in bit operations defined above to avoid branches in vector SIMD code. */
            static inline vMaskType JUCE_VECTOR_CALLTYPE greaterThan(SIMDRegister a, SIMDRegister b) noexcept
            {
                return toMaskType(NativeOps::greaterThan(a.value, b.value));
            }

            /** Returns a SIMDRegister of the corresponding integral type where each element has each bit set
                if the corresponding element of a is greater than or equal to the corresponding element of b, or zero
               otherwise. The result can then be used in bit operations defined above to avoid branches in vector SIMD code.
             */
            static inline vMaskType JUCE_VECTOR_CALLTYPE greaterThanOrEqual(SIMDRegister a, SIMDRegister b) noexcept
            {
            ...
            static inline SIMDRegister JUCE_VECTOR_CALLTYPE min(SIMDRegister a, SIMDRegister b) noexcept
            {
            ...
            static inline SIMDRegister JUCE_VECTOR_CALLTYPE max(SIMDRegister a, SIMDRegister b) noexcept
            {
            ...
            static inline SIMDRegister JUCE_VECTOR_CALLTYPE multiplyAdd(SIMDRegister a, const SIMDRegister b,
                                                                        SIMDRegister c) noexcept
            {
            ...
            inline ElementType sum() const noexcept
            {
                return CmplxOps::sum(value);
            }
        ...
        struct CmplxSIMDOps<std::complex<Scalar>>
        {
            ...
            static inline vSIMDType JUCE_VECTOR_CALLTYPE expand(std::complex<Scalar> s) noexcept
            {
                ...
                for (int i = 0; i < n; ++i) u.floats[i] = (i & 1) == 0 ? s.real() : s.imag();
...
//==============================================================================
    ...
    template <typename Type>
    inline dsp::SIMDRegister<Type> JUCE_VECTOR_CALLTYPE jmin(dsp::SIMDRegister<Type> a, dsp::SIMDRegister<Type> b)
    {
        return dsp::SIMDRegister<Type>::min(a, b);
    }
    template <typename Type>
    inline dsp::SIMDRegister<Type> JUCE_VECTOR_CALLTYPE jmax(dsp::SIMDRegister<Type> a, dsp::SIMDRegister<Type> b)
    {
