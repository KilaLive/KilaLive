...
namespace juce
{
    namespace dsp
    {
        ...
        namespace SampleTypeHelpers  // Internal classes needed for handling sample type classes
        {
            ...
            template <>
            struct ElementType<long double>
            {
        ...
        */
        ...
        public:
            ...
            */
            ...
            /** Creates an AudioBlock from a pointer to an array of channels.
                ...
                throughout the life-time of the AudioBlock and released when no longer needed.
            */
            forcedinline AudioBlock(SampleType* const* channelData, size_t numberOfChannels, size_t startSampleIndex,
            ...
            */
            AudioBlock(HeapBlock<char>& heapBlockToUseForAllocation, size_t numberOfChannels, size_t numberOfSamples,
            ...
            */
            ...
            forcedinline AudioBlock& clear() noexcept
            {
                ...
                for (size_t ch = 0; ch < numChannels; ++ch) FloatVectorOperations::clear(channelPtr(ch), n);
            ...
            forcedinline AudioBlock& JUCE_VECTOR_CALLTYPE fill(SampleType value) noexcept
            {
                ...
                for (size_t ch = 0; ch < numChannels; ++ch) FloatVectorOperations::fill(channelPtr(ch), value, n);
            ...
            */
            ...
            forcedinline AudioBlock& JUCE_VECTOR_CALLTYPE add(SampleType value) noexcept
            {
                ...
                for (size_t ch = 0; ch < numChannels; ++ch) FloatVectorOperations::add(channelPtr(ch), value, n);
            ...
            forcedinline AudioBlock& add(AudioBlock src) noexcept
            {
                ...
                for (size_t ch = 0; ch < numChannels; ++ch) FloatVectorOperations::add(channelPtr(ch), src.channelPtr(ch), n);
            ...
            /** Adds each source1 value to the corresponding source2 value and stores it in the destination array of the
             * receiver. */
            ...
            /** Subtracts each source2 value from the corresponding source1 value and stores it in the destination array of
             * the receiver. */
            ...
            forcedinline AudioBlock& JUCE_VECTOR_CALLTYPE multiply(SampleType value) noexcept
            {
                ...
                for (size_t ch = 0; ch < numChannels; ++ch) FloatVectorOperations::multiply(channelPtr(ch), value, n);
            ...
            /** Multiplies each source1 value to the corresponding source2 value and stores it in the destination array of the
             * receiver. */
            ...
            AudioBlock& multiply(LinearSmoothedValue<SampleType>& value) noexcept
            {
                ...
                else
                {
                    for (size_t i = 0; i < numSamples; ++i)
                    {
                        ...
                        for (size_t ch = 0; ch < numChannels; ++ch) channelPtr(ch)[i] *= scaler;
                    }
                }
            ...
            AudioBlock& multiply(AudioBlock src, LinearSmoothedValue<SampleType>& value) noexcept
            {
                ...
                else
                {
                    ...
                    for (size_t i = 0; i < n; ++i)
                    {
                        ...
                        for (size_t ch = 0; ch < numChannels; ++ch) channelPtr(ch)[i] = scaler * src.getChannelPointer(ch)[i];
                    }
                }
            ...
            forcedinline AudioBlock& replaceWithAbsoluteValueOf(AudioBlock src) noexcept
            {
                ...
                for (size_t ch = 0; ch < numChannels; ++ch) FloatVectorOperations::abs(channelPtr(ch), src.channelPtr(ch), n);
            ...
            */
            ...
            static void process(AudioBlock inBlock, AudioBlock outBlock, FunctionType&& function)
            {
                ...
                for (ChannelCountType c = 0; c < numChans; ++c)
                {
                    ...
                    for (size_t i = 0; i < len; ++i) dst[i] = function(src[i]);
                }
            }
        ...
        private:
            ...
            {
                return reinterpret_cast<NumericType*>(getChannelPointer(ch));
            }
            const NumericType* channelPtr(size_t ch) const noexcept
            {
                return reinterpret_cast<const NumericType*>(getChannelPointer(ch));
            }
