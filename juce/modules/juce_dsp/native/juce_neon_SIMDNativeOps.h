...
#ifdef _MSC_VER
#define DECLARE_NEON_SIMD_CONST(type, name) static __declspec(align(16)) const type name[16 / sizeof(type)]
...
#else
#define DECLARE_NEON_SIMD_CONST(type, name) static const type name[16 / sizeof(type)] __attribute__((aligned(16)))
...
#define DEFINE_NEON_SIMD_CONST(type, class_type, name)                                                                       \
    const type SIMDNativeOps<class_type>::name[16 / sizeof(type)] __attribute__((aligned(16)))
        ...
        struct SIMDNativeOps<int8_t>
        {
            ...
            static forcedinline bool allEqual(vSIMDType a, vSIMDType b) noexcept
            {
                return (SIMDNativeOps<int32_t>::sum(notEqual(a, b)) == 0);
            }
            static forcedinline vSIMDType multiplyAdd(vSIMDType a, vSIMDType b, vSIMDType c) noexcept
            {
        ...
        struct SIMDNativeOps<uint8_t>
        {
            ...
            static forcedinline bool allEqual(vSIMDType a, vSIMDType b) noexcept
            {
                return (SIMDNativeOps<uint32_t>::sum(notEqual(a, b)) == 0);
            }
            static forcedinline vSIMDType multiplyAdd(vSIMDType a, vSIMDType b, vSIMDType c) noexcept
            {
