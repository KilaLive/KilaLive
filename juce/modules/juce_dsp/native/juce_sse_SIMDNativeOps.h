...
#ifdef _MSC_VER
#define DECLARE_SSE_SIMD_CONST(type, name) static __declspec(align(16)) const type name[16 / sizeof(type)]
...
#else
#define DECLARE_SSE_SIMD_CONST(type, name) static const type name[16 / sizeof(type)] __attribute__((aligned(16)))
...
#define DEFINE_SSE_SIMD_CONST(type, class_type, name)                                                                        \
    const type SIMDNativeOps<class_type>::name[16 / sizeof(type)] __attribute__((aligned(16)))
        ...
        struct SIMDNativeOps<double>
        {
            ...
            {
                return *reinterpret_cast<const __m128d*>(a);
            }
            ...
            static forcedinline __m128d oddevensum(__m128d a) noexcept
            {
                return a;
            }
        ...
        struct SIMDNativeOps<int8_t>
        {
            ...
            {
                return *reinterpret_cast<const __m128i*>(a);
            }
            ...
            static forcedinline __m128i JUCE_VECTOR_CALLTYPE load(const int8_t* a) noexcept
            {
                ...
                return _mm_set_epi8(b[15], b[14], b[13], b[12], b[11], b[10], b[9], b[8], b[7], b[6], b[5], b[4], b[3], b[2],
                                    b[1], b[0]);
            }
            ...
            static forcedinline int8_t JUCE_VECTOR_CALLTYPE sum(__m128i a) noexcept
            {
                ...
                for (std::size_t i = 0; i < (sizeof(vSIMDType) / sizeof(int8_t)); ++i) sum += src[i];
        ...
        struct SIMDNativeOps<int16_t>
        {
            ...
            static forcedinline int16_t JUCE_VECTOR_CALLTYPE sum(__m128i a) noexcept
            {
                ...
                for (std::size_t i = 0; i < (sizeof(vSIMDType) / sizeof(int16_t)); ++i) sum += src[i];
        ...
        struct SIMDNativeOps<uint16_t>
        {
            ...
            static forcedinline uint16_t JUCE_VECTOR_CALLTYPE sum(__m128i a) noexcept
            {
                ...
                for (std::size_t i = 0; i < (sizeof(vSIMDType) / sizeof(uint16_t)); ++i) sum += src[i];
        ...
        struct SIMDNativeOps<int32_t>
        {
            ...
            static forcedinline int32_t JUCE_VECTOR_CALLTYPE sum(__m128i a) noexcept
            {
                ...
                for (std::size_t i = 0; i < (sizeof(vSIMDType) / sizeof(int32_t)); ++i) sum += src[i];
        ...
        struct SIMDNativeOps<uint32_t>
        {
            ...
            static forcedinline uint32_t JUCE_VECTOR_CALLTYPE sum(__m128i a) noexcept
            {
                ...
                for (std::size_t i = 0; i < (sizeof(vSIMDType) / sizeof(uint32_t)); ++i) sum += src[i];
        ...
        struct SIMDNativeOps<int64_t>
        {
            ...
            static forcedinline __m128i greaterThanOrEqual(__m128i a, __m128i b) noexcept
            {
                return bit_or(greaterThan(a, b), equal(a, b));
            }
            static forcedinline __m128i JUCE_VECTOR_CALLTYPE multiplyAdd(__m128i a, __m128i b, __m128i c) noexcept
