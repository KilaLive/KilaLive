    ...
    namespace dsp
    {
                                                         ...
                                                         size_t numPoints)
        {
            ...
            const auto initFn = [functionToApproximate, minInputValueToUse, maxInputValueToUse, numPoints](size_t i) {
                return functionToApproximate(jlimit(
        ...
        double LookupTableTransform<FloatType>::calculateMaxRelativeError(
            const std::function<FloatType(FloatType)>& functionToApproximate, FloatType minInputValue,
            FloatType maxInputValue, size_t numPoints, size_t numTestPoints)
        {
            jassert(maxInputValue > minInputValue);

            if (numTestPoints == 0) numTestPoints = 100 * numPoints;  // use default
            ...
            {
                auto inputValue =
                    jmap(FloatType(i), FloatType(0), FloatType(numTestPoints - 1), minInputValue, maxInputValue);
                ...
                maxError = jmax(maxError,
                                calculateRelativeDifference((double)referenceOutputValue, (double)approximatedOutputValue));
            }
        ...
        double LookupTableTransform<FloatType>::calculateRelativeDifference(double x, double y) noexcept
        {
            ...
            if (absX < eps)
            {
                if (absY >= eps) return absDiff / absY;
