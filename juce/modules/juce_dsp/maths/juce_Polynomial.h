        ...
        public:
            ...
            */
            ...
            FloatingType operator()(FloatingType x) const noexcept
            {
                ...
                for (int i = coeffs.size(); --i >= 0;) y = (x * y) + coeffs.getUnchecked(i);
            ...
            Polynomial<FloatingType> withGain(double gain) const
            {
                ...
                for (auto& c : result.coeffs) c *= gain;
            ...
            {
                if (coeffs.size() < other.coeffs.size()) return other.getSumWith(*this);

                auto result = *this;

                for (int i = 0; i < other.coeffs.size(); ++i) result[i] += other[i];
            ...
            Polynomial<FloatingType> getProductWith(const Polynomial<FloatingType>& other) const
            {
                ...
                for (int i = 0; i < N; ++i)
                {
                    ...
                    for (int j = 0; j < Nmax; ++j)
                        if (j >= 0 && j < N1 && i - j >= 0 && i - j < N2) value = value + (*this)[j] * other[i - j];
