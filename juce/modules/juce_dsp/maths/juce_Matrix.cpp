...
namespace juce
{
    namespace dsp
    {
        ...
        Matrix<ElementType> Matrix<ElementType>::toeplitz(const Matrix& vector, size_t size)
        {
            ...
            for (size_t i = 0; i < size; ++i) result(i, i) = vector(0, 0);

            for (size_t i = 1; i < size; ++i)
                for (size_t j = i; j < size; ++j) result(j, j - i) = result(j - i, j) = vector(i, 0);
        ...
        Matrix<ElementType> Matrix<ElementType>::hankel(const Matrix& vector, size_t size, size_t offset)
        {
            ...
            for (size_t i = 0; i < size; ++i) result(i, i) = vector((2 * i) + offset, 0);

            for (size_t i = 1; i < size; ++i)
                for (size_t j = i; j < size; ++j) result(j, j - i) = result(j - i, j) = vector(i + 2 * (j - i) + offset, 0);
        ...
        Matrix<ElementType>& Matrix<ElementType>::swapRows(size_t rowOne, size_t rowTwo) noexcept
        {
            ...
            for (size_t i = 0; i < columns; ++i) std::swap(p[offset1 + i], p[offset2 + i]);
        ...
        Matrix<ElementType> Matrix<ElementType>::operator*(const Matrix<ElementType>& other) const
        {
            ...
            for (size_t i = 0; i < n; ++i)
            {
                ...
                for (size_t k = 0; k < p; ++k)
                {
                    ...
                    for (size_t j = 0; j < m; ++j) dst[offsetMat + j] += ak * b[offsetrhs + j];
        ...
        bool Matrix<ElementType>::compare(const Matrix& a, const Matrix& b, ElementType tolerance) noexcept
        {
            ...
            auto* bPtr = b.begin();
            for (auto aValue : a)
                if (std::abs(aValue - *bPtr++) > tolerance) return false;
        ...
        bool Matrix<ElementType>::solve(Matrix& b) const noexcept
        {
            ...
            case 3:
            {
                ...
                x[0] = ((A(0, 1) * A(1, 2) - A(0, 2) * A(1, 1)) * b2 + (-A(0, 1) * A(2, 2) + A(0, 2) * A(2, 1)) * b1
                        + (A(1, 1) * A(2, 2) - A(1, 2) * A(2, 1)) * b0)
                       * factor;

                x[1] = -((A(0, 0) * A(1, 2) - A(0, 2) * A(1, 0)) * b2 + (-A(0, 0) * A(2, 2) + A(0, 2) * A(2, 0)) * b1
                         + (A(1, 0) * A(2, 2) - A(1, 2) * A(2, 0)) * b0)
                       * factor;

                x[2] = ((A(0, 0) * A(1, 1) - A(0, 1) * A(1, 0)) * b2 + (-A(0, 0) * A(2, 1) + A(0, 1) * A(2, 0)) * b1
            ...
            default:
            {
                ...
                for (size_t j = 0; j < n; ++j)
                {
                    if (M(j, j) == 0)
                    {
                        ...
                        for (size_t k = 0; k < n; ++k) M(j, k) += M(i, k);
                    ...
                    for (size_t k = j + 1; k < n; ++k)
                    {
                        ...
                        for (size_t l = 0; l < n; ++l) M(k, l) += u * M(j, l);
                ...
                for (int k = static_cast<int>(n) - 2; k >= 0; --k)
                    for (size_t i = static_cast<size_t>(k) + 1; i < n; ++i) x[k] -= M(static_cast<size_t>(k), i) * x[i];
            }
            }
        ...
        String Matrix<ElementType>::toString() const
        {
            ...
            for (size_t i = 0; i < n; ++i)
            {
                ...
                if (i % columns == (columns - 1)) result << newLine;
            }
