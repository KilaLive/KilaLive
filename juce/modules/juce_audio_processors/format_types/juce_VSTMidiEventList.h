    ...
    */
    ...
    public:
        ...
        void clear()
        {
            ...
            if (events != nullptr) events->numberOfEvents = 0;
        }
        ...
        void addEvent(const void* const midiData, const int numBytes, const int frameOffset)
        {
            ...
            else
            {
                ...
                if (se->type == vstSysExEventType) delete[] se->sysExDump;
        ...
        static void addEventsToMidiBuffer(const VstEventBlock* events, MidiBuffer& dest)
        {
            for (int i = 0; i < events->numberOfEvents; ++i)
            {
                ...
                if (e != nullptr)
                {
                    ...
                    {
                        dest.addEvent((const juce::uint8*)((const VstMidiEvent*)e)->midiData, 4, e->sampleOffset);
                    }
                    else if (e->type == vstSysExEventType)
                    {
                        dest.addEvent((const juce::uint8*)((const VstSysExEvent*)e)->sysExDump,
                                      (int)((const VstSysExEvent*)e)->sysExDumpSize, e->sampleOffset);
                    }
                }
            }
        }
        ...
        void ensureSize(int numEventsNeeded)
        {
            if (numEventsNeeded > numEventsAllocated)
            {
                ...
                for (int i = numEventsAllocated; i < numEventsNeeded; ++i) events->events[i] = allocateVSTEvent();
        ...
        void freeEvents()
        {
            if (events != nullptr)
            {
                for (int i = numEventsAllocated; --i >= 0;) freeVSTEvent(events->events[i]);
    ...
    private:
        ...
        {
            auto e = (VstEvent*)std::calloc(
                1, sizeof(VstMidiEvent) > sizeof(VstSysExEvent) ? sizeof(VstMidiEvent) : sizeof(VstSysExEvent));
            e->type = vstMidiEventType;
            e->size = sizeof(VstMidiEvent);
            return e;
        }
        ...
        {
            if (e->type == vstSysExEventType) delete[](((VstSysExEvent*)e)->sysExDump);
