...
#define warnOnFailure(x) x
        ...
        static int getHashForTUID(const TUID& tuid) noexcept
        {
            ...
            for (int i = 0; i < numElementsInArray(tuid); ++i) value = (value * 31) + tuid[i];
        ...
        static void fillDescriptionWith(PluginDescription& description, ObjectType& object)
        {
            ...
            if (description.manufacturerName.trim().isEmpty()) description.manufacturerName = toString(object.vendor).trim();
        }

        static void createPluginDescription(PluginDescription& description, const File& pluginFile, const String& company,
                                            const String& name, const PClassInfo& info, PClassInfo2* info2,
                                            PClassInfoW* infoW, int numInputs, int numOutputs)
        {
            ...
            if (description.category.isEmpty()) description.category = toString(info.category).trim();
        ...
        static int getNumSingleDirectionBusesFor(Vst::IComponent* component, bool checkInputs, bool checkAudioChannels)
        {
        ...
        static int getNumSingleDirectionChannelsFor(Vst::IComponent* component, bool checkInputs, bool checkAudioChannels)
        {
            jassert(component != nullptr);
        ...
        static void setStateForAllBusesOfType(Vst::IComponent* component, bool state, bool activateInputs,
        ...
        static void toProcessContext(Vst::ProcessContext& context, AudioPlayHead* playHead, double sampleRate)
        {
            ...
            if (playHead != nullptr)
            {
                ...
                case AudioPlayHead::fps23976:
                    fr.framesPerSecond = 24;
                    fr.flags           = FrameRate::kPullDownRate;
                    break;
                ...
                case AudioPlayHead::fps2997drop:
                    fr.framesPerSecond = 30;
                    fr.flags           = FrameRate::kPullDownRate | FrameRate::kDropRate;
                    break;
                ...
                case AudioPlayHead::fps30drop:
                    fr.framesPerSecond = 30;
                    fr.flags           = FrameRate::kDropRate;
                    break;
                case AudioPlayHead::fps60:
                    fr.framesPerSecond = 60;
                    fr.flags           = 0;
                    break;
                                 ...
                                 public Vst::IUnitHandler
        {
            ...
            tresult PLUGIN_API performEdit(Vst::ParamID paramID, Vst::ParamValue valueNormalized) override
            {
                if (plugin != nullptr)
                {
...

                    {
                        ...
                        plugin->inputParameterChanges->addParameterData(paramID, eventIndex)
                            ->addPoint(0, valueNormalized, eventIndex);
                    }
            ...
            tresult PLUGIN_API restartComponent(Steinberg::int32 flags) override
            {
                if (plugin != nullptr)
                {
                    if (hasFlag(flags, Vst::kReloadComponent)) plugin->reset();
                    ...
                    if (hasFlag(flags, Vst::kIoChanged))
                    {
                        ...
                        plugin->prepareToPlay(sampleRate >= 8000 ? sampleRate : 44100.0, blockSize > 0 ? blockSize : 1024);
                    }
            ...
            {
                ContextMenu(VST3PluginInstance& pluginInstance)
                ...
                tresult PLUGIN_API removeItem(const Item& toRemove, IContextMenuTarget* target) override
                {
                    for (int i = items.size(); --i >= 0;)
                    {
                        ...
                        if (item.item.tag == toRemove.tag && item.target == target) items.remove(i);
                    }
                ...
                tresult PLUGIN_API getItem(Steinberg::int32 tag, Item& result, IContextMenuTarget** target) override
                {
                    for (int i = 0; i < items.size(); ++i)
                    {
                        ...
                        if (item.item.tag == tag)
                        {
                            ...
                            if (target != nullptr) *target = item.target;
                ...
                tresult PLUGIN_API popup(Steinberg::UCoord x, Steinberg::UCoord y) override
                {
                    ...
                    for (int i = 0; i < items.size(); ++i)
                    {
                        ...
                        {
                            menuToUse->addItem(item.tag != 0 ? (int)item.tag : (int)zeroTagReplacement, toString(item.name),
                    ...
                    if (auto* ed = owner.getActiveEditor())
                        options =
                            options.withTargetScreenArea(ed->getScreenBounds().translated((int)x, (int)y).withSize(1, 1));
                    ...
                    topLevelMenu->showMenuAsync(options,
                                                ModalCallbackFunction::create(menuFinished, ComSmartPtr<ContextMenu>(this)));
#endif
            ...
            private:
                ...
                void handleResult(int result)
                {
                    ...
                    if (result == zeroTagReplacement) result = 0;
                    ...
                    for (int i = 0; i < items.size(); ++i)
                    {
                        ...
                        {
                            if (item.target != nullptr) item.target->executeMenuItem((Steinberg::int32)result);
            ...
            {
                if (plugin != nullptr) return new ContextMenu(*plugin);
            ...
            {
                if (plugin != nullptr) plugin->syncProgramNames();
        ...
        private:
            ...
            {
                if (plugin == nullptr || plugin->editController == nullptr) return -1;
            ...
            {
                AttributeList(VST3HostContext* o)
                    : owner(o)
                {
                }
                ...
                tresult PLUGIN_API getInt(AttrID id, Steinberg::int64& result) override
                {
                    ...
                    if (findMessageOnQueueWithID(id, result)) return kResultTrue;
                ...
                tresult PLUGIN_API getString(AttrID id, Vst::TChar* result, Steinberg::uint32 length) override
                {
                    ...
                    if (findMessageOnQueueWithID(id, stringToFetch))
                    {
                        ...
                        str.copyTo(
                            result, 0,
                            (Steinberg::int32)jmin(length, (Steinberg::uint32)std::numeric_limits<Steinberg::int32>::max()));
        ...
        struct DescriptionFactory
        {
            ...
            Result findDescriptionsAndPerform(const File& file)
            {
                ...
                for (Steinberg::int32 i = 0; i < numClasses; ++i)
                {
                    ...
                    if (foundNames.contains(name, true)) continue;
...

                    {
                        ...
                        if (component.loadFrom(factory, info.cid))
                        {
                            if (component->initialize(vst3HostContext->getFUnknown()) == kResultOk)
                            {
                                ...
                                createPluginDescription(desc, file, companyName, name, info, info2.get(), infoW.get(),
        ...
        struct MatchingDescriptionFinder : public DescriptionFactory
        {
            MatchingDescriptionFinder(VST3HostContext* h, IPluginFactory* f, const PluginDescription& desc)
                : DescriptionFactory(h, f)
                , description(desc)
            {
            }

            static const char* getSuccessString() noexcept
            {
            ...
            {
                if (description.isDuplicateOf(desc)) return Result::fail(getSuccessString());
        ...
        struct DLLHandle
        {
            ...
            {
                if (modulePath.trim().isNotEmpty()) open(modulePath);
            }
            ...
            ~DLLHandle()
            {
                ...
                if (auto exitFn = (ExitModuleFn)getFunction("ExitDll")) exitFn();
                ...
                if (bundleRef != nullptr)
                {
                    ...
                    if (auto exitFn = (ExitModuleFn)getFunction("bundleExit")) exitFn();
            ...
            */
            ...
            {
                if (factory == nullptr)
                    if (auto proc = (GetFactoryProc)getFunction("GetPluginFactory")) factory = proc();
...
#else
                if (bundleRef == nullptr) return nullptr;
        ...
        private:
            ...
            bool open(const String& filePath)
            {
                ...
                if (CFURLRef url = CFURLCreateFromFileSystemRepresentation(0, (const UInt8*)utf8, (CFIndex)std::strlen(utf8),
                                                                           file.isDirectory()))
                {
            ...
            */
            ...
            static VST3ModuleHandle::Ptr findOrCreateModule(const File& file, const PluginDescription& description)
            {
                for (auto* module : getActiveModules())
                    ...
                    if (module->file == file && module->name == description.name) return module;

                VST3ModuleHandle::Ptr m(new VST3ModuleHandle(file));

                if (!m->open(file, description)) m = nullptr;
        ...
        //==============================================================================
        struct VST3PluginWindow : public AudioProcessorEditor, public ComponentMovementWatcher, public IPlugFrame
        {
...
#if JUCE_WINDOWS
                        SetWindowPos(pluginHandle, 0, pos.x, pos.y, w, h, isVisible() ? SWP_SHOWWINDOW : SWP_HIDEWINDOW);
#elif JUCE_MAC
...
#if JUCE_WINDOWS
                        SetWindowPos(pluginHandle, 0, pos.x, pos.y, rect.getWidth(), rect.getHeight(),
        ...
        private:
            ...
            {
                comp.setBounds((int)rect.left, (int)rect.top, jmax(10, std::abs((int)rect.getWidth())),
            ...
            void attachPluginWindow()
            {
                if (pluginHandle == nullptr)
                {
                    ...
                    if (peer != nullptr) pluginHandle = (HandleFormat)peer->getNativeHandle();
#elif JUCE_MAC
                    embeddedComponent.setBounds(getLocalBounds());
                    ...
                    if (pluginHandle != nullptr) warnOnFailure(view->attached(pluginHandle, defaultVST3WindowType));
                }
            }
...
#pragma warning(disable : 4996)  // warning about overriding deprecated methods
        ...
        {
            VST3ComponentHolder(const VST3ModuleHandle::Ptr& m)
                : module(m)
            {
            ...
            {
                if (!isComponentInitialised && !initialise()) return false;
                ...
                if (editController == nullptr) editController.loadFrom(component);
                    ...
                    for (int i = 0; i < n; ++i)
                        if (component->getBusInfo(Vst::kAudio, Vst::kOutput, i, bus) == kResultOk)
                            totalNumOutputChannels +=
                                ((bus.flags & Vst::BusInfo::kDefaultActive) != 0 ? bus.channelCount : 0);

                    createPluginDescription(description, module->file, factoryInfo.vendor, module->name, info, info2.get(),
                                            infoW.get(), totalNumInputChannels, totalNumOutputChannels);
            ...
            bool initialise()
            {
                ...
                int classIdx;
                if ((classIdx = getClassIndex(module->name)) < 0) return false;

                PClassInfo info;
                if (factory->getClassInfo(classIdx, &info) != kResultOk) return false;

                if (!component.loadFrom(factory, info.cid) || component == nullptr) return false;

                cidOfComponent = FUID(info.cid);

                if (warnOnFailure(component->initialize(host->getFUnknown())) != kResultOk) return false;
            ...
            int getClassIndex(const String& className) const
            {
                ...
                for (Steinberg::int32 j = 0; j < numClasses; ++j)
                    if (factory->getClassInfo(j, &info) == kResultOk && std::strcmp(info.category, kVstAudioEffectClass) == 0
        ...
        struct VST3PluginInstance : public AudioPluginInstance
        {
            ...
            bool initialise()
            {
                ...
                if (!holder->initialise()) return false;

                if (!isControllerInitialised)
                {
                    if (!holder->fetchController(editController)) return false;
                }
            ...
            void repopulateArrangements(Array<Vst::SpeakerArrangement>& inputArrangements,
                                        Array<Vst::SpeakerArrangement>& outputArrangements) const
            {
                ...
                for (int i = 0; i < numInputAudioBuses; ++i) inputArrangements.add(getArrangementForBus(processor, true, i));

                for (int i = 0; i < numOutputAudioBuses; ++i)
                    outputArrangements.add(getArrangementForBus(processor, false, i));
            }

            void processorLayoutsToArrangements(Array<Vst::SpeakerArrangement>& inputArrangements,
            ...
            void prepareToPlay(double newSampleRate, int estimatedSamplesPerBlock) override
            {
                ...
                if (isActive && getSampleRate() == newSampleRate && getBlockSize() == estimatedSamplesPerBlock) return;
                ...
                for (int i = 0; i < numInputBuses; ++i)
                    warnOnFailure(
                        holder->component->activateBus(Vst::kAudio, Vst::kInput, i, getBus(true, i)->isEnabled() ? 1 : 0));

                for (int i = 0; i < numOutputBuses; ++i)
                    warnOnFailure(
                        holder->component->activateBus(Vst::kAudio, Vst::kOutput, i, getBus(false, i)->isEnabled() ? 1 : 0));
            ...
            {
                if (!isActive) return;  // Avoids redundantly calling things like setActive

                isActive = false;

                setStateForAllMidiBuses(false);

                if (processor != nullptr) warnOnFailure(processor->setProcessing(false));

                if (holder->component != nullptr) warnOnFailure(holder->component->setActive(false));
            }
            ...
            void processBlock(AudioBuffer<float>& buffer, MidiBuffer& midiMessages) override
            {
                ...
                if (isActive && processor != nullptr) processAudio(buffer, midiMessages, Vst::kSample32);
            }
            ...
            void processBlock(AudioBuffer<double>& buffer, MidiBuffer& midiMessages) override
            {
                ...
                if (isActive && processor != nullptr) processAudio(buffer, midiMessages, Vst::kSample64);
            }

            template <typename FloatType>
            void processAudio(AudioBuffer<FloatType>& buffer, MidiBuffer& midiMessages, Vst::SymbolicSampleSizes sampleSize)
            {
                using namespace Vst;
                ...
                for (int i = getTotalNumInputChannels(); i < buffer.getNumChannels(); ++i) buffer.clear(i, 0, numSamples);
            ...
            bool isBusesLayoutSupported(const BusesLayout& layouts) const override
            {
                ...
                if (!isActive) return canApplyBusesLayout(layouts);
                ...
                for (int dir = 0; dir < 2; ++dir)
                {
                    ...
                    for (int i = 0; i < n; ++i)
                        if (getChannelLayoutOfBus(isInput, i).isDiscreteLayout()) return false;
                }
            ...
            bool syncBusLayouts(const BusesLayout& layouts) const
            {
                ...
                for (int i = 0; i < layouts.inputBuses.size(); ++i)
                {
                    ...
                    inputArrangements.add(getVst3SpeakerArrangement(
                        requested.isDisabled() ? getBus(true, i)->getLastEnabledLayout() : requested));
                }
                ...
                for (int i = 0; i < layouts.outputBuses.size(); ++i)
                {
                    ...
                    outputArrangements.add(getVst3SpeakerArrangement(
                        requested.isDisabled() ? getBus(false, i)->getLastEnabledLayout() : requested));
                }
            ...
            bool canApplyBusesLayout(const BusesLayout& layouts) const override
            {
                ...
                if (!result) syncBusLayouts(getBusesLayout());
            ...
            struct TrackPropertiesAttributeList : public Vst::IAttributeList
            {
                ...
                tresult PLUGIN_API getString(AttrID id, Vst::TChar* string, Steinberg::uint32 size) override
                {
                    if (!std::strcmp(id, Vst::ChannelContext::kChannelNameKey))
                    {
                        ...
                        str.copyTo(
                            string, 0,
                            (Steinberg::int32)jmin(size, (Steinberg::uint32)std::numeric_limits<Steinberg::int32>::max()));
            ...
            String getChannelName(int channelIndex, bool forInput, bool forAudioChannel) const
            {
                ...
                for (int i = 0; i < numBuses; ++i)
                {
                    ...
                    if (channelIndex < numCountedChannels) return toString(busInfo.name);
                }
            ...
            //==============================================================================
            /** May return a negative value as a means of informing us that the plugin has "infinite tail," or 0 for "no
             * tail." */
            double getTailLengthSeconds() const override
            {
                if (processor != nullptr)
                {
                    ...
                    if (sampleRate > 0.0) return jlimit(0, 0x7fffffff, (int)processor->getTailSamples()) / sampleRate;
                }
            ...
            {
                if (auto* view = tryCreatingView()) return new VST3PluginWindow(this, view);
            ...
            bool hasEditor() const override
            {
                ...
                if (getActiveEditor() != nullptr) return true;
            ...
            {
                if (editController != nullptr) return (int)editController->getParameterCount();
            ...
            void setStateInformation(const void* data, int sizeInBytes) override
            {
                ...
                if (head != nullptr)
                {
                    ...
                    if (editController != nullptr)
                    {
                        ...
                        ComSmartPtr<Steinberg::MemoryStream> controllerStream =
                            createMemoryStreamForState(*head, "IEditController");

                        if (controllerStream != nullptr) editController->setState(controllerStream);
                    }
                }
            }
            ...
            {
                ComSmartPtr<Steinberg::MemoryStream> memoryStream =
                    new Steinberg::MemoryStream(rawData.getData(), (int)rawData.getSize());

                if (memoryStream == nullptr || holder->component == nullptr) return false;

                return Steinberg::Vst::PresetFile::loadPreset(memoryStream, holder->cidOfComponent, holder->component,
                                                              editController, nullptr);
            }
            ...
            struct ParamValueQueueList : public Vst::IParameterChanges
            {
                ...
                Vst::IParamValueQueue* PLUGIN_API addParameterData(const Vst::ParamID& id, Steinberg::int32& index) override
                {
                    ...
                    index                       = numQueuesUsed++;
                    ParamValueQueue* valueQueue = (index < queues.size() ? queues[index] : queues.add(new ParamValueQueue()));
                ...
                struct ParamValueQueue : public Vst::IParamValueQueue
                {
                    ...
                    void setParamID(Vst::ParamID pID) noexcept
                    {
                        paramID = pID;
                    }
        ...
        private:
            ...
            */
            ...
            static AudioProcessor::BusesProperties getBusProperties(ComSmartPtr<Vst::IComponent>& component)
            {
                ...
                for (int dirIdx = 0; dirIdx < 2; ++dirIdx)
                {
                    ...
                    for (int i = 0; i < numBuses; ++i)
                    {
                        ...
                        if (component->getBusInfo(Vst::kAudio, dir, (Steinberg::int32)i, info) != kResultOk) continue;

                        if (info.channelCount == 0) continue;
            ...
            Vst::BusInfo getBusInfo(bool forInput, bool forAudio, int index = 0) const
            {
                ...
                holder->component->getBusInfo(busInfo.mediaType, busInfo.direction, (Steinberg::int32)index, busInfo);
                return busInfo;
            }
            ...
            {
                VST3BufferExchange<FloatType>::mapBufferToBuses(inputBuses, inputBusMap.get<FloatType>(),
                                                                cachedBusLayouts.inputBuses, buffer);
                VST3BufferExchange<FloatType>::mapBufferToBuses(outputBuses, outputBusMap.get<FloatType>(),
            ...
            Vst::ParameterInfo getParameterInfoForIndex(int index) const
            {
                ...
                if (processor != nullptr) editController->getParameterInfo(index, paramInfo);
            ...
            Vst::ProgramListInfo getProgramListInfo(int index) const
            {
                ...
                if (unitInfo != nullptr) unitInfo->getProgramListInfo(index, paramInfo);
            ...
            void syncProgramNames()
            {
                ...
                if (processor == nullptr || editController == nullptr) return;
                ...
                if (unitInfo != nullptr)
                {
                    ...
                    {
                        if (unitInfo->getUnitInfo(idx, uInfo) == kResultOk && uInfo.id == programUnitID)
                        {
                ...
                if (editController != nullptr && paramInfo.stepCount > 0)
                {
                    ...
                    for (int i = 0; i < numPrograms; ++i)
                    {
                        auto valueNormalized =
                            static_cast<Vst::ParamValue>(i) / static_cast<Vst::ParamValue>(paramInfo.stepCount);
...
#pragma warning(pop)
    ...
    {
        if (auto vst3 = dynamic_cast<VST3Classes::VST3PluginInstance*>(api)) return vst3->setStateFromPresetFile(rawData);
                                                ...
                                                void (*callback)(void*, AudioPluginInstance*, const String&))
    {
        ...
        if (fileMightContainThisPluginType(description.fileOrIdentifier))
        {
            ...
            if (const VST3Classes::VST3ModuleHandle::Ptr module =
                    VST3Classes::VST3ModuleHandle::findOrCreateModule(file, description))
            {
                ...
                if (holder->initialise())
                {
                    ...
                    if (!result->initialise()) result.reset();
                }
            }
        ...
        if (result == nullptr) errorMsg = String(NEEDS_TRANS("Unable to load XXX plug-in file")).replace("XXX", "VST-3");
