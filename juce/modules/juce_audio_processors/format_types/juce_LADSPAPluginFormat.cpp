...
#define JUCE_LADSPA_LOG(x)
    ...
    public:
        ...
        ~LADSPAPluginInstance()
        {
            ...
            if (handle != nullptr && plugin != nullptr && plugin->cleanup != nullptr) plugin->cleanup(handle);
        ...
        void initialise(double initialSampleRate, int initialBlockSize)
        {
            ...
            if (initialised || plugin == nullptr || handle == nullptr) return;
            ...
            for (unsigned int i = 0; i < plugin->PortCount; ++i)
            {
                ...
                if ((portDesc & LADSPA_PORT_CONTROL) != 0) parameters.add(i);
        ...
        {
            if (plugin != nullptr && plugin->Label != nullptr) return plugin->Label;
        ...
        int getUID() const
        {
            if (plugin != nullptr && plugin->UniqueID != 0) return (int)plugin->UniqueID;
        ...
        {
            if (handle != nullptr && plugin->deactivate != nullptr) plugin->deactivate(handle);
        ...
        void processBlock(AudioBuffer<float>& buffer, MidiBuffer& midiMessages)
        {
            ...
            if (initialised && plugin != nullptr && handle != nullptr)
            {
                ...
                if (plugin->run_adding != nullptr)
                {
                    ...
                    for (int i = 0; i < outputs.size(); ++i)
                        if (i < buffer.getNumChannels()) buffer.copyFrom(i, 0, tempBuffer, i, 0, numSamples);
        ...
        {
            if (isPositiveAndBelow(index, getTotalNumInputChannels())) return String(plugin->PortNames[inputs[index]]).trim();
        ...
        {
            return plugin != nullptr && (plugin->PortDescriptors[parameters[index]] & LADSPA_PORT_INPUT) != 0;
        }
        ...
        const String getParameterText(int index)
        {
            if (plugin != nullptr)
            {
                ...
                if (LADSPA_IS_HINT_INTEGER(hint.HintDescriptor)) return String((int)parameterValues[index].scaled);
    ...
    private:
        ...
        {
            if (useLog && low > 0 && high > 0) return expf(logf(low) * (1.0f - alpha) + logf(high) * alpha);
        ...
        float getNewParamScaled(const LADSPA_PortRangeHint& hint, float newValue) const
        {
            ...
            if (LADSPA_IS_HINT_TOGGLED(desc)) return (newValue < 0.5f) ? 0.0f : 1.0f;
        ...
        ParameterValue getParamValue(const LADSPA_PortRangeHint& hint) const
        {
            ...
            if (LADSPA_IS_HINT_HAS_DEFAULT(desc))
            {
                ...
                if (LADSPA_IS_HINT_BOUNDED_BELOW(desc) && LADSPA_IS_HINT_DEFAULT_MINIMUM(desc))
                    return ParameterValue(lower, 0.0f);
                if (LADSPA_IS_HINT_BOUNDED_ABOVE(desc) && LADSPA_IS_HINT_DEFAULT_MAXIMUM(desc))
                    return ParameterValue(upper, 1.0f);
                ...
                if (LADSPA_IS_HINT_BOUNDED_BELOW(desc))
                {
                    ...
                    if (LADSPA_IS_HINT_DEFAULT_LOW(desc))
                        return ParameterValue(scaledValue(lower, upper, 0.25f, useLog), 0.25f);
                    if (LADSPA_IS_HINT_DEFAULT_MIDDLE(desc))
                        return ParameterValue(scaledValue(lower, upper, 0.50f, useLog), 0.50f);
                    if (LADSPA_IS_HINT_DEFAULT_HIGH(desc))
                        return ParameterValue(scaledValue(lower, upper, 0.75f, useLog), 0.75f);
                }
            }
    ...
    void LADSPAPluginFormat::findAllTypesForFile(OwnedArray<PluginDescription>& results, const String& fileOrIdentifier)
    {
        ...
        ScopedPointer<LADSPAPluginInstance> instance(
            dynamic_cast<LADSPAPluginInstance*>(createInstanceFromDescription(desc, 44100.0, 512)));

        if (instance == nullptr || !instance->isValid()) return;
                                                  ...
                                                  void (*callback)(void*, AudioPluginInstance*, const String&))
    {
        ...
        if (result == nullptr) errorMsg = String(NEEDS_TRANS("Unable to load XXX plug-in file")).replace("XXX", "LADSPA");
