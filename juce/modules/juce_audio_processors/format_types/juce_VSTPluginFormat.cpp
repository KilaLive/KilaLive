...
namespace juce
{
    ...
    namespace
    {
        ...
        {
            return magic == (int32)ByteOrder::littleEndianInt(name) || magic == (int32)ByteOrder::bigEndianInt(name);
        }

        static int32 fxbName(const char* name) noexcept
        {
            return (int32)ByteOrder::littleEndianInt(name);
        }
        static int32 fxbSwap(int32 x) noexcept
        {
    ...
    namespace
    {
        ...
        struct IdleCallRecursionPreventer
        {
            ...
            {
                if (isMessageThread) ++insideVSTCallback;
            }
...
//==============================================================================
    ...
    namespace
    {
        ...
        Window getChildWindow(Window windowToCheck)
        {
            ...
            if (numChildren > 0) return childWindows[0];
    ...
    struct ModuleHandle : public ReferenceCountedObject
    {
        ...
        static Ptr findOrCreateModule(const File& file)
        {
            for (auto* module : getActiveModules())
                if (module->file == file) return module;
        ...
        bool open()
        {
            if (moduleMain != nullptr) return true;
            ...
            if (moduleMain == nullptr) moduleMain = (MainCall)module.getFunction("main");
...
#if JUCE_WINDOWS
                if (vstXml == nullptr) vstXml = XmlDocument::parse(getDLLResource(file, "VSTXML", 1));
#endif
            }
        ...
        bool open()
        {
            ...
            if (file.hasFileExtension(".vst"))
            {
                ...
                if (CFURLRef url = CFURLCreateFromFileSystemRepresentation(0, (const UInt8*)utf8, (CFIndex)strlen(utf8),
                                                                           file.isDirectory()))
                {
                            ...
                            if (moduleMain != nullptr)
                            {
                                if (CFTypeRef name = CFBundleGetValueForInfoDictionaryKey(bundleRef, CFSTR("CFBundleName")))
                                {
                                    if (CFGetTypeID(name) == CFStringGetTypeID())
                                    {
                                        ...
                                        if (CFStringGetCString((CFStringRef)name, buffer, sizeof(buffer),
                                                               CFStringGetSystemEncoding()))
                                            pluginName = buffer;
                                    }
                                }

                                if (pluginName.isEmpty()) pluginName = file.getFileNameWithoutExtension();
                                ...
                                if (!vstXmlFiles.isEmpty()) vstXml.reset(XmlDocument::parse(vstXmlFiles.getReference(0)));
                            }
                        }
        ...
        void close()
        {
            if (bundleRef != 0)
            {
                ...
                if (CFGetRetainCount(bundleRef) == 1) CFBundleUnloadExecutable(bundleRef);

                if (CFGetRetainCount(bundleRef) > 0) CFRelease(bundleRef);
            }
        }
...
#pragma warning(disable : 4996)  // warning about overriding deprecated methods
    ...
    //==============================================================================
    //==============================================================================
    struct VSTPluginInstance : public AudioPluginInstance, private Timer, private AsyncUpdater
    {
            ...
            if (vstEffect != nullptr && vstEffect->interfaceIdentifier == juceVstInterfaceIdentifier)
            {
                struct VSTDeleter : public CallbackMessage
                {
                    VSTDeleter(VSTPluginInstance& inInstance, WaitableEvent& inEvent)
                        : vstInstance(inInstance)
                        , completionSignal(inEvent)
                    {
                    }
        ...
        void cleanup()
        {
            ...
            {
#if JUCE_MAC
                if (vstModule->resFileId != 0) UseResFile(vstModule->resFileId);
#endif
        ...
        static VSTPluginInstance* create(const ModuleHandle::Ptr& newModule, double initialSampleRate, int initialBlockSize)
        {
            if (auto* newEffect = constructEffect(newModule))
            {
                ...
                newEffect->dispatchFunction(newEffect, plugInOpcodeSetSampleRate, 0, 0, 0,
        ...
        void fillInPluginDescription(PluginDescription& desc) const override
        {
...

            {
                ...
                if (desc.descriptiveName.isEmpty()) desc.descriptiveName = name;
            }
        ...
        {
            if (initialised || vstEffect == nullptr) return;
            ...
            if (getSampleRate() > 0) dispatch(plugInOpcodeSetSampleRate, 0, 0, 0, (float)getSampleRate());

            if (getBlockSize() > 0) dispatch(plugInOpcodeSetBlockSize, 0, jmax(32, getBlockSize()), 0, 0);
        ...
        int getUID() const
        {
            ...
            if (uid == 0) uid = vstModule->file.hashCode();
        ...
        {
            if (vstEffect == nullptr) return 0.0;
        ...
        VstPlugInCategory getVstCategory() const noexcept
        {
            return (VstPlugInCategory)dispatch(plugInOpcodeGetPlugInCategory, 0, 0, 0, 0);
        }

        int pluginCanDo(const char* text) const
        {
            return (int)dispatch(plugInOpcodeCanPlugInDo, 0, 0, (void*)text, 0);
        }
        ...
        void prepareToPlay(double rate, int samplesPerBlockExpected) override
        {
            ...
            vstHostTime.flags = vstTimingInfoFlagNanosecondsValid | vstTimingInfoFlagAutomationWriteModeActive
            ...
            if (initialised)
            {
                ...
                {
                    int32 vstPrecision =
                        isUsingDoublePrecision() ? vstProcessingSampleTypeDouble : vstProcessingSampleTypeFloat;
        ...
        bool isBusesLayoutSupported(const BusesLayout& layouts) const override
        {
            ...
            if (numInputBuses > 1 || numOutputBuses > 1) return (layouts == getBusesLayout());
        ...
        bool hasEditor() const override
        {
            return vstEffect != nullptr && (vstEffect->flags & vstEffectFlagHasEditor) != 0;
        }
#endif
        ...
        {
            if (!isValidChannel(index, true)) return false;
        ...
        {
            if (!isValidChannel(index, false)) return false;
        ...
        {
            return isPositiveAndBelow(index, isInput ? getTotalNumInputChannels() : getTotalNumOutputChannels());
        }

        //==============================================================================
        int getNumParameters() override
        {
            return vstEffect != nullptr ? vstEffect->numParameters : 0;
        }
        ...
        {
            return getTextForOpcode(index, plugInOpcodeGetParameterName);
        }
        const String getParameterText(int index) override
        {
            return getTextForOpcode(index, plugInOpcodeGetParameterText);
        }
        String getParameterLabel(int index) const override
        {
            return getTextForOpcode(index, plugInOpcodeGetParameterLabel);
        }
        ...
        {
            return (int)dispatch(plugInOpcodeGetCurrentProgram, 0, 0, 0, 0);
        }
        ...
        const String getProgramName(int index) override
        {
            if (index >= 0)
            {
                if (index == getCurrentProgram()) return getCurrentProgramName();
        ...
        {
            if (dispatch(plugInOpcodeIdle, 0, 0, 0, 0) == 0) stopTimer();
        }
        ...
        pointer_sized_int handleCallback(int32 opcode, int32 index, pointer_sized_int value, void* ptr, float opt)
        {
            ...
            case hostOpcodeParameterChanged:
                ...
                break;
            case hostOpcodePreAudioProcessingEvents:
                handleMidiFromPlugin((const VstEventBlock*)ptr);
                break;
            case hostOpcodeGetTimingInfo:
                return getVSTTime();
            case hostOpcodeIdle:
                handleIdle();
                break;
            case hostOpcodeWindowSize:
                setWindowSize(index, (int)value);
                return 1;
            case hostOpcodeUpdateView:
                triggerAsyncUpdate();
                break;
            case hostOpcodeIOModified:
                setLatencySamples(vstEffect->latency);
                break;
            case hostOpcodeNeedsIdle:
                startTimer(50);
                break;

            case hostOpcodeGetSampleRate:
                return (pointer_sized_int)(getSampleRate() > 0 ? getSampleRate() : defaultVSTSampleRateValue);
            case hostOpcodeGetBlockSize:
                return (pointer_sized_int)(getBlockSize() > 0 ? getBlockSize() : defaultVSTBlockSizeValue);
            case hostOpcodePlugInWantsMidi:
                wantsMidiMessages = true;
                break;
            case hostOpcodeGetDirectory:
                return getVstDirectory();

            case hostOpcodeTempoAt:
                return (pointer_sized_int)(extraFunctions != nullptr ? extraFunctions->getTempoAt((int64)value) : 0);
            case hostOpcodeGetAutomationState:
                return (pointer_sized_int)(extraFunctions != nullptr ? extraFunctions->getAutomationState() : 0);
        ...
        static pointer_sized_int handleGeneralCallback(int32 opcode, int32 /*index*/, pointer_sized_int /*value*/, void* ptr,
                                                       float /*opt*/)
        {
            switch (opcode)
            {
            case hostOpcodeCanHostDo:
                return handleCanDo((const char*)ptr);
            case hostOpcodeVstVersion:
                return 2400;
            case hostOpcodeCurrentId:
                return shellUIDToCreate;
            case hostOpcodeGetNumberOfAutomatableParameters:
                return 0;
            case hostOpcodeGetAutomationState:
                return 1;
            case hostOpcodeGetManufacturerVersion:
                return 0x0101;

            case hostOpcodeGetManufacturerName:
            case hostOpcodeGetProductName:
                return getHostName((char*)ptr);

            case hostOpcodeGetSampleRate:
                return (pointer_sized_int)defaultVSTSampleRateValue;
            case hostOpcodeGetBlockSize:
                return (pointer_sized_int)defaultVSTBlockSizeValue;
            case hostOpcodeSetOutputSampleRate:
                return 0;
        ...
        bool loadFromFXBFile(const void* const data, const size_t dataSize)
        {
            ...
            if ((!compareMagic(set->chunkMagic, "CcnK")) || fxbSwap(set->version) > fxbVersionNum) return false;
                ...
                if (fxbSwap(set->numPrograms) >= 0)
                {
                    ...
                    for (int i = 0; i < fxbSwap(set->numPrograms); ++i)
                    {
                        if (i != oldProg)
                        {
                            ...
                            if (((const char*)prog) - ((const char*)set) >= (ssize_t)dataSize) return false;
            ...
            else if (compareMagic(set->fxMagic, "FxCk"))
            {
                ...
                for (int i = 0; i < fxbSwap(prog->numParams); ++i) setParameter(i, fxbSwapFloat(prog->params[i]));
            }
            else if (compareMagic(set->fxMagic, "FBCh"))
            {
                ...
                if ((size_t)fxbSwap(cset->chunkSize) + sizeof(fxChunkSet) - 8 > (size_t)dataSize) return false;
            ...
            else if (compareMagic(set->fxMagic, "FPCh"))
            {
                ...
                if ((size_t)fxbSwap(cset->chunkSize) + sizeof(fxProgramSet) - 8 > (size_t)dataSize) return false;
        ...
        bool saveToFXBFile(MemoryBlock& dest, bool isFXB, int maxSizeMB = 128)
        {
            ...
            else
            {
                if (isFXB)
                {
                    ...
                    if (oldProgram >= 0) setCurrentProgram(oldProgram);
        ...
        {
            return vstEffect != nullptr && (vstEffect->flags & vstEffectFlagDataInChunks) != 0;
        }
        ...
        bool setChunkData(const void* data, const int size, bool isPreset)
        {
            if (size > 0 && usesChunks())
            {
                ...
                if (!isPreset) updateStoredProgramNames();
    ...
    private:
        ...
        {
            static const char* canDos[] = {"supplyIdle",      "sendVstEvents",    "sendVstMidiEvent",
                                           "sendVstTimeInfo", "receiveVstEvents", "receiveVstMidiEvent",
                                           "supportShell",    "sizeWindow",       "shellCategory"};

            for (int i = 0; i < numElementsInArray(canDos); ++i)
                if (strcmp(canDos[i], name) == 0) return 1;
        ...
        static pointer_sized_int getHostName(char* name)
        {
            ...
            if (auto* app = JUCEApplicationBase::getInstance()) hostName = app->getApplicationName();
...
#pragma warning(pop)
        ...
        void handleIdle()
        {
            if (insideVSTCallback == 0 && MessageManager::getInstance()->isThisTheMessageThread())
            {
...
#if JUCE_MAC
                if (getActiveEditor() != nullptr) dispatch(plugInOpcodeEditorIdle, 0, 0, 0, 0);
#endif
                ...
                for (int i = ComponentPeer::getNumPeers(); --i >= 0;)
                    if (auto* p = ComponentPeer::getPeer(i)) p->performAnyPendingRepaintsNow();
            }
        }
...
#if JUCE_MAC
                if (module->resFileId != 0) UseResFile(module->resFileId);
#endif
        ...
        static BusesProperties queryBusIO(VstEffectInterface* effect)
        {
            ...
            if (effect->numInputChannels == 0 && effect->numOutputChannels == 0) return returnValue;
            ...
            {
                SpeakerMappings::VstSpeakerConfigurationHolder canonicalIn(
                    AudioChannelSet::canonicalChannelSet(effect->numInputChannels));
                SpeakerMappings::VstSpeakerConfigurationHolder canonicalOut(
                    AudioChannelSet::canonicalChannelSet(effect->numOutputChannels));
            ...
            if (!getSpeakerArrangementWrapper(effect, inArr, outArr)) inArr = outArr = nullptr;
            ...
            for (int dir = 0; dir < 2; ++dir)
            {
                ...
                {
                    if (effect->dispatchFunction(effect, opcode, ch, 0, &pinProps, 0.0f) == 0) break;
                    ...
                    if ((pinProps.flags & vstPinInfoFlagValid) != 0)
                    {
                        ...
                        if (layout.isDisabled()) break;
                    }
                    else if (arr == nullptr)
                    {
                        layout = ((pinProps.flags & vstPinInfoFlagIsStereo) != 0 ? AudioChannelSet::stereo()
                ...
                if (!busAdded && maxChannels > 0)
                {
                    ...
                    if (effect->dispatchFunction(effect, opcode, 0, 0, &pinProps, 0.0f) != 0) busName = pinProps.text;
        ...
        static bool pluginHasDefaultChannelLayouts(VstEffectInterface* effect)
        {
            ...
            if (getSpeakerArrangementWrapper(effect, inArr, outArr)) return true;
            ...
            for (int dir = 0; dir < 2; ++dir)
            {
                ...
                for (int ch = 0; ch < maxChannels; ch += channels)
                {
                    ...
                    if (effect->dispatchFunction(effect, opcode, ch, 0, &pinProps, 0.0f) == 0) return false;

                    if ((pinProps.flags & vstPinInfoFlagValid) != 0) return true;
        ...
        static bool getSpeakerArrangementWrapper(VstEffectInterface* effect, VstSpeakerConfiguration* inArr,
                                                 VstSpeakerConfiguration* outArr)
        {
            ...
            if (effect->numInputChannels == 0) return false;

            return (effect->dispatchFunction(effect, plugInOpcodeGetSpeakerArrangement, 0,
                                             reinterpret_cast<pointer_sized_int>(&inArr), &outArr, 0.0f)
                    != 0);
        }

        //==============================================================================
        template <typename FloatType>
        void processAudio(AudioBuffer<FloatType>& buffer, MidiBuffer& midiMessages, AudioBuffer<FloatType>& tmpBuffer,
                          HeapBlock<FloatType*>& channelBuffer)
        {
            ...
            if (initialised)
            {
                if (auto* currentPlayHead = getPlayHead())
                {
                    ...
                    if (currentPlayHead->getCurrentPosition(position))
                    {
                        ...
                        vstHostTime.flags |= vstTimingInfoFlagTempoValid | vstTimingInfoFlagTimeSignatureValid
                                             | vstTimingInfoFlagMusicalPositionValid | vstTimingInfoFlagLastBarPositionValid;
                        ...
                        if (newTransportFlags
                            != (vstHostTime.flags
                                & (vstTimingInfoFlagCurrentlyPlaying | vstTimingInfoFlagCurrentlyRecording)))
                            vstHostTime.flags = (vstHostTime.flags
                                                 & ~(vstTimingInfoFlagCurrentlyPlaying | vstTimingInfoFlagCurrentlyRecording))
                                                | newTransportFlags | vstTimingInfoFlagTransportChanged;
                        else
                        ...
                        case AudioPlayHead::fps24:
                            setHostTimeFrameRate(vstSmpteRateFps24, 24.0, position.timeInSeconds);
                            break;
                        case AudioPlayHead::fps25:
                            setHostTimeFrameRate(vstSmpteRateFps25, 25.0, position.timeInSeconds);
                            break;
                        case AudioPlayHead::fps30:
                            setHostTimeFrameRate(vstSmpteRateFps30, 30.0, position.timeInSeconds);
                            break;
                        case AudioPlayHead::fps60:
                            setHostTimeFrameRate(vstSmpteRateFps60, 60.0, position.timeInSeconds);
                            break;

                        case AudioPlayHead::fps23976:
                            setHostTimeFrameRateDrop(vstSmpteRateFps239, 24.0, position.timeInSeconds);
                            break;
                        case AudioPlayHead::fps2997:
                            setHostTimeFrameRateDrop(vstSmpteRateFps2997, 30.0, position.timeInSeconds);
                            break;
                        case AudioPlayHead::fps2997drop:
                            setHostTimeFrameRateDrop(vstSmpteRateFps2997drop, 30.0, position.timeInSeconds);
                            break;
                        case AudioPlayHead::fps30drop:
                            setHostTimeFrameRateDrop(vstSmpteRateFps30drop, 30.0, position.timeInSeconds);
                            break;
                        case AudioPlayHead::fps60drop:
                            setHostTimeFrameRateDrop(vstSmpteRateFps599, 60.0, position.timeInSeconds);
                            break;
                        default:
                            break;
                        }
                ...
                if (wantsMidiMessages)
                {
                    ...
                    while (iter.getNextEvent(midiData, numBytesOfMidiData, samplePosition))
                        midiEventsToSend.addEvent(midiData, numBytesOfMidiData, jlimit(0, numSamples - 1, samplePosition));

                    vstEffect->dispatchFunction(vstEffect, plugInOpcodePreAudioProcessingEvents, 0, 0,
                                                midiEventsToSend.events, 0);
                }
                ...
                {
                    if (numSamples > tmpBuffer.getNumSamples()) tmpBuffer.setSize(tmpBuffer.getNumChannels(), numSamples);
            ...
            else
            {
                ...
                for (int i = getTotalNumOutputChannels(); --i >= 0;) buffer.clear(i, 0, buffer.getNumSamples());
            }
        ...
        String getCurrentProgramName()
        {
            ...
            if (vstEffect != nullptr)
            {
                ...
                {
                    while (programNames.size() < index) programNames.add(String());
        ...
        void setParamsInProgramBlock(fxProgram* prog)
        {
            ...
            for (int i = 0; i < numParams; ++i) prog->params[i] = fxbSwapFloat(getParameter(i));
        }
        ...
        void createTempParameterStore(MemoryBlock& dest)
        {
            ...
            for (int i = 0; i < getNumParameters(); ++i) p[i] = getParameter(i);
        }
        ...
        void restoreFromTempParameterStore(const MemoryBlock& m)
        {
            ...
            for (int i = 0; i < getNumParameters(); ++i) setParameter(i, p[i]);
        }
        ...
        {
            return vstEffect != nullptr ? vstEffect->plugInVersion : 0;
        }
        ...
        String getVersion() const
        {
            ...
            if (v == 0 || (int)v == -1) v = (unsigned int)getVersionNumber();
...
//==============================================================================
    ...
    public:
...
#if JUCE_WINDOWS
                if (pluginHWND != 0) MoveWindow(pluginHWND, pos.getX(), pos.getY(), getWidth(), getHeight(), TRUE);
#elif JUCE_LINUX
                if (pluginWindow != 0)
                {
                    XMoveResizeWindow(display, pluginWindow, pos.getX(), pos.getY(), (unsigned int)getWidth(),
        ...
        void paint(Graphics& g) override
        {
            if (isOpen)
            {
                ...
                if (pluginWindow != 0)
                {
                    ...
                    XClearArea(display, pluginWindow, clip.getX(), clip.getY(), static_cast<unsigned int>(clip.getWidth()),
        ...
        void timerCallback() override
        {
            if (isShowing())
            {
                ...
                if (pluginWindow == 0)
                {
                    ...
                    if (pluginWindow != 0) componentMovedOrResized(true, true);
                }
#endif
            }
        }
        ...
        void setScaleFactor(float newScale) override
        {
            ...
            dispatch(plugInOpcodeManufacturerSpecific, presonusVendorID, presonusSetContentScaleFactor, nullptr, newScale);
        }

        void sendScaleFactorIfNotUnity()
        {
            if (scaleFactor != 1.0f) setScaleFactor(scaleFactor);
        }
    ...
    private:
        ...
        {
            return plugin.getPluginDescription().manufacturerName.containsIgnoreCase("Loud Technologies");
        }
        ...
        {
            if (isOpen || parentWindow == 0) return;
        ...
        {
            if (isOpen || getWindowHandle() == 0) return;
...
#pragma warning(pop)
            ...
            if (rect != nullptr)
            {
                    ...
                    || ((w == 0 && rw > 0) || (h == 0 && rh > 0)))
                {
                    ...
                    {
                        SetWindowPos(pluginHWND, 0, 0, 0, rw, rh,
            ...
            if (pluginWindow != 0) XMapRaised(display, pluginWindow);
#endif
...
#pragma warning(pop)
        ...
        static LRESULT CALLBACK vstHookWndProc(HWND hW, UINT message, WPARAM wParam, LPARAM lParam)
        {
            for (int i = activeVSTWindows.size(); --i >= 0;)
            {
                ...
                {
                    if (message == WM_CHAR || message == WM_KEYDOWN || message == WM_SYSKEYDOWN || message == WM_KEYUP
                        || message == WM_SYSKEYUP || message == WM_APPCOMMAND)
                    {
                        SendMessage((HWND)w->getTopLevelComponent()->getWindowHandle(), message, wParam, lParam);
                    }

                    if (w != nullptr)  // (may have been deleted in SendMessage callback)
                        return CallWindowProc((WNDPROC)w->originalWndProc, (HWND)w->pluginHWND, message, wParam, lParam);
                }
            }
        ...
        {
#if JUCE_SUPPORT_CARBON
            if (carbonWrapper != nullptr) carbonWrapper->setSize(getWidth(), getHeight());
#endif

            if (cocoaWrapper != nullptr) cocoaWrapper->setSize(getWidth(), getHeight());
        }
#endif
...
#pragma warning(pop)
    ...
    AudioProcessorEditor* VSTPluginInstance::createEditor()
    {
        ...
        return nullptr;
#else
        return hasEditor() ? new VSTPluginWindow(*this) : nullptr;
#endif
    }

    //==============================================================================
    // entry point for all callbacks from the plugin
    static pointer_sized_int VSTINTERFACECALL audioMaster(VstEffectInterface* effect, int32 opcode, int32 index,
    ...
    static VSTPluginInstance* createAndUpdateDesc(VSTPluginFormat& format, PluginDescription& desc)
    {
        if (auto* p = format.createInstanceFromDescription(desc, 44100.0, 512))
        {
            ...
            {
#if JUCE_MAC
                if (instance->vstModule->resFileId != 0) UseResFile(instance->vstModule->resFileId);
#endif
    ...
    void VSTPluginFormat::findAllTypesForFile(OwnedArray<PluginDescription>& results, const String& fileOrIdentifier)
    {
        ...
        else
        {
            ...
            for (;;)
            {
                ...
                if (shellInstance != nullptr)
                {
                    ...
                    if (!arrayContainsPlugin(results, desc)) results.add(new PluginDescription(desc));
                }
            }
        }
    }

    void VSTPluginFormat::createPluginInstance(const PluginDescription& desc, double sampleRate, int blockSize,
                                               void* userData, void (*callback)(void*, AudioPluginInstance*, const String&))
    {
        ...
        if (fileMightContainThisPluginType(desc.fileOrIdentifier))
        {
            ...
            if (auto module = ModuleHandle::findOrCreateModule(file))
            {
                ...
                if (result != nullptr && !result->initialiseEffect(sampleRate, blockSize)) result.reset();
            }
        ...
        if (result == nullptr) errorMsg = String(NEEDS_TRANS("Unable to load XXX plug-in file")).replace("XXX", "VST-2");
    ...
    void VSTPluginFormat::recursiveFileSearch(StringArray& results, const File& dir, const bool recursive)
    {
        ...
        while (iter.next())
        {
            ...
            if (recursive && (!isPlugin) && f.isDirectory()) recursiveFileSearch(results, f, true);
        }
    }
    ...
    FileSearchPath VSTPluginFormat::getDefaultLocationsToSearch()
    {
        ...
        return FileSearchPath(
            SystemStats::getEnvironmentVariable("VST_PATH", "/usr/lib/vst;/usr/local/lib/vst;~/.vst").replace(":", ";"));
#elif JUCE_WINDOWS
    ...
    const XmlElement* VSTPluginFormat::getVSTXML(AudioPluginInstance* plugin)
    {
        if (auto* vst = dynamic_cast<VSTPluginInstance*>(plugin))
            if (vst->vstModule != nullptr) return vst->vstModule->vstXml.get();
    ...
    {
        if (auto* vst = dynamic_cast<VSTPluginInstance*>(plugin)) return vst->loadFromFXBFile(data, dataSize);
    ...
    {
        if (auto* vst = dynamic_cast<VSTPluginInstance*>(plugin)) return vst->saveToFXBFile(dest, asFXB);
    ...
    {
        if (auto* vst = dynamic_cast<VSTPluginInstance*>(plugin)) return vst->getChunkData(result, isPreset, 128);
    ...
    {
        if (auto* vst = dynamic_cast<VSTPluginInstance*>(plugin)) return vst->setChunkData(data, size, isPreset);

        return false;
    }

    AudioPluginInstance* VSTPluginFormat::createCustomVSTFromMainCall(void* entryPointFunction, double initialSampleRate,
                                                                      int initialBufferSize)
    {
        ...
        if (module->open())
        {
            ...
            if (result != nullptr)
                if (result->initialiseEffect(initialSampleRate, initialBufferSize)) return result.release();
        }
    ...
    void VSTPluginFormat::setExtraFunctions(AudioPluginInstance* plugin, ExtraFunctions* functions)
    {
        ...
        if (auto* vst = dynamic_cast<VSTPluginInstance*>(plugin)) std::swap(vst->extraFunctions, f);
    }
    ...
    pointer_sized_int JUCE_CALLTYPE VSTPluginFormat::dispatcher(AudioPluginInstance* plugin, int32 opcode, int32 index,
                                                                pointer_sized_int value, void* ptr, float opt)
    {
        if (auto* vst = dynamic_cast<VSTPluginInstance*>(plugin)) return vst->dispatch(opcode, index, value, ptr, opt);
