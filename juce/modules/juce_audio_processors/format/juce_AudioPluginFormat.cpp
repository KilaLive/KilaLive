...
namespace juce
{
    namespace AudioPluginFormatHelpers
    {
        struct CallbackInvoker
        {
            struct InvokeOnMessageThread : public CallbackMessage
            {
                ...
                {
                    compCallback->completionCallback(instance, error);
                }
            ...
            //==============================================================================
            CallbackInvoker(AudioPluginFormat::InstantiationCompletionCallback* cc)
                : completion(cc)
            {
            }
    ...
    struct EventSignaler : public AudioPluginFormat::InstantiationCompletionCallback
    {
        EventSignaler(WaitableEvent& inEvent, AudioPluginInstance*& inInstance, String& inErrorMessage)
            ...
            , outErrorMessage(inErrorMessage)
        {
        }
    ...
    AudioPluginInstance* AudioPluginFormat::createInstanceFromDescription(const PluginDescription& desc,
                                                                          ...
                                                                          String& errorMessage)
    {
        if (MessageManager::getInstance()->isThisTheMessageThread() && requiresUnblockedMessageThreadDuringCreation(desc))
        {
        ...
        if (!MessageManager::getInstance()->isThisTheMessageThread())
            ...
            createPluginInstance(desc, initialSampleRate, initialBufferSize, eventSignaler.get(),
                                 EventSignaler::staticCompletionCallback);
    ...
    void AudioPluginFormat::createPluginInstanceAsync(const PluginDescription& description, double initialSampleRate,
                                                      ...
                                                      AudioPluginFormat::InstantiationCompletionCallback* callback)
    {
        ...
        struct InvokeOnMessageThread : public CallbackMessage
        {
            InvokeOnMessageThread(AudioPluginFormat* myself, const PluginDescription& descriptionParam,
                                  double initialSampleRateParam, int initialBufferSizeParam,
                                  AudioPluginFormat::InstantiationCompletionCallback* callbackParam)
                : owner(myself)
                , descr(descriptionParam)
                , sampleRate(initialSampleRateParam)
                , bufferSize(initialBufferSizeParam)
                , call(callbackParam)
            {
            }
