...
namespace juce
{
    ...
    struct GraphRenderSequence
    {
        ...
        void perform(AudioBuffer<FloatType>& buffer, MidiBuffer& midiMessages, AudioPlayHead* audioPlayHead)
        {
            ...
            if (numSamples > maxSamples)
            {
                ...
                AudioBuffer<FloatType> endAudio(buffer.getArrayOfWritePointers(), buffer.getNumChannels(), maxSamples,
            ...
            {
                const Context context{renderingBuffer.getArrayOfWritePointers(), midiBuffers.begin(), audioPlayHead,
                                      numSamples};

                for (auto* op : renderOps) op->perform(context);
            }
        ...
        {
            createOp([=](const Context& c) {
                FloatVectorOperations::copy(c.audioBuffers[dstIndex], c.audioBuffers[srcIndex], c.numSamples);
            });
        }
        ...
        {
            createOp([=](const Context& c) {
                FloatVectorOperations::add(c.audioBuffers[dstIndex], c.audioBuffers[srcIndex], c.numSamples);
            });
        }
        ...
        {
            createOp(
                [=](const Context& c) { c.midiBuffers[dstIndex].addEvents(c.midiBuffers[srcIndex], 0, c.numSamples, 0); });
        }
        ...
        void addProcessOp(const AudioProcessorGraph::Node::Ptr& node, const Array<int>& audioChannelsUsed, int totalNumChans,
                          int midiBuffer)
        {
        ...
        void prepareBuffers(int blockSize)
        {
            ...
            for (auto&& m : midiBuffers) m.ensureSize(defaultMIDIBufferSize);
        }
    ...
    private:
        ...
        void createOp(LambdaType&& fn)
        {
            ...
            {
                LambdaOp(LambdaType&& f)
                    : function(static_cast<LambdaType&&>(f))
                {
                }
        ...
        {
            ProcessOp(const AudioProcessorGraph::Node::Ptr& n, const Array<int>& audioChannelsUsed, int totalNumChans,
                      int midiBuffer)
                : node(n)
                ...
                , midiBufferToUse(midiBuffer)
            {
                ...
                while (audioChannelsToUse.size() < totalChans) audioChannelsToUse.add(0);
            }
            ...
            void perform(const Context& c) override
            {
                ...
                for (int i = 0; i < totalChans; ++i) audioChannels[i] = c.audioBuffers[audioChannelsToUse.getUnchecked(i)];
    ...
    struct RenderSequenceBuilder
    {
        ...
        struct AssignedBuffer
        {
            ...
            {
                return {{(NodeID)zeroNodeID, 0}};
            }
            static AssignedBuffer createFree() noexcept
            {
                return {{(NodeID)freeNodeID, 0}};
            }

            bool isReadOnlyEmpty() const noexcept
            {
                return channel.nodeID == (NodeID)zeroNodeID;
            }
            bool isFree() const noexcept
            {
                return channel.nodeID == (NodeID)freeNodeID;
            }
            ...
            {
                channel = {(NodeID)freeNodeID, 0};
            }
            void setAssignedToNonExistentNode() noexcept
            {
                channel = {(NodeID)anonNodeID, 0};
            }
        ...
        int getInputLatencyForNode(NodeID nodeID) const
        {
            ...
            for (auto&& c : graph.getConnections())
                if (c.destination.nodeID == nodeID) maxLatency = jmax(maxLatency, getNodeDelay(c.source.nodeID));
        ...
        void createOrderedNodeList()
        {
            for (auto* node : graph.getNodes())
            {
                ...
                for (; j < orderedNodes.size(); ++j)
                    if (graph.isAnInputTo(*node, *orderedNodes.getUnchecked(j))) break;

                orderedNodes.insert(j, node);
            }
        }

        int findBufferForInputAudioChannel(AudioProcessorGraph::Node& node, const int inputChan, const int ourRenderingIndex,
                                           const int maxLatency)
        {
            ...
            if (sources.isEmpty())
            {
                if (inputChan >= numOuts) return readOnlyEmptyBufferIndex;
            ...
            if (sources.size() == 1)
            {
                ...
                if (nodeDelay < maxLatency) sequence.addDelayChannelOp(bufIndex, maxLatency - nodeDelay);
        ...
        int findBufferForInputMidiChannel(AudioProcessorGraph::Node& node, int ourRenderingIndex)
        {
            ...
            if (sources.isEmpty())
            {
                ...
                if (processor.acceptsMidi() || processor.producesMidi()) sequence.addClearMidiBufferOp(midiBufferToUse);
            ...
            for (int i = 0; i < sources.size(); ++i)
            {
                if (i != reusableInputIndex)
                {
                    ...
                    if (srcIndex >= 0) sequence.addAddMidiBufferOp(srcIndex, midiBufferToUse);
                }
            }
        ...
        void createRenderingOpsForNode(AudioProcessorGraph::Node& node, const int ourRenderingIndex)
        {
            ...
            for (int inputChan = 0; inputChan < numIns; ++inputChan)
            {
                ...
                if (inputChan < numOuts) audioBuffers.getReference(index).channel = {node.nodeID, inputChan};
            }
            ...
            if (numOuts == 0) totalLatency = maxLatency;

            sequence.addProcessOp(&node, audioChannelsToUse, totalChans, midiBufferToUse);
        }

        //==============================================================================
        Array<AudioProcessorGraph::NodeAndChannel> getSourcesForChannel(AudioProcessorGraph::Node& node,
                                                                        int                        inputChannelIndex)
        {
            ...
            for (auto&& c : graph.getConnections())
                if (c.destination == nc) results.add(c.source);
        ...
        {
            for (auto& b : buffers)
                if (b.isAssigned() && !isBufferNeededLater(stepIndex, -1, b.channel)) b.setFree();
        }

        bool isBufferNeededLater(int stepIndexToSearchFrom, int inputChannelOfIndexToIgnore,
            ...
            while (stepIndexToSearchFrom < orderedNodes.size())
            {
                ...
                else
                {
                    for (int i = 0; i < node->getProcessor()->getTotalNumInputChannels(); ++i)
                        if (i != inputChannelOfIndexToIgnore && graph.isConnected({output, {node->nodeID, i}})) return true;
                }
    ...
    bool AudioProcessorGraph::Connection::operator<(const Connection& other) const noexcept
    {
        if (source.nodeID != other.source.nodeID) return source.nodeID < other.source.nodeID;

        if (destination.nodeID != other.destination.nodeID) return destination.nodeID < other.destination.nodeID;

        if (source.channelIndex != other.source.channelIndex) return source.channelIndex < other.source.channelIndex;
    ...
    void AudioProcessorGraph::Node::prepare(double newSampleRate, int newBlockSize, AudioProcessorGraph* graph,
                                            ProcessingPrecision precision)
    {
        if (!isPrepared)
        {
            ...
            processor->setProcessingPrecision(processor->supportsDoublePrecisionProcessing() ? precision : singlePrecision);
    ...
    {
        return otherNode == other.otherNode && thisChannel == other.thisChannel && otherChannel == other.otherChannel;
    }
    ...
    void AudioProcessorGraph::topologyChanged()
    {
        ...
        if (isPrepared) triggerAsyncUpdate();
    }
    ...
    {
        if (node != nullptr) return removeNode(node->nodeID);
    ...
    std::vector<AudioProcessorGraph::Connection> AudioProcessorGraph::getConnections() const
    {
        ...
        for (auto& n : nodes) getNodeConnections(*n, connections);
    ...
    bool AudioProcessorGraph::isConnected(Node* source, int sourceChannel, Node* dest, int destChannel) const noexcept
    {
        for (auto& o : source->outputs)
            if (o.otherNode == dest && o.thisChannel == sourceChannel && o.otherChannel == destChannel) return true;
    ...
    bool AudioProcessorGraph::isConnected(const Connection& c) const noexcept
    {
        if (auto* source = getNodeForId(c.source.nodeID))
            if (auto* dest = getNodeForId(c.destination.nodeID))
                return isConnected(source, c.source.channelIndex, dest, c.destination.channelIndex);
        ...
        if (auto* source = getNodeForId(srcID))
            if (auto* dest = getNodeForId(destID))
                for (auto& out : source->outputs)
                    if (out.otherNode == dest) return true;
    ...
    bool AudioProcessorGraph::isAnInputTo(Node& src, Node& dst, int recursionCheck) const noexcept
    {
        for (auto&& i : dst.inputs)
            if (i.otherNode == &src) return true;

        if (recursionCheck > 0)
            for (auto&& i : dst.inputs)
                if (isAnInputTo(src, *i.otherNode, recursionCheck - 1)) return true;
    ...
    bool AudioProcessorGraph::canConnect(Node* source, int sourceChannel, Node* dest, int destChannel) const noexcept
    {
        ...
        if (sourceChannel < 0 || destChannel < 0 || source == dest || sourceIsMIDI != destIsMIDI) return false;

        if (source == nullptr || (!sourceIsMIDI && sourceChannel >= source->processor->getTotalNumOutputChannels())
            || (sourceIsMIDI && !source->processor->producesMidi()))
            return false;

        if (dest == nullptr || (!destIsMIDI && destChannel >= dest->processor->getTotalNumInputChannels())
    ...
    bool AudioProcessorGraph::disconnectNode(NodeID nodeID)
    {
        if (auto* node = getNodeForId(nodeID))
        {
            ...
            {
                for (auto c : connections) removeConnection(c);
    ...
    bool AudioProcessorGraph::removeIllegalConnections()
    {
        ...
        for (auto* node : nodes)
        {
            ...
            for (auto c : connections)
                if (!isConnectionLegal(c)) anyRemoved = removeConnection(c) || anyRemoved;
        }
    ...
    {
        for (auto* node : nodes)
            if (!node->isPrepared) return true;
    ...
    void AudioProcessorGraph::buildRenderingSequence()
    {
        ...
        if (anyNodesNeedPreparing())
        {
            ...
            for (auto* node : nodes) node->prepare(getSampleRate(), getBlockSize(), this, getProcessingPrecision());
        }
    ...
    {
        if (renderSequenceFloat != nullptr) renderSequenceFloat->prepareBuffers(estimatedSamplesPerBlock);

        if (renderSequenceDouble != nullptr) renderSequenceDouble->prepareBuffers(estimatedSamplesPerBlock);
    ...
    void AudioProcessorGraph::releaseResources()
    {
        ...
        for (auto* n : nodes) n->unprepare();

        if (renderSequenceFloat != nullptr) renderSequenceFloat->releaseBuffers();

        if (renderSequenceDouble != nullptr) renderSequenceDouble->releaseBuffers();
    }
    ...
    void AudioProcessorGraph::reset()
    {
        ...
        for (auto* n : nodes) n->getProcessor()->reset();
    }
    ...
    void AudioProcessorGraph::setNonRealtime(bool isProcessingNonRealtime) noexcept
    {
        ...
        for (auto* n : nodes) n->getProcessor()->setNonRealtime(isProcessingNonRealtime);
    }
    ...
    void AudioProcessorGraph::processBlock(AudioBuffer<float>& buffer, MidiBuffer& midiMessages)
    {
        ...
        if (renderSequenceFloat != nullptr) renderSequenceFloat->perform(buffer, midiMessages, getPlayHead());
    }
    ...
    void AudioProcessorGraph::processBlock(AudioBuffer<double>& buffer, MidiBuffer& midiMessages)
    {
        ...
        if (renderSequenceDouble != nullptr) renderSequenceDouble->perform(buffer, midiMessages, getPlayHead());
    }
    ...
    void AudioProcessorGraph::AudioGraphIOProcessor::fillInPluginDescription(PluginDescription& d) const
    {
        ...
        if (type == audioOutputNode && graph != nullptr) d.numInputChannels = graph->getTotalNumInputChannels();

        d.numOutputChannels = getTotalNumOutputChannels();

        if (type == audioInputNode && graph != nullptr) d.numOutputChannels = graph->getTotalNumOutputChannels();
    }
    ...
    void AudioProcessorGraph::AudioGraphIOProcessor::setParentGraph(AudioProcessorGraph* const newGraph)
    {
        ...
        if (graph != nullptr)
        {
            setPlayConfigDetails(type == audioOutputNode ? graph->getTotalNumOutputChannels() : 0,
                                 type == audioInputNode ? graph->getTotalNumInputChannels() : 0, getSampleRate(),
                                 getBlockSize());
