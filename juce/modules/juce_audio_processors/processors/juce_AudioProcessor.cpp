...
namespace juce
{
    ...
    //==============================================================================
    StringArray AudioProcessor::getAlternateDisplayNames() const
    {
    ...
    bool AudioProcessor::addBus(bool isInput)
    {
        ...
        if (!canApplyBusCountChange(isInput, true, busesProps)) return false;
    ...
    bool AudioProcessor::removeBus(bool inputBus)
    {
        ...
        if (numBuses == 0) return false;

        if (!canRemoveBus(inputBus)) return false;

        BusProperties busesProps;

        if (!canApplyBusCountChange(inputBus, false, busesProps)) return false;
    ...
    {
        jassert(arr.inputBuses.size() == getBusCount(true) && arr.outputBuses.size() == getBusCount(false));

        if (arr == getBusesLayout()) return true;

        auto copy = arr;

        if (!canApplyBusesLayout(copy)) return false;
    ...
    bool AudioProcessor::setBusesLayoutWithoutEnabling(const BusesLayout& arr)
    {
        ...
        jassert(arr.inputBuses.size() == numIns && arr.outputBuses.size() == numOuts);

        auto request = arr;
        auto current = getBusesLayout();

        for (int i = 0; i < numIns; ++i)
            if (request.getNumChannels(true, i) == 0) request.getChannelSet(true, i) = current.getChannelSet(true, i);

        for (int i = 0; i < numOuts; ++i)
            if (request.getNumChannels(false, i) == 0) request.getChannelSet(false, i) = current.getChannelSet(false, i);

        if (!checkBusesLayoutSupported(request)) return false;
        ...
        for (int dir = 0; dir < 2; ++dir)
        {
            ...
            for (int i = 0; i < (isInput ? numIns : numOuts); ++i)
            {
                ...
                if (!bus.isEnabled())
                {
                    if (!set.isDisabled()) bus.lastLayout = set;
    ...
    {
        if (auto* bus = (isInput ? inputBuses : outputBuses)[busIndex]) return bus->getCurrentLayout();
    ...
    bool AudioProcessor::setChannelLayoutOfBus(bool isInputBus, int busIndex, const AudioChannelSet& layout)
    {
        if (auto* bus = getBus(isInputBus, busIndex))
        {
            ...
            if (layouts.getChannelSet(isInputBus, busIndex) == layout) return applyBusLayouts(layouts);
    ...
    {
        if (layouts.inputBuses.size() == inputBuses.size() && layouts.outputBuses.size() == outputBuses.size())
    ...
    void AudioProcessor::getNextBestLayout(const BusesLayout& desiredLayout, BusesLayout& actualLayouts) const
    {
        ...
        for (int dir = 0; dir < 2; ++dir)
        {
            ...
            for (int busIndex = 0; busIndex < requestedLayouts.size(); ++busIndex)
            {
                ...
                {
                    auto& oppositeLayout =
                        (oppositeDirection ? currentState.inputBuses : currentState.outputBuses).getReference(busIndex);
                ...
                if (std::abs(defaultLayout.size() - requested.size()) < distance)
                {
                    ...
                    if (checkBusesLayoutSupported(currentState)) bestSupported = currentState;
                }
            }
        }
    ...
    int AudioProcessor::getChannelIndexInProcessBlockBuffer(bool isInput, int busIndex, int channelIndex) const noexcept
    {
        ...
        for (int i = 0; i < ioBus.size() && i < busIndex; ++i) channelIndex += getChannelCountOfBus(isInput, i);

        return channelIndex;
    }

    int AudioProcessor::getOffsetInBusBufferForAbsoluteChannelIndex(bool isInput, int absoluteChannelIndex,
                                                                    int& busIndex) const noexcept
    {
        ...
        for (busIndex = 0;
             busIndex < numBuses && absoluteChannelIndex >= (numChannels = getChannelLayoutOfBus(isInput, busIndex).size());
             ++busIndex)
    ...
    void AudioProcessor::sendParamChangeMessageToListeners(int parameterIndex, float newValue)
    {
        if (isPositiveAndBelow(parameterIndex, getNumParameters()))
        {
            for (int i = listeners.size(); --i >= 0;)
                if (auto* l = getListenerLocked(i)) l->audioProcessorParameterChanged(this, parameterIndex, newValue);
        }
        else
        {
    ...
    void AudioProcessor::beginParameterChangeGesture(int parameterIndex)
    {
        if (isPositiveAndBelow(parameterIndex, getNumParameters()))
        {
            ...
            for (int i = listeners.size(); --i >= 0;)
                if (auto* l = getListenerLocked(i)) l->audioProcessorParameterChangeGestureBegin(this, parameterIndex);
        }
        else
        {
            jassertfalse;  // called with an out-of-range parameter index!
    ...
    void AudioProcessor::endParameterChangeGesture(int parameterIndex)
    {
        if (isPositiveAndBelow(parameterIndex, getNumParameters()))
        {
            ...
            for (int i = listeners.size(); --i >= 0;)
                if (auto* l = getListenerLocked(i)) l->audioProcessorParameterChangeGestureEnd(this, parameterIndex);
        }
        else
        {
            jassertfalse;  // called with an out-of-range parameter index!
    ...
    {
        for (int i = listeners.size(); --i >= 0;)
            if (auto* l = getListenerLocked(i)) l->audioProcessorChanged(this);
    }
    ...
    {
        if (auto* p = getParamChecked(index)) return p->getValue();
    ...
    {
        if (auto* p = getParamChecked(index)) p->setValue(newValue);
    }

    float AudioProcessor::getParameterDefaultValue(int index)
    {
        if (auto* p = managedParameters[index]) return p->getDefaultValue();
    ...
    {
        if (auto* p = getParamChecked(index)) return p->getName(512);
    ...
    String AudioProcessor::getParameterID(int index)
    {
        ...
        if (auto* p = dynamic_cast<AudioProcessorParameterWithID*>(managedParameters[index])) return p->paramID;
    ...
    {
        if (auto* p = managedParameters[index]) return p->getName(maximumStringLength);
    ...
    {
        if (auto* p = managedParameters[index]) return p->getText(p->getValue(), maximumStringLength);
    ...
    {
        if (auto* p = managedParameters[index]) return p->getNumSteps();
    ...
    {
        if (auto* p = managedParameters[index]) return p->isDiscrete();
    ...
    {
        if (auto* p = managedParameters[index]) return p->getLabel();
    ...
    {
        if (auto* p = managedParameters[index]) return p->isAutomatable();
    ...
    {
        if (auto* p = managedParameters[index]) return p->isOrientationInverted();
    ...
    {
        if (auto* p = managedParameters[index]) return p->isMetaParameter();
    ...
    {
        if (auto* p = managedParameters[index]) return p->getCategory();
    ...
    void AudioProcessor::checkForDupedParamIDs()
    {
        if (shouldCheckParamsForDupeIDs)
        {
            ...
            for (auto p : managedParameters)
                if (auto* withID = dynamic_cast<AudioProcessorParameterWithID*>(p)) ids.add(withID->paramID);
    ...
    void AudioProcessor::processBlockBypassed(AudioBuffer<float>& buffer, MidiBuffer& midi)
    {
        processBypassed(buffer, midi);
    }
    void AudioProcessor::processBlockBypassed(AudioBuffer<double>& buffer, MidiBuffer& midi)
    {
        processBypassed(buffer, midi);
    }
    ...
    {
        return buses.size() > 0 ? AudioChannelSet::getChannelTypeName(buses[0]->getCurrentLayout().getTypeOfChannel(index))
                                : String();
    }

    const String AudioProcessor::getInputChannelName(int index) const
    {
        return getChannelName(inputBuses, index);
    }
    const String AudioProcessor::getOutputChannelName(int index) const
    {
    ...
    {
        return index < 2 && buses.size() > 0 && buses[0]->getCurrentLayout() == AudioChannelSet::stereo();
    }
    ...
    {
        (inputBus ? inputBuses : outputBuses)
            .add(new Bus(*this, ioConfig.busName, ioConfig.defaultLayout, ioConfig.isActivatedByDefault));
                                                                        ...
                                                                        const Array<InOutChannelPair>& legacyLayouts) const
    {
        ...
        {
            if (inChannels == 0)
                *inBus = AudioChannelSet::disabled();
            else if (inChannels == currentInLayout.size())
                *inBus = currentInLayout;
            else if (inChannels == currentOutLayout.size())
                *inBus = currentOutLayout;
            else
                *inBus = AudioChannelSet::canonicalChannelSet(inChannels);
        }

        if (outBus != nullptr)
        {
            if (outChannels == 0)
                *outBus = AudioChannelSet::disabled();
            else if (outChannels == currentOutLayout.size())
                *outBus = currentOutLayout;
            else if (outChannels == currentInLayout.size())
                *outBus = currentInLayout;
            else
                *outBus = AudioChannelSet::canonicalChannelSet(outChannels);
        }
    ...
    {
        if (layouts.inputBuses.size() > 1 || layouts.outputBuses.size() > 1) return false;
    ...
    {
        if (layouts == getBusesLayout()) return true;
        ...
        if (layouts.inputBuses.size() != numInputBuses || layouts.outputBuses.size() != numOutputBuses) return false;
    ...
    void AudioProcessor::audioIOChanged(bool busNumberChanged, bool channelNumChanged)
    {
        ...
        for (int dir = 0; dir < 2; ++dir)
        {
            ...
            for (int i = 0; i < num; ++i)
                if (auto* bus = getBus(isInput, i)) bus->updateChannelCount();
        }
        ...
        auto countTotalChannels = [](const OwnedArray<AudioProcessor::Bus>& buses) noexcept
        {
            ...
            for (auto* bus : buses) n += bus->getNumberOfChannels();
        ...
        if (busNumberChanged) numBusesChanged();

        if (channelNumChanged) numChannelsChanged();
    ...
    {
        if (activeEditor != nullptr) return activeEditor;
    ...
    bool AudioProcessor::canApplyBusCountChange(bool isInput, bool isAdding, AudioProcessor::BusProperties& outProperties)
    {
    ...
    //==============================================================================
    AudioProcessor::Bus::Bus(AudioProcessor& processor, const String& busName, const AudioChannelSet& defaultLayout,
                             bool isDfltEnabled)
        : owner(processor)
        , name(busName)
        , layout(isDfltEnabled ? defaultLayout : AudioChannelSet())
        , dfltLayout(defaultLayout)
    ...
    bool AudioProcessor::Bus::isInput() const noexcept
    {
    ...
    AudioProcessor::Bus::BusDirectionAndIndex AudioProcessor::Bus::getDirectionAndIndex() const noexcept
    {
        ...
        if (!di.isInput) di.index = owner.outputBuses.indexOf(this);
    ...
    bool AudioProcessor::Bus::setCurrentLayoutWithoutEnabling(const AudioChannelSet& set)
    {
        if (!set.isDisabled())
        {
            if (isEnabled()) return setCurrentLayout(set);
    ...
    bool AudioProcessor::Bus::setNumberOfChannels(int channels)
    {
        ...
        if (owner.setChannelLayoutOfBus(di.isInput, di.index, AudioChannelSet::canonicalChannelSet(channels))) return true;

        if (channels == 0) return false;

        auto namedSet = AudioChannelSet::namedChannelSet(channels);

        if (!namedSet.isDisabled() && owner.setChannelLayoutOfBus(di.isInput, di.index, namedSet)) return true;
    ...
    int AudioProcessor::Bus::getMaxSupportedChannels(int limit) const
    {
        for (int ch = limit; ch > 0; --ch)
            if (isNumberOfChannelsSupported(ch)) return ch;
    ...
    bool AudioProcessor::Bus::isLayoutSupported(const AudioChannelSet& set, BusesLayout* ioLayout) const
    {
        ...
        if (actualBuses.getReference(di.index) == set) return true;

        auto desiredLayout = currentLayout;

        (di.isInput ? desiredLayout.inputBuses : desiredLayout.outputBuses).getReference(di.index) = set;

        owner.getNextBestLayout(desiredLayout, currentLayout);

        if (ioLayout != nullptr) *ioLayout = currentLayout;
    ...
    AudioChannelSet AudioProcessor::Bus::supportedLayoutWithChannels(int channels) const
    {
...

        {
            ...
            if (!(set = AudioChannelSet::namedChannelSet(channels)).isDisabled() && isLayoutSupported(set)) return set;

            if (!(set = AudioChannelSet::discreteChannels(channels)).isDisabled() && isLayoutSupported(set)) return set;
        }

        for (auto& set : AudioChannelSet::channelSetsWithNumberOfChannels(channels))
            if (isLayoutSupported(set)) return set;
    ...
    //==============================================================================
    void AudioProcessor::BusesProperties::addBus(bool isInput, const String& name, const AudioChannelSet& dfltLayout,
                                                         ...
                                                         const bool             idForAudioSuite) const
    {
        ...
        for (int dir = 0; dir < 2; ++dir)
        {
            ...
            if (set == AudioChannelSet::disabled())
                aaxFormatIndex = 0;
            else if (set == AudioChannelSet::mono())
                aaxFormatIndex = 1;
            else if (set == AudioChannelSet::stereo())
                aaxFormatIndex = 2;
            else if (set == AudioChannelSet::createLCR())
                aaxFormatIndex = 3;
            else if (set == AudioChannelSet::createLCRS())
                aaxFormatIndex = 4;
            else if (set == AudioChannelSet::quadraphonic())
                aaxFormatIndex = 5;
            else if (set == AudioChannelSet::create5point0())
                aaxFormatIndex = 6;
            else if (set == AudioChannelSet::create5point1())
                aaxFormatIndex = 7;
            else if (set == AudioChannelSet::create6point0())
                aaxFormatIndex = 8;
            else if (set == AudioChannelSet::create6point1())
                aaxFormatIndex = 9;
            else if (set == AudioChannelSet::create7point0())
                aaxFormatIndex = 10;
            else if (set == AudioChannelSet::create7point1())
                aaxFormatIndex = 11;
            else if (set == AudioChannelSet::create7point0SDDS())
                aaxFormatIndex = 12;
            else if (set == AudioChannelSet::create7point1SDDS())
                aaxFormatIndex = 13;
            else if (set == AudioChannelSet::create7point0point2())
                aaxFormatIndex = 14;
            else if (set == AudioChannelSet::create7point1point2())
                aaxFormatIndex = 15;
            else if (set == AudioChannelSet::ambisonic(1))
                aaxFormatIndex = 16;
            else if (set == AudioChannelSet::ambisonic(2))
                aaxFormatIndex = 17;
            else if (set == AudioChannelSet::ambisonic(3))
                aaxFormatIndex = 18;
            else
            {
    ...
    AudioProcessorParameter::Category AudioProcessorParameter::getCategory() const
    {
    ...
    bool AudioProcessorParameter::isDiscrete() const
    {
        return false;
    }
    ...
    {
        return timeInSamples == other.timeInSamples && ppqPosition == other.ppqPosition
               && editOriginTime == other.editOriginTime && ppqPositionOfLastBarStart == other.ppqPositionOfLastBarStart
               && frameRate == other.frameRate && isPlaying == other.isPlaying && isRecording == other.isRecording
               && bpm == other.bpm && timeSigNumerator == other.timeSigNumerator
               && timeSigDenominator == other.timeSigDenominator && ppqLoopStart == other.ppqLoopStart
               && ppqLoopEnd == other.ppqLoopEnd && isLooping == other.isLooping;
    }
