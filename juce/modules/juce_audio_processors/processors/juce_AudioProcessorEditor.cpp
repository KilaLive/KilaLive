...
namespace juce
{
    ...
    bool AudioProcessorEditor::supportsHostMIDIControllerPresence(bool)
    {
        return true;
    }
    ...
    void AudioProcessorEditor::setResizeLimits(int newMinimumWidth, int newMinimumHeight, int newMaximumWidth,
    ...
    void AudioProcessorEditor::editorResized(bool wasResized)
    {
        if (wasResized)
        {
            ...
            if (auto* peer = getPeer()) resizerHidden = peer->isFullScreen() || peer->isKioskMode();
            ...
            if (!resizable)
                if (auto w = getWidth())
                    if (auto h = getHeight()) defaultConstrainer.setSizeLimits(w, h, w, h);
        }
    }
