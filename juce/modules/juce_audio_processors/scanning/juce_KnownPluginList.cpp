...
namespace juce
{
    ...
    PluginDescription* KnownPluginList::getTypeForFile(const String& fileOrIdentifier) const
    {
        ...
        for (auto* desc : types)
            if (desc->fileOrIdentifier == fileOrIdentifier) return desc;
    ...
    PluginDescription* KnownPluginList::getTypeForIdentifierString(const String& identifierString) const
    {
        ...
        for (auto* desc : types)
            if (desc->matchesIdentifierString(identifierString)) return desc;
    ...
    bool KnownPluginList::isListingUpToDate(const String& fileOrIdentifier, AudioPluginFormat& formatToUse) const
    {
        if (getTypeForFile(fileOrIdentifier) == nullptr) return false;

        ScopedLock lock(typesArrayLock);

        for (auto* d : types)
            if (d->fileOrIdentifier == fileOrIdentifier && formatToUse.pluginNeedsRescanning(*d)) return false;
    ...
    bool KnownPluginList::scanAndAddFile(const String& fileOrIdentifier, const bool dontRescanIfAlreadyInList,
                                         OwnedArray<PluginDescription>& typesFound, AudioPluginFormat& format)
    {
        const ScopedLock sl(scanLock);

        if (dontRescanIfAlreadyInList && getTypeForFile(fileOrIdentifier) != nullptr)
        {
            ...
            if (!needsRescanning) return false;
        }

        if (blacklist.contains(fileOrIdentifier)) return false;
...

        {
            ...
            if (scanner != nullptr)
            {
                if (!scanner->findPluginTypesFor(format, found, fileOrIdentifier)) addToBlacklist(fileOrIdentifier);
            }
            else
            {
    ...
    void KnownPluginList::scanAndAddDragAndDroppedFiles(AudioPluginFormatManager& formatManager, const StringArray& files,
    ...
    {
        if (scanner != nullptr) scanner->scanFinished();
    }
    ...
    struct PluginSorter
    {
        ...
        bool operator()(const PluginDescription* first, const PluginDescription* second) const
        {
            ...
            case KnownPluginList::sortByCategory:
                diff = first->category.compareNatural(second->category, false);
                break;
            case KnownPluginList::sortByManufacturer:
                diff = first->manufacturerName.compareNatural(second->manufacturerName, false);
                break;
            ...
            case KnownPluginList::sortByFileSystemLocation:
                ...
                break;
            case KnownPluginList::sortByInfoUpdateTime:
                diff = compare(first->lastInfoUpdateTime, second->lastInfoUpdateTime);
                break;
            default:
                break;
            }

            if (diff == 0) diff = first->name.compareNatural(second->name, false);
    ...
    void KnownPluginList::sort(const SortMethod method, bool forwards)
    {
        if (method != defaultOrder)
        {
            ...
            if (oldOrder != newOrder) sendChangeMessage();
        }
    }
    ...
    XmlElement* KnownPluginList::createXml() const
    {
...

        {
            ...
            for (int i = types.size(); --i >= 0;) e->prependChildElement(types.getUnchecked(i)->createXml());
        }

        for (auto& b : blacklist) e->createNewChildElement("BLACKLISTED")->setAttribute("id", b);
    ...
    struct PluginTreeUtils
    {
        ...
        static void buildTreeByFolder(KnownPluginList::PluginTree& tree, const Array<PluginDescription*>& allPlugins)
        {
            ...
            {
                auto path = pd->fileOrIdentifier.replaceCharacter('\\', '/').upToLastOccurrenceOf("/", false, false);

                if (path.substring(1, 2) == ":") path = path.substring(2);
        ...
        static void optimiseFolders(KnownPluginList::PluginTree& tree, bool concatenateName)
        {
            for (int i = tree.subFolders.size(); --i >= 0;)
            {
                ...
                if (sub.plugins.isEmpty())
                {
                    ...
                    {
                        if (concatenateName) s->folder = sub.folder + "/" + s->folder;
        ...
        static void buildTreeByCategory(KnownPluginList::PluginTree& tree, const Array<PluginDescription*>& sorted,
                                        const KnownPluginList::SortMethod sortMethod)
        {
            ...
            for (auto* pd : sorted)
            {
                auto thisType = (sortMethod == KnownPluginList::sortByCategory ? pd->category : pd->manufacturerName);

                if (!thisType.containsNonWhitespaceChars()) thisType = "Other";
        ...
        static void addPlugin(KnownPluginList::PluginTree& tree, PluginDescription* const pd, String path)
        {
            ...
            else
            {
                ...
                if (path.containsChar(':'))
                    path =
                        path.fromFirstOccurrenceOf(":", false, false);  // avoid the special AU formatting nonsense on Mac..
#endif
        ...
        static bool addToMenu(const KnownPluginList::PluginTree& tree, PopupMenu& m,
                              const OwnedArray<PluginDescription>& allPlugins, const String& currentlyTickedPluginID)
        {
            bool isTicked = false;
            ...
            for (auto* plugin : tree.plugins)
            {
                ...
                if (containsDuplicateNames(tree.plugins, name)) name << " (" << plugin->pluginFormatName << ')';
    ...
    KnownPluginList::PluginTree* KnownPluginList::createTree(const SortMethod sortMethod) const
    {
        ...
        {
            for (auto* p : sorted) tree->plugins.add(p);
        }
    ...
    //==============================================================================
    void KnownPluginList::addToMenu(PopupMenu& menu, const SortMethod sortMethod, const String& currentlyTickedPluginID) const
    {
    ...
    {
        if (auto* job = ThreadPoolJob::getCurrentThreadPoolJob()) return job->shouldExit();
