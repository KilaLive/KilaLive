...
namespace juce
{
    ...
    public:
        ...
        void paintRowBackground(Graphics& g, int /*rowNumber*/, int /*width*/, int /*height*/, bool rowIsSelected) override
        {
            ...
            const auto c =
                rowIsSelected ? defaultColour.interpolatedWith(owner.findColour(ListBox::textColourId), 0.5f) : defaultColour;
    ...
    void PluginListComponent::removeSelectedPlugins()
    {
        ...
        for (int i = table.getNumRows(); --i >= 0;)
            if (selected.contains(i)) removePluginItem(i);
    }
    ...
    void PluginListComponent::removeMissingPlugins()
    {
        for (int i = list.getNumTypes(); --i >= 0;)
            if (!formatManager.doesPluginStillExist(*list.getType(i))) list.removeType(i);
    }
    ...
    {
        if (pluginList != nullptr) pluginList->optionsMenuCallback(result);
    }
    ...
    void PluginListComponent::optionsMenuCallback(int result)
    {
        ...
        default:
            if (AudioPluginFormat* format = formatManager.getFormat(result - 10))
                if (defaultPathToScan.isEmpty())
                {
                    scanFor(*format);
                }
                else
                {
                    scanFor(*format, StringArray(), defaultPathToScan);
                }
    ...
    void PluginListComponent::setTheDefaultPath(String& defaultPath)
    {
        defaultPathToScan.clear();
        defaultPathToScan = defaultPath;
    }
    ...
    public:
        Scanner(PluginListComponent& plc, AudioPluginFormat& format, const StringArray& filesOrIdentifiers,
                ...
                const String& title, const String& text, String defaultPath = String())
            : owner(plc)
            , formatToScan(format)
            ...
            , defaultSearchPath(defaultPath)
        {
            ...
            if (!defaultSearchPath.isEmpty())
            {
                ...
                // String currentWorkingDirectory = String("###currentWorkingDirectory: ")+f.getFullPathName();
                // Logger::outputDebugString(currentWorkingDirectory);
                // JUCELOG_INFO(currentWorkingDirectory);

                // defaultPath = f.getFullPathName() + String("\\") + defaultPath;

                // String defaultVST_Path = String("###defaultVST_Path: ") + defaultPath;
                // Logger::outputDebugString(defaultVST_Path);
                // JUCELOG_INFO(defaultVST_Path);

                FileSearchPath path1(defaultPath);
                pathList.setSize(500, 300);
                pathList.setPath(path1);
                startScan();
            }
            ...
            else if (filesOrIdentifiersToScan.isEmpty() && path.getNumPaths() > 0)
            {
#if !JUCE_IOS
                if (propertiesToUse != nullptr) path = getLastSearchPath(*propertiesToUse, formatToScan);
#endif
                ...
                pathChooserWindow.enterModalState(
                    true, ModalCallbackFunction::forComponent(startScanCallback, &pathChooserWindow, this), false);
            }
            else
            {
                startScan();
    ...
    private:
            ...
            for (int i = 0; i < pathList.getPath().getNumPaths(); ++i)
            {
                ...
                if (isStupidPath(f))
                {
                    AlertWindow::showOkCancelBox(
                        AlertWindow::WarningIcon, TRANS("Plugin Scanning"),
                        TRANS("If you choose to scan folders that contain non-plugin files, "
                            ...
                            + newLine
                            + TRANS("Are you sure you want to scan the folder \"XYZ\"?").replace("XYZ", f.getFullPathName()),
                        TRANS("Scan"), String(), nullptr, ModalCallbackFunction::create(warnAboutStupidPathsCallback, this));
                    return;
                }
            }
        ...
        static bool isStupidPath(const File& f)
        {
            ...
            if (roots.contains(f)) return true;

            File::SpecialLocationType pathsThatWouldBeStupidToScan[] = {
                File::globalApplicationsDirectory, File::userHomeDirectory,    File::userDocumentsDirectory,
                File::userDesktopDirectory,        File::tempDirectory,        File::userMusicDirectory,
                File::userMoviesDirectory,         File::userPicturesDirectory};
            ...
            for (int i = 0; i < numElementsInArray(pathsThatWouldBeStupidToScan); ++i)
            {
                ...
                if (f == sillyFolder || sillyFolder.isAChildOf(f)) return true;
            }
        ...
        void startScan()
        {
            ...
            scanner.reset(new PluginDirectoryScanner(owner.list, formatToScan, pathList.getPath(), true,
                                                     owner.deadMansPedalFile, allowAsync));
        ...
        void timerCallback() override
        {
            if (pool == nullptr)
            {
                if (doNextScan()) startTimer(20);
            }

            if (!progressWindow.isCurrentlyModal()) finished = true;
    ...
    void PluginListComponent::scanFor(AudioPluginFormat& format, const StringArray& filesOrIdentifiersToScan)
    {
        currentScanner.reset(
            new Scanner(*this, format, filesOrIdentifiersToScan, propertiesToUse, allowAsync, numThreads,
                        dialogTitle.isNotEmpty() ? dialogTitle : TRANS("Scanning for plug-ins..."),
                        dialogText.isNotEmpty() ? dialogText : TRANS("Searching for all possible plug-in files...")));
    }

    void PluginListComponent::scanFor(AudioPluginFormat& format, const StringArray& filesOrIdentifiersToScan,
                                      const String& defaultSearchPath)
    {
        currentScanner.reset(new Scanner(
            *this, format, filesOrIdentifiersToScan, propertiesToUse, allowAsync, numThreads,
            dialogTitle.isNotEmpty() ? dialogTitle : TRANS("Scanning for plug-ins..."),
            dialogText.isNotEmpty() ? dialogText : TRANS("Searching for all possible plug-in files..."), defaultSearchPath));
    }
    ...
    void PluginListComponent::scanFinished(const StringArray& failedFiles)
    {
        ...
        for (auto& f : failedFiles) shortNames.add(File::createFileWithoutCheckingPath(f).getFileName());

        currentScanner.reset();  // mustn't delete this before using the failed files array

        if (shortNames.size() > 0)
            AlertWindow::showMessageBoxAsync(
                AlertWindow::InfoIcon, TRANS("Scan complete"),
