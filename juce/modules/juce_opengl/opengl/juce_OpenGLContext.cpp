...
namespace juce
{
    ...
    {
        iOSBackgroundProcessCheck()
        {
            isBackgroundProcess();
            appBecomingInactiveCallbacks.add(this);
        }
        ~iOSBackgroundProcessCheck()
        {
        ...
        void appBecomingInactive() override
        {
            ...
            while (--counter > 0 && isForeground.get() != 0) Thread::sleep(1);
        }
    ...
    //==============================================================================
    class OpenGLContext::CachedImage : public CachedComponentImage, private ThreadPoolJob
    {
    public:
        CachedImage(OpenGLContext& c, Component& comp, const OpenGLPixelFormat& pixFormat, void* contextToShare)
            : ThreadPoolJob("OpenGL Rendering")
            , context(c)
            , component(comp)
        {
            nativeContext.reset(
                new NativeContext(component, pixFormat, contextToShare, c.useMultisampling, c.versionRequired));
        ...
        void stop()
        {
            if (renderThread != nullptr)
            {
                ...
                if (workQueue.size() > 0)
                {
                    if (!renderThread->contains(this)) resume();

                    while (workQueue.size() != 0) Thread::sleep(20);
                }
        ...
        void resume()
        {
            if (renderThread != nullptr) renderThread->addJob(this, false);
        }
        ...
        bool renderFrame()
        {
            ...
            if (context.renderComponents && isUpdating)
            {
                ...
                if (lastMMLockReleaseTime + 1 >= Time::getMillisecondCounter()) Thread::sleep(2);

                while (!shouldExit())
                {
                    doWorkWhileWaitingForLock(false);

                    if (mmLock.retryLock()) break;
                }

                if (shouldExit()) return false;
            }

            if (!context.makeActive()) return false;
        ...
        void updateViewportSize(bool canTriggerUpdate)
        {
            if (auto* peer = component.getPeer())
            {
                ...
                auto newScale = Desktop::getInstance().getDisplays().getDisplayContaining(lastScreenBounds.getCentre()).scale;
                ...
                if (scale != newScale || viewportArea != newArea)
                {
                    ...
                    if (canTriggerUpdate) invalidateAll();
                }
            }
        }
        ...
        {
#if JUCE_OPENGL3
            if (vertexArrayObject != 0) context.extensions.glBindVertexArray(vertexArrayObject);
#endif
        }
        ...
        void checkViewportBounds()
        {
            ...
            if (lastScreenBounds != screenBounds) updateViewportSize(true);
        }
        ...
        void paintComponent()
        {
            ...
            if (!ensureFrameBufferSize()) return;
            ...
            if (!invalid.isEmpty())
            {
                ...
                if (!context.isActive()) context.makeActive();
            }
        ...
        void paintOwner(LowLevelGraphicsContext& llgc)
        {
...
#endif
            {
                ...
                static Random rng;
                g.fillAll(Colour((uint8)rng.nextInt(255), (uint8)rng.nextInt(255), (uint8)rng.nextInt(255), (uint8)0x50));
            }
#endif
        }
        ...
        {
            {
                ...
                do
                {
                    if (shouldExit()) return ThreadPoolJob::jobHasFinished;
        ...
        bool initialiseOnThread()
        {
            ...
            if (!nativeContext->initialiseOnRenderThread(context)) return false;
            ...
            if (context.renderer != nullptr) context.renderer->newOpenGLContextCreated();
        ...
        {
            if (context.renderer != nullptr) context.renderer->openGLContextClosing();

#if JUCE_OPENGL3
            if (vertexArrayObject != 0) context.extensions.glDeleteVertexArrays(1, &vertexArrayObject);
#endif
        ...
        struct BlockingWorker : public OpenGLContext::AsyncWorker
        {
            ...
            {
                if (originalWorker != nullptr) (*originalWorker)(calleeContext);
        ...
        bool doWorkWhileWaitingForLock(bool contextIsAlreadyActive)
        {
            ...
            for (OpenGLContext::AsyncWorker::Ptr work = workQueue.removeAndReturn(0); work != nullptr && (!shouldExit());
                 work                                 = workQueue.removeAndReturn(0))
            {
                if ((!contextActivated) && (!contextIsAlreadyActive))
                {
                    if (!context.makeActive()) break;
            ...
            if (contextActivated) OpenGLContext::deactivateCurrentContext();
        ...
        void execute(OpenGLContext::AsyncWorker::Ptr workerToUse, bool shouldBlock, bool calledFromDestructor = false)
        {
            ...
            {
                BlockingWorker* blocker =
                    (shouldBlock ? new BlockingWorker(static_cast<OpenGLContext::AsyncWorker::Ptr&&>(workerToUse)) : nullptr);
                OpenGLContext::AsyncWorker::Ptr worker =
                    (blocker != nullptr ? blocker : static_cast<OpenGLContext::AsyncWorker::Ptr&&>(workerToUse));
                workQueue.add(worker);

                messageManagerLock.abort();
                context.triggerRepaint();

                if (blocker != nullptr) blocker->block();
            }
            else
            {
    ...
    //==============================================================================
    class OpenGLContext::Attachment : public ComponentMovementWatcher, private Timer
    {
    public:
        ...
        void componentMovedOrResized(bool /*wasMoved*/, bool /*wasResized*/) override
        {
            ...
            if (isAttached(comp) != canBeAttached(comp)) componentVisibilityChanged();

            if (comp.getWidth() > 0 && comp.getHeight() > 0 && context.nativeContext != nullptr)
            {
                if (auto* c = CachedImage::get(comp)) c->handleResize();
    ...
    private:
        ...
        static bool isShowingOrMinimised(const Component& c)
        {
            ...
            if (auto* p = c.getParentComponent()) return isShowingOrMinimised(*p);
        ...
        {
            auto& comp           = *getComponent();
            auto* newCachedImage = new CachedImage(context, comp, context.openGLPixelFormat, context.contextToShareWith);
        ...
        {
            if (auto* cachedImage = CachedImage::get(*getComponent())) cachedImage->checkViewportBounds();
        }
    };
    ...
    {
        if (auto* ci = CachedImage::get(c)) return &(ci->context);
    ...
    {
        if (auto* cachedImage = getCachedImage()) cachedImage->triggerRepaint();
    }

    void OpenGLContext::swapBuffers()
    {
        if (nativeContext != nullptr) nativeContext->swapBuffers();
    }
    ...
    {
        if (auto* comp = getTargetComponent()) return CachedImage::get(*comp);
    ...
    void OpenGLContext::setImageCacheSize(size_t newSize) noexcept
    {
        imageCacheMaxSize = newSize;
    }
    size_t OpenGLContext::getImageCacheSize() const noexcept
    {
    ...
    void OpenGLContext::overrideCanBeAttached(bool newCanAttach)
    {
        if (overrideCanAttach != newCanAttach)
        {
            ...
            if (auto* a = attachment.get()) a->update();
        }
    }
    ...
    struct DepthTestDisabler
    {
        DepthTestDisabler() noexcept
        {
            ...
            if (wasEnabled) glDisable(GL_DEPTH_TEST);
        }
    ...
    //==============================================================================
    void OpenGLContext::copyTexture(const Rectangle<int>& targetClipArea, const Rectangle<int>& anchorPosAndTextureSize,
                                    const int contextWidth, const int contextHeight, bool flippedVertically)
    {
        if (contextWidth <= 0 || contextHeight <= 0) return;
        ...
        if (areShadersAvailable())
        {
            struct OverlayShaderProgram : public ReferenceCountedObject
            {
                ...
                static const OverlayShaderProgram& select(OpenGLContext& context)
                {
                    ...
                    OverlayShaderProgram* program =
                        static_cast<OverlayShaderProgram*>(context.getAssociatedObject(programValueID));
                ...
                struct ProgramBuilder
                {
                    ProgramBuilder(OpenGLShaderProgram& prog)
                    {
                        prog.addVertexShader(OpenGLHelpers::translateVertexShaderToV3(
                            ...
                            "texturePos = (position - vec2 (textureBounds[0], textureBounds[1])) / vec2 (textureBounds[2], "
                            "textureBounds[3]);"
                            "texturePos = vec2 (texturePos.x, vOffsetAndScale.x + vOffsetAndScale.y * texturePos.y);"
                            "}"));

                        prog.addFragmentShader(
                            OpenGLHelpers::translateFragmentShaderToV3("uniform sampler2D imageTexture;"
                                                                       "varying " JUCE_HIGHP " vec2 texturePos;"
                                                                       "void main()"
                                                                       "{"
                ...
                struct Params
                {
                    ...
                    void set(const float targetWidth, const float targetHeight, const Rectangle<float>& bounds,
                             bool flipVertically) const
                    {
                        ...
                        vOffsetAndScale.set(flipVertically ? 0.0f : 1.0f, flipVertically ? 1.0f : -1.0f);
                    }
    ...
    void OpenGLContext::NativeContext::surfaceDestroyed(jobject holder)
    {
        ...
        if (auto* cachedImage = CachedImage::get(component))
        {
            ...
            if (auto* threadPool = cachedImage->renderThread.get()) threadPool->waitForJobToFinish(cachedImage, -1);
        }
    }
#endif
