...
namespace juce
{
    ...
    namespace OpenGLRendering
    {
        ...
        struct CachedImageList : public ReferenceCountedObject, private ImagePixelData::Listener
        {
            CachedImageList(OpenGLContext& c) noexcept
                : context(c)
                , totalSize(0)
                , maxCacheSize(c.getImageCacheSize())
            {
            }
            ...
            TextureInfo getTextureFor(const Image& image)
            {
                ...
                if (c == nullptr)
                {
                    ...
                    while (totalSize > maxCacheSize && images.size() > 1 && totalSize > 0) removeOldestItem();
                }
            ...
            struct CachedImage
            {
                ...
                ~CachedImage()
                {
                    if (pixelData != nullptr) pixelData->listeners.remove(&owner);
                }
        ...
        private:
            ...
            {
                if (CachedImage* c = findCachedImage(im)) c->textureNeedsReloading = true;
            }
            ...
            CachedImage* findCachedImage(ImagePixelData* const pixelData) const
            {
                for (int i = 0; i < images.size(); ++i)
                {
                    ...
                    if (c->pixelData == pixelData) return c;
                }
            ...
            void removeOldestItem()
            {
                ...
                for (int i = 0; i < images.size(); ++i)
                {
                    ...
                    if (oldest == nullptr || c->lastUsed < oldest->lastUsed) oldest = c;
                }
        ...
        struct Target
        {
            ...
            Target(OpenGLContext& c, OpenGLFrameBuffer& fb, Point<int> origin) noexcept
                : context(c)
                , frameBufferID(fb.getFrameBufferID())
        ...
        struct ShaderPrograms : public ReferenceCountedObject
        {
            ...
            struct ShaderBase : public ShaderProgramHolder
            {
                ...
                {
                    context.extensions.glVertexAttribPointer((GLuint)positionAttribute.attributeID, 2, GL_SHORT, GL_FALSE, 8,
                                                             (void*)0);
                    context.extensions.glVertexAttribPointer((GLuint)colourAttribute.attributeID, 4, GL_UNSIGNED_BYTE,
                                                             GL_TRUE, 8, (void*)4);
            ...
            struct MaskedShaderParams
            {
                ...
                void setBounds(Rectangle<int> area, const Target& target, GLint textureIndex) const
                {
                    ...
                    maskBounds.set(area.getX() - target.bounds.getX(), area.getY() - target.bounds.getY(), area.getWidth(),
                                   area.getHeight());
                }
...
#define JUCE_DECLARE_VARYING_PIXELPOS "varying " JUCE_HIGHP " vec2 pixelPos;"
            ...
            struct SolidColourProgram : public ShaderBase
            {
                SolidColourProgram(OpenGLContext& context)
                    : ShaderBase(context, JUCE_DECLARE_VARYING_COLOUR "void main() { gl_FragColor = frontColour; }")
                {
                }
            };

#define JUCE_DECLARE_MASK_UNIFORMS                                                                                           \
    "uniform sampler2D maskTexture;"                                                                                         \
    "uniform ivec4 maskBounds;"
#define JUCE_FRAGCOORD_TO_MASK_POS                                                                                           \
...
#define JUCE_GET_MASK_ALPHA "texture2D (maskTexture, " JUCE_FRAGCOORD_TO_MASK_POS ").a"
            ...
            struct SolidColourMaskedProgram : public ShaderBase
            {
                SolidColourMaskedProgram(OpenGLContext& context)
                    : ShaderBase(context, JUCE_DECLARE_MASK_UNIFORMS JUCE_DECLARE_VARYING_COLOUR JUCE_DECLARE_VARYING_PIXELPOS
                                 "void main() {"
            ...
            struct RadialGradientParams
            {
                ...
                {
                    auto          t = AffineTransform::fromTargetPoints(p1, Point<float>(), p2, Point<float>(1.0f, 0.0f), p3,
...
#define JUCE_MATRIX_TIMES_FRAGCOORD                                                                                          \
...
#define JUCE_GET_TEXTURE_COLOUR "(frontColour.a * texture2D (gradientTexture, vec2 (gradientPos, 0.5)))"
            ...
            struct RadialGradientProgram : public ShaderBase
            {
                RadialGradientProgram(OpenGLContext& context)
                    : ShaderBase(context,
                                 JUCE_DECLARE_VARYING_PIXELPOS JUCE_DECLARE_RADIAL_UNIFORMS JUCE_DECLARE_VARYING_COLOUR
                                 "void main()"
            ...
            struct RadialGradientMaskedProgram : public ShaderBase
            {
                RadialGradientMaskedProgram(OpenGLContext& context)
                    : ShaderBase(context, JUCE_DECLARE_VARYING_PIXELPOS JUCE_DECLARE_RADIAL_UNIFORMS
                                              JUCE_DECLARE_VARYING_COLOUR JUCE_DECLARE_MASK_UNIFORMS
                                 "void main()"
                                 "{" JUCE_MEDIUMP " float gradientPos = length (" JUCE_MATRIX_TIMES_FRAGCOORD ");"
...
#define JUCE_DECLARE_LINEAR_UNIFORMS                                                                                         \
...
#define JUCE_CALC_LINEAR_GRAD_POS1                                                                                           \
    JUCE_MEDIUMP " float gradientPos = (pixelPos.y - (gradientInfo.y + (gradientInfo.z * (pixelPos.x - gradientInfo.x)))) "  \
                 "/ gradientInfo.w;"
#define JUCE_CALC_LINEAR_GRAD_POS2                                                                                           \
    JUCE_MEDIUMP " float gradientPos = (pixelPos.x - (gradientInfo.x + (gradientInfo.z * (pixelPos.y - gradientInfo.y)))) "  \
                 "/ gradientInfo.w;"
            ...
            struct LinearGradient1Program : public ShaderBase
            {
                LinearGradient1Program(OpenGLContext& context)
                    : ShaderBase(
                          ...
                          "void main()"
                          "{" JUCE_CALC_LINEAR_GRAD_POS1 "gl_FragColor = " JUCE_GET_TEXTURE_COLOUR ";"
                          "}")
                    , gradientParams(program)
                {
                }
            ...
            struct LinearGradient1MaskedProgram : public ShaderBase
            {
                LinearGradient1MaskedProgram(OpenGLContext& context)
                    : ShaderBase(
                          context,
                          JUCE_DECLARE_LINEAR_UNIFORMS  // gradientInfo: x = x1, y = y1, z = (y2 - y1) / (x2 - x1), w = length
                              JUCE_DECLARE_MASK_UNIFORMS "void main()"
            ...
            struct LinearGradient2Program : public ShaderBase
            {
                LinearGradient2Program(OpenGLContext& context)
                    : ShaderBase(context, JUCE_DECLARE_LINEAR_UNIFORMS  // gradientInfo: x = x1, y = y1, z = (x2 - x1) / (y2 -
                                                                        // y1), y = y1, w = length
                                 "void main()"
                                 "{" JUCE_CALC_LINEAR_GRAD_POS2 "gl_FragColor = " JUCE_GET_TEXTURE_COLOUR ";"
                                 "}")
                    , gradientParams(program)
                {
                }
            ...
            struct ImageParams
            {
                ...
                void setMatrix(const AffineTransform& trans, int imageWidth, int imageHeight, float fullWidthProportion,
                               float fullHeightProportion, float targetX, float targetY, bool isForTiling) const
                {
                    auto t = trans.translated(-targetX, -targetY)
                                 .inverted()
                                 .scaled(fullWidthProportion / imageWidth, fullHeightProportion / imageHeight);
                ...
                void setMatrix(const AffineTransform& trans, const TextureInfo& textureInfo, float targetX, float targetY,
                               bool isForTiling) const
                {
                    setMatrix(trans, textureInfo.imageWidth, textureInfo.imageHeight, textureInfo.fullWidthProportion,
                              textureInfo.fullHeightProportion, targetX, targetY, isForTiling);
                }

                OpenGLShaderProgram::Uniform imageTexture, matrix, imageLimits;
            };

#define JUCE_DECLARE_IMAGE_UNIFORMS                                                                                          \
    "uniform sampler2D imageTexture;"                                                                                        \
    "uniform " JUCE_MEDIUMP                                                                                                  \
    " vec2 imageLimits;" JUCE_DECLARE_MATRIX_UNIFORM JUCE_DECLARE_VARYING_COLOUR JUCE_DECLARE_VARYING_PIXELPOS
#define JUCE_GET_IMAGE_PIXEL "texture2D (imageTexture, vec2 (texturePos.x, 1.0 - texturePos.y))"
#define JUCE_CLAMP_TEXTURE_COORD                                                                                             \
    JUCE_HIGHP " vec2 texturePos = clamp (" JUCE_MATRIX_TIMES_FRAGCOORD ", vec2 (0, 0), imageLimits);"
#define JUCE_MOD_TEXTURE_COORD JUCE_HIGHP " vec2 texturePos = mod (" JUCE_MATRIX_TIMES_FRAGCOORD ", imageLimits);"
            ...
            struct ImageProgram : public ShaderBase
            {
                ImageProgram(OpenGLContext& context)
                    : ShaderBase(context,
                                 JUCE_DECLARE_VARYING_COLOUR "uniform sampler2D imageTexture;"
                                                             "varying " JUCE_HIGHP " vec2 texturePos;"
                                                             "void main()"
                                                             "{"
                                                             "gl_FragColor = frontColour.a * " JUCE_GET_IMAGE_PIXEL ";"
                                                             "}",
                                 "uniform " JUCE_MEDIUMP " vec2 imageLimits;" JUCE_DECLARE_MATRIX_UNIFORM
            ...
            struct ImageMaskedProgram : public ShaderBase
            {
                ImageMaskedProgram(OpenGLContext& context)
                    : ShaderBase(context, JUCE_DECLARE_IMAGE_UNIFORMS JUCE_DECLARE_MASK_UNIFORMS
                                 "void main()"
                                 "{" JUCE_CLAMP_TEXTURE_COORD "gl_FragColor = frontColour.a * " JUCE_GET_IMAGE_PIXEL
                                 " * " JUCE_GET_MASK_ALPHA ";"
                                 "}")
                    , imageParams(program)
                    , maskParams(program)
            ...
            struct TiledImageProgram : public ShaderBase
            {
                TiledImageProgram(OpenGLContext& context)
                    : ShaderBase(context, JUCE_DECLARE_IMAGE_UNIFORMS
                                 "void main()"
                                 "{" JUCE_MOD_TEXTURE_COORD "gl_FragColor = frontColour.a * " JUCE_GET_IMAGE_PIXEL ";"
                                 "}")
                    , imageParams(program)
                {
                }
            ...
            struct MaskTextureProgram : public ShaderBase
            {
                MaskTextureProgram(OpenGLContext& context)
                    : ShaderBase(context,
                                 JUCE_DECLARE_IMAGE_UNIFORMS "void main()"
        ...
        struct StateHelpers
        {
            ...
            struct TextureCache
            {
                ...
                OpenGLTexture* getTexture(ActiveTextures& activeTextures, int w, int h)
                {
                    ...
                    for (int i = 0; i < numTexturesToCache - 2; ++i)
                    {
                        ...
                        if (t->getWidth() == w && t->getHeight() == h) return textures.removeAndReturn(i);
                    }
            ...
            struct ShaderQuadQueue
            {
                ...
                void add(int x, int y, int w, int h, PixelARGB colour) noexcept
                {
...
#if JUCE_BIG_ENDIAN
                    auto rgba = (GLuint)((colour.getRed() << 24) | (colour.getGreen() << 16) | (colour.getBlue() << 8)
                                         | colour.getAlpha());
#else
                    auto rgba     = (GLuint)((colour.getAlpha() << 24) | (colour.getBlue() << 16) | (colour.getGreen() << 8)
                                         | colour.getRed());
#endif
                    ...
                    if (numVertices > maxVertices) draw();
                }
            ...
            private:
                ...
                void draw() noexcept
                {
                    context.extensions.glBufferSubData(GL_ARRAY_BUFFER, 0,
                                                       (GLsizeiptr)((size_t)numVertices * sizeof(VertexInfo)), vertexData);
        ...
        struct GLState
        {
            ...
            void setShaderForGradientFill(const ColourGradient& g, const AffineTransform& transform, int maskTextureID,
                                          const Rectangle<int>* maskArea)
            {
                ...
                auto t  = transform.translated(0.5f - target.bounds.getX(), 0.5f - target.bounds.getY());
                auto p1 = g.point1.transformedBy(t);
                auto p2 = g.point2.transformedBy(t);
                auto p3 = Point<float>(g.point1.x + (g.point2.y - g.point1.y), g.point1.y - (g.point2.x - g.point1.x))
                              .transformedBy(t);
                ...
                if (maskParams != nullptr) maskParams->setBounds(*maskArea, target, 1);
                                            ...
                                            int maskTextureID, const Rectangle<int>* maskArea, bool isTiledFill)
            {
                ...
                imageParams->setMatrix(transform, textureInfo, (float)target.bounds.getX(), (float)target.bounds.getY(),
                                       isTiledFill);
        ...
        struct SavedState : public RenderingHelpers::SavedStateBase<SavedState>
        {
            ...
            SavedState(GLState* s)
                : BaseClass(s->target.bounds)
                , state(s)
            {
            }
            ...
            SavedState* beginTransparencyLayer(float opacity)
            {
                ...
                if (clip != nullptr)
                {
                    ...
                    state->flush();
                    s->transparencyLayer =
                        Image(OpenGLImageType().create(Image::ARGB, clipBounds.getWidth(), clipBounds.getHeight(), true));
                    s->previousTarget.reset(new Target(state->target));
                    state->target = Target(state->target.context, *OpenGLImageType::getFrameBufferFrom(s->transparencyLayer),
                                           clipBounds.getPosition());
            ...
            typedef RenderingHelpers::GlyphCache<RenderingHelpers::CachedGlyphEdgeTable<SavedState>, SavedState>
                GlyphCacheType;
            ...
            void drawGlyph(int glyphNumber, const AffineTransform& trans)
            {
                if (clip != nullptr)
                {
                    ...
                    else
                    {
                        ...
                        const ScopedPointer<EdgeTable> et(
                            font.getTypeface()->getEdgeTableForGlyph(glyphNumber, t, fontHeight));
            ...
            void renderImageTransformed(IteratorType& iter, const Image& src, int alpha, const AffineTransform& trans,
                                        Graphics::ResamplingQuality, bool tiledFill) const
            {
        ...
        struct NonShaderContext : public LowLevelGraphicsSoftwareRenderer
        {
            NonShaderContext(const Target& t, const Image& im)
                : LowLevelGraphicsSoftwareRenderer(im)
                , target(t)
                , image(im)
            {
            }
            ...
            ~NonShaderContext()
            {
                ...
                target.makeActive();
                target.context.copyTexture(target.bounds, Rectangle<int>(texture.getWidth(), texture.getHeight()),
                                           target.bounds.getWidth(), target.bounds.getHeight(), false);
    ...
    LowLevelGraphicsContext* createOpenGLGraphicsContext(OpenGLContext& context, unsigned int frameBufferID, int width,
                                                         int height)
    {
    ...
    //==============================================================================
    struct CustomProgram : public ReferenceCountedObject, public OpenGLRendering::ShaderPrograms::ShaderBase
    {
        ...
        static CustomProgram* getOrCreate(LowLevelGraphicsContext& gc, const String& hashName, const String& code,
                                          String& errorMessage)
        {
            if (auto* c = get(hashName)) return c;
    ...
    OpenGLGraphicsContextCustomShader::OpenGLGraphicsContextCustomShader(const String& fragmentShaderCode)
        : code(String(JUCE_DECLARE_VARYING_COLOUR JUCE_DECLARE_VARYING_PIXELPOS "\n#define pixelAlpha frontColour.a\n")
               + fragmentShaderCode)
    ...
    OpenGLShaderProgram* OpenGLGraphicsContextCustomShader::getProgram(LowLevelGraphicsContext& gc) const
    {
        ...
        if (CustomProgram* c = CustomProgram::getOrCreate(gc, hashName, code, errorMessage)) return &(c->program);
    ...
    Result OpenGLGraphicsContextCustomShader::checkCompilation(LowLevelGraphicsContext& gc)
    {
        ...
        if (CustomProgram::getOrCreate(gc, hashName, code, errorMessage) != nullptr) return Result::ok();
