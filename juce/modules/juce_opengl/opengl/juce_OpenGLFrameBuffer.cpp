...
namespace juce
{
    ...
    {
    public:
        Pimpl(OpenGLContext& c, const int w, const int h, const bool wantsDepthBuffer, const bool wantsStencilBuffer)
            : context(c)
            , width(w)
            , height(h)
            , textureID(0)
            ...
            , hasStencilBuffer(false)
        {
            ...
            if (context.extensions.glGenFramebuffers == nullptr) return;
#endif
            ...
            if (wantsDepthBuffer || wantsStencilBuffer)
            {
                ...
                context.extensions.glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER,
                                                             depthOrStencilBuffer);

                if (wantsStencilBuffer)
                    context.extensions.glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, GL_RENDERBUFFER,
                                                                 depthOrStencilBuffer);
        ...
        ~Pimpl()
        {
            if (OpenGLHelpers::isContextActive())
            {
                ...
                if (depthOrStencilBuffer != 0) context.extensions.glDeleteRenderbuffers(1, &depthOrStencilBuffer);

                if (frameBufferID != 0) context.extensions.glDeleteFramebuffers(1, &frameBufferID);
    ...
    bool OpenGLFrameBuffer::initialise(OpenGLContext& context, int width, int height)
    {
        ...
        if (!pimpl->createdOk()) pimpl.reset();
    ...
    {
        if (!image.isARGB()) return initialise(context, image.convertedToFormat(Image::ARGB));
    ...
    bool OpenGLFrameBuffer::reloadSavedCopy(OpenGLContext& context)
    {
        if (savedState != nullptr)
        {
            ...
            if (state->restore(context, *this)) return true;
    ...
    {
        if (!makeCurrentRenderingTarget()) return false;

        glPixelStorei(GL_PACK_ALIGNMENT, 4);
        glReadPixels(area.getX(), area.getY(), area.getWidth(), area.getHeight(), JUCE_RGBA_FORMAT, GL_UNSIGNED_BYTE, target);
    ...
    bool OpenGLFrameBuffer::writePixels(const PixelARGB* data, const Rectangle<int>& area)
    {
        ...
        pimpl->context.copyTexture(area, Rectangle<int>(area.getX(), area.getY(), tex.getWidth(), tex.getHeight()),
