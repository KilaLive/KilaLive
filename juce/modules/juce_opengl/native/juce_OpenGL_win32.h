...
namespace juce
{
    ...
    {
    public:
        NativeContext(Component& component, const OpenGLPixelFormat& pixelFormat, void* contextToShareWith,
                      bool /*useMultisampling*/, OpenGLVersion)
        {
            ...
            if (pixFormat != 0) SetPixelFormat(dc, pixFormat, &pfd);
            ...
            if (renderContext != 0)
            {
                ...
                if (contextToShareWith != nullptr) wglShareLists((HGLRC)contextToShareWith, renderContext);
        ...
        void updateWindowPosition(Rectangle<int> bounds)
        {
            if (nativeWindow != nullptr)
                SetWindowPos((HWND)nativeWindow->getNativeHandle(), 0, bounds.getX(), bounds.getY(), bounds.getWidth(),
                             bounds.getHeight(), SWP_NOACTIVATE | SWP_NOZORDER | SWP_NOOWNERZORDER);
        }
...
#define JUCE_DECLARE_WGL_EXTENSION_FUNCTION(name, returnType, params)                                                        \
    typedef returnType(__stdcall* type_##name) params;                                                                       \
    type_##name name;
...
#undef JUCE_INIT_WGL_FUNCTION
        ...
        void createNativeWindow(Component& component)
        {
            ...
            if (auto* peer = topComp->getPeer()) updateWindowPosition(peer->getAreaCoveredBy(component));
        ...
        int wglChoosePixelFormatExtension(const OpenGLPixelFormat& pixelFormat) const
        {
            ...
            if (wglChoosePixelFormatARB != nullptr)
            {
                ...
                if (pixelFormat.multisamplingLevel > 0 && OpenGLHelpers::isExtensionSupported("GL_ARB_multisample"))
                {
