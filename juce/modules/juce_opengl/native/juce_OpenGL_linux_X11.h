...
namespace juce
{
    ...
    private:
        struct DummyComponent : public Component
        {
            ...
            {
                if (commandId == 0) native.triggerRepaint();
            }
    ...
    public:
        NativeContext(Component& comp, const OpenGLPixelFormat& cPixelFormat, void* shareContext, bool /*useMultisampling*/,
                      OpenGLVersion)
            : component(comp)
            ...
            , dummy(*this)
        {
            ...
            GLint attribs[] = {GLX_RGBA,
                               GLX_DOUBLEBUFFER,
                               GLX_RED_SIZE,
                               cPixelFormat.redBits,
                               GLX_GREEN_SIZE,
                               cPixelFormat.greenBits,
                               GLX_BLUE_SIZE,
                               cPixelFormat.blueBits,
                               GLX_ALPHA_SIZE,
                               cPixelFormat.alphaBits,
                               GLX_DEPTH_SIZE,
                               cPixelFormat.depthBufferBits,
                               GLX_STENCIL_SIZE,
                               cPixelFormat.stencilBufferBits,
                               GLX_ACCUM_RED_SIZE,
                               cPixelFormat.accumulationBufferRedBits,
                               GLX_ACCUM_GREEN_SIZE,
                               cPixelFormat.accumulationBufferGreenBits,
                               GLX_ACCUM_BLUE_SIZE,
                               cPixelFormat.accumulationBufferBlueBits,
                               GLX_ACCUM_ALPHA_SIZE,
                               cPixelFormat.accumulationBufferAlphaBits,
                               None};

            bestVisual = glXChooseVisual(display, DefaultScreen(display), attribs);
            if (bestVisual == nullptr) return;
            ...
            embeddedWindow =
                XCreateWindow(display, windowH, glBounds.getX(), glBounds.getY(), (unsigned int)jmax(1, glBounds.getWidth()),
                              (unsigned int)jmax(1, glBounds.getHeight()), 0, bestVisual->depth, InputOutput,
                              bestVisual->visual, CWBorderPixel | CWColormap | CWEventMask, &swa);
        ...
        ~NativeContext()
        {
            ...
            if (bestVisual != nullptr) XFree(bestVisual);
        ...
        {
            return renderContext != 0 && glXMakeCurrent(display, embeddedWindow, renderContext);
        }
        ...
        void updateWindowPosition(Rectangle<int> newBounds)
        {
            ...
            ScopedXLock xlock(display);
            XMoveResizeWindow(display, embeddedWindow, physicalBounds.getX(), physicalBounds.getY(),
        ...
        {
            if (numFramesPerSwap == swapFrames) return true;
        ...
        void triggerRepaint()
        {
            if (context != nullptr) context->triggerRepaint();
        }
