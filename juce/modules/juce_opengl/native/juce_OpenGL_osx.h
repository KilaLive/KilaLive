...
namespace juce
{
    ...
    {
    public:
        NativeContext(Component& component, const OpenGLPixelFormat& pixFormat, void* contextToShare,
                      bool shouldUseMultisampling, OpenGLVersion version)
            : lastSwapTime(0)
            , minSwapTimeMs(0)
            , underrunCounter(0)
        {
            ...
            view = [cls.createInstance() initWithFrame:NSMakeRect(0, 0, 100.0f, 100.0f) pixelFormat:format];
            ...
            renderContext =
                [[[NSOpenGLContext alloc] initWithFormat:format shareContext:(NSOpenGLContext*)contextToShare] autorelease];
                                  ...
                                  const OpenGLPixelFormat& pixFormat, bool shouldUseMultisampling)
        {
            ...
            attribs[numAttribs++] = version >= openGL3_2 ? NSOpenGLProfileVersion3_2Core : NSOpenGLProfileVersionLegacy;
#endif
            ...
            attribs[numAttribs++] =
                (NSOpenGLPixelFormatAttribute)(pixFormat.redBits + pixFormat.greenBits + pixFormat.blueBits);
            ...
            attribs[numAttribs++] = (NSOpenGLPixelFormatAttribute)(
                pixFormat.accumulationBufferRedBits + pixFormat.accumulationBufferGreenBits
        ...
        bool makeActive() const noexcept
        {
            ...
            if ([renderContext view] != view) [renderContext setView:view];
        ...
        void swapBuffers()
        {
            ...
            if (minSwapTimeMs > 0)
            {
                ...
                {
                    if (underrunCounter > 0) --underrunCounter;
                }
            }
        ...
        bool setSwapInterval(int numFramesPerSwap)
        {
            ...
            [renderContext setValues:(const GLint*)&numFramesPerSwap forParameter:NSOpenGLCPSwapInterval];
            return true;
        }
        ...
        {
            GLint numFrames = 0;
            [renderContext getValues:&numFrames forParameter:NSOpenGLCPSwapInterval];
        ...
        struct MouseForwardingNSOpenGLViewClass : public ObjCClass<NSOpenGLView>
        {
            MouseForwardingNSOpenGLViewClass()
                : ObjCClass<NSOpenGLView>("JUCEGLView_")
            {
    ...
    {
        if (auto* context = OpenGLContext::getContextAttachedTo(comp)) context->overrideCanBeAttached(shouldSuspend);

        for (auto* child : comp.getChildren()) componentPeerAboutToChange(*child, shouldSuspend);
    }
