...
#define JUCE_RTAS_PLUGINGESTALT_IS_CACHEABLE 1
    ...
    public:
        ...
        //==============================================================================
        class JuceCustomUIView : public CCustomView, public Timer
        {
        public:
            //==============================================================================
            ...
            void updateSize()
            {
                ...
                if (editorComp->getWidth() != 0 && editorComp->getHeight() != 0)
                {
                    ...
                    if (oldRect.right != r.right || oldRect.bottom != r.bottom) startTimer(50);
                }
            }
        ...
        private:
            ...
            {
            public:
                EditorCompWrapper(void* const hostWindow_, Component* const editorComp, JuceCustomUIView* const owner_)
                ...
                {
                    if (Component* const ed = getEditor()) ed->setBounds(getLocalBounds());
                ...
                {
#if !JucePlugin_EditorRequiresKeyboardFocus
                    if (hasKeyboardFocus(true)) passFocusToHostWindow(hostWindow);
#endif
                }
#endif
        ...
        {
            if (JuceCustomUIView* const v = getView()) v->updateSize();
        ...
        void SetViewPort(GrafPtr port) override
        {
            ...
            if (JuceCustomUIView* const v = getView()) v->attachToWindow(port);
        }

        //==============================================================================
        ComponentResult GetDelaySamplesLong(long* aNumSamples) override
        {
            if (aNumSamples != nullptr) *aNumSamples = juceFilter != nullptr ? juceFilter->getLatencySamples() : 0;
        ...
        void EffectInit() override
        {
            ...
            {
                OSType rtasParamID =
                    static_cast<OSType>(usingManagedParameters ? juceFilter->getParameterID(i).hashCode() : i);
            ...
            if (MIDILogIn() == noErr)
            {
                ...
                if (CEffectType* const type = dynamic_cast<CEffectType*>(this->GetProcessType()))
                {
                    ...
                    midiBufferNode =
                        new CEffectMIDIOtherBufferedNode(&mMIDIWorld, 8192, eLocalNode, nodeName + 1, midiBuffer);
            ...
            channels.calloc(jmax(juceFilter->getTotalNumInputChannels(), juceFilter->getTotalNumOutputChannels()));
        ...
        void RenderAudio(float** inputs, float** outputs, long numSamples) override
        {
            ...
            if (midiBufferNode != nullptr)
            {
                ...
                if (midiBufferNode->FillMIDIBuffer(mRTGlobals->mRunningTime, numSamples) == noErr)
                {
                    ...
                    for (int i = 0; i < numMidiEvents; ++i)
                    {
                        ...
                        midiEvents.addEvent(m.mData, m.mLength, jlimit(0, (int)numSamples - 1, (int)m.mTimestamp));
                    }
                }
            }
#endif
...

            {
                ...
                {
                    for (int i = 0; i < numOut; ++i) FloatVectorOperations::clear(outputs[i], numSamples);
                }
                else
                {
                    {
                        ...
                        for (; i < numIn; ++i) channels[i] = inputs[i];
                    }
        ...
        ComponentResult GetChunk(OSType chunkID, SFicPlugInChunk* chunk) override
        {
            if (chunkID == juceChunkType)
            {
                if (tempFilterData.getSize() == 0) juceFilter->getStateInformation(tempFilterData);
        ...
        ComponentResult SetChunk(OSType chunkID, SFicPlugInChunk* chunk) override
        {
            if (chunkID == juceChunkType)
            {
                ...
                {
                    juceFilter->setStateInformation((void*)chunk->fData, chunk->fSize - sizeof(SFicPlugInChunkHeader));
                }
    ...
    private:
        ...
        {
            return roundToInt(jlimit(-(double)0x80000000, (double)0x7fffffff, n * (double)0xffffffff - (double)0x80000000));
        }
        ...
        public:
            ...
            long GetOrientation() const
            {
                return processor.isParameterOrientationInverted(index)
                           ? kDAE_RightMinLeftMax | kDAE_TopMinBottomMax | kDAE_RotarySingleDotMode
                                 | kDAE_RotaryRightMinLeftMax
                           : kDAE_LeftMinRightMax | kDAE_BottomMinTopMax | kDAE_RotarySingleDotMode
            ...
            for (int i = 0; i < numConfigs; ++i)
            {
                if (channelConfigs[i][0] <= 8 && channelConfigs[i][1] <= 8)
                {
                    ...
                    CEffectType* const type =
                        new CEffectTypeRTAS(pluginId, JucePlugin_RTASProductId, JucePlugin_RTASCategory);
                    ...
                    type->DefineStemFormats(
                        getFormatForChans(channelConfigs[i][0] != 0 ? channelConfigs[i][0] : channelConfigs[i][1]),
