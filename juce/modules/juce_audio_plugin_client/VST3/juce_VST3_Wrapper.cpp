...
//==============================================================================
    ...
    public:
        ...
        {
            if (host != nullptr) host->queryInterface(FUnknown::iid, (void**)&hostContext);
        }
...
#pragma clang diagnostic pop
        ...
        tresult PLUGIN_API initialize(FUnknown* context) override
        {
            ...
            {
                if (hostContext != nullptr) hostContext->release();

                hostContext = context;

                if (hostContext != nullptr) hostContext->addRef();
            }
        ...
        {
            if (auto* pluginInstance = getPluginInstance()) pluginInstance->removeListener(this);
        ...
        {
            Param(AudioProcessor& p, int index, Vst::ParamID paramID)
                : owner(p)
                , paramIndex(index)
            {
                info.id = paramID;
            ...
            bool setNormalized(Vst::ParamValue v) override
            {
                ...
                if (v != valueNormalized)
                {
                    ...
                    // during playback
                    if (owner.vst3IsPlaying.get() == 0) owner.setParameter(paramIndex, static_cast<float>(v));
            ...
            {
                return juce::String(
                    juce::CharPointer_UTF16(reinterpret_cast<const juce::CharPointer_UTF16::CharType*>(text)));
            }
        ...
        struct BypassParam : public Vst::Parameter
        {
            ...
            bool fromString(const Vst::TChar* text, Vst::ParamValue& outValueNormalized) const override
            {
                ...
                if (paramValueString.equalsIgnoreCase("on") || paramValueString.equalsIgnoreCase("yes")
                ...
                if (paramValueString.equalsIgnoreCase("off") || paramValueString.equalsIgnoreCase("no")
                ...
                if (varValue.isDouble() || varValue.isInt() || varValue.isInt64() || varValue.isBool())
                {
        ...
        {
            ProgramChangeParameter(AudioProcessor& p)
                : owner(p)
            {
                ...
                info.defaultNormalizedValue =
                    static_cast<Vst::ParamValue>(owner.getCurrentProgram()) / static_cast<Vst::ParamValue>(info.stepCount);
                info.unitId = Vst::kRootUnitId;
            ...
            bool setNormalized(Vst::ParamValue v) override
            {
                ...
                if (!isPositiveAndBelow((int)program, owner.getNumPrograms())) return false;
            ...
            if (auto* instance = getPluginInstance())
            {
                if (list != nullptr)
                {
...

                    {
                        ...
                        if (list->getString(Vst::ChannelContext::kChannelNameKey, channelName, sizeof(channelName))
                            == kResultTrue)
...

                    {
                        ...
                        if (list->getInt(Vst::ChannelContext::kChannelColorKey, colour) == kResultTrue)
                            trackProperties.colour = Colour(
                                Vst::ChannelContext::GetRed((uint32)colour), Vst::ChannelContext::GetGreen((uint32)colour),
                    ...
                    if (MessageManager::getInstance()->isThisTheMessageThread())
                        ...
                        MessageManager::callAsync(
                            [trackProperties, instance] { instance->updateTrackProperties(trackProperties); });
                }
            }
        ...
        tresult PLUGIN_API setComponentState(IBStream* stream) override
        {
            ...
            if (auto* handler = getComponentHandler()) handler->restartComponent(Vst::kParamValuesChanged);
        ...
        IPlugView* PLUGIN_API createView(const char* name) override
        {
            if (auto* pluginInstance = getPluginInstance())
            {
                if (pluginInstance->hasEditor() && name != nullptr && strcmp(name, Vst::ViewType::kEditor) == 0)
                {
            ...
            if (auto* pluginInstance = getPluginInstance())
            {
                if (pluginInstance->getNumPrograms() > 1)
                    EditController::setParamNormalized(
                        paramPreset, static_cast<Vst::ParamValue>(pluginInstance->getCurrentProgram())
        ...
        {
            if (audioProcessor != nullptr) return audioProcessor->get();
    ...
    private:
        ...
        void setupParameters()
        {
            if (auto* pluginInstance = getPluginInstance())
            {
                ...
                parameterToMidiControllerOffset = static_cast<Vst::ParamID>(
                    usingManagedParameter ? paramMidiControllerOffset : parameters.getParameterCount());
            ...
            for (int c = 0, p = 0; c < numMIDIChannels; ++c)
            {
                for (int i = 0; i < Vst::kCountCtrlNumber; ++i, ++p)
                {
                    ...
                    parameters.addParameter(new Vst::Parameter(toString("MIDI CC " + String(c) + "|" + String(i)),
                                                               static_cast<Vst::ParamID>(p) + parameterToMidiControllerOffset,
                                                               0, 0, 0, 0, Vst::kRootUnitId));
                }
            }
        }
        ...
        static Vst::ParamID generateVSTParamIDForIndex(AudioProcessor* const pluginFilter, int paramIndex)
        {
            ...
            if (isPositiveAndBelow(paramIndex, n))
            {
                ...
                return managedParameter ? paramHash : static_cast<Vst::ParamID>(juceParamID.getIntValue());
            }
        ...
        inline Vst::ParamID getVSTParamIDForIndex(int paramIndex) const noexcept
        {
            return usingManagedParameter ? vstParamIDs.getReference(paramIndex) : static_cast<Vst::ParamID>(paramIndex);
        }
#endif

        //==============================================================================
        class JuceVST3Editor : public Vst::EditorView, public Steinberg::IPlugViewContentScaleSupport, private Timer
        {
        public:
            ...
            {
                if (parent == nullptr || isPlatformTypeSupported(type) == kResultFalse) return kResultFalse;

                if (component == nullptr) component = new ContentWrapperComponent(*this, pluginInstance);
                ...
                if (getHostType().isWavelab()) startTimer(200);
            ...
            tresult PLUGIN_API onSize(ViewRect* newSize) override
            {
                if (newSize != nullptr)
                {
                    ...
                    if (component != nullptr)
                    {
                        ...
                        if (auto* peer = component->getPeer()) peer->updateBounds();
                    }
            ...
            tresult PLUGIN_API checkSizeConstraint(ViewRect* rectToCheck) override
            {
                if (rectToCheck != nullptr && component != nullptr)
                {
                    ...
                    {
                        // checkSizeConstraint
                        auto juceRect = editor->getLocalArea(
                            component, Rectangle<int>::leftTopRightBottom(rectToCheck->left, rectToCheck->top,
        ...
        private:
            ...
            struct ContentWrapperComponent : public Component
            {
                ...
                {
                    if (pluginEditor != nullptr) return getLocalArea(pluginEditor, pluginEditor->getLocalBounds());
                ...
                {
                    if (isResizingChildToFitParent) return;
    ...
    namespace
    {
        ...
        struct AudioBusPointerHelper
        {
        };
        template <>
        struct AudioBusPointerHelper<float>
        {
            static inline float** impl(Vst::AudioBusBuffers& data) noexcept
            {
                return data.channelBuffers32;
            }
        };
        template <>
        struct AudioBusPointerHelper<double>
        {
            static inline double** impl(Vst::AudioBusBuffers& data) noexcept
            {
                return data.channelBuffers64;
            }
        };

        template <typename FloatType>
        struct ChooseBufferHelper
        {
        };
        template <>
        struct ChooseBufferHelper<float>
        {
            static inline AudioBuffer<float>& impl(AudioBuffer<float>& f, AudioBuffer<double>&) noexcept
            {
                return f;
            }
        };
        template <>
        struct ChooseBufferHelper<double>
        {
            static inline AudioBuffer<double>& impl(AudioBuffer<float>&, AudioBuffer<double>& d) noexcept
            {
                return d;
            }
        };
    ...
    public:
        ...
        ~JuceVST3Component()
        {
            if (pluginInstance != nullptr)
                if (pluginInstance->getPlayHead() == this) pluginInstance->setPlayHead(nullptr);
        }
        ...
        {
            if (host != hostContext) host.loadFrom(hostContext);
        ...
        tresult PLUGIN_API connect(IConnectionPoint* other) override
        {
            if (other != nullptr && juceVST3EditController == nullptr) juceVST3EditController.loadFrom(other);
        ...
        tresult PLUGIN_API setActive(TBool state) override
        {
            ...
            else
            {
                ...
                sampleRate = processSetup.sampleRate > 0.0 ? processSetup.sampleRate : sampleRate;

                bufferSize = processSetup.maxSamplesPerBlock > 0 ? (int)processSetup.maxSamplesPerBlock : bufferSize;
        ...
        bool isBypassed()
        {
            return comPluginInstance->isBypassed;
        }
        void setBypassed(bool bypassed)
        {
            comPluginInstance->isBypassed = bypassed;
        }
        ...
        void setStateInformation(const void* data, int sizeAsInt)
        {
            ...
            if ((size_t)size >= jucePrivDataIdentifierSize + sizeof(int64))
            {
                ...
                String magic(CharPointer_UTF8(buffer + size - jucePrivDataIdentifierSize), CharPointer_UTF8(buffer + size));
                ...
                if (magic == kJucePrivateDataIdentifier)
                {
                    ...
                    std::memcpy(&privateDataSize, buffer + ((size_t)size - jucePrivDataIdentifierSize - sizeof(uint64)),
                                sizeof(uint64));

                    privateDataSize = ByteOrder::swapIfBigEndian(privateDataSize);
                    size -= privateDataSize + jucePrivDataIdentifierSize + sizeof(uint64);

                    if (privateDataSize > 0) setJucePrivateStateInformation(buffer + size, static_cast<int>(privateDataSize));

                    size -= sizeof(uint64);
                }
            }

            if (size >= 0) pluginInstance->setStateInformation(data, static_cast<int>(size));
        }
        ...
        bool loadVST2CcnKBlock(const char* data, int size)
        {
            ...
            setStateInformation(bank->chunk, jmin((int)(size - (bank->chunk - data)), (int)htonl(bank->chunkSize)));
            return true;
        }
        ...
        bool loadVST3PresetFile(const char* data, int size)
        {
            ...
            for (int i = 0; i < entryCount; ++i)
            {
                ...
                if (entryOffset + 20 > size) return false;
        ...
        bool loadVST2CompatibleState(const char* data, int size)
        {
            ...
            if (header == 'VstW') return loadVST2VstWBlock(data, size);

            if (header == 'CcnK') return loadVST2CcnKBlock(data, size);
        ...
        bool readFromMemoryStream(IBStream* state)
        {
            ...
            if (s != nullptr && s->getStreamSize(size) == kResultOk && size > 0
                && size < 1024 * 1024 * 100)  // (some hosts seem to return junk for the size)
            {
                ...
                for (len = 0; bytesRead > 0 && len < static_cast<int>(block.getSize()); len += bytesRead)
                    if (state->read(block.getData(), static_cast<int32>(block.getSize()), &bytesRead) != kResultOk) break;
                ...
                if (getHostType().isAdobeAudition())
                    if (block.getSize() >= 5 && memcmp(block.getData(), "VC2!E", 5) == 0) return false;
        ...
        bool readFromUnknownStream(IBStream* state)
        {
...

            {
                ...
                for (;;)
                {
                    ...
                    if (bytesRead <= 0 || (status != kResultTrue && !getHostType().isWavelab())) break;
            ...
            return dataSize > 0 && dataSize < 0x7fffffff && loadStateData(allData.getData(), (int)dataSize);
        }
        ...
        tresult PLUGIN_API setState(IBStream* state) override
        {
            ...
            FUnknownPtr<IBStream> stateRefHolder(
                state);  // just in case the caller hasn't properly ref-counted the stream object

            if (state->seek(0, IBStream::kIBSeekSet, nullptr) == kResultTrue)
            {
                if (!getHostType().isFruityLoops() && readFromMemoryStream(state)) return kResultTrue;

                if (readFromUnknownStream(state)) return kResultTrue;
            }
        ...
        {
            if (getPluginInstance().getNumPrograms() > 0) return 1;
        ...
        tresult PLUGIN_API getProgramPitchName(Vst::ProgramListID, Steinberg::int32, Steinberg::int16,
        ...
        tresult PLUGIN_API getUnitByBus(Vst::MediaType, Vst::BusDirection, Steinberg::int32, Steinberg::int32,
        ...
        int getNumAudioBuses(bool isInput) const
        {
            ...
            for (int i = 0; i < numConfigs && hasOnlyZeroChannels == true; ++i)
                if (configs[i][isInput ? 0 : 1] != 0) hasOnlyZeroChannels = false;
        ...
        {
            if (type == Vst::kAudio) return getNumAudioBuses(dir == Vst::kInput);

            if (type == Vst::kEvent)
            {
                if (dir == Vst::kInput) return isMidiInputBusEnabled ? 1 : 0;

                if (dir == Vst::kOutput) return isMidiOutputBusEnabled ? 1 : 0;
            }

            return 0;
        }

        tresult PLUGIN_API getBusInfo(Vst::MediaType type, Vst::BusDirection dir, Steinberg::int32 index,
                                      Vst::BusInfo& info) override
        {
            if (type == Vst::kAudio)
            {
                if (index < 0 || index >= getNumAudioBuses(dir == Vst::kInput)) return kResultFalse;
        ...
        tresult PLUGIN_API activateBus(Vst::MediaType type, Vst::BusDirection dir, Steinberg::int32 index,
                                       TBool state) override
        {
            ...
            if (type == Vst::kAudio)
            {
                ...
                if (auto* bus = pluginInstance->getBus(dir == Vst::kInput, index))
                {
                    ...
                    if ((dir == Vst::kInput ? compLayout.inputBuses : compLayout.outputBuses).getReference(index)
                        != targetLayout)
                        return kResultFalse;
#endif
        ...
        bool checkBusFormatsAreNotDiscrete()
        {
            ...
            for (int i = 0; i < numInputBuses; ++i)
            {
                ...
                if (layout.isDiscreteLayout() && !layout.isDisabled()) return false;
            }
                                              ...
                                              Vst::SpeakerArrangement* outputs, Steinberg::int32 numOuts) override
        {
            ...
            if (numIns > numInputBuses || numOuts > numOutputBuses) return false;

            auto requested = pluginInstance->getBusesLayout();

            for (int i = 0; i < numIns; ++i) requested.getChannelSet(true, i) = getChannelSetForSpeakerArrangement(inputs[i]);
...
#ifdef JucePlugin_PreferredChannelConfigurations
            short configs[][2] = {JucePlugin_PreferredChannelConfigurations};
            if (!AudioProcessor::containsLayout(requested, configs)) return kResultFalse;
#endif

            return pluginInstance->setBusesLayoutWithoutEnabling(requested) ? kResultTrue : kResultFalse;
        }

        tresult PLUGIN_API getBusArrangement(Vst::BusDirection dir, Steinberg::int32 index,
        ...
        tresult PLUGIN_API canProcessSampleSize(Steinberg::int32 symbolicSampleSize) override
        {
            return (symbolicSampleSize == Vst::kSample32
                    || (getPluginInstance().supportsDoublePrecisionProcessing() && symbolicSampleSize == Vst::kSample64))
                       ? kResultTrue
                       : kResultFalse;
        }
        ...
        {
            if (canProcessSampleSize(newSetup.symbolicSampleSize) != kResultTrue) return kResultFalse;
        ...
        {
            if (!state) getPluginInstance().reset();
            ...
            for (Steinberg::int32 i = 0; i < numParamsChanged; ++i)
            {
                if (auto* paramQueue = paramChanges.getParameterData(i))
                {
                    ...
                    if (paramQueue->getPoint(numPoints - 1, offsetSamples, value) == kResultTrue)
                    {
                        ...
                        if (vstParamID == vstBypassParameterId) setBypassed(static_cast<float>(value) != 0.0f);
#if JUCE_VST3_EMULATE_MIDI_CC_WITH_PARAMETERS
        ...
        void addParameterChangeToMidiBuffer(const Steinberg::int32 offsetSamples, const Vst::ParamID id, const double value)
        {
            ...
            if (juceVST3EditController->getMidiControllerForParameter(id, channel, ctrlNumber))
            {
                if (ctrlNumber == Vst::kAfterTouch)
                    midiBuffer.addEvent(MidiMessage::channelPressureChange(channel, jlimit(0, 127, (int)(value * 128.0))),
                                        offsetSamples);
                else if (ctrlNumber == Vst::kPitchBend)
                    midiBuffer.addEvent(MidiMessage::pitchWheel(channel, jlimit(0, 0x3fff, (int)(value * 0x4000))),
                                        offsetSamples);
                else
                    midiBuffer.addEvent(MidiMessage::controllerEvent(channel, jlimit(0, 127, ctrlNumber),
                                                                     jlimit(0, 127, (int)(value * 128.0))),
                                        offsetSamples);
            }
        }
        ...
        {
            if (pluginInstance == nullptr) return kResultFalse;
            ...
            if (data.inputParameterChanges != nullptr) processParameterChanges(*data.inputParameterChanges);

#if JucePlugin_WantsMidiInput
            if (data.inputEvents != nullptr) MidiEventList::toMidiBuffer(midiBuffer, *data.inputEvents);
#endif

            if (getHostType().isWavelab())
            {
                const int numInputChans = (data.inputs != nullptr && data.inputs[0].channelBuffers32 != nullptr)
                                              ? (int)data.inputs[0].numChannels
                                              : 0;
                const int numOutputChans = (data.outputs != nullptr && data.outputs[0].channelBuffers32 != nullptr)
                                               ? (int)data.outputs[0].numChannels
                                               : 0;
            ...
            if (processSetup.symbolicSampleSize == Vst::kSample32)
                processAudio<float>(data, channelListFloat);
            else if (processSetup.symbolicSampleSize == Vst::kSample64)
                processAudio<double>(data, channelListDouble);
            else
                jassertfalse;

#if JucePlugin_ProducesMidiOutput
            if (data.outputEvents != nullptr) MidiEventList::toEventList(*data.outputEvents, midiBuffer);
#endif
    ...
    private:
        ...
        */
        ...
        void processAudio(Vst::ProcessData& data, Array<FloatType*>& channelList)
        {
...

            {
                ...
                for (int bus = 0; bus < n && totalOutputChans < plugInOutputChannels; ++bus)
                {
                    ...
                    {
                        const int numChans =
                            jmin(pluginInstance->getChannelCountOfBus(false, bus), plugInOutputChannels - totalOutputChans);
...

            {
                ...
                for (int bus = 0; bus < n && totalInputChans < plugInInputChannels; ++bus)
                {
                    if (bus < vstInputs)
                    {
                        ...
                        {
                            const int numChans =
                                jmin((int)data.inputs[bus].numChannels, plugInInputChannels - totalInputChans);
                            ...
                            for (int i = 0; i < numChans; ++i)
                            {
                                if (busChannels[i] != nullptr)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if (dst != src) FloatVectorOperations::copy(dst, src, (int)data.numSamples);
                                    }
                                }
                    ...
                    {
                        auto numChans =
                            jmin(pluginInstance->getChannelCountOfBus(true, bus), plugInInputChannels - totalInputChans);
            ...
            if (tmpBufferNeedsClearing) ChooseBufferHelper<FloatType>::impl(emptyBufferFloat, emptyBufferDouble).clear();
        ...
        FloatType* getTmpBufferForChannel(int channel, int numSamples) noexcept
        {
            ...
            if (numSamples > buffer.getNumSamples() || channel >= buffer.getNumChannels()) return nullptr;
        ...
        {
            return static_cast<int>(paramID);
        }
#else
        void cacheParameterIDs()
        {
            ...
            vstBypassParameterId =
                static_cast<Vst::ParamID>(usingManagedParameter ? JuceVST3EditController::paramBypass : numParameters);
        ...
        inline int getJuceIndexForVSTParamID(Vst::ParamID paramID) const noexcept
        {
            return usingManagedParameter ? paramMap[static_cast<int32>(paramID)] : static_cast<int>(paramID);
        }
#endif
...
#define JUCE_EXPORTED_FUNCTION extern "C" __attribute__((visibility("default")))
    ...
    JUCE_EXPORTED_FUNCTION bool bundleExit()
    {
        if (shutdownModule())
        {
            if (--numBundleRefs == 0)
            {
                for (int i = 0; i < bundleRefs.size(); ++i) CFRelease(bundleRefs.getUnchecked(i));
    ...
    {
        JucePluginFactory()
            : factoryInfo(JucePlugin_Manufacturer, JucePlugin_ManufacturerWebsite, JucePlugin_ManufacturerEmail,
        ...
        {
            if (globalFactory == this) globalFactory = nullptr;
        }
            ...
            for (auto* entry : classes)
            {
                if (doUIDsMatch(entry->infoW.cid, cid))
                {
                    if (auto* instance = entry->createFunction(host))
                    {
                        ...
                        if (instance->queryInterface(iidToQuery, obj) == kResultOk) return kResultOk;
                    }
    ...
    private:
        ...
        tresult PLUGIN_API getPClassInfo(Steinberg::int32 index, PClassInfoType* info)
        {
            if (info != nullptr)
            {
                ...
                if (auto* entry = classes[(int)index])
                {
                    if (entry->isUnicode) return kResultFalse;
...
#pragma comment(linker, "/EXPORT:" __FUNCTION__ "=" __FUNCDNAME__)
    ...
    if (globalFactory == nullptr)
    {
        ...
        static const PClassInfo2 componentClass(JuceVST3Component::iid, PClassInfo::kManyInstances, kVstAudioEffectClass,
                                                JucePlugin_Name, JucePlugin_Vst3ComponentFlags, JucePlugin_Vst3Category,
                                                JucePlugin_Manufacturer, JucePlugin_VersionString, kVstVersionString);

        globalFactory->registerClass(componentClass, createComponentInstance);

        static const PClassInfo2 controllerClass(JuceVST3EditController::iid, PClassInfo::kManyInstances,
                                                 kVstComponentControllerClass, JucePlugin_Name, JucePlugin_Vst3ComponentFlags,
