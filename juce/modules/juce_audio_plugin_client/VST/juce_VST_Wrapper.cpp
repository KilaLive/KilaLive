...
namespace
{
    ...
    static HWND getWindowParent(HWND w) noexcept
    {
    ...
    static HWND findMDIParentOf(HWND w)
    {
        ...
        while (w != 0)
        {
            ...
            if (String(windowType).equalsIgnoreCase("MDIClient")) return parent;
            ...
            if (dw > 100 || dh > 100) break;

            w = parent;

            if (dw == 2 * frameThickness) break;
        }
...
*/
class JuceVSTWrapper : public AudioProcessorListener, public AudioPlayHead, private Timer, private AsyncUpdater
{
private:
    //==============================================================================
    ...
    struct VstTempBuffers
    {
        ...
        {
            for (auto* c : tempChannels) delete[] c;
...
public:
        ...
        , processor(af)
    {
        ...
        if (processor->isMidiEffect()) maxNumInChannels = maxNumOutChannels = 2;
        ...
        if (processor->supportsDoublePrecisionProcessing()) vstEffect.flags |= vstEffectFlagInplaceDoubleAudio;
    ...
    VstEffectInterface* getVstEffectInterface() noexcept
    {
        return &vstEffect;
    }

    template <typename FloatType>
    void internalProcessReplacing(FloatType** inputs, FloatType** outputs, int32 numSamples,
                                  VstTempBuffers<FloatType>& tmpBuffers)
    {
        ...
        if (firstProcessCallback)
        {
            ...
            if (getHostType().isWavelab())
            {
                ...
                if (priority <= THREAD_PRIORITY_NORMAL && priority >= THREAD_PRIORITY_LOWEST) processor->setNonRealtime(true);
            }
#endif
        }
...

        {
            ...
            else
            {
                ...
                for (i = 0; i < numOut; ++i)
                {
                    ...
                    if (i < numIn)
                    {
                        if (chan != inputs[i]) memcpy(chan, inputs[i], sizeof(FloatType) * (size_t)numSamples);
                    }
                    else
                    {
                ...
                for (; i < numIn; ++i) tmpBuffers.channels[i] = inputs[i];
                ...
                for (i = 0; i < numOut; ++i)
                    if (auto* chan = tmpBuffers.tempChannels.getUnchecked(i))
                        if (auto* dest = outputs[i]) memcpy(dest, chan, sizeof(FloatType) * (size_t)numSamples);
            }
        }
    ...
    static void processDoubleReplacingCB(VstEffectInterface* vstInterface, double** inputs, double** outputs,
                                         int32 sampleFrames)
    {
    ...
    void resume()
    {
        if (processor != nullptr)
        {
            ...
            */
            ...
            {
                if (hostCallback != nullptr) hostCallback(&vstEffect, hostOpcodePlugInWantsMidi, 0, 1, 0, 0);
            }

            if (getHostType().isAbletonLive() && hostCallback != nullptr
    ...
    bool getCurrentPosition(AudioPlayHead::CurrentPositionInfo& info) override
    {
        ...
        if (ti == nullptr || ti->sampleRate <= 0) return false;
        ...
        if ((ti->flags & vstTimingInfoFlagSmpteValid) != 0)
        {
            ...
            switch (ti->smpteRate)
            {
            case vstSmpteRateFps239:
                rate = AudioPlayHead::fps23976;
                fps  = 24.0 * 1000.0 / 1001.0;
                break;
            case vstSmpteRateFps24:
                rate = AudioPlayHead::fps24;
                fps  = 24.0;
                break;
            case vstSmpteRateFps25:
                rate = AudioPlayHead::fps25;
                fps  = 25.0;
                break;
            case vstSmpteRateFps2997:
                rate = AudioPlayHead::fps2997;
                fps  = 30.0 * 1000.0 / 1001.0;
                break;
            case vstSmpteRateFps30:
                rate = AudioPlayHead::fps30;
                fps  = 30.0;
                break;
            case vstSmpteRateFps2997drop:
                rate = AudioPlayHead::fps2997drop;
                fps  = 30.0 * 1000.0 / 1001.0;
                break;
            case vstSmpteRateFps30drop:
                rate = AudioPlayHead::fps30drop;
                fps  = 30.0;
                break;

            case vstSmpteRate16mmFilm:
            case vstSmpteRate35mmFilm:
                fps = 24.0;
                break;

            case vstSmpteRateFps249:
                fps = 25.0 * 1000.0 / 1001.0;
                break;
            case vstSmpteRateFps599:
                fps = 60.0 * 1000.0 / 1001.0;
                break;
            case vstSmpteRateFps60:
                fps = 60;
                break;
    ...
    {
        if (processor == nullptr) return 0.0f;
    ...
    {
        if (hostCallback != nullptr) hostCallback(&vstEffect, hostOpcodeParameterChanged, index, 0, 0, newValue);
    }

    void audioProcessorParameterChangeGestureBegin(AudioProcessor*, int index) override
    {
        if (hostCallback != nullptr) hostCallback(&vstEffect, hostOpcodeParameterChangeGestureBegin, index, 0, 0, 0);
    }

    void audioProcessorParameterChangeGestureEnd(AudioProcessor*, int index) override
    {
        if (hostCallback != nullptr) hostCallback(&vstEffect, hostOpcodeParameterChangeGestureEnd, index, 0, 0, 0);
    }
    ...
    void audioProcessorChanged(AudioProcessor*) override
    {
        ...
        if (hostCallback != nullptr) hostCallback(&vstEffect, hostOpcodeUpdateView, 0, 0, 0, 0);
    ...
    {
        if (hostCallback != nullptr) hostCallback(&vstEffect, hostOpcodeIOModified, 0, 0, 0, 0);
    }

    bool getPinProperties(VstPinInfo& properties, bool direction, int index) const
    {
        if (processor->isMidiEffect()) return false;
        ...
        if ((channelIdx = processor->getOffsetInBusBufferForAbsoluteChannelIndex(direction, index, busIdx)) >= 0)
        {
...
#else
            if (channelSet.size() > 1) label += " " + AudioChannelSet::getAbbreviatedChannelTypeName(channelType);
#endif
            ...
            if (channelType == AudioChannelSet::left || channelType == AudioChannelSet::leftSurround
                || channelType == AudioChannelSet::leftCentre || channelType == AudioChannelSet::leftSurroundSide
                || channelType == AudioChannelSet::topFrontLeft || channelType == AudioChannelSet::topRearLeft
                || channelType == AudioChannelSet::leftSurroundRear || channelType == AudioChannelSet::wideLeft)
    ...
    struct SpeakerMappings : private AudioChannelSet  // (inheritance only to give easier access to items in the namespace)
    {
        struct Mapping
        {
            ...
            bool matches(const Array<ChannelType>& chans) const noexcept
            {
                ...
                for (int i = 0; i < n; ++i)
                {
                    ...
                    if (channels[i] != chans.getUnchecked(i)) return false;
                }
        ...
        static AudioChannelSet vstArrangementTypeToChannelSet(const VstSpeakerConfiguration& arr)
        {
            ...
            for (auto* m = getMappings(); m->vst2 != vstSpeakerConfigTypeEmpty; ++m)
            {
                if (m->vst2 == arr.type)
                {
                    ...
                    for (int i = 0; m->channels[i] != 0; ++i) s.addChannel(m->channels[i]);
        ...
        static int32 channelSetToVstArrangementType(AudioChannelSet channels)
        {
            ...
            for (auto* m = getMappings(); m->vst2 != vstSpeakerConfigTypeEmpty; ++m)
                if (m->matches(chans)) return m->vst2;
        ...
        static const Mapping* getMappings() noexcept
        {
            static const Mapping mappings[] = {
                ...
                {vstSpeakerConfigTypeLRLsRsSlSr,
                 {left, right, leftSurround, rightSurround, leftSurroundRear, rightSurroundRear, unknown}},
                {vstSpeakerConfigTypeLRCLfeLsRsCs,
                 {left, right, centre, LFE, leftSurround, rightSurround, surround, unknown}},
                {vstSpeakerConfigTypeLRLfeLsRsSlSr,
                 {left, right, LFE, leftSurround, rightSurround, leftSurroundRear, rightSurroundRear, unknown}},
                {vstSpeakerConfigTypeLRCLsRsLcRc,
                 {left, right, centre, leftSurround, rightSurround, topFrontLeft, topFrontRight, unknown}},
                {vstSpeakerConfigTypeLRCLsRsSlSr,
                 {left, right, centre, leftSurround, rightSurround, leftSurroundRear, rightSurroundRear, unknown}},
                {vstSpeakerConfigTypeLRCLfeLsRsLcRc,
                 {left, right, centre, LFE, leftSurround, rightSurround, topFrontLeft, topFrontRight, unknown}},
                {vstSpeakerConfigTypeLRCLfeLsRsSlSr,
                 {left, right, centre, LFE, leftSurround, rightSurround, leftSurroundRear, rightSurroundRear, unknown}},
                {vstSpeakerConfigTypeLRCLsRsLcRcCs,
                 {left, right, centre, leftSurround, rightSurround, topFrontLeft, topFrontRight, surround, unknown}},
                {vstSpeakerConfigTypeLRCLsRsCsSlSr,
                 {left, right, centre, leftSurround, rightSurround, surround, leftSurroundRear, rightSurroundRear, unknown}},
                {vstSpeakerConfigTypeLRCLfeLsRsLcRcCs,
                 {left, right, centre, LFE, leftSurround, rightSurround, topFrontLeft, topFrontRight, surround, unknown}},
                {vstSpeakerConfigTypeLRCLfeLsRsCsSlSr,
                 {left, right, centre, LFE, leftSurround, rightSurround, surround, leftSurroundRear, rightSurroundRear,
                  unknown}},
                {vstSpeakerConfigTypeLRCLfeLsRsTflTfcTfrTrlTrrLfe2,
                 {left, right, centre, LFE, leftSurround, rightSurround, topFrontLeft, topFrontCentre, topFrontRight,
                  topRearLeft, topRearRight, LFE2, unknown}},
        ...
        static inline int32 getSpeakerType(AudioChannelSet::ChannelType type) noexcept
        {
            ...
            case AudioChannelSet::left:
                return vstIndividualSpeakerTypeLeft;
            case AudioChannelSet::right:
                return vstIndividualSpeakerTypeRight;
            case AudioChannelSet::centre:
                return vstIndividualSpeakerTypeCentre;
            case AudioChannelSet::LFE:
                return vstIndividualSpeakerTypeLFE;
            case AudioChannelSet::leftSurround:
                return vstIndividualSpeakerTypeLeftSurround;
            case AudioChannelSet::rightSurround:
                return vstIndividualSpeakerTypeRightSurround;
            case AudioChannelSet::leftCentre:
                return vstIndividualSpeakerTypeLeftCentre;
            case AudioChannelSet::rightCentre:
                return vstIndividualSpeakerTypeRightCentre;
            case AudioChannelSet::surround:
                return vstIndividualSpeakerTypeSurround;
            case AudioChannelSet::leftSurroundRear:
                return vstIndividualSpeakerTypeLeftRearSurround;
            case AudioChannelSet::rightSurroundRear:
                return vstIndividualSpeakerTypeRightRearSurround;
            case AudioChannelSet::topMiddle:
                return vstIndividualSpeakerTypeTopMiddle;
            case AudioChannelSet::topFrontLeft:
                return vstIndividualSpeakerTypeTopFrontLeft;
            case AudioChannelSet::topFrontCentre:
                return vstIndividualSpeakerTypeTopFrontCentre;
            case AudioChannelSet::topFrontRight:
                return vstIndividualSpeakerTypeTopFrontRight;
            case AudioChannelSet::topRearLeft:
                return vstIndividualSpeakerTypeTopRearLeft;
            case AudioChannelSet::topRearCentre:
                return vstIndividualSpeakerTypeTopRearCentre;
            case AudioChannelSet::topRearRight:
                return vstIndividualSpeakerTypeTopRearRight;
            case AudioChannelSet::LFE2:
                return vstIndividualSpeakerTypeLFE2;
            default:
                break;
            }
    ...
    void timerCallback() override
    {
        ...
        if (chunkMemoryTime > 0 && chunkMemoryTime < juce::Time::getApproximateMillisecondCounter() - 2000 && !recursionCheck)
        {
            chunkMemory.reset();
            chunkMemoryTime = 0;
        }

        if (editorComp != nullptr) editorComp->checkVisibility();
    }

    void createEditorComp()
    {
        if (hasShutdown || processor == nullptr) return;
    ...
    void deleteEditor(bool canDeleteLaterIfModal)
    {
        JUCE_AUTORELEASEPOOL
        {
            ...
            if (editorComp != nullptr)
            {
                ...
                if (auto* ed = editorComp->getEditorComp()) processor->editorBeingDeleted(ed);
    ...
    {
        if (hasShutdown) return 0;

        switch (opCode)
        {
        case plugInOpcodeOpen:
            return handleOpen(args);
        case plugInOpcodeClose:
            return handleClose(args);
        case plugInOpcodeSetCurrentProgram:
            return handleSetCurrentProgram(args);
        case plugInOpcodeGetCurrentProgram:
            return handleGetCurrentProgram(args);
        case plugInOpcodeSetCurrentProgramName:
            return handleSetCurrentProgramName(args);
        case plugInOpcodeGetCurrentProgramName:
            return handleGetCurrentProgramName(args);
        case plugInOpcodeGetParameterLabel:
            return handleGetParameterLabel(args);
        case plugInOpcodeGetParameterText:
            return handleGetParameterText(args);
        case plugInOpcodeGetParameterName:
            return handleGetParameterName(args);
        case plugInOpcodeSetSampleRate:
            return handleSetSampleRate(args);
        case plugInOpcodeSetBlockSize:
            return handleSetBlockSize(args);
        case plugInOpcodeResumeSuspend:
            return handleResumeSuspend(args);
        case plugInOpcodeGetEditorBounds:
            return handleGetEditorBounds(args);
        case plugInOpcodeOpenEditor:
            return handleOpenEditor(args);
        case plugInOpcodeCloseEditor:
            return handleCloseEditor(args);
        case plugInOpcodeIdentify:
            return (pointer_sized_int)ByteOrder::bigEndianInt("NvEf");
        case plugInOpcodeGetData:
            return handleGetData(args);
        case plugInOpcodeSetData:
            return handleSetData(args);
        case plugInOpcodePreAudioProcessingEvents:
            return handlePreAudioProcessingEvents(args);
        case plugInOpcodeIsParameterAutomatable:
            return handleIsParameterAutomatable(args);
        case plugInOpcodeParameterValueForText:
            return handleParameterValueForText(args);
        case plugInOpcodeGetProgramName:
            return handleGetProgramName(args);
        case plugInOpcodeGetInputPinProperties:
            return handleGetInputPinProperties(args);
        case plugInOpcodeGetOutputPinProperties:
            return handleGetOutputPinProperties(args);
        case plugInOpcodeGetPlugInCategory:
            return handleGetPlugInCategory(args);
        case plugInOpcodeSetSpeakerConfiguration:
            return handleSetSpeakerConfiguration(args);
        case plugInOpcodeSetBypass:
            return handleSetBypass(args);
        case plugInOpcodeGetPlugInName:
            return handleGetPlugInName(args);
        case plugInOpcodeGetManufacturerProductName:
            return handleGetPlugInName(args);
        case plugInOpcodeGetManufacturerName:
            return handleGetManufacturerName(args);
        case plugInOpcodeGetManufacturerVersion:
            return handleGetManufacturerVersion(args);
        case plugInOpcodeManufacturerSpecific:
            return handleManufacturerSpecific(args);
        case plugInOpcodeCanPlugInDo:
            return handleCanPlugInDo(args);
        case plugInOpcodeGetTailSize:
            return handleGetTailSize(args);
        case plugInOpcodeKeyboardFocusRequired:
            return handleKeyboardFocusRequired(args);
        case plugInOpcodeGetVstInterfaceVersion:
            return handleGetVstInterfaceVersion(args);
        case plugInOpcodeGetCurrentMidiProgram:
            return handleGetCurrentMidiProgram(args);
        case plugInOpcodeGetSpeakerArrangement:
            return handleGetSpeakerConfiguration(args);
        case plugInOpcodeSetNumberOfSamplesToProcess:
            return handleSetNumberOfSamplesToProcess(args);
        case plugInOpcodeSetSampleFloatType:
            return handleSetSampleFloatType(args);
        case pluginOpcodeGetNumMidiInputChannels:
            return handleGetNumMidiInputChannels();
        case pluginOpcodeGetNumMidiOutputChannels:
            return handleGetNumMidiOutputChannels();
        default:
            return 0;
        }
    }
    ...
    struct EditorCompWrapper : public Component
    {
...
#if JUCE_WINDOWS
            if (!getHostType().isReceptor()) addMouseListener(this, true);
#endif
        ...
        {
#if JUCE_MAC
            if (hostWindow != 0) checkWindowVisibilityVST(hostWindow, this, wrapper.useNSView);
#endif
        }
        ...
        void resized() override
        {
            if (auto* ed = getEditorComp())
            {
                ...
                if (!getHostType().isBitwigStudio()) updateWindowSize();
            }

#if JUCE_MAC && !JUCE_64BIT
            if (!wrapper.useNSView) updateEditorCompBoundsVST(this);
#endif
        }
        ...
        {
            if (auto* ed = getEditorComp()) return getLocalArea(ed, ed->getLocalBounds());
...
#if JUCE_MAC
                    if (wrapper.useNSView) setTopLeftPosition(0, getHeight() - pos.getHeight());
#endif
...
#if JUCE_MAC
                    resizeHostWindow(pos.getWidth(),
                                     pos.getHeight());  // (doing this a second time seems to be necessary in tracktion)
#endif
                }
            }
        }
        ...
        void resizeHostWindow(int newWidth, int newHeight)
        {
            ...
            {
                auto status =
                    host(wrapper.getVstEffectInterface(), hostOpcodeCanHostDo, 0, 0, const_cast<char*>("sizeWindow"), 0);

                if (status == (pointer_sized_int)1 || getHostType().isAbletonLive())
                {
                    isInSizeWindow = true;
                    sizeWasSuccessful =
                        (host(wrapper.getVstEffectInterface(), hostOpcodeWindowSize, newWidth, newHeight, 0, 0) != 0);
            ...
            if (!sizeWasSuccessful)
            {
                ...
                while (w != 0)
                {
                    ...
                    if (String(windowType).equalsIgnoreCase("MDIClient")) break;
...
private:
    ...
    static JuceVSTWrapper* getWrapper(VstEffectInterface* v) noexcept
    {
        return static_cast<JuceVSTWrapper*>(v->effectPointer);
    }
    ...
    static inline int32 convertHexVersionToDecimal(const unsigned int hexVersion)
    {
        ...
        // Encoding B
        if (major < 1) return major * 1000 + minor * 100 + bugfix * 10;

        // Encoding E
        if (major > 100) return major * 10000000 + minor * 100000 + bugfix * 1000;
    ...
    void deleteTempChannels(VstTempBuffers<FloatType>& tmpBuffers)
    {
        ...
        if (processor != nullptr)
            tmpBuffers.tempChannels.insertMultiple(0, nullptr, vstEffect.numInputChannels + vstEffect.numOutputChannels);
    }
    ...
    void findMaxTotalChannels(int& maxTotalIns, int& maxTotalOuts)
    {
        ...
        if (numInputBuses > 1 || numOutputBuses > 1)
        {
            ...
            for (int i = 0; i < numInputBuses; ++i) maxTotalIns += processor->getChannelCountOfBus(true, i);

            for (int i = 0; i < numOutputBuses; ++i) maxTotalOuts += processor->getChannelCountOfBus(false, i);
        }
        else
        {
    ...
    pointer_sized_int handleClose(VstOpCodeArguments)
    {
        ...
        if (MessageManager::getInstance()->isThisTheMessageThread()) deleteEditor(false);
    ...
    pointer_sized_int handleSetSpeakerConfiguration(VstOpCodeArguments args)
    {
        ...
        if (processor->isMidiEffect()) return 0;
        ...
        if (pluginInput != nullptr && pluginInput->numberOfChannels > 0 && numIns == 0) return 0;

        if (pluginOutput != nullptr && pluginOutput->numberOfChannels > 0 && numOuts == 0) return 0;
        ...
        if (!AudioProcessor::containsLayout(layouts, configs)) return 0;
#endif
    ...
    {
        if (handleManufacturerSpecificVST2Opcode(args.index, args.value, args.ptr, args.opt)) return 1;
    ...
    pointer_sized_int handleCanPlugInDo(VstOpCodeArguments args)
    {
        ...
        if (matches("receiveVstEvents") || matches("receiveVstMidiEvent") || matches("receiveVstMidiEvents"))
        {
#if JucePlugin_WantsMidiInput || JucePlugin_IsMidiEffect
            return 1;
#else
        ...
        if (matches("sendVstEvents") || matches("sendVstMidiEvent") || matches("sendVstMidiEvents"))
        {
#if JucePlugin_ProducesMidiOutput || JucePlugin_IsMidiEffect
            return 1;
#else
        ...
        if (matches("receiveVstTimeInfo") || matches("conformsToWindowRules") || matches("supportsViewDpiScaling")
            || matches("bypass"))
        {
            return 1;
        }
        ...
        if (matches("openCloseAnyThread")) return -1;

        if (matches("MPE")) return processor->supportsMPE() ? 1 : 0;
    ...
    {
        if (processor != nullptr) return (pointer_sized_int)(processor->getTailLengthSeconds() * sampleRate);
    ...
    pointer_sized_int handleGetSpeakerConfiguration(VstOpCodeArguments args)
    {
        ...
        if (pluginHasSidechainsOrAuxs() || processor->isMidiEffect()) return false;
        ...
        cachedInArrangement.malloc(
            speakerBaseSize + (static_cast<std::size_t>(inputLayout.size()) * sizeof(VstSpeakerConfiguration)), 1);
        cachedOutArrangement.malloc(
            speakerBaseSize + (static_cast<std::size_t>(outputLayout.size()) * sizeof(VstSpeakerConfiguration)), 1);
        ...
        if (!isProcessing)
        {
            if (processor != nullptr)
            {
                processor->setProcessingPrecision(
                    (args.value == vstProcessingSampleTypeDouble && processor->supportsDoublePrecisionProcessing())
    ...
    pointer_sized_int handleSetContentScaleFactor(float scale)
    {
        if (editorScaleFactor != scale)
        {
            ...
            if (editorComp != nullptr)
            {
                if (auto* ed = editorComp->getEditorComp()) ed->setScaleFactor(editorScaleFactor);

                if (editorComp != nullptr) editorComp->updateWindowSize();
            }
#endif
        }
...
{
    if (reason == DLL_PROCESS_ATTACH) Process::setCurrentModuleInstanceHandle(instance);
