...
#define VST3_REPLACEMENT_AVAILABLE 1
    ...
    void JUCE_API getUUIDForVST2ID(bool forControllerUID, uint8 uuid[16])
    {
        ...
        sscanf_s
#endif
            (uidString, "%08lX%04X%04X%02X%02X%02X%02X%02X%02X%02X%02X", &p0, &p1, &p2, &p3[0], &p3[1], &p3[2], &p3[3],
             &p3[4], &p3[5], &p3[6], &p3[7]);
        ...
        for (int i = 0; i < 4; ++i) uuid[i + 0] = q0.bytes[i];

        for (int i = 0; i < 2; ++i) uuid[i + 4] = q1.bytes[i];

        for (int i = 0; i < 2; ++i) uuid[i + 6] = q2.bytes[i];

        for (int i = 0; i < 8; ++i) uuid[i + 8] = static_cast<uint8>(p3[i]);
    }
#else
...
{
#if JucePlugin_Enable_IAA && JucePlugin_Build_Standalone && JUCE_IOS && (!JUCE_USE_CUSTOM_PLUGIN_STANDALONE_APP)
    if (getPluginLoadedAs() == AudioProcessor::wrapperType_Standalone) return juce_isInterAppAudioConnected();
#endif
...
{
#if JucePlugin_Enable_IAA && JucePlugin_Build_Standalone && JUCE_IOS && (!JUCE_USE_CUSTOM_PLUGIN_STANDALONE_APP)
    if (getPluginLoadedAs() == AudioProcessor::wrapperType_Standalone) juce_switchToHostApplication();
#endif
}
...
#if JucePlugin_Enable_IAA && JucePlugin_Build_Standalone && JUCE_IOS && (!JUCE_USE_CUSTOM_PLUGIN_STANDALONE_APP)
        if (isInterAppAudioConnected()) return juce_getIAAHostIcon(size);
#endif
