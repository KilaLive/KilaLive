...
namespace juce
{
    OSCMessage::OSCMessage(const OSCAddressPattern& ap) noexcept
        : addressPattern(ap)
    {
    }
    ...
    void OSCMessage::addBlob(const MemoryBlock& blob)
    {
        arguments.add(OSCArgument(blob));
    }
    void OSCMessage::addArgument(OSCArgument arg)
    {
        arguments.add(arg);
    }
