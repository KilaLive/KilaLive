...
namespace juce
{
    namespace
    {
        ...
        */
        struct OSCOutputStream
        {
            ...
            {
                if (!output.writeString(value)) return false;
            ...
            bool writeTypeTagString(const OSCTypeList& typeList)
            {
                ...
                if (typeList.size() > 0) output.write(typeList.begin(), (size_t)typeList.size());
            ...
            {
                if (!writeAddressPattern(msg.getAddressPattern())) return false;

                OSCTypeList typeList;

                for (auto& arg : msg) typeList.add(arg.getType());

                if (!writeTypeTagString(typeList)) return false;

                for (auto& arg : msg)
                    if (!writeArgument(arg)) return false;
            ...
            {
                if (!writeString("#bundle")) return false;

                if (!writeTimeTag(bundle.getTimeTag())) return false;

                for (auto& element : bundle)
                    if (!writeBundleElement(element)) return false;
            ...
            bool writeBundleElement(const OSCBundle::Element& element)
            {
                ...
                if (element.isBundle())
                {
                    if (!writeBundle(element.getBundle())) return false;
                }
                else
                {
                    if (!writeMessage(element.getMessage())) return false;
                }
    ...
    struct OSCSender::Pimpl
    {
        ...
        bool send(const OSCMessage& message, const String& hostName, int portNumber)
        {
            ...
            return outStream.writeMessage(message) && sendOutputStream(outStream, hostName, portNumber);
        }
        ...
        bool send(const OSCBundle& bundle, const String& hostName, int portNumber)
        {
            ...
            return outStream.writeBundle(bundle) && sendOutputStream(outStream, hostName, portNumber);
        }
    ...
    private:
        ...
        bool sendOutputStream(OSCOutputStream& outStream, const String& hostName, int portNumber)
        {
            if (socket != nullptr)
            {
                ...
                const int bytesWritten = socket->write(hostName, portNumber, outStream.getData(), streamSize);
    ...
    bool OSCSender::sendToIPAddress(const String& host, int port, const OSCMessage& message)
    {
        return pimpl->send(message, host, port);
    }
    bool OSCSender::sendToIPAddress(const String& host, int port, const OSCBundle& bundle)
    {
...
//==============================================================================
    ...
    public:
        OSCBinaryWriterTests()
            : UnitTest("OSCBinaryWriter class", "OSC")
        {
        }
        ...
        void runTest()
        {
            ...
            beginTest("writing OSC arguments");
            {
                ...
                const uint8       testBlobRepresentation[] = {0x00, 0x00, 0x00, 0x05, 0xBB, 0xCC,
                                                        0xDD, 0xEE, 0xFF, 0x00, 0x00, 0x00};  // padded to size % 4 == 0
            ...
            {
                {
                    ...
                    uint8 check[52] = {'/',  't',  'e',  's',  't',  '\0', '\0', '\0', ',',  'i',  'f',  's',  'b',
            ...
            {
                {
                    ...
                    uint8 check[] = {
                        '#',  'b',  'u',  'n',  'd',  'l',  'e',  '\0', 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01,
    ...
    public:
        OSCRoundTripTests()
            : UnitTest("OSCRoundTripTests class", "OSC")
        {
        }
