...
namespace juce
{
    ...
    //==============================================================================
    OSCTimeTag::OSCTimeTag() noexcept
        : rawTimeTag(rawTimeTagRepresentingImmediately)
    {
    }

    OSCTimeTag::OSCTimeTag(uint64 t) noexcept
        : rawTimeTag(t)
    {
    }
    ...
    Time OSCTimeTag::toTime() const noexcept
    {
        ...
        const int64 juceTimeInMillis =
            int64((seconds * 1000) + (uint64)roundToInt(fractionalPartInMillis) - millisecondsBetweenOscAndJuceEpochs);
