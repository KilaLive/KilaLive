...
namespace juce
{
    namespace
    {
        ...
        public:
            ...
            {
                if (pattern == patternEnd) return matchTerminator(target, targetEnd);
        ...
        private:
            ...
            {
                if (target == targetEnd || c != target.getAndAdvance()) return false;
            ...
            {
                if (target == targetEnd) return false;
            ...
            {
                if (target == targetEnd) return pattern == patternEnd;

                if (match(pattern, patternEnd, target, targetEnd)) return true;
            ...
            {
                if (pattern == patternEnd) return false;
            ...
            //==============================================================================
            static bool matchStringSet(const StringArray& set, CharPtr pattern, CharPtr patternEnd, CharPtr target,
                                       CharPtr targetEnd)
            {
                if (set.size() == 0) return match(pattern, patternEnd, target, targetEnd);

                for (auto& str : set)
                    if (str.getCharPointer().compareUpTo(target, str.length()) == 0)
                        if (match(pattern, patternEnd, target + str.length(), targetEnd)) return true;

                return false;
            }

            //==============================================================================
            static bool matchInsideCharSet(CharPtr pattern, CharPtr patternEnd, CharPtr target, CharPtr targetEnd)
            {
                if (pattern == patternEnd) return false;
                ...
                while (pattern != patternEnd)
                {
                    ...
                    case '-':
                        if (!addCharRangeToSet(set, pattern, patternEnd, target, targetEnd)) return false;
            ...
            //==============================================================================
            static bool matchCharSet(const Array<juce_wchar>& set, bool setIsNegated, CharPtr pattern, CharPtr patternEnd,
            ...
            //==============================================================================
            static bool matchCharSetNegated(const Array<juce_wchar>& set, CharPtr pattern, CharPtr patternEnd, CharPtr target,
                                            CharPtr targetEnd)
            {
                for (auto c : set)
            ...
            //==============================================================================
            static bool matchCharSetNotNegated(const Array<juce_wchar>& set, CharPtr pattern, CharPtr patternEnd,
                                               CharPtr target, CharPtr targetEnd)
            {
                for (auto c : set)
                    if (*target == c)
                        if (match(pattern, patternEnd, target + 1, targetEnd)) return true;

                return false;
            }

            //==============================================================================
            static bool addCharRangeToSet(Array<juce_wchar>& set, CharPtr pattern, CharPtr /*patternEnd*/, CharPtr target,
                                          CharPtr targetEnd)
            {
                if (target == targetEnd) return false;
                ...
                if (rangeEnd == ',' || rangeEnd == '{' || rangeEnd == '}' || set.size() == 0) return false;

                while (rangeEnd > rangeStart) set.add(++rangeStart);
        ...
        {
            return OSCPatternMatcherImpl<String::CharPointerType>::match(
                pattern.getCharPointer(), pattern.getCharPointer().findTerminatingNull(), target.getCharPointer(),
        ...
        struct OSCAddressTokeniser
        {
            ...
            static bool containsOnlyAllowedPrintableASCIIChars(const String& string) noexcept
            {
                for (auto charPtr = string.getCharPointer(); !charPtr.isEmpty();)
                {
                    ...
                    if (!isPrintableASCIIChar(c) || isDisallowedChar(c)) return false;
                }
            ...
            {
                if (address.isEmpty()) throw OSCFormatError("OSC format error: address string cannot be empty.");
    ...
    {
        if (!containsWildcards()) return asString == address.asString;

        if (oscSymbols.size() != address.oscSymbols.size()) return false;

        for (int i = 0; i < oscSymbols.size(); ++i)
            if (!matchOscPattern(oscSymbols[i], address.oscSymbols[i])) return false;
...
//==============================================================================
    ...
    public:
        ...
        void runTest()
        {
            beginTest("construction and parsing");
            {
                ...
                expectThrowsType(OSCAddress(String::fromUTF8("/nonasciicharacter\xc3\xbc"
                                                             "blabla")),
                                 OSCFormatError);
    ...
    public:
        OSCAddressPatternTests()
            : UnitTest("OSCAddressPattern class", "OSC")
        {
        }
        ...
        void runTest()
        {
            beginTest("construction and parsing");
            {
                ...
                expectThrowsType(OSCAddressPattern(String::fromUTF8("/nonasciicharacter\xc3\xbc"
                                                                    "blabla")),
                                 OSCFormatError);
    ...
    public:
        OSCPatternMatcherTests()
            : UnitTest("OSCAddress class / pattern matching", "OSC")
        {
        }
