...
namespace juce
{
    AudioFormatWriter::AudioFormatWriter(OutputStream* const out, const String& formatName_, const double rate,
                                         const unsigned int numChannels_, const unsigned int bitsPerSample_)
        : sampleRate(rate)
    ...
    AudioFormatWriter::AudioFormatWriter(OutputStream* const out, const String& formatName_, const double rate,
                                         const AudioChannelSet& channelLayout_, const unsigned int bitsPerSample_)
        : sampleRate(rate)
    ...
    bool AudioFormatWriter::writeFromAudioReader(AudioFormatReader& reader, int64 startSample, int64 numSamplesToRead)
    {
        ...
        while (numSamplesToRead > 0)
        {
            ...
            if (!reader.read(buffers, (int)numChannels, startSample, numToDo, false)) return false;
            ...
            if (!write(const_cast<const int**>(buffers), numToDo)) return false;
    ...
    bool AudioFormatWriter::writeFromAudioSource(AudioSource& source, int numSamplesToRead, const int samplesPerBlock)
    {
        ...
        while (numSamplesToRead > 0)
        {
            ...
            if (!writeFromAudioSampleBuffer(tempBuffer, 0, numToDo)) return false;
    ...
    bool AudioFormatWriter::writeFromFloatArrays(const float* const* channels, int numSourceChannels, int numSamples)
    {
        ...
        if (isFloatingPoint()) return write((const int**)channels, numSamples);
        ...
        for (int i = 0; i < numSourceChannels; ++i) chans[i] = scratch + (i * maxSamples);
        ...
        while (numSamples > 0)
        {
            ...
            for (int i = 0; i < numSourceChannels; ++i) convertFloatsToInts(chans[i], channels[i] + startSample, numToDo);

            if (!write((const int**)chans, numToDo)) return false;
    ...
    bool AudioFormatWriter::writeFromAudioSampleBuffer(const AudioBuffer<float>& source, int startSample, int numSamples)
    {
        ...
        if (startSample == 0) return writeFromFloatArrays(source.getArrayOfReadPointers(), numSourceChannels, numSamples);

        const float* chans[256];
        jassert((int)numChannels < numElementsInArray(chans));

        for (int i = 0; i < numSourceChannels; ++i) chans[i] = source.getReadPointer(i, startSample);
    ...
    public:
        ...
        {
            if (numSamples <= 0 || !isRunning) return true;
            ...
            if (size1 + size2 < numSamples) return false;
        ...
        int writePendingData()
        {
            ...
            if (size1 <= 0) return 10;

            writer->writeFromAudioSampleBuffer(buffer, start1, size1);

            const ScopedLock sl(thumbnailLock);
            if (receiver != nullptr) receiver->addBlock(samplesWritten, buffer, start1, size1);
            ...
            if (size2 > 0)
            {
                ...
                if (receiver != nullptr) receiver->addBlock(samplesWritten, buffer, start2, size2);
        ...
        {
            if (newReceiver != nullptr) newReceiver->reset(buffer.getNumChannels(), writer->getSampleRate(), 0);
    ...
    AudioFormatWriter::ThreadedWriter::ThreadedWriter(AudioFormatWriter* writer, TimeSliceThread& backgroundThread,
                                                      int numSamplesToBuffer)
        : buffer(new AudioFormatWriter::ThreadedWriter::Buffer(backgroundThread, writer, (int)writer->numChannels,
                                                               numSamplesToBuffer))
    {
    }
