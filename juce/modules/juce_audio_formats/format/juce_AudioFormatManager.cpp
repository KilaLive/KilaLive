...
namespace juce
{
    ...
    void AudioFormatManager::registerFormat(AudioFormat* newFormat, bool makeThisTheDefaultFormat)
    {
        ...
        if (newFormat != nullptr)
        {
            ...
            {
                if (af->getFormatName() == newFormat->getFormatName()) jassertfalse;  // trying to add the same format twice!
            }
#endif

            if (makeThisTheDefaultFormat) defaultFormatIndex = getNumKnownFormats();
    ...
    {
        if (!fileExtension.startsWithChar('.')) return findFormatForFileExtension("." + fileExtension);

        for (auto* af : knownFormats)
            if (af->getFileExtensions().contains(fileExtension, true)) return af;
    ...
    String AudioFormatManager::getWildcardForAllFormats() const
    {
        ...
        for (auto* af : knownFormats) extensions.addArray(af->getFileExtensions());

        extensions.trim();
        extensions.removeEmptyStrings();

        for (auto& e : extensions) e = (e.startsWithChar('.') ? "*" : "*.") + e;
        ...
        for (auto* af : knownFormats)
            if (af->canHandleFile(file))
                if (auto* in = file.createInputStream())
                    if (auto* r = af->createReaderFor(in, true)) return r;
