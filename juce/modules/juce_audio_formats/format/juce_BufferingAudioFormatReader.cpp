...
namespace juce
{
    BufferingAudioReader::BufferingAudioReader(AudioFormatReader* sourceReader, TimeSliceThread& timeSliceThread,
                                               int samplesToBuffer)
        ...
        , timeoutMs(0)
    {
        ...
        for (int i = 3; --i >= 0;) readNextBufferChunk();
    ...
    BufferingAudioReader::BufferedBlock* BufferingAudioReader::getBlockContaining(int64 pos) const noexcept
    {
        for (int i = blocks.size(); --i >= 0;)
        {
            ...
            if (b->range.contains(pos)) return b;
        }
    ...
    bool BufferingAudioReader::readNextBufferChunk()
    {
        ...
        for (int i = blocks.size(); --i >= 0;) newBlocks.removeObject(blocks.getUnchecked(i), false);
