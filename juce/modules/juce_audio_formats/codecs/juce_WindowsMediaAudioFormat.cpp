    ...
    namespace WindowsMediaCodec
    {
        ...
        public:
            ...
            JUCE_COMRESULT Read(void* dest, ULONG numBytes, ULONG* bytesRead)
            {
                ...
                if (bytesRead != nullptr) *bytesRead = numRead;
            ...
            JUCE_COMRESULT Seek(LARGE_INTEGER position, DWORD origin, ULARGE_INTEGER* resultPosition)
            {
                ...
                else if (origin == STREAM_SEEK_END)
                {
                    ...
                    if (len < 0) return E_NOTIMPL;

                    newPos += len;
                }

                if (resultPosition != nullptr) resultPosition->QuadPart = newPos;

                return source.setPosition(newPos) ? S_OK : E_NOTIMPL;
            }

            JUCE_COMRESULT CopyTo(IStream* destStream, ULARGE_INTEGER numBytesToDo, ULARGE_INTEGER* bytesRead,
        ...
        public:
            ...
            {
                JUCE_LOAD_WINAPI_FUNCTION(wmvCoreLib, WMCreateSyncReader, wmCreateSyncReader, HRESULT,
                ...
                if (wmCreateSyncReader != nullptr)
                {
                    ...
                    if (SUCCEEDED(hr)) hr = wmSyncReader->OpenStream(new JuceIStream(*input));
            ...
            ~WMAudioReader()
            {
                if (wmSyncReader != nullptr) wmSyncReader->Close();
            }
                             ...
                             int numSamples) override
            {
                ...
                while (numSamples > 0)
                {
                    if (!bufferedRange.contains(startSampleInFile))
                    {
                        ...
                        if (hasJumped) wmSyncReader->SetRange((QWORD)(startSampleInFile * 10000000 / (int64)sampleRate), 0);
                        ...
                        if (sampleBuffer != nullptr)
                        {
                            ...
                            if (hasJumped)
                                ...
                                bufferedRange.setStart(
                                    bufferedRange.getEnd());  // (because the positions returned often aren't continguous)
