...
#undef min
    ...
    public:
        ...
        {
            if (auto* value = vorbis_comment_query(comment, name, 0)) metadataValues.set(metadataName, value);
        }
                         ...
                         int numSamples) override
        {
            while (numSamples > 0)
            {
                    ...
                    || startSampleInFile + numSamples > reservoirStart + samplesInReservoir)
                {
                    ...
                    if (reservoirStart != (int)ov_pcm_tell(&ovFile)) ov_pcm_seek(&ovFile, reservoirStart);
                    ...
                    if (numToRead > 0) reservoir.clear(offset, numToRead);
                }
            }
    ...
    {
    public:
        OggWriter(OutputStream* out, double rate, unsigned int numChans, unsigned int bitsPerSamp, int qualityIndex,
                  const StringPairArray& metadata)
            : AudioFormatWriter(out, oggFormatName, rate, numChans, bitsPerSamp)
        {
            vorbis_info_init(&vi);

            if (vorbis_encode_init_vbr(&vi, (int)numChans, (int)rate, jlimit(0.0f, 1.0f, qualityIndex * 0.1f)) == 0)
            {
                ...
                for (;;)
                {
                    if (ogg_stream_flush(&os, &og) == 0) break;
                ...
                if (numSamples > 0)
                {
                    ...
                    for (int i = (int)numChannels; --i >= 0;)
                    {
                        if (auto* dst = vorbisBuffer[i])
                        {
                            ...
                            {
                                for (int j = 0; j < numSamples; ++j) dst[j] = (float)(src[j] * gain);
                            }
                        }
                    }
                }
    ...
    private:
        ...
        void addMetadata(const StringPairArray& metadata, const char* name, const char* vorbisName)
        {
            ...
            if (s.isNotEmpty()) vorbis_comment_add_tag(&vc, vorbisName, const_cast<char*>(s.toRawUTF8()));
        }
    ...
    {
        return {8000, 11025, 12000, 16000, 22050, 32000, 44100, 48000, 88200, 96000, 176400, 192000};
    }
    ...
    AudioFormatWriter* OggVorbisAudioFormat::createWriterFor(OutputStream* out, double sampleRate, unsigned int numChannels,
                                                             int bitsPerSample, const StringPairArray& metadataValues,
                                                             int qualityOptionIndex)
    {
        if (out == nullptr) return nullptr;

        ScopedPointer<OggWriter> w(
            new OggWriter(out, sampleRate, numChannels, (unsigned int)bitsPerSample, qualityOptionIndex, metadataValues));
