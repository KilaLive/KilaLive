...
#pragma pack(push, 1)
        ...
        struct BASCChunk
        {
            ...
            void addToMetadata(StringPairArray& metadata) const
            {
                ...
                if (rootNoteSet) metadata.set(AiffAudioFormat::appleRootNote, String(rootNote));
                ...
                if (keyString != nullptr) metadata.set(AiffAudioFormat::appleKey, keyString);
            }
...
#pragma pack(pop)
        ...
        namespace CATEChunk
        {
            ...
            {
                static const char* appleGenres[] = {"Rock/Blues",   "Electronic/Dance",  "Jazz",       "Urban",
                ...
                for (int i = 0; i < numElementsInArray(appleGenres); ++i)
                    if (tag == appleGenres[i]) return true;
        ...
        namespace MarkChunk
        {
            static bool metaDataContainsZeroIdentifiers(const StringPairArray& values)
            {
                ...
                {
                    if (key.startsWith(noteString)) continue;  // zero identifier IS valid in a COMT chunk

                    if (key.startsWith(cueString) && key.contains(identifierString))
                        if (values.getValue(key, "-1").getIntValue() == 0) return true;
                }
            ...
            static void create(MemoryBlock& block, const StringPairArray& values)
            {
                ...
                if (numCues > 0)
                {
                    ...
                    for (int i = 0; i < numCues; ++i)
                    {
                        ...
                        auto labelLength =
                            jmin((size_t)254,
                                 label.getNumBytesAsUTF8());  // seems to need null terminator even though it's a pstring
    ...
    public:
            ...
            : AudioFormatReader(in, aiffFormatName)
        {
                ...
                if (nextType == chunkName("AIFF") || nextType == chunkName("AIFC"))
                {
                    ...
                    while (input->getPosition() < end)
                    {
                        ...
                        if (type == chunkName("FVER"))
                        {
                            ...
                            if (ver != 0 && ver != (int)0xa2805140) break;
                        }
                        ...
                        else if (type == chunkName("SSND"))
                        {
                            ...
                            lengthInSamples =
                                (bytesPerFrame > 0) ? jmin(lengthInSamples, ((int64)length) / (int64)bytesPerFrame) : 0;
                        }
                        else if (type == chunkName("MARK"))
                        {
                            ...
                            for (uint16 i = 0; i < numCues; ++i)
                            {
                                ...
                                if ((stringLength & 1) == 0) input->readByte();
                        ...
                        {
                            metadataValues.set(AiffAudioFormat::appleTag, AiffFileHelpers::CATEChunk::read(*input, length));
                        }
                        else if ((hasGotVer && hasGotData && hasGotType) || chunkEnd < input->getPosition()
                                 || input->isExhausted())
                        {
                            break;
                        }
            ...
            if (metadataValues.size() > 0) metadataValues.set("MetaDataSource", "AIFF");
        }
                         ...
                         int numSamples) override
        {
            ...
            while (numSamples > 0)
            {
                ...
                if (littleEndian)
                    copySampleData<AudioData::LittleEndian>(bitsPerSample, usesFloatingPointData, destSamples,
                                                            startOffsetInDestBuffer, numDestChannels, tempBuffer,
                                                            (int)numChannels, numThisTime);
                else
                    copySampleData<AudioData::BigEndian>(bitsPerSample, usesFloatingPointData, destSamples,
                                                         startOffsetInDestBuffer, numDestChannels, tempBuffer,
                                                         (int)numChannels, numThisTime);
        ...
        static void copySampleData(unsigned int bitsPerSample, bool usesFloatingPointData, int* const* destSamples,
    ...
    {
    public:
        AiffAudioFormatWriter(OutputStream* out, double rate, unsigned int numChans, unsigned int bits,
        ...
        {
            if ((bytesWritten & 1) != 0) output->writeByte(0);
        ...
        bool write(const int** data, int numSamples) override
        {
            ...
            {
            case 8:
                WriteHelper<AudioData::Int8, AudioData::Int32, AudioData::BigEndian>::write(
                    tempBlock.getData(), (int)numChannels, data, numSamples);
                break;
            case 16:
                WriteHelper<AudioData::Int16, AudioData::Int32, AudioData::BigEndian>::write(
                    tempBlock.getData(), (int)numChannels, data, numSamples);
                break;
            case 24:
                WriteHelper<AudioData::Int24, AudioData::Int32, AudioData::BigEndian>::write(
                    tempBlock.getData(), (int)numChannels, data, numSamples);
                break;
            case 32:
                WriteHelper<AudioData::Int32, AudioData::Int32, AudioData::BigEndian>::write(
                    tempBlock.getData(), (int)numChannels, data, numSamples);
                break;
            default:
                jassertfalse;
            ...
            if (bytesWritten + bytes >= (size_t)0xfff00000 || !output->write(tempBlock.getData(), bytes))
            {
    ...
    public:
        MemoryMappedAiffReader(const File& f, const AiffAudioFormatReader& reader)
            : MemoryMappedAudioFormatReader(f, reader, reader.dataChunkStart, reader.bytesPerFrame * reader.lengthInSamples,
                         ...
                         int numSamples) override
        {
            ...
            if (littleEndian)
                AiffAudioFormatReader::copySampleData<AudioData::LittleEndian>(
                    bitsPerSample, usesFloatingPointData, destSamples, startOffsetInDestBuffer, numDestChannels,
        ...
        void getSample(int64 sample, float* result) const noexcept override
        {
            ...
            if (littleEndian)
            {
                ...
                {
                case 8:
                    ReadHelper<AudioData::Float32, AudioData::UInt8, AudioData::LittleEndian>::read(dest, 0, 1, source, 1,
                                                                                                    num);
                    break;
                case 16:
                    ReadHelper<AudioData::Float32, AudioData::Int16, AudioData::LittleEndian>::read(dest, 0, 1, source, 1,
                                                                                                    num);
                    break;
                case 24:
                    ReadHelper<AudioData::Float32, AudioData::Int24, AudioData::LittleEndian>::read(dest, 0, 1, source, 1,
                                                                                                    num);
                    break;
                case 32:
                    if (usesFloatingPointData)
                        ReadHelper<AudioData::Float32, AudioData::Float32, AudioData::LittleEndian>::read(dest, 0, 1, source,
                                                                                                          1, num);
                    else
            ...
            else
            {
                ...
                {
                case 8:
                    ReadHelper<AudioData::Float32, AudioData::UInt8, AudioData::BigEndian>::read(dest, 0, 1, source, 1, num);
                    break;
                case 16:
                    ReadHelper<AudioData::Float32, AudioData::Int16, AudioData::BigEndian>::read(dest, 0, 1, source, 1, num);
                    break;
                case 24:
                    ReadHelper<AudioData::Float32, AudioData::Int24, AudioData::BigEndian>::read(dest, 0, 1, source, 1, num);
                    break;
                case 32:
                    if (usesFloatingPointData)
                        ReadHelper<AudioData::Float32, AudioData::Float32, AudioData::BigEndian>::read(dest, 0, 1, source, 1,
                                                                                                       num);
                    else
                        ReadHelper<AudioData::Float32, AudioData::Int32, AudioData::BigEndian>::read(dest, 0, 1, source, 1,
                                                                                                     num);
                    break;
                default:
                    jassertfalse;
        ...
        void readMaxLevels(int64 startSampleInFile, int64 numSamples, Range<float>* results, int numChannelsToRead) override
        {
                ...
                || !mappedSection.contains(Range<int64>(startSampleInFile, startSampleInFile + numSamples)))
            {
                ...
                for (int i = 0; i < numChannelsToRead; ++i) results[i] = Range<float>();
    ...
    private:
        ...
        {
            return littleEndian
                       ? scanMinAndMaxInterleaved<SampleType, AudioData::LittleEndian>(channel, startSampleInFile, numSamples)
    ...
    {
        if (AudioFormat::canHandleFile(f)) return true;
    ...
    AudioFormatReader* AiffAudioFormat::createReaderFor(InputStream* sourceStream, bool deleteStreamIfOpeningFails)
    {
        ...
        if (w->sampleRate > 0 && w->numChannels > 0) return w.release();

        if (!deleteStreamIfOpeningFails) w->input = nullptr;
    ...
    MemoryMappedAudioFormatReader* AiffAudioFormat::createMemoryMappedReader(FileInputStream* fin)
    {
        if (fin != nullptr)
        {
            ...
            if (reader.lengthInSamples > 0) return new MemoryMappedAiffReader(fin->getFile(), reader);
        }

        return nullptr;
    }

    AudioFormatWriter* AiffAudioFormat::createWriterFor(OutputStream* out, double sampleRate, unsigned int numberOfChannels,
