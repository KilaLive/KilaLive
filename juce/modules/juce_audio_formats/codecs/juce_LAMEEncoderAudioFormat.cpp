...
namespace juce
{
    ...
    {
    public:
        Writer(OutputStream* destStream, const String& formatName, const File& appFile, int vbr, int cbr, double sampleRate,
               unsigned int numberOfChannels, int bitsPerSample, const StringPairArray& metadata)
            : AudioFormatWriter(destStream, formatName, sampleRate, numberOfChannels, (unsigned int)bitsPerSample)
            , vbrLevel(vbr)
            , cbrBitrate(cbr)
        {
            ...
            {
                writer = wavFormat.createWriterFor(out, sampleRate, numChannels, bitsPerSample, metadata, 0);
    ...
    StringArray LAMEEncoderAudioFormat::getQualityOptions()
    {
        ...
        for (int i = 0; i < numElementsInArray(cbrRates); ++i) opts.add(String(cbrRates[i]) + " Kb/s CBR");
    ...
    AudioFormatWriter* LAMEEncoderAudioFormat::createWriterFor(OutputStream* streamToWriteTo, double sampleRateToUse,
                                                               unsigned int numberOfChannels, int bitsPerSample,
                                                               const StringPairArray& metadataValues, int qualityOptionIndex)
    {
        if (streamToWriteTo == nullptr) return nullptr;
        ...
        return new Writer(streamToWriteTo, getFormatName(), lameApp, vbr, cbr, sampleRateToUse, numberOfChannels,
