...
namespace juce
{
    ...
    struct CoreAudioFormatMetatdata
    {
        ...
        static void findTempoEvents(MidiFile& midiFile, StringPairArray& midiMetadata)
        {
            ...
            for (int i = 0; i < numTempoEvents; ++i)
            {
                ...
                if (tempo > 0.0)
                {
                    if (i == 0) midiMetadata.set(CoreAudioFormat::tempo, String(tempo));

                    if (numTempoEvents > 1) tempoSequence << String(tempo) << ',' << tempoEvents.getEventTime(i) << ';';
                }
            }

            if (tempoSequence.getDataSize() > 0) midiMetadata.set("tempo sequence", tempoSequence.toUTF8());
        }
        ...
        static double getTempoFromTempoMetaEvent(MidiMessageSequence::MidiEventHolder* holder)
        {
            if (holder != nullptr)
            {
                ...
                if (midiMessage.isTempoMetaEvent())
                {
                    ...
                    if (tempoSecondsPerQuarterNote > 0.0) return 60.0 / tempoSecondsPerQuarterNote;
                }
            }
        ...
        static void findTimeSigEvents(MidiFile& midiFile, StringPairArray& midiMetadata)
        {
            ...
            for (int i = 0; i < numTimeSigEvents; ++i)
            {
                ...
                if (i == 0) midiMetadata.set(CoreAudioFormat::timeSig, timeSigString);

                if (numTimeSigEvents > 1) timeSigSequence << timeSigString << ',' << timeSigEvents.getEventTime(i) << ';';
            }

            if (timeSigSequence.getDataSize() > 0) midiMetadata.set("time signature sequence", timeSigSequence.toUTF8());
        }
        ...
        static void findKeySigEvents(MidiFile& midiFile, StringPairArray& midiMetadata)
        {
            ...
            for (int i = 0; i < numKeySigEvents; ++i)
            {
                ...
                static const char* majorKeys[] = {"Cb", "Gb", "Db", "Ab", "Eb", "Bb", "F", "C",
                                                  "G",  "D",  "A",  "E",  "B",  "F#", "C#"};
                static const char* minorKeys[] = {"Ab", "Eb", "Bb", "F",  "C",  "G",  "D", "A",
                                                  "E",  "B",  "F#", "C#", "G#", "D#", "A#"};

                String keySigString(isMajor ? majorKeys[key] : minorKeys[key]);

                if (!isMajor) keySigString << 'm';

                if (i == 0) midiMetadata.set(CoreAudioFormat::keySig, keySigString);

                if (numKeySigEvents > 1) keySigSequence << keySigString << ',' << keySigEvents.getEventTime(i) << ';';
            }

            if (keySigSequence.getDataSize() > 0) midiMetadata.set("key signature sequence", keySigSequence.toUTF8());
        }
        ...
        static StringPairArray parseInformationChunk(InputStream& input)
        {
            ...
            for (uint32 i = 0; i < numEntries; ++i) infoStrings.set(input.readString(), input.readString());
            ...
            if (isCafFile)
            {
                while (!input.isExhausted())
                {
                    ...
                    else if (chunkHeader.chunkType == chunkName("data"))
                    {
                        ...
                        if (chunkHeader.chunkSize == -1) break;
    ...
    {
    public:
        CoreAudioReader(InputStream* inp)
            : AudioFormatReader(inp, coreAudioFormatName)
        {
            usesFloatingPointData = true;
            bitsPerSample         = 32;

            if (input != nullptr) CoreAudioFormatMetatdata::read(*input, metadataValues);

            OSStatus status = AudioFileOpenWithCallbacks(this, &readCallback,
            ...
            if (status == noErr)
            {
                ...
                if (status == noErr)
                {
                    ...
                    ExtAudioFileGetProperty(audioFileRef, kExtAudioFileProperty_FileDataFormat,
                    ...
                    ExtAudioFileGetProperty(audioFileRef, kExtAudioFileProperty_FileLengthFrames, &sizeOfLengthProperty,
                                            &lengthInSamples);
                    ...
                    status =
                        AudioFileGetPropertyInfo(audioFileID, kAudioFilePropertyChannelLayout, &sizeOfLayout, &isWritable);
                    ...
                    if (status == noErr)
                    {
                        ...
                        status =
                            AudioFileGetProperty(audioFileID, kAudioFilePropertyChannelLayout, &sizeOfLayout, caLayout.get());
                    ...
                    destinationAudioFormat.mBytesPerPacket =
                        destinationAudioFormat.mFramesPerPacket * destinationAudioFormat.mBytesPerFrame;

                    status = ExtAudioFileSetProperty(audioFileRef, kExtAudioFileProperty_ClientDataFormat,
                    ...
                    if (status == noErr)
                    {
                        ...
                        {
                            for (int i = 0; i < static_cast<int>(numChannels); ++i) channelMap[i] = i;
                        }
    ...
    private:
        ...
        static OSStatus readCallback(void* inClientData, SInt64 inPosition, UInt32 requestCount, void* buffer,
                                     UInt32* actualCount)
        {
    ...
    //==============================================================================
    AudioFormatReader* CoreAudioFormat::createReaderFor(InputStream* sourceStream, bool deleteStreamIfOpeningFails)
    {
        ScopedPointer<CoreAudioReader> r(new CoreAudioReader(sourceStream));

        if (r->ok) return r.release();
    ...
    AudioFormatWriter* CoreAudioFormat::createWriterFor(OutputStream*, double /*sampleRateToUse*/,
...
#define DEFINE_CHANNEL_LAYOUT_TAG_ENTRY(x, y)                                                                                \
    ...
    public:
        CoreAudioLayoutsUnitTest()
            : UnitTest("Core Audio Layout <-> JUCE channel layout conversion", "Audio")
        {
        }
        ...
        void runTest() override
        {
...

            {
                ...
                for (auto tagEntry : knownTags)
                {
                    ...
                    expect(labels.size() == (tagEntry.tag & 0xffff),
                           "Tag \"" + String(tagEntry.name) + "\" has incorrect channel count");
                }
            }
...

            {
                ...
                for (auto tagEntry : knownTags)
                {
                    if (tagEntry.equivalentChannelSet.isDisabled()) continue;
    ...
    private:
        ...
        const Array<CoreAudioChannelLayoutTag>& getAllKnownLayoutTags() const
        {
            static CoreAudioChannelLayoutTag tags[] = {
                ...
                DEFINE_CHANNEL_LAYOUT_TAG_ENTRY(kAudioChannelLayoutTag_AudioUnit_7_0_Front,
                ...
                // DEFINE_CHANNEL_LAYOUT_DFL_ENTRY (kAudioChannelLayoutTag_TMH_10_2_full), no indication on how to handle this
                // tag
