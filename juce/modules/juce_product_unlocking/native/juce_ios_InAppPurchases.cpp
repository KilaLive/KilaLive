...
{
    template <>
    struct ContainerDeletePolicy<SKProductsRequest>
    {
    ...
    template <>
    struct ContainerDeletePolicy<SKReceiptRefreshRequest>
    {
        static void destroy(NSObject* o)
        {
            [o release];
    ...
    template <>
    struct ContainerDeletePolicy<NSObject<SKProductsRequestDelegate, SKPaymentTransactionObserver>>
    {
        static void destroy(NSObject* o)
        {
            [o release];
    ...
    private:
        struct Class : public ObjCClass<NSObject<SKProductsRequestDelegate, SKPaymentTransactionObserver>>
        {
            //==============================================================================
            Class()
                : ObjCClass<NSObject<SKProductsRequestDelegate, SKPaymentTransactionObserver>>(
                      "SKDelegateAndPaymentObserverBase_")
            {
                ...
                addMethod(@selector(paymentQueue:restoreCompletedTransactionsFailedWithError:),
                          restoreCompletedTransactionsFailedWithError, "v@:@@");
            ...
            //==============================================================================
            static SKDelegateAndPaymentObserver& getThis(id self)
            {
                return *getIvar<SKDelegateAndPaymentObserver*>(self, "self");
            }
            static void setThis(id self, SKDelegateAndPaymentObserver* s)
            {
                object_setInstanceVariable(self, "self", s);
            }

            //==============================================================================
            static void didReceiveResponse(id self, SEL, SKProductsRequest* request, SKProductsResponse* response)
            {
                getThis(self).didReceiveResponse(request, response);
            }
            static void requestDidFinish(id self, SEL, SKRequest* request)
            {
                getThis(self).requestDidFinish(request);
            }
            static void requestDidFailWithError(id self, SEL, SKRequest* request, NSError* err)
            {
                getThis(self).requestDidFailWithError(request, err);
            }
            static void updatedTransactions(id self, SEL, SKPaymentQueue* queue, NSArray<SKPaymentTransaction*>* trans)
            {
                getThis(self).updatedTransactions(queue, trans);
            }
            static void restoreCompletedTransactionsFailedWithError(id self, SEL, SKPaymentQueue* q, NSError* err)
            {
                getThis(self).restoreCompletedTransactionsFailedWithError(q, err);
            }
            static void restoreCompletedTransactionsFinished(id self, SEL, SKPaymentQueue* queue)
            {
                getThis(self).restoreCompletedTransactionsFinished(queue);
            }
            static void updatedDownloads(id self, SEL, SKPaymentQueue* queue, NSArray<SKDownload*>* downloads)
            {
    ...
    struct InAppPurchases::Pimpl : public SKDelegateAndPaymentObserver
    {
        ...
        struct DownloadImpl : public Download
        {
            ...
            String getProductId() const override
            {
                return nsStringToJuce(download.contentIdentifier);
            }
            String getContentVersion() const override
            {
                return nsStringToJuce(download.contentVersion);
            }
        ...
        {
            PendingDownloadsTransaction(SKPaymentTransaction* t)
                : transaction(t)
            {
            ...
            {
                for (SKDownload* download in transactionToUse.downloads) downloads.add(new DownloadImpl(download));
            }
...
#endif
                    if (state != SKDownloadStateFinished && state != SKDownloadStateFailed
        ...
        {
            auto* productsRequest = [[SKProductsRequest alloc]
                initWithProductIdentifiers:[NSSet setWithArray:createNSArrayFromStringArray(productIdentifiers)]];

            pendingProductInfoRequests.add(
                new PendingProductInfoRequest{PendingProductInfoRequest::Type::query, productsRequest});
        ...
        void purchaseProduct(const String& productIdentifier, bool, const StringArray&, bool)
        {
            if (![SKPaymentQueue canMakePayments])
            {
                owner.listeners.call(
                    [&](Listener& l) { l.productPurchaseFinished({}, false, NEEDS_TRANS("Payments not allowed")); });
                return;
            }
            ...
            pendingProductInfoRequests.add(
                new PendingProductInfoRequest{PendingProductInfoRequest::Type::purchase, productsRequest});
        ...
        void restoreProductsBoughtList(bool includeDownloadInfo, const String& subscriptionsSharedSecret)
        {
            ...
            else
            {
                ...
                pendingReceiptRefreshRequests.add(
                    new PendingReceiptRefreshRequest{subscriptionsSharedSecret, [receiptRequest retain]});
        ...
        void didReceiveResponse(SKProductsRequest* request, SKProductsResponse* response) override
        {
            for (auto i = 0; i < pendingProductInfoRequests.size(); ++i)
            {
                ...
                {
                    if (pendingRequest.type == PendingProductInfoRequest::Type::query)
                        notifyProductsInfoReceived(response.products);
                    else if (pendingRequest.type == PendingProductInfoRequest::Type::purchase)
                        startPurchase(response.products);
                    else
                        break;
        ...
        void updatedTransactions(SKPaymentQueue*, NSArray<SKPaymentTransaction*>* transactions) override
        {
            for (SKPaymentTransaction* transaction in transactions)
            {
                ...
                case SKPaymentTransactionStateFailed:
                    processTransactionFinish(transaction, false);
                    break;
                case SKPaymentTransactionStatePurchased:
                    processTransactionFinish(transaction, true);
                    break;
                ...
                default:
                    jassertfalse;
                    break;  // Unexpected transaction state
                }
            }
        }
        ...
        {
            owner.listeners.call(
                [&](Listener& l) { l.purchasesListRestored({}, false, nsStringToJuce(error.localizedDescription)); });
        }

        void restoreCompletedTransactionsFinished(SKPaymentQueue*) override
        {
            owner.listeners.call(
                [this](Listener& l) { l.purchasesListRestored(restoredPurchases, true, NEEDS_TRANS("Success")); });
            ...
            for (SKDownload* download in downloads)
            {
                if (auto* pendingDownload = getPendingDownloadFor(download))
                {
                    ...
                    case SKDownloadStatePaused:
                        owner.listeners.call([&](Listener& l) { l.productDownloadPaused(*pendingDownload); });
                        break;
                    case SKDownloadStateActive:
                        owner.listeners.call([&](Listener& l) {
                            l.productDownloadProgressUpdate(*pendingDownload, download.progress,
                    ...
                    case SKDownloadStateCancelled:
                        processDownloadFinish(pendingDownload, download);
                        break;

                    default:
                        jassertfalse;
                        break;  // Unexpected download state
                    }
                }
            }
        }
        ...
        void notifyProductsInfoReceived(NSArray<SKProduct*>* products)
        {
            ...
            for (SKProduct* skProduct in products) productsToReturn.add(SKProductToIAPProduct(skProduct));
        ...
        void startPurchase(NSArray<SKProduct*>* products)
        {
            ...
            {
                owner.listeners.call([](Listener& l) {
                    l.productPurchaseFinished({}, false, NEEDS_TRANS("Your app is not setup for payments"));
                });
            }
        }
        ...
        Array<Download*> removeInvalidDownloads(const Array<Download*>& downloadsToUse)
        {
            ...
            for (int i = downloads.size(); --i >= 0;)
            {
                ...
                if (!hasPendingDownload) downloads.remove(i);
            }
        ...
        bool hasDownloadInPendingDownloadsTransaction(const Download& download)
        {
            for (auto* pdt : pendingDownloadsTransactions)
                for (auto* pendingDownload : pdt->downloads)
                    if (pendingDownload == &download) return true;
        ...
        void processTransactionFinish(SKPaymentTransaction* transaction, bool success)
        {
            ...
            auto purchaseTime =
                Time(1000 * (int64)transaction.transactionDate.timeIntervalSince1970).toString(true, true, true, true);
            ...
            if (transaction.transactionState == SKPaymentTransactionStateFailed || transaction.downloads == nil ||
            ...
            else
            {
                ...
                for (auto* download : pdt->downloads) downloads.add(download);
            }
            ...
            if (transaction.transactionState == SKPaymentTransactionStateRestored)
                ...
                owner.listeners.call([&](Listener& l) {
                    l.productPurchaseFinished({purchase, downloads}, success,
        ...
        PendingDownloadsTransaction* getPendingDownloadsTransactionSKDownloadFor(SKDownload* download)
        {
            for (auto* pdt : pendingDownloadsTransactions)
                for (auto* pendingDownload : pdt->downloads)
                    if (pendingDownload->download == download) return pdt;
        ...
        Download* getPendingDownloadFor(SKDownload* download)
        {
            if (auto* pdt = getPendingDownloadsTransactionSKDownloadFor(download))
                for (auto* pendingDownload : pdt->downloads)
                    if (pendingDownload->download == download) return pendingDownload;
        ...
        void processDownloadFinish(Download* pendingDownload, SKDownload* download)
        {
            if (auto* pdt = getPendingDownloadsTransactionSKDownloadFor(download))
            {
                ...
                auto contentURL =
                    state == SKDownloadStateFinished ? URL(nsStringToJuce(download.contentURL.absoluteString)) : URL();
        ...
        void processReceiptRefreshResponseWithSubscriptionsSharedSecret(const String& secret)
        {
            ...
            if (auto* receiptData = [NSData dataWithContentsOfURL:receiptURL])
                ...
                owner.listeners.call(
                    [&](Listener& l) { l.purchasesListRestored({}, false, NEEDS_TRANS("Receipt fetch failed")); });
        }
        ...
        void fetchReceiptDetailsFromAppStore(NSData* receiptData, const String& secret)
        {
            ...
            if (secret.isNotEmpty()) [requestContents setObject:juceStringToNS(secret) forKey:nsStringLiteral("password")];

            NSError* error;
            auto*    requestData = [NSJSONSerialization dataWithJSONObject:requestContents options:0 error:&error];
            if (requestData == nil)
            {
            ...
            auto* task = [[NSURLSession sharedSession]
                dataTaskWithRequest:storeRequest
                  completionHandler:^(NSData* data, NSURLResponse*, NSError* connectionError) {
                    ...
                    if (auto inAppPurchases = getAs<NSArray>(receipt[nsStringLiteral("in_app")]))
                    {
                        ...
                        for (id inAppPurchaseData in inAppPurchases)
                        {
                            if (auto* purchaseData = getAs<NSDictionary>(inAppPurchaseData))
                            {
                                ...
                                if (purchaseData[nsStringLiteral("cancellation_date")] != nil) continue;

                                if (auto transactionId =
                                        getAs<NSString>(purchaseData[nsStringLiteral("original_transaction_id")]))
                                {
                                    if (auto productId = getAs<NSString>(purchaseData[nsStringLiteral("product_id")]))
                                    {
                                        auto purchaseTime =
                                            getPurchaseDateMs(purchaseData[nsStringLiteral("purchase_date_ms")]);
                        ...
                        MessageManager::callAsync([this, purchases] {
                            owner.listeners.call(
                                [&](Listener& l) { l.purchasesListRestored(purchases, true, NEEDS_TRANS("Success")); });
                        });
                        return;
                    }
                }
        ...
        static String SKPaymentTransactionStateToString(SKPaymentTransactionState state)
        {
            ...
            {
            case SKPaymentTransactionStatePurchasing:
                return NEEDS_TRANS("Purchasing");
            case SKPaymentTransactionStatePurchased:
                return NEEDS_TRANS("Success");
            case SKPaymentTransactionStateFailed:
                return NEEDS_TRANS("Failure");
            case SKPaymentTransactionStateRestored:
                return NEEDS_TRANS("Restored");
            case SKPaymentTransactionStateDeferred:
                return NEEDS_TRANS("Deferred");
            default:
                jassertfalse;
        ...
        static NSArray<SKDownload*>* downloadsToSKDownloads(const Array<Download*>& downloads)
        {
            ...
            for (const auto& d : downloads)
                if (auto impl = dynamic_cast<DownloadImpl*>(d)) [skDownloads addObject:impl->download];
        ...
        {
            if (o == nil || ![o isKindOfClass:[ObjCType class]]) return nil;
