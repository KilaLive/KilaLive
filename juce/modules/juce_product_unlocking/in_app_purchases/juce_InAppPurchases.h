    ...
    public:
        ...
        struct Listener
        {
            ...
            /** Called whenever a purchase is complete, with additional state whether the purchase completed successfully.
                ...
                For hosted content (iOS only), the downloads array within PurchaseInfo will contain all download objects
               corresponding with the purchase. For non-hosted content, the downloads array will be empty.
                ...
                only the newest set of downloads and the newest orderId will be valid, the old ones should be not used
               anymore!
            */
            ...
            /** Called when a list of all purchases is restored. This can be used to figure out to
               ...
               anymore!
            */
            virtual void purchasesListRestored(const Array<PurchaseInfo>&, bool /*success*/,
            ...
            /** iOS only: Called when a product download progress gets updated. If the download was interrupted in the last
                ...
                If the download finishes in the background before you start the app again, you will receive
               productDownloadFinished callback instead. The download will only stop when it is explicitly cancelled or when
               it is finished.
            */
            ...
            /** iOS only: Called when a product download is paused. This may also be called after the application starts, if
                ...
                Only after the download is finished successfully or cancelled you will stop receiving this callback on
               startup.
            */
        ...
        /** Asynchronously requests to buy a product with given id.
            ...
            @param isSubscription                  (Android only) defines if a product a user wants to buy is a subscription
           or a one-time purchase. On iOS, type of the product is derived implicitly.

            @param upgradeOrDowngradeFromSubscriptionsWithProductIdentifiers (Android only) specifies subscriptions that will
           be replaced by the one being purchased now. Used only when buying a subscription that is an upgrade or downgrade
           from other ones.

            @param creditForUnusedSubscription     (Android only) controls whether a user should be credited for any unused
           subscription time on the products that are being upgraded or downgraded.
        */
        ...
        /** Asynchronously asks about a list of products that a user has already bought. Upon completion,
           Listener::purchasesListReceived() callback will be invoked. The user may be prompted to login first.

            @param includeDownloadInfo      (iOS only) if true, then after restoration is successfull, the downloads array
           passed to Listener::purchasesListReceived() callback will contain all the download objects corresponding with the
           purchase. In the opposite case, the downloads array will be empty.

            @param subscriptionsSharedSecret    (iOS only) required when not including download information and when there are
                                                auto-renewable subscription set up with this app. Refer to In-App-Purchase
