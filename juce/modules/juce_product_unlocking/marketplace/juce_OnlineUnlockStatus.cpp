...
namespace juce
{
    ...
    */
    ...
    struct KeyFileUtils
    {
        static XmlElement createKeyFileContent(const String& appName, const String& userEmail, const String& userName,
                                               const String& machineNumbers, const String& machineNumbersAttributeName)
        {
            XmlElement xml("key");
        ...
        static String createKeyFileComment(const String& appName, const String& userEmail, const String& userName,
                                           const String& machineNumbers)
        {
            String comment;
            comment << "Keyfile for " << appName << newLine;

            if (userName.isNotEmpty()) comment << "User: " << userName << newLine;

            comment << "Email: " << userEmail << newLine << "Machine numbers: " << machineNumbers << newLine
        ...
        static String createKeyFile(String comment, const XmlElement& xml, RSAKey rsaPrivateKey)
        {
...
//==============================================================================
    ...
    static var machineNumberAllowed(StringArray numbersFromKeyFile, StringArray localMachineNumbers)
    {
        var result;
    ...
    void OnlineUnlockStatus::load()
    {
        ...
        if (machineNumberAllowed(StringArray("1234"), localMachineNums)) status.removeProperty(unlockedProp, nullptr);
        ...
        {
            if (!doesProductIDMatch(data.appID)) status.removeProperty(expiryTimeProp, nullptr);

            if (!machineNumberAllowed(data.machineNumbers, localMachineNums)) status.removeProperty(expiryTimeProp, nullptr);
        }
        else
        {
            if (!doesProductIDMatch(data.appID)) status.removeProperty(unlockedProp, nullptr);

            if (!machineNumberAllowed(data.machineNumbers, localMachineNums)) status.removeProperty(unlockedProp, nullptr);
        }
    }
    ...
    String OnlineUnlockStatus::MachineIDUtilities::getEncodedIDString(const String& input)
    {
        ...
        return platform + MD5((input + "salt_1" + platform).toUTF8()).toHexString().substring(0, 9).toUpperCase();
    }
    ...
    void OnlineUnlockStatus::MachineIDUtilities::addMACAddressesToList(StringArray& ids)
    {
        ...
        for (int i = 0; i < addresses.size(); ++i) ids.add(getEncodedIDString(addresses.getReference(i).toString()));
    }

    StringArray OnlineUnlockStatus::MachineIDUtilities::getLocalMachineIDs()
    {
        auto identifiers = SystemStats::getDeviceIdentifiers();
        for (auto& identifier : identifiers) identifier = getEncodedIDString(identifier);
    ...
    bool OnlineUnlockStatus::applyKeyFile(String keyFileContent)
    {
        ...
        if (data.licensee.isNotEmpty() && data.email.isNotEmpty() && doesProductIDMatch(data.appID))
        {
            ...
            if ((!dummyResult) && actualResult) status.setProperty(unlockedProp, actualResult, nullptr);
    ...
    {
        const char* urlsToTry[] = {"http://google.com",
                                   "http://bing.com",
                                   "http://amazon.com",
                                   "https://google.com",
                                   "https://bing.com",
        ...
        for (const char** url = urlsToTry; *url != nullptr; ++url)
            if (canConnectToWebsite(URL(*url))) return true;
    ...
    OnlineUnlockStatus::UnlockResult OnlineUnlockStatus::handleXmlReply(XmlElement xml)
    {
        ...
        if (r.errorMessage.isEmpty() && r.informativeMessage.isEmpty() && r.urlToLaunch.isEmpty() && !r.succeeded)
            r.errorMessage =
                TRANS("Unexpected or corrupted reply from XYZ").replace("XYZ", getWebsiteName()) + "...\n\n"
    ...
    OnlineUnlockStatus::UnlockResult OnlineUnlockStatus::attemptWebserverUnlock(const String& email, const String& password)
    {
        ...
        if (xml != nullptr) return handleXmlReply(*xml);
    ...
    //==============================================================================
    String KeyGeneration::generateKeyFile(const String& appName, const String& userEmail, const String& userName,
                                          const String& machineNumbers, const RSAKey& privateKey)
    {
    ...
    String KeyGeneration::generateExpiringKeyFile(const String& appName, const String& userEmail, const String& userName,
                                                  const String& machineNumbers, const Time expiryTime,
