...
namespace juce
{
    ...
    struct OnlineUnlockForm::OverlayComp : public Component, private Thread, private Timer, private Button::Listener
    {
        ...
        void paint(Graphics& g) override
        {
            ...
            g.drawFittedText(TRANS("Contacting XYZ...").replace("XYZ", form.status.getWebsiteName()),
                             getLocalBounds().reduced(20, 0).removeFromTop(proportionOfHeight(0.6f)), Justification::centred,
                             5);
        }
        ...
        void timerCallback() override
        {
            ...
            {
                AlertWindow::showMessageBoxAsync(AlertWindow::WarningIcon, TRANS("Registration Failed"), result.errorMessage);
            }
            else if (result.informativeMessage.isNotEmpty())
            {
                AlertWindow::showMessageBoxAsync(AlertWindow::InfoIcon, TRANS("Registration Complete!"),
    ...
    OnlineUnlockForm::OnlineUnlockForm(OnlineUnlockStatus& s, const String& userInstructions, bool hasCancelButton,
        ...
        , showOverlayCancelButton(overlayHasCancelButton)
    {
        ...
        if (hasCancelButton) addAndMakeVisible(cancelButton);
    ...
    void OnlineUnlockForm::resized()
    {
        ...
        */
        ...
        const int gap = 20;
        buttonArea    = buttonArea.withSizeKeepingCentre(
            registerButton.getWidth() + (cancelButton.isVisible() ? gap + cancelButton.getWidth() : 0), buttonHeight);
        ...
        if (unlockingOverlay != nullptr) unlockingOverlay->setBounds(getLocalBounds());
    }
    ...
    void OnlineUnlockForm::showBubbleMessage(const String& text, Component& target)
    {
        ...
        bubble->showAt(getLocalArea(&target, target.getLocalBounds()), attString, 500,  // numMillisecondsBeforeRemoving
