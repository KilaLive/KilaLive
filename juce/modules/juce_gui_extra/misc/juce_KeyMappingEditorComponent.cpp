...
namespace juce
{
    ...
    {
    public:
        ChangeKeyButton(KeyMappingEditorComponent& kec, const CommandID command, const String& keyName, const int keyIndex)
            : Button(keyName)
            , owner(kec)
            , commandID(command)
            , keyNum(keyIndex)
        {
            ...
            setTooltip(keyIndex < 0 ? TRANS("Adds a new key-mapping") : TRANS("Click to change this key-mapping"));
        }

        void paintButton(Graphics& g, bool /*isOver*/, bool /*isDown*/) override
        {
            getLookAndFeel().drawKeymapChangeButton(g, getWidth(), getHeight(), *this, keyNum >= 0 ? getName() : String());
        }
        ...
        public:
            KeyEntryWindow(KeyMappingEditorComponent& kec)
                : AlertWindow(TRANS("New key-mapping"), TRANS("Please press a key combination now..."), AlertWindow::NoIcon)
                , owner(kec)
            {
                ...
                for (auto* child : getChildren()) child->setWantsKeyboardFocus(false);
        ...
        {
            if (result != 0 && button != nullptr) button->setNewKey(newKey, true);
        }
        ...
        void setNewKey(const KeyPress& newKey, bool dontAskUser)
        {
            if (newKey.isValid())
            {
                ...
                if (previousCommand == 0 || dontAskUser)
                {
                    ...
                    if (keyNum >= 0) owner.getMappings().removeKeyPress(commandID, keyNum);
                ...
                else
                {
                    AlertWindow::showOkCancelBox(
                        AlertWindow::WarningIcon, TRANS("Change key-mapping"),
                        ...
                        TRANS("Re-assign"), TRANS("Cancel"), this,
                        ModalCallbackFunction::forComponent(assignNewKeyCallback, this, KeyPress(newKey)));
                }
            }
        }
    ...
    public:
        ...
        void paint(Graphics& g) override
        {
            ...
            g.drawFittedText(TRANS(owner.getCommandManager().getNameOfCommand(commandID)), 4, 0,
                             jmax(40, getChildComponent(0)->getX() - 5), getHeight(), Justification::centredLeft, true);
        }
            ...
            if (isNowOpen)
            {
                if (getNumSubItems() == 0)
                    for (auto command : owner.getCommandManager().getCommandsInCategory(categoryName))
                        if (owner.shouldCommandBeIncluded(command)) addSubItem(new MappingItem(owner, command));
            }
            else
            {
                clearSubItems();
    ...
    //==============================================================================
    class KeyMappingEditorComponent::TopLevelItem : public TreeViewItem, private ChangeListener
    {
    public:
        ...
        void changeListenerCallback(ChangeBroadcaster*) override
        {
            ...
            for (auto category : owner.getCommandManager().getCommandCategories())
            {
                ...
                for (auto command : owner.getCommandManager().getCommandsInCategory(category))
                    if (owner.shouldCommandBeIncluded(command)) ++count;

                if (count > 0) addSubItem(new CategoryItem(owner, category));
            }
        }
    ...
    {
        if (result != 0 && owner != nullptr) owner->getMappings().resetToDefaultMappings();
    }
        ...
        , resetButton(TRANS("reset to defaults"))
    {
        ...
        if (showResetToDefaultButton)
        {
            ...
            resetButton.onClick = [this] {
                AlertWindow::showOkCancelBox(
                    AlertWindow::QuestionIcon, TRANS("Reset to defaults"),
                    TRANS("Are you sure you want to reset all the key-mappings to their default state?"), TRANS("Reset"), {},
                    this, ModalCallbackFunction::forComponent(resetKeyMappingsToDefaultsCallback, this));
            };
        }
    ...
    //==============================================================================
    void KeyMappingEditorComponent::setColours(Colour mainBackground, Colour textColour)
    {
