...
namespace juce
{
        ...
        void paint(Graphics& g) override
        {
            ...
            g.drawImageTransformed(
                ...
                RectanglePlacement(RectanglePlacement::stretchToFit)
                    .getTransformToFit(colours.getBounds().toFloat(), getLocalBounds().reduced(edge).toFloat()),
                false);
        }
    ...
    private:
        ...
        void updateMarker()
        {
            marker.setBounds(roundToInt((getWidth() - edge * 2) * s), roundToInt((getHeight() - edge * 2) * (1.0f - v)),
                             edge * 2, edge * 2);
        }
    ...
    public:
        ...
        void paint(Graphics& g) override
        {
            ...
            Path p;
            p.addTriangle(1.0f, 1.0f, cw * 0.3f, ch * 0.5f, 1.0f, ch - 1.0f);

            p.addTriangle(cw - 1.0f, 1.0f, cw * 0.7f, ch * 0.5f, cw - 1.0f, ch - 1.0f);
    ...
    public:
        ...
        void paint(Graphics& g) override
        {
            ...
            for (float i = 0.0f; i <= 1.0f; i += 0.02f) cg.addColour(i, Colour(i, 1.0f, 1.0f, 1.0f));
    ...
    public:
        ...
        void paint(Graphics& g) override
        {
            ...
            g.fillCheckerBoard(getLocalBounds().toFloat(), 6.0f, 6.0f, Colour(0xffdddddd).overlaidWith(col),
        ...
        , edgeGap(edge)
    {
        ...
        if ((flags & showSliders) != 0)
        {
            ...
            for (int i = 4; --i >= 0;) sliders[i]->onValueChange = [this] { changeColour(); };
        }
    ...
    void ColourSelector::update(NotificationType notification)
    {
        ...
        if ((flags & showColourAtTop) != 0) repaint(previewArea);

        if (notification != dontSendNotification) sendChangeMessage();

        if (notification == sendNotificationSync) dispatchPendingMessages();
    }
    ...
    void ColourSelector::paint(Graphics& g)
    {
        ...
        if ((flags & showColourAtTop) != 0)
        {
            ...
            g.fillCheckerBoard(previewArea.toFloat(), 10.0f, 10.0f, Colour(0xffdddddd).overlaidWith(currentColour),
                               Colour(0xffffffff).overlaidWith(currentColour));

            g.setColour(Colours::white.overlaidWith(currentColour).contrasting());
            g.setFont(Font(14.0f, Font::bold));
            g.drawText(currentColour.toDisplayString((flags & showAlphaChannel) != 0), previewArea, Justification::centred,
                       false);
        }
        ...
        if ((flags & showSliders) != 0)
        {
            ...
            for (int i = 4; --i >= 0;)
            {
                if (sliders[i]->isVisible())
                    g.drawText(sliders[i]->getName() + ":", 0, sliders[i]->getY(), sliders[i]->getX() - 8,
                               sliders[i]->getHeight(), Justification::centredRight, false);
            }
        }
    }
    ...
    void ColourSelector::resized()
    {
        ...
        if ((flags & showColourspace) != 0)
        {
            ...
            colourSpace->setBounds(edgeGap, y, getWidth() - hueWidth - edgeGap - 4,
                                   getHeight() - topSpace - sliderSpace - swatchSpace - edgeGap);

            hueSelector->setBounds(colourSpace->getRight() + 4, y, getWidth() - edgeGap - (colourSpace->getRight() + 4),
        ...
        if ((flags & showSliders) != 0)
        {
            ...
            {
                sliders[i]->setBounds(proportionOfWidth(0.2f), y, proportionOfWidth(0.72f), sliderHeight - 2);
        ...
        if (numSwatches > 0)
        {
            ...
            for (int i = 0; i < swatchComponents.size(); ++i)
            {
                ...
                sc->setBounds(x + xGap / 2, y + yGap / 2, swatchWidth - xGap, swatchHeight - yGap);
    ...
    {
        if (sliders[0] != nullptr)
            setCurrentColour(Colour((uint8)sliders[0]->getValue(), (uint8)sliders[1]->getValue(),
                                    (uint8)sliders[2]->getValue(), (uint8)sliders[3]->getValue()));
    }
