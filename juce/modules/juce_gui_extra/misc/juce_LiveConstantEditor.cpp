...
namespace juce
{
    ...
    {
        //==============================================================================
        class AllComponentRepainter : private Timer, private DeletedAtShutdown
        {
        public:
        ...
        private:
            void timerCallback() override
            {
                ...
                for (int i = TopLevelWindow::getNumTopLevelWindows(); --i >= 0;)
                    if (auto* c = TopLevelWindow::getTopLevelWindow(i)) repaintAndResizeAllComps(c, alreadyDone);

                auto& desktop = Desktop::getInstance();

                for (int i = desktop.getNumComponents(); --i >= 0;)
                    if (auto* c = desktop.getComponent(i)) repaintAndResizeAllComps(c, alreadyDone);
            }

            static void repaintAndResizeAllComps(Component::SafePointer<Component> c, Array<Component*>& alreadyDone)
            {
        ...
        int64 parseInt(String s)
        {
            ...
            if (s.startsWithChar('-')) return -parseInt(s.substring(1));

            if (s.startsWith("0x")) return s.substring(2).getHexValue64();
        ...
        LivePropertyEditorBase::LivePropertyEditorBase(LiveValueBase& v, CodeDocument& d)
            : value(v)
            , document(d)
            , sourceEditor(document, &tokeniser)
        {
            setSize(600, 100);
        ...
        {
        public:
            ValueListHolderComponent(ValueList& l)
                : valueList(l)
            {
                setVisible(true);
            }
            ...
            void resized() override
            {
                ...
                for (int i = 0; i < editors.size(); ++i) editors.getUnchecked(i)->setBounds(r.removeFromTop(itemHeight));
            }
        ...
        //==============================================================================
        class ValueList::EditorWindow : public DocumentWindow, private DeletedAtShutdown
        {
        public:
        ...
        {
            if (editorWindow == nullptr) editorWindow = new EditorWindow(*this);
        ...
        CodeDocument& ValueList::getDocument(const File& file)
        {
            ...
            if (index >= 0) return *documents.getUnchecked(index);
        ...
        //==============================================================================
        struct ColourEditorComp : public Component, private ChangeListener
        {
            ColourEditorComp(LivePropertyEditorBase& e)
                : editor(e)
            {
            ...
            {
                g.fillCheckerBoard(getLocalBounds().toFloat(), 6.0f, 6.0f, Colour(0xffdddddd).overlaidWith(getColour()),
        ...
        Component* createBoolSlider(LivePropertyEditorBase& editor)
        {
