...
namespace juce
{
    ...
    CodeEditorComponent::ColourScheme XmlTokeniser::getDefaultColourScheme()
    {
        ...
        const Type types[] = {{"Error", 0xffcc0000},    {"Comment", 0xff00aa00},     {"Keyword", 0xff0000cc},
                              {"Operator", 0xff225500}, {"Identifier", 0xff000000},  {"String", 0xff990099},
                              {"Bracket", 0xff000055},  {"Punctuation", 0xff004400}, {"Preprocessor Text", 0xff660000}};
    ...
    static void skipToEndOfXmlDTD(Iterator& source) noexcept
    {
        ...
        for (;;)
        {
            ...
            if (c == 0 || (c == '>' && lastWasQuestionMark)) break;
    ...
    int XmlTokeniser::readNextToken(CodeDocument::Iterator& source)
    {
        ...
        default:
            if (CppTokeniserFunctions::isIdentifierStart(firstChar)) CppTokeniserFunctions::parseIdentifier(source);
