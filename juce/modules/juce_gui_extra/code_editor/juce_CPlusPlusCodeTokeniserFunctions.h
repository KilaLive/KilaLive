...
namespace juce
{
    ...
    struct CppTokeniserFunctions
    {
        ...
        {
            static const char* const keywords2Char[] = {"do", "if", "or", nullptr};

            static const char* const keywords3Char[] = {"and", "asm", "for", "int", "new", "not", "try", "xor", nullptr};

            static const char* const keywords4Char[] = {"auto", "bool", "case", "char", "else", "enum",
                                                        "goto", "long", "this", "true", "void", nullptr};

            static const char* const keywords5Char[] = {"bitor", "break", "catch", "class", "compl", "const",
                                                        "false", "final", "float", "or_eq", "short", "throw",
                                                        "union", "using", "while", nullptr};
            ...
            static const char* const keywords7Char[] = {"__cdecl", "_Pragma", "alignas", "alignof", "concept",
                                                        "default", "mutable", "nullptr", "private", "typedef",
                                                        "uint8_t", "virtual", "wchar_t", nullptr};

            static const char* const keywordsOther[] = {"@class",
                                                        "@dynamic",
                                                        "@end",
                                                        "@implementation",
                                                        "@interface",
        ...
        static bool skipNumberSuffix(Iterator& source)
        {
            ...
            if (c == 'l' || c == 'L' || c == 'u' || c == 'U') source.skip();

            if (CharacterFunctions::isLetterOrDigit(source.peekNextChar())) return false;
        ...
        static bool parseHexLiteral(Iterator& source) noexcept
        {
            ...
            if (source.nextChar() != '0') return false;
        ...
        static bool parseOctalLiteral(Iterator& source) noexcept
        {
            ...
            if (!isOctalDigit(source.nextChar())) return false;

            while (isOctalDigit(source.peekNextChar())) source.skip();
        ...
        static bool parseFloatLiteral(Iterator& source) noexcept
        {
            ...
            if (hasExponent)
            {
                ...
                if (numExpDigits == 0) return false;
            }
        ...
        static void skipPreprocessorLine(Iterator& source) noexcept
        {
            ...
            for (;;)
            {
                ...
                if (c == '\n' || c == '\r')
                {
                    ...
                    if (lastWasBackslash) skipPreprocessorLine(source);
        ...
        {
            if (source.peekNextChar() == c) source.skip();
        }
        ...
        static int readNextToken(Iterator& source)
        {
            ...
            case '.':
            {
                ...
                if (result == CPlusPlusCodeTokeniser::tokenType_error)
                {
                    ...
                    if (firstChar == '.') return CPlusPlusCodeTokeniser::tokenType_punctuation;
                }
            ...
            default:
                if (isIdentifierStart(firstChar)) return parseIdentifier(source);
        ...
        */
        ...
        /** Takes a UTF8 string and writes it to a stream using standard C++ escape sequences for any
            ...
            @see addEscapeChars
        */
        static void writeEscapeChars(OutputStream& out, const char* utf8, const int numBytesToRead, const int maxCharsOnLine,
                                     const bool breakAtNewLines, const bool replaceSingleQuotes, const bool allowStringBreaks)
        {
            int  charsOnLine          = 0;
            ...
            for (int i = 0; i < numBytesToRead || numBytesToRead < 0; ++i)
            {
                ...
                default:
                    if (c >= 32 && c < 127
                        && !(lastWasHexEscapeCode  // (have to avoid following a hex escape sequence with a valid hex digit)
