...
namespace juce
{
    ...
    public:
        ...
        bool update(CodeDocument& codeDoc, int lineNum, CodeDocument::Iterator& source, CodeTokeniser* tokeniser,
                    const int tabSpaces, const CodeDocument::Position& selStart, const CodeDocument::Position& selEnd)
        {
            ...
            else if (lineNum < codeDoc.getNumLines())
            {
                ...
                createTokens(pos.getPosition(), pos.getLineText(), source, *tokeniser, newTokens);
            }
            ...
            if (selStart.getLineNumber() <= lineNum && selEnd.getLineNumber() >= lineNum)
            {
                ...
                newHighlightStart = indexToColumn(jmax(0, selStart.getPosition() - lineStart.getPosition()), line, tabSpaces);
                newHighlightEnd   = indexToColumn(
                    jmin(lineEnd.getPosition() - lineStart.getPosition(), selEnd.getPosition() - lineStart.getPosition()),
                    line, tabSpaces);
            }
        ...
        void draw(CodeEditorComponent& owner, Graphics& g, const Font& fontToUse, const float rightClip, const float x,
                  const int y, const int lineH, const float characterWidth) const
        {
            AttributedString as;
            ...
            for (auto& token : tokens)
            {
                ...
                if (tokenX > rightClip) break;

                as.append(token.text.initialSectionNotContaining("\r\n"), fontToUse,
                          owner.getColourForTokenType(token.tokenType));
                column += token.length;
            }
    ...
    private:
        struct SyntaxToken
        {
            ...
            {
                return tokenType == other.tokenType && length == other.length && text == other.text;
            }
        ...
        static void createTokens(int startPosition, const String& lineText, CodeDocument::Iterator& source,
                                 CodeTokeniser& tokeniser, Array<SyntaxToken>& newTokens)
        {
            ...
            for (;;)
            {
                ...
                if (tokenEnd > 0)
                {
                    ...
                    addToken(newTokens, lineText.substring(start, tokenEnd), tokenEnd - start, tokenType);

                    if (tokenEnd >= lineLength) break;
                }
    ...
    public:
        ...
        void paint(Graphics& g) override
        {
            ...
            const int   lastLineToDraw =
                jmin(editor.lines.size(), clip.getBottom() / lineH + 1, lastNumLines - editor.firstLineOnScreen);
            ...
            for (int i = firstLineToDraw; i < lastLineToDraw; ++i)
                ga.addFittedText(lineNumberFont, String(editor.firstLineOnScreen + i + 1), 0, (float)(lineH * i), w,
                                 lineHeightFloat, Justification::centredRight, 1, 0.2f);
        ...
        , codeTokeniser(tokeniser)
    {
        ...
        if (codeTokeniser != nullptr) setColourScheme(codeTokeniser->getDefaultColourScheme());
    ...
    void CodeEditorComponent::resized()
    {
        ...
        if (gutter != nullptr) gutter->setBounds(0, 0, getGutterSize() - 2, getHeight());

        verticalScrollBar.setBounds(getWidth() - scrollbarThickness, 0, scrollbarThickness, getHeight() - scrollbarThickness);

        horizontalScrollBar.setBounds(getGutterSize(), getHeight() - scrollbarThickness, visibleWidth, scrollbarThickness);
        updateScrollBars();
    }
    ...
    void CodeEditorComponent::rebuildLineTokens()
    {
        ...
        if (numNeeded != lines.size())
        {
            ...
            for (int i = numNeeded; --i >= 0;) lines.add(new CodeEditorLine());
        ...
        {
            if (lines.getUnchecked(i)->update(document, firstLineOnScreen + i, source, codeTokeniser, spacesPerTab,
        ...
        if (minLineToRepaint <= maxLineToRepaint)
            repaint(0, lineHeight * minLineToRepaint - 1, verticalScrollBar.getX(),
                    lineHeight * (1 + maxLineToRepaint - minLineToRepaint) + 2);

        if (gutter != nullptr) gutter->documentChanged(document, firstLineOnScreen);
    }
    ...
    {
        if (isHighlightActive()) rebuildLineTokensAsync();
    ...
    {
        newFirstLineOnScreen = jlimit(0, jmax(0, document.getNumLines() - 1), newFirstLineOnScreen);
    ...
    {
        return {roundToInt((getGutterSize() - xOffset * charWidth)
                           + indexToColumn(pos.getLineNumber(), pos.getIndexInLine()) * charWidth),
                (pos.getLineNumber() - firstLineOnScreen) * lineHeight, roundToInt(charWidth), lineHeight};
    }
    ...
    void CodeEditorComponent::insertText(const String& newText)
    {
        if (!readOnly)
        {
            ...
            if (newText.isNotEmpty()) document.insertText(caretPos, newText);
    ...
    bool CodeEditorComponent::deleteWhitespaceBackwardsToTabStop()
    {
        if (getHighlightedRegion().isEmpty() && !readOnly)
        {
            for (;;)
            {
                ...
                if (currentColumn <= 0 || (currentColumn % spacesPerTab) == 0) break;
    ...
    void CodeEditorComponent::indentSelectedLines(const int spacesToAdd)
    {
        if (!readOnly)
        {
            ...
            if (lineEnd > lineStart && selectionEnd.getIndexInLine() == 0) --lineEnd;
    ...
    bool CodeEditorComponent::copyToClipboard()
    {
        ...
        if (selection.isNotEmpty()) SystemClipboard::copyTextToClipboard(selection);
    ...
    bool CodeEditorComponent::pasteFromClipboard()
    {
        ...
        if (clip.isNotEmpty()) insertText(clip);
    ...
    void CodeEditorComponent::moveLineDelta(const int delta, const bool selecting)
    {
        ...
        if (columnToTryToMaintain < 0) columnToTryToMaintain = indexToColumn(pos.getLineNumber(), pos.getIndexInLine());
    ...
    bool CodeEditorComponent::moveCaretDown(const bool selecting)
    {
        ...
        if (caretPos.getLineNumber() == document.getNumLines() - 1)
            moveCaretTo(CodeDocument::Position(document, std::numeric_limits<int>::max(), std::numeric_limits<int>::max()),
                        selecting);
        else
    ...
    bool CodeEditorComponent::scrollUp()
    {
        ...
        if (caretPos.getLineNumber() < firstLineOnScreen) moveLineDelta(1, false);
    ...
    bool CodeEditorComponent::scrollDown()
    {
        ...
        if (caretPos.getLineNumber() >= firstLineOnScreen + linesOnScreen) moveLineDelta(-1, false);
    ...
    bool CodeEditorComponent::moveCaretToStartOfLine(const bool selecting)
    {
        ...
        if (index >= caretPos.getIndexInLine() && caretPos.getIndexInLine() > 0) index = 0;
    ...
    void CodeEditorComponent::selectRegion(const CodeDocument::Position& start, const CodeDocument::Position& end)
    {
    ...
    {
        return {selectionStart.getPosition(), selectionEnd.getPosition()};
    }
    ...
    {
        const CommandID ids[] = {StandardApplicationCommandIDs::cut,       StandardApplicationCommandIDs::copy,
                                 StandardApplicationCommandIDs::paste,     StandardApplicationCommandIDs::del,
                                 StandardApplicationCommandIDs::selectAll, StandardApplicationCommandIDs::undo,
    ...
    void CodeEditorComponent::getCommandInfo(const CommandID commandID, ApplicationCommandInfo& result)
    {
        ...
        case StandardApplicationCommandIDs::cut:
            result.setInfo(TRANS("Cut"), TRANS("Copies the currently selected text to the clipboard and deletes it."),
                           "Editing", 0);
    ...
    {
        if (editor != nullptr && menuResult != 0) editor->performPopupMenuAction(menuResult);
    }
    ...
    void CodeEditorComponent::mouseDown(const MouseEvent& e)
    {
        ...
        if (e.mods.isPopupMenu())
        {
            ...
            if (getHighlightedRegion().isEmpty())
            {
                ...
                if (start.getPosition() < end.getPosition()) selectRegion(start, end);
            }
    ...
    {
        if (!e.mods.isPopupMenu()) moveCaretTo(getPositionAt(e.x, e.y), true);
    }
    ...
    String CodeEditorComponent::getTabString(const int numSpaces) const
    {
        return String::repeatedString(useSpacesForTabs ? " " : "\t",
                                      useSpacesForTabs ? numSpaces : (numSpaces / spacesPerTab));
    }
    ...
    void CodeEditorComponent::clearCachedIterators(const int firstLineToBeInvalid)
    {
        ...
        for (i = cachedIterators.size(); --i >= 0;)
            if (cachedIterators.getUnchecked(i)->getLine() < firstLineToBeInvalid) break;
    ...
    void CodeEditorComponent::updateCachedIterators(int maxLineNum)
    {
        ...
        if (cachedIterators.size() == 0) cachedIterators.add(new CodeDocument::Iterator(document));
        ...
        if (codeTokeniser != nullptr)
        {
            for (;;)
            {
                ...
                if (last.getLine() >= maxLineNum) break;
    ...
    void CodeEditorComponent::State::restoreState(CodeEditorComponent& editor) const
    {
        ...
        if (lastTopLine > 0 && lastTopLine < editor.getDocument().getNumLines()) editor.scrollToLine(lastTopLine);
    }
