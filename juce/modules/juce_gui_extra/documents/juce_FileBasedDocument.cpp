...
namespace juce
{
    FileBasedDocument::FileBasedDocument(const String& fileExtension_, const String& fileWildcard_,
                                         const String& openFileDialogTitle_, const String& saveFileDialogTitle_)
    ...
    Result FileBasedDocument::loadFrom(const File& newFile, const bool showMessageOnFailure)
    {
        ...
        if (showMessageOnFailure)
            AlertWindow::showMessageBoxAsync(AlertWindow::WarningIcon, TRANS("Failed to open file..."),
    ...
    {
        FileChooser fc(openFileDialogTitle, getLastDocumentOpened(), fileWildcard);

        if (fc.browseForFileToOpen()) return loadFrom(fc.getResult(), showMessageOnFailure);
    ...
    static bool askToOverwriteFile(const File& newFile)
    {
        return AlertWindow::showOkCancelBox(
            AlertWindow::WarningIcon, TRANS("File already exists"),
            TRANS("There's already a file called: FLNM").replace("FLNM", newFile.getFullPathName()) + "\n\n"
    ...
    {
        return saveAs(documentFile, false, askUserForFileIfNotSpecified, showMessageOnFailure);
    }
                                                            ...
                                                            const bool showMessageOnFailure)
    {
        if (newFile == File())
        {
            if (askUserForFileIfNotSpecified) return saveAsInteractive(true);
        ...
        if (warnAboutOverwritingExistingFiles && newFile.exists() && !askToOverwriteFile(newFile)) return userCancelledSave;
        ...
        if (showMessageOnFailure)
            AlertWindow::showMessageBoxAsync(AlertWindow::WarningIcon, TRANS("Error writing to file..."),
    ...
    {
        if (!hasChangedSinceSaved()) return savedOk;

        const int r = AlertWindow::showYesNoCancelBox(
            AlertWindow::QuestionIcon, TRANS("Closing document..."),
            TRANS("Do you want to save the changes to \"DCNM\"?").replace("DCNM", getDocumentTitle()), TRANS("Save"),
            TRANS("Discard changes"), TRANS("Cancel"));
    ...
    FileBasedDocument::SaveResult FileBasedDocument::saveAsInteractive(const bool warnAboutOverwritingExistingFiles)
    {
        ...
        if (legalFilename.isEmpty()) legalFilename = "unnamed";
        ...
        if (fc.browseForFileToSave(warnAboutOverwritingExistingFiles))
        {
            ...
            if (chosen.getFileExtension().isEmpty())
            {
                ...
                if (chosen.exists() && !askToOverwriteFile(chosen)) return userCancelledSave;
            }
