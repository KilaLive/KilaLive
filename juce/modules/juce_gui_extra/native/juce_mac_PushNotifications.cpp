...
{
    template <>
    struct ContainerDeletePolicy<NSObject<NSApplicationDelegate, NSUserNotificationCenterDelegate>>
    {
    ...
    namespace PushNotificationsDelegateDetailsOsx
    {
        ...
        NSUserNotification* juceNotificationToNSUserNotification(const PushNotifications::Notification& n,
                                                                 bool isEarlierThanMavericks, bool isEarlierThanYosemite)
        {
            ...
            {
                auto* soundName = juceStringToNS(
                    soundToPlayString.fromLastOccurrenceOf("/", false, false).upToLastOccurrenceOf(".", false, false));
            ...
            if (n.actions.size() > 0) notification.actionButtonTitle = juceStringToNS(n.actions.getReference(0).title);
            ...
            if (!isEarlierThanMavericks)
            {
                ...
                auto* imageDirectory =
                    n.icon.contains("/") ? juceStringToNS(n.icon.upToLastOccurrenceOf("/", false, true)) : [NSString string];

                auto* imageName =
                    juceStringToNS(n.icon.fromLastOccurrenceOf("/", false, false).upToLastOccurrenceOf(".", false, false));
                ...
                {
                    imagePath = [[NSBundle mainBundle] pathForResource:imageName ofType:imageExtension];
                }
                else
                {
                    imagePath =
                        [[NSBundle mainBundle] pathForResource:imageName ofType:imageExtension inDirectory:imageDirectory];
                }
                ...
                if (!isEarlierThanYosemite)
                {
                    if (n.actions.size() > 1)
                    {
                        ...
                        for (int a = 1; a < n.actions.size(); ++a)
                            [additionalActions addObject:[NSUserNotificationAction
                                                             actionWithIdentifier:juceStringToNS(n.actions[a].identifier)
                                                                             ...
                                                                             bool                isEarlierThanYosemite)
        {
            ...
            {
                notif.triggerIntervalSec =
                    n.deliveryRepeatInterval.second + (n.deliveryRepeatInterval.nanosecond / 1000000000.);
            }
            else
            {
            ...
            if (n.actionButtonTitle != nil)
            {
                ...
                if (!isEarlierThanMavericks)
                {
                    if (n.hasReplyButton) action.style = Action::text;

                    if (n.responsePlaceholder != nil) action.textInputPlaceholder = nsStringToJuce(n.responsePlaceholder);
                }
    ...
    private:
        ...
        {
            Class()
                : ObjCClass<NSObject<NSApplicationDelegate, NSUserNotificationCenterDelegate>>(
    ...
    struct PushNotifications::Pimpl : private PushNotificationsDelegate
    {
        ...
        {
            if (isEarlierThanLion) return;

            settings = settingsToUse;

            NSRemoteNotificationType types = NSUInteger((bool)settings.allowBadge);

            if (isAtLeastMountainLion) types |= ((bool)settings.allowSound << 1 | (bool)settings.allowAlert << 2);
        ...
        void requestSettingsUsed()
        {
            ...
            settings.allowBadge =
                [NSApplication sharedApplication].enabledRemoteNotificationTypes & NSRemoteNotificationTypeBadge;

            if (isAtLeastMountainLion)
            {
                settings.allowSound =
                    [NSApplication sharedApplication].enabledRemoteNotificationTypes & NSRemoteNotificationTypeSound;
                settings.allowAlert =
                    [NSApplication sharedApplication].enabledRemoteNotificationTypes & NSRemoteNotificationTypeAlert;
            }
        ...
        {
            auto* notification = PushNotificationsDelegateDetailsOsx::juceNotificationToNSUserNotification(
                n, isEarlierThanMavericks, isEarlierThanYosemite);
        ...
        void getDeliveredNotifications() const
        {
            ...
            for (NSUserNotification* n in [NSUserNotificationCenter defaultUserNotificationCenter].deliveredNotifications)
                notifs.add(PushNotificationsDelegateDetailsOsx::nsUserNotificationToJuceNotification(
                    n, isEarlierThanMavericks, isEarlierThanYosemite));
        ...
        void removeDeliveredNotification(const String& identifier)
        {
            ...
            auto nsNotification = PushNotificationsDelegateDetailsOsx::juceNotificationToNSUserNotification(
                n, isEarlierThanMavericks, isEarlierThanYosemite);
        ...
        {
            auto n = PushNotificationsDelegateDetailsOsx::nsUserNotificationToJuceNotification(
                notification, isEarlierThanMavericks, isEarlierThanYosemite);
            ...
            else
            {
                ...
                owner.listeners.call(
                    [&](Listener& l) { l.handleNotificationAction(notification.remote, n, actionIdentifier, reply); });
            }
        }

        bool shouldPresentNotification(NSUserNotification* notification) override
        {
            return true;
        }
