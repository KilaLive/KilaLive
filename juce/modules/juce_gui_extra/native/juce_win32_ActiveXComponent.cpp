...
namespace juce
{
    ...
    namespace ActiveXHelpers
    {
        ...
        struct JuceOleInPlaceFrame : public ComBaseClassHelper<IOleInPlaceFrame>
        {
            ...
            {
                if (activeObject != nullptr) return activeObject->TranslateAcceleratorW(lpmsg);
        ...
        struct JuceIOleInPlaceSite : public ComBaseClassHelper<IOleInPlaceSite>
        {
            ...
            JUCE_COMRESULT GetWindowContext(LPOLEINPLACEFRAME* lplpFrame, LPOLEINPLACEUIWINDOW* lplpDoc, LPRECT, LPRECT,
                                            LPOLEINPLACEFRAMEINFO lpFrameInfo)
            {
                /* Note: if you call AddRef on the frame here, then some types of object (e.g. web browser control) cause
                   leaks.. If you don't call AddRef then others crash (e.g. QuickTime).. Bit of a catch-22, so letting it leak
            ...
            {
                if (frame != nullptr) return frame->OfferKeyTranslation(&msg);
        ...
        struct JuceIOleClientSite : public ComBaseClassHelper<IOleClientSite>
        {
            ...
            LRESULT offerEventToActiveXControl(::MSG& msg)
            {
                if (inplaceSite != nullptr) return inplaceSite->offerEventToActiveXControl(msg);
        ...
        static inline void offerActiveXMouseEventToPeer(ComponentPeer* peer, HWND hwnd, UINT message, LPARAM lParam)
        {
            ...
            case WM_RBUTTONUP:
            {
                ...
                peer->handleMouseEvent(MouseInputSource::InputSourceType::mouse,
                                       ...
                                       ModifierKeys::getCurrentModifiersRealtime(), MouseInputSource::invalidPressure,
                                       MouseInputSource::invalidOrientation, getMouseEventTime());
                break;
            }
    ...
    public:
        ...
        {
            if (controlHWND != 0)
                MoveWindow(controlHWND, newBounds.getX(), newBounds.getY(), newBounds.getWidth(), newBounds.getHeight(),
                           TRUE);
        }

        void setControlVisible(bool shouldBeVisible) const
        {
            if (controlHWND != 0) ShowWindow(controlHWND, shouldBeVisible ? SW_SHOWNA : SW_HIDE);
        }

        //==============================================================================
        void componentMovedOrResized(bool /*wasMoved*/, bool /*wasResized*/) override
        {
            if (auto* peer = owner.getTopLevelComponent()->getPeer()) setControlBounds(peer->getAreaCoveredBy(owner));
        }
    ...
    {
        if (control == nullptr) g.fillAll(Colours::lightgrey);
    }
    ...
    bool ActiveXControlComponent::createControl(const void* controlIID)
    {
        ...
        if (auto* peer = getPeer())
        {
            ...
            HRESULT hr = OleCreate(*(const IID*)controlIID, __uuidof(IOleObject), 1 /*OLERENDER_DRAW*/, 0,
                                   newControl->clientSite, newControl->storage, (void**)&(newControl->control));
