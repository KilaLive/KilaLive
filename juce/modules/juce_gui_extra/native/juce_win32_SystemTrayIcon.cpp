...
namespace juce
{
    ...
    public:
        ...
        void handleTaskBarEvent(const LPARAM lParam)
        {
            ...
            {
                if (lParam == WM_LBUTTONDOWN || lParam == WM_RBUTTONDOWN || lParam == WM_LBUTTONDBLCLK
            ...
            else
            {
                ...
                const MouseEvent e(Desktop::getInstance().getMainMouseSource(), {}, eventMods,
                                   ...
                                   MouseInputSource::invalidTiltY, &owner, &owner, eventTime, {}, eventTime, 1, false);
        ...
        static Pimpl* getPimpl(HWND hwnd)
        {
            if (JuceWindowIdentifier::isJUCEWindow(hwnd))
                if (ComponentPeer* peer = (ComponentPeer*)GetWindowLongPtr(hwnd, 8))
                    if (SystemTrayIconComponent* const iconComp =
                            dynamic_cast<SystemTrayIconComponent*>(&(peer->getComponent())))
        ...
        {
            if (Pimpl* const p = getPimpl(hwnd)) return p->windowProc(hwnd, message, wParam, lParam);
    ...
    private:
        ...
        enum
        {
            WM_TRAYNOTIFY = WM_USER + 3001
        };

        void notify(DWORD message) noexcept
        {
            Shell_NotifyIcon(message, &iconData);
        }
    ...
    {
        if (pimpl != nullptr) pimpl->setToolTip(tooltip);
    }
    ...
    {
        if (pimpl != nullptr) pimpl->showBubble(title, content);
    }
