...
namespace juce
{
    ...
    public:
        ...
        static void setBlocking(int fd, bool shouldBlock)
        {
            ...
            fcntl(fd, F_SETFL, (shouldBlock ? (flags & ~O_NONBLOCK) : (flags | O_NONBLOCK)));
        }
        ...
        void tryNextRead()
        {
            while (true)
            {
                ...
                if (!receivingLength) buffer.realloc(len);
                ...
                if (pos == len)
                {
                    ...
                    if (!receivingLength) parseJSON(String(buffer.getData(), bufferLength.len));

                    receivingLength = (!receivingLength);
                }
            }

            if (errno != EAGAIN && errno != EWOULDBLOCK && responder != nullptr) responder->receiverHadError();
        }
        ...
        static void sendCommand(int outChannel, const String& cmd, const var& params)
        {
            ...
            if (!params.isVoid()) obj->setProperty(getParamIdentifier(), params);
    ...
    private:
        void parseJSON(const String& json)
        {
            ...
            if (!object.isVoid())
            {
                ...
                if (responder != nullptr) responder->handleCommand(cmd, params);
            }
        }

        static Identifier getCmdIdentifier()
        {
            static Identifier Id("cmd");
            return Id;
        }
        static Identifier getParamIdentifier()
        {
            static Identifier Id("params");
            return Id;
        }
    ...
    public:
        ...
        int entry()
        {
            ...
            auto setHardwarePolicy = reinterpret_cast<SetHardwareAcclPolicyFunctionPtr>(
                dlsym(RTLD_DEFAULT, "webkit_settings_set_hardware_acceleration_policy"));

            if (setHardwarePolicy != nullptr) setHardwarePolicy(settings, 2 /*WEBKIT_HARDWARE_ACCELERATION_POLICY_NEVER*/);
            ...
            g_signal_connect(webview, "decide-policy", G_CALLBACK(decidePolicyCallback), this);

            g_signal_connect(webview, "load-changed", G_CALLBACK(loadChangedCallback), this);

            g_signal_connect(webview, "load-failed", G_CALLBACK(loadFailedCallback), this);
        ...
        {
            WebKitPolicyDecision* decision = (WebKitPolicyDecision*)((int64)params.getProperty("decision_id", var(0)));
        ...
        bool onNewWindow(String /*frameName*/, WebKitNavigationAction* action, WebKitPolicyDecision* decision)
        {
            if (decision != nullptr)
            {
                DynamicObject::Ptr params = new DynamicObject;
        ...
        bool onDecidePolicy(WebKitPolicyDecision* decision, WebKitPolicyDecisionType decisionType)
        {
            ...
            case WEBKIT_POLICY_DECISION_TYPE_NAVIGATION_ACTION:
            {
                ...
                return onNavigation(String(frameName != nullptr ? frameName : ""),
                                    webkit_navigation_policy_decision_get_navigation_action(navigationDecision), decision);
            }
            break;
    ...
    private:
        ...
        static gboolean decidePolicyCallback(WebKitWebView*, WebKitPolicyDecision* decision,
                                             WebKitPolicyDecisionType decisionType, gpointer user)
        {
        ...
        static void loadFailedCallback(WebKitWebView*, WebKitLoadEvent /*loadEvent*/, gchar* /*failing_uri*/, GError* error,
                                       gpointer user)
        {
            GtkChildProcess& owner = *reinterpret_cast<GtkChildProcess*>(user);
    ...
    public:
        ...
        void goToURL(const String& url, const StringArray* headers, const MemoryBlock* postData)
        {
            ...
            if (headers != nullptr) params->setProperty("headers", var(*headers));

            if (postData != nullptr) params->setProperty("postData", var(*postData));
        ...
        void goBack()
        {
            CommandReceiver::sendCommand(outChannel, "goBack", var());
        }
        void goForward()
        {
            CommandReceiver::sendCommand(outChannel, "goForward", var());
        }
        void refresh()
        {
            CommandReceiver::sendCommand(outChannel, "refresh", var());
        }
        void stop()
        {
            CommandReceiver::sendCommand(outChannel, "stop", var());
        }

        void resized()
        {
            if (xembed != nullptr) xembed->setBounds(owner.getLocalBounds());
        }
    ...
    private:
        ...
        void launchChild()
        {
            ...
            if (pid == 0)
            {
                ...
                for (int i = 0; i < arguments.size(); ++i) argv[i] = arguments[i].toRawUTF8();
        ...
        void run() override
        {
            while (!threadShouldExit())
            {
                ...
                while (result == 0 || (result < 0 && errno == EINTR)) result = select(max_fd + 1, &set, NULL, NULL, NULL);
        ...
        void handleCommandOnMessageThread(const String& cmd, const var& params)
        {
            ...
            if (cmd == "pageAboutToLoad")
                handlePageAboutToLoad(url, params);
            else if (cmd == "pageFinishedLoading")
                owner.pageFinishedLoading(url);
            else if (cmd == "windowCloseRequest")
                owner.windowCloseRequest();
            else if (cmd == "newWindowAttemptingToLoad")
                owner.newWindowAttemptingToLoad(url);
            else if (cmd == "pageLoadHadNetworkError")
                handlePageLoadHadNetworkError(params);
        ...
        void handlePageLoadHadNetworkError(const var& params)
        {
            ...
            if (owner.pageLoadHadNetworkError(error)) goToURL(String("data:text/plain,") + error, nullptr, nullptr);
        }
    ...
    {
        if (browser != nullptr) browser->resized();
    }
    ...
    int juce_gtkWebkitMain(int argc, const char* argv[])
    {
        ...
        GtkChildProcess child(String(argv[2]).getIntValue(), String(argv[3]).getIntValue());
        return child.entry();
    }
