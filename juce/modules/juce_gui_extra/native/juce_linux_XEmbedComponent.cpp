...
namespace juce
{
        ...
        public:
            ...
            static Window getCurrentFocusWindow(ComponentPeer* peerToLookFor)
            {
                ...
                if (peerToLookFor != nullptr)
                    if (auto* foundKeyWindow = keyWindows[peerToLookFor]) return foundKeyWindow->keyProxy;
    ...
    public:
            ...
            , allowResize(shouldAllowResize)
        {
            ...
            if (clientInitiated) setClient(x11Window, true);
        ...
        ~Pimpl()
        {
            ...
            if (host != 0)
            {
                ...
                const long mask = NoEventMask | KeyPressMask | KeyReleaseMask | EnterWindowMask | LeaveWindowMask
                                  | PointerMotionMask | KeymapStateMask | ExposureMask | StructureNotifyMask
                                  | FocusChangeMask;
        ...
        void setClient(Window xembedClient, bool shouldReparent)
        {
            ...
            if (xembedClient != 0)
            {
                ...
                if (shouldReparent) XReparentWindow(dpy, client, host, 0, 0);

                if (supportsXembed) sendXEmbedEvent(CurrentTime, XEMBED_EMBEDDED_NOTIFY, 0, (long)host, xembedVersion);
        ...
        void broughtToFront()
        {
            if (client != 0 && supportsXembed) sendXEmbedEvent(CurrentTime, XEMBED_WINDOW_ACTIVATE);
        }
    ...
    private:
        ...
        void componentMovedOrResized(Component&, bool, bool) override
        {
            if (host != 0 && lastPeer != nullptr)
            {
                ...
                if (client != 0 && XGetWindowAttributes(dpy, client, &attr))
                {
                    ...
                    {
                        XMoveResizeWindow(dpy, client, 0, 0, static_cast<unsigned int>(newBounds.getWidth()),
                                          static_cast<unsigned int>(newBounds.getHeight()));
                    }
                }
            }
        ...
        void createHostWindow()
        {
            ...
            host = XCreateWindow(dpy, root, 0, 0, 1, 1, 0, CopyFromParent, InputOutput, CopyFromParent,
                                 CWEventMask | CWBorderPixel | CWBackPixmap | CWOverrideRedirect, &swa);
        }
        ...
        {
            if (auto peer = owner.getPeer()) return reinterpret_cast<Window>(peer->getNativeHandle());
        ...
        Display* getDisplay()
        {
            return reinterpret_cast<Display*>(x11display.display);
        }
        ...
        {
            if (a == atoms.XembedInfo) updateMapping();
        }
        ...
        void configureNotify()
        {
            ...
            if (XGetWindowAttributes(dpy, client, &attr))
            {
                ...
                Point<int> topLeftInPeer = (peer != nullptr ? peer->getComponent().getLocalPoint(&owner, Point<int>(0, 0))
                ...
                if (peer != nullptr) newBounds = owner.getLocalArea(&peer->getComponent(), newBounds);

                jassert(newBounds.getX() == 0 && newBounds.getY() == 0);

                if (newBounds != owner.getLocalBounds()) owner.setSize(newBounds.getWidth(), newBounds.getHeight());
            }
        }
        ...
        void handleXembedCmd(const ::Time& /*xTime*/, long opcode, long /*detail*/, long /*data1*/, long /*data2*/)
        {
            ...
            case XEMBED_REQUEST_FOCUS:
                if (wantsFocus) owner.grabKeyboardFocus();
                break;

            case XEMBED_FOCUS_NEXT:
                if (wantsFocus) owner.moveKeyboardFocusToSibling(true);
                break;

            case XEMBED_FOCUS_PREV:
                if (wantsFocus) owner.moveKeyboardFocusToSibling(false);
                break;
            }
        }
        ...
        bool handleX11Event(const XEvent& e)
        {
            ...
            else if (e.xany.window == host && host != 0)
            {
                ...
                case CreateNotify:
                    if (e.xcreatewindow.parent != e.xcreatewindow.window && e.xcreatewindow.parent == host
                        && e.xcreatewindow.window != client)
                    {
                ...
                case ClientMessage:
                    ...
                    {
                        handleXembedCmd((::Time)e.xclient.data.l[0], e.xclient.data.l[1], e.xclient.data.l[2],
            ...
            if (eventArg != nullptr)
            {
                ...
                if (auto w = e.xany.window)
                    for (auto* widget : getWidgets())
                        if (w == widget->host || w == widget->client) return widget->handleX11Event(e);
            }
            else
            {
                for (auto* widget : getWidgets())
                    if (widget->owner.getPeer() == p) widget->peerChanged(nullptr);
            }
        ...
        static Window getCurrentFocusWindow(ComponentPeer* p)
        {
            ...
            {
                for (auto* widget : getWidgets())
                    if (widget->owner.getPeer() == p && widget->owner.hasKeyboardFocus(false)) return widget->client;
            }
    ...
    void XEmbedComponent::focusGained(FocusChangeType changeType)
    {
        pimpl->focusGained(changeType);
    }
    void XEmbedComponent::focusLost(FocusChangeType changeType)
    {
        pimpl->focusLost(changeType);
    }
    void XEmbedComponent::broughtToFront()
    {
        pimpl->broughtToFront();
    }
    unsigned long XEmbedComponent::getHostWindowID()
    {
