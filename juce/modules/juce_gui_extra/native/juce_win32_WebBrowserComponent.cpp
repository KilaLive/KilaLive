...
namespace juce
{
    ...
    public:
        ...
        ~Pimpl()
        {
            if (connectionPoint != nullptr) connectionPoint->Unadvise(adviseCookie);

            if (browser != nullptr) browser->Release();
        }
    ...
    private:
        ...
        //==============================================================================
        struct EventHandler : public ComBaseClassHelper<IDispatch>, public ComponentMovementWatcher
        {
            EventHandler(WebBrowserComponent& w)
                : ComponentMovementWatcher(&w)
                , owner(w)
            {
            }
            ...
            JUCE_COMRESULT Invoke(DISPID      dispIdMember, REFIID /*riid*/, LCID /*lcid*/, WORD /*wFlags*/,
                    ...
                    *pDispParams->rgvarg[0].pboolVal = VARIANT_FALSE;
                    ...
                    if (statusCode < 0)
                    {
                        ...
                        auto   size          = FormatMessage(
                            ...
                            nullptr, statusCode, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (LPTSTR)&messageBuffer, 0,
                            nullptr);

                        String message(messageBuffer, size);
                        LocalFree(messageBuffer);

                        if (!owner.pageLoadHadNetworkError(message)) *pDispParams->rgvarg[0].pboolVal = VARIANT_TRUE;
                    }
        ...
        private:
            ...
            {
                return (v->vt & VT_BYREF) != 0 ? *v->pbstrVal : v->bstrVal;
            }
    ...
    void WebBrowserComponent::goToURL(const String& url, const StringArray* headers, const MemoryBlock* postData)
    {
        ...
        if (browser->browser == nullptr) checkWindowAssociation();
    ...
    {
        if (browser->browser != nullptr) browser->browser->Stop();
    }
    ...
    void WebBrowserComponent::goBack()
    {
        ...
        if (browser->browser != nullptr) browser->browser->GoBack();
    }
    ...
    void WebBrowserComponent::goForward()
    {
        ...
        if (browser->browser != nullptr) browser->browser->GoForward();
    }

    void WebBrowserComponent::refresh()
    {
        if (browser->browser != nullptr) browser->browser->Refresh();
    }
            ...
            for (;;)
            {
                ...
                if (::FindNextUrlCacheEntryA(urlCacheHandle, entry.getData(), &entrySize) == 0)
                {
                    if (GetLastError() == ERROR_INSUFFICIENT_BUFFER)
                    {
                        ...
                        if (::FindNextUrlCacheEntryA(urlCacheHandle, entry.getData(), &entrySize) != 0) continue;
                    }
