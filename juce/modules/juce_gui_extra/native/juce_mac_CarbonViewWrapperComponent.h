...
namespace juce
{
    ...
    */
    class CarbonViewWrapperComponent : public Component, public ComponentMovementWatcher, public Timer
    {
    public:
        ...
        {
            if (embeddedView == 0) return false;
        ...
        void createWindow()
        {
            if (wrapperWindow == nil)
            {
                ...
                if (wrapperWindow == 0) return;

                carbonWindow = [[NSWindow alloc] initWithWindowRef:wrapperWindow];

                [getOwnerWindow() addChildWindow:carbonWindow ordered:NSWindowAbove];
                ...
                if (carbonWindow.childWindows.count > 0)
                    if (NSWindow* floatingChildWindow = [[carbonWindow childWindows] objectAtIndex:0])
                        [getOwnerWindow() addChildWindow:floatingChildWindow ordered:NSWindowAbove];
                ...
                InstallWindowEventHandler(wrapperWindow, upp, sizeof(windowEventTypes) / sizeof(EventTypeSpec),
        ...
        void setOurSizeToEmbeddedViewSize()
        {
            ...
            if (getEmbeddedViewSize(w, h))
            {
                if (w != getWidth() || h != getHeight())
                {
                    ...
                    if (Component* p = getParentComponent()) p->setSize(w, h);
                }
                else
                {
        ...
        void componentMovedOrResized(Component& component, bool wasMoved, bool wasResized) override
        {
            ...
            if (&component == getTopLevelComponent()) setEmbeddedWindowToOurSize();
        }
        ...
        OSStatus carbonEventHandler(EventHandlerCallRef /*nextHandlerRef*/, EventRef event)
        {
            ...
            case kEventWindowGetClickActivation:
            {
                ...
                SetEventParameter(event, kEventParamClickActivation, typeClickActivationResult, sizeof(ClickActivationResult),
                                  &howToHandleClick);

                if (embeddedView != 0) HIViewSetNeedsDisplay(embeddedView, true);
