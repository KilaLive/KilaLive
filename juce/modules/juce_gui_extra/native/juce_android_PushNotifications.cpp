...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD) FIELD(extras, "extras", "Landroid/os/Bundle;")
...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD)                                                          \
    METHOD(addData, "addData",                                                                                               \
           "(Ljava/lang/String;Ljava/lang/String;)Lcom/google/firebase/messaging/RemoteMessage$Builder;")                    \
...
#undef JNI_CLASS_MEMBERS
    ...
    bool PushNotifications::Notification::isValid() const noexcept
    {
        ...
        if (apiAtLeast26) return isValidForPreApi26 && channelId.isNotEmpty();
    ...
    struct PushNotifications::Pimpl
    {
        ...
        void sendLocalNotification(const PushNotifications::Notification& n)
        {
            ...
            if (notificationManager.get() != 0)
            {
                ...
                env->CallVoidMethod(notificationManager.get(), NotificationManagerBase.notify, tag.get(), id,
                                    notification.get());
            }
        }
        ...
        void getDeliveredNotifications() const
        {
            ...
            if (notificationManager.get() != 0)
            {
                auto statusBarNotifications = LocalRef<jobjectArray>((jobjectArray)env->CallObjectMethod(
                    notificationManager, NotificationManagerApi23.getActiveNotifications));

                const int numNotifications = env->GetArrayLength(statusBarNotifications.get());

                for (int i = 0; i < numNotifications; ++i)
                {
                    auto statusBarNotification =
                        LocalRef<jobject>(env->GetObjectArrayElement(statusBarNotifications.get(), (jsize)i));
                    auto notification = LocalRef<jobject>(
                        env->CallObjectMethod(statusBarNotification, StatusBarNotification.getNotification));
        ...
        void notifyListenersAboutLocalNotification(const LocalRef<jobject>& intent)
        {
            ...
            else if (actionString.contains(notificationButtonActionString))
            {
                ...
                auto actionTitle = actionString
                                       .fromLastOccurrenceOf(prefix, false, false)  // skip prefix
            ...
            else if (actionString.contains(notificationTextInputActionString))
            {
                ...
                auto actionIndex =
                    actionString.fromLastOccurrenceOf(prefix, false, false).upToFirstOccurrenceOf(".", false, false);
                auto resultKeyString = javaString(actionTitle + actionIndex);

                auto remoteInputResult = LocalRef<jobject>(
                    env->CallStaticObjectMethod(RemoteInput, RemoteInput.getResultsFromIntent, intent.get()));
                String responseString;

                if (remoteInputResult.get() != 0)
                {
                    auto charSequence = LocalRef<jobject>(
                        env->CallObjectMethod(remoteInputResult, JavaBundle.getCharSequence, resultKeyString.get()));
                    auto responseStringRef =
                        LocalRef<jstring>((jstring)env->CallObjectMethod(charSequence, JavaCharSequence.toString));
                    responseString = juceString(responseStringRef.get());
                }

                owner.listeners.call(
                    [&](Listener& l) { l.handleNotificationAction(true, notification, actionTitle, responseString); });
            }
#endif
        }
        ...
        String getDeviceToken() const
        {
            ...
            auto instanceId =
                LocalRef<jobject>(env->CallStaticObjectMethod(FirebaseInstanceId, FirebaseInstanceId.getInstance));
        ...
        void notifyListenersTokenRefreshed(const String& token)
        {
            ...
            MessageManager::callAsync(
                [this, token] { owner.listeners.call([&](Listener& l) { l.deviceTokenRefreshed(token); }); });
#else
            ignoreUnused(token);
#endif
        }
        ...
        void subscribeToTopic(const String& topic)
        {
            ...
            auto firebaseMessaging =
                LocalRef<jobject>(env->CallStaticObjectMethod(FirebaseMessaging, FirebaseMessaging.getInstance));
        ...
        void sendUpstreamMessage(const String& serverSenderId, const String& collapseKey, const String& messageId,
                                 const String& messageType, int timeToLive, const StringPairArray& additionalData)
        {
#if defined(JUCE_FIREBASE_MESSAGING_SERVICE_CLASSNAME)
            auto* env = getEnv();

            auto messageBuilder = LocalRef<jobject>(env->NewObject(RemoteMessageBuilder, RemoteMessageBuilder.constructor,
            ...
            for (const auto& key : keys)
                env->CallObjectMethod(messageBuilder, RemoteMessageBuilder.addData, javaString(key).get(),
        ...
        {
#if defined(JUCE_FIREBASE_MESSAGING_SERVICE_CLASSNAME)
            MessageManager::callAsync([this] { owner.listeners.call([](Listener& l) { l.remoteNotificationsDeleted(); }); });
#endif
        }
        ...
        static LocalRef<jobject> getNotificationManager()
        {
            ...
            return LocalRef<jobject>(
                env->CallObjectMethod(android.activity, JuceAppActivity.getSystemService, javaString("notification").get()));
        }
        ...
        static LocalRef<jobject> createNotificationBuilder(const PushNotifications::Notification& n)
        {
            ...
            if (builderClass == 0) return LocalRef<jobject>(0);
            ...
            if (apiAtLeast26)
                builderConstructor =
                    env->GetMethodID(builderClass, "<init>", "(Landroid/content/Context;Ljava/lang/String;)V");
            else
                builderConstructor = env->GetMethodID(builderClass, "<init>", "(Landroid/content/Context;)V");

            jassert(builderConstructor != 0);

            if (builderConstructor == 0) return LocalRef<jobject>(0);

            if (apiAtLeast26)
                return LocalRef<jobject>(
                    env->NewObject(builderClass, builderConstructor, android.activity.get(), javaString(n.channelId).get()));
        ...
        static void setupRequiredFields(const PushNotifications::Notification& n, LocalRef<jobject>& notificationBuilder)
        {
            ...
            auto notifyIntent  = LocalRef<jobject>(env->NewObject(AndroidIntent, AndroidIntent.constructorWithContextAndClass,
                                                                 android.activity.get(), activityClass.get()));

            auto packageNameString =
                LocalRef<jstring>((jstring)(android.activity.callObjectMethod(JuceAppActivity.getPackageName)));
            auto actionStringSuffix = javaString(".JUCE_NOTIFICATION." + n.identifier);
            auto actionString       = LocalRef<jstring>(
                (jstring)env->CallObjectMethod(packageNameString, JavaString.concat, actionStringSuffix.get()));
            ...
            auto notifyPendingIntent = LocalRef<jobject>(env->CallStaticObjectMethod(
                AndroidPendingIntent, AndroidPendingIntent.getActivity, android.activity.get(), 1002, notifyIntent.get(), 0));
            ...
            if (n.publicVersion != nullptr)
            {
                ...
                auto publicVersion =
                    LocalRef<jobject>(env->CallObjectMethod(publicNotificationBuilder, NotificationBuilderApi16.build));
        ...
        static LocalRef<jobject> juceNotificationToBundle(const PushNotifications::Notification& n)
        {
            ...
            env->CallVoidMethod(bundle, JavaBundle.putString, javaString("soundToPlay").get(),
                                javaString(n.soundToPlay.toString(true)).get());
            env->CallVoidMethod(bundle, JavaBundle.putBundle, javaString("properties").get(),
                                varToBundleWithPropertiesString(n.properties).get());
            ...
            env->CallVoidMethod(bundle, JavaBundle.putBoolean, javaString("progressIndeterminate").get(),
            ...
            env->CallVoidMethod(bundle, JavaBundle.putString, javaString("groupSortKey").get(),
                                javaString(n.groupSortKey).get());
            ...
            env->CallVoidMethod(bundle, JavaBundle.putInt, javaString("ledBlinkPatternMsToBeOn").get(),
                                n.ledBlinkPattern.msToBeOn);
            env->CallVoidMethod(bundle, JavaBundle.putInt, javaString("ledBlinkPatternMsToBeOff").get(),
            ...
            if (size > 0)
            {
                ...
                for (int i = 0; i < size; ++i) elements[i] = (jlong)n.vibrationPattern[i];
        ...
        static void setupOptionalFields(const PushNotifications::Notification& n, LocalRef<jobject>& notificationBuilder)
        {
            ...
            if (n.subtitle.isNotEmpty())
                env->CallObjectMethod(notificationBuilder, NotificationBuilderBase.setContentInfo,
                                      javaString(n.subtitle).get());
            ...
            {
                env->CallObjectMethod(notificationBuilder, NotificationBuilderBase.setSound,
                                      juceUrlToAndroidUri(n.soundToPlay).get());
            }

            if (n.largeIcon.isValid())
                env->CallObjectMethod(notificationBuilder, NotificationBuilderBase.setLargeIcon,
                                      imagetoJavaBitmap(n.largeIcon).get());
            ...
            {
                env->CallObjectMethod(notificationBuilder, NotificationBuilderBase.setLights, n.ledColour.getARGB(),
                                      n.ledBlinkPattern.msToBeOn, n.ledBlinkPattern.msToBeOff);
            }
            ...
            env->CallObjectMethod(notificationBuilder, NotificationBuilderBase.setProgress, n.progress.max,
                                  n.progress.current, n.progress.indeterminate);
            ...
            if (n.groupSortKey.isNotEmpty())
                env->CallObjectMethod(notificationBuilder, NotificationBuilderApi20.setSortKey,
                                      javaString(n.groupSortKey).get());
            ...
            if (categoryString.isNotEmpty())
                env->CallObjectMethod(notificationBuilder, NotificationBuilderApi21.setCategory,
                                      javaString(categoryString).get());
...
#if __ANDROID_API__ >= 24
            const bool useChronometer = n.timestampVisibility == PushNotifications::Notification::chronometer;
            const bool useCountDownChronometer =
                n.timestampVisibility == PushNotifications::Notification::countDownChronometer;

            env->CallObjectMethod(notificationBuilder, NotificationBuilderApi24.setChronometerCountDown,
                                  useCountDownChronometer);
            env->CallObjectMethod(notificationBuilder, NotificationBuilderApi16.setUsesChronometer,
                                  useChronometer | useCountDownChronometer);
#endif
                                                     ...
                                                     LocalRef<jobject>&                     notificationBuilder)
        {
            ...
            auto activityClass = LocalRef<jobject>(env->CallObjectMethod(android.activity, JavaObject.getClass));
            auto deleteIntent  = LocalRef<jobject>(env->NewObject(AndroidIntent, AndroidIntent.constructorWithContextAndClass,
                                                                 android.activity.get(), activityClass.get()));
            ...
            auto deletePendingIntent = LocalRef<jobject>(env->CallStaticObjectMethod(
                AndroidPendingIntent, AndroidPendingIntent.getActivity, android.activity.get(), 1002, deleteIntent.get(), 0));
        ...
        static void setupActions(const PushNotifications::Notification& n, LocalRef<jobject>& notificationBuilder)
        {
            ...
            for (const auto& action : n.actions)
            {
                auto activityClass = LocalRef<jobject>(env->CallObjectMethod(android.activity, JavaObject.getClass));
                auto notifyIntent =
                    LocalRef<jobject>(env->NewObject(AndroidIntent, AndroidIntent.constructorWithContextAndClass,
                                                     android.activity.get(), activityClass.get()));
                ...
                auto packageNameString =
                    LocalRef<jstring>((jstring)(android.activity.callObjectMethod(JuceAppActivity.getPackageName)));
                const String notificationActionString =
                    isTextStyle ? ".JUCE_NOTIFICATION_TEXT_INPUT_ACTION." : ".JUCE_NOTIFICATION_BUTTON_ACTION.";
                auto actionStringSuffix =
                    javaString(notificationActionString + n.identifier + "." + String(actionIndex) + "." + action.title);
                auto actionString = LocalRef<jstring>(
                    (jstring)env->CallObjectMethod(packageNameString, JavaString.concat, actionStringSuffix.get()));
                ...
                auto notifyPendingIntent =
                    LocalRef<jobject>(env->CallStaticObjectMethod(AndroidPendingIntent, AndroidPendingIntent.getActivity,
                                                                  android.activity.get(), 1002, notifyIntent.get(), 0));
...
#if __ANDROID_API__ >= 20
                auto actionBuilder =
                    LocalRef<jobject>(env->NewObject(NotificationActionBuilder, NotificationActionBuilder.constructor, iconId,
                                                     javaString(action.title).get(), notifyPendingIntent.get()));
                ...
                if (isTextStyle)
                {
                    ...
                    auto remoteInputBuilder = LocalRef<jobject>(
                        env->NewObject(RemoteInputBuilder, RemoteInputBuilder.constructor, resultKey.get()));

                    if (!action.textInputPlaceholder.isEmpty())
                        env->CallObjectMethod(remoteInputBuilder, RemoteInputBuilder.setLabel,
                                              javaString(action.textInputPlaceholder).get());
...
#else
                env->CallObjectMethod(notificationBuilder, NotificationBuilderApi16.addAction, iconId,
                                      javaString(action.title).get(), notifyPendingIntent.get());
#endif
        ...
        static LocalRef<jobject> juceUrlToAndroidUri(const URL& url)
        {
            ...
            auto uriString =
                LocalRef<jstring>((jstring)env->CallObjectMethod(schemeString, JavaString.concat, packageNameString.get()));
        ...
        static LocalRef<jobject> imagetoJavaBitmap(const Image& image)
        {
            ...
            auto bitmapConfig = LocalRef<jobject>(
                env->CallStaticObjectMethod(AndroidBitmapConfig, AndroidBitmapConfig.valueOf, javaString("ARGB_8888").get()));

            auto bitmap = LocalRef<jobject>(env->CallStaticObjectMethod(
                AndroidBitmap, AndroidBitmap.createBitmap, image.getWidth(), image.getHeight(), bitmapConfig.get()));
        ...
        static String typeToCategory(PushNotifications::Notification::Type t)
        {
            switch (t)
            {
            case PushNotifications::Notification::unspecified:
                return {};
            case PushNotifications::Notification::alarm:
                return "alarm";
            case PushNotifications::Notification::call:
                return "call";
            case PushNotifications::Notification::email:
                return "email";
            case PushNotifications::Notification::error:
                return "err";
            case PushNotifications::Notification::event:
                return "event";
            case PushNotifications::Notification::message:
                return "msg";
            case PushNotifications::Notification::taskProgress:
                return "progress";
            case PushNotifications::Notification::promo:
                return "promo";
            case PushNotifications::Notification::recommendation:
                return "recommendation";
            case PushNotifications::Notification::reminder:
                return "reminder";
            case PushNotifications::Notification::service:
                return "service";
            case PushNotifications::Notification::social:
                return "social";
            case PushNotifications::Notification::status:
                return "status";
            case PushNotifications::Notification::system:
                return "sys";
            case PushNotifications::Notification::transport:
                return "transport";
            }
        ...
        static var bundleWithPropertiesStringToVar(const LocalRef<jobject>& bundle)
        {
            ...
            auto varString = LocalRef<jstring>(
                (jstring)env->CallObjectMethod(bundle, JavaBundle.getString, javaString("properties").get()));
        ...
        static PushNotifications::Notification localNotificationBundleToJuceNotification(const LocalRef<jobject>& bundle)
        {
            ...
            if (bundle.get() != 0)
            {
                ...
                n.lockScreenAppearance = (PushNotifications::Notification::LockScreenAppearance)getIntFromBundle(
                    env, "lockScreenAppearance", bundle);
                ...
                n.timestampVisibility = (PushNotifications::Notification::TimestampVisibility)getIntFromBundle(
                    env, "timestampVisibility", bundle);
                n.badgeIconType =
                    (PushNotifications::Notification::BadgeIconType)getIntFromBundle(env, "badgeIconType", bundle);
                n.groupAlertBehaviour = (PushNotifications::Notification::GroupAlertBehaviour)getIntFromBundle(
                    env, "groupAlertBehaviour", bundle);
        ...
        static Array<int> getLongArrayFromBundle(JNIEnv* env, const String& key, const LocalRef<jobject>& bundle)
        {
            ...
            {
                auto array =
                    LocalRef<jlongArray>((jlongArray)env->CallObjectMethod(bundle, JavaBundle.getLongArray, keyString.get()));
                ...
                for (int i = 0; i < size; ++i) resultArray.add((int)*elements++);
        ...
        static PushNotifications::Notification javaNotificationToJuceNotification(const LocalRef<jobject>& notification)
        {
            ...
            auto notificationData =
                LocalRef<jobject>(env->CallObjectMethod(extras, JavaBundle.getBundle, javaString("notificationData").get()));
        ...
        static var bundleToVar(const LocalRef<jobject>& bundle)
        {
            if (bundle.get() != 0)
            {
                ...
                for (;;)
                {
                    if (!env->CallBooleanMethod(iterator, JavaIterator.hasNext)) break;
                    ...
                    const var value = juceString(classAsString) == "java.lang.Bundle" ? bundleToVar(object)
                                                                                      : var(juceString(objectAsString.get()));
        ...
        static PushNotifications::Notification firebaseRemoteNotificationToJuceNotification(jobject remoteNotification)
        {
            ...
            auto collapseKey =
                LocalRef<jstring>((jstring)env->CallObjectMethod(remoteNotification, RemoteMessage.getCollapseKey));
            auto from = LocalRef<jstring>((jstring)env->CallObjectMethod(remoteNotification, RemoteMessage.getFrom));
            auto messageId =
                LocalRef<jstring>((jstring)env->CallObjectMethod(remoteNotification, RemoteMessage.getMessageId));
            auto messageType =
                LocalRef<jstring>((jstring)env->CallObjectMethod(remoteNotification, RemoteMessage.getMessageType));
            ...
            {
                auto body =
                    LocalRef<jstring>((jstring)env->CallObjectMethod(notification, RemoteMessageNotification.getBody));
                auto bodyLocalizationKey = LocalRef<jstring>(
                    (jstring)env->CallObjectMethod(notification, RemoteMessageNotification.getBodyLocalizationKey));
                auto clickAction =
                    LocalRef<jstring>((jstring)env->CallObjectMethod(notification, RemoteMessageNotification.getClickAction));
                auto color =
                    LocalRef<jstring>((jstring)env->CallObjectMethod(notification, RemoteMessageNotification.getColor));
                auto icon =
                    LocalRef<jstring>((jstring)env->CallObjectMethod(notification, RemoteMessageNotification.getIcon));
                auto sound =
                    LocalRef<jstring>((jstring)env->CallObjectMethod(notification, RemoteMessageNotification.getSound));
                auto tag = LocalRef<jstring>((jstring)env->CallObjectMethod(notification, RemoteMessageNotification.getTag));
                auto title =
                    LocalRef<jstring>((jstring)env->CallObjectMethod(notification, RemoteMessageNotification.getTitle));
                auto titleLocalizationKey = LocalRef<jstring>(
                    (jstring)env->CallObjectMethod(notification, RemoteMessageNotification.getTitleLocalizationKey));
                auto link = LocalRef<jobject>(env->CallObjectMethod(notification, RemoteMessageNotification.getLink));

                auto bodyLocalizationArgs = LocalRef<jobjectArray>(
                    (jobjectArray)env->CallObjectMethod(notification, RemoteMessageNotification.getBodyLocalizationArgs));
                auto titleLocalizationArgs = LocalRef<jobjectArray>(
                    (jobjectArray)env->CallObjectMethod(notification, RemoteMessageNotification.getTitleLocalizationArgs));
                ...
                propertiesDynamicObject->setProperty("titleLocalizationArgs",
                                                     jobjectArrayToStringArray(titleLocalizationArgs));
                propertiesDynamicObject->setProperty(
                    "link",
                    link.get() != 0 ? juceString((jstring)env->CallObjectMethod(link, AndroidUri.toString)) : String());
            }
        ...
        void setupChannels(const Array<ChannelGroup>& groups, const Array<Channel>& channels)
        {
            ...
            if (env->CallStaticIntMethod(JuceAppActivity, JuceAppActivity.getAndroidSDKVersion) < 26) return;

            auto notificationManager = getNotificationManager();

            jassert(notificationManager.get() != 0);

            if (notificationManager.get() == 0) return;
            ...
            for (const auto& g : groups)
            {
                ...
                {
                    auto group =
                        LocalRef<jobject>(env->NewObject(NotificationChannelGroup, NotificationChannelGroup.constructor,
                                                         javaString(g.identifier).get(), javaString(g.name).get()));
                    env->CallVoidMethod(notificationManager, NotificationManagerApi26.createNotificationChannelGroup,
                                        group.get());
                }
            }
            ...
            for (const auto& c : channels)
            {
                ...
                if (c.identifier.isEmpty() || c.name.isEmpty() || c.groupId.isEmpty()) continue;

                auto channel =
                    LocalRef<jobject>(env->NewObject(NotificationChannel, NotificationChannel.constructor,
                ...
                if (size > 0)
                {
                    ...
                    for (int i = 0; i < size; ++i) elements[i] = (jlong)c.vibrationPattern[i];
                ...
                auto audioAttributesBuilder =
                    LocalRef<jobject>(env->NewObject(AudioAttributesBuilder, AudioAttributesBuilder.constructor));
                ...
                auto audioAttributes =
                    LocalRef<jobject>(env->CallObjectMethod(audioAttributesBuilder, AudioAttributesBuilder.build));
                env->CallVoidMethod(channel, NotificationChannel.setSound, juceUrlToAndroidUri(c.soundToPlay).get(),
                                    audioAttributes.get());
        ...
        static bool intentActionContainsAnyOf(jobject intent, const StringArray& strings, bool includePackageName)
        {
            ...
            String packageName =
                includePackageName
                    ? juceString((jstring)env->CallObjectMethod(android.activity, JuceAppActivity.getPackageName))
                    : String{};

            String intentAction = juceString((jstring)env->CallObjectMethod(intent, AndroidIntent.getAction));

            for (const auto& string : strings)
                if (intentAction.contains(packageName + string)) return true;
        ...
        {
            return intentActionContainsAnyOf(
                intent, {".JUCE_NOTIFICATION.", ".JUCE_NOTIFICATION_BUTTON_ACTION.", ".JUCE_NOTIFICATION_TEXT_INPUT_ACTION."},
                true);
        }
        ...
        static bool isRemoteNotificationIntent(jobject intent)
        {
            ...
            int categoriesNum = categories != 0 ? env->CallIntMethod(categories, JavaSet.size) : 0;

            if (categoriesNum == 0) return false;

            if (!env->CallBooleanMethod(categories, JavaSet.contains, javaString("android.intent.category.LAUNCHER").get()))
                return false;

            if (!intentActionContainsAnyOf(intent, StringArray("android.intent.action.MAIN"), false)) return false;
    ...
    bool juce_handleNotificationIntent(void* intent)
    {
        ...
        {
            if (instance) instance->pimpl->notifyListenersAboutLocalNotificationDeleted(LocalRef<jobject>((jobject)intent));

            return true;
        }
        else if (PushNotifications::Pimpl::isLocalNotificationIntent((jobject)intent))
        {
            if (instance) instance->pimpl->notifyListenersAboutLocalNotification(LocalRef<jobject>((jobject)intent));
    ...
    {
        if (auto* instance = PushNotifications::getInstanceWithoutCreating())
            instance->pimpl->notifyListenersAboutRemoteNotificationFromService(
                LocalRef<jobject>(static_cast<jobject>(remoteMessage)));
    }
    ...
    {
        if (auto* instance = PushNotifications::getInstanceWithoutCreating())
            instance->pimpl->notifyListenersAboutUpstreamMessageSendingError(
                LocalRef<jstring>(static_cast<jstring>(messageId)), LocalRef<jstring>(static_cast<jstring>(error)));
    }
