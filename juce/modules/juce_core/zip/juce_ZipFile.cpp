...
namespace juce
{
    ...
    struct ZipFile::ZipEntryHolder
    {
        ZipEntryHolder(const char* buffer, int fileNameLen)
        {
            ...
            entry.fileTime =
                parseFileTime(readUnalignedLittleEndianShort(buffer + 12), readUnalignedLittleEndianShort(buffer + 14));
                ...
                if (readUnalignedLittleEndianInt(buffer + i) == 0x06054b50)
                {
                    ...
                    if (offset >= 4)
                    {
                        ...
                        if (in.readInt() != 0x02014b50)
                        {
                            ...
                            if (in.readInt() == 0x02014b50) offset -= 4;
                        }
                    }
    ...
    struct ZipFile::ZipInputStream : public InputStream
    {
            ...
            , inputStream(zf.inputStream)
        {
            ...
            if (inputStream != nullptr && inputStream->setPosition(zei.streamOffset) && inputStream->read(buffer, 30) == 30
                && ByteOrder::littleEndianInt(buffer) == 0x04034b50)
            {
                headerSize = 30 + ByteOrder::littleEndianShort(buffer + 26) + ByteOrder::littleEndianShort(buffer + 28);
            }
        }
        ...
        {
#if JUCE_DEBUG
            if (inputStream != nullptr && inputStream == file.inputStream) file.streamCounter.numOpenStreams--;
#endif
        }
        ...
        int read(void* buffer, int howMany) override
        {
            ...
            if (inputStream == nullptr) return 0;
    ...
    ZipFile::ZipFile(InputStream* stream, bool deleteStreamWhenDestroyed)
        : inputStream(stream)
    {
        if (deleteStreamWhenDestroyed) streamToDelete.reset(inputStream);
    ...
    {
        if (auto* zei = entries[index]) return &(zei->entry);
    ...
    int ZipFile::getIndexOfFileName(const String& fileName, bool ignoreCase) const noexcept
    {
        for (int i = 0; i < entries.size(); ++i)
        {
            ...
            if (ignoreCase ? entryFilename.equalsIgnoreCase(fileName) : entryFilename == fileName) return i;
        }
    ...
    InputStream* ZipFile::createStreamForEntry(const int index)
    {
        ...
        if (auto* zei = entries[index])
        {
            ...
            if (zei->isCompressed)
            {
                stream = new GZIPDecompressorInputStream(stream, true, GZIPDecompressorInputStream::deflateFormat,
    ...
    {
        for (int i = 0; i < entries.size(); ++i)
            if (&entries.getUnchecked(i)->entry == &entry) return createStreamForEntry(i);
            ...
            if (centralDirectoryPos >= 0 && centralDirectoryPos < in->getTotalLength())
            {
                ...
                if (in->readIntoMemoryBlock(headerData, (ssize_t)size) == size)
                {
                    ...
                    for (int i = 0; i < numEntries; ++i)
                    {
                        ...
                        if (pos + 46 + fileNameLen > size) break;

                        entries.add(new ZipEntryHolder(buffer, fileNameLen));

                        pos += 46 + fileNameLen + readUnalignedLittleEndianShort(buffer + 30)
    ...
    Result ZipFile::uncompressEntry(int index, const File& targetDirectory, bool shouldOverwriteFiles)
    {
        ...
        if (entryPath.isEmpty()) return Result::ok();
        ...
        if (in == nullptr) return Result::fail("Failed to open the zip file for reading");

        if (targetFile.exists())
        {
            if (!shouldOverwriteFiles) return Result::ok();
...

        {
            ...
            if (out.failedToOpen()) return Result::fail("Failed to write to target file: " + targetFile.getFullPathName());
    ...
    struct ZipFile::Builder::Item
    {
        ...
        bool writeData(OutputStream& target, const int64 overallStartPosition)
        {
            ...
            if (compressionLevel > 0)
            {
                ...
                if (!writeSource(compressor)) return false;
            }
            else
            {
                if (!writeSource(compressedData)) return false;
            }
    ...
    {
        items.add(
            new Item(file, nullptr, compression, path.isEmpty() ? file.getFileName() : path, file.getLastModificationTime()));
    }
    ...
    bool ZipFile::Builder::writeToStream(OutputStream& target, double* const progress) const
    {
        ...
        {
            if (progress != nullptr) *progress = (i + 0.5) / items.size();

            if (!items.getUnchecked(i)->writeData(target, fileStart)) return false;
        }

        auto directoryStart = target.getPosition();

        for (auto* item : items)
            if (!item->writeDirectoryEntry(target)) return false;
