...
namespace juce
{
    ...
    public:
            ...
            : compLevel((compressionLevel < 0 || compressionLevel > 9) ? -1 : compressionLevel)
        {
            ...
            streamIsValid =
                (deflateInit2(&stream, compLevel, Z_DEFLATED, windowBits != 0 ? windowBits : MAX_WBITS, 8, strategy) == Z_OK);
        }

        ~GZIPCompressorHelper()
        {
            if (streamIsValid) zlibNamespace::deflateEnd(&stream);
        }
        ...
        bool write(const uint8* data, size_t dataSize, OutputStream& out)
        {
            ...
            while (dataSize > 0)
                if (!doNextBlock(data, dataSize, out, Z_NO_FLUSH)) return false;
        ...
        void finish(OutputStream& out)
        {
            ...
            while (!finished) doNextBlock(data, dataSize, out, Z_FINISH);
        }
    ...
    private:
        ...
        bool doNextBlock(const uint8*& data, size_t& dataSize, OutputStream& out, const int flushMode)
        {
            ...
            if (streamIsValid)
            {
                ...
                auto result    = isFirstDeflate ? deflateParams(&stream, compLevel, strategy) : deflate(&stream, flushMode);
    ...
    GZIPCompressorOutputStream::GZIPCompressorOutputStream(OutputStream* out, int compressionLevel, bool deleteDestStream,
                                                           int windowBits)
            ...
            for (int i = 100; --i >= 0;)
            {
...

                {
                    ...
                    for (int j = rng.nextInt(100); --j >= 0;)
                    {
                        ...
                        for (int k = (int)data.getSize(); --k >= 0;) data[k] = (char)rng.nextInt(255);
                ...
                expectEquals((int)uncompressed.getDataSize(), (int)original.getDataSize());

                if (original.getDataSize() == uncompressed.getDataSize())
                    expect(memcmp(uncompressed.getData(), original.getData(), original.getDataSize()) == 0);
            }
        }
    };
