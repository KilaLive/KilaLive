...
#define JUCE_DISABLE_JUCE_VERSION_PRINTING 1
    ...
    StringArray SystemStats::getDeviceIdentifiers()
    {
        ...
        else
        {
            ...
            for (auto& address : addresses) ids.add(address.toString());
        }
    ...
    struct CPUInformation
    {
        ...
        bool hasMMX = false, hasSSE = false, hasSSE2 = false, hasSSE3 = false, has3DNow = false, hasSSSE3 = false,
             hasSSE41 = false, hasSSE42 = false, hasAVX = false, hasAVX2 = false, hasNeon = false;
    };
    ...
    bool SystemStats::hasMMX() noexcept
    {
        return getCPUInformation().hasMMX;
    }
    bool SystemStats::has3DNow() noexcept
    {
        return getCPUInformation().has3DNow;
    }
    bool SystemStats::hasSSE() noexcept
    {
        return getCPUInformation().hasSSE;
    }
    bool SystemStats::hasSSE2() noexcept
    {
        return getCPUInformation().hasSSE2;
    }
    bool SystemStats::hasSSE3() noexcept
    {
        return getCPUInformation().hasSSE3;
    }
    bool SystemStats::hasSSSE3() noexcept
    {
        return getCPUInformation().hasSSSE3;
    }
    bool SystemStats::hasSSE41() noexcept
    {
        return getCPUInformation().hasSSE41;
    }
    bool SystemStats::hasSSE42() noexcept
    {
        return getCPUInformation().hasSSE42;
    }
    bool SystemStats::hasAVX() noexcept
    {
        return getCPUInformation().hasAVX;
    }
    bool SystemStats::hasAVX2() noexcept
    {
        return getCPUInformation().hasAVX2;
    }
    bool SystemStats::hasNeon() noexcept
    {
    ...
    String SystemStats::getStackBacktrace()
    {
        ...
        for (int i = 0; i < frames; ++i)
        {
            ...
            if (SymFromAddr(process, (DWORD64)stack[i], &displacement, symbol))
            {
                ...
                if (::SymGetModuleInfo64(process, symbol->ModBase, &moduleInfo)) result << moduleInfo.ModuleName << ": ";
    ...
    bool SystemStats::isRunningInAppExtensionSandbox() noexcept
    {
        ...
        if (firstQuery)
        {
            ...
            if (bundle.isDirectory()) isRunningInAppSandbox = (bundle.getFileExtension() == ".appex");
        }
