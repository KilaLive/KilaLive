...
namespace juce
{
    ...
    {
        if (category.isEmpty()) return getAllTests();

        Array<UnitTest*> unitTests;

        for (auto* test : getAllTests())
            if (test->getCategory() == category) unitTests.add(test);
    ...
    StringArray UnitTest::getAllCategories()
    {
        ...
        for (auto* test : getAllTests())
            if (test->getCategory().isNotEmpty()) categories.addIfNotAlreadyThere(test->getCategory());
    ...
    void UnitTestRunner::runTests(const Array<UnitTest*>& tests, int64 randomSeed)
    {
        ...
        if (randomSeed == 0) randomSeed = Random().nextInt(0x7ffffff);
    ...
    void UnitTestRunner::endTest()
    {
        if (auto* r = results.getLast())
        {
            ...
            {
                String m("FAILED!!  ");
                m << r->failures << (r->failures == 1 ? " test" : " tests") << " failed, out of a total of "
                  << (r->passes + r->failures);
    ...
    {
        {
            ...
            if (failureMessage.isNotEmpty()) message << ": " << failureMessage;
