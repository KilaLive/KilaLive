...
namespace juce
{
        ...
        , outputFile(loggingFile)
    {
        ...
        appendToFile(outputFile,
                     "**** Counter for \"" + name + "\" started at: " + Time::getCurrentTime().toString(true, true));
    }
    ...
    {
        return String((int64)(secs * (secs < 0.01 ? 1000000.0 : 1000.0) + 0.5)) + (secs < 0.01 ? " microsecs" : " millisecs");
    }
    ...
    String PerformanceCounter::Statistics::toString() const
    {
        ...
        s << "Performance count for \"" << name << "\" over " << numRuns << " run(s)" << newLine
          << "Average = " << timeToString(averageSeconds) << ", minimum = " << timeToString(minimumSeconds)
          << ", maximum = " << timeToString(maximumSeconds) << ", total = " << timeToString(totalSeconds);
    ...
    PerformanceCounter::Statistics PerformanceCounter::getStatisticsAndReset()
    {
        ...
        if (s.numRuns > 0) s.averageSeconds = s.totalSeconds / s.numRuns;
