...
namespace juce
{
    namespace TimeHelpers
    {
        ...
        static inline String formatString(const String& format, const std::tm* const tm)
        {
...
#ifdef JUCE_MSVC
            if (tm->tm_year < -1900 || tm->tm_year > 8099) return {};  // Visual Studio's library can only handle 0 -> 9999 AD
#endif
            ...
            for (size_t bufferSize = 256;; bufferSize += 256)
            {
                ...
                if (numChars > 0 || format.isEmpty()) return String(StringType(buffer), StringType(buffer) + (int)numChars);
            }
        }
        ...
        {
            return 24 * 3600 * (daysFrom1970(t.tm_year + 1900, t.tm_mon) + (t.tm_mday - 1)) + 3600 * t.tm_hour + 60 * t.tm_min
                   + t.tm_sec;
        }
               ...
               bool useLocalTime) noexcept
    {
        ...
        millisSinceEpoch = 1000 * (useLocalTime ? (int64)mktime(&t) : TimeHelpers::mktime_utc(t)) + milliseconds;
    }
    ...
    uint32 Time::getMillisecondCounter() noexcept
    {
        ...
        if (now < TimeHelpers::lastMSCounterValue.get())
        {
            ...
            // go backwards..
            if (now < TimeHelpers::lastMSCounterValue.get() - (uint32)1000) TimeHelpers::lastMSCounterValue = now;
        }
        else
        {
    ...
    //==============================================================================
    String Time::toString(const bool includeDate, const bool includeTime, const bool includeSeconds,
                          const bool use24HourClock) const noexcept
    {
        ...
        if (includeTime)
        {
            ...
            result << (use24HourClock ? getHours() : getHoursInAmPmFormat()) << (mins < 10 ? ":0" : ":") << mins;
    ...
    /** Crash when _get_tzname return unicode char and assigned to juce::String
    ...
    String Time::getUTCOffsetString(bool includeSemiColon) const
    {
        if (int seconds = getUTCOffsetSeconds())
        {
            ...
            return String::formatted(includeSemiColon ? "%+03d:%02d" : "%+03d%02d", minutes / 60, minutes % 60);
        }
    ...
    String Time::toISO8601(bool includeDividerCharacters) const
    {
        return String::formatted(
                   includeDividerCharacters ? "%04d-%02d-%02dT%02d:%02d:%06.03f" : "%04d%02d%02dT%02d%02d%06.03f", getYear(),
                   getMonth() + 1, getDayOfMonth(), getHours(), getMinutes(), getSeconds() + getMilliseconds() / 1000.0)
    ...
    static int parseFixedSizeIntAndSkip(String::CharPointerType& t, int numChars, char charToSkip) noexcept
    {
        ...
        if (charToSkip != 0 && *t == (juce_wchar)charToSkip) ++t;
    ...
    Time Time::getCompilationDate()
    {
        ...
        return Time(dateTokens[2].getIntValue(), getMonthNumberForCompileDate(dateTokens[0]), dateTokens[1].getIntValue(),
