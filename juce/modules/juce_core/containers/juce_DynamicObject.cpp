...
namespace juce
{
    ...
    {
        if (auto function = properties[method].getNativeFunction()) return function(args);
    ...
    void DynamicObject::cloneAllProperties()
    {
        for (int i = properties.size(); --i >= 0;)
            if (auto* v = properties.getVarPointerAt(i)) *v = v->clone();
    }
                                    ...
                                    int maximumDecimalPlaces)
    {
        ...
        {
            if (!allOnOneLine) JSONFormatter::writeSpaces(out, indentLevel + JSONFormatter::indentSize);
        ...
        if (!allOnOneLine) JSONFormatter::writeSpaces(out, indentLevel);
