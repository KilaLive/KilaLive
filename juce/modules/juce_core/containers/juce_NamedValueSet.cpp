...
namespace juce
{
    ...
    NamedValueSet::NamedValue::NamedValue(NamedValue&& other) noexcept
        : NamedValue(static_cast<Identifier&&>(other.name), static_cast<var&&>(other.value))
    {
    }
    ...
    bool NamedValueSet::operator==(const NamedValueSet& other) const noexcept
    {
        ...
        if (num != other.values.size()) return false;
        ...
        for (int i = 0; i < num; ++i)
        {
            ...
            {
                if (values.getReference(i).value != other.values.getReference(i).value) return false;
            }
            else
            {
                ...
                for (int j = i; j < num; ++j)
                {
                    if (auto* otherVal = other.getVarPointer(values.getReference(j).name))
                        if (values.getReference(j).value == *otherVal) continue;
    ...
    {
        if (auto* v = getVarPointer(name)) return *v;
    ...
    bool NamedValueSet::set(const Identifier& name, var&& newValue)
    {
        ...
        {
            if (v->equalsWithSameType(newValue)) return false;
    ...
    int NamedValueSet::indexOf(const Identifier& name) const noexcept
    {
        ...
        for (int i = 0; i < numValues; ++i)
            if (values.getReference(i).name == name) return i;
    ...
    {
        if (isPositiveAndBelow(index, values.size())) return values.getReference(index).name;
    ...
    {
        if (isPositiveAndBelow(index, values.size())) return values.getReference(index).value;
