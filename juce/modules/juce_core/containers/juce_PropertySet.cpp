...
namespace juce
{
    ...
    String PropertySet::getValue(StringRef keyName, const String& defaultValue) const noexcept
    {
        ...
        if (index >= 0) return properties.getAllValues()[index];

        return fallbackProperties != nullptr ? fallbackProperties->getValue(keyName, defaultValue) : defaultValue;
    }
    ...
    int PropertySet::getIntValue(StringRef keyName, const int defaultValue) const noexcept
    {
        ...
        if (index >= 0) return properties.getAllValues()[index].getIntValue();

        return fallbackProperties != nullptr ? fallbackProperties->getIntValue(keyName, defaultValue) : defaultValue;
    }
    ...
    double PropertySet::getDoubleValue(StringRef keyName, const double defaultValue) const noexcept
    {
        ...
        if (index >= 0) return properties.getAllValues()[index].getDoubleValue();

        return fallbackProperties != nullptr ? fallbackProperties->getDoubleValue(keyName, defaultValue) : defaultValue;
    }
    ...
    bool PropertySet::getBoolValue(StringRef keyName, const bool defaultValue) const noexcept
    {
        ...
        if (index >= 0) return properties.getAllValues()[index].getIntValue() != 0;

        return fallbackProperties != nullptr ? fallbackProperties->getBoolValue(keyName, defaultValue) : defaultValue;
    }
    ...
    {
        setValue(keyName, xml == nullptr ? var() : var(xml->createDocument("", true)));
    }
    ...
    void PropertySet::addAllPropertiesFrom(const PropertySet& source)
    {
        ...
        for (int i = 0; i < source.properties.size(); ++i)
            setValue(source.properties.getAllKeys()[i], source.properties.getAllValues()[i]);
    }
    ...
    void PropertySet::restoreFromXml(const XmlElement& xml)
    {
        ...
        forEachXmlChildElementWithTagName(xml, e, "VALUE")
        {
            ...
            {
                properties.set(e->getStringAttribute("name"), e->getStringAttribute("val"));
            }
        }

        if (properties.size() > 0) propertyChanged();
    }
