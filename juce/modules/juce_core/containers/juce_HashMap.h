...
namespace juce
{
    ...
    */
    struct DefaultHashFunctions
    {
        ...
        {
            return generateHash((pointer_sized_uint)key, upperLimit);
        }
    };
    ...
    */
    ...
    public:
        ...
        */
        explicit HashMap(int numberOfSlots = defaultHashTableSize, HashFunctionType hashFunction = HashFunctionType())
        ...
        */
        inline ValueType operator[](KeyTypeParameter keyToLookFor) const
        {
            ...
            if (auto* entry = getEntry(getSlot(keyToLookFor), keyToLookFor)) return entry->value;
        ...
        */
        inline ValueType& getReference(KeyTypeParameter keyToLookFor)
        {
            ...
            if (auto* entry = getEntry(firstEntry, keyToLookFor)) return entry->value;

            auto* entry = new HashEntry(keyToLookFor, ValueType(), firstEntry);
            hashSlots.set(hashIndex, entry);
            ++totalNumItems;

            if (totalNumItems > (getNumSlots() * 3) / 2) remapTable(getNumSlots() * 2);
        ...
        bool containsValue(ValueTypeParameter valueToLookFor) const
        {
            ...
            for (auto i = getNumSlots(); --i >= 0;)
                for (auto* entry = hashSlots.getUnchecked(i); entry != nullptr; entry = entry->nextEntry)
                    if (entry->value == valueToLookFor) return true;
        ...
        */
        struct Iterator
        {
            ...
            */
            bool next() noexcept
            {
                if (entry != nullptr) entry = entry->nextEntry;

                while (entry == nullptr)
                {
                    if (index >= hashMap.getNumSlots()) return false;
    ...
    private:
        ...
        static inline HashEntry* getEntry(HashEntry* firstEntry, KeyType keyToLookFor) noexcept
        {
            for (auto* entry = firstEntry; entry != nullptr; entry = entry->nextEntry)
                if (entry->key == keyToLookFor) return entry;
