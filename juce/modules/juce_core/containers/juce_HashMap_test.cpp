...
namespace juce
{
    struct HashMapTest : public UnitTest
    {
        ...
        struct AccessTest
        {
            ...
            static void run(UnitTest& u)
            {
                ...
                for (auto pair : groundTruth.pairs) u.expectEquals(hashMap[pair.key], pair.value);
            }
        };
        ...
        public:
            RandomKeys(int maxUniqueKeys, int seed)
                : r(seed)
            {
                for (int i = 0; i < maxUniqueKeys; ++i) keys.add(generateRandomKey(r));
            }
    ...
    String HashMapTest::RandomKeys<String>::generateRandomKey(Random& rnd)
    {
        ...
        for (int i = 0; i < len; ++i) str += static_cast<char>(rnd.nextInt(95) + 32);
