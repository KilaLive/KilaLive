...
namespace juce
{
    ...
    //==============================================================================
    void AbstractFifo::prepareToWrite(int numToWrite, int& startIndex1, int& blockSize1, int& startIndex2,
    ...
    void AbstractFifo::finishedWrite(int numWritten) noexcept
    {
        ...
        if (newEnd >= bufferSize) newEnd -= bufferSize;

        validEnd = newEnd;
    }

    void AbstractFifo::prepareToRead(int numWanted, int& startIndex1, int& blockSize1, int& startIndex2,
                                     int& blockSize2) const noexcept
    {
        auto vs = validStart.get();
        auto ve = validEnd.get();
    ...
    {
        if (fifo != nullptr) finish(*fifo, blockSize1 + blockSize2);
    }
...
//==============================================================================
    ...
    public:
        AbstractFifoTests()
            : UnitTest("Abstract Fifo", "Containers")
        {
        }
        ...
        struct WriteThread : public Thread
        {
            ...
            void run()
            {
                ...
                while (!threadShouldExit())
                {
                    ...
                    jassert(writer.blockSize1 == 0 || (writer.startIndex1 >= 0 && writer.startIndex1 < fifo.getTotalSize()));
                    jassert(writer.blockSize2 == 0 || (writer.startIndex2 >= 0 && writer.startIndex2 < fifo.getTotalSize()));
