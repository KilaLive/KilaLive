...
namespace juce
{
    ...
    public:
        ...
        virtual bool equals(const ValueUnion& data, const ValueUnion& otherData, const VariantType& otherType) const
            noexcept                                                                   = 0;
    ...
    public:
        ...
        bool equals(const ValueUnion& data, const ValueUnion& otherData, const VariantType& otherType) const noexcept override
        {
            if (otherType.isDouble() || otherType.isString()) return otherType.equals(otherData, data, *this);
    ...
    public:
        ...
        String toString(const ValueUnion& data) const override
        {
            return String(data.doubleValue, 20);
        }
        bool toBool(const ValueUnion& data) const noexcept override
        {
    ...
    private:
        static inline const String* getString(const ValueUnion& data) noexcept
        {
            return reinterpret_cast<const String*>(data.stringValue);
        }
        static inline String* getString(ValueUnion& data) noexcept
        {
            return reinterpret_cast<String*>(data.stringValue);
        }
    };
    ...
    public:
        ...
        void createCopy(ValueUnion& dest, const ValueUnion& source) const override
        {
            ...
            if (dest.objectValue != nullptr) dest.objectValue->incReferenceCount();
        }
        ...
        {
            if (auto* d = original.getDynamicObject()) return d->clone().get();
    ...
    public:
        ...
        {
            if (auto* a = dynamic_cast<RefCountedArray*>(data.objectValue)) return &(a->array);
        ...
        var clone(const var& original) const override
        {
            ...
            if (auto* array = toArray(original.value))
            {
                ...
                for (auto& i : *array) arrayCopy.add(i.clone());
            }
        ...
        void writeToStream(const ValueUnion& data, OutputStream& output) const override
        {
            if (auto* array = toArray(data))
            {
                ...
                for (auto& i : *array) i.writeToStream(buffer);
    ...
    public:
        ...
        void cleanUp(ValueUnion& data) const noexcept override
        {
            if (data.methodValue != nullptr) delete data.methodValue;
        }
        void createCopy(ValueUnion& dest, const ValueUnion& source) const override
        {
    ...
    var::var(const Array<var>& v)
        : type(&VariantType_Array::instance)
    {
    ...
    var::var(const void* v, size_t sz)
        : type(&VariantType_Binary::instance)
    {
        ...
        : type(&VariantType_Array::instance)
    {
        ...
        for (auto& i : v) strings.add(var(i));

        value.objectValue = new VariantType_Array::RefCountedArray(strings);
    }

    var::var(ReferenceCountedObject* const object)
        : type(&VariantType_Object::instance)
    {
        value.objectValue = object;

        if (object != nullptr) object->incReferenceCount();
    }

    var var::undefined() noexcept
    {
        return var(VariantType_Undefined::instance);
    }
    ...
    {
        if (auto* o = getDynamicObject()) return o->getProperty(propertyName);
    ...
    {
        if (auto* o = getDynamicObject()) return o->getProperties().getWithDefault(propertyName, defaultReturnValue);
    ...
    {
        if (auto* o = getDynamicObject()) return o->hasProperty(propertyName);
    ...
    int var::size() const
    {
        if (auto* array = getArray()) return array->size();
    ...
    {
        if (auto* array = getArray()) return array;

        Array<var> tempVar;
        if (!isVoid()) tempVar.add(*this);
    ...
    {
        if (auto* const array = getArray()) array->remove(index);
    }
    ...
    {
        if (auto* const array = getArray()) return array->indexOf(n);
    ...
    var var::readFromStream(InputStream& input)
    {
        ...
        if (numBytes > 0)
        {
            ...
            case varMarker_Array:
            {
                ...
                for (int i = input.readCompressedInt(); --i >= 0;) destArray->add(readFromStream(input));
