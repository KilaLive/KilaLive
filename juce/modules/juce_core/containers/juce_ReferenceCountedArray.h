    ...
    */
    ...
    public:
        ...
        */
        ...
        {
            const typename ReferenceCountedArray<OtherObjectClass, OtherCriticalSection>::ScopedLockType lock(
                other.getLock());
            numUsed = other.size();
            data.setAllocatedSize(numUsed);
            memcpy(data.elements, other.getRawDataPointer(), (size_t)numUsed * sizeof(ObjectClass*));
        ...
        */
        ...
        ReferenceCountedArray& operator=(
            const ReferenceCountedArray<OtherObjectClass, TypeOfCriticalSectionToUse>& other) noexcept
        {
            auto otherCopy = other;
            swapWith(otherCopy);
            return *this;
        ...
        */
        int indexOf(const ObjectClass* objectToLookFor) const noexcept
        {
            ...
            while (e != endPointer)
            {
                if (objectToLookFor == *e) return static_cast<int>(e - data.elements.get());
        ...
        */
        bool contains(const ObjectClass* objectToLookFor) const noexcept
        {
            ...
            while (e != endPointer)
            {
                if (objectToLookFor == *e) return true;
        ...
        */
        ObjectClass* add(ObjectClass* newObject) noexcept
        {
            ...
            if (newObject != nullptr) newObject->incReferenceCount();
        ...
        */
        ...
        {
            if (indexToInsertAt < 0) return add(newObject);

            const ScopedLockType lock(getLock());

            if (indexToInsertAt > numUsed) indexToInsertAt = numUsed;
            ...
            if (numToMove > 0) memmove(e + 1, e, sizeof(ObjectClass*) * (size_t)numToMove);
        ...
        void addArray(const ReferenceCountedArray& arrayToAddFrom, int startIndex = 0, int numElementsToAdd = -1) noexcept
        {
...

            {
                ...
                if (numElementsToAdd > 0)
                {
                    ...
                    while (--numElementsToAdd >= 0) add(arrayToAddFrom.getUnchecked(startIndex++));
                }
            }
        }
        ...
        */
        ...
        int indexOfSorted(ElementComparator& comparator, const ObjectClass* objectToLookFor) const noexcept
        {
            ...
            while (s < e)
            {
                if (comparator.compareElements(objectToLookFor, data.elements[s]) == 0) return s;
        ...
        */
        void remove(int indexToRemove)
        {
            ...
            if (isPositiveAndBelow(indexToRemove, numUsed))
            {
                ...
                if (numberToShift > 0) memmove(e, e + 1, sizeof(ObjectClass*) * (size_t)numberToShift);

                if ((numUsed << 1) < data.numAllocated) minimiseStorageOverheads();
            }
        }
        ...
        void removeRange(int startIndex, int numberToRemove)
        {
            ...
            if (endIndex > start)
            {
                for (int i = start; i < endIndex; ++i)
                {
                    ...
                    data.elements[i] =
                        nullptr;  // (in case one of the destructors accesses this array and hits a dangling pointer)
                }
        ...
        */
        void removeLast(int howManyToRemove = 1)
        {
            ...
            while (--howManyToRemove >= 0) remove(numUsed - 1);
        }
    ...
    private:
        ...
        {
            while (numUsed > 0) releaseObject(data.elements[--numUsed]);
        ...
        {
            if (o != nullptr && o->decReferenceCountWithoutDeleting()) ContainerDeletePolicy<ObjectClass>::destroy(o);
        }
    };
