...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD)                                                          \
    METHOD(query, "query",                                                                                                   \
           "(Landroid/net/Uri;[Ljava/lang/String;Ljava/lang/String;[Ljava/lang/String;Ljava/lang/String;)Landroid/database/" \
           "Cursor;")                                                                                                        \
...
#undef JNI_CLASS_MEMBERS
    ...
    public:
        static LocalRef<jobject> getStreamForContentUri(const URL& url, bool inputStream)
        {
            ...
            if (contentResolver)
                return LocalRef<jobject>((env->CallObjectMethod(
                    contentResolver.get(), inputStream ? ContentResolver.openInputStream : ContentResolver.openOutputStream,
                    urlToUri(url).get())));
        ...
        static File getLocalFileFromContentUri(const URL& url)
        {
            ...
            if (authority == "com.android.externalstorage.documents")
            {
                ...
                if (storagePath != File()) return storagePath.getChildFile(subpath);
            }
            ...
            else if (authority == "com.android.providers.media.documents" && documentId.isNotEmpty())
            {
                ...
                return getCursorDataColumn(URL("content://media/external/" + type + "/media"), "_id=?", StringArray{mediaId});
            }
    ...
    private:
        ...
        static String getCursorDataColumn(const URL& url, const String& selection = {}, const StringArray& selectionArgs = {})
        {
            ...
            if (contentResolver)
            {
                ...
                {
                    args =
                        LocalRef<jobjectArray>(env->NewObjectArray(selectionArgs.size(), JavaString, javaString("").get()));
                ...
                LocalRef<jobject> cursor(env->CallObjectMethod(contentResolver.get(), ContentResolver.query, uri.get(),
                    ...
                    if (env->CallBooleanMethod(cursor.get(), AndroidCursor.moveToFirst) != 0)
                    {
                        ...
                        if (columnIndex >= 0)
                        {
                            LocalRef<jstring> value(
                                (jstring)env->CallObjectMethod(cursor.get(), AndroidCursor.getString, columnIndex));

                            if (value) return juceString(value.get());
                        }
                    }
        ...
        static File getWellKnownFolder(const String& folderId)
        {
            ...
            LocalRef<jobject> downloadFolder(env->CallStaticObjectMethod(
                AndroidEnvironment, AndroidEnvironment.getExternalStoragePublicDirectory, javaString(folderId).get()));
        ...
        static File getStorageDevicePath(const String& storageId)
        {
            ...
            if (storageId == "primary") return getPrimaryStorageDirectory();

            auto storageDevices = getSecondaryStorageDirectories();

            for (auto storageDevice : storageDevices)
                if (getStorageIdForMountPoint(storageDevice) == storageId) return storageDevice;
        ...
        {
            auto* env = getEnv();
            return juceFile(LocalRef<jobject>(
                env->CallStaticObjectMethod(AndroidEnvironment, AndroidEnvironment.getExternalStorageDirectory)));
        }
        ...
        static Array<File> getSecondaryStorageDirectories()
        {
            ...
            if (getSDKVersion() >= 19)
            {
                auto*            env = getEnv();
                static jmethodID m =
                    (env->GetMethodID(JuceAppActivity, "getExternalFilesDirs", "(Ljava/lang/String;)[Ljava/io/File;"));
                if (m == 0) return {};

                auto paths = convertFileArray(LocalRef<jobject>(android.activity.callObjectMethod(m, nullptr)));

                for (auto path : paths) results.add(getMountPointForFile(path));
            }
            else
            {
                // on older SDKs other external storages are located "next" to the primary
                // storage mount point
                auto mountFolder = getMountPointForFile(getPrimaryStorageDirectory()).getParentDirectory();

                // don't include every folder. Only folders which are actually mountpoints
                juce_statStruct info;
                if (!juce_stat(mountFolder.getFullPathName(), info)) return {};
                ...
                while (iter.next())
                {
                    ...
                    if (juce_stat(candidate.getFullPathName(), info) && info.st_dev != rootFsDevice) results.add(candidate);
                }
            }
        ...
        static Array<File> convertFileArray(LocalRef<jobject> obj)
        {
            ...
            for (int i = 0; i < n; ++i)
                files.add(juceFile(LocalRef<jobject>(env->GetObjectArrayElement((jobjectArray)obj.get(), (jsize)i))));
        ...
        static File juceFile(LocalRef<jobject> obj)
        {
            ...
            if (env->IsInstanceOf(obj.get(), JavaFile) != 0)
                return File(
                    juceString(LocalRef<jstring>((jstring)env->CallObjectMethod(obj.get(), JavaFile.getAbsolutePath))));
        ...
        {
            static int sdkVersion = getEnv()->CallStaticIntMethod(JuceAppActivity, JuceAppActivity.getAndroidSDKVersion);
        ...
        {
            return LocalRef<jobject>(
                getEnv()->CallStaticObjectMethod(AndroidUri, AndroidUri.parse, javaString(url.toString(true)).get()));
        }

        //==============================================================================
        static String getStringUsingDataColumn(const String& columnNameToUse, JNIEnv* env, const LocalRef<jobject>& uri,
                                               const LocalRef<jobject>& contentResolver)
        {
            ...
            if (env->CallBooleanMethod(cursor.get(), AndroidCursor.moveToFirst) != 0)
            {
                ...
                if (columnIndex >= 0)
                {
                    ...
                    if (value) fileName = juceString(value.get());
                }
            }
    ...
    File File::getSpecialLocation(const SpecialLocationType type)
    {
        ...
        case commonDocumentsDirectory:
            return getSpecialFile(JuceAppActivity.getDocumentsFolder);
        case userPicturesDirectory:
            return getSpecialFile(JuceAppActivity.getPicturesFolder);
        case userMusicDirectory:
            return getSpecialFile(JuceAppActivity.getMusicFolder);
        case userMoviesDirectory:
            return getSpecialFile(JuceAppActivity.getMoviesFolder);
    ...
    public:
        SingleMediaScanner(const String& filename)
            : msc(getEnv()->NewObject(
                  MediaScannerConnection, MediaScannerConnection.constructor, android.activity.get(),
    ...
    void FileOutputStream::flushInternal()
    {
        if (fileHandle != 0)
        {
            if (fsync(getFD(fileHandle)) == -1) status = getResultForErrno();
