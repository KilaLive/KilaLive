...
namespace juce
{
            ...
            if (getifaddrs(&addrs) != -1)
            {
                for (struct ifaddrs* i = addrs; i != nullptr; i = i->ifa_next)
                {
                    ...
                    if (ioctl(s, SIOCGIFHWADDR, &ifr) == 0)
                    {
                        ...
                        if (!ma.isNull()) result.addIfNotAlreadyThere(ma);
                    }
                }
    ...
    bool JUCE_CALLTYPE Process::openEmailWithAttachments(const String& /* targetEmailAddress */,
                                                         const String& /* emailSubject */, const String& /* bodyText */,
...
//==============================================================================
    ...
    public:
        Pimpl(WebInputStream& pimplOwner, const URL& urlToCopy, const bool shouldUsePost)
            : owner(pimplOwner)
            , url(urlToCopy)
            , isPost(shouldUsePost)
            , httpRequestCmd(shouldUsePost ? "POST" : "GET")
        {
        }
        ...
        {
            {
                ...
                if (hasBeenCancelled) return false;
            }
        ...
        int read(void* buffer, int bytesToRead)
        {
            ...
            if (isChunked && !readingChunk)
            {
                ...
                if (bytesToRead > chunkEnd - position) bytesToRead = static_cast<int>(chunkEnd - position);
            }
        ...
        bool setPosition(int64 wantedPos)
        {
            ...
            if (wantedPos != position)
            {
                ...
                if (wantedPos < position) return false;
    ...
    private:
        ...
        void closeSocket(bool resetLevelsOfRedirection = true)
        {
            ...
            if (resetLevelsOfRedirection) levelsOfRedirection = 0;
        }
        ...
        int createConnection(WebInputStream::Listener* listener, const int numRedirects)
        {
            ...
            if (isPost) WebInputStream::createHeadersAndPostData(url, headers, postData);
            ...
            int    hostPort;
            if (!decomposeURL(address, hostName, hostPath, hostPort)) return 0;
            ...
            {
                if (!decomposeURL(proxyURL, proxyName, proxyPath, proxyPort)) return 0;
            ...
            if (getaddrinfo(serverName.toUTF8(), String(port).toUTF8(), &hints, &result) != 0 || result == 0) return 0;

            {
                const ScopedLock lock(createSocketLock);

                socketHandle = hasBeenCancelled ? -1 : socket(result->ai_family, result->ai_socktype, 0);
            }
            ...
            if (responseHeader.isNotEmpty())
            {
                ...
                const int status = responseHeader.fromFirstOccurrenceOf(" ", false, false).substring(0, 3).getIntValue();

                String location(findHeaderItem(headerLines, "Location:"));

                if (++levelsOfRedirection <= numRedirects && status >= 300 && status < 400 && location.isNotEmpty()
                    && location != address)
                {
                    if (!(location.startsWithIgnoreCase("http://") || location.startsWithIgnoreCase("https://")
                          || location.startsWithIgnoreCase("ftp://")))
                    {
                        // The following is a bit dodgy. Ideally, we should do a proper transform of the relative URI to a
                        // target URI
                ...
                if (contentLengthString.isNotEmpty()) contentLength = contentLengthString.getLargeIntValue();
        ...
        String readResponse(const uint32 timeOutTime)
        {
            ...
            while (numConsecutiveLFs < 2 && buffer.getDataSize() < 32768 && Time::getMillisecondCounter() <= timeOutTime
                   && !(finished || isError()))
            {
                char c = 0;
                if (read(&c, 1) != 1) return {};
            ...
            if (header.startsWithIgnoreCase("HTTP/")) return header;

            return {};
        }

        static void writeValueIfNotPresent(MemoryOutputStream& dest, const String& headers, const String& key,
                                           const String& value)
        {
            if (!headers.containsIgnoreCase(key)) dest << "\r\n" << key << ' ' << value;
        }

        static void writeHost(MemoryOutputStream& dest, const String& httpRequestCmd, const String& path, const String& host,
                              int port)
        {
            ...
            if (port != 80) dest << ':' << port;
        }

        static MemoryBlock createRequestHeader(const String& hostName, const int hostPort, const String& proxyName,
                                               const int proxyPort, const String& hostPath, const String& originalURL,
                                               const String& userHeaders, const MemoryBlock& postData, const bool isPost,
                                               const String& httpRequestCmd)
        {
            ...
            writeValueIfNotPresent(header, userHeaders, "User-Agent:",
            ...
            if (isPost) writeValueIfNotPresent(header, userHeaders, "Content-Length:", String((int)postData.getSize()));

            if (userHeaders.isNotEmpty()) header << "\r\n" << userHeaders;

            header << "\r\n\r\n";

            if (isPost) header << postData;
                               ...
                               WebInputStream& pimplOwner, WebInputStream::Listener* listener)
        {
            ...
            {
                if (Time::getMillisecondCounter() > timeOutTime) return false;

                const int numToSend = jmin(1024, (int)(requestHeader.getSize() - totalHeaderSent));

                if (send(socketHandle, static_cast<const char*>(requestHeader.getData()) + totalHeaderSent, (size_t)numToSend,
                         0)
                    != numToSend)
                    return false;
                ...
                if (listener != nullptr
                    && !listener->postDataSendProgress(pimplOwner, (int)totalHeaderSent, (int)requestHeader.getSize()))
                    return false;
            }
        ...
        static bool decomposeURL(const String& url, String& host, String& path, int& port)
        {
            ...
            if (nextColon > nextSlash && nextSlash > 0) nextColon = -1;
        ...
        static String findHeaderItem(const StringArray& lines, const String& itemName)
        {
            for (int i = 0; i < lines.size(); ++i)
                if (lines[i].startsWithIgnoreCase(itemName)) return lines[i].substring(itemName.length()).trim();
