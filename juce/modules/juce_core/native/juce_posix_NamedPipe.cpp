...
namespace juce
{
    ...
    public:
        ...
        int read(char* destBuffer, int maxBytesToRead, int timeOutMilliseconds)
        {
            ...
            while (bytesRead < maxBytesToRead)
            {
                ...
                if (numRead <= 0)
                {
                    if (errno != EWOULDBLOCK || stopReadOperation || hasExpired(timeoutEnd)) return -1;

                    const int maxWaitingTime = 30;
                    waitForInput(pipeIn, timeoutEnd == 0
                                             ? maxWaitingTime
    ...
    private:
        ...
        int openPipe(const String& name, int flags, const uint32 timeoutEnd)
        {
            for (;;)
            {
                ...
                if (p != -1 || hasExpired(timeoutEnd) || stopReadOperation) return p;
    ...
    bool NamedPipe::openInternal(const String& pipeName, const bool createPipe, bool mustNotExist)
    {
        ...
        pimpl.reset(new Pimpl(
            File::getSpecialLocation(File::tempDirectory).getChildFile(File::createLegalFileName(pipeName)).getFullPathName(),
            createPipe));
#else
        String file(pipeName);

        if (!File::isAbsolutePath(file)) file = "/tmp/" + File::createLegalFileName(file);
    ...
    {
        ScopedReadLock sl(lock);
        return pimpl != nullptr ? pimpl->write(static_cast<const char*>(sourceBuffer), numBytesToWrite, timeOutMilliseconds)
                                : -1;
    }
