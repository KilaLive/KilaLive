...
namespace juce
{
    ...
    void* JUCE_CALLTYPE Process::getCurrentModuleInstanceHandle() noexcept
    {
        if (currentModuleHandle == nullptr)
        {
            ...
            if (status == 0 || currentModuleHandle == nullptr) currentModuleHandle = GetModuleHandleA(nullptr);
        }
    ...
    {
        return handle != nullptr
                   ? (void*)GetProcAddress((HMODULE)handle, functionName.toUTF8())  // (void* cast is required for mingw)
                   : nullptr;
    }
    ...
    public:
            ...
            , refCount(1)
        {
            ...
            if (handle == 0) handle = CreateMutexW(0, TRUE, ("Local\\" + name).toWideCharPointer());
    ...
    public:
            ...
            , writePipe(0)
        {
            ...
            if (CreatePipe(&readPipe, &writePipe, &securityAtts, 0) && SetHandleInformation(readPipe, HANDLE_FLAG_INHERIT, 0))
            {
                ...
                ok =
                    CreateProcess(nullptr, const_cast<LPWSTR>(command.toWideCharPointer()), nullptr, nullptr, TRUE,
        ...
        ~ActiveProcess()
        {
            ...
            if (readPipe != 0) CloseHandle(readPipe);

            if (writePipe != 0) CloseHandle(writePipe);
        }
        ...
        int read(void* dest, int numNeeded) const noexcept
        {
            ...
            while (ok && numNeeded > 0)
            {
                ...
                if (!PeekNamedPipe((HANDLE)readPipe, nullptr, 0, nullptr, &available, nullptr)) break;
                ...
                {
                    DWORD numRead = 0;
                    if (!ReadFile((HANDLE)readPipe, dest, numToDo, &numRead, nullptr)) break;
    ...
    bool ChildProcess::start(const String& command, int streamFlags)
    {
        ...
        if (!activeProcess->ok) activeProcess = nullptr;
    ...
    bool ChildProcess::start(const StringArray& args, int streamFlags)
    {
        ...
        for (int i = 0; i < args.size(); ++i)
        {
            ...
            if (arg.containsAnyOf("\" ")) arg = arg.replace("\"", "\\\"").quoted();
    ...
    private:
        ...
        static void __stdcall callbackFunction(UINT, UINT, DWORD_PTR userInfo, DWORD_PTR, DWORD_PTR)
        {
            if (Pimpl* const timer = reinterpret_cast<Pimpl*>(userInfo))
                if (timer->periodMs != 0) timer->owner.hiResTimerCallback();
        }
