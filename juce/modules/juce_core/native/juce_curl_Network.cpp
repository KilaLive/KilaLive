    ...
    public:
            ...
            , httpRequest(isPost ? "POST" : "GET")
        {
            ...
            if (multi != nullptr)
            {
                ...
                if (curl != nullptr)
                    if (curl_multi_add_handle(multi, curl) == CURLM_OK) return;
            }
        ...
        bool isExhausted()
        {
            return (isError() || finished) && curlBuffer.getSize() == 0;
        }
        int64 getPosition()
        {
            return streamPos;
        ...
        bool setPosition(int64 wantedPos)
        {
            ...
            if (amountToSkip < 0) return false;

            if (amountToSkip == 0) return true;
        ...
        bool setOptions()
        {
            ...
            if (isPost) WebInputStream::createHeadersAndPostData(url, requestHeaders, headersAndPostData);
                ...
                && curl_easy_setopt(curl, CURLOPT_FOLLOWLOCATION, (maxRedirects > 0 ? 1 : 0)) == CURLE_OK)
            {
                ...
                if (hasSpecialRequestCmd)
                    if (curl_easy_setopt(curl, CURLOPT_CUSTOMREQUEST, httpRequest.toRawUTF8()) != CURLE_OK) return false;
        ...
        bool connect(WebInputStream::Listener* webInputListener)
        {
            ...
            if (isPost) postBuffer = &headersAndPostData;
            ...
            while ((!finished) && curlBuffer.getSize() == 0)
            {
                ...
                if (isPost && listener != nullptr && lastPos != postPosition)
                {
                    ...
                    if (!listener->postDataSendProgress(owner, static_cast<int>(lastPos),
        ...
        void singleStep()
        {
...

            {
                ...
                if ((lastError = (int)curl_multi_timeout(multi, &curl_timeo)) != CURLM_OK) return;
            }
...

            {
                ...
                if ((lastError = (int)curl_multi_fdset(multi, &fdread, &fdwrite, &fdexcep, &maxfd)) != CURLM_OK) return;
            }
            ...
            if ((lastError = curlRet) != CURLM_OK) return;
            ...
            while (len > 0)
            {
                ...
                if (bufferBytes == 0)
                {
                    // do not call curl again if we are finished
                    {
                        ...
                        if (finished || curl == nullptr) return static_cast<int>(pos);
                    }
                ...
                if (bufferBytes > 0)
                {
                    ...
                    if (!skip) memcpy(addBytesToPointer(buffer, pos), curlBuffer.getData(), max);

                    pos += max;
                    streamPos += static_cast<int64>(max);
                    len -= max;

                    if (removeSection) curlBuffer.removeSection(0, max);
                }
            }
        ...
        {
            if (curl == nullptr || lastError != CURLE_OK) return 0;
            ...
            if (len > max) curlBuffer.append(ptr + max, len - max);
        ...
        {
            if (curl == nullptr || postBuffer == nullptr || lastError != CURLE_OK) return 0;
