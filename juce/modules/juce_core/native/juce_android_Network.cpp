...
#undef JNI_CLASS_MEMBERS
    ...
    {
        if (getScheme() == "content") return AndroidContentUriResolver::getFileNameFromContentUri(*this);
    ...
    public:
        ...
        bool connect(WebInputStream::Listener* /*listener*/)
        {
            ...
            else
            {
                ...
                if (postData.getSize() > 0)
                {
                    ...
                    env->SetByteArrayRegion(postDataArray, 0, static_cast<jsize>(postData.getSize()),
...

                {
                    ...
                    if (!hasBeenCancelled)
                        stream = GlobalRef(LocalRef<jobject>(env->CallStaticObjectMethod(
                            JuceAppActivity, JuceAppActivity.createHTTPStream, javaString(address).get(), (jboolean)isPost,
                            postDataArray, javaString(headers).get(), (jint)timeOutMs, statusCodeArray,
                            responseHeaderBuffer.get(), (jint)numRedirectsToFollow, javaString(httpRequest).get())));
                }

                if (stream != 0 && !stream.callBooleanMethod(HTTPStream.connect)) stream.clear();
                ...
                if (postDataArray != 0) env->DeleteLocalRef(postDataArray);
                ...
                if (stream != 0)
                {
                    ...
                    {
                        LocalRef<jstring> headersString(
                            (jstring)env->CallObjectMethod(responseHeaderBuffer.get(), StringBuffer.toString));
        ...
        bool setPosition(int64 wantedPos)
        {
            if (isContentURL)
            {
                if (wantedPos < readPosition) return false;
        ...
        int read(void* buffer, int bytesToRead)
        {
            ...
            if (numBytes > 0) env->GetByteArrayRegion(javaArray, 0, numBytes, static_cast<jbyte*>(buffer));

            env->DeleteLocalRef(javaArray);

            readPosition += jmax(0, numBytes);

            if (numBytes == -1) eofStreamReached = true;
    ...
    static void addAddress(const sockaddr_in* addr_in, Array<IPAddress>& result)
    {
        ...
        if (addr != INADDR_NONE) result.addIfNotAlreadyThere(IPAddress(ntohl(addr)));
    }
    ...
    static void findIPAddresses(int sock, Array<IPAddress>& result)
    {
        ...
        do
        {
            ...
            if (ioctl(sock, SIOCGIFCONF, &cfg) < 0 && errno != EINVAL) return;
        ...
        for (size_t i = 0; i < (size_t)cfg.ifc_len / (size_t)sizeof(struct ifreq); ++i)
        {
            ...
            if (item.ifr_addr.sa_family == AF_INET) addAddress((const sockaddr_in*)&item.ifr_addr, result);
        }
#endif
    }
