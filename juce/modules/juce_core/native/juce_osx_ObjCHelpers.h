...
namespace juce
{
    ...
    static inline NSArray* createNSArrayFromStringArray(const StringArray& strings)
    {
        ...
        for (auto string : strings) [array addObject:juceStringToNS(string)];
    ...
    static NSArray* varArrayToNSArray(const var& varToParse)
    {
        ...
        if (!varToParse.isArray()) return nil;
    ...
    {
        return NSMakeRect(static_cast<CGFloat>(r.getX()), static_cast<CGFloat>(r.getY()), static_cast<CGFloat>(r.getWidth()),
    ...
    static inline MsgSendSuperFn getMsgSendSuperFn() noexcept
    {
        return (MsgSendSuperFn)(void*)objc_msgSendSuper;
    }
    ...
    {
        inline NSObjectRetainer(ObjectType* o)
            : object(o)
        {
            [object retain];
        }
