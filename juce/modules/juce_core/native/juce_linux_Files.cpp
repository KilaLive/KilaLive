...
namespace juce
{
    ...
    bool File::isOnCDRomDrive() const
    {
        ...
        return statfs(getFullPathName().toUTF8(), &buf) == 0 && buf.f_type == (short)U_ISOFS_SUPER_MAGIC;
    }
    ...
    File File::getSpecialLocation(const SpecialLocationType type)
    {
        ...
        {
            if (const char* homeDir = getenv("HOME")) return File(CharPointer_UTF8(homeDir));

            if (auto* pw = getpwuid(getuid())) return File(CharPointer_UTF8(pw->pw_dir));
        ...
        case tempDirectory:
        {
            if (const char* tmpDir = getenv("TMPDIR")) return File(CharPointer_UTF8(tmpDir));

            return File("/tmp");
        }

        case invokedExecutableFile:
            if (juce_argv != nullptr && juce_argc > 0) return File(CharPointer_UTF8(juce_argv[0]));
    ...
    bool File::moveToTrash() const
    {
        ...
        if (!trashCan.isDirectory()) trashCan = "~/.local/share/Trash/files";

        if (!trashCan.isDirectory()) return false;

        return moveFileTo(trashCan.getNonexistentChildFile(getFileNameWithoutExtension(), getFileExtension()));
    }
    ...
    static bool isFileExecutable(const String& filename)
    {
        ...
        return juce_stat(filename, info) && S_ISREG(info.st_mode) && access(filename.toUTF8(), X_OK) == 0;
    }
    ...
    bool Process::openDocument(const String& fileName, const String& parameters)
    {
        ...
        if (/*URL::isProbablyAWebsiteURL (fileName)
            ...
            || File::createFileWithoutCheckingPath(fileName).isDirectory() || !isFileExecutable(fileName))
        {
            // create a command that tries to launch a bunch of likely browsers
            static const char* const browserNames[] = {"xdg-open",      "/etc/alternatives/x-www-browser",
                                                       "firefox",       "mozilla",
