...
#define INVALID_FILE_ATTRIBUTES ((DWORD)-1)
    ...
    namespace WindowsFileHelpers
    {
        ...
        bool changeAtts(const String& path, DWORD bitsToSet, DWORD bitsToClear) noexcept
        {
            ...
            if (oldAtts == INVALID_FILE_ATTRIBUTES) return false;

            auto newAtts = ((oldAtts | bitsToSet) & ~bitsToClear);

            return newAtts == oldAtts || SetFileAttributes(path.toWideCharPointer(), newAtts) != FALSE;
        }
        ...
        {
            if (path.isNotEmpty() && path[1] == ':' && path[2] == 0) path << '\\';
            ...
            if (PathStripToRoot(pathCopy)) path = static_cast<const WCHAR*>(pathCopy);
        ...
        File getSpecialFolderPath(int type)
        {
            ...
            if (SHGetSpecialFolderPath(0, path, type, FALSE)) return File(String(path));
...
//==============================================================================
    ...
    StringRef File::getSeparatorString()
    {
        return "\\";
    }
    ...
    {
        return fullPath.isNotEmpty() && (WindowsFileHelpers::getAtts(fullPath) & FILE_ATTRIBUTE_DIRECTORY) == 0;
    }
    ...
    {
        if (fullPath.isEmpty()) return true;
    ...
    {
        return WindowsFileHelpers::changeAtts(fullPath, shouldBeReadOnly ? FILE_ATTRIBUTE_READONLY : 0,
    ...
    bool File::moveToTrash() const
    {
        ...
        fos.fFlags =
            FOF_ALLOWUNDO | FOF_NOERRORUI | FOF_SILENT | FOF_NOCONFIRMATION | FOF_NOCONFIRMMKDIR | FOF_RENAMEONCOLLISION;
    ...
    bool File::replaceInternal(const File& dest) const
    {
        ...
        return ReplaceFile(dest.getFullPathName().toWideCharPointer(), fullPath.toWideCharPointer(), 0,
                           REPLACEFILE_IGNORE_MERGE_ERRORS, lpExclude, lpReserved)
               != 0;
    }

    Result File::createDirectoryInternal(const String& fileName) const
    {
        return CreateDirectory(fileName.toWideCharPointer(), 0) ? Result::ok() : WindowsFileHelpers::getResultForLastError();
    }
    ...
    {
        HANDLE h = CreateFile(file.getFullPathName().toWideCharPointer(), GENERIC_WRITE, FILE_SHARE_READ, 0, OPEN_ALWAYS,
    ...
    {
        if (fileHandle != nullptr)
            if (!FlushFileBuffers((HANDLE)fileHandle)) status = WindowsFileHelpers::getResultForLastError();
    }

    Result FileOutputStream::truncate()
    {
        if (fileHandle == nullptr) return status;

        flush();
        return SetEndOfFile((HANDLE)fileHandle) ? Result::ok() : WindowsFileHelpers::getResultForLastError();
    }
    ...
    void MemoryMappedFile::openInternal(const File& file, AccessMode mode, bool exclusive)
    {
        ...
        HANDLE h = CreateFile(file.getFullPathName().toWideCharPointer(), accessMode,
                              exclusive ? 0 : (FILE_SHARE_READ | (mode == readWrite ? FILE_SHARE_WRITE : 0)), 0, createType,
        ...
        if (h != INVALID_HANDLE_VALUE)
        {
            ...
            if (mappingHandle != 0)
            {
                address = MapViewOfFile(mappingHandle, access, (DWORD)(range.getStart() >> 32), (DWORD)range.getStart(),
                                        (SIZE_T)range.getLength());

                if (address == nullptr) range = Range<int64>();
    ...
    MemoryMappedFile::~MemoryMappedFile()
    {
        ...
        if (fileHandle != nullptr) CloseHandle((HANDLE)fileHandle);
    }
    ...
    bool File::setFileTimesInternal(int64 modificationTime, int64 accessTime, int64 creationTime) const
    {
        ...
        bool   ok = false;
        HANDLE h  = CreateFile(fullPath.toWideCharPointer(), GENERIC_WRITE, FILE_SHARE_READ, 0, OPEN_ALWAYS,
                              FILE_ATTRIBUTE_NORMAL, 0);
        ...
        if (h != INVALID_HANDLE_VALUE)
        {
            ...
            ok = SetFileTime(h, timeToFileTime(creationTime, &c), timeToFileTime(accessTime, &a),
    ...
    void File::findFileSystemRoots(Array<File>& destArray)
    {
        ...
        for (int i = 0; i < roots.size(); ++i) destArray.add(roots[i]);
    }
    ...
    uint64 File::getFileIdentifier() const
    {
        ...
        HANDLE h = CreateFile(getFullPathName().toWideCharPointer(), GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING,
        ...
        if (h != INVALID_HANDLE_VALUE)
        {
            ...
            if (GetFileInformationByHandle(h, &info)) result = (((uint64)info.nFileIndexHigh) << 32) | info.nFileIndexLow;
    ...
    bool File::isOnHardDisk() const
    {
        ...
        if (fullPath.toLowerCase()[0] <= 'b' && fullPath[1] == ':') return n != DRIVE_REMOVABLE;
    ...
    bool File::isOnRemovableDrive() const
    {
        ...
        return n == DRIVE_CDROM || n == DRIVE_REMOTE || n == DRIVE_REMOVABLE || n == DRIVE_RAMDISK;
    }
    ...
    String File::getVersion() const
    {
        ...
        if (GetFileVersionInfo(getFullPathName().toWideCharPointer(), 0, bufferSize, buffer))
        {
            ...
            {
                result << (int)HIWORD(vffi->dwFileVersionMS) << '.' << (int)LOWORD(vffi->dwFileVersionMS) << '.'
                       << (int)HIWORD(vffi->dwFileVersionLS) << '.' << (int)LOWORD(vffi->dwFileVersionLS);
            }
        }
    ...
    File File::getLinkedTarget() const
    {
        ...
        {
            HANDLE h = CreateFile(getFullPathName().toWideCharPointer(), GENERIC_READ, FILE_SHARE_READ, nullptr,
            ...
            if (h != INVALID_HANDLE_VALUE)
            {
                if (DWORD requiredSize = getFinalPathNameByHandle(h, nullptr, 0, 0 /* FILE_NAME_NORMALIZED */))
                {
                    ...
                    if (getFinalPathNameByHandle(h, buffer, requiredSize, 0 /* FILE_NAME_NORMALIZED */) > 0)
                    {
                        ...
                        return File(path.startsWith(prefix) ? path.substring(prefix.length()) : path);
                    }
                }
        ...
        if (SUCCEEDED(shellLink.CoCreateInstance(CLSID_ShellLink)) && SUCCEEDED(shellLink.QueryInterface(persistFile))
    ...
    public:
        Pimpl(const File& directory, const String& wildCard)
            : directoryWithWildCard(directory.getFullPathName().isNotEmpty()
                                        ? File::addTrailingSeparator(directory.getFullPathName()) + wildCard
                                        : String())
        ...
        bool next(String& filenameFound, bool* const isDir, bool* const isHidden, int64* const fileSize, Time* const modTime,
                  Time* const creationTime, bool* const isReadOnly)
        {
            ...
            {
                if (FindNextFile(handle, &findData) == 0) return false;
            }
    ...
    bool DirectoryIterator::NativeIterator::next(String& filenameFound, bool* const isDir, bool* const isHidden,
    ...
    {
        HINSTANCE hInstance =
            ShellExecute(0, 0, fileName.toWideCharPointer(), parameters.toWideCharPointer(), 0, SW_SHOWDEFAULT);
    ...
    void File::revealToUser() const
    {
        ...
        JUCE_LOAD_WINAPI_FUNCTION(dll, SHOpenFolderAndSelectItems, shOpenFolderAndSelectItems, HRESULT,
    ...
    public:
            ...
            , shouldStop(false)
        {
            if (createPipe)
            {
                pipeH = CreateNamedPipe(filename.toWideCharPointer(), PIPE_ACCESS_DUPLEX | FILE_FLAG_OVERLAPPED, 0,
                                        PIPE_UNLIMITED_INSTANCES, 4096, 4096, 0, 0);

                if (mustNotExist && GetLastError() == ERROR_ALREADY_EXISTS) closePipeHandle();
            }
        }
        ...
        bool connect(const int timeOutMs)
        {
            if (!ownsPipe)
            {
                if (pipeH != INVALID_HANDLE_VALUE) return true;
                ...
                {
                    {
                        ...
                        if (pipeH == INVALID_HANDLE_VALUE)
                            pipeH = CreateFile(filename.toWideCharPointer(), GENERIC_READ | GENERIC_WRITE, 0, 0,
                                               OPEN_EXISTING, FILE_FLAG_OVERLAPPED, 0);
                    }

                    if (pipeH != INVALID_HANDLE_VALUE) return true;

                    if (shouldStop || (timeOutMs >= 0 && Time::getCurrentTime() > timeOutEnd)) return false;
        ...
        int read(void* destBuffer, const int maxBytesToRead, const int timeOutMilliseconds)
        {
            while (connect(timeOutMilliseconds))
            {
                ...
                if (ReadFile(pipeH, destBuffer, (DWORD)maxBytesToRead, &numRead, &over.over)) return (int)numRead;

                const DWORD lastError = GetLastError();

                if (lastError == ERROR_IO_PENDING)
                {
                    if (!waitForIO(over, timeOutMilliseconds)) return -1;

                    if (GetOverlappedResult(pipeH, &over.over, &numRead, FALSE)) return (int)numRead;
                }
        ...
        int write(const void* sourceBuffer, int numBytesToWrite, int timeOutMilliseconds)
        {
            if (connect(timeOutMilliseconds))
            {
                ...
                if (WriteFile(pipeH, sourceBuffer, (DWORD)numBytesToWrite, &numWritten, &over.over)) return (int)numWritten;

                if (GetLastError() == ERROR_IO_PENDING)
                {
                    if (!waitForIO(over, timeOutMilliseconds)) return -1;

                    if (GetOverlappedResult(pipeH, &over.over, &numWritten, FALSE)) return (int)numWritten;

                    if (GetLastError() == ERROR_BROKEN_PIPE && ownsPipe) disconnectPipe();
                }
            }
    ...
    private:
        ...
        bool waitForIO(OverlappedEvent& over, int timeOutMilliseconds)
        {
            ...
            DWORD  waitResult =
                WaitForMultipleObjects(2, handles, FALSE, timeOutMilliseconds >= 0 ? timeOutMilliseconds : INFINITE);
