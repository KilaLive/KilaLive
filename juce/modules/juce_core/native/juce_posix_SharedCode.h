...
namespace juce
{
    ...
    bool WaitableEvent::wait(const int timeOutMillisecs) const noexcept
    {
        ...
        if (!manualReset) triggered = false;
    ...
    bool Process::setMaxNumberOfFileHandles(int newMaxNumber) noexcept
    {
        ...
        {
            if (newMaxNumber <= 0 && lim.rlim_cur == RLIM_INFINITY && lim.rlim_max == RLIM_INFINITY) return true;

            if (lim.rlim_cur >= (rlim_t)newMaxNumber) return true;
        }
        ...
        MaxNumFileHandlesInitialiser() noexcept
        {
            ...
            if (!Process::setMaxNumberOfFileHandles(0))
            {
                for (int num = JUCE_PREFERRED_MAX_FILE_HANDLES; num > 256; num -= 1024)
                    if (Process::setMaxNumberOfFileHandles(num)) break;
            }
        }
    };
...
#define JUCE_STAT stat
        ...
        void updateStatInfoForFile(const String& path, bool* const isDir, int64* const fileSize, Time* const modTime,
                                   Time* const creationTime, bool* const isReadOnly)
        {
            ...
            if (isReadOnly != nullptr) *isReadOnly = access(path.toUTF8(), W_OK) != 0;
        }
    ...
    bool File::isDirectory() const
    {
        ...
        return fullPath.isNotEmpty() && (juce_stat(fullPath, info) && ((info.st_mode & S_IFDIR) != 0));
    }
    ...
    {
        if (exists()) return (hasEffectiveRootFilePermissions() || access(fullPath.toUTF8(), W_OK) == 0);

        if ((!isDirectory()) && fullPath.containsChar(getSeparatorChar())) return getParentDirectory().hasWriteAccess();
    ...
    static bool setFileModeFlags(const String& fullPath, mode_t flags, bool shouldSet) noexcept
    {
        ...
        if (!juce_stat(fullPath, info)) return false;
    ...
    bool File::setFileTimesInternal(int64 modificationTime, int64 accessTime, int64 /*creationTime*/) const
    {
        ...
        if ((modificationTime != 0 || accessTime != 0) && juce_stat(fullPath, info))
        {
            ...
            times.modtime =
                modificationTime != 0 ? static_cast<time_t>(modificationTime / 1000) : static_cast<time_t>(info.st_mtime);
    ...
    {
        if (!exists() && !isSymbolicLink()) return true;

        if (isDirectory()) return rmdir(fullPath.toUTF8()) == 0;
    ...
    {
        if (rename(fullPath.toUTF8(), dest.getFullPathName().toUTF8()) == 0) return true;
    ...
    {
        if (handle != 0 && lseek(getFD(handle), (off_t)pos, SEEK_SET) == pos) return pos;
    ...
    {
        if (fileHandle != 0) close(getFD(fileHandle));
    }
    ...
    ssize_t FileOutputStream::writeInternal(const void* const data, const size_t numBytes)
    {
        ...
        if (fileHandle != 0)
        {
            ...
            if (result == -1) status = getResultForErrno();
        }
    ...
    {
        if (fileHandle != 0 && fsync(getFD(fileHandle)) == -1) status = getResultForErrno();
    }
#endif

    Result FileOutputStream::truncate()
    {
        if (fileHandle == 0) return status;
    ...
    {
        if (const char* s = ::getenv(name.toUTF8())) return String::fromUTF8(s);
    ...
    void MemoryMappedFile::openInternal(const File& file, AccessMode mode, bool exclusive)
    {
        ...
        fileHandle = open(file.getFullPathName().toUTF8(), mode == readWrite ? (O_CREAT + O_RDWR) : O_RDONLY, 00644);

        if (fileHandle != -1)
        {
            void* m = mmap(0, (size_t)range.getLength(), mode == readWrite ? (PROT_READ | PROT_WRITE) : PROT_READ,
                           exclusive ? MAP_PRIVATE : MAP_SHARED, fileHandle, (off_t)range.getStart());
    ...
    {
        if (address != nullptr) munmap(address, (size_t)range.getLength());
    ...
    {
        struct statfs buf;
        if (juce_doStatFS(*this, buf)) return (int64)buf.f_bsize * (int64)buf.f_blocks;
...
//==============================================================================
    ...
    public:
        Pimpl(const String&, int)
            : handle(1)
            , refCount(1)  // On iOS just fake success..
        {
        }
    ...
    public:
            ...
            , refCount(1)
        {
            ...
            if (!tempFolder.isDirectory()) tempFolder = "/tmp";
        ...
        bool createLockFile(const File& file, const int timeOutMillisecs)
        {
            ...
            if (handle != 0)
            {
                ...
                for (;;)
                {
                    ...
                    if (error != EINTR)
                    {
                        if (error == EBADF || error == ENOTSUP) return false;

                        if (timeOutMillisecs == 0 || (timeOutMillisecs > 0 && Time::currentTimeMillis() >= endTime)) break;
    ...
    void InterProcessLock::exit()
    {
        ...
        if (pimpl != nullptr && --(pimpl->refCount) == 0) pimpl.reset();
    }
...
#define JUCE_ANDROID_REALTIME_THREAD_AVAILABLE 1
    ...
    void Thread::launchThread()
    {
        ...
        if (attrPtr != nullptr) pthread_attr_destroy(attrPtr);
    }
...
#if ((JUCE_LINUX && (__GLIBC__ * 1000 + __GLIBC_MINOR__) >= 2012) || JUCE_ANDROID && __ANDROID_API__ >= 9)
    ...
    bool Thread::setThreadPriority(void* handle, int priority)
    {
        ...
        if (handle == nullptr) handle = (void*)pthread_self();
...
#define SUPPORT_AFFINITIES 1
    ...
    void JUCE_CALLTYPE Thread::setCurrentThreadAffinityMask(const uint32 affinityMask)
    {
        ...
        for (int i = 0; i < 32; ++i)
            if ((affinityMask & (1 << i)) != 0) CPU_SET(i, &affinity);
    ...
    public:
            ...
            if (pipe(pipeHandles) == 0)
            {
                ...
                else if (result == 0)
                {
                    ...
                    for (int i = 0; i < arguments.size(); ++i)
                        if (arguments[i].isNotEmpty()) argv.add(const_cast<char*>(arguments[i].toRawUTF8()));
        ...
        ~ActiveProcess()
        {
            if (readHandle != 0) fclose(readHandle);

            if (pipeHandle != 0) close(pipeHandle);
        }
...
#error  // the zlib headers define this function as NULL!
            ...
            if (readHandle == 0 && childPID != 0) readHandle = fdopen(pipeHandle, "r");

            if (readHandle != 0) return (int)fread(dest, 1, (size_t)numBytes, readHandle);
        ...
        uint32 getExitCode() const noexcept
        {
            if (childPID != 0)
            {
                ...
                if (pid >= 0 && WIFEXITED(childState)) return WEXITSTATUS(childState);
            }
    ...
    bool ChildProcess::start(const StringArray& args, int streamFlags)
    {
        ...
        if (activeProcess->childPID == 0) activeProcess.reset();
    ...
    private:
        ...
        void timerThread()
        {
            ...
            while (!destroyThread)
            {
                ...
                if (isRunning) owner.hiResTimerCallback();
        ...
        struct Clock
        {
            ...
            bool wait(pthread_cond_t& cond, pthread_mutex_t& mutex) noexcept
            {
                ...
                if (!hasExpired(left)) return (pthread_cond_timedwait_relative_np(&cond, &mutex, &left) != ETIMEDOUT);
            ...
            bool wait(pthread_cond_t& cond, pthread_mutex_t& mutex) noexcept
            {
                ...
                if (!hasExpired(absExpire)) return (pthread_cond_timedwait(&cond, &mutex, &absExpire) != ETIMEDOUT);
        ...
        static bool setThreadToRealtime(pthread_t thread, uint64 periodMs)
        {
            ...
            return thread_policy_set(pthread_mach_thread_np(thread), THREAD_TIME_CONSTRAINT_POLICY, (thread_policy_t)&policy,
