...
#define INTERNET_OPTION_DISABLE_AUTODIAL 70
    ...
    public:
                ...
                if (!isError())
                {
                    ...
                    if (HttpQueryInfo(request, HTTP_QUERY_STATUS_CODE | HTTP_QUERY_FLAG_NUMBER, &status, &statusSize, 0))
                    {
                            ...
                            && (statusCode == 301 || statusCode == 302 || statusCode == 303 || statusCode == 307))
                        {
                            ...
                            if (!(newLocation.startsWithIgnoreCase("http://") || newLocation.startsWithIgnoreCase("https://")
        ...
        bool setPosition(int64 wantedPos)
        {
            ...
            if (wantedPos != position)
            {
                ...
                if (position == wantedPos) return true;
    ...
    private:
        ...
        void closeConnection()
        {
            ...
            if (requestCopy != 0) InternetCloseHandle(requestCopy);
        ...
        void createConnection(const String& address, WebInputStream::Listener* listener)
        {
            ...
            if (sessionHandle != 0)
            {
                ...
                HeapBlock<TCHAR> file(fileNumChars), server(serverNumChars), username(usernameNumChars),
                    password(passwordNumChars);
        ...
        void openConnection(URL_COMPONENTS& uc, HINTERNET sessionHandle, const String& address,
                            WebInputStream::Listener* listener)
        {
...

            {
                ...
                connection = hasBeenCancelled
                                 ? 0
                                 : InternetConnect(sessionHandle, uc.lpszHostName, uc.nPort, uc.lpszUserName, uc.lpszPassword,
                                                   isFtp ? (DWORD)INTERNET_SERVICE_FTP : (DWORD)INTERNET_SERVICE_HTTP, 0, 0);
            }
        ...
        void openHTTPConnection(URL_COMPONENTS& uc, const String& address, WebInputStream::Listener* listener)
        {
...

            {
                ...
                request = hasBeenCancelled ? 0
                                           : HttpOpenRequest(connection, httpRequestCmd.toWideCharPointer(), uc.lpszUrlPath,
            ...
            if (request != 0)
            {
                ...
                if (HttpSendRequestEx(request, &buffers, 0, HSR_INITIATE, 0))
                {
                    ...
                    for (;;)
                    {
                        ...
                        if (bytesToDo > 0
                            && !InternetWriteFile(request, static_cast<const char*>(postData.getData()) + bytesSent,
                        ...
                        {
                            if (HttpEndRequest(request, 0, 0, 0)) return;
                        ...
                        if (listener != nullptr && !listener->postDataSendProgress(owner, bytesSent, (int)postData.getSize()))
                            break;
                    }
                }
            }
    ...
    struct GetAdaptersAddressesHelper
    {
        bool callGetAdaptersAddresses()
        {
            ...
            JUCE_LOAD_WINAPI_FUNCTION(dll, GetAdaptersAddresses, getAdaptersAddresses, DWORD,
                                      (ULONG, ULONG, PVOID, PIP_ADAPTER_ADDRESSES, PULONG))

            if (getAdaptersAddresses == nullptr) return false;

            adaptersAddresses.malloc(1);
            ULONG len = sizeof(IP_ADAPTER_ADDRESSES);

            if (getAdaptersAddresses(AF_UNSPEC, GAA_FLAG_INCLUDE_PREFIX, NULL, adaptersAddresses, &len)
    ...
    namespace MACAddressHelpers
    {
        ...
        static void getViaGetAdaptersAddresses(Array<MACAddress>& result)
        {
            ...
            {
                for (PIP_ADAPTER_ADDRESSES adapter = addressesHelper.adaptersAddresses; adapter != nullptr;
                     adapter                       = adapter->Next)
                {
                    if (adapter->PhysicalAddressLength >= 6) addAddress(result, MACAddress(adapter->PhysicalAddress));
                }
            }
        }
    ...
    void IPAddress::findAllAddresses(Array<IPAddress>& result, bool includeIPv6)
    {
        ...
        if (includeIPv6) result.addIfNotAlreadyThere(IPAddress::local(true));
    ...
    //==============================================================================
    bool JUCE_CALLTYPE Process::openEmailWithAttachments(const String& targetEmailAddress, const String& emailSubject,
                                                         const String& bodyText, const StringArray& filesToAttach)
    {
        ...
        if (mapiSendMail == nullptr) return false;
        ...
        if (targetEmailAddress_.isEmpty()) targetEmailAddress_ = " ";  // (Windows Mail can't deal with a blank address)
