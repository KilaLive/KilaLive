...
namespace juce
{
    struct RegistryKeyWrapper
    {
        ...
        static bool setValue(const String& regValuePath, const DWORD type, const void* data, size_t dataSize,
                             const DWORD wow64Flags)
        {
            const RegistryKeyWrapper key(regValuePath, true, wow64Flags);

            return key.key != 0
                   && RegSetValueEx(key.key, key.wideCharValueName, 0, type, reinterpret_cast<const BYTE*>(data),
                                    (DWORD)dataSize)
                          == ERROR_SUCCESS;
        }
        ...
        static uint32 getBinaryValue(const String& regValuePath, MemoryBlock& result, DWORD wow64Flags)
        {
            ...
            if (key.key != 0)
            {
                for (unsigned long bufferSize = 1024;; bufferSize *= 2)
                {
                    ...
                    auto err =
                        RegQueryValueEx(key.key, key.wideCharValueName, 0, &type, (LPBYTE)result.getData(), &bufferSize);
        ...
        static bool valueExists(const String& regValuePath, const DWORD wow64Flags)
        {
            ...
            auto result = RegQueryValueEx(key.key, key.wideCharValueName, 0, &type, buffer, &bufferSize);
    ...
    {
        if (deleteKeyNonRecursive(regKeyPath, mode)) return true;
    ...
    bool JUCE_CALLTYPE WindowsRegistry::registerFileAssociation(const String& fileExtension,
                                                                ...
                                                                const bool registerForCurrentUserOnly, WoW64Mode mode)
    {
        auto root = registerForCurrentUserOnly ? "HKEY_CURRENT_USER\\Software\\Classes\\" : "HKEY_CLASSES_ROOT\\";
        auto key  = root + symbolicDescription;

        return setValue(root + fileExtension + "\\", symbolicDescription, mode) && setValue(key + "\\", fullDescription, mode)
