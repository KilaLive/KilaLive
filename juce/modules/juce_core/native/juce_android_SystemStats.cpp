...
#undef JNI_CLASS_MEMBERS
    ...
    JNIClassBase::JNIClassBase(const char* cp)
        : classPath(cp)
        , classRef(0)
    {
        getClasses().add(this);
    ...
    void JNIClassBase::initialiseAllClasses(JNIEnv* env)
    {
        ...
        for (int i = classes.size(); --i >= 0;) classes.getUnchecked(i)->initialise(env);
    }

    void JNIClassBase::releaseAllClasses(JNIEnv* env)
    {
        const Array<JNIClassBase*>& classes = getClasses();
        for (int i = classes.size(); --i >= 0;) classes.getUnchecked(i)->release(env);
    }
                                          ...
                                          LocalRef<jobject> subclass)
    {
        ...
        for (auto i = 0; i < interfaceNames.size(); ++i)
        {
            ...
            if (aClass != nullptr)
            {
                if (i == 0) classLoader = LocalRef<jobject>(env->CallObjectMethod(aClass, JavaClass.getClassLoader));
        ...
        auto invocationHandler = LocalRef<jobject>(env->CallObjectMethod(
            android.activity, JuceAppActivity.createInvocationHandler, reinterpret_cast<jlong>(implementer)));
        ...
        return LocalRef<jobject>(env->CallStaticObjectMethod(JavaProxy, JavaProxy.newProxyInstance, classLoader.get(),
    ...
    LocalRef<jobject> CreateJavaInterface(AndroidInterfaceImplementer* implementer, const StringArray& interfaceNames)
    {
        return CreateJavaInterface(implementer, interfaceNames,
                                   LocalRef<jobject>(getEnv()->NewObject(JavaObject, JavaObject.constructor)));
    }
...

    {
        if (invocationHandler != nullptr)
            getEnv()->CallVoidMethod(android.activity, JuceAppActivity.invocationHandlerContextDeleted,
    ...
    JUCE_JNI_CALLBACK(JUCE_JOIN_MACRO(JUCE_ANDROID_ACTIVITY_CLASSNAME, _00024NativeInvocationHandler), dispatchInvoke,
                      jobject,
                      (JNIEnv * env, jobject /*object*/, jlong thisPtr, jobject proxy, jobject method, jobjectArray args))
    {
        return juce_invokeImplementer(env, thisPtr, proxy, method, args);
    }

    JUCE_JNI_CALLBACK(JUCE_JOIN_MACRO(JUCE_ANDROID_ACTIVITY_CLASSNAME, _00024NativeInvocationHandler), dispatchFinalize, void,
                      (JNIEnv * env, jobject /*object*/, jlong thisPtr))
    {
    ...
    public:
        static JniEnvThreadHolder& getInstance() noexcept
        {
            ...
            try
            {
                if (instance == nullptr) instance = new JniEnvThreadHolder;
            }
            catch (...)
            {
                jassertfalse;
        ...
        {
            return reinterpret_cast<JNIEnv*>(pthread_getspecific(getInstance().threadKey));
        }
...
#undef JNI_CLASS_MEMBERS
        ...
        static inline String getSystemProperty(const String& name)
        {
            return juceString(LocalRef<jstring>(
                (jstring)getEnv()->CallStaticObjectMethod(SystemClass, SystemClass.getProperty, javaString(name).get())));
        }

        static inline String getLocaleValue(bool isRegion)
        {
            return juceString(LocalRef<jstring>(
                (jstring)getEnv()->CallStaticObjectMethod(JuceAppActivity, JuceAppActivity.getLocaleValue, isRegion)));
        }
...
#undef JNI_CLASS_MEMBERS
    ...
    int SystemStats::getMemorySizeInMegabytes()
    {
        ...
        if (sysinfo(&sysi) == 0) return static_cast<int>((sysi.totalram * sysi.mem_unit) / (1024 * 1024));
#endif
    ...
    {
        if (const char* user = getenv("USER")) return CharPointer_UTF8(user);

        if (struct passwd* const pw = getpwuid(getuid())) return CharPointer_UTF8(pw->pw_name);
