...
//==============================================================================
        ...
        if (auto size = GetFileVersionInfoSize(filename, &handle))
        {
            ...
            if (GetFileVersionInfo(filename, handle, size, data))
            {
                ...
                if (VerQueryValue(data, (LPCTSTR) _T("\\"), (void**)&info, &verSize))
                    if (size > 0 && info != nullptr && info->dwSignature == 0xfeef04bd) return (uint32)info->dwFileVersionMS;
            }
        }
    ...
    bool SystemStats::isOperatingSystem64Bit()
    {
        ...
        return fnIsWow64Process != nullptr && fnIsWow64Process(GetCurrentProcess(), &isWow64) && isWow64 != FALSE;
#endif
    }
    ...
    String SystemStats::getEnvironmentVariable(const String& name, const String& defaultValue)
    {
        ...
        if (GetLastError() == ERROR_ENVVAR_NOT_FOUND) return String(defaultValue);
...
#error "unknown compiler?"
    ...
    int SystemStats::getCpuSpeedInMegaherz()
    {
        ...
        for (;;)
        {
            ...
            if (millisElapsed > 80)
            {
                ...
                if (millisElapsed > 500 || (lastResult == newResult && newResult > 100)) return newResult;
    ...
    {
        TCHAR buffer[256] = {0};
        if (GetLocaleInfo(locale, key, buffer, 255) > 0) return buffer;
    ...
    String SystemStats::getDisplayLanguage()
    {
        ...
        if (getUserDefaultUILanguage == nullptr) return "en";
        ...
        if (region.isNotEmpty()) mainLang << '-' << region;
