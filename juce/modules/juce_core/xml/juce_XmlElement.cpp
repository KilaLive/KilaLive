...
namespace juce
{
    inline static bool isValidXmlNameStartCharacter(juce_wchar character) noexcept
    {
        return character == ':' || character == '_' || (character >= 'a' && character <= 'z')
               || (character >= 'A' && character <= 'Z') || (character >= 0xc0 && character <= 0xd6)
               || (character >= 0xd8 && character <= 0xf6) || (character >= 0xf8 && character <= 0x2ff)
               || (character >= 0x370 && character <= 0x37d) || (character >= 0x37f && character <= 0x1fff)
               || (character >= 0x200c && character <= 0x200d) || (character >= 0x2070 && character <= 0x218f)
    ...
    {
        return isValidXmlNameStartCharacter(character) || character == '-' || character == '.' || character == 0xb7
               || (character >= '0' && character <= '9') || (character >= 0x300 && character <= 0x036f)
    ...
    void generateLegalCharLookupTable()
    {
        ...
        {
            static const unsigned char legalChars[] = {0,   0,   0,   0,   187, 255, 255, 175,
                                                       255, 255, 255, 191, 254, 255, 255, 127};
            return c < sizeof(legalChars) * 8 && (legalChars[c >> 3] & (1 << (c & 7))) != 0;
        }
    ...
    void XmlElement::writeElementAsText(OutputStream& outputStream, const int indentationLevel,
                                        const int lineWrapLength) const
    {
        using namespace XmlOutputFunctions;

        if (indentationLevel >= 0) writeSpaces(outputStream, (size_t)indentationLevel);
        ...
        if (!isTextElement())
        {
            ...
            if (auto* child = firstChildElement.get())
            {
                ...
                for (; child != nullptr; child = child->nextListItem)
                {
                    ...
                    {
                        if (indentationLevel >= 0 && !lastWasTextNode) outputStream << newLine;
    ...
    String XmlElement::createDocument(StringRef dtdToUse, const bool allOnOneLine, const bool includeXmlHeader,
                                      StringRef encodingType, const int lineWrapLength) const
    {
    ...
    void XmlElement::writeToStream(OutputStream& output, StringRef dtdToUse, bool allOnOneLine, bool includeXmlHeader,
                                   StringRef encodingType, int lineWrapLength) const
    {
        ...
        if (!allOnOneLine) output << newLine;
    }
    ...
    XmlElement* XmlElement::getNextElementWithTagName(StringRef requiredTagName) const
    {
        ...
        while (e != nullptr && !e->hasTagName(requiredTagName)) e = e->nextListItem;
    ...
    {
        if (auto* att = attributes[index].get()) return att->name.toString();
    ...
    {
        if (auto* att = attributes[index].get()) return att->value;
    ...
    {
        if (auto* att = getAttribute(attributeName)) return att->value;
    ...
    {
        if (auto* att = getAttribute(attributeName)) return att->value.getIntValue();
    ...
    {
        if (auto* att = getAttribute(attributeName)) return att->value.getDoubleValue();
    ...
    bool XmlElement::getBoolAttribute(StringRef attributeName, const bool defaultReturnValue) const
    {
        if (auto* att = getAttribute(attributeName))
        {
            ...
            return firstChar == '1' || firstChar == 't' || firstChar == 'y' || firstChar == 'T' || firstChar == 'Y';
        }

        return defaultReturnValue;
    }

    bool XmlElement::compareAttribute(StringRef attributeName, StringRef stringToCompareAgainst, const bool ignoreCase) const
        noexcept
    {
        if (auto* att = getAttribute(attributeName))
            return ignoreCase ? att->value.equalsIgnoreCase(stringToCompareAgainst) : att->value == stringToCompareAgainst;
    ...
    XmlElement* XmlElement::getChildByName(StringRef childName) const noexcept
    {
        ...
        for (auto* child = firstChildElement.get(); child != nullptr; child = child->nextListItem)
            if (child->hasTagName(childName)) return child;
    ...
    XmlElement* XmlElement::getChildByAttribute(StringRef attributeName, StringRef attributeValue) const noexcept
    {
        ...
        for (auto* child = firstChildElement.get(); child != nullptr; child = child->nextListItem)
            if (child->compareAttribute(attributeName, attributeValue)) return child;
    ...
    bool XmlElement::replaceChildElement(XmlElement* const currentChildElement, XmlElement* const newNode) noexcept
    {
        if (newNode != nullptr)
        {
            if (auto* p = firstChildElement.findPointerTo(currentChildElement))
            {
                if (currentChildElement != newNode) delete p->replaceNext(newNode);
    ...
    void XmlElement::removeChildElement(XmlElement* const childToRemove, const bool shouldDeleteTheChild) noexcept
    {
        if (childToRemove != nullptr)
        {
            firstChildElement.remove(childToRemove);

            if (shouldDeleteTheChild) delete childToRemove;
        }
    }

    bool XmlElement::isEquivalentTo(const XmlElement* const other, const bool ignoreOrderOfAttributes) const noexcept
    {
        if (this != other)
        {
            if (other == nullptr || tagName != other->tagName) return false;
            ...
            if (ignoreOrderOfAttributes)
            {
                ...
                for (auto* att = attributes.get(); att != nullptr; att = att->nextListItem)
                {
                    if (!other->compareAttribute(att->name, att->value)) return false;

                    ++totalAtts;
                }

                if (totalAtts != other->getNumAttributes()) return false;
            }
            else
            {
                ...
                for (;;)
                {
                    ...
                    if (thisAtt->name != otherAtt->name || thisAtt->value != otherAtt->value)
                    {
                        return false;
                    }
            ...
            for (;;)
            {
                ...
                if (!thisChild->isEquivalentTo(otherChild, ignoreOrderOfAttributes)) return false;
    ...
    void XmlElement::deleteAllChildElementsWithTagName(StringRef name) noexcept
    {
        for (auto* child = firstChildElement.get(); child != nullptr;)
        {
            ...
            if (child->hasTagName(name)) removeChildElement(child, true);
    ...
    {
        if (this == elementToLookFor || elementToLookFor == nullptr) return nullptr;

        for (auto* child = firstChildElement.get(); child != nullptr; child = child->nextListItem)
        {
            if (elementToLookFor == child) return this;

            if (auto* found = child->findParentElementOf(elementToLookFor)) return found;
        }
    ...
    {
        if (isTextElement()) return getText();

        if (getNumChildElements() == 1) return firstChildElement.get()->getAllSubText();
    ...
    {
        if (auto* child = getChildByName(childTagName)) return child->getAllSubText();
    ...
    {
        if (text.isEmpty() || !isValidXmlNameStartCharacter(text.text.getAndAdvance())) return false;

        for (;;)
        {
            if (text.isEmpty()) return true;

            if (!isValidXmlNameBodyCharacter(text.text.getAndAdvance())) return false;
        }
    }
    ...
    void XmlElement::deleteAllTextElements() noexcept
    {
        for (auto* child = firstChildElement.get(); child != nullptr;)
        {
            ...
            if (child->isTextElement()) removeChildElement(child, true);
