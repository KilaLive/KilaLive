...
namespace juce
{
    ...
    namespace XmlIdentifierChars
    {
        ...
        {
            return CharacterFunctions::isLetterOrDigit(c) || c == '_' || c == '-' || c == ':' || c == '.';
        }
        ...
        if (originalText.isEmpty() && inputSource != nullptr)
        {
            ...
            if (in != nullptr)
            {
                ...
                if (data.getDataSize() > 2)
                {
                    ...
                    {
                        if (CharPointer_UTF8::isByteOrderMark(text)) text += 3;
                                                  ...
                                                  const bool              onlyReadOuterDocumentElement)
    {
        ...
        else
        {
            ...
            if (!errorOccurred) return result.release();
        }
    ...
    bool XmlDocument::parseHeader()
    {
        ...
        if (CharacterFunctions::compareUpTo(input, CharPointer_ASCII("<?xml"), 5) == 0)
        {
            ...
            if (headerEnd.isEmpty()) return false;
        ...
        if (*input == '<')
        {
            ...
            for (;;)
            {
                ...
                if (c == '>')
                {
                    ...
                    if (alsoParseSubElements) readChildElements(*node);
                ...
                if (XmlIdentifierChars::isIdentifierChar(c))
                {
                    ...
                    if (attNameEnd != input)
                    {
                        ...
                        {
                            setLastError("expected '=' after attribute '" + String(attNameStart, attNameEnd) + "'", false);
                            return node;
                        }
                    }
                }
                ...
                {
                    if (!outOfData) setLastError("illegal character found in " + node->getTagName() + ": '" + c + "'", false);
                }
        ...
        for (;;)
        {
            ...
            if (*input == '<')
            {
                ...
                if (c1 == '/')
                {
                    ...
                    if (closeTag >= 0) input += closeTag + 1;
            ...
            else  // must be a character block
            {
                ...
                for (;;)
                {
                    ...
                    if (c == '&')
                    {
                        ...
                        if (entity.startsWithChar('<') && entity[1] != 0)
                        {
                            ...
                            while (auto* n = readNextElement(true)) childAppender.append(n);
                ...
                if (contentShouldBeUsed) childAppender.append(XmlElement::createTextElement(textElementContent.toUTF8()));
            }
        }
    }
                ...
                else
                {
                    ...
                    if (openBracket > 0)
                    {
                        ...
                        if (closeBracket > openBracket)
                            tokenisedDTD.addTokens(dtdText.substring(openBracket + 1, closeBracket), true);
                    }
                }

                for (int i = tokenisedDTD.size(); --i >= 0;)
                {
                    if (tokenisedDTD[i].startsWithChar('%') && tokenisedDTD[i].endsWithChar(';'))
                    {
                        ...
                        for (int j = newToks.size(); --j >= 0;) tokenisedDTD.insert(i, newToks[j]);
                    }
                }
            }
            ...
            if (tokenisedDTD[i] == entity)
            {
                if (tokenisedDTD[i - 1].equalsIgnoreCase("<!entity"))
                {
                    ...
                    while (ampersand >= 0)
                    {
                        ...
                        ent = ent.substring(0, ampersand) + resolved + ent.substring(semiColon + 1);
    ...
    String XmlDocument::getParameterEntity(const String& entity)
    {
        for (int i = 0; i < tokenisedDTD.size(); ++i)
        {
            if (tokenisedDTD[i] == entity && tokenisedDTD[i - 1] == "%" && tokenisedDTD[i - 2].equalsIgnoreCase("<!entity"))
            {
                auto ent = tokenisedDTD[i + 1].trimCharactersAtEnd(">");

                if (ent.equalsIgnoreCase("system")) return getFileContents(tokenisedDTD[i + 2].trimCharactersAtEnd(">"));
