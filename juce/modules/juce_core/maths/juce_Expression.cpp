...
namespace juce
{
    ...
    public:
        ...
        {
            for (int i = getNumInputs(); --i >= 0;) getInput(i)->renameSymbol(oldSymbol, newName, scope, recursionDepth);
        }
        ...
        {
            for (int i = getNumInputs(); --i >= 0;) getInput(i)->visitAllSymbols(visitor, scope, recursionDepth);
        }
    ...
    struct Expression::Helpers
    {
        ...
        {
            if (depth > 256) throw EvaluationError("Recursive symbol references");
        }
        ...
        protected:
            ...
            TermPtr createDestinationTerm(const Scope& scope, const Term* input, double overallTarget,
                                          Term* topLevelTerm) const
            {
                jassert(input == left || input == right);
                if (input != left && input != right) return TermPtr();
        ...
        public:
            ...
            {
                if (oldSymbol.symbolName == symbol && scope.getScopeUID() == oldSymbol.scopeUID) symbol = newName;
            }
        ...
        public:
            ...
            int getInputIndexFor(const Term* possibleInput) const
            {
                for (int i = 0; i < parameters.size(); ++i)
                    if (parameters.getReference(i).term == possibleInput) return i;
            ...
            String toString() const
            {
                if (parameters.size() == 0) return functionName + "()";
        ...
        private:
            ...
            {
            public:
                SymbolRenamingVisitor(const TermPtr& t, const Expression::Symbol& symbol_, const String& newName_,
        ...
        public:
            ...
            TermPtr createTermToEvaluateInput(const Scope& scope, const Term* t, double overallTarget,
                                              Term* topLevelTerm) const
            {
                ignoreUnused(t);
                jassert(t == input);
                ...
                return new Negate(dest == nullptr
                                      ? new Constant(overallTarget, false)
                                      : dest->createTermToEvaluateInput(scope, this, overallTarget, topLevelTerm));
            }

            String toString() const
            {
                if (input->getOperatorPrecedence() > 0) return "-(" + input->toString() + ")";
        ...
        public:
            ...
            TermPtr createTermToEvaluateInput(const Scope& scope, const Term* input, double overallTarget,
                                              Term* topLevelTerm) const
            {
                const TermPtr newDest(createDestinationTerm(scope, input, overallTarget, topLevelTerm));
                if (newDest == nullptr) return TermPtr();
        ...
        public:
                                              ...
                                              Term* topLevelTerm) const
            {
                ...
                if (input == left) return new Add(newDest, right->clone());
        ...
        {
        public:
            Multiply(Term* const l, Term* const r)
                : BinaryTerm(l, r)
            {
            }
        ...
        {
        public:
            Divide(Term* const l, Term* const r)
                : BinaryTerm(l, r)
            {
            }
                                              ...
                                              Term* topLevelTerm) const
            {
                ...
                if (input == left) return new Multiply(newDest, right->clone());
        ...
        static Term* findDestinationFor(Term* const topLevel, const Term* const inputTerm)
        {
            ...
            if (inputIndex >= 0) return topLevel;
        ...
        static Constant* findTermToAdjust(Term* const term, const bool mustBeFlagged)
        {
            ...
            if (term->getType() == constantType)
            {
                ...
                if (c->isResolutionTarget || !mustBeFlagged) return c;
            }

            if (term->getType() == functionType) return nullptr;
        ...
        {
            if (t->getType() == Expression::symbolType) return true;

            for (int i = t->getNumInputs(); --i >= 0;)
                if (containsAnySymbols(t->getInput(i))) return true;
        ...
        public:
            ...
            {
                if (text.isEmpty()) return new Constant(0.0, false);
        ...
        private:
            ...
            TermPtr readSymbolOrFunction()
            {
                ...
                if (readIdentifier(identifier))
                {
                    if (readOperator("("))  // method call...
                    {
                        ...
                        if (param == nullptr)
                        {
                            if (readOperator(")")) return func.release();
                        ...
                        while (readOperator(","))
                        {
                            ...
                            if (param == nullptr) return parseError("Expected expression after \",\"");
                    ...
                    if (readOperator("."))
                    {
                        ...
                        if (rhs == nullptr) return parseError("Expected symbol or function after \".\"");

                        if (identifier == "this") return rhs;
            ...
            {
                if (!readOperator("(")) return TermPtr();

                const TermPtr e(readExpression());
                if (e == nullptr || !readOperator(")")) return TermPtr();
    ...
    Expression Expression::adjustedToGiveNewResult(const double targetValue, const Expression::Scope& scope) const
    {
        ...
        if (termToAdjust == nullptr) termToAdjust = Helpers::findTermToAdjust(newTerm.get(), false);
        ...
        {
            if (Helpers::TermPtr reverseTerm =
                    parent->createTermToEvaluateInput(scope, termToAdjust, targetValue, newTerm.get()))
    ...
    Expression Expression::withRenamedSymbol(const Expression::Symbol& oldSymbol, const String& newName,
                                             const Scope& scope) const
    {
        jassert(newName.toLowerCase().containsOnly("abcdefghijklmnopqrstuvwxyz0123456789_"));

        if (oldSymbol.symbolName == newName) return *this;
    ...
    {
        if (symbol.isNotEmpty()) throw Helpers::EvaluationError("Unknown symbol: " + symbol);
    ...
    double Expression::Scope::evaluateFunction(const String& functionName, const double* parameters, int numParams) const
    {
        if (numParams > 0)
        {
            if (functionName == "min")
            {
                ...
                for (int i = 1; i < numParams; ++i) v = jmin(v, parameters[i]);
            ...
            {
                double v = parameters[0];
                for (int i = 1; i < numParams; ++i) v = jmax(v, parameters[i]);
