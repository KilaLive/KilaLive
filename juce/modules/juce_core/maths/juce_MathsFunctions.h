...
#define literal64bit(longLiteral) (longLiteral##LL)
    ...
    */
    ...
    int numElementsInArray(Type (&array)[N])
    {
        (void)array;
        (void)sizeof(
            0 [array]);  // This line should cause an error if you pass an object with a user-defined subscript operator
        return N;
    }
...
#define JUCE_UNDENORMALISE(x)
    ...
    namespace TypeHelpers
    {
        /** The ParameterType struct is used to find the best type to use when passing some kind
           ...
           TypeHelpers::ParameterType<MyObject>::type)" would evaluate to "myfunction (int, const MyObject&)", keeping any
           primitive types as pass-by-value, but passing objects as a const reference, to avoid copying.
        */
        template <typename Type>
...
//==============================================================================
    ...
    JUCE_DEPRECATED_ATTRIBUTE inline int roundDoubleToInt(double value) noexcept
    {
        return roundToInt(value);
    }
    JUCE_DEPRECATED_ATTRIBUTE inline int roundFloatToInt(float value) noexcept
    {
        return roundToInt(value);
    }

    // This old function isn't needed - just use std::abs() instead
    JUCE_DEPRECATED_ATTRIBUTE inline int64 abs64(int64 n) noexcept
    {
        return std::abs(n);
    }
#endif
