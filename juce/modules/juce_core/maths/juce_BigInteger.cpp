...
namespace juce
{
    ...
    uint32* BigInteger::getValues() const noexcept
    {
        ...
        return heapAllocation != nullptr ? heapAllocation : (uint32*)preallocated;
    }
    ...
    uint32* BigInteger::ensureSize(const size_t numVals)
    {
        if (numVals > allocatedSize)
        {
            ...
            else
            {
                ...
                for (auto* values = getValues(); oldSize < allocatedSize; ++oldSize) values[oldSize] = 0;
            }
        }
...
#pragma intrinsic(_BitScanReverse)
    ...
    int BigInteger::countNumberOfSetBits() const noexcept
    {
        ...
        for (int i = (int)sizeNeededToHold(highestBit); --i >= 0;) total += countNumberOfBits(values[i]);
    ...
    void BigInteger::shiftLeft(int bits, const int startBit)
    {
        ...
        else
        {
            ...
            if (wordsToMove > 0)
            {
                for (int i = (int)numOriginalInts; i >= 0; --i) values[(size_t)i + wordsToMove] = values[i];
        ...
        else
        {
            ...
            else
            {
                ...
                if (bits != 0)
                {
                    ...
                    for (size_t i = 0; i < top; ++i) values[i] = (values[i] >> bits) | (values[i + 1] << invBits);
    ...
    void BigInteger::montgomeryMultiplication(const BigInteger& other, const BigInteger& modulus, const BigInteger& modulusp,
                                              const int k)
    {
        *this *= other;
        auto t = *this;
    ...
    MemoryBlock BigInteger::toMemoryBlock() const
    {
        ...
        for (int i = 0; i < numBytes; ++i) mb[i] = (char)((values[i / 4] >> ((i & 3) * 8)) & 0xff);
    ...
    void BigInteger::loadFromMemoryBlock(const MemoryBlock& data)
    {
        ...
        for (int i = (int)(numBytes & ~3u); i < (int)numBytes; ++i) this->setBitRangeAsInt(i << 3, 8, (uint32)data[i]);
    ...
    void writeLittleEndianBitsInBuffer(void* buffer, uint32 startBit, uint32 numBits, uint32 value) noexcept
    {
        ...
        if (numBits > 0) *data = (uint8)((*data & (0xff << numBits)) | value);
    }
...
//==============================================================================
    ...
    public:
        ...
        static BigInteger getBigRandom(Random& r)
        {
            ...
            while (b < 2) r.fillBitsRandomly(b, 0, r.nextInt(150) + 1);
