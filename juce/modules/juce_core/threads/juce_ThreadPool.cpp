...
namespace juce
{
    ...
    public:
        ...
        void run() override
        {
            while (!threadShouldExit())
                if (!pool.runNextJob(*this)) wait(500);
        }
    ...
    {
        if (auto* t = dynamic_cast<ThreadPool::ThreadPoolThread*>(Thread::getCurrentThread())) return t->currentJob;
    ...
    {
        for (int i = jmax(1, numThreads); --i >= 0;) threads.add(new ThreadPoolThread(*this, threadStackSize));

        for (auto* t : threads) t->startThread();
    }
    ...
    void ThreadPool::stopThreads()
    {
        ...
        for (auto* t : threads) t->stopThread(500);
    }
    ...
    bool ThreadPool::waitForJobToFinish(const ThreadPoolJob* const job, const int timeOutMs) const
    {
        if (job != nullptr)
        {
            ...
            while (contains(job))
            {
                if (timeOutMs >= 0 && Time::getMillisecondCounter() >= start + (uint32)timeOutMs) return false;
    ...
    bool ThreadPool::removeJob(ThreadPoolJob* const job, const bool interruptIfRunning, const int timeOutMs)
    {
        bool                      dontWait = true;
        ...
        if (job != nullptr)
        {
            ...
            if (jobs.contains(job))
            {
                if (job->isActive)
                {
                    if (interruptIfRunning) job->signalJobShouldExit();
                                   ...
                                   ThreadPool::JobSelector* const selectedJobsToRemove)
    {
                ...
                for (int i = jobs.size(); --i >= 0;)
                {
                    ...
                    if (selectedJobsToRemove == nullptr || selectedJobsToRemove->isJobSuitable(job))
                    {
                        if (job->isActive)
                        {
                            ...
                            if (interruptRunningJobs) job->signalJobShouldExit();
                        }
                        else
                        {
                            jobs.remove(i);
        ...
        for (;;)
        {
            for (int i = jobsToWaitFor.size(); --i >= 0;)
            {
                ...
                if (!isJobRunning(job)) jobsToWaitFor.remove(i);
            }

            if (jobsToWaitFor.size() == 0) break;
    ...
    StringArray ThreadPool::getNamesOfAllJobs(const bool onlyReturnActiveJobs) const
    {
        ...
        for (auto* job : jobs)
            if (job->isActive || !onlyReturnActiveJobs) s.add(job->getJobName());
    ...
    bool ThreadPool::setThreadPriorities(const int newPriority)
    {
        ...
        for (auto* t : threads)
            if (!t->setPriority(newPriority)) ok = false;
    ...
    void ThreadPool::addToDeleteList(OwnedArray<ThreadPoolJob>& deletionList, ThreadPoolJob* const job) const
    {
        ...
        if (job->shouldBeDeleted) deletionList.add(job);
    }
