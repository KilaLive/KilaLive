...
namespace juce
{
    ...
    void Thread::threadEntryPoint()
    {
        ...
        if (threadName.isNotEmpty()) setCurrentThreadName(threadName);

        if (startSuspensionEvent.wait(10000))
        {
            jassert(getCurrentThreadId() == threadId.get());

            if (affinityMask != 0) setCurrentThreadAffinityMask(affinityMask);
        ...
        if (shouldDeleteThis) delete this;
    }
    ...
    {
        if (auto* currentThread = getCurrentThread()) return currentThread->threadShouldExit();
    ...
    bool Thread::waitForThreadToExit(const int timeOutMilliseconds) const
    {
        ...
        {
            if (timeOutMilliseconds >= 0 && Time::getMillisecondCounter() > timeoutEnd) return false;
    ...
    bool Thread::stopThread(const int timeOutMilliseconds)
    {
        ...
        if (isThreadRunning())
        {
            ...
            if (timeOutMilliseconds != 0) waitForThreadToExit(timeOutMilliseconds);
    ...
    bool Thread::setPriority(int newPriority)
    {
        ...
        if (getCurrentThreadId() == getThreadId()) return setCurrentThreadPriority(newPriority);
