...
namespace juce
{
    Uuid::Uuid()
    {
        ...
        for (size_t i = 0; i < sizeof(uuid); ++i) uuid[i] = (uint8)(r.nextInt(256));
    ...
    int Uuid::compare(Uuid other) const noexcept
    {
        for (size_t i = 0; i < sizeof(uuid); ++i)
            if (int diff = uuid[i] - (int)other.uuid[i]) return diff > 0 ? 1 : -1;
    ...
    {
        return getHexRegion(0, 4) + "-" + getHexRegion(4, 2) + "-" + getHexRegion(6, 2) + "-" + getHexRegion(8, 2) + "-"
               + getHexRegion(10, 6);
    }
