...
namespace juce
{
    SubregionStream::SubregionStream(InputStream* sourceStream, int64 start, int64 length, bool deleteSourceWhenDestroyed)
    ...
    int64 SubregionStream::getTotalLength()
    {
        ...
        return lengthOfSourceStream >= 0 ? jmin(lengthOfSourceStream, srcLen) : srcLen;
    }
    ...
    int SubregionStream::read(void* destBuffer, int maxBytesToRead)
    {
        ...
        if (lengthOfSourceStream < 0) return source->read(destBuffer, maxBytesToRead);
    ...
    {
        if (lengthOfSourceStream >= 0 && getPosition() >= lengthOfSourceStream) return true;
