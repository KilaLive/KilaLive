...
namespace juce
{
    ...
    bool OutputStream::writeText(const String& text, bool asUTF16, bool writeUTF16ByteOrderMark)
    {
        if (asUTF16)
        {
            if (writeUTF16ByteOrderMark) write("\x0ff\x0fe", 2);
            ...
            for (;;)
            {
                ...
                if (c == '\n' && !lastCharWasReturn) writeShort((short)'\r');

                lastCharWasReturn = (c == L'\r');

                if (!writeShort((short)c)) return false;
            }
        }
        else
        {
            ...
            for (;;)
            {
                ...
                {
                    if (t > src)
                        if (!write(src, (size_t)(t - src))) return false;
    ...
    {
        if (data.getSize() > 0) stream.write(data.getData(), data.getSize());
    ...
    JUCE_API OutputStream& JUCE_CALLTYPE operator<<(OutputStream& stream, const File& fileToRead)
    {
        ...
        if (in.openedOk()) return stream << in;
