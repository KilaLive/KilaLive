...
namespace juce
{
    static inline int calcBufferStreamBufferSize(int requestedSize, InputStream* source) noexcept
    {
        ...
        if (sourceSize >= 0 && sourceSize < requestedSize) return jmax(32, (int)sourceSize);
    ...
    bool BufferedInputStream::ensureBuffered()
    {
        ...
        if (position < bufferStart || position >= bufferEndOverlap)
        {
            ...
            if (position < lastReadPos && position >= bufferEndOverlap && position >= bufferStart)
            {
                auto bytesToKeep = (int)(lastReadPos - position);
                memmove(buffer, buffer + (int)(position - bufferStart), (size_t)bytesToKeep);

                bufferStart = position;
                bytesRead   = source->read(buffer + bytesToKeep, (int)(bufferSize - bytesToKeep));
            ...
            else
            {
                ...
                if (!source->setPosition(bufferStart)) return false;
            ...
            while (bytesRead < bufferSize) buffer[bytesRead++] = 0;
        }
    ...
    int BufferedInputStream::read(void* destBuffer, int maxBytesToRead)
    {
        ...
        if (position >= bufferStart && position + maxBytesToRead <= lastReadPos)
        {
        ...
        while (maxBytesToRead > 0)
        {
            ...
            if (!ensureBuffered() || oldLastReadPos == lastReadPos || isExhausted()) break;
        }
