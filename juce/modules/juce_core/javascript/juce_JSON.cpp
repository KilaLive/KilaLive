...
namespace juce
{
    struct JSONParser
    {
        ...
        static Result parseAny(String::CharPointerType& t, var& result)
        {
            ...
            case '-':
                ...
                if (!CharacterFunctions::isDigit(*t2)) break;
            ...
            case 'f':  // "false"
                if (t2.getAndAdvance() == 'a' && t2.getAndAdvance() == 'l' && t2.getAndAdvance() == 's'
                    && t2.getAndAdvance() == 'e')
                {
                    t      = t2;
                    result = var(false);
    ...
    private:
        ...
        {
            String m(message);
            if (location != nullptr) m << ": \"" << String(*location, 20) << '"';
        ...
        static Result parseNumber(String::CharPointerType& t, var& result, const bool isNegative)
        {
            ...
            for (;;)
            {
                ...
                if (CharacterFunctions::isWhitespace(c) || c == ',' || c == '}' || c == ']' || c == 0)
                {
                    t = previousChar;
                    break;
                }
    ...
    struct JSONFormatter
    {
                ...
                default:
                    ...
                    else
                    {
                        if (CharPointer_UTF16::getBytesRequiredFor(c) > 2)
                        {
                            ...
                            for (int i = 0; i < 2; ++i) writeEscapedChar(out, (unsigned short)chars[i]);
                        }
                        else
                        {
        ...
        static void writeArray(OutputStream& out, const Array<var>& array, int indentLevel, bool allOnOneLine,
    ...
    var JSON::parse(const String& text)
    {
        ...
        if (!parse(text, result)) result = var();
    ...
    var JSON::fromString(StringRef text)
    {
        ...
        if (!JSONParser::parseAny(text.text, result)) result = var();
    ...
    Result JSON::parseQuotedString(String::CharPointerType& t, var& result)
    {
        ...
        if (quote == '"' || quote == '\'') return JSONParser::parseString(quote, t, result);
    ...
    public:
        ...
        static var createRandomVar(Random& r, int depth)
        {
            ...
            case 6:
            {
                ...
                for (int i = 1 + r.nextInt(30); --i >= 0;) v.append(createRandomVar(r, depth + 1));
