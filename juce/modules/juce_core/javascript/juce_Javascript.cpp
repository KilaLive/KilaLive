...
#define JUCE_JS_OPERATORS(X)                                                                                                 \
    X(semicolon, ";")                                                                                                        \
    X(dot, ".") X(comma, ",") X(openParen, "(") X(closeParen, ")") X(openBrace, "{") X(closeBrace, "}") X(openBracket, "[")  \
        X(closeBracket, "]") X(colon, ":") X(question, "?") X(typeEquals, "===") X(equals, "==") X(assign, "=")              \
            X(typeNotEquals, "!==") X(notEquals, "!=") X(logicalNot, "!") X(plusEquals, "+=") X(plusplus, "++") X(plus, "+") \
                X(minusEquals, "-=") X(minusminus, "--") X(minus, "-") X(timesEquals, "*=") X(times, "*")                    \
                    X(divideEquals, "/=") X(divide, "/") X(moduloEquals, "%=") X(modulo, "%") X(xorEquals, "^=")             \
                        X(bitwiseXor, "^") X(andEquals, "&=") X(logicalAnd, "&&") X(bitwiseAnd, "&") X(orEquals, "|=")       \
                            X(logicalOr, "||") X(bitwiseOr, "|") X(leftShiftEquals, "<<=") X(lessThanOrEqual, "<=")          \
                                X(leftShift, "<<") X(lessThan, "<") X(rightShiftUnsigned, ">>>") X(rightShiftEquals, ">>=")  \
                                    X(rightShift, ">>") X(greaterThanOrEqual, ">=") X(greaterThan, ">")

#define JUCE_JS_KEYWORDS(X)                                                                                                  \
    X(var, "var")                                                                                                            \
    X(if_, "if") X(else_, "else") X(do_, "do") X(null_, "null") X(while_, "while") X(for_, "for") X(break_, "break")         \
        X(continue_, "continue") X(undefined, "undefined") X(function, "function") X(return_, "return") X(true_, "true")     \
    ...
    struct JavascriptEngine::RootObject : public DynamicObject
    {
        ...
        struct Scope
        {
            ...
            var findFunctionCall(const CodeLocation& location, const var& targetObject, const Identifier& functionName) const
            {
                ...
                {
                    if (auto* prop = getPropertyPointer(o, functionName)) return *prop;
                    ...
                    {
                        if (auto* prop = getPropertyPointer(p, functionName)) return *prop;
                    }

                    // if there's a class with an overridden DynamicObject::hasMethod, this avoids an error
                    if (o->hasMethod(functionName)) return {};
                }

                if (targetObject.isString())
                    if (auto* m = findRootClassProperty(StringClass::getClassName(), functionName)) return *m;

                if (targetObject.isArray())
                    if (auto* m = findRootClassProperty(ArrayClass::getClassName(), functionName)) return *m;

                if (auto* m = findRootClassProperty(ObjectClass::getClassName(), functionName)) return *m;
            ...
            {
                if (auto* cls = root->getProperty(className).getDynamicObject()) return getPropertyPointer(cls, propName);
            ...
            {
                if (auto* v = getPropertyPointer(scope, name)) return *v;

                return parent != nullptr ? parent->findSymbolInParentScopes(name) : var::undefined();
            }
            ...
            bool findAndInvokeMethod(const Identifier& function, const var::NativeFunctionArgs& args, var& result) const
            {
                ...
                for (int i = 0; i < props.size(); ++i)
                    if (auto* o = props.getValueAt(i).getDynamicObject())
                        if (Scope(this, root, o).findAndInvokeMethod(function, args, result)) return true;
        ...
        struct BlockStatement : public Statement
        {
            ...
            ResultCode perform(const Scope& s, var* returnedValue) const override
            {
                for (auto* statement : statements)
                    if (auto r = statement->perform(s, returnedValue)) return r;
        ...
        struct LoopStatement : public Statement
        {
            ...
            ResultCode perform(const Scope& s, var* returnedValue) const override
            {
                ...
                while (isDoLoop || condition->getResult(s))
                {
                    ...
                    if (isDoLoop && r != continueWasHit && !condition->getResult(s)) break;
                }
        ...
        struct DotOperator : public Expression
        {
            ...
            var getResult(const Scope& s) const override
            {
                ...
                if (auto* o = p.getDynamicObject())
                    if (auto* v = getPropertyPointer(o, child)) return *v;
        ...
        struct ArraySubscript : public Expression
        {
            ...
            var getResult(const Scope& s) const override
            {
                ...
                if (const auto* array = arrayVar.getArray())
                    if (key.isInt() || key.isInt64() || key.isDouble()) return (*array)[static_cast<int>(key)];

                if (auto* o = arrayVar.getDynamicObject())
                    if (key.isString())
                        if (auto* v = getPropertyPointer(o, Identifier(key))) return *v;
            ...
            void assign(const Scope& s, const var& newValue) const override
            {
                ...
                if (auto* array = arrayVar.getArray())
                {
                    ...
                    {
                        const int i = key;
                        while (array->size() < i) array->add(var::undefined());
        ...
        struct BinaryOperator : public BinaryOperatorBase
        {
            ...
            var getResult(const Scope& s) const override
            {
                ...
                if ((a.isUndefined() || a.isVoid()) && (b.isUndefined() || b.isVoid())) return getWithUndefinedArg();

                if (isNumericOrUndefined(a) && isNumericOrUndefined(b))
                    return (a.isDouble() || b.isDouble()) ? getWithDoubles(a, b) : getWithInts(a, b);

                if (a.isArray() || a.isObject()) return getWithArrayOrObject(a, b);
        ...
        struct FunctionCall : public Expression
        {
            ...
            var invokeFunction(const Scope& s, const var& function, const var& thisObject) const
            {
                ...
                for (auto* a : arguments) argVars.add(a->getResult(s));

                const var::NativeFunctionArgs args(thisObject, argVars.begin(), argVars.size());

                if (var::NativeFunction nativeFunction = function.getNativeFunction()) return nativeFunction(args);

                if (auto* fo = dynamic_cast<FunctionObject*>(function.getObject())) return fo->invoke(s, args);

                if (auto* dot = dynamic_cast<DotOperator*>(object.get()))
                    if (auto* o = thisObject.getDynamicObject())
                        if (o->hasMethod(
                                dot->child))  // allow an overridden DynamicObject::invokeMethod to accept a method call.
        ...
        struct NewOperator : public FunctionCall
        {
            ...
            var getResult(const Scope& s) const override
            {
                ...
                if (!(isFunc || classOrFunc.getDynamicObject() != nullptr)) return var::undefined();
        ...
        struct ArrayDeclaration : public Expression
        {
            ...
            var getResult(const Scope& s) const override
            {
                ...
                for (int i = 0; i < values.size(); ++i) a.add(values.getUnchecked(i)->getResult(s));
        ...
        struct FunctionObject : public DynamicObject
        {
            ...
            void writeAsJSON(OutputStream& out, int /*indentLevel*/, bool /*allOnOneLine*/,
...
#define JUCE_JS_COMPARE_KEYWORD(name, str)                                                                                   \
    if (len == sizeof(str) - 1 && matchToken(TokenTypes::name, len)) return TokenTypes::name;
                ...
                if (parseStringLiteral(*p) || (*p == '.' && parseFloatLiteral())) return TokenTypes::literal;
    ...
    if (matchToken(TokenTypes::name, sizeof(str) - 1)) return TokenTypes::name;
        ...
        struct ExpressionTreeBuilder : private TokenIterator
        {
            ...
            void parseFunctionParamsAndBody(FunctionObject& fo)
            {
                ...
                while (currentType != TokenTypes::closeParen)
                {
                    ...
                    if (currentType != TokenTypes::closeParen) match(TokenTypes::comma);
                }
        ...
        private:
            ...
            Statement* parseStatement()
            {
                ...
                if (matchesAny(TokenTypes::openParen, TokenTypes::openBracket)) return matchEndOfStatement(parseFactor());
            ...
            {
                if (matchIf(TokenTypes::semicolon)) return new ReturnStatement(location, new Expression(location));
            ...
            Statement* parseFunction()
            {
                ...
                if (name.isNull()) throwError("Functions defined at statement-level must have a name");
            ...
            Statement* parseDoOrWhileLoop(bool isDoLoop)
            {
                ...
                if (!isDoLoop) s->body.reset(parseStatement());
            ...
            {
                Identifier i;
                if (currentType == TokenTypes::identifier) i = currentValue.toString();
            ...
            Expression* parseSuffixes(Expression* e)
            {
                ...
                if (matchIf(TokenTypes::dot)) return parseSuffixes(new DotOperator(location, input, parseIdentifier()));
            ...
            Expression* parseFactor()
            {
                ...
                if (matchIf(TokenTypes::openBrace))
                {
                    ...
                    while (currentType != TokenTypes::closeBrace)
                    {
                        ...
                        match((currentType == TokenTypes::literal && currentValue.isString()) ? TokenTypes::literal
                        ...
                        if (currentType != TokenTypes::closeBrace) match(TokenTypes::comma);
                    }
                ...
                if (matchIf(TokenTypes::openBracket))
                {
                    ...
                    while (currentType != TokenTypes::closeBracket)
                    {
                        ...
                        if (currentType != TokenTypes::closeBracket) match(TokenTypes::comma);
                    }
                ...
                if (matchIf(TokenTypes::function))
                {
                    ...
                    if (name.isValid()) throwError("Inline functions definitions cannot have a name");
                ...
                if (matchIf(TokenTypes::new_))
                {
                    ...
                    while (matchIf(TokenTypes::dot)) name.reset(new DotOperator(location, name, parseIdentifier()));
        ...
        struct ArrayClass : public DynamicObject
        {
            ...
            {
                if (auto* array = a.thisObject.getArray()) return array->contains(get(a, 0));
            ...
            {
                if (auto* array = a.thisObject.getArray()) array->removeAllInstancesOf(get(a, 0));
            ...
            static var join(Args a)
            {
                ...
                if (auto* array = a.thisObject.getArray())
                    for (auto& v : *array) strings.add(v.toString());
            ...
            static var push(Args a)
            {
                if (auto* array = a.thisObject.getArray())
                {
                    for (int i = 0; i < a.numArguments; ++i) array->add(a.arguments[i]);
            ...
            static var splice(Args a)
            {
                if (auto* array = a.thisObject.getArray())
                {
                    ...
                    const int num = a.numArguments > 1 ? jlimit(0, arraySize - start, getInt(a, 1)) : arraySize - start;

                    Array<var> itemsRemoved;
                    itemsRemoved.ensureStorageAllocated(num);

                    for (int i = 0; i < num; ++i) itemsRemoved.add(array->getReference(start + i));

                    array->removeRange(start, num);

                    for (int i = 2; i < a.numArguments; ++i) array->insert(start++, get(a, i));
            ...
            static var indexOf(Args a)
            {
                if (auto* array = a.thisObject.getArray())
                {
                    ...
                    for (int i = (a.numArguments > 1 ? getInt(a, 1) : 0); i < array->size(); ++i)
                        if (array->getReference(i) == target) return i;
                }
        ...
        struct StringClass : public DynamicObject
        {
            ...
            static var split(Args a)
            {
                ...
                if (sep.isNotEmpty())
                    ...
                    for (auto pos = str.getCharPointer(); !pos.isEmpty(); ++pos) strings.add(String::charToString(*pos));

                var array;

                for (auto& s : strings) array.append(s);
        ...
        struct MathClass : public DynamicObject
        {
            ...
            {
                return (isInt(a, 0) && isInt(a, 1)) ? var(jmin(getInt(a, 0), getInt(a, 1)))
                                                    : var(jmin(getDouble(a, 0), getDouble(a, 1)));
            }
            static var Math_max(Args a)
            {
                return (isInt(a, 0) && isInt(a, 1)) ? var(jmax(getInt(a, 0), getInt(a, 1)))
        ...
        struct IntegerClass : public DynamicObject
        {
            ...
            static var parseInt(Args a)
            {
                ...
                return s[0] == '0' ? (s[1] == 'x' ? s.substring(2).getHexValue64() : getOctalValue(s)) : s.getLargeIntValue();
            }
        };
        ...
        {
            if (auto* root = dynamic_cast<RootObject*>(a.thisObject.getObject())) root->execute(getString(a, 0));
        ...
        {
            if (auto* root = dynamic_cast<RootObject*>(a.thisObject.getObject())) return root->evaluate(getString(a, 0));
