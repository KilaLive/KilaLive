...
namespace juce
{
    ...
    {
        if (fallback != nullptr && !translations.containsKey(text)) return fallback->translate(text);
    ...
    {
        if (fallback != nullptr && !translations.containsKey(text)) return fallback->translate(text, resultIfNotFound);
        ...
        for (auto& l : lines)
        {
            ...
            if (line.startsWithChar('"'))
            {
                ...
                if (originalText.isNotEmpty())
                {
                    ...
                    if (newText.isNotEmpty()) translations.set(originalText, newText);
                }
            }
    ...
    JUCE_API String translate(const String& text, const String& resultIfNotFound)
    {
        ...
        if (auto* mappings = LocalisedStrings::getCurrentMappings()) return mappings->translate(text, resultIfNotFound);
