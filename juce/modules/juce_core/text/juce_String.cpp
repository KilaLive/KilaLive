...
#error "JUCE_STRINGS_ARE_UNICODE is deprecated! All strings are now unicode by default."
    ...
    public:
        ...
        {
            if (text.getAddress() == nullptr || text.isEmpty()) return CharPointerType(&(emptyString.text));
        ...
        {
            if (text.getAddress() == nullptr || text.isEmpty() || maxChars == 0) return CharPointerType(&(emptyString.text));
        ...
        {
            if (start.getAddress() == nullptr || start.isEmpty()) return CharPointerType(&(emptyString.text));
        ...
        static CharPointerType createFromCharPointer(const CharPointerType start, const CharPointerType end)
        {
            ...
            auto numBytes =
                (size_t)(reinterpret_cast<const char*>(end.getAddress()) - reinterpret_cast<const char*>(start.getAddress()));
        ...
        static void retain(const CharPointerType text) noexcept
        {
            ...
            if (b != (StringHolder*)&emptyString) ++(b->refCount);
        }
        ...
        static CharPointerType makeUniqueWithByteSize(const CharPointerType text, size_t numBytes)
        {
            ...
            if (b->allocatedNumBytes >= numBytes && b->refCount.get() <= 0) return text;
    ...
    private:
        static inline StringHolder* bufferFromText(const CharPointerType text) noexcept
        {
            ...
            return reinterpret_cast<StringHolder*>(
                reinterpret_cast<char*>(text.getAddress())
...
#error "native wchar_t size is unknown"
    ...
    String::String(double number)
        : text(NumberToStringConverters::createFromDouble(number, 0))
    {
    }
    ...
    {
        return t != nullptr ? text.compareIgnoreCase(CharPointer_UTF8(t)) == 0 : isEmpty();
    }
    ...
    static int naturalStringCompare(String::CharPointerType s1, String::CharPointerType s2, bool isCaseSensitive) noexcept
    {
        ...
        for (;;)
        {
            ...
            {
                auto result = (*s1 == '0' || *s2 == '0') ? stringCompareLeft(s1, s2) : stringCompareRight(s1, s2);
    ...
    {
        appendCharPointer(this == &textToAppend ? String(textToAppend).text : textToAppend.text, maxCharsToTake);
    }
    ...
    void String::appendCharPointer(const CharPointerType startOfTextToAppend, const CharPointerType endOfTextToAppend)
    {
        jassert(startOfTextToAppend.getAddress() != nullptr && endOfTextToAppend.getAddress() != nullptr);

        auto extraBytesNeeded = getAddressDifference(endOfTextToAppend.getAddress(), startOfTextToAppend.getAddress());
    ...
    JUCE_API String& JUCE_CALLTYPE operator<<(String& s1, const unsigned long number)
    {
        return s1 += String(number);
    }
    ...
    int String::indexOfWholeWordIgnoreCase(StringRef word) const noexcept
    {
        if (word.isNotEmpty())
        {
            ...
            for (int i = 0; i <= end; ++i)
            {
                if (t.compareIgnoreCaseUpTo(word.text, wordLen) == 0 && (i == 0 || !(t - 1).isLetterOrDigit())
                    && !(t + wordLen).isLetterOrDigit())
                    return i;
    ...
    String String::paddedLeft(const juce_wchar padCharacter, int minimumLength) const
    {
        ...
        String result(
            PreallocationBytes(currentByteSize + (size_t)extraChars * CharPointerType::getBytesRequiredFor(padCharacter)));
        auto n = result.text;
    ...
    String String::replaceSection(int index, int numCharsToReplace, StringRef stringToInsert) const
    {
        ...
        auto remainderBytes =
            (size_t)(((char*)startOfRemainder.findTerminatingNull().getAddress()) - (char*)startOfRemainder.getAddress());
    ...
    String String::replace(StringRef stringToReplace, StringRef stringToInsert, const bool ignoreCase) const
    {
        ...
        while ((i = (ignoreCase ? result.indexOfIgnoreCase(i, stringToReplace) : result.indexOf(i, stringToReplace))) >= 0)
        {
    ...
    struct StringCreationHelper
    {
        ...
        StringCreationHelper(const String::CharPointerType s)
            : source(s)
            , allocatedBytes(StringHolder::getAllocatedNumBytes(s))
        {
            result.preallocateBytes(allocatedBytes);
            dest = result.getCharPointer();
        }
    ...
    String String::createHex(uint8 n)
    {
        return hexToString(n);
    }
    String String::createHex(uint16 n)
    {
        return hexToString(n);
    }
    String String::createHex(uint32 n)
    {
        return hexToString(n);
    }
    String String::createHex(uint64 n)
    {
        return hexToString(n);
    }
    ...
    String String::toHexString(const void* const d, const int size, const int groupSize)
    {
        ...
        for (int i = 0; i < size; ++i)
        {
            ...
            if (groupSize > 0 && (i % groupSize) == (groupSize - 1) && i < (size - 1)) dest.write((juce_wchar)' ');
        }
    ...
    String String::createStringFromData(const void* const unknownData, int size)
    {
        ...
        if (size == 1) return charToString((juce_wchar)data[0]);

        if (CharPointer_UTF16::isByteOrderMarkBigEndian(data) || CharPointer_UTF16::isByteOrderMarkLittleEndian(data))
        {
            ...
            {
                for (int i = 0; i < numChars; ++i) builder.write((juce_wchar)ByteOrder::swapIfLittleEndian(src[i]));
            }
            else
            {
                for (int i = 0; i < numChars; ++i) builder.write((juce_wchar)ByteOrder::swapIfBigEndian(src[i]));
            }
    ...
    struct StringEncodingConverter
    {
        static CharPointerType_Dest convert(const String& s)
        {
            ...
            if (source.isEmpty()) return CharPointerType_Dest(reinterpret_cast<const DestChar*>(&emptyChar));
    ...
    struct StringCopier
    {
        static size_t copyToBuffer(const CharPointerType_Src source, typename CharPointerType_Dest::CharType* const buffer,
