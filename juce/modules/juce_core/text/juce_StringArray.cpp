...
namespace juce
{
    ...
    {
        std::sort(strings.begin(), strings.end(), [](const String& a, const String& b) { return a.compareNatural(b) < 0; });
    }
    ...
    int StringArray::addTokens(StringRef text, StringRef breakCharacters, StringRef quoteCharacters)
    {
        ...
        if (text.isNotEmpty())
        {
            ...
            {
                auto tokenEnd = CharacterFunctions::findEndOfToken(t, breakCharacters.text, quoteCharacters.text);
    ...
    StringArray StringArray::fromTokens(StringRef stringToTokenise, StringRef breakCharacters, StringRef quoteCharacters)
    {
        StringArray s;
    ...
    void StringArray::appendNumbersToDuplicates(bool ignoreCase, bool appendNumberToFirstInstance,
                                                CharPointer_UTF8 preNumberString, CharPointer_UTF8 postNumberString)
    {
        if (preNumberString.getAddress() == nullptr) preNumberString = CharPointer_UTF8(" (");

        if (postNumberString.getAddress() == nullptr) postNumberString = CharPointer_UTF8(")");
