...
namespace juce
{
    struct FallbackDownloadTask : public URL::DownloadTask, public Thread
    {
        FallbackDownloadTask(FileOutputStream* outputStreamToUse, size_t bufferSizeToUse, WebInputStream* streamToUse,
        ...
        void run() override
        {
            ...
            {
                if (listener != nullptr) listener->progress(this, downloaded, contentLength);
                ...
                if (actual < 0 || threadShouldExit() || stream->isError()) break;
            ...
            if (threadShouldExit() || stream->isError()) error = true;

            if (contentLength > 0 && downloaded < contentLength) error = true;

            finished = true;

            if (listener != nullptr && !threadShouldExit()) listener->finished(this, !error);
        }
    ...
    //==============================================================================
    URL::DownloadTask* URL::DownloadTask::createFallbackDownloader(const URL& urlToUse, const File& targetFileToUse,
                                                                   const String& extraHeadersToUse, Listener* listenerToUse,
                                                                   bool usePostRequest)
    {
    ...
    URL::URL(File localFile)
    {
        ...
        if (!url.startsWithChar(L'/')) url = "/" + url;
    ...
    void URL::init()
    {
        ...
        if (i >= 0)
        {
            do
            {
                ...
                {
                    addParameter(
                        removeEscapeChars(equalsPos < 0 ? url.substring(i + 1, nextAmp) : url.substring(i + 1, equalsPos)),
    ...
    {
        return url == other.url && postData == other.postData && parameterNames == other.parameterNames
               && parameterValues == other.parameterValues && filesToUpload == other.filesToUpload;
    }
    ...
    namespace URLHelpers
    {
        static String getMangledParameters(const URL& url)
        {
            ...
            for (int i = 0; i < url.getParameterNames().size(); ++i)
            {
                ...
                if (val.isNotEmpty()) p << '=' << URL::addEscapeChars(val, true);
            }
        ...
        static int findEndOfScheme(const String& url)
        {
            ...
            while (CharacterFunctions::isLetterOrDigit(url[i]) || url[i] == '+' || url[i] == '-' || url[i] == '.') ++i;
    ...
    {
        if (includeGetParameters && parameterNames.size() > 0) return url + "?" + URLHelpers::getMangledParameters(*this);
    ...
    String URL::getSubPath() const
    {
        ...
        return startOfPath <= 0 ? String() : url.substring(startOfPath);
    }
    ...
    URL URL::withNewSubPath(const String& newPath) const
    {
        ...
        if (startOfPath > 0) u.url = url.substring(0, startOfPath);
    ...
    void URL::createHeadersAndPostData(String& headers, MemoryBlock& postDataToWrite) const
    {
        ...
        else
        {
            ...
            if (!headers.containsIgnoreCase("Content-Type")) headers << "Content-Type: application/x-www-form-urlencoded\r\n";
    ...
    bool URL::isProbablyAWebsiteURL(const String& possibleURL)
    {
        ...
        for (auto* protocol : validProtocols)
            if (possibleURL.startsWithIgnoreCase(protocol)) return true;

        if (possibleURL.containsChar('@') || possibleURL.containsChar(' ')) return false;

        const String topLevelDomain(
            possibleURL.upToFirstOccurrenceOf("/", false, false).fromLastOccurrenceOf(".", false, false));
    ...
    bool URL::isProbablyAnEmailAddress(const String& possibleEmailAddress)
    {
        ...
        return atSign > 0 && possibleEmailAddress.lastIndexOfChar('.') > (atSign + 1)
    ...
    {
        if (u.bookmark.get() == nullptr) return nullptr;
    ...
    {
        static void flush(OutputStream* o)
        {
            o->flush();
        }
    };
    ...
    public:
        ...
        ~iOSFileStreamWrapper()
        {
            ...
            if (NSData* bookmark = (NSData*)getURLBookmark(url))
            {
                ...
                if (error == nil)
                {
                    if (isBookmarkStale) updateStaleBookmark(nsURL, url);
        ...
        File getLocalFileAccess(URL& urlToUse)
        {
            if (NSData* bookmark = (NSData*)getURLBookmark(urlToUse))
            {
                ...
                if (error == nil)
                {
                    ...
                    if (isBookmarkStale) updateStaleBookmark(nsURL, urlToUse);
    ...
    //==============================================================================
    InputStream* URL::createInputStream(const bool usePostCommand, OpenStreamProgressCallback* const progressCallback,
                                        void* const progressCallbackContext, String headers, const int timeOutMs,
                                        StringPairArray* const responseHeaders, int* statusCode,
                                        const int numRedirectsToFollow, String httpRequestCmd) const
    {
        if (isLocalFile())
        {
#if JUCE_IOS
        ...
        struct ProgressCallbackCaller : public WebInputStream::Listener
        {
            ProgressCallbackCaller(OpenStreamProgressCallback* progressCallbackToUse, void* progressCallbackContextToUse)
                : callback(progressCallbackToUse)
                , data(progressCallbackContextToUse)
            {
            }
            ...
            ProgressCallbackCaller(const ProgressCallbackCaller& o)
                : callback(o.callback)
                , data(o.data)
            {
                jassertfalse;
        ...
        if (headers.isNotEmpty()) wi->withExtraHeaders(headers);

        if (timeOutMs != 0) wi->withConnectionTimeout(timeOutMs);

        if (httpRequestCmd.isNotEmpty()) wi->withCustomRequestCommand(httpRequestCmd);

        wi->withNumRedirectsToFollow(numRedirectsToFollow);

        bool success = wi->connect(callbackCaller.get());

        if (statusCode != nullptr) *statusCode = wi->getStatusCode();

        if (responseHeaders != nullptr) *responseHeaders = wi->getResponseHeaders();

        if (!success || wi->isError()) return nullptr;
    ...
    String URL::readEntireTextStream(bool usePostCommand) const
    {
        ...
        if (in != nullptr) return in->readEntireStreamAsString();
    ...
    //==============================================================================
    URL URL::withParameter(const String& parameterName, const String& parameterValue) const
    {
        URL u(*this);
    ...
    URL URL::withUpload(Upload* const f) const
    {
        ...
        for (int i = u.filesToUpload.size(); --i >= 0;)
            if (u.filesToUpload.getObjectPointerUnchecked(i)->parameterName == f->parameterName) u.filesToUpload.remove(i);
    ...
    URL URL::withDataToUpload(const String& parameterName, const String& filename, const MemoryBlock& fileContentToUpload,
    ...
    String URL::removeEscapeChars(const String& s)
    {
        ...
        if (!result.containsChar('%')) return result;
    ...
    String URL::addEscapeChars(const String& s, bool isParameter, bool roundBracketsAreLegal)
    {
        ...
        if (roundBracketsAreLegal) legalChars += "()";
        ...
        for (int i = 0; i < utf8.size(); ++i)
        {
            ...
            if (!(CharacterFunctions::isLetterOrDigit(c) || legalChars.containsChar((juce_wchar)c)))
            {
                utf8.set(i, '%');
    ...
    bool URL::launchInDefaultBrowser() const
    {
        ...
        if (u.containsChar('@') && !u.containsChar(':')) u = "mailto:" + u;
