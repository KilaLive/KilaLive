...
namespace juce
{
    ...
    IPAddress::IPAddress(const uint8 bytes[], bool IPv6) noexcept
        : isIPv6(IPv6)
    {
        for (int i = 0; i < (isIPv6 ? 16 : 4); ++i) address[i] = bytes[i];

        if (!isIPv6) zeroUnusedBytes();
    }
    ...
    IPAddress::IPAddress(const String& adr)
    {
        ...
        if (!isIPv6)
        {
            ...
            for (int i = 0; i < 4; ++i) address[i] = (uint8)tokens[i].getIntValue();
        }
        else
        {
            StringArray tokens;
            ...
            if (tokens.contains(StringRef()))  // if :: shorthand has been used
            {
                ...
                while (tokens.size() < 8) tokens.insert(idx, "0");
            }
    ...
    String IPAddress::toString() const
    {
        if (!isIPv6)
        {
            ...
            for (int i = 1; i < 4; ++i) s << '.' << (int)address[i];
    ...
    IPAddress IPAddress::any(bool IPv6) noexcept
    {
    ...
    String IPAddress::getFormattedAddress(const String& unformattedAddress)
    {
        jassert(unformattedAddress.contains(":")
                && !unformattedAddress.contains("::"));  // needs to be an unformatted IPv6 address!
        ...
        for (int i = 0; i < tokens.size(); ++i)
        {
            ...
            if (t.getHexValue32() == 0x0000)
            {
                ...
                if (t.length() > 1) addressString = addressString.replace(String::repeatedString("0", t.length()), "0");

                if (isFirst && numZerosTemp != 0 && numZeros > numZerosTemp) isFirst = false;
            }
            else
            {
                ...
                if (numZeros > 0)
                {
                    if (numZeros > numZerosTemp) numZerosTemp = numZeros;
        ...
        if (numZerosTemp > numZeros) numZeros = numZerosTemp;
        ...
        if (numZeros > 1)
        {
            ...
            {
                auto zeroString =
                    isFirst ? "0" + String::repeatedString(":0", numZeros - 1) : String::repeatedString(":0", numZeros);

                addressString = addressString.replaceFirstOccurrenceOf(zeroString, ":");

                if (isLast) addressString << ':';
            }
        }

        if (portString.isNotEmpty()) addressString = "[" + addressString + "]" + portString;
    ...
    void IPAddress::findAllAddresses(Array<IPAddress>& result, bool includeIPv6)
    {
        ...
        for (ifa = ifaddr; ifa != nullptr; ifa = ifa->ifa_next)
        {
            ...
            if (ifa->ifa_addr->sa_family == AF_INET)
                addAddress((const sockaddr_in*)ifa->ifa_addr, result);
            else if (ifa->ifa_addr->sa_family == AF_INET6 && includeIPv6)
                addAddress((const sockaddr_in6*)ifa->ifa_addr, result);
        }
