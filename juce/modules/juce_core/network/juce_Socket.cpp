...
#define AI_NUMERICSERV 0x1000
    ...
    namespace SocketHelpers
    {
        ...
        {
            return handle > 0 && setOption(handle, SO_RCVBUF, (int)65536) && setOption(handle, SO_SNDBUF, (int)65536)
                   && (isDatagram ? ((!allowBroadcast) || setOption(handle, SO_BROADCAST, (int)1))
                                  : setOption(handle, IPPROTO_TCP, TCP_NODELAY, (int)1));
        }

        static void closeSocket(volatile int& handle, CriticalSection& readLock, bool isListener, int portNumber,
                                bool& connected) noexcept
        {
            ...
            if (h != (unsigned)SOCKET_ERROR || connected) closesocket(h);
        ...
        {
            if (handle <= 0 || !isValidPortNumber(port)) return false;
        ...
        static int getBoundPort(SocketHandle handle) noexcept
        {
            if (handle > 0)
            {
                ...
                if (getsockname(handle, (struct sockaddr*)&addr, &len) == 0) return ntohs(addr.sin_port);
            }
        ...
        static String getConnectedAddress(SocketHandle handle) noexcept
        {
            ...
            if (getpeername(handle, (struct sockaddr*)&addr, &len) >= 0) return inet_ntoa(addr.sin_addr);

            return String("0.0.0.0");
        }

        static int readSocket(SocketHandle handle, void* const destBuffer, const int maxBytesToRead, bool volatile& connected,
                              bool blockUntilSpecifiedAmountHasArrived, CriticalSection& readLock, String* senderIP = nullptr,
                              int* senderPort = nullptr) noexcept
        {
            ...
            while (bytesRead < maxBytesToRead)
            {
                ...
                {
                    if (bytesRead == 0 && blockUntilSpecifiedAmountHasArrived) bytesRead = -1;
                ...
                if (!blockUntilSpecifiedAmountHasArrived) break;
            }

            return (int)bytesRead;
        }

        static int waitForReadiness(const volatile int& handle, CriticalSection& readLock, const bool forReading,
                                    const int timeoutMsecs) noexcept
        {
            // avoid race-condition
            CriticalSection::ScopedTryLockType lock(readLock);
...
#if JUCE_WINDOWS
            if (select((int)h + 1, prset, pwset, 0, timeoutp) < 0) return -1;
#else
            {
                int result;
                while ((result = select(h + 1, prset, pwset, 0, timeoutp)) < 0 && errno == EINTR)
                {
                }
...

            {
                ...
                if (getsockopt(h, SOL_SOCKET, SO_ERROR, (char*)&opt, &len) < 0 || opt != 0) return -1;
            }
        ...
        static addrinfo* getAddressInfo(const bool isDatagram, const String& hostName, int portNumber)
        {
            ...
            if (getaddrinfo(hostName.toRawUTF8(), String(portNumber).toRawUTF8(), &hints, &info) == 0) return info;

            return nullptr;
        }

        static bool connectSocket(int volatile& handle, CriticalSection& readLock, const String& hostName,
                                  const int portNumber, const int timeOutMillisecs) noexcept
        {
            bool success = false;
                    ...
                    if (newHandle != invalidSocket)
                    {
                        ...
                        if (!success)
                        {
...
#endif
                            {
                                ...
                                if (waitForReadiness(cvHandle, readLock, false, timeOutMillisecs) == 1) success = true;
                            }
                        }
        ...
        static bool multicast(int handle, const String& multicastIPAddress, const String& interfaceIPAddress,
                              bool join) noexcept
        {
            struct ip_mreq mreq;
            ...
            if (interfaceIPAddress.isNotEmpty()) mreq.imr_interface.s_addr = inet_addr(interfaceIPAddress.toRawUTF8());

            return setsockopt(handle, IPPROTO_IP, join ? IP_ADD_MEMBERSHIP : IP_DROP_MEMBERSHIP, (const char*)&mreq,
                              sizeof(mreq))
                   == 0;
        }
    }  // namespace SocketHelpers
    ...
    int StreamingSocket::read(void* destBuffer, const int maxBytesToRead, bool shouldBlock)
    {
        return (connected && !isListener)
                   ? SocketHelpers::readSocket(handle, destBuffer, maxBytesToRead, connected, shouldBlock, readLock)
                   : -1;
    }

    int StreamingSocket::write(const void* sourceBuffer, const int numBytesToWrite)
    {
        if (isListener || !connected) return -1;
    ...
    {
        return connected ? SocketHelpers::waitForReadiness(handle, readLock, readyForReading, timeoutMsecs) : -1;
    }
    ...
    bool StreamingSocket::connect(const String& remoteHostName, const int remotePortNumber, const int timeOutMillisecs)
    {
        ...
        connected = SocketHelpers::connectSocket(handle, readLock, remoteHostName, remotePortNumber, timeOutMillisecs);
    ...
    bool StreamingSocket::createListener(const int newPortNumber, const String& localHostName)
    {
        ...
        if (SocketHelpers::bindSocket(handle, portNumber, localHostName) && listen(handle, SOMAXCONN) >= 0)
        {
            connected = true;
            return true;
        }
    ...
    StreamingSocket* StreamingSocket::waitForNextConnection() const
    {
        ...
        if (connected && isListener)
        {
            ...
            if (newSocket >= 0 && connected)
                return new StreamingSocket(inet_ntoa(((struct sockaddr_in*)&address)->sin_addr), portNumber, newSocket);
        }
    ...
    {
        if (lastServerAddress != nullptr) freeaddrinfo(static_cast<struct addrinfo*>(lastServerAddress));
    ...
    //==============================================================================
    int DatagramSocket::waitUntilReady(const bool readyForReading, const int timeoutMsecs) const
    {
        if (handle < 0) return -1;
    ...
    {
        if (handle < 0 || !isBound) return -1;

        bool connected = true;

        SocketHelpers::setSocketBlockingState(handle, shouldBlock);
        return SocketHelpers::readSocket(handle, destBuffer, maxBytesToRead, connected, shouldBlock, readLock);
    }
    ...
    int DatagramSocket::write(const String& remoteHostname, int remotePortNumber, const void* sourceBuffer,
                              int numBytesToWrite)
    {
        jassert(SocketHelpers::isValidPortNumber(remotePortNumber));
        ...
        if (info == nullptr || remoteHostname != lastServerHost || remotePortNumber != lastServerPort)
        {
            ...
            if ((info = SocketHelpers::getAddressInfo(true, remoteHostname, remotePortNumber)) == nullptr) return -1;

            lastServerHost = remoteHostname;
            lastServerPort = remotePortNumber;
        }

        return (int)::sendto(handle, (const char*)sourceBuffer, (juce_recvsend_size_t)numBytesToWrite, 0, info->ai_addr,
                             (socklen_t)info->ai_addrlen);
    }

    bool DatagramSocket::joinMulticast(const String& multicastIPAddress)
    {
        if (!isBound || handle < 0) return false;
