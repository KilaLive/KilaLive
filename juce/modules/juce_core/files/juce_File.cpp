    ...
    String File::parseAbsolutePath(const String& p)
    {
        ...
        if (path.startsWithChar('~'))
        {
            if (path[1] == getSeparatorChar() || path[1] == 0)
            {
                ...
                path = File::getSpecialLocation(File::userHomeDirectory).getFullPathName() + path.substring(1);
            }
            else
            {
...
#define NAMES_ARE_CASE_SENSITIVE 1
    ...
    bool File::deleteRecursively() const
    {
        ...
        if (isDirectory())
            for (auto& f : findChildFiles(File::findFilesAndDirectories, false)) worked = f.deleteRecursively() && worked;
...
#if JUCE_WINDOWS
        if (r.indexOf((juce_wchar)'/') >= 0) return getChildFile(String(r).replaceCharacter('/', '\\'));
#endif
    ...
    int File::getNumberOfChildFiles(const int whatToLookFor, const String& wildCardPattern) const
    {
        ...
        for (DirectoryIterator di(*this, false, wildCardPattern, whatToLookFor); di.next();) ++total;
    ...
    //==============================================================================
    File File::getNonexistentChildFile(const String& suggestedPrefix, const String& suffix, bool putNumbersInBrackets) const
    {
    ...
    File File::getNonexistentSibling(const bool putNumbersInBrackets) const
    {
        ...
        return getParentDirectory().getNonexistentChildFile(getFileNameWithoutExtension(), getFileExtension(),
    ...
    String File::getFileExtension() const
    {
        ...
        if (indexOfDot > fullPath.lastIndexOfChar(getSeparatorChar())) return fullPath.substring(indexOfDot);
    ...
    {
        if (possibleSuffix.isEmpty()) return fullPath.lastIndexOfChar('.') <= fullPath.lastIndexOfChar(getSeparatorChar());
    ...
    FileInputStream* File::createInputStream() const
    {
        ...
        if (fin->openedOk()) return fin.release();
    ...
    FileOutputStream* File::createOutputStream(size_t bufferSize) const
    {
        ...
        return out->failedToOpen() ? nullptr : out.release();
    }
    ...
    bool File::appendText(const String& text, const bool asUnicode, const bool writeUnicodeHeaderBytes) const
    {
    ...
    bool File::replaceWithText(const String& textToWrite, const bool asUnicode, const bool writeUnicodeHeaderBytes) const
    {
        TemporaryFile tempFile(*this, TemporaryFile::useHiddenFile);
    ...
    String File::getRelativePathFrom(const File& dir) const
    {
        ...
        auto dirPath = addTrailingSeparator(dir.existsAsFile() ? dir.getParentDirectory().getFullPathName() : dir.fullPath);
        ...
        if (commonBitLength == 0 || (commonBitLength == 1 && thisPath[1] == getSeparatorChar())) return fullPath;
    ...
    bool File::createSymbolicLink(const File& linkFileToCreate, bool overwriteExisting) const
    {
        ...
        return true;
#elif JUCE_MSVC
        return CreateSymbolicLink(linkFileToCreate.getFullPathName().toWideCharPointer(), fullPath.toWideCharPointer(),
        ...
        void runTest() override
        {
...

            {
                ...
                for (int i = 0; i < roots.size(); ++i)
                    if (roots[i].exists()) ++numRootsExisting;
            ...
            expect(
                std::abs((int)(tempFile.getLastModificationTime().toMilliseconds() - Time::getCurrentTime().toMilliseconds()))
                < 3000);
            ...
            expectEquals(tempFile.getRelativePathFrom(demoFolder.getParentDirectory()),
                         demoFolder.getFileName() + File::getSeparatorString() + tempFile.getFileName());
            expectEquals(demoFolder.getParentDirectory().getRelativePathFrom(tempFile),
                         ".." + File::getSeparatorString() + ".." + File::getSeparatorString()
