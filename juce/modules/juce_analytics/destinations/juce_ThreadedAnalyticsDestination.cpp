...
namespace juce
{
    ...
    void ThreadedAnalyticsDestination::stopAnalyticsThread(int timeout)
    {
        ...
        if (dispatcher.eventQueue.size() > 0) saveUnloggedEvents(dispatcher.eventQueue);
    }
    ...
    void ThreadedAnalyticsDestination::EventDispatcher::run()
    {
        ...
        // front of the queue.
        {
            ...
            for (auto rit = restoredEventQueue.rbegin(); rit != restoredEventQueue.rend(); ++rit) eventQueue.push_front(*rit);
        }
        ...
        while (!threadShouldExit())
        {
            ...
            if (eventsToSendCapacity > 0)
            {
                ...
                if (numEventsInQueue > 0)
                {
                    ...
                    for (size_t i = 0; i < (size_t)numEventsToAdd; ++i) eventsToSend.add(eventQueue[i]);
                }
            }
            ...
            if (!eventsToSend.isEmpty())
            {
                if (parent.logBatchedEvents(eventsToSend))
                {
                    ...
                    for (auto i = 0; i < eventsToSend.size(); ++i) eventQueue.pop_front();
...
//==============================================================================
    ...
    namespace DestinationTestHelpers
    {
        ...
        {
            BasicDestination(std::deque<AnalyticsEvent>& loggedEvents, std::deque<AnalyticsEvent>& unloggedEvents)
            ...
            bool logBatchedEvents(const Array<AnalyticsEvent>& events) override
            {
                ...
                if (loggingIsEnabled)
                {
                    ...
                    for (auto& event : events) loggedEventQueue.push_back(event);
    ...
    struct ThreadedAnalyticsDestinationTests : public UnitTest
    {
        ...
        void runTest() override
        {
...

            {
                ...
                for (auto& event : testEvents) destination.logEvent(event);
