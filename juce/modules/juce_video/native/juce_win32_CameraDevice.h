...
{
    Pimpl(const String&, int index, int minWidth, int minHeight, int maxWidth, int maxHeight, bool /*highQuality*/)
        : isRecording(false)
        ...
        , previewMaxFPS(60)
    {
...

        {
            ...
            hr = captureGraphBuilder->FindInterface(&PIN_CATEGORY_CAPTURE, 0, filter, IID_IAMStreamConfig,
                                                    (void**)streamConfig.resetAndGetPointerAddress());

            if (streamConfig != nullptr)
            {
                getVideoSizes(streamConfig);

                if (!selectVideoSize(streamConfig, minWidth, minHeight, maxWidth, maxHeight)) return;
            }
        }
        ...
        if (!connectFilters(filter, smartTee)) return;
    ...
    ~Pimpl()
    {
        if (mediaControl != nullptr) mediaControl->Stop();
    ...
    void removeListener(CameraDevice::Listener* listenerToRemove)
    {
        ...
        if (listeners.size() == 0) removeUser();
    }
    ...
    void callListeners(const Image& image)
    {
        ...
        for (int i = listeners.size(); --i >= 0;)
            if (CameraDevice::Listener* const l = listeners[i]) l->imageReceived(image);
    }

    void addUser()
    {
        if (openedSuccessfully && activeUsers++ == 0) mediaControl->Run();
    }

    void removeUser()
    {
        if (openedSuccessfully && --activeUsers == 0) mediaControl->Stop();
    }
...

        {
...

            {
                ...
                for (int i = 0; i < height; ++i)
                    memcpy(destData.getLinePointer((height - 1) - i), buffer + lineStride * i, lineStride);
            }

            imageNeedsFlipping = true;
        }

        if (listeners.size() > 0) callListeners(loadingImage);
    ...
    void drawCurrentImage(Graphics& g, Rectangle<int> area)
    {
        ...
        Rectangle<int> centred(
            RectanglePlacement(RectanglePlacement::centred).appliedTo(Rectangle<int>(width, height), area));
        ...
        g.drawImage(activeImage, centred.getX(), centred.getY(), centred.getWidth(), centred.getHeight(), 0, 0, width,
                    height);
    }
            ...
            if (SUCCEEDED(hr))
            {
                ...
                if (SUCCEEDED(hr))
                {
                    ...
                    if (SUCCEEDED(hr))
                    {
                        ...
                        String prof(
                            "<profile version=\"589824\" storageformat=\"1\" name=\"Quality\" description=\"Quality type for "
                            "output.\">"
                        ...
                        hr = profileManager->LoadProfileByData(prof.toWideCharPointer(),
                        ...
                        if (SUCCEEDED(hr))
                        {
                            ...
                            if (getPin(asfWriter, PINDIR_INPUT, asfWriterInputPin, "Video Input 01"))
                            {
                                ...
                                if (SUCCEEDED(hr) && openedSuccessfully && activeUsers > 0 && SUCCEEDED(mediaControl->Run()))
                                {
                                    previewMaxFPS = (quality < 2) ? 15 : 25;  // throttle back the preview comps to try to
                                                                              // leave the cpu free for encoding

                                    if ((quality & 0x00ff0000)
                                        != 0)  // (internal hacky way to pass explicit frame rates for testing)
    ...
    static ComSmartPtr<IBaseFilter> enumerateCameras(StringArray* names, const int deviceIndexToOpen)
    {
        ...
        if (SUCCEEDED(pDevEnum.CoCreateInstance(CLSID_SystemDeviceEnum)))
        {
            ...
            HRESULT                   hr =
                pDevEnum->CreateClassEnumerator(CLSID_VideoInputDeviceCategory, enumerator.resetAndGetPointerAddress(), 0);
                ...
                while (enumerator->Next(1, moniker.resetAndGetPointerAddress(), &fetched) == S_OK)
                {
                    ...
                    if (SUCCEEDED(hr))
                    {
                        ...
                        if (SUCCEEDED(hr))
                        {
                            ...
                            if (SUCCEEDED(hr))
                            {
                                if (names != nullptr) names->add(var.bstrVal);

                                if (index == deviceIndexToOpen) return captureFilter;
    ...
    struct GrabberCallback : public ComBaseClassHelperBase<ISampleGrabberCB>
    {
        GrabberCallback(Pimpl& p)
            : ComBaseClassHelperBase<ISampleGrabberCB>(0)
            , owner(p)
        {
        }
        ...
        {
            if (refId == IID_ISampleGrabberCB) return castToType<ISampleGrabberCB>(result);
...
private:
    ...
    bool selectVideoSize(IAMStreamConfig* const streamConfig, const int minWidth, const int minHeight, const int maxWidth,
                         const int maxHeight)
    {
        ...
        if (size == sizeof(VIDEO_STREAM_CONFIG_CAPS))
        {
            ...
            for (int i = 0; i < count; ++i)
            {
                ...
                if (SUCCEEDED(hr))
                {
                    if (scc.InputSize.cx >= minWidth && scc.InputSize.cy >= minHeight && scc.InputSize.cx <= maxWidth
    ...
    static bool getPin(IBaseFilter* filter, const PIN_DIRECTION wantedDirection, ComSmartPtr<IPin>& result,
    ...
    bool connectFilters(IBaseFilter* const first, IBaseFilter* const second) const
    {
        ...
        return getPin(first, PINDIR_OUTPUT, out) && getPin(second, PINDIR_INPUT, in)
    ...
    bool addGraphToRot()
    {
        ...
        if (FAILED(GetRunningObjectTable(0, rot.resetAndGetPointerAddress()))) return false;
    ...
    void removeGraphFromRot()
    {
        ...
        if (SUCCEEDED(GetRunningObjectTable(0, rot.resetAndGetPointerAddress()))) rot->Revoke(graphRegistrationID);
    }
...
//==============================================================================
struct CameraDevice::ViewerComponent : public Component, public ChangeListener
{
    ...
    void changeListenerCallback(ChangeBroadcaster*) override
    {
        ...
        if (now >= lastRepaintTime + (1000 / maxFPS))
        {
            ...
            if (owner != nullptr) maxFPS = owner->previewMaxFPS;
        }
    }
...
{
    for (int i = viewerComps.size(); --i >= 0;) viewerComps.getUnchecked(i)->ownerDeleted();
}
