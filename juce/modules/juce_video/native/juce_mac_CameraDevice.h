...
struct CameraDevice::Pimpl
{
          ...
          bool useHighQuality)
    {
        JUCE_AUTORELEASEPOOL
        {
            ...
            session.sessionPreset = useHighQuality ? AVCaptureSessionPresetHigh : AVCaptureSessionPresetMedium;
    ...
    {
        if (getVideoConnection() == nullptr) refreshConnections();
    }
    ...
    void startRecordingToFile(const File& file, int /*quality*/)
    {
        ...
        [fileOutput startRecordingToOutputFileURL:createNSURLFromFile(file) recordingDelegate:callbackDelegate];
    }
            ...
            for (AVCaptureConnection* connection in imageOutput.connections)
                if ([connection isActive] && [connection isEnabled])
                    for (AVCaptureInputPort* port in [connection inputPorts])
                        if ([[port mediaType] isEqual:AVMediaTypeVideo]) return connection;
    ...
    void handleImageCapture(const void* data, size_t size)
    {
        ...
        if (!listeners.isEmpty())
        {
            for (int i = listeners.size(); --i >= 0;)
                if (auto* l = listeners[i]) l->imageReceived(image);

            if (!listeners.isEmpty()) triggerImageCapture();
        }
    }
            ...
            [imageOutput
                captureStillImageAsynchronouslyFromConnection:videoConnection
                                            completionHandler:^(CMSampleBufferRef sampleBuffer, NSError*) {
                                              ...
                                              jassert(CMBlockBufferIsRangeContiguous(
                                                  buffer, 0, size));  // TODO: need to add code to handle this if it happens
                                              char* data = nullptr;
    ...
    void addListener(CameraDevice::Listener* listenerToAdd)
    {
        ...
        if (listeners.size() == 1) triggerImageCapture();
    }
...
private:
    ...
    {
        DelegateClass()
            : ObjCClass<NSObject>("JUCECameraDelegate_")
        {
            addIvar<Pimpl*>("owner");
            ...
            addMethod(@selector(captureOutput:didStartRecordingToOutputFileAtURL:fromConnections:),
                      didStartRecordingToOutputFileAtURL, "v@:@@@");
            addMethod(@selector(captureOutput:didPauseRecordingToOutputFileAtURL:fromConnections:),
                      didPauseRecordingToOutputFileAtURL, "v@:@@@");
            addMethod(@selector(captureOutput:didResumeRecordingToOutputFileAtURL:fromConnections:),
                      didResumeRecordingToOutputFileAtURL, "v@:@@@");
            addMethod(@selector(captureOutput:willFinishRecordingToOutputFileAtURL:fromConnections:error:),
                      willFinishRecordingToOutputFileAtURL, "v@:@@@@");
