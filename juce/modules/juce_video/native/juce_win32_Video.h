...
struct VideoComponent::Pimpl : public Component
{
    ...
    void play()
    {
        if (videoLoaded) context->play();
    }

    void stop()
    {
        if (videoLoaded) context->pause();
    }

    void setPosition(double newPosition)
    {
        if (videoLoaded) context->setPosition(newPosition);
    }
    ...
    {
        if (videoLoaded) context->setSpeed(newSpeed);
    }

    Rectangle<int> getNativeSize() const
    {
        return videoLoaded ? context->getVideoSize() : Rectangle<int>();
    }
    ...
    {
        if (videoLoaded) context->setVolume(newVolume);
    }
    ...
    void updateContextPosition()
    {
        ...
        if (getWidth() > 0 && getHeight() > 0)
            if (auto* peer = getTopLevelComponent()->getPeer()) context->updateWindowPosition(peer->getAreaCoveredBy(*this));
    }
...
private:
    ...
    struct ComponentWatcher : public ComponentMovementWatcher
    {
        ...
        {
            if (owner.videoLoaded) owner.updateContextPosition();
        }

        void componentPeerChanged() override
        {
            if (owner.videoLoaded) owner.recreateNativeWindowAsync();
        }

        void componentVisibilityChanged() override
        {
            if (owner.videoLoaded) owner.updateContextVisibility();
        }
    ...
    struct DirectShowContext : public AsyncUpdater
    {
        ...
        void repaint()
        {
            if (hasVideo) videoRenderer->repaintVideo(nativeWindow->hwnd, nativeWindow->hdc);
        }

        void updateVideoPosition()
        {
            if (hasVideo) videoRenderer->setVideoPosition(nativeWindow->hwnd);
        }

        void displayResolutionChanged()
        {
            if (hasVideo) videoRenderer->displayModeChanged();
        }
        ...
        void peerChanged()
        {
            ...
            if (videoRenderer != nullptr) videoRenderer->setVideoWindow(nullptr);

            createNativeWindow();

            mediaEvent->SetNotifyWindow((OAHWND)hwnd, graphEventID, 0);
            if (videoRenderer != nullptr) videoRenderer->setVideoWindow(hwnd);
        }
        ...
        Result loadFile(const String& fileOrURLPath)
        {
            ...
            if (!createNativeWindow()) return Result::fail("Can't create window");
            ...
            if (SUCCEEDED(hr))
            {
                if (SystemStats::getOperatingSystemType() >= SystemStats::WinVista)
                {
                    ...
                    if (FAILED(hr)) videoRenderer = nullptr;
                }
            ...
            if (SUCCEEDED(hr)) hr = mediaEvent->SetNotifyWindow((OAHWND)hwnd, graphEventID, 0);
        ...
        static Result getErrorMessageFromResult(HRESULT hr)
        {
            ...
            case VFW_E_UNSUPPORTED_STREAM:
                return Result::fail("Unsupported stream");
            case VFW_E_CANNOT_CONNECT:
                return Result::fail("Cannot connect");
            case VFW_E_CANNOT_LOAD_SOURCE_FILTER:
                return Result::fail("Cannot load source filter");
            }
        ...
        void release()
        {
            ...
            if (mediaEvent != nullptr) mediaEvent->SetNotifyWindow(0, 0, 0);

            if (videoRenderer != nullptr) videoRenderer->setVideoWindow(0);
            ...
            if (nativeWindow != nullptr) deleteNativeWindow();
        }
        ...
        Rectangle<int> getVideoSize() const noexcept
        {
            ...
            if (hasVideo) videoRenderer->getVideoSize(width, height);
    ...
    private:
        ...
        bool isRendererConnected()
        {
            ...
            if (SUCCEEDED(hr)) hr = enumPins->Reset();

            ComSmartPtr<IPin> pin;

            while (SUCCEEDED(hr) && enumPins->Next(1, pin.resetAndGetPointerAddress(), nullptr) == S_OK)
            {
                ...
                if (SUCCEEDED(hr))
                {
                    ...
                    if (SUCCEEDED(hr) && direction == PINDIR_INPUT) return true;
                }
        ...
        struct NativeWindowClass : private DeletedAtShutdown
        {
            ...
            {
                return (LPCTSTR)(pointer_sized_uint)MAKELONG(atom, 0);
            }
        ...
        private:
            ...
            ~NativeWindowClass()
            {
                if (atom != 0) UnregisterClass(getWindowClassName(), (HINSTANCE)Process::getCurrentModuleInstanceHandle());
        ...
        struct NativeWindow
        {
            NativeWindow(HWND parentToAddTo, void* userData)
            {
                ...
                if (wc->isRegistered())
                {
                    ...
                    hwnd = CreateWindowEx(exstyle, wc->getWindowClassName(), L"", type, 0, 0, 0, 0, parentToAddTo, 0,
            ...
            {
                SetWindowPos(hwnd, 0, newBounds.getX(), newBounds.getY(), newBounds.getWidth(), newBounds.getHeight(),
