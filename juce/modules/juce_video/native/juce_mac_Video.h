...
struct VideoComponent::Pimpl : public BaseClass
{
    ...
    void setPosition(double newPosition)
    {
        ...
        {
            CMTime t = {(CMTimeValue)(100000.0 * newPosition), (CMTimeScale)100000, kCMTimeFlags_Valid};

            [p seekToTime:t toleranceBefore:kCMTimeZero toleranceAfter:kCMTimeZero];
        }
    }

    double getPosition() const
    {
        if (auto* p = getAVPlayer()) return toSeconds([p currentTime]);
    ...
    {
        if (auto* p = getAVPlayer()) return [p rate];
    ...
    {
        if (auto* player = getAVPlayer()) return toSeconds([[player currentItem] duration]);
    ...
    {
        if (auto* p = getAVPlayer()) return [p volume];
...
private:
    ...
    AVPlayer* getAVPlayer() const noexcept
    {
