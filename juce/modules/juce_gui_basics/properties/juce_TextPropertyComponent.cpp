...
{
    //==============================================================================
    class TextPropertyComponent::LabelComp : public Label, public FileDragAndDropTarget
    {
    public:
        ...
        void paintOverChildren(Graphics& g) override
        {
            if (getText().isEmpty() && !isBeingEdited())
            {
                ...
                g.drawFittedText(textToDisplayWhenEmpty, textArea, getJustificationType(),
                                 jmax(1, (int)(textArea.getHeight() / labelFont.getHeight())), getMinimumHorizontalScale());
            }
        }
    ...
    //==============================================================================
    TextPropertyComponent::TextPropertyComponent(const String& name, int maxNumChars, bool isMultiLine, bool isEditable)
        : PropertyComponent(name)
    {
        createEditor(maxNumChars, isMultiLine, isEditable);
    }

    TextPropertyComponent::TextPropertyComponent(const Value& valueToControl, const String& name, int maxNumChars,
                                                 bool isMultiLine, bool isEditable)
        : TextPropertyComponent(name, maxNumChars, isMultiLine, isEditable)
    {
        textEditor->getTextValue().referTo(valueToControl);
    }

    TextPropertyComponent::TextPropertyComponent(const ValueWithDefault& valueToControl, const String& name, int maxNumChars,
                                                 bool isMultiLine, bool isEditable)
        : TextPropertyComponent(name, maxNumChars, isMultiLine, isEditable)
    {
    ...
    void TextPropertyComponent::textWasEdited()
    {
        ...
        if (getText() != newText) setText(newText);

        callListeners();
    }

    void TextPropertyComponent::addListener(TextPropertyComponent::Listener* l)
    {
        listenerList.add(l);
    }
    void TextPropertyComponent::removeListener(TextPropertyComponent::Listener* l)
    {
    ...
    {
        if (textEditor != nullptr) textEditor->setInterestedInFileDrag(isInterested);
    }

    void TextPropertyComponent::setEditable(bool isEditable)
    {
        if (textEditor != nullptr) textEditor->setEditable(isEditable, isEditable);
    }
