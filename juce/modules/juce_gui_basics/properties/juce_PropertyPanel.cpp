...
namespace juce
{
    ...
    {
        SectionComponent(const String& sectionTitle, const Array<PropertyComponent*>& newProperties, bool sectionIsOpen)
        ...
        int getPreferredHeight() const
        {
            ...
            if (isOpen)
                for (auto* propertyComponent : propertyComps) y += propertyComponent->getPreferredHeight();
        ...
        void setOpen(bool open)
        {
            if (isOpen != open)
            {
                ...
                for (auto* propertyComponent : propertyComps) propertyComponent->setVisible(open);

                if (auto* propertyPanel = findParentComponentOfClass<PropertyPanel>()) propertyPanel->resized();
            }
        }

        void refreshAll() const
        {
            for (auto* propertyComponent : propertyComps) propertyComponent->refresh();
        }

        void mouseUp(const MouseEvent& e) override
        {
            if (e.getMouseDownX() < titleHeight && e.x < titleHeight && e.getNumberOfClicks() != 2) mouseDoubleClick(e);
        }

        void mouseDoubleClick(const MouseEvent& e) override
        {
            if (e.y < titleHeight) setOpen(!isOpen);
        }
    ...
    struct PropertyPanel::PropertyHolderComponent : public Component
    {
        ...
        void refreshAll() const
        {
            for (auto* section : sections) section->refreshAll();
        }
    ...
    PropertyPanel::PropertyPanel(const String& name)
        : Component(name)
    {
        init();
    }
    ...
    void PropertyPanel::paint(Graphics& g)
    {
        if (isEmpty())
        {
            ...
            g.setFont(14.0f);
            g.drawText(messageWhenEmpty, getLocalBounds().withHeight(30), Justification::centred, true);
        }
    }
    ...
    void PropertyPanel::addSection(const String& sectionTitle, const Array<PropertyComponent*>& newProperties,
                                   bool shouldBeOpen, int indexToInsertAt)
    {
        jassert(sectionTitle.isNotEmpty());

        if (isEmpty()) repaint();

        propertyHolderComponent->insertSection(indexToInsertAt,
                                               new SectionComponent(sectionTitle, newProperties, shouldBeOpen));
        updatePropHolderLayout();
    }
    ...
    StringArray PropertyPanel::getSectionNames() const
    {
        ...
        {
            if (section->getName().isNotEmpty()) s.add(section->getName());
        }
    ...
    {
        if (auto* s = propertyHolderComponent->getSectionWithNonEmptyName(sectionIndex)) return s->isOpen;
    ...
    {
        if (auto* s = propertyHolderComponent->getSectionWithNonEmptyName(sectionIndex)) s->setOpen(shouldBeOpen);
    }

    void PropertyPanel::setSectionEnabled(int sectionIndex, bool shouldBeEnabled)
    {
        if (auto* s = propertyHolderComponent->getSectionWithNonEmptyName(sectionIndex)) s->setEnabled(shouldBeEnabled);
    }
    ...
    void PropertyPanel::restoreOpennessState(const XmlElement& xml)
    {
        if (xml.hasTagName("PROPERTYPANELSTATE"))
        {
            ...
            {
                setSectionOpen(sections.indexOf(e->getStringAttribute("name")), e->getBoolAttribute("open"));
            }
