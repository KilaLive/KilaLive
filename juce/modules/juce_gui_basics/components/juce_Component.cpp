...
namespace juce
{
                ...
                if (auto* list = p->mouseListeners.get())
                {
                    if (list->numDeepMouseListeners > 0)
                    {
                        ...
                        for (int i = list->numDeepMouseListeners; --i >= 0;)
                        {
                            ...
                            if (checker2.shouldBailOut()) return;
    ...
    struct FocusRestorer
    {
        ...
        ~FocusRestorer()
        {
            if (lastFocus != nullptr && lastFocus->isShowing() && !lastFocus->isCurrentlyBlockedByAnotherModalComponent())
    ...
    struct ScalingHelpers
    {
        ...
        {
            return scale != 1.0f ? Rectangle<int>(roundToInt(pos.getX() / scale), roundToInt(pos.getY() / scale),
                                                  roundToInt(pos.getWidth() / scale), roundToInt(pos.getHeight() / scale))
                                 : pos;
        }
    ...
    struct Component::ComponentHelpers
    {
        ...
        static Identifier getColourPropertyID(int colourID)
        {
            ...
            *t        = 0;
            ...
            for (int i = (int)sizeof(colourPropertyPrefix) - 1; --i >= 0;) *--t = colourPropertyPrefix[i];
        ...
        {
            return isPositiveAndBelow(localPoint.x, comp.getWidth()) && isPositiveAndBelow(localPoint.y, comp.getHeight())
        ...
        {
            if (comp.isTransformed()) pos = pos.transformedBy(comp.getTransform().inverted());
        ...
        {
            if (comp.isTransformed()) pos = pos.transformedBy(comp.getTransform());
        ...
        static PointOrRect convertToParentSpace(const Component& comp, PointOrRect pointInLocalSpace)
        {
            ...
            if (comp.affineTransform != nullptr) pointInLocalSpace = pointInLocalSpace.transformedBy(*comp.affineTransform);

            return pointInLocalSpace;
        }

        template <typename PointOrRect>
        static PointOrRect convertFromDistantParentSpace(const Component* parent, const Component& target,
                                                         const PointOrRect& coordInParent)
        {
            auto* directParent = target.getParentComponent();
            jassert(directParent != nullptr);

            if (directParent == parent) return convertFromParentSpace(target, coordInParent);
        ...
        static PointOrRect convertCoordinate(const Component* target, const Component* source, PointOrRect p)
        {
            while (source != nullptr)
            {
                ...
                if (source->isParentOf(target)) return convertFromDistantParentSpace(source, *target, p);
            ...
            if (topLevelComp == target) return p;
                ...
                if (child.isVisible() && !child.isTransformed())
                {
                    ...
                    if (!newClip.isEmpty())
                    {
                        ...
                        else
                        {
                            ...
                            if (clipObscuredRegions(child, g, newClip - childPos, childPos + delta)) wasClipped = true;
                        }
                    }
                }
            }
        ...
        {
            if (auto* p = comp.getParentComponent()) return p->getLocalBounds();
        ...
        {
            if (auto* cached = c.getCachedComponentImage()) cached->releaseResources();

            for (auto* child : c.childComponentList) releaseAllCachedImageResources(*child);
        }
    };
    ...
    Component::~Component()
    {
        ...
        while (childComponentList.size() > 0) removeChildComponent(childComponentList.size() - 1, false, true);
    ...
    void Component::addToDesktop(int styleWanted, void* nativeWindowToAttachTo)
    {
        ...
        if (peer == nullptr || styleWanted != peer->getStyleFlags())
        {
            ...
            if (peer != nullptr)
            {
                ...
                internalHierarchyChanged();  // give comps a chance to react to the peer change before the old peer is
                                             // deleted.
            ...
            if (safePointer != nullptr)
            {
                ...
                if (oldRenderingEngine >= 0) peer->setCurrentRenderingEngine(oldRenderingEngine);
    ...
    struct StandardCachedComponentImage : public CachedComponentImage
    {
        ...
        void paint(Graphics& g) override
        {
            ...
            {
                image = Image(owner.isOpaque() ? Image::RGB : Image::ARGB, jmax(1, imageBounds.getWidth()),
            ...
            g.drawImageTransformed(image,
                                   AffineTransform::scale(compBounds.getWidth() / (float)imageBounds.getWidth(),
                                                          compBounds.getHeight() / (float)imageBounds.getHeight()),
                                   false);
        }
    ...
    void Component::setBufferedToImage(bool shouldBeBuffered)
    {
        ...
        if (shouldBeBuffered)
        {
            if (cachedImage == nullptr) cachedImage.reset(new StandardCachedComponentImage(*this));
        }
        else
        {
            cachedImage.reset();
    ...
    void Component::toFront(bool setAsForeground)
    {
        ...
        if (flags.hasHeavyweightPeerFlag)
        {
            if (auto* peer = getPeer())
            {
                ...
                if (setAsForeground && !hasKeyboardFocus(true)) grabKeyboardFocus();
            }
        }
            ...
            if (childList.getLast() != this)
            {
                ...
                if (index >= 0)
                {
                    ...
                    if (!flags.alwaysOnTopFlag)
                    {
                        ...
                        while (insertIndex > 0 && childList.getUnchecked(insertIndex)->isAlwaysOnTop()) --insertIndex;
                    }
    ...
    {
        return roundToInt(proportion * boundsRelativeToParent.getWidth());
    }
    int Component::proportionOfHeight(float proportion) const noexcept
    {
        return roundToInt(proportion * boundsRelativeToParent.getHeight());
    }

    int Component::getParentWidth() const noexcept
    {
        return parentComponent != nullptr ? parentComponent->getWidth() : getParentMonitorArea().getWidth();
    }

    int Component::getParentHeight() const noexcept
    {
        return parentComponent != nullptr ? parentComponent->getHeight() : getParentMonitorArea().getHeight();
    }
    ...
    void Component::sendMovedResizedMessages(bool wasMoved, bool wasResized)
    {
        ...
        if (!checker.shouldBailOut())
            componentListeners.callChecked(
                checker, [=](ComponentListener& l) { l.componentMovedOrResized(*this, wasMoved, wasResized); });
    }
    ...
    void Component::setCentrePosition(int x, int y)
    {
    ...
    {
        setCentrePosition(roundToInt(getParentWidth() * x), roundToInt(getParentHeight() * y));
    }

    void Component::setBoundsRelative(Rectangle<float> target)
    {
        setBounds((target * Point<float>((float)getParentWidth(), (float)getParentHeight())).toNearestInt());
    }
    ...
    {
        auto parentArea = ComponentHelpers::getParentOrMainMonitorBounds(*this).transformedBy(getTransform().inverted());

        setBounds(parentArea.getCentreX() - width / 2, parentArea.getCentreY() - height / 2, width, height);
    }
    ...
    void Component::setBoundsToFit(Rectangle<int> targetArea, Justification justification, bool onlyReduceInSize)
    {
        ...
        if (onlyReduceInSize && getWidth() <= targetArea.getWidth() && getHeight() <= targetArea.getHeight())
        {
        ...
        else
        {
            ...
            if (sourceRatio <= targetRatio)
                sourceArea.setHeight(jmin(targetArea.getHeight(), roundToInt(targetArea.getWidth() * sourceRatio)));
            else
                sourceArea.setWidth(jmin(targetArea.getWidth(), roundToInt(targetArea.getHeight() / sourceRatio)));
        }

        if (!sourceArea.isEmpty()) setBounds(justification.appliedToRectangle(sourceArea, targetArea));
    }
    ...
    void Component::setInterceptsMouseClicks(bool allowClicks, bool allowClicksOnChildComponents) noexcept
    {
        flags.ignoresMouseClicksFlag    = !allowClicks;
        flags.allowChildMouseClicksFlag = allowClicksOnChildComponents;
    }

    void Component::getInterceptsMouseClicks(bool& allowsClicksOnThisComponent, bool& allowsClicksOnChildComponents) const
        noexcept
    {
    ...
    void Component::addChildComponent(Component& child, int zOrder)
    {
        ...
        if (child.parentComponent != this)
        {
            ...
            if (!child.isAlwaysOnTop())
            {
                ...
                while (zOrder > 0)
                {
                    if (!childComponentList.getUnchecked(zOrder - 1)->isAlwaysOnTop()) break;
        ...
        if (child != nullptr)
        {
            ...
            if (currentlyFocusedComponent == child || child->isParentOf(currentlyFocusedComponent))
            {
                if (sendParentEvents)
                {
                    ...
                    if (thisPointer == nullptr) return child;
    ...
    {
        while (!childComponentList.isEmpty()) removeChildComponent(childComponentList.size() - 1);
    }

    void Component::deleteAllChildren()
    {
        while (!childComponentList.isEmpty()) delete (removeChildComponent(childComponentList.size() - 1));
    }
...
//==============================================================================
    ...
    int Component::runModalLoop()
    {
        if (!MessageManager::getInstance()->isThisTheMessageThread())
        {
            ...
            return (int)(pointer_sized_int)MessageManager::getInstance()->callFunctionOnMessageThread(
                &ComponentHelpers::runModalLoopCallback, this);
        }
    ...
    //==============================================================================
    void Component::enterModalState(bool shouldTakeKeyboardFocus, ModalComponentManager::Callback* callback,
    ...
    void Component::exitModalState(int returnValue)
    {
        if (isCurrentlyModal(false))
        {
            if (MessageManager::getInstance()->isThisTheMessageThread())
            {
                ...
                // If any of the mouse sources are over another Component when we exit the modal state then send a mouse enter
                // event
            ...
            else
            {
                ...
                MessageManager::callAsync([=] {
                    if (auto* c = target.get()) c->exitModalState(returnValue);
                });
            }
        }
    }
    ...
    bool Component::isCurrentlyModal(bool onlyConsiderForemostModalComponent) const noexcept
    {
        ...
        return onlyConsiderForemostModalComponent ? mcm.isFrontModalComponent(this) : mcm.isModal(this);
    }
    ...
    bool Component::isCurrentlyBlockedByAnotherModalComponent() const
    {
        ...
        return !(mc == nullptr || mc == this || mc->isParentOf(this) || mc->canModalEventBeSentToComponent(this));
    }
    ...
    void Component::internalRepaintUnchecked(Rectangle<int> area, bool isEntireComponent)
    {
        ...
        if (flags.visibleFlag)
        {
            if (cachedImage != nullptr)
                if (!(isEntireComponent ? cachedImage->invalidateAll() : cachedImage->invalidate(area))) return;
            ...
            if (flags.hasHeavyweightPeerFlag)
            {
                if (auto* peer = getPeer())
                {
                    ...
                    auto scaled     = area
                                  * Point<float>(peerBounds.getWidth() / (float)getWidth(),
    ...
    void Component::paintComponentAndChildren(Graphics& g)
    {
        ...
        else
        {
            ...
            if (!(ComponentHelpers::clipObscuredRegions(*this, g, clipBounds, {}) && g.isClipEmpty())) paint(g);
            ...
            if (child.isVisible())
            {
                ...
                else if (clipBounds.intersects(child.getBounds()))
                {
                    ...
                    else if (g.reduceClipRegion(child.getBounds()))
                    {
                        ...
                        if (nothingClipped || !g.isClipEmpty()) child.paintWithinParentContext(g);
                    }
    ...
    void Component::paintEntireComponent(Graphics& g, bool ignoreAlphaLevel)
    {
        ...
        if (effect != nullptr)
        {
            ...
            Image effectImage(flags.opaqueFlag ? Image::RGB : Image::ARGB, scaledBounds.getWidth(), scaledBounds.getHeight(),
                              !flags.opaqueFlag);
            {
    ...
    //==============================================================================
    Image Component::createComponentSnapshot(Rectangle<int> areaToGrab, bool clipImageToComponentBounds, float scaleFactor)
    {
        auto r = areaToGrab;
        ...
        if (w != getWidth() || h != getHeight())
            g.addTransform(AffineTransform::scale(w / (float)r.getWidth(), h / (float)r.getHeight()));
    ...
    LookAndFeel& Component::getLookAndFeel() const noexcept
    {
        for (auto* c = this; c != nullptr; c = c->parentComponent)
            if (c->lookAndFeel != nullptr) return *(c->lookAndFeel);
    ...
    {
        if (properties.remove(ComponentHelpers::getColourPropertyID(colourID))) colourChanged();
    }

    void Component::setColour(int colourID, Colour colour)
    {
        if (properties.set(ComponentHelpers::getColourPropertyID(colourID), (int)colour.getARGB())) colourChanged();
    }
    ...
    {
        return affineTransform == nullptr ? boundsRelativeToParent : boundsRelativeToParent.transformedBy(*affineTransform);
    }
    ...
    void Component::mouseWheelMove(const MouseEvent& e, const MouseWheelDetails& wheel)
    {
        ...
        if (parentComponent != nullptr) parentComponent->mouseWheelMove(e.getEventRelativeTo(parentComponent), wheel);
    }

    void Component::mouseMagnify(const MouseEvent& e, float magnifyAmount)
    {
        // the base class just passes this event up to its parent..
        if (parentComponent != nullptr) parentComponent->mouseMagnify(e.getEventRelativeTo(parentComponent), magnifyAmount);
    }
...
// thread, you'll need to use a MessageManagerLock object to make sure it's thread-safe.
        ...
        if (getParentComponent() != nullptr) ASSERT_MESSAGE_MANAGER_IS_LOCKED;
#endif
    ...
    {
        if (auto* current = getCurrentlyModalComponent()) current->inputAttemptWhenModal();
    }
    ...
    void Component::postCommandMessage(int commandID)
    {
        ...
        MessageManager::callAsync([=] {
            if (auto* c = target.get()) c->handleCommandMessage(commandID);
        });
    }
    ...
    //==============================================================================
    void Component::addMouseListener(MouseListener* newListener, bool wantsEventsForAllNestedChildComponents)
    {
        // if component methods are being called from threads other than the message
        // thread, you'll need to use a MessageManagerLock object to make sure it's thread-safe.
        ASSERT_MESSAGE_MANAGER_IS_LOCKED
    ...
    void Component::removeMouseListener(MouseListener* listenerToRemove)
    {
        ...
        if (mouseListeners != nullptr) mouseListeners->removeListener(listenerToRemove);
    }
    ...
    void Component::internalMouseEnter(MouseInputSource source, Point<float> relativePos, Time time)
    {
        ...
        const MouseEvent me(source, relativePos, source.getCurrentModifiers(), MouseInputSource::invalidPressure,
                            ...
                            MouseInputSource::invalidTiltX, MouseInputSource::invalidTiltY, this, this, time, relativePos,
                            time, 0, false);
        mouseEnter(me);
    ...
    void Component::internalMouseDown(MouseInputSource source, Point<float> relativePos, Time time, float pressure,
                                      float orientation, float rotation, float tiltX, float tiltY)
    {
        ...
        if (isCurrentlyBlockedByAnotherModalComponent())
        {
            ...
            if (isCurrentlyBlockedByAnotherModalComponent())
            {
                ...
                const MouseEvent me(source, relativePos, source.getCurrentModifiers(), pressure, orientation, rotation, tiltX,
                                    tiltY, this, this, time, relativePos, time, source.getNumberOfMultipleClicks(), false);
        ...
        const MouseEvent me(source, relativePos, source.getCurrentModifiers(), pressure, orientation, rotation, tiltX, tiltY,
                            this, this, time, relativePos, time, source.getNumberOfMultipleClicks(), false);
        mouseDown(me);
    ...
    void Component::internalMouseUp(MouseInputSource source, Point<float> relativePos, Time time,
                                    const ModifierKeys oldModifiers, float pressure, float orientation, float rotation,
                                    float tiltX, float tiltY)
    {
        if (flags.mouseDownWasBlocked && isCurrentlyBlockedByAnotherModalComponent()) return;

        BailOutChecker checker(this);

        if (flags.repaintOnMouseActivityFlag) repaint();

        const MouseEvent me(source, relativePos, oldModifiers, pressure, orientation, rotation, tiltX, tiltY, this, this,
                            time, getLocalPoint(nullptr, source.getLastMouseDownPosition()), source.getLastMouseDownTime(),
                            source.getNumberOfMultipleClicks(), source.hasMouseMovedSignificantlySincePressed());
        mouseUp(me);
                                      ...
                                      float orientation, float rotation, float tiltX, float tiltY)
    {
        if (!isCurrentlyBlockedByAnotherModalComponent())
        {
            ...
            const MouseEvent me(source, relativePos, source.getCurrentModifiers(), pressure, orientation, rotation, tiltX,
                                tiltY, this, this, time, getLocalPoint(nullptr, source.getLastMouseDownPosition()),
    ...
    void Component::internalMouseWheel(MouseInputSource source, Point<float> relativePos, Time time,
                                       const MouseWheelDetails& wheel)
    {
        auto&          desktop = Desktop::getInstance();
        BailOutChecker checker(this);
        ...
        else
        {
            ...
            if (!checker.shouldBailOut()) MouseListenerList::sendWheelEvent(*this, checker, me, wheel);
        }
    }

    void Component::internalMagnifyGesture(MouseInputSource source, Point<float> relativePos, Time time, float amount)
    {
        if (!isCurrentlyBlockedByAnotherModalComponent())
        {
            const MouseEvent me(source, relativePos, source.getCurrentModifiers(), MouseInputSource::invalidPressure,
    ...
    {
        if (flags.hasHeavyweightPeerFlag) Desktop::getInstance().componentBroughtToFront(this);
    ...
    void Component::internalFocusGain(FocusChangeType cause, const WeakReference<Component>& safePointer)
    {
        ...
        if (safePointer != nullptr) internalChildFocusChange(cause, safePointer);
    }
    ...
    {
        if (flags.isFocusContainerFlag || parentComponent == nullptr) return new KeyboardFocusTraverser();
        ...
        if (currentlyFocusedComponent != this)
        {
            ...
            if (auto* peer = getPeer())
            {
                ...
                if (peer->isFocused() && currentlyFocusedComponent != this)
                {
                    ...
                    if (componentLosingFocus != nullptr) componentLosingFocus->internalFocusLoss(cause);

                    if (currentlyFocusedComponent == this) internalFocusGain(cause, safePointer);
                }
            }
        }
    }
    ...
    void Component::moveKeyboardFocusToSibling(bool moveToNext)
    {
        ...
        if (parentComponent != nullptr)
        {
            ...
            if (traverser != nullptr)
            {
                auto* nextComp = moveToNext ? traverser->getNextComponent(this) : traverser->getPreviousComponent(this);
                traverser.reset();
                ...
                if (nextComp != nullptr)
                {
                    if (nextComp->isCurrentlyBlockedByAnotherModalComponent())
                    {
                        ...
                        if (nextCompPointer == nullptr || nextComp->isCurrentlyBlockedByAnotherModalComponent()) return;
                    }
    ...
    {
        return (currentlyFocusedComponent == this) || (trueIfChildIsFocused && isParentOf(currentlyFocusedComponent));
    }
    ...
    {
        if (auto* c = getCurrentlyFocusedComponent()) c->giveAwayFocus(true);
    }
    ...
    {
        return (!flags.isDisabledFlag) && (parentComponent == nullptr || parentComponent->isEnabled());
    }
    ...
    void Component::setEnabled(bool shouldBeEnabled)
    {
        if (flags.isDisabledFlag == shouldBeEnabled)
        {
            ...
            if (parentComponent == nullptr || parentComponent->isEnabled()) sendEnablementChangeMessage();
    ...
    {
        for (auto& ms : Desktop::getInstance().getMouseSources())
            if (ms.isDragging() && ms.getComponentUnderMouse() == this) return true;
    ...
    bool Component::isMouseOverOrDragging(bool includeChildren) const
    {
        for (auto& ms : Desktop::getInstance().getMouseSources())
        {
            ...
            if ((c == this || (includeChildren && isParentOf(c))) && ((!ms.isTouch()) || ms.isDragging())) return true;
        }
    ...
    {
        if (keyListeners != nullptr) keyListeners->removeFirstMatchingValue(listenerToRemove);
    }
    ...
    {
        if (parentComponent != nullptr) parentComponent->modifierKeysChanged(modifiers);
    }
