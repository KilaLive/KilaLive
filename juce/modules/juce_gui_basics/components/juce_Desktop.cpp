...
namespace juce
{
    ...
    {
        if (instance == nullptr) instance = new Desktop();
    ...
    Component* Desktop::findComponentAt(Point<int> screenPosition) const
    {
        ...
        for (int i = desktopComponents.size(); --i >= 0;)
        {
            ...
            if (c->isVisible())
            {
                ...
                if (c->contains(relative)) return c->getComponentAt(relative);
            }
        }
    ...
    LookAndFeel& Desktop::getDefaultLookAndFeel() noexcept
    {
        ...
        {
            if (defaultLookAndFeel == nullptr) defaultLookAndFeel.reset(new LookAndFeel_V4());
    ...
    void Desktop::setDefaultLookAndFeel(LookAndFeel* newDefaultLookAndFeel)
    {
        ...
        for (int i = getNumComponents(); --i >= 0;)
            if (auto* c = getComponent(i)) c->sendLookAndFeelChange();
    }
    ...
    void Desktop::componentBroughtToFront(Component* c)
    {
        ...
        if (index >= 0)
        {
            ...
            if (!c->isAlwaysOnTop())
            {
                ...
                while (newIndex > 0 && desktopComponents.getUnchecked(newIndex - 1)->isAlwaysOnTop()) --newIndex;
    ...
    void Desktop::incrementMouseWheelCounter() noexcept
    {
        ++mouseWheelCounter;
    }
    ...
    //==============================================================================
    void Desktop::addFocusChangeListener(FocusChangeListener* l)
    {
        focusListeners.add(l);
    }
    void Desktop::removeFocusChangeListener(FocusChangeListener* l)
    {
    ...
    {
        if (lastFakeMouseMove != getMousePositionFloat()) sendMouseMove();
    }
        ...
        if (!mouseListeners.isEmpty())
        {
            ...
            if (auto* target = findComponentAt(lastFakeMouseMove.roundToInt()))
            {
                ...
                const MouseEvent me(getMainMouseSource(), pos, ModifierKeys::getCurrentModifiers(),
                                    ...
                                    MouseInputSource::invalidRotation, MouseInputSource::invalidTiltX,
                                    MouseInputSource::invalidTiltY, target, target, now, pos, now, 0, false);
    ...
    RectangleList<int> Desktop::Displays::getRectangleList(bool userAreasOnly) const
    {
        ...
        for (auto& d : displays) rl.addWithoutMerging(userAreasOnly ? d.userArea : d.totalArea);
    ...
    bool operator==(const Desktop::Displays::Display& d1, const Desktop::Displays::Display& d2) noexcept
    {
        return d1.userArea == d2.userArea && d1.totalArea == d2.totalArea && d1.scale == d2.scale && d1.isMain == d2.isMain;
    }
    ...
    void Desktop::Displays::refresh()
    {
        ...
        if (oldDisplays != displays)
        {
            for (int i = ComponentPeer::getNumPeers(); --i >= 0;)
                if (auto* peer = ComponentPeer::getPeer(i)) peer->handleScreenSizeChange();
        }
    }
    ...
    bool Desktop::isOrientationEnabled(DisplayOrientation orientation) const noexcept
    {
        ...
        jassert(orientation == upright || orientation == upsideDown || orientation == rotatedClockwise
                || orientation == rotatedAntiClockwise);
