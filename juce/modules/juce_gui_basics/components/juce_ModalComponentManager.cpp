...
namespace juce
{
    struct ModalComponentManager::ModalItem : public ComponentMovementWatcher
    {
        ...
        void cancel()
        {
            if (isActive)
            {
                ...
                if (auto* mcm = ModalComponentManager::getInstanceWithoutCreating()) mcm->triggerAsyncUpdate();
            }
        }
    ...
    {
        if (component != nullptr) stack.add(new ModalItem(component, autoDelete));
    }
    ...
    void ModalComponentManager::endModal(Component* component)
    {
        for (int i = stack.size(); --i >= 0;)
        {
            ...
            if (item->component == component) item->cancel();
        }
    }
    ...
    Component* ModalComponentManager::getModalComponent(int index) const
    {
        ...
        for (int i = stack.size(); --i >= 0;)
        {
            ...
            if (item->isActive)
                if (n++ == index) return item->component;
        }
    ...
    {
        for (auto* item : stack)
            if (item->isActive && item->component == comp) return true;
            ...
            if (auto* peer = c->getPeer())
            {
                if (peer != lastOne)
                {
                    if (lastOne == nullptr)
                    {
                        ...
                        if (topOneShouldGrabFocus) peer->grabFocus();
                    }
                    else
                    {
    ...
    bool ModalComponentManager::cancelAllModalComponents()
    {
        ...
        for (int i = numModal; --i >= 0;)
            if (auto* c = getModalComponent(i)) c->exitModalState(0);
...
//==============================================================================
    ...
    int ModalComponentManager::runEventLoopForCurrentComponent()
    {
        ...
        if (auto* currentlyModal = getModalComponent(0))
        {
            ...
            attachCallback(currentlyModal, ModalCallbackFunction::create([&](int r) {
                               returnValue = r;
                               finished    = true;
                           }));
    ...
    {
        LambdaCallback(std::function<void(int)> fn) noexcept
            : function(fn)
        {
        }
