...
namespace juce
{
    ...
    public:
            ...
            , shadow(ds)
        {
            ...
            if (comp->isOnDesktop())
            {
                ...
                addToDesktop(ComponentPeer::windowIgnoresMouseClicks | ComponentPeer::windowIsTemporary
        ...
        {
            if (Component* c = target) shadow.drawForRectangle(g, getLocalArea(c, c->getLocalBounds()));
        }
        ...
        {
            if (target != nullptr) return target->getDesktopScaleFactor();
    ...
    void DropShadower::updateShadows()
    {
        ...
        if (owner->isShowing() && owner->getWidth() > 0 && owner->getHeight() > 0
            && (Desktop::canUseSemiTransparentWindows() || owner->getParentComponent() != nullptr))
        {
            while (shadowWindows.size() < 4) shadowWindows.add(new ShadowWindow(owner, shadow));
