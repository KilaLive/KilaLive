...
#define JUCE_USE_DARK_SPLASH_SCREEN 1
    ...
    struct ReportingThreadContainer : public ChangeListener, public DeletedAtShutdown
    {
    ...
    //==============================================================================
    struct ReportingThread : public Thread, private ChangeBroadcaster
    {
        ReportingThread(ReportingThreadContainer& container, String& address, String& userAgent, StringPairArray& parameters)
            ...
            , headers("User-Agent: " + userAgent)
        {
            ...
            for (auto& key : parameters.getAllKeys())
                if (parameters[key].isNotEmpty()) postData.add(key + "=" + URL::addEscapeChars(parameters[key], true));
        ...
        ~ReportingThread()
        {
            ...
            if (webStream != nullptr) webStream->cancel();
    ...
    JUCESplashScreen::JUCESplashScreen(Component& parent)
    {
        ...
        if (!appUsageReported)
        {
            ...
            if (appUsageReportingLock.isLocked() && !appUsageReported)
            {
                ...
                if (agentCPUVendor.isEmpty()) agentCPUVendor = "CPU";

                auto agentOSName = osName.replaceCharacter('.', '_').replace("iOS", "iPhone OS");
#if JUCE_IOS
                agentOSName << " like Mac OS X";
#endif

                String userAgent;
                userAgent << "Mozilla/5.0 (" << deviceDescription << ";" << agentCPUVendor << " " << agentOSName << ";"
                          << SystemStats::getDisplayLanguage() << ")";

                ReportingThreadContainer::getInstance()->sendReport("https://www.google-analytics.com/collect", userAgent,
                                                                    data);
        ...
        if (splashDisplayTime == 0 || Time::getMillisecondCounter() < splashDisplayTime + (uint32)millisecondsToDisplaySplash)
        {
    ...
    void JUCESplashScreen::paint(Graphics& g)
    {
        ...
        Point<float> bottomRight(0.9f * r.getWidth(), 0.9f * r.getHeight());

        ColourGradient cg(Colour(0x00000000),
                          Line<float>(0.0f, r.getHeight(), r.getWidth(), 0.0f).findNearestPointTo(bottomRight),
        ...
        if (splashDisplayTime == 0) splashDisplayTime = Time::getMillisecondCounter();

        if (!isTimerRunning()) startTimer(millisecondsToDisplaySplash);
    }
    ...
    void JUCESplashScreen::parentSizeChanged()
    {
        if (auto* p = getParentComponent())
            setBounds(
                p->getLocalBounds().removeFromBottom(splashScreenLogoHeight * 3).removeFromRight(splashScreenLogoWidth * 3));
    }
    ...
    {
        if (!hasStartedFading) return getLogoArea(getLocalBounds().toFloat()).contains((float)x, (float)y);
