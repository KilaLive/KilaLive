...
namespace juce
{
    ...
    void ResizableWindow::initialise(const bool shouldAddToDesktop)
    {
        ...
        */
        ...
        if (shouldAddToDesktop) addToDesktop();
    }
    ...
    void ResizableWindow::setContent(Component* newContentComponent, bool takeOwnership,
                                     bool resizeToFitWhenContentChangesSize)
    {
        ...
        if (resizeToFitWhenContentChangesSize) childBoundsChanged(contentComponent);
    ...
    void ResizableWindow::setContentComponent(Component* const newContentComponent, const bool deleteOldOne,
    ...
    void ResizableWindow::setContentComponentSize(int width, int height)
    {
        ...
        setSize(width + border.getLeftAndRight(), height + border.getTopAndBottom());
    }

    BorderSize<int> ResizableWindow::getBorderThickness()
    {
        if (isUsingNativeTitleBar() || isKioskMode()) return {};
    ...
    //==============================================================================
    void ResizableWindow::setResizable(const bool shouldBeResizable, const bool useBottomRightCornerResizer)
    {
        ...
        if (isUsingNativeTitleBar()) recreateDesktopWindow();
    ...
    {
        return resizableCorner != nullptr || resizableBorder != nullptr;
    }

    void ResizableWindow::setResizeLimits(int newMinimumWidth, int newMinimumHeight, int newMaximumWidth,
                                          int newMaximumHeight) noexcept
    {
        // if you've set up a custom constrainer then these settings won't have any effect..
        jassert(constrainer == &defaultConstrainer || constrainer == nullptr);

        if (constrainer == nullptr) setConstrainer(&defaultConstrainer);

        defaultConstrainer.setSizeLimits(newMinimumWidth, newMinimumHeight, newMaximumWidth, newMaximumHeight);
    ...
    void ResizableWindow::paint(Graphics& g)
    {
        ...
        lf.fillResizableWindowBackground(g, getWidth(), getHeight(), getBorderThickness(), *this);

        if (!isFullScreen()) lf.drawResizableWindowBorder(g, getWidth(), getHeight(), getBorderThickness(), *this);
    ...
    void ResizableWindow::setBackgroundColour(Colour newColour)
    {
        ...
        if (!Desktop::canUseSemiTransparentWindows()) backgroundColour = newColour.withAlpha(1.0f);
        ...
        if (shouldBeFullScreen != isFullScreen())
        {
            ...
            if (isOnDesktop())
            {
                if (auto* peer = getPeer())
                {
                    ...
                    if ((!shouldBeFullScreen) && !lastPos.isEmpty()) setBounds(lastPos);
                }
                else
                {
                    jassertfalse;
    ...
    {
        if (isOnDesktop())
            if (auto* peer = getPeer()) return peer->isKioskMode();
    ...
    {
        if (!(isFullScreen() || isMinimised() || isKioskMode())) lastNonFullScreenPos = getBounds();
    }

    void ResizableWindow::updatePeerConstrainer()
    {
        if (isOnDesktop())
            if (auto* peer = getPeer()) peer->setConstrainer(constrainer);
    }

    void ResizableWindow::parentSizeChanged()
    {
        if (isFullScreen() && getParentComponent() != nullptr) setBounds(getParentComponent()->getLocalBounds());
    }
    ...
    bool ResizableWindow::restoreWindowStateFromString(const String& s)
    {
        ...
        if (tokens.size() != firstCoord + 4) return false;

        Rectangle<int> newPos(tokens[firstCoord].getIntValue(), tokens[firstCoord + 1].getIntValue(),
                              tokens[firstCoord + 2].getIntValue(), tokens[firstCoord + 3].getIntValue());

        if (newPos.isEmpty()) return false;

        auto* peer = isOnDesktop() ? getPeer() : nullptr;

        if (peer != nullptr) peer->getFrameSize().addTo(newPos);

        {
            ...
            if (onScreenArea.getWidth() * onScreenArea.getHeight() < 32 * 32)
            {
                ...
                newPos.setSize(jmin(newPos.getWidth(), screen.getWidth()), jmin(newPos.getHeight(), screen.getHeight()));
        ...
        if (fs) setBoundsConstrained(newPos);
    ...
    {
        if (dragStarted) dragger.dragComponent(this, e, constrainer);
    }
