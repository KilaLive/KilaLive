...
namespace juce
{
        ...
        : content(c)
    {
        ...
        else
        {
            ...
            updatePosition(area, Desktop::getInstance().getDisplays().getDisplayContaining(area.getCentre()).userArea);
    ...
    //==============================================================================
    class CallOutBoxCallback : public ModalComponentManager::Callback, private Timer
    {
    public:
        CallOutBoxCallback(Component* c, const Rectangle<int>& area, Component* parent)
            : content(c)
            , callout(*c, area, parent)
        {
        ...
        {
            if (!Process::isForegroundProcess()) callout.dismiss();
        }
    ...
    {
        if (dismissalMouseClicksAreAlwaysConsumed || targetArea.contains(getMouseXYRelative() + getBounds().getPosition()))
        {
            ...
            if (elapsed.inMilliseconds() > 200) dismiss();
        }
        else
        {
            exitModalState(0);
    ...
    enum
    {
        callOutBoxDismissCommandId = 0x4f83a04b
    };
    ...
    void CallOutBox::updatePosition(const Rectangle<int>& newAreaToPointTo, const Rectangle<int>& newAreaToFitIn)
    {
        ...
        Line<float> lines[4] = {
            {targets[0].translated(-hwReduced, hh - arrowIndent), targets[0].translated(hwReduced, hh - arrowIndent)},
        ...
        for (int i = 0; i < 4; ++i)
        {
            ...
            if (!centrePointArea.intersects(lines[i])) distanceFromCentre += 1000.0f;
    ...
    void CallOutBox::refreshPath()
    {
        ...
        outline.addBubble(content.getBounds().toFloat().expanded(gap, gap), getLocalBounds().toFloat(),
                          targetPoint - getPosition().toFloat(), 9.0f, arrowSize * 0.7f);
    }
