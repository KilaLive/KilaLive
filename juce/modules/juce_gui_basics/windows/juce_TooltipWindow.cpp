...
namespace juce
{
        ...
        , millisecondsBeforeTipAppears(delayMs)
    {
        ...
        if (parentComp != nullptr) parentComp->addChildComponent(this);

        if (Desktop::getInstance().getMainMouseSource().canHover()) startTimer(123);
    }
    ...
    void TooltipWindow::displayTip(Point<int> screenPos, const String& tip)
    {
        ...
        if (!reentrant)
        {
            ...
            {
                updatePosition(tip, screenPos, Desktop::getInstance().getDisplays().getDisplayContaining(screenPos).userArea);

                addToDesktop(ComponentPeer::windowHasDropShadow | ComponentPeer::windowIsTemporary
                             | ComponentPeer::windowIgnoresKeyPresses | ComponentPeer::windowIgnoresMouseClicks);
            }
    ...
    {
        if (Process::isForegroundProcess() && !ModifierKeys::getCurrentModifiers().isAnyMouseButtonDown())
        {
            if (auto* ttc = dynamic_cast<TooltipClient*>(&c))
                if (!c.isCurrentlyBlockedByAnotherModalComponent()) return ttc->getTooltip();
        }
    ...
    void TooltipWindow::timerCallback()
    {
        ...
        if (tipChanged || mouseWasClicked || mouseMovedQuickly) lastCompChangeTime = now;
