...
namespace juce
{
    ...
    ComponentPeer* ComponentPeer::getPeerFor(const Component* const component) noexcept
    {
        for (auto* peer : Desktop::getInstance().peers)
            if (&(peer->getComponent()) == component) return peer;
    ...
    void ComponentPeer::handleMouseWheel(MouseInputSource::InputSourceType type, Point<float> pos, int64 time,
                                         const MouseWheelDetails& wheel, int touchIndex)
    {
        if (auto* mouse = Desktop::getInstance().mouseSources->getOrCreateMouseInputSource(type, touchIndex))
            MouseInputSource(*mouse).handleWheel(*this, pos, time, wheel);
    }

    void ComponentPeer::handleMagnifyGesture(MouseInputSource::InputSourceType type, Point<float> pos, int64 time,
                                             float scaleFactor, int touchIndex)
    {
        if (auto* mouse = Desktop::getInstance().mouseSources->getOrCreateMouseInputSource(type, touchIndex))
    ...
    void ComponentPeer::handlePaint(LowLevelGraphicsContext& contextToPaintTo)
    {
        ...
        if (component.isTransformed()) g.addTransform(component.getTransform());
    ...
    Component* ComponentPeer::getTargetForKeyPress()
    {
        ...
        if (c->isCurrentlyBlockedByAnotherModalComponent())
            if (auto* currentModalComp = Component::getCurrentlyModalComponent()) c = currentModalComp;
    ...
    bool ComponentPeer::handleKeyPress(const int keyCode, const juce_wchar textCharacter)
    {
        ...
        return handleKeyPress(KeyPress(keyCode, ModifierKeys::getCurrentModifiers().withoutMouseButtons(), textCharacter));
    }
        ...
        for (auto* target = getTargetForKeyPress(); target != nullptr; target = target->getParentComponent())
        {
            ...
            if (auto* keyListeners = target->keyListeners.get())
            {
                for (int i = keyListeners->size(); --i >= 0;)
                {
                    ...
                    if (keyWasUsed || deletionChecker == nullptr) return keyWasUsed;
            ...
            if (keyWasUsed || deletionChecker == nullptr) break;
    ...
    void ComponentPeer::handleModifierKeysChange()
    {
        ...
        if (target == nullptr) target = Component::getCurrentlyFocusedComponent();

        if (target == nullptr) target = &component;
    ...
    TextInputTarget* ComponentPeer::findCurrentTextInputTarget()
    {
        ...
        if (c == &component || component.isParentOf(c))
            if (auto* ti = dynamic_cast<TextInputTarget*>(c))
                if (ti->isTextInputActive()) return ti;
    ...
    void ComponentPeer::handleMovedOrResized()
    {
        ...
        if (component.flags.hasHeavyweightPeerFlag && !nowMinimised)
        {
            ...
            if (wasMoved || wasResized)
            {
                ...
                if (wasResized) component.repaint();
        ...
        if (!isFullScreen()) lastNonFullscreenBounds = component.getBounds();
    }
    ...
    void ComponentPeer::handleFocusGain()
    {
        ...
        if (component.isParentOf(lastFocusedComponent) && lastFocusedComponent->isShowing()
    ...
    namespace DragHelpers
    {
        ...
        static Component* findDragAndDropTarget(Component* c, const ComponentPeer::DragInfo& info, Component* lastOne)
        {
            for (; c != nullptr; c = c->getParentComponent())
                if (isSuitableTarget(info, c) && (c == lastOne || isInterested(info, c))) return c;
    ...
    bool ComponentPeer::handleDragMove(const ComponentPeer::DragInfo& info)
    {
        ...
        if (!DragHelpers::isSuitableTarget(info, newTarget)) return false;
        ...
        if (WeakReference<Component> targetComp = dragAndDropTargetComponent)
        {
            ...
            if (DragHelpers::isSuitableTarget(info, targetComp))
            {
                if (targetComp->isCurrentlyBlockedByAnotherModalComponent())
                {
                    ...
                    if (targetComp->isCurrentlyBlockedByAnotherModalComponent()) return true;
                }
                ...
                MessageManager::callAsync([=] {
                    ...
                    {
                        if (DragHelpers::isFileDrag(info))
                            dynamic_cast<FileDragAndDropTarget*>(c)->filesDropped(infoCopy.files, infoCopy.position.x,
                                                                                  infoCopy.position.y);
                        else
                            dynamic_cast<TextDragAndDropTarget*>(c)->textDropped(infoCopy.text, infoCopy.position.x,
                                                                                 infoCopy.position.y);
                    }
                });
