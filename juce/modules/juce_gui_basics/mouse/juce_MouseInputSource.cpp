...
namespace juce
{
    ...
    public:
        ...
        {
            if (!ComponentPeer::isValidPeer(lastPeer)) lastPeer = nullptr;
        ...
        Component* findComponentAt(Point<float> screenPos)
        {
            ...
            {
                auto relativePos =
                    ScalingHelpers::unscaledScreenPosToScaled(peer->getComponent(), peer->globalToLocal(screenPos));
                auto& comp = peer->getComponent();
                auto  pos  = relativePos.roundToInt();

                // (the contains() call is needed to test for overlapping desktop windows)
                if (comp.contains(pos)) return comp.getComponentAt(pos);
            }
        ...
        {
            return unboundedMouseOffset
                   + (inputType != MouseInputSource::InputSourceType::touch ? MouseInputSource::getCurrentRawMousePosition()
                                                                            : lastScreenPos);
        }
...
#define JUCE_MOUSE_EVENT_DBG(desc)                                                                                           \
    DBG("Mouse " << desc << " #" << index << ": " << screenPosToLocalPos(comp, screenPos).toString()                         \
...
#define JUCE_MOUSE_EVENT_DBG(desc)
        ...
        void sendMouseDown(Component& comp, Point<float> screenPos, Time time)
        {
            ...
            comp.internalMouseDown(MouseInputSource(this), screenPosToLocalPos(comp, screenPos), time, pressure, orientation,
        ...
        void sendMouseDrag(Component& comp, Point<float> screenPos, Time time)
        {
            ...
            comp.internalMouseDrag(MouseInputSource(this), screenPosToLocalPos(comp, screenPos), time, pressure, orientation,
                                   rotation, tiltX, tiltY);
        }
        ...
        void sendMouseUp(Component& comp, Point<float> screenPos, Time time, const ModifierKeys oldMods)
        {
            ...
            comp.internalMouseUp(MouseInputSource(this), screenPosToLocalPos(comp, screenPos), time, oldMods, pressure,
                                 orientation, rotation, tiltX, tiltY);
        }
        ...
        {
            if (buttonState == newButtonState) return false;

            // (avoid sending a spurious mouse-drag when we receive a mouse-up)
            if (!(isDragging() && !newButtonState.isAnyMouseButtonDown())) setScreenPos(screenPos, time, false);
            ...
            if (buttonState.isAnyMouseButtonDown())
            {
                if (auto* current = getComponentUnderMouse())
                {
                    ...
                    buttonState =
                        newButtonState;  // must change this before calling sendMouseUp, in case it runs a modal loop
        ...
        void setComponentUnderMouse(Component* const newComponent, Point<float> screenPos, Time time)
        {
            ...
            if (newComponent != current)
            {
                ...
                if (current != nullptr) sendMouseEnter(*current, screenPos, time);
        ...
        {
            if (!isDragging()) setComponentUnderMouse(findComponentAt(newScreenPos), newScreenPos, time);
            ...
            if (newScreenPos != lastScreenPos || forceUpdate)
            {
                ...
                if (auto* current = getComponentUnderMouse())
                {
                    if (isDragging())
                    {
                        ...
                        if (isUnboundedMouseModeOn) handleUnboundedDrag(*current);
                    }
                    else
                    {
        ...
        void handleEvent(ComponentPeer& newPeer, Point<float> positionWithinPeer, Time time, const ModifierKeys newMods,
                         float newPressure, float newOrientation, PenDetails pen)
        {
            lastTime = time;
            ...
            else
            {
                ...
                if (auto* peer = getPeer())
                {
                    ...
                    if (peer != nullptr) setScreenPos(screenPos, time, shouldUpdate);
                }
            }
        }

        Component* getTargetForGesture(ComponentPeer& peer, Point<float> positionWithinPeer, Time time,
        ...
        void handleWheel(ComponentPeer& peer, Point<float> positionWithinPeer, Time time, const MouseWheelDetails& wheel)
        {
            ...
            if (Component* target = lastNonInertialWheelTarget) sendMouseWheel(*target, screenPos, time, wheel);
        }
        ...
        int getNumberOfMultipleClicks() const noexcept
        {
            ...
            if (!hasMouseMovedSignificantlySincePressed())
            {
                ...
                {
                    if (mouseDowns[0].canBePartOfMultipleClickWith(mouseDowns[i],
                                                                   MouseEvent::getDoubleClickTimeout() * jmin(i, 2)))
                        ++numClicks;
                    else
                        break;
                }
        ...
        {
            return mouseMovedSignificantlySincePressed || lastTime > mouseDowns[0].time + RelativeTime::milliseconds(300);
        }
        ...
        void handleUnboundedDrag(Component& current)
        {
            ...
            else if (isCursorVisibleUntilOffscreen && (!unboundedMouseOffset.isOrigin())
        ...
        void revealCursor(bool forcedUpdate)
        {
            ...
            if (auto* current = getComponentUnderMouse()) mc = current->getLookAndFeel().getMouseCursorFor(*current);
    ...
    private:
        ...
        struct RecentMouseDown
        {
            ...
            bool canBePartOfMultipleClickWith(const RecentMouseDown& other, int maxTimeBetweenMs) const noexcept
            {
                return time - other.time < RelativeTime::milliseconds(maxTimeBetweenMs)
                       ...
                       && buttons == other.buttons && peerID == other.peerID;
            }
        ...
        void registerMouseDown(Point<float> screenPos, Time time, Component& component, const ModifierKeys modifiers,
                               bool isTouchSource) noexcept
        {
            for (int i = numElementsInArray(mouseDowns); --i > 0;) mouseDowns[i] = mouseDowns[i - 1];
        ...
        void registerMouseDrag(Point<float> screenPos) noexcept
        {
            mouseMovedSignificantlySincePressed =
                mouseMovedSignificantlySincePressed || mouseDowns[0].position.getDistanceFrom(screenPos) >= 4;
        }
    ...
    {
        return (getType() == MouseInputSource::InputSourceType::mouse);
    }
    bool MouseInputSource::isTouch() const noexcept
    {
        return (getType() == MouseInputSource::InputSourceType::touch);
    }
    bool MouseInputSource::isPen() const noexcept
    {
        return (getType() == MouseInputSource::InputSourceType::pen);
    }
    bool MouseInputSource::canHover() const noexcept
    {
        return !isTouch();
    }
    ...
    bool MouseInputSource::isDragging() const noexcept
    {
        return pimpl->isDragging();
    }
    Point<float> MouseInputSource::getScreenPosition() const noexcept
    {
        return pimpl->getScreenPosition();
    }
    Point<float> MouseInputSource::getRawScreenPosition() const noexcept
    {
        return pimpl->getRawScreenPosition();
    }
    ModifierKeys MouseInputSource::getCurrentModifiers() const noexcept
    {
    ...
    bool MouseInputSource::isPressureValid() const noexcept
    {
        return pimpl->isPressureValid();
    }
    float MouseInputSource::getCurrentOrientation() const noexcept
    {
        return pimpl->orientation;
    }
    bool MouseInputSource::isOrientationValid() const noexcept
    {
        return pimpl->isOrientationValid();
    }
    float MouseInputSource::getCurrentRotation() const noexcept
    {
        return pimpl->rotation;
    }
    bool MouseInputSource::isRotationValid() const noexcept
    {
    ...
    bool MouseInputSource::isTiltValid(bool isX) const noexcept
    {
    ...
    void MouseInputSource::handleEvent(ComponentPeer& peer, Point<float> pos, int64 time, ModifierKeys mods, float pressure,
                                       float orientation, const PenDetails& penDetails)
    {
    ...
    struct MouseInputSource::SourceList : public Timer
    {
        ...
        {
            return isPositiveAndBelow(index, sourceArray.size()) ? &sourceArray.getReference(index) : nullptr;
        }
        ...
        MouseInputSource* getOrCreateMouseInputSource(MouseInputSource::InputSourceType type, int touchIndex = 0)
        {
            ...
            else if (type == MouseInputSource::InputSourceType::touch)
            {
                ...
                for (auto& m : sourceArray)
                    if (type == m.getType() && touchIndex == m.getIndex()) return &m;

                if (canUseTouch()) return addSource(touchIndex, type);
            }
