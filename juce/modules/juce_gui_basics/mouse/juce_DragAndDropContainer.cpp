...
namespace juce
{
    ...
    //==============================================================================
    class DragAndDropContainer::DragImageComponent : public Component, private Timer
    {
    public:
        DragImageComponent(const Image& im, const var& desc, Component* const sourceComponent,
                           const MouseInputSource* draggingSource, DragAndDropContainer& ddc, Point<int> offset)
            ...
            , originalInputSourceType(draggingSource->getType())
        {
            ...
            if (mouseDragSource == nullptr) mouseDragSource = sourceComponent;
        ...
        ~DragImageComponent()
        {
            ...
            if (mouseDragSource != nullptr)
            {
                ...
                if (auto* current = getCurrentlyOver())
                    if (current->isInterestedInDragSource(sourceDetails)) current->itemDragExit(sourceDetails);
            }
        ...
        void mouseUp(const MouseEvent& e) override
        {
            ...
            {
                if (mouseDragSource != nullptr) mouseDragSource->removeMouseListener(this);
        ...
        {
            if (e.originalComponent != this && isOriginalInputSource(e.source)) updateLocation(true, e.getScreenPosition());
        }
        ...
        void updateLocation(const bool canDoExternalDrag, Point<int> screenPos)
        {
            ...
            if (newTargetComp != currentlyOverComp)
            {
                ...
                if (newTarget != nullptr && newTarget->isInterestedInDragSource(details)) newTarget->itemDragEnter(details);
            }
    ...
    private:
        ...
        static Component* findDesktopComponentBelow(Point<int> screenPos)
        {
            ...
            for (auto i = desktop.getNumComponents(); --i >= 0;)
            {
                ...
                if (auto* hit = c->getComponentAt(c->getLocalPoint(nullptr, screenPos))) return hit;
            }
        ...
        void setNewScreenPos(Point<int> screenPos)
        {
            ...
            if (auto* p = getParentComponent()) newPos = p->getLocalPoint(nullptr, newPos);
        ...
        void sendDragMove(DragAndDropTarget::SourceDetails& details) const
        {
            if (auto* target = getCurrentlyOver())
                if (target->isInterestedInDragSource(details)) target->itemDragMove(details);
        }
        ...
        void dismissWithAnimation(const bool shouldSnapBack)
        {
            ...
            {
                auto target = sourceDetails.sourceComponent->localPointToGlobal(
                    sourceDetails.sourceComponent->getLocalBounds().getCentre());
                auto ourCentre = localPointToGlobal(getLocalBounds().getCentre());

                animator.animateComponent(this, getBounds() + (target - ourCentre), 0.0f, 120, true, 1.0, 1.0);
            }
            else
            {
    ...
    void DragAndDropContainer::startDragging(const var& sourceDescription, Component* sourceComponent, Image dragImage,
    ...
    {
        if (isAlreadyDragging(sourceComponent)) return;
        ...
        {
            dragImage =
                sourceComponent->createComponentSnapshot(sourceComponent->getLocalBounds()).convertedToFormat(Image::ARGB);
            ...
            for (auto y = dragImage.getHeight(); --y >= 0;)
            {
                ...
                for (auto x = dragImage.getWidth(); --x >= 0;)
                {
                    ...
                    if (distance > lo)
                    {
                        auto alpha = (distance > hi) ? 0 : (hi - distance) / (float)(hi - lo) + random.nextFloat() * 0.008f;
        ...
        auto* dragImageComponent = dragImageComponents.add(
            new DragImageComponent(dragImage, sourceDescription, sourceComponent, draggingSource, *this, imageOffset));

        if (allowDraggingToExternalWindows)
        {
            if (!Desktop::canUseSemiTransparentWindows()) dragImageComponent->setOpaque(true);
        ...
        if (auto* peer = dragImageComponent->getPeer()) peer->performAnyPendingRepaintsNow();
#endif
    ...
    var DragAndDropContainer::getCurrentDragDescription() const
    {
        ...
        return dragImageComponents.size() != 0 ? dragImageComponents[0]->sourceDetails.description : var();
    }

    var DragAndDropContainer::getDragDescriptionForIndex(int index) const
    {
        if (!isPositiveAndBelow(index, dragImageComponents.size())) return {};
    ...
    bool DragAndDropContainer::shouldDropFilesWhenDraggedExternally(const DragAndDropTarget::SourceDetails&, StringArray&,
                                                                    bool&)
    {
        return false;
    }
    ...
    bool DragAndDropContainer::isAlreadyDragging(Component* component) const noexcept
    {
        ...
        {
            if (dragImageComp->sourceDetails.sourceComponent == component) return true;
        }
