...
namespace juce
{
    ...
    {
        if (cursorHandle != nullptr) cursorHandle->release();
    }

    MouseCursor& MouseCursor::operator=(const MouseCursor& other)
    {
        if (other.cursorHandle != nullptr) other.cursorHandle->retain();
    ...
    {
        return cursorHandle != nullptr ? cursorHandle->isStandardType(type) : (type == NormalCursor);
    }
