...
namespace juce
{
    namespace KeyboardFocusHelpers
    {
        ...
        static void findAllFocusableComponents(Component* parent, Array<Component*>& comps)
        {
            if (parent->getNumChildComponents() != 0)
            {
                ...
                for (auto* c : parent->getChildren())
                    if (c->isVisible() && c->isEnabled()) localComps.add(c);

                // This will sort so that they are ordered in terms of left-to-right
                // and then top-to-bottom.
                std::stable_sort(localComps.begin(), localComps.end(), [](const Component* a, const Component* b) {
                    auto explicitOrder1 = getOrder(a);
                    auto explicitOrder2 = getOrder(b);

                    if (explicitOrder1 != explicitOrder2) return explicitOrder1 < explicitOrder2;
                ...
                {
                    if (c->getWantsKeyboardFocus()) comps.add(c);

                    if (!c->isFocusContainer()) findAllFocusableComponents(c, comps);
                }
            }
        }
        ...
        static Component* findFocusContainer(Component* c)
        {
            ...
            if (c != nullptr)
                while (c->getParentComponent() != nullptr && !c->isFocusContainer()) c = c->getParentComponent();
    ...
    Component* KeyboardFocusTraverser::getDefaultComponent(Component* parentComponent)
    {
        ...
        if (parentComponent != nullptr) KeyboardFocusHelpers::findAllFocusableComponents(parentComponent, comps);
