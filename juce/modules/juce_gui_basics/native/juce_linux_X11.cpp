...
namespace juce
{
    ...
    namespace X11ErrorHandling
    {
        ...
        int ioErrorHandler(::Display*)
        {
            ...
            if (JUCEApplicationBase::isStandaloneApp()) MessageManager::getInstance()->stopDispatchLoop();
    ...
    XWindowSystem::~XWindowSystem() noexcept
    {
        if (JUCEApplicationBase::isStandaloneApp()) X11ErrorHandling::removeXErrorHandlers();
    ...
    ::Display* XWindowSystem::displayRef() noexcept
    {
        if (++displayCount == 1)
        {
            ...
            if (displayName.isEmpty()) displayName = ":0.0";
    ...
    void XWindowSystem::initialiseXDisplay() noexcept
    {
        ...
        juce_messageWindowHandle = XCreateWindow(display, RootWindow(display, screen), 0, 0, 1, 1, 0, 0, InputOnly,
                                                 DefaultVisual(display, screen), CWEventMask, &swa);
        ...
        LinuxEventLoop::setWindowSystemFd(fd, [this](int /*fd*/) {
            do
            {
                XEvent evt;
    ...
    //==============================================================================
    GetXProperty::GetXProperty(::Display* display, Window window, Atom atom, long offset, long length, bool shouldDelete,
                               Atom requestedType)
    {
        success = (XGetWindowProperty(display, window, atom, offset, length, (Bool)shouldDelete, requestedType, &actualType,
                                      &actualFormat, &numItems, &bytesLeft, &data)
                   == Success)
                  && data != nullptr;
    }
