...
namespace juce
{
    ...
    public:
        ...
        enum
        {
            charsAvailableForResult = 32768
        };

        Win32NativeFileChooser(Component* parent, int flags, FilePreviewComponent* previewComp, const File& startingFile,
                               const String& titleToUse, const String& filtersToUse)
            : Thread("Native Win32 FileChooser")
            , owner(parent)
            , title(titleToUse)
            , filtersString(filtersToUse)
            ...
            , shouldCancel(0)
        {
            ...
            {
                startingFile.getFileName().copyToUTF16(files, static_cast<size_t>(charsAvailableForResult) * sizeof(WCHAR));
                initialPath = parentDirectory.getFullPathName();
            }

            if (!selectsDirectories)
            {
                if (previewComp != nullptr) customComponent = new CustomComponentHolder(previewComp);
        ...
        void cancel()
        {
            ...
            if (auto hwnd = nativeDialogRef.get()) EndDialog(hwnd, 0);
        }
    ...
    private:
        ...
        public:
            ...
            {
                if (Component* const c = getChildComponent(0)) c->setBounds(getLocalBounds());
            }
        ...
        Array<URL> openDialog(bool async)
        {
            ...
            if (selectsDirectories)
            {
                ...
                if (list != nullptr && SUCCEEDED(SHGetMalloc(&al))) al->Free(list);

                if (files[0] != 0)
                {
                    File result(String(files.get()));

                    if (returnedString.isNotEmpty()) result = result.getSiblingFile(returnedString);
        ...
        void run() override
        {
            ...
            MessageManager::callAsync([safeThis, r] {
                ...
                if (safeThis->owner != nullptr) safeThis->owner->exitModalState(r.size() > 0 ? 1 : 0);
            });
        }
        ...
        DWORD getOpenFilenameFlags(bool async)
        {
            ...
            if (warnAboutOverwrite) ofFlags |= OFN_OVERWRITEPROMPT;

            if (selectMultiple) ofFlags |= OFN_ALLOWMULTISELECT;

            if (async || customComponent != nullptr) ofFlags |= OFN_ENABLEHOOK;
        ...
        void initDialog(HWND hdlg)
        {
            ...
            else
            {
                ...
                if (customComponent)
                {
                    ...
                    SetWindowPos(hdlg, 0, r.left, r.top, componentWidth + jmax(150, (int)(r.right - r.left)),
                                 jmax(150, (int)(r.bottom - r.top)), SWP_NOACTIVATE | SWP_NOOWNERZORDER | SWP_NOZORDER);
                ...
                if (FilePreviewComponent* comp = dynamic_cast<FilePreviewComponent*>(customComponent->getChildComponent(0)))
                {
                    ...
                    else
                    {
                        ...
                        MessageManager::callAsync(
                            [safeComp, selectedFile]() mutable { safeComp->selectedFileChanged(selectedFile); });
                    }
                }
            }
        }
        ...
        static int CALLBACK browseCallbackProc(HWND hWnd, UINT msg, LPARAM lParam, LPARAM lpData)
        {
            ...
            case BFFM_INITIALIZED:
                self->initialised(hWnd);
                break;
            case BFFM_VALIDATEFAILEDW:
                self->validateFailed(String((LPCWSTR)lParam));
                break;
            case BFFM_VALIDATEFAILEDA:
                self->validateFailed(String((const char*)lParam));
                break;
            default:
                break;
            }
        ...
        static UINT_PTR CALLBACK openCallback(HWND hwnd, UINT uiMsg, WPARAM /*wParam*/, LPARAM lParam)
        {
            ...
            case WM_DESTROY:
            {
                if (auto* self = getNativeDialogList()[hdlg]) self->destroyDialog(hdlg);
    ...
    public:
                                                           ...
                                                           fileChooser.title, fileChooser.filters))
        {
            ...
            setBounds(mainMon.getX() + mainMon.getWidth() / 4, mainMon.getY() + mainMon.getHeight() / 4, 0, 0);
        ...
        void launch() override
        {
            ...
            enterModalState(true, ModalCallbackFunction::create([safeThis](int) {
                                if (safeThis != nullptr) safeThis->owner.finished(safeThis->nativeFileChooser->results);
                            }));
    ...
    FileChooser::Pimpl* FileChooser::showPlatformDialog(FileChooser& owner, int flags, FilePreviewComponent* preview)
    {
