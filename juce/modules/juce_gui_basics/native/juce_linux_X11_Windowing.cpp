...
#define JUCE_X11_SUPPORTS_XEMBED 0
    ...
    bool KeyPress::isKeyCurrentlyDown(const int keyCode)
    {
        ...
        if (auto display = xDisplay.display)
        {
            ...
            else
            {
                ...
                if (keysym == (XK_Tab & 0xff) || keysym == (XK_Return & 0xff) || keysym == (XK_Escape & 0xff)
            ...
            if (!isChecked)
            {
                ...
                if (display != nullptr)
                {
                    ...
                    if (XShmQueryVersion(display, &major, &minor, &pixmaps))
                    {
                        ...
                        if (auto* xImage = XShmCreateImage(display, DefaultVisual(display, DefaultScreen(display)), 24,
                                                           ZPixmap, 0, &segmentInfo, 50, 50))
                        {
                            if ((segmentInfo.shmid =
                                     shmget(IPC_PRIVATE, (size_t)(xImage->bytes_per_line * xImage->height), IPC_CREAT | 0777))
                                >= 0)
                            {
                            ...
                            if (trappedErrorCode != 0) isAvailable = false;
                        }
                    }
                }
            }
    ...
    namespace XRender
    {
        ...
        static bool isAvailable(::Display* display)
        {
            ...
            if (!hasLoaded)
            {
                if (display != nullptr)
                {
                    ...
                    if (xRenderQueryVersion != nullptr && xRenderFindStandardFormat != nullptr && xRenderFindFormat != nullptr
                        && xRenderFindVisualFormat != nullptr)
                    {
                        int major, minor;
                        if (xRenderQueryVersion(display, &major, &minor)) return true;
                    }
                }
        ...
        {
            return display != nullptr && XGetSelectionOwner(display, Atoms::getCreating("_NET_WM_CM_S0")) != 0;
        }
            ...
            if (isAvailable())
            {
                ...
                if (pictFormat == nullptr)
                {
                    ...
                    pictFormat = xRenderFindFormat(display,
                                                   PictFormatType | PictFormatDepth | PictFormatRedMask | PictFormatRed
                                                       | PictFormatGreenMask | PictFormatGreen | PictFormatBlueMask
                                                       | PictFormatBlue | PictFormatAlphaMask | PictFormatAlpha,
                                                   &desiredFormat, 0);
                }
            }
    ...
    namespace Visuals
    {
        static Visual* findVisualWithDepth(::Display* display, const int desiredDepth) noexcept
        {
            ...
            if (XVisualInfo* xvinfos = XGetVisualInfo(display, desiredMask, &desiredVisual, &numVisuals))
            {
                ...
                if (XSHMHelpers::isShmAvailable(display))
                {
                    ...
                    if (XRender::isAvailable(display))
                    {
                        if (XRenderPictFormat* pictFormat = XRender::findPictureFormat(display))
                        {
                            ...
                            if (XVisualInfo* xvinfos =
                                    XGetVisualInfo(display, VisualScreenMask | VisualDepthMask | VisualBitsPerRGBMask,
                                                   &desiredVisual, &numVisuals))
                            {
                                for (int i = 0; i < numVisuals; ++i)
                                {
                                    XRenderPictFormat* pictVisualFormat =
                                        XRender::xRenderFindVisualFormat(display, xvinfos[i].visual);

                                    if (pictVisualFormat != nullptr && pictVisualFormat->type == PictTypeDirect
                    ...
                    if (visual == nullptr)
                    {
                        ...
                        if (visual != nullptr) matchedDepth = 32;
                    }
                }
#endif
            }
    ...
    {
    public:
        XBitmapImage(::Display* d, Image::PixelFormat format, int w, int h, bool clearImage, unsigned int imageDepth_,
                     Visual* visual)
            ...
            , display(d)
        {
            ...
            if ((imageDepth > 16) && XSHMHelpers::isShmAvailable(display))
            {
                ...
                xImage =
                    XShmCreateImage(display, visual, imageDepth, ZPixmap, 0, &segmentInfo, (unsigned int)w, (unsigned int)h);
...
#endif
            {
                ...
                if (!XInitImage(xImage)) jassertfalse;
            }
        }
    ...
    public:
        ...
        ExtendedInfo& findDisplayForRect(Rectangle<int> bounds, bool isScaledBounds)
        {
            ...
            for (int i = 0; i < infos.size(); ++i)
            {
                ...
                if (isScaledBounds) displayBounds = (displayBounds.withZeroOrigin() / dpy.scale) + dpy.topLeftScaled;
        ...
        ExtendedInfo& findDisplayForPoint(Point<int> pt, bool isScaledPoint)
        {
            ...
            for (int i = 0; i < infos.size(); ++i)
            {
                ...
                if (isScaledPoint) displayBounds = (displayBounds.withZeroOrigin() / dpy.scale) + dpy.topLeftScaled;

                if (displayBounds.contains(pt)) return dpy;
        ...
        {
            if (instance == nullptr) new DisplayGeometry(display, masterScale);
    ...
    private:
        ...
        static Array<XineramaScreenInfo> XineramaQueryDisplays(::Display* display)
        {
            ...
            if (XQueryExtension(display, "XINERAMA", &major_opcode, &first_event, &first_error))
            {
                ...
                if (libXinerama == nullptr)
                {
                    ...
                    if (libXinerama == nullptr) libXinerama = dlopen("libXinerama.so.1", RTLD_GLOBAL | RTLD_NOW);
        ...
        private:
            XRandrWrapper()
            {
                if (libXrandr == nullptr)
                {
                    ...
                    if (libXrandr == nullptr) libXrandr = dlopen("libXrandr.so.2", RTLD_GLOBAL | RTLD_NOW);
        ...
        public:
            ...
            {
                if (instance == nullptr) instance = new XRandrWrapper();
            ...
            {
                if (getScreenResourcesPtr != nullptr) return getScreenResourcesPtr(display, window);
            ...
            {
                if (getOutputInfoPtr != nullptr) return getOutputInfoPtr(display, resources, output);
            ...
            {
                if (getCrtcInfoPtr != nullptr) return getCrtcInfoPtr(display, resources, crtc);
            ...
            {
                if (getOutputPrimaryPtr != nullptr) return getOutputPrimaryPtr(display, window);
        ...
        private:
            ...
            {
                if (freeScreenResourcesPtr != nullptr) freeScreenResourcesPtr(ptr);
            }

            void freeOutputInfo(XRROutputInfo* ptr)
            {
                if (freeOutputInfoPtr != nullptr) freeOutputInfoPtr(ptr);
            }

            void freeCrtcInfo(XRRCrtcInfo* ptr)
            {
                if (freeCrtcInfoPtr != nullptr) freeCrtcInfoPtr(ptr);
            }
        ...
        static double getScaleForDisplay(const String& name, const ExtendedInfo& info)
        {
                        ...
                        if (dconf.getExitCode() == 0 && jsonOutput.isNotEmpty())
                        {
                            ...
                            if (DynamicObject* object = jsonVar.getDynamicObject())
                            {
                                ...
                                if (!scaleFactorVar.isVoid())
                                {
                                    ...
                                    if (scaleFactor > 0.0) return scaleFactor;
                                }
                            }
                        }
                    }
...

            {
                ...
                if (File("/usr/bin/gsettings").existsAsFile()
                    && gsettings.start("/usr/bin/gsettings get org.gnome.desktop.interface scaling-factor",
                    ...
                    for (int i = 0; i < numMonitors; ++i)
                    {
                        if ((screens = xrandr.getScreenResources(display, RootWindow(display, i))).get())
                        {
                            for (int j = 0; j < screens->noutput; ++j)
                            {
                                ...
                                if (!mainDisplay) mainDisplay = screens->outputs[j];
                                ...
                                if ((output = xrandr.getOutputInfo(display, screens.get(), screens->outputs[j])).get())
                                {
                                    ...
                                    {
                                        ExtendedInfo e;
                                        e.totalBounds = Rectangle<int>(crtc->x, crtc->y, (int)crtc->width, (int)crtc->height);
                                        e.usableBounds =
                                            e.totalBounds
                                                .withZeroOrigin();  // Support for usable area is not implemented in JUCE yet
                                        e.topLeftScaled = e.totalBounds.getTopLeft();
                                        e.isMain        = (mainDisplay == screens->outputs[j]) && (i == 0);
                                        e.dpi           = getDisplayDPI(display, 0);

                                        // The raspberry pi returns a zero sized display, so we need to guard for
                                        // divide-by-zero
                                        if (output->mm_width > 0 && output->mm_height > 0)
                                            e.dpi = ((static_cast<double>(crtc->width) * 25.4 * 0.5)
                ...
                for (int index = 0; index < numMonitors; ++index)
                {
                    for (int j = numMonitors; --j >= 0;)
                    {
                        if (screens[j].screen_number == index)
                        {
                            ExtendedInfo e;
                            e.totalBounds =
                                Rectangle<int>(screens[j].x_org, screens[j].y_org, screens[j].width, screens[j].height);
                            e.usableBounds =
                                e.totalBounds.withZeroOrigin();  // Support for usable area is not implemented in JUCE yet
                            e.topLeftScaled =
                                e.totalBounds.getTopLeft();  // this will be overwritten by updatePositions later
                            e.isMain = (index == 0);
                            e.scale  = masterScale;
                ...
                if (hints != None)
                {
                    ...
                    for (int i = 0; i < numMonitors; ++i)
                    {
                        ...
                        if (prop.success && prop.actualType == XA_CARDINAL && prop.actualFormat == 32 && prop.numItems == 4)
                        {
                            ...
                            ExtendedInfo e;
                            e.totalBounds =
                                Rectangle<int>((int)position[0], (int)position[1], (int)position[2], (int)position[3]);
                            e.usableBounds =
                                e.totalBounds.withZeroOrigin();  // Support for usable area is not implemented in JUCE yet
                            e.topLeftScaled =
                                e.totalBounds.getTopLeft();  // this will be overwritten by updatePositions later
        ...
        void updateScaledDisplayCoordinate(bool updateYCoordinates)
        {
            ...
            for (auto& i : infos) copy.add(&i);

            std::sort(copy.begin(), copy.end(), [updateYCoordinates](const ExtendedInfo* a, const ExtendedInfo* b) {
                if (updateYCoordinates) return a->totalBounds.getY() < b->totalBounds.getY();
            ...
            for (int i = 1; i < copy.size(); ++i)
            {
                ...
                for (int j = i - 1; j >= 0; --j)
                {
                    ...
                    if (prevCoordinate == curCoordinate)
                    {
                        ...
                        topLeftScaled +=
                            Point<int>(other.totalBounds.getWidth(), other.totalBounds.getHeight()) / other.scale;
    ...
    {
        if (ptr != nullptr) DisplayGeometry::XRandrWrapper::getInstance().freeScreenResources(ptr);
    }

    void ContainerDeletePolicy<XRROutputInfo>::destroy(XRROutputInfo* ptr)
    {
        if (ptr != nullptr) DisplayGeometry::XRandrWrapper::getInstance().freeOutputInfo(ptr);
    }

    void ContainerDeletePolicy<XRRCrtcInfo>::destroy(XRRCrtcInfo* ptr)
    {
        if (ptr != nullptr) DisplayGeometry::XRandrWrapper::getInstance().freeCrtcInfo(ptr);
    }
#endif
    ...
    namespace PixmapHelpers
    {
        Pixmap createColourPixmapFromImage(::Display* display, const Image& image)
        {
            ...
            for (int y = 0; y < (int)height; ++y)
                for (int x = 0; x < (int)width; ++x) colour[index++] = image.getPixelAt(x, y).getARGB();

            XImage* ximage = XCreateImage(display, CopyFromParent, 24, ZPixmap, 0, reinterpret_cast<char*>(colour.getData()),
                                          width, height, 32, 0);

            Pixmap pixmap = XCreatePixmap(display, DefaultRootWindow(display), width, height, 24);
        ...
        Pixmap createMaskPixmapFromImage(::Display* display, const Image& image)
        {
            ...
            for (unsigned int y = 0; y < height; ++y)
            {
                for (unsigned int x = 0; x < width; ++x)
                {
                    ...
                    if (image.getPixelAt((int)x, (int)y).getAlpha() >= 128) mask[offset] |= bit;
                }
            }

            return XCreatePixmapFromBitmapData(display, DefaultRootWindow(display), mask.getData(), width, height, 1, 0, 1);
        }
    }  // namespace PixmapHelpers
    ...
    public:
            ...
            , isAlwaysOnTop(comp.isAlwaysOnTop())
        {
            ...
            if (isAlwaysOnTop) ++numAlwaysOnTopPeers;
        ...
        void setBounds(const Rectangle<int>& newBounds, bool isNowFullScreen) override
        {
            ...
            if (windowH != 0)
            {
                ...
                XMoveResizeWindow(display, windowH, physicalBounds.getX() - windowBorder.getLeft(),
                                  physicalBounds.getY() - windowBorder.getTop(), (unsigned int)physicalBounds.getWidth(),
        ...
        bool isMinimised() const override
        {
            ...
            return prop.success && prop.actualType == atoms->state && prop.actualFormat == 32 && prop.numItems > 0
        ...
        void setFullScreen(const bool shouldBeFullScreen) override
        {
            ...
            {
                if (shouldBeFullScreen) r = Desktop::getInstance().getDisplays().getMainDisplay().userArea;

                if (!r.isEmpty()) setBounds(ScalingHelpers::scaledScreenPosToUnscaled(component, r), shouldBeFullScreen);
        ...
        bool isChildWindowOf(Window possibleParent) const
        {
            ...
            {
                if (windowList != nullptr) XFree(windowList);
        ...
        bool isParentWindowOf(Window possibleChild) const
        {
            ...
            {
                if (possibleChild == windowH) return true;
        ...
        {
            if (!bounds.withZeroOrigin().contains(localPos)) return false;
            ...
            for (int i = Desktop::getInstance().getNumComponents(); --i >= 0;)
            {
                ...
                if (auto* peer = c->getPeer())
                    if (peer->contains(localPos + bounds.getPosition() - peer->getBounds().getPosition(), true)) return false;
            }

            if (trueIfInAChildWindow) return true;
        ...
        void toBehind(ComponentPeer* other) override
        {
            ...
            {
                if (otherPeer->styleFlags & windowIsTemporary) return;
        ...
        {
#if JUCE_X11_SUPPORTS_XEMBED
            if (Window w = (Window)juce_getCurrentFocusWindow(this)) return w;
#endif
        ...
        void grabFocus() override
        {
            ...
            if (windowH != 0 && XGetWindowAttributes(display, windowH, &atts) && atts.map_state == IsViewable && !isFocused())
            {
        ...
        void setIcon(const Image& newIcon) override
        {
            ...
            if (wmHints == nullptr) wmHints = XAllocWMHints();
        ...
        void handleWindowMessage(XEvent& event)
        {
            ...
            default:
                ...
                {
                    ScopedXLock xlock(display);
                    if (event.xany.type == XShmGetEventBase(display)) repainter->notifyPaintCompleted();
                }
#endif
                break;
            }
        ...
        void handleKeyPressEvent(XKeyEvent& keyEvent)
        {
...

            {
                ...
                if (oldLocale.isNotEmpty()) ::setlocale(LC_ALL, oldLocale.toRawUTF8());

                unicodeChar = *CharPointer_UTF8(utf8);
                keyCode     = (int)unicodeChar;

                if (keyCode < 0x20)
                    keyCode = (int)XkbKeycodeToKeysym(display, (::KeyCode)keyEvent.keycode, 0,
                                                      currentModifiers.isShiftDown() ? 1 : 0);
            ...
            if (utf8[0] != 0 || ((sym & 0xff00) == 0 && sym >= 8)) keyPressed = true;

            if (oldMods != currentModifiers) handleModifierKeysChange();

            if (keyDownChange) handleKeyUpOrDown(true);

            if (keyPressed) handleKeyPress(keyCode, unicodeChar);
        }
        ...
        static bool isKeyReleasePartOfAutoRepeat(const XKeyEvent& keyReleaseEvent)
        {
            if (XPending(display))
            {
                ...
                return e.type == KeyPressEventType && e.xkey.keycode == keyReleaseEvent.keycode
        ...
        void handleKeyReleaseEvent(const XKeyEvent& keyEvent)
        {
            if (!isKeyReleasePartOfAutoRepeat(keyEvent))
            {
                ...
                if (keyDownChange) handleKeyUpOrDown(false);
            }
        }
        ...
        void handleMotionNotifyEvent(const XPointerMovedEvent& movedEvent)
        {
            ...
            if (dragState->dragging) handleExternalDragMotionNotify();
        ...
        void handleExposeEvent(XExposeEvent& exposeEvent)
        {
            ...
            {
                Window child;
                XTranslateCoordinates(display, exposeEvent.window, windowH, exposeEvent.x, exposeEvent.y, &exposeEvent.x,
                                      &exposeEvent.y, &child);
            }
            ...
            repaint(Rectangle<int>(exposeEvent.x, exposeEvent.y, exposeEvent.width, exposeEvent.height) / currentScaleFactor);

            while (XEventsQueued(display, QueuedAfterFlush) > 0)
            {
                XPeekEvent(display, &nextEvent);
                if (nextEvent.type != Expose || nextEvent.xany.window != exposeEvent.window) break;

                XNextEvent(display, &nextEvent);
                const XExposeEvent& nextExposeEvent = (const XExposeEvent&)nextEvent.xexpose;
                repaint(Rectangle<int>(nextExposeEvent.x, nextExposeEvent.y, nextExposeEvent.width, nextExposeEvent.height)
                        / currentScaleFactor);
            }
        }
        ...
        void handleConfigureNotifyEvent(XConfigureEvent& confEvent)
        {
            ...
            if ((styleFlags & windowHasTitleBar) != 0 && component.isCurrentlyBlockedByAnotherModalComponent())
            {
                if (auto* currentModalComp = Component::getCurrentlyModalComponent())
                    currentModalComp->inputAttemptWhenModal();
            }

            if (confEvent.window == windowH && confEvent.above != 0 && isFrontWindow()) handleBroughtToFront();
        }
        ...
        void handleReparentNotifyEvent()
        {
            ...
            if (parentWindow == windowH || parentWindow == wRoot) parentWindow = 0;
            ...
            if (clientMsg.message_type == atoms->protocols && clientMsg.format == 32)
            {
                ...
                else if (atom == atoms->protocolList[Atoms::TAKE_FOCUS])
                {
                    if ((getStyleFlags() & juce::ComponentPeer::windowIgnoresKeyPresses) == 0)
                    {
                        ...
                        ScopedXLock xlock(display);
                        if (clientMsg.window != 0 && XGetWindowAttributes(display, clientMsg.window, &atts))
                        {
                            if (atts.map_state == IsViewable)
                                XSetInputFocus(display, (clientMsg.window == windowH ? getFocusWindow() : clientMsg.window),
        ...
        {
            if (dummy != nullptr) glRepaintListeners.addIfNotAlreadyThere(dummy);
        }

        void removeOpenGLRepaintListener(Component* dummy)
        {
            if (dummy != nullptr) glRepaintListeners.removeAllInstancesOf(dummy);
        }

        void repaintOpenGLContexts()
        {
            for (int i = 0; i < glRepaintListeners.size(); ++i)
                if (auto* c = glRepaintListeners[i]) c->handleCommandMessage(0);
        }
        ...
        unsigned long createKeyProxy()
        {
            ...
            if (keyProxy == 0 && windowH != 0)
            {
                ...
                keyProxy = XCreateWindow(display, windowH, -1, -1, 1, 1, 0, 0, InputOnly, CopyFromParent, CWEventMask, &swa);
    ...
    private:
        ...
        public:
                ...
                , display(d)
            {
                ...
                if (useARGBImagesForRendering)
                {
                    ...
                    XImage* const testImage = XShmCreateImage(display, DefaultVisual(display, DefaultScreen(display)), 24,
                                                              ZPixmap, 0, &segmentinfo, 64, 64);
            ...
            {
                if (!isTimerRunning()) startTimer(repaintTimerPeriod);
            ...
            void performAnyPendingRepaintsNow()
            {
                ...
                if (!totalArea.isEmpty())
                {
                    ...
                    {
#if JUCE_USE_XSHM
                        image = Image(new XBitmapImage(display, useARGBImagesForRendering ? Image::ARGB : Image::RGB,
#else
                        image = Image(new XBitmapImage(display, Image::RGB,
#endif
                                                       (totalArea.getWidth() + 31) & ~31, (totalArea.getHeight() + 31) & ~31,
                    ...
                    if (peer.depth == 32)
                        for (auto& i : originalRepaintRegion) image.clear(i - totalArea.getPosition());

                    {
                        ScopedPointer<LowLevelGraphicsContext> context(
                            peer.getComponent().getLookAndFeel().createGraphicsContext(image, -totalArea.getPosition(),
                                                                                       adjustedList));
...
#if JUCE_USE_XSHM
                        if (xbitmap->isUsingXShm()) ++shmPaintsPending;
#endif

                        xbitmap->blitToWindow(peer.windowH, i.getX(), i.getY(), (unsigned int)i.getWidth(),
                                              (unsigned int)i.getHeight(), i.getX() - totalArea.getX(),
                                              i.getY() - totalArea.getY());
                    }
                }
...
#if JUCE_USE_XSHM
            void notifyPaintCompleted() noexcept
            {
                --shmPaintsPending;
            }
#endif
        ...
        static bool updateKeyModifiersFromSym(KeySym sym, const bool press) noexcept
        {
            ...
            case XK_Num_Lock:
                if (press) Keys::numLock = !Keys::numLock;

                break;

            case XK_Caps_Lock:
                if (press) Keys::capsLock = !Keys::capsLock;
            ...
            currentModifiers = press ? currentModifiers.withFlags(modifier) : currentModifiers.withoutFlags(modifier);
        ...
        void addWindowButtons(Window wndH)
        {
            ...
            if (hints != None)
            {
                ...
                if ((styleFlags & windowHasCloseButton) != 0) motifHints.functions |= 32; /* MWM_FUNC_CLOSE */
        ...
        void setWindowType()
        {
            ...
            if (component.isAlwaysOnTop()) netHints[numHints++] = Atoms::getIfExists(display, "_NET_WM_STATE_ABOVE");

            if (numHints > 0) xchangeProperty(windowH, atoms->windowState, XA_ATOM, 32, &netHints, numHints);
        }
        ...
        void createWindow(Window parentToAddTo)
        {
            ...
            windowH =
                XCreateWindow(display, parentToAddTo != 0 ? parentToAddTo : root, 0, 0, 1, 1, 0, depth, InputOutput, visual,
                              CWBorderPixel | CWColormap | CWBackPixmap | CWEventMask | CWOverrideRedirect, &swa);
            ...
            xchangeProperty(windowH, atoms->XdndTypeList, XA_ATOM, 32, atoms->allowedMimeTypes,
                            numElementsInArray(atoms->allowedMimeTypes));
            xchangeProperty(windowH, atoms->XdndActionList, XA_ATOM, 32, atoms->allowedActions,
        ...
        {
            return NoEventMask | KeyPressMask | KeyReleaseMask | EnterWindowMask | LeaveWindowMask | PointerMotionMask
                   | KeymapStateMask | ExposureMask | StructureNotifyMask | FocusChangeMask
        ...
        static int64 getEventTime(::Time t)
        {
            ...
            if (eventTimeOffset == 0x12345678) eventTimeOffset = Time::currentTimeMillis() - thisMessageTime;
            ...
            else if (windowBorder.getTopAndBottom() == 0 && windowBorder.getLeftAndRight() == 0)
            {
                ...
                if (hints != None)
                {
                    ...
                    if (prop.success && prop.actualFormat == 32)
                    {
                        ...
                        windowBorder = BorderSize<int>((int)sizes[2], (int)sizes[0], (int)sizes[3], (int)sizes[1]);
                    }
                }
            }
        }
        ...
        void updateWindowBounds()
        {
            ...
            if (windowH != 0)
            {
                ...
                if (XGetGeometry(display, (::Drawable)windowH, &root, &wx, &wy, &ww, &wh, &bw, &bitDepth))
                    if (!XTranslateCoordinates(display, windowH, root, 0, 0, &wx, &wy, &child)) wx = wy = 0;

                Rectangle<int> physicalBounds(wx, wy, (int)ww, (int)wh);

                currentScaleFactor = DisplayGeometry::getInstance().findDisplayForRect(physicalBounds, false).scale;
        ...
        void sendExternalDragAndDropEnter(const Window targetWindow)
        {
            ...
            for (int i = 0; i < 3; ++i) msg.data.l[i + 2] = (long)dragState->allowedTypes[i];
        ...
        {
            if (dragState->dragging) externalResetDragAndDrop();
        }
        ...
        void handleExternalSelectionRequest(const XEvent& evt)
        {
            ...
            if (dragState->allowedTypes.contains(targetType))
            {
                ...
                xchangeProperty(evt.xselectionrequest.requestor, evt.xselectionrequest.property, targetType, 8,
                                dragState->textOrFiles.toRawUTF8(), (int)dragState->textOrFiles.getNumBytesAsUTF8());
            }
            ...
            if (dragState->expectingStatus)
            {
                        ...
                        || (Atom)clientMsg.data.l[4] == atoms->XdndActionPrivate))
                {
                    if ((clientMsg.data.l[1] & 2) == 0)  // target requests silent rectangle
                        dragState->silentRect.setBounds((int)clientMsg.data.l[2] >> 16, (int)clientMsg.data.l[2] & 0xffff,
        ...
        {
            if (dragState->dragging) XUngrabPointer(display, CurrentTime);
        ...
        void handleExternalDragMotionNotify()
        {
            ...
            {
                if (dragState->targetWindow != None) sendExternalDragAndDropLeave(dragState->targetWindow);
                ...
                GetXProperty prop(display, targetWindow, atoms->XdndAware, 0, 2, false, AnyPropertyType);
            ...
            if (!dragState->expectingStatus) sendExternalDragAndDropPosition(targetWindow);
        }

        void handleDragAndDropPosition(const XClientMessageEvent& clientMsg)
        {
            if (dragAndDropSourceWindow == 0) return;

            dragAndDropSourceWindow = (::Window)clientMsg.data.l[0];

            Point<int> dropPos((int)clientMsg.data.l[2] >> 16, (int)clientMsg.data.l[2] & 0xffff);
            ...
            if (dragInfo.position != dropPos)
            {
                ...
                if (dragInfo.isEmpty()) updateDraggedFileList(clientMsg);

                if (!dragInfo.isEmpty()) handleDragMove(dragInfo);
            }
        }
        ...
        void handleDragAndDropDataReceived()
        {
            ...
            if (!dragInfoCopy.isEmpty()) handleDragDrop(dragInfoCopy);
        }
        ...
        void handleDragAndDropEnter(const XClientMessageEvent& clientMsg)
        {
            ...
            if ((clientMsg.data.l[1] & 1) != 0)
            {
                ...
                if (prop.success && prop.actualType == XA_ATOM && prop.actualFormat == 32 && prop.numItems != 0)
                {
                    ...
                    for (unsigned long i = 0; i < prop.numItems; ++i)
                        if (types[i] != None) srcMimeTypeAtomList.add(types[i]);
                }
            }
            ...
            {
                for (int i = 2; i < 5; ++i)
                    if (clientMsg.data.l[i] != None) srcMimeTypeAtomList.add((unsigned long)clientMsg.data.l[i]);
        ...
        void handleDragAndDropSelection(const XEvent& evt)
        {
            ...
            if (evt.xselection.property != None)
            {
...

                {
                    ...
                    for (;;)
                    {
                        GetXProperty prop(display, evt.xany.window, evt.xselection.property, dropData.getSize() / 4, 65536,
                                          false, AnyPropertyType);

                        if (!prop.success) break;

                        dropData.append(prop.data, prop.numItems * (size_t)prop.actualFormat / 8);

                        if (prop.bytesLeft <= 0) break;
                    }
                ...
                if (finishAfterDropDataReceived) handleDragAndDropDataReceived();
            }
        }
        ...
        void updateDraggedFileList(const XClientMessageEvent& clientMsg)
        {
            ...
            {
                ScopedXLock xlock(display);
                XConvertSelection(display, atoms->XdndSelection, dragAndDropCurrentMimeType,
                                  Atoms::getCreating(display, "JXSelectionWindowProperty"), windowH,
        ...
        bool isWindowDnDAware(Window w) const
        {
            ...
            for (int i = 0; i < numProperties; ++i)
                if (properties[i] == atoms->XdndAware) dndAwarePropFound = true;

            if (properties != nullptr) XFree(properties);
        ...
        {
            if (targetWindow == None) return None;

            if (isWindowDnDAware(targetWindow)) return targetWindow;

            Window       child, phonyWin;
            int          phony;
            unsigned int uphony;

            XQueryPointer(display, targetWindow, &phonyWin, &child, &phony, &phony, &phony, &phony, &uphony);
        ...
        bool externalDragInit(bool isText, const String& textOrFiles)
        {
            ...
            if (XGrabPointer(display, windowH, True, pointerGrabMask, GrabModeAsync, GrabModeAsync, None, None, CurrentTime)
                == GrabSuccess)
            {
                ...
                xchangeProperty(windowH, atoms->XdndTypeList, XA_ATOM, 32, dragState->allowedTypes.getRawDataPointer(),
...
#endif
                {
                    if (auto* peer = LinuxComponentPeer::getPeerFor(event.xany.window)) peer->handleWindowMessage(event);
                }
            }
    ...
    ModifierKeys ModifierKeys::getCurrentModifiersRealtime() noexcept
    {
        ...
        if (auto display = xDisplay.display)
        {
            ...
            if (XQueryPointer(display, RootWindow(display, DefaultScreen(display)), &root, &child, &x, &y, &winx, &winy,
                              &mask)
                != False)
            {
            ...
            LinuxComponentPeer::currentModifiers =
                LinuxComponentPeer::currentModifiers.withoutMouseButtons().withFlags(mouseMods);
        }
    ...
    {
        if (enableOrDisable) comp->setBounds(getDisplays().getMainDisplay().totalArea);
    }
    ...
    void Desktop::Displays::findDisplays(float masterScale)
    {
        ...
        if (auto display = xDisplay.display)
        {
            ...
            for (mainDisplayIdx = 0; mainDisplayIdx < geometry.infos.size(); ++mainDisplayIdx)
            {
                ...
                if (info.isMain) break;
            }

            // no main display found then use the first
            if (mainDisplayIdx >= geometry.infos.size()) mainDisplayIdx = 0;
    ...
    bool Desktop::canUseSemiTransparentWindows() noexcept
    {
        ...
        if (XRender::hasCompositingWindowManager())
        {
            ...
            return Visuals::findVisualFormat(display, desiredDepth, matchedDepth) != 0 && matchedDepth == desiredDepth;
        }
#endif
    ...
    Point<float> MouseInputSource::getCurrentRawMousePosition()
    {
        ...
        if (XQueryPointer(display, RootWindow(display, DefaultScreen(display)), &root, &child, &x, &y, &winx, &winy, &mask)
            == False)
        {
    ...
    void Desktop::setScreenSaverEnabled(const bool isEnabled)
    {
        if (screenSaverAllowed != isEnabled)
        {
            ...
            if (auto display = xDisplay.display)
            {
                ...
                ScopedXLock xlock(display);
                if (xScreenSaverSuspend != nullptr) xScreenSaverSuspend(display, !isEnabled);
            }
        }
    }
    ...
    {
        if (auto* linuxPeer = dynamic_cast<LinuxComponentPeer*>(peer)) bounds *= linuxPeer->getCurrentScale();
    ...
    {
        if (auto* linuxPeer = dynamic_cast<LinuxComponentPeer*>(peer)) linuxPeer->addOpenGLRepaintListener(dummy);
    }

    void juce_LinuxRemoveRepaintListener(ComponentPeer* peer, Component* dummy)
    {
        if (auto* linuxPeer = dynamic_cast<LinuxComponentPeer*>(peer)) linuxPeer->removeOpenGLRepaintListener(dummy);
    }

    unsigned long juce_createKeyProxyWindow(ComponentPeer* peer)
    {
        if (auto* linuxPeer = dynamic_cast<LinuxComponentPeer*>(peer)) return linuxPeer->createKeyProxy();
    ...
    {
        if (auto* linuxPeer = dynamic_cast<LinuxComponentPeer*>(peer)) linuxPeer->deleteKeyProxy();
    }

//==============================================================================
#if JUCE_MODAL_LOOPS_PERMITTED
    void JUCE_CALLTYPE NativeMessageBox::showMessageBox(AlertWindow::AlertIconType iconType, const String& title,
                                                        const String& message, Component* /* associatedComponent */)
    {
        AlertWindow::showMessageBox(iconType, title, message);
    }
#endif

    void JUCE_CALLTYPE NativeMessageBox::showMessageBoxAsync(AlertWindow::AlertIconType iconType, const String& title,
                                                             const String& message, Component* associatedComponent,
    ...
    bool JUCE_CALLTYPE NativeMessageBox::showOkCancelBox(AlertWindow::AlertIconType iconType, const String& title,
                                                         const String& message, Component* associatedComponent,
                                                         ModalComponentManager::Callback* callback)
    {
        return AlertWindow::showOkCancelBox(iconType, title, message, String(), String(), associatedComponent, callback);
    }

    int JUCE_CALLTYPE NativeMessageBox::showYesNoCancelBox(AlertWindow::AlertIconType iconType, const String& title,
                                                           const String& message, Component* associatedComponent,
                                                           ModalComponentManager::Callback* callback)
    {
        return AlertWindow::showYesNoCancelBox(iconType, title, message, String(), String(), String(), associatedComponent,
                                               callback);
    }

    int JUCE_CALLTYPE NativeMessageBox::showYesNoBox(AlertWindow::AlertIconType iconType, const String& title,
                                                     const String& message, Component* associatedComponent,
                                                     ModalComponentManager::Callback* callback)
    {
        return AlertWindow::showOkCancelBox(iconType, title, message, TRANS("Yes"), TRANS("No"), associatedComponent,
                                            callback);
    }
    ...
    void* CustomMouseCursorInfo::create() const
    {
...
#if JUCE_USE_XCURSOR
        {
            ...
            if (!hasBeenLoaded)
            {
                ...
                if (void* h = dlopen("libXcursor.so.1", RTLD_GLOBAL | RTLD_NOW))
                {
                    ...
                    if (xcursorSupportsARGB == nullptr || xcursorImageCreate == nullptr || xcursorImageLoadCursor == nullptr
                        || xcursorImageDestroy == nullptr || !xcursorSupportsARGB(display))
            ...
            if (xcursorSupportsARGB != nullptr)
            {
                if (XcursorImage* xcImage = xcursorImageCreate((int)imageW, (int)imageH))
                {
                    ...
                    for (int y = 0; y < (int)imageH; ++y)
                        for (int x = 0; x < (int)imageW; ++x) *dest++ = image.getPixelAt(x, y).getARGB();
        ...
        if (!XQueryBestCursor(display, root, imageW, imageH, &cursorW, &cursorH)) return nullptr;
        ...
        Pixmap sourcePixmap =
            XCreatePixmapFromBitmapData(display, root, sourcePlane.getData(), cursorW, cursorH, 0xffff, 0, 1);
        ...
        void* result = (void*)XCreatePixmapCursor(display, sourcePixmap, maskPixmap, &white, &black, (unsigned int)hotspotX,
    ...
    void* MouseCursor::createStandardMouseCursor(MouseCursor::StandardCursorType type)
    {
        ...
        if (display == nullptr) return None;
        ...
        case CopyingCursor:
        {
            static unsigned char copyCursorData[] = {
                71,  73,  70,  56,  57,  97,  21,  0,  21,  0,   145, 0,   0,   0,   0,   0,   255, 255, 255, 0,   128,
                128, 255, 255, 255, 33,  249, 4,   1,  0,   0,   3,   0,   44,  0,   0,   0,   0,   21,  0,   21,  0,
                0,   2,   72,  4,   134, 169, 171, 16, 199, 98,  11,  79,  90,  71,  161, 93,  56,  111, 78,  133, 218,
                215, 137, 31,  82,  154, 100, 200, 86, 91,  202, 142, 12,  108, 212, 87,  235, 174, 15,  54,  214, 126,
                237, 226, 37,  96,  59,  141, 16,  37, 18,  201, 142, 157, 230, 204, 51,  112, 252, 114, 147, 74,  83,
    ...
    {
        if (auto* lp = dynamic_cast<LinuxComponentPeer*>(peer)) lp->showMouseCursor((Cursor)getHandle());
    }

    void MouseCursor::showInAllWindows() const
    {
        for (int i = ComponentPeer::getNumPeers(); --i >= 0;) showInWindow(ComponentPeer::getPeer(i));
    }
    ...
    static LinuxComponentPeer* getPeerForDragEvent(Component* sourceComp)
    {
        ...
        if (sourceComp != nullptr)
            if (auto* lp = dynamic_cast<LinuxComponentPeer*>(sourceComp->getPeer())) return lp;
                                                              ...
                                                              Component* sourceComp)
    {
        ...
        if (auto* lp = getPeerForDragEvent(sourceComp)) return lp->externalDragFileInit(files, canMoveFiles);
    ...
    bool DragAndDropContainer::performExternalDragDropOfText(const String& text, Component* sourceComp)
    {
        ...
        if (auto* lp = getPeerForDragEvent(sourceComp)) return lp->externalDragTextInit(text);
