...
namespace juce
{
    ...
    {
        ChildProcess child;
        const bool   ok = child.start("which " + String(executable)) && child.readAllProcessOutput().trim().isNotEmpty();

        child.waitForProcessToFinish(60 * 1000);
        return ok;
    }

    class FileChooser::Native : public FileChooser::Pimpl, private Timer
    {
    public:
        ...
        void runModally() override
        {
            ...
            while (child.isRunning())
                if (!MessageManager::getInstance()->runDispatchLoopUntil(20)) break;
    ...
    private:
        ...
        void finish(bool shouldKill)
        {
            ...
            if (result.isNotEmpty())
            {
                ...
                for (auto& token : tokens) selection.add(URL(File::getCurrentWorkingDirectory().getChildFile(token)));
            }
        ...
        {
            return SystemStats::getEnvironmentVariable("KDE_FULL_SESSION", String()).equalsIgnoreCase("true");
        }
        ...
        void addKDialogArgs()
        {
            ...
            if (owner.title.isNotEmpty()) args.add("--title=" + owner.title);
            ...
            else
            {
                ...
                if (isSave) startPath = startPath.getChildFile(owner.startingFile.getFileName());
            }
        ...
        void addZenityArgs()
        {
            ...
            if (owner.filters.isNotEmpty() && owner.filters != "*" && owner.filters != "*.*")
            {
                ...
                for (int i = 0; i < tokens.size(); ++i) args.add("--file-filter=" + tokens[i]);
            }
            ...
            if (uint64 topWindowID = getTopWindowID()) setenv("WINDOWID", String(topWindowID).toRawUTF8(), true);
        }
