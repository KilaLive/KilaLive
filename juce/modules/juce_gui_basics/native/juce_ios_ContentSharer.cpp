...
{
    template <>
    struct ContainerDeletePolicy<UIActivityViewController>
    {
    ...
    template <>
    struct ContainerDeletePolicy<NSObject<UIPopoverPresentationControllerDelegate>>
    {
        static void destroy(NSObject* o)
        {
            [o release];
    ...
    //==============================================================================
    class ContentSharer::ContentSharerNativeImpl : public ContentSharer::Pimpl, private Component
    {
    public:
        ...
        void shareFiles(const Array<URL>& files) override
        {
            ...
            for (const auto& f : files)
            {
                ...
                else
                {
                    ...
                    auto fileName = juceStringToNS(
                        filePath.fromLastOccurrenceOf("/", false, false).upToLastOccurrenceOf(".", false, false));

                    auto fileExt = juceStringToNS(filePath.fromLastOccurrenceOf(".", false, false));

                    if ([fileDirectory length] == NSUInteger(0))
                        nativeFilePath = [[NSBundle mainBundle] pathForResource:fileName ofType:fileExt];
                    else
                        nativeFilePath =
                            [[NSBundle mainBundle] pathForResource:fileName ofType:fileExt inDirectory:fileDirectory];
                }

                if (nativeFilePath != nil) [urls addObject:[NSURL fileURLWithPath:nativeFilePath]];
            }
    ...
    private:
        void share(NSArray* items)
        {
            ...
            controller.get().completionWithItemsHandler =
                ^(UIActivityType type, BOOL completed, NSArray* returnedItems, NSError* error) {
                  ignoreUnused(type);
                  ignoreUnused(returnedItems);

                  succeeded = completed;

                  if (error != nil) errorDescription = nsStringToJuce([error localizedDescription]);
            ...
            enterModalState(
                true, ModalCallbackFunction::create([this](int) { owner.sharingFinished(succeeded, errorDescription); }),
                false);
        }
            ...
            if (peer != newPeer)
            {
                ...
                if (isIPad())
                {
                    ...
                    popoverController.sourceRect =
                        CGRectMake(0.f, screenBounds.size.height - 10.f, screenBounds.size.width, 10.f);
                ...
                if (peer->view.window != nil) peer->view.window.autoresizesSubviews = YES;
            }
        }
