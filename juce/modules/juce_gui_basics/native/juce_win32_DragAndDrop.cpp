...
namespace juce
{
    namespace DragAndDropHelpers
    {
        ...
        struct JuceDropSource : public ComBaseClassHelper<IDropSource>
        {
            ...
            JUCE_COMRESULT QueryContinueDrag(BOOL escapePressed, DWORD keys) override
            {
                ...
                if ((keys & (MK_LBUTTON | MK_RBUTTON)) == 0) return DRAGDROP_S_DROP;
        ...
        {
            JuceEnumFormatEtc(const FORMATETC* f)
                : format(f)
            {
            }
            ...
            {
                if (index + (int)celt >= 1) return S_FALSE;
        ...
        public:
            ...
            {
                if ((pFormatEtc->tymed & format->tymed) != 0 && pFormatEtc->cfFormat == format->cfFormat
            ...
            {
                if (f == 0) return E_INVALIDARG;

                if (f->tymed == format->tymed && f->cfFormat == format->cfFormat && f->dwAspect == format->dwAspect)
                    return S_OK;
    ...
    bool DragAndDropContainer::performExternalDragDropOfFiles(const StringArray& files, const bool canMove, Component*)
    {
        ...
        return DragAndDropHelpers::performDragDrop(
            &format, &medium, canMove ? (DWORD)(DROPEFFECT_COPY | DROPEFFECT_MOVE) : (DWORD)DROPEFFECT_COPY);
    }
