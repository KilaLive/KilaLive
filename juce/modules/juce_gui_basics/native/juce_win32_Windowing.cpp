...
#define GET_POINTERID_WPARAM(wParam) (LOWORD(wParam))
    ...
    static void checkForPointerAPI()
    {
        ...
        canUsePointerAPI =
            (getPointerTypeFunction != nullptr && getPointerTouchInfo != nullptr && getPointerPenInfo != nullptr);
    }
            ...
            if (setProcessDPIAwareness == nullptr)
            {
                ...
                if (shcoreModule != 0)
                {
                    setProcessDPIAwareness =
                        (SetProcessDPIAwarenessFunc)GetProcAddress(shcoreModule, "SetProcessDpiAwareness");
                    ...
                    if (setProcessDPIAwareness != nullptr
                        ...
                        && SUCCEEDED(
                               setProcessDPIAwareness(Process_System_DPI_Aware)))  // (keep using this mode temporarily..)
                        return;
                }
                ...
                if (setProcessDPIAware == nullptr)
                {
                    ...
                    if (setProcessDPIAware != nullptr) setProcessDPIAware();
                }
            }
        }
#endif
    ...
    {
        return JUCEApplicationBase::isStandaloneApp() ? getGlobalDPI() / 96.0 : 1.0;
    }
    ...
    public:
            ...
            : ImagePixelData(format, w, h)
        {
            ...
            hBitmap = CreateDIBSection(hdc, (BITMAPINFO*)&(bitmapInfo), DIB_RGB_COLORS, (void**)&bitmapData, 0, 0);

            previousBitmap = SelectObject(hdc, hBitmap);

            if (format == Image::ARGB && clearImage) zeromem(bitmapData, (size_t)std::abs(h * lineStride));
        ...
        ~WindowsBitmapImage()
        {
            SelectObject(
                hdc,
                previousBitmap);  // Selecting the previous bitmap before deleting the DC avoids a warning in BoundsChecker
            DeleteDC(hdc);
        ...
        void blitToWindow(HWND hwnd, HDC dc, bool transparent, int x, int y, uint8 updateLayeredWindowAlpha) noexcept
        {
            ...
            if (transparent)
            {
                ...
                SIZE  size = {windowBounds.right - windowBounds.left, windowBounds.bottom - windowBounds.top};
            ...
            {
                StretchDIBits(dc, x, y, width, height, 0, 0, width, height, bitmapData, (const BITMAPINFO*)&bitmapInfo,
    ...
    namespace IconConverters
    {
        Image createImageFromHICON(HICON icon)
        {
            ...
            struct ScopedICONINFO : public ICONINFO
            {
                ...
                ~ScopedICONINFO()
                {
                    if (hbmColor != nullptr) ::DeleteObject(hbmColor);
            ...
            if (!SUCCEEDED(::GetIconInfo(icon, &info))) return {};

            BITMAP bm;

            if (!(::GetObject(info.hbmColor, sizeof(BITMAP), &bm) && bm.bmWidth > 0 && bm.bmHeight > 0)) return {};
            ...
            if (auto* tempDC = ::GetDC(nullptr))
            {
                if (auto* dc = ::CreateCompatibleDC(tempDC))
                {
                    ...
                    if (auto* dib = ::CreateDIBSection(tempDC, (BITMAPINFO*)&header, DIB_RGB_COLORS, (void**)&bitmapImageData,
                                                       nullptr, 0))
                    {
                        ...
                        for (int i = 0; i < numPixels; ++i) opacityMask[i] = (bitmapImageData[i] == 0);
                        ...
                        auto imageHasAlphaChannel = [&imageData, numPixels]() {
                            for (int i = 0; i < numPixels; ++i)
                                if (imageData.data[i * 4] != 0) return true;

                            return false;
                        };

                        if (!imageHasAlphaChannel())
                            for (int i = 0; i < numPixels; ++i) imageData.data[i * 4] = opacityMask[i] ? 0xff : 0x00;
    ...
    //==============================================================================
    JUCE_COMCLASS(ITipInvocation, "37c994e7-432b-4834-a2f7-dce1f13b834b")
        : public IUnknown
    {
        static CLSID getCLSID() noexcept
        {
    ...
    struct OnScreenKeyboard : public DeletedAtShutdown, private Timer
    {
        void activate()
        {
            shouldBeActive = true;
    ...
    JUCE_COMCLASS(IUIViewSettingsInterop, "3694dbf9-8f68-44be-8ff5-195c98ede8a6")
        : public IInspectable
    {
        virtual HRESULT STDMETHODCALLTYPE GetForWindow(HWND, REFIID, void**) = 0;
    };

    JUCE_COMCLASS(IUIViewSettings, "c63657f6-8850-470d-88f8-455e16ea2c26")
        : public IInspectable
    {
    ...
    struct UWPUIViewSettings
    {
        UWPUIViewSettings()
        {
            ...
            if (dll.h != nullptr)
            {
                ...
                if (roInitialize == nullptr || roGetActivationFactory == nullptr || createHString == nullptr
                    || deleteHString == nullptr)
                    return;

                auto status = roInitialize(1);

                if (status != S_OK && status != S_FALSE && (unsigned)status != 0x80010106L) return;

                LPCWSTR uwpClassName = L"Windows.UI.ViewManagement.UIViewSettings";
                HSTRING uwpClassId;

                if (createHString(uwpClassName, (::UINT32)wcslen(uwpClassName), &uwpClassId) != S_OK || uwpClassId == nullptr)
                    return;
                ...
                if (status != S_OK || viewSettingsInterop == nullptr) return;
        ...
        {
            if (viewSettingsInterop == nullptr) return false;

            ComSmartPtr<IUIViewSettings> viewSettings;

            if (viewSettingsInterop->GetForWindow(hWnd, __uuidof(IUIViewSettings),
                                                  (void**)viewSettings.resetAndGetPointerAddress())
                    == S_OK
                && viewSettings != nullptr)
            {
                IUIViewSettings::UserInteractionMode mode;

                if (viewSettings->GetUserInteractionMode(&mode) == S_OK) return mode == IUIViewSettings::Touch;
            }
    ...
    public:
            ...
            , currentRenderingEngine(softwareRenderingEngine)
        {
                ...
                && ((!hasTitleBar()) || SystemStats::getOperatingSystemType() < SystemStats::WinVista))
            {
                ...
                if (shadower != nullptr) shadower->setOwner(&component);
            }
        ...
        ~HWNDComponentPeer()
        {
            ...
            if (currentWindowIcon != 0) DestroyIcon(currentWindowIcon);
        ...
        void updateBorderSize()
        {
            ...
            if (GetWindowInfo(hwnd, &info))
                windowBorder =
                    BorderSize<int>(info.rcClient.top - info.rcWindow.top, info.rcClient.left - info.rcWindow.left,
                                    info.rcWindow.bottom - info.rcClient.bottom, info.rcWindow.right - info.rcClient.right);

#if JUCE_DIRECT2D
            if (direct2DContext != nullptr) direct2DContext->resized();
#endif
        }
        ...
        void setBounds(const Rectangle<int>& bounds, bool isNowFullScreen) override
        {
            ...
            const bool hasResized =
                (oldBounds.getWidth() != bounds.getWidth() || oldBounds.getHeight() != bounds.getHeight());
        ...
        Point<int> getScreenPosition() const
        {
            ...
            return {r.left + windowBorder.getLeft(), r.top + windowBorder.getTop()};
        }
        ...
        {
            if (shouldBeMinimised != isMinimised()) ShowWindow(hwnd, shouldBeMinimised ? SW_MINIMIZE : SW_SHOWNORMAL);
        }
        ...
        void setFullScreen(bool shouldBeFullScreen) override
        {
            ...
            {
                if (constrainer != nullptr) constrainer->resizeStart();
                ...
                if (!fullScreen)
                {
                    ...
                    if (hasTitleBar()) ShowWindow(hwnd, SW_SHOWNORMAL);
                ...
                if (deletionChecker != nullptr) handleMovedOrResized();

                if (constrainer != nullptr) constrainer->resizeEnd();
            }
        }

        bool isFullScreen() const override
        {
            if (!hasTitleBar()) return fullScreen;
        ...
        bool contains(Point<int> localPos, bool trueIfInAChildWindow) const override
        {
            ...
            if (!(isPositiveAndBelow(localPos.x, r.right - r.left) && isPositiveAndBelow(localPos.y, r.bottom - r.top)))
                return false;

            POINT p = {localPos.x + r.left + windowBorder.getLeft(), localPos.y + r.top + windowBorder.getTop()};
        ...
        bool setAlwaysOnTop(bool alwaysOnTop) override
        {
            ...
            if (shadower != nullptr) handleBroughtToFront();
        ...
        void textInputRequired(Point<int>, TextInputTarget&) override
        {
            ...
            if (uwpViewSettings.isTabletModeActivatedForWindow(hwnd)) OnScreenKeyboard::getInstance()->activate();
        }
        ...
        void dismissPendingTextInput() override
        {
            ...
            if (uwpViewSettings.isTabletModeActivatedForWindow(hwnd)) OnScreenKeyboard::getInstance()->deactivate();
        }
        ...
        {
            if (h != 0 && JuceWindowIdentifier::isJUCEWindow(h)) return (HWNDComponentPeer*)GetWindowLongPtr(h, 8);
        ...
        {
            FileDropTarget(HWNDComponentPeer& p)
                : peer(p)
            {
            }
            ...
            {
                if (peerIsDeleted) return S_FALSE;
            ...
            JUCE_COMRESULT DragOver(DWORD /*grfKeyState*/, POINTL mousePos, DWORD* pdwEffect) override
            {
                ...
                *pdwEffect        = peer.handleDragMove(dragInfo) ? (DWORD)DROPEFFECT_COPY : (DWORD)DROPEFFECT_NONE;
                return S_OK;
            }
            ...
            JUCE_COMRESULT Drop(IDataObject* pDataObject, DWORD /*grfKeyState*/, POINTL mousePos, DWORD* pdwEffect) override
            {
                ...
                dragInfo.position = getMousePos(mousePos).roundToInt();
                *pdwEffect        = peer.handleDragDrop(dragInfo) ? (DWORD)DROPEFFECT_COPY : (DWORD)DROPEFFECT_NONE;
                return S_OK;
            }
        ...
        private:
            Point<float> getMousePos(POINTL mousePos) const
            {
                ...
                return comp.getLocalPoint(nullptr,
                                          ScalingHelpers::unscaledScreenPosToScaled(
                                              comp.getDesktopScaleFactor(),
                                              Point<float>(static_cast<float>(mousePos.x), static_cast<float>(mousePos.y))));
            }
            ...
            void parseFileList(const CharType* names, const SIZE_T totalLen)
            {
                ...
                {
                    unsigned int len = 0;
                    while (i + len < totalLen && names[i + len] != 0) ++len;
            ...
            HRESULT updateFileList(IDataObject* const dataObject)
            {
                ...
                {
                    dragInfo.text =
                        String(CharPointer_UTF16((const WCHAR*)textData.data),
            ...
            if (m.message == WM_KEYDOWN || m.message == WM_KEYUP)
                if (Component::getCurrentlyFocusedComponent() != nullptr)
                    if (auto* h = getOwnerOfWindow(m.hwnd))
                        return m.message == WM_KEYDOWN ? h->doKeyDown(m.wParam) : h->doKeyUp(m.wParam);
    ...
    private:
        ...
        bool      fullScreen = false, isDragging = false, isMouseOver = false, hasCreatedCaret = false,
        ...
        private:
            ...
            static bool isHWNDBlockedByModalComponents(HWND h)
            {
                for (int i = Desktop::getInstance().getNumComponents(); --i >= 0;)
                    if (auto* c = Desktop::getInstance().getComponent(i))
                        if ((!c->isCurrentlyBlockedByAnotherModalComponent()) && IsChild((HWND)c->getWindowHandle(), h))
                            return false;
            ...
            static bool checkEventBlockedByModalComps(const MSG& m)
            {
                ...
                case WM_POINTERDOWN:
                    ...
                    {
                        if (auto* modal = Component::getCurrentlyModalComponent(0)) modal->inputAttemptWhenModal();
        ...
        void createWindow()
        {
            ...
            if (hasTitleBar())
            {
                ...
                if ((styleFlags & windowIsResizable) != 0) type |= WS_THICKFRAME;
            }
            ...
            hwnd = CreateWindowEx(exstyle, WindowClassHolder::getInstance()->getWindowClassName(), L"", type, 0, 0, 0, 0,
                                  parentToAddTo, 0, (HINSTANCE)Process::getCurrentModuleInstanceHandle(), 0);
            ...
            if (hwnd != 0)
            {
                ...
                if (dropTarget == nullptr)
                {
                    ...
                    if (dontRepaint) peer = getOwnerOfWindow(parentToAddTo);
                ...
                if (canUseMultiTouch()) registerTouchWindow(hwnd, 0);
                ...
                if (!JUCEApplication::isStandaloneApp()) settingChangeCallback = forceDisplayUpdate;
        ...
        static BOOL CALLBACK clipChildWindowCallback(HWND hwnd, LPARAM context)
        {
            if (IsWindowVisible(hwnd))
            {
                ...
                if (parent == info.peer->hwnd)
                {
                    ...
                    Rectangle<int> clip(pos.x, pos.y, r.right - r.left, r.bottom - r.top);

                    info.clip->subtract(clip - info.origin);

                    if (info.savedDC == 0) info.savedDC = SaveDC(info.dc);
            ...
            if (w > 0 && h > 0)
            {
                ...
                if (regionType == COMPLEXREGION && !transparent)
                {
                    ...
                    if (res > 0 && res <= sizeof(rgnData))
                    {
                        ...
                        if (hdr->iType == RDH_RECTANGLES && hdr->rcBound.right - hdr->rcBound.left >= w
                            && hdr->rcBound.bottom - hdr->rcBound.top >= h)
                        {
                            ...
                            for (int i = (int)((RGNDATA*)rgnData)->rdh.nCount; --i >= 0;)
                            {
                                if (rects->right <= x + w && rects->bottom <= y + h)
                                {
                                    ...
                                    contextClip.addWithoutMerging(
                                        Rectangle<int>(cx - x, rects->top - y, rects->right - cx, rects->bottom - rects->top)
                ...
                {
                    if (transparent)
                        for (auto& i : contextClip) offscreenImage.clear(i);

                    {
                        ScopedPointer<LowLevelGraphicsContext> context(component.getLookAndFeel().createGraphicsContext(
                            offscreenImage, Point<int>(-x, -y), contextClip));
                        handlePaint(*context);
                    }
                ...
                if (childClipInfo.savedDC != 0) RestoreDC(dc, childClipInfo.savedDC);
            }
        }

        //==============================================================================
        void doMouseEvent(Point<float> position, float pressure, float orientation = 0.0f,
                          ModifierKeys mods = currentModifiers)
        {
            handleMouseEvent(MouseInputSource::InputSourceType::mouse, position, mods, pressure, orientation,
                             getMouseEventTime());
        }
...
#if JUCE_DIRECT2D
            if (SystemStats::getOperatingSystemType() >= SystemStats::Windows7) s.add("Direct2D");
#endif
        ...
        {
            if (SystemStats::getOperatingSystemType() >= SystemStats::WinVista) return 0;
        ...
        {
            if (registerTouchWindow == nullptr) return false;
        ...
        static bool areOtherTouchSourcesActive()
        {
            for (auto& ms : Desktop::getInstance().getMouseSources())
                if (ms.isDragging()
                    && (ms.getType() == MouseInputSource::InputSourceType::touch
        ...
        void doMouseMove(Point<float> position, bool isMouseDownEvent)
        {
            ...
            if (isTouchEvent() || areOtherTouchSourcesActive()) return;
            ...
            if (!isMouseOver)
            {
                ...
                if (isMouseDownEvent) ModifierKeys::getCurrentModifiersRealtime();

                updateKeyModifiers();

#if JUCE_MODULE_AVAILABLE_juce_audio_plugin_client
                if (modProvider != nullptr) currentModifiers = currentModifiers.withFlags(modProvider->getWin32Modifiers());
#endif
                ...
                if (!TrackMouseEvent(&tme)) jassertfalse;

                Desktop::getInstance().getMainMouseSource().forceMouseCursorUpdate();
            }
            else if (!isDragging)
            {
                if (!contains(position.roundToInt(), false)) return;
            }
            ...
            if (!Desktop::getInstance().getMainMouseSource().isDragging()) modsToSend = modsToSend.withoutMouseButtons();

            if (now >= lastMouseTime + minTimeBetweenMouses)
            {
                lastMouseTime = now;
                doMouseEvent(position, MouseInputSource::invalidPressure, MouseInputSource::invalidOrientation, modsToSend);
            }
        }
        ...
        void doMouseUp(Point<float> position, const WPARAM wParam)
        {
            ...
            if ((wParam & (MK_LBUTTON | MK_RBUTTON | MK_MBUTTON)) == 0 && hwnd == GetCapture()) ReleaseCapture();
            ...
            if (wasDragging) doMouseEvent(position, MouseInputSource::invalidPressure);
        }
        ...
        void doCaptureChanged()
        {
            ...
            if (isDragging) doMouseUp(getCurrentMousePos(), (WPARAM)0);
        }
        ...
        void doMouseExit()
        {
            ...
            if (!areOtherTouchSourcesActive()) doMouseEvent(getCurrentMousePos(), MouseInputSource::invalidPressure);
        }
        ...
        static MouseInputSource::InputSourceType getPointerType(WPARAM wParam)
        {
            if (getPointerTypeFunction != nullptr)
            {
                ...
                {
                    if (pointerType == 2) return MouseInputSource::InputSourceType::touch;

                    if (pointerType == 3) return MouseInputSource::InputSourceType::pen;
                }
            }
                ...
                if (auto* peer = findPeerUnderMouse(localPos))
                {
                    ...
                    case 3: /*GID_ZOOM*/
                        if (gi.dwFlags != 1 /*GF_BEGIN*/ && lastMagnifySize > 0)
                            peer->handleMagnifyGesture(MouseInputSource::InputSourceType::touch, localPos,
                                                       getMouseEventTime(),
        ...
        LRESULT doTouchEvent(const int numInputs, HTOUCHINPUT eventHandle)
        {
            if ((styleFlags & windowIgnoresMouseClicks) != 0)
                if (auto* parent = getOwnerOfWindow(GetParent(hwnd)))
                    if (parent != this) return parent->doTouchEvent(numInputs, eventHandle);
        ...
        bool handleTouchInput(const TOUCHINPUT& touch, const bool isDown, const bool isUp,
                              const float touchPressure = MouseInputSource::invalidPressure, const float orientation = 0.0f)
        {
            auto isCancel = false;
            ...
            if (isDown)
            {
                ...
                handleMouseEvent(MouseInputSource::InputSourceType::touch, pos, modsToSend.withoutMouseButtons(), pressure,
                                 orientation, time, {}, touchIndex);
            ...
            else if (isUp)
            {
                ...
                if (!currentTouches.areAnyTouchesActive()) isCancel = true;
            }
            else
            {
            ...
            handleMouseEvent(MouseInputSource::InputSourceType::touch, pos, modsToSend, pressure, orientation, time, {},
                             touchIndex);

            if (!isValidPeer(this)) return false;

            if (isUp)
            {
                handleMouseEvent(MouseInputSource::InputSourceType::touch, {-10.0f, -10.0f},
                                 currentModifiers.withoutMouseButtons(), pressure, orientation, time, {}, touchIndex);
        ...
        {
            if (!canUsePointerAPI) return false;
            ...
            if (pointerType == MouseInputSource::InputSourceType::touch)
            {
                ...
                if (!getPointerTouchInfo(GET_POINTERID_WPARAM(wParam), &touchInfo)) return false;

                const auto pressure =
                    touchInfo.touchMask & TOUCH_MASK_PRESSURE ? touchInfo.pressure : MouseInputSource::invalidPressure;
                const auto orientation = touchInfo.touchMask & TOUCH_MASK_ORIENTATION
                                             ? degreesToRadians(static_cast<float>(touchInfo.orientation))
                                             : MouseInputSource::invalidOrientation;

                if (!handleTouchInput(emulateTouchEventFromPointer(lParam, wParam), isDown, isUp, pressure, orientation))
                    return false;
            }
            else if (pointerType == MouseInputSource::InputSourceType::pen)
            {
                ...
                const auto pressure =
                    (penInfo.penMask & PEN_MASK_PRESSURE) ? penInfo.pressure / 1024.0f : MouseInputSource::invalidPressure;

                if (!handlePenInput(penInfo,
                                    globalToLocal(Point<float>(static_cast<float>(GET_X_LPARAM(lParam)),
        ...
        bool handlePenInput(POINTER_PEN_INFO penInfo, Point<float> pos, const float pressure, bool isDown, bool isUp)
        {
            ...
            penDetails.rotation = (penInfo.penMask & PEN_MASK_ROTATION)
                                      ? degreesToRadians(static_cast<float>(penInfo.rotation))
                                      : MouseInputSource::invalidRotation;
            ...
            if (isDown)
            {
                ...
                // this forces a mouse-enter/up event, in case for some reason we didn't get a mouse-up before.
                handleMouseEvent(MouseInputSource::InputSourceType::pen, pos, modsToSend.withoutMouseButtons(), pressure,
            ...
            if (isUp)
            {
                handleMouseEvent(MouseInputSource::InputSourceType::pen, {-10.0f, -10.0f}, currentModifiers, pressure,
                                 MouseInputSource::invalidOrientation, time, penDetails);
        ...
        bool doKeyUp(const WPARAM key)
        {
            ...
            return handleKeyUpOrDown(false) || Component::getCurrentlyModalComponent() != nullptr;
        }
            ...
            default:
...

                {
                    ...
                    if (!PeekMessage(&msg, hwnd, WM_CHAR, WM_DEADCHAR, PM_NOREMOVE))
                    {
                        ...
                        WCHAR text[16] = {0};
                        if (ToUnicode((UINT)key, scanCode, keyState, text, 8, 0) != 1) text[0] = 0;
        ...
        {
            if (HWND parentH = GetParent(hwnd)) PostMessage(parentH, message, wParam, lParam);
        }
        ...
        Rectangle<int> getCurrentScaledBounds(float scale) const
        {
            return ScalingHelpers::unscaledScreenPosToScaled(
                scale, windowBorder.addedTo(ScalingHelpers::scaledScreenPosToUnscaled(scale, component.getBounds())));
        }
        ...
        LRESULT handleSizeConstraining(RECT& r, const WPARAM wParam)
        {
            if (isConstrainedNativeWindow())
            {
                ...
                constrainer->checkBounds(pos, current, Desktop::getInstance().getDisplays().getTotalBounds(true),
        ...
        bool handlePositionChanged()
        {
            ...
            {
                if (!areOtherTouchSourcesActive()) doMouseEvent(pos, MouseInputSource::invalidPressure);

                if (!isValidPeer(this)) return true;
            }
        ...
        void doSettingChange()
        {
            ...
            {
                auto& display =
                    Desktop::getInstance().getDisplays().getDisplayContaining(component.getScreenBounds().getCentre());
    ...
    private:
        static void* callFunctionIfNotLocked(MessageCallbackFunction* callback, void* userData)
        {
            ...
            if (mm.currentThreadHasLockedMessageManager()) return callback(userData);
        ...
        LRESULT peerWindowProc(HWND h, UINT message, WPARAM wParam, LPARAM lParam)
        {
            ...
            //==============================================================================
            case WM_NCHITTEST:
                if ((styleFlags & windowIgnoresMouseClicks) != 0) return HTTRANSPARENT;

                if (!hasTitleBar()) return HTCLIENT;
            ...
            case WM_NCPAINT:
                ...
                if (hasTitleBar()) break;  // let the DefWindowProc handle drawing the frame.

                return 0;

            case WM_ERASEBKGND:
            case WM_NCCALCSIZE:
                if (hasTitleBar()) break;
            ...
            case WM_POINTERDOWN:
                if (handlePointerInput(wParam, lParam, true, false)) return 0;
                break;
            ...
            //==============================================================================
            case WM_MOUSEMOVE:
                doMouseMove(getPointFromLParam(lParam), false);
                return 0;
            ...
            case WM_TOUCH:
                if (getTouchInputInfo != nullptr) return doTouchEvent((int)wParam, (HTOUCHINPUT)lParam);
            ...
            case WM_KEYUP:
            case WM_SYSKEYUP:
                if (doKeyUp(wParam)) return 0;

                forwardMessageToParent(message, wParam, lParam);
                break;

            case WM_CHAR:
                if (doKeyChar((int)wParam, lParam)) return 0;

                forwardMessageToParent(message, wParam, lParam);
                break;

            case WM_APPCOMMAND:
                if (doAppCommand(lParam)) return TRUE;
            ...
            case WM_POINTERACTIVATE:
            case WM_MOUSEACTIVATE:
                if (!component.getMouseClickGrabsKeyboardFocus()) return MA_NOACTIVATE;
            ...
            case WM_CLOSE:
                if (!component.isCurrentlyBlockedByAnotherModalComponent()) handleUserClosingWindow();
            ...
            case WM_SYSCOMMAND:
                ...
                {
                case SC_CLOSE:
                    if (sendInputAttemptWhenModalMessage()) return 0;
                ...
                case SC_KEYMENU:
                    ...
                    if (hasTitleBar() && h == GetCapture()) ReleaseCapture();

                    break;

                case SC_MAXIMIZE:
                    if (!sendInputAttemptWhenModalMessage()) setFullScreen(true);
        ...
        bool sendInputAttemptWhenModalMessage()
        {
            ...
            {
                if (Component* const current = Component::getCurrentlyModalComponent()) current->inputAttemptWhenModal();
        ...
        struct IMEHandler
        {
            ...
            void handleStartComposition(ComponentPeer& owner)
            {
                ...
                if (auto* target = owner.findCurrentTextInputTarget()) target->insertTextAtCaret(String());
            }
        ...
        private:
            ...
            int getCompositionCaretPos(HIMC hImc, LPARAM lParam, const String& currentIMEString) const
            {
                ...
                if ((lParam & CS_NOMOVECARET) != 0) return compositionRange.getStart();
                ...
                if ((lParam & GCS_COMPATTR) != 0)
                {
                    ...
                    if (attributeSizeBytes > 0)
                    {
                        ...
                        for (selectionStart = 0; selectionStart < attributeSizeBytes; ++selectionStart)
                            if (attributes[selectionStart] == ATTR_TARGET_CONVERTED
                                || attributes[selectionStart] == ATTR_TARGET_NOTCONVERTED)
                                break;

                        for (selectionEnd = selectionStart; selectionEnd < attributeSizeBytes; ++selectionEnd)
                            if (attributes[selectionEnd] != ATTR_TARGET_CONVERTED
                                && attributes[selectionEnd] != ATTR_TARGET_NOTCONVERTED)
                                break;
                    }
                }
            ...
            {
                if (compositionInProgress) target->setHighlightedRegion(compositionRange);

                target->insertTextAtCaret(newContent);
                compositionRange.setLength(newContent.length());

                if (newSelection.getStart() < 0) newSelection = Range<int>::emptyRange(compositionRange.getEnd());
                    ...
                    if (clauseDataSizeBytes > 0)
                    {
                        ...
                        if (ImmGetCompositionString(hImc, GCS_COMPCLAUSE, clauseData, (DWORD)clauseDataSizeBytes) > 0)
                            for (size_t i = 0; i + 1 < numItems; ++i)
                                result.add(Range<int>((int)clauseData[i], (int)clauseData[i + 1])
                                           + compositionRange.getStart());
                    }
                }
    ...
    {
        return new HWNDComponentPeer(component, ComponentPeer::windowIgnoresMouseClicks, (HWND)parentHWND, true);
    }
    ...
    ModifierKeys ModifierKeys::getCurrentModifiersRealtime() noexcept
    {
        ...
        HWNDComponentPeer::currentModifiers = HWNDComponentPeer::currentModifiers.withoutMouseButtons().withFlags(mouseMods);
    ...
    bool KeyPress::isKeyCurrentlyDown(const int keyCode)
    {
        ...
        {
            if (k >= (SHORT)'a' && k <= (SHORT)'z') k += (SHORT)'A' - (SHORT)'a';

            // Only translate if extendedKeyModifier flag is not set
            const SHORT translatedValues[] = {(SHORT)',',   VK_OEM_COMMA, (SHORT)'+',    VK_OEM_PLUS, (SHORT)'-',
                                              VK_OEM_MINUS, (SHORT)'.',   VK_OEM_PERIOD, (SHORT)';',  VK_OEM_1,
                                              (SHORT)':',   VK_OEM_1,     (SHORT)'/',    VK_OEM_2,    (SHORT)'?',
                                              VK_OEM_2,     (SHORT)'[',   VK_OEM_4,      (SHORT)']',  VK_OEM_6};

            for (int i = 0; i < numElementsInArray(translatedValues); i += 2)
                if (k == translatedValues[i]) k = translatedValues[i + 1];
        }
    ...
    static BOOL CALLBACK enumAlwaysOnTopWindows(HWND hwnd, LPARAM lParam)
    {
        if (IsWindowVisible(hwnd))
        {
            ...
            if (processID == GetCurrentProcessId())
            {
                ...
                if (GetWindowInfo(hwnd, &info) && (info.dwExStyle & WS_EX_TOPMOST) != 0)
                {
    ...
    {
    public:
        WindowsMessageBox(AlertWindow::AlertIconType iconType, const String& boxTitle, const String& m,
                          Component* associatedComponent, UINT extraFlags, ModalComponentManager::Callback* cb,
                          const bool runAsync)
            ...
            , callback(cb)
        {
            if (runAsync) triggerAsyncUpdate();
        }
        ...
        void handleAsyncUpdate() override
        {
            ...
            if (callback != nullptr) callback->modalStateFinished(result);
    ...
    bool JUCE_CALLTYPE NativeMessageBox::showOkCancelBox(AlertWindow::AlertIconType iconType, const String& title,
                                                         ...
                                                         ModalComponentManager::Callback* callback)
    {
        ScopedPointer<WindowsMessageBox> mb(
            new WindowsMessageBox(iconType, title, message, associatedComponent, MB_OKCANCEL, callback, callback != nullptr));
        if (callback == nullptr) return mb->getResult() != 0;
                                                           ...
                                                           ModalComponentManager::Callback* callback)
    {
        ...
        if (callback == nullptr) return mb->getResult();
    ...
    {
        ScopedPointer<WindowsMessageBox> mb(
            new WindowsMessageBox(iconType, title, message, associatedComponent, MB_YESNO, callback, callback != nullptr));
        if (callback == nullptr) return mb->getResult();
    ...
    bool MouseInputSource::SourceList::addSource()
    {
        ...
        {
            addSource(numSources,
                      numSources == 0 ? MouseInputSource::InputSourceType::mouse : MouseInputSource::InputSourceType::touch);
            return true;
        }
    ...
    {
        SetCursorPos(roundToInt(newPosition.x), roundToInt(newPosition.y));
    }
    ...
    {
        if (auto* tlw = dynamic_cast<TopLevelWindow*>(kioskModeComp)) tlw->setUsingNativeTitleBar(!enableOrDisable);

        if (enableOrDisable) kioskModeComp->setBounds(getDisplays().getMainDisplay().totalArea);
    }
    ...
    static BOOL CALLBACK enumMonitorsProc(HMONITOR hm, HDC, LPRECT r, LPARAM userInfo)
    {
        ...
        if (getDPIForMonitor != nullptr)
        {
            ...
            if (SUCCEEDED(getDPIForMonitor(hm, MDT_Default, &dpiX, &dpiY))) dpi = (dpiX + dpiY) / 2.0;
        }
    ...
    void Desktop::Displays::findDisplays(float masterScale)
    {
        ...
        for (int i = 1; i < monitors.size(); ++i)
            if (monitors.getReference(i).isMain) monitors.swap(i, 0);
        ...
        for (int i = 0; i < monitors.size(); ++i)
        {
            ...
            if (d.isMain) d.userArea = d.userArea.getIntersection(rectangleFromRECT(workArea) / masterScale);
    ...
    static HICON extractFileHICON(const File& file)
    {
        ...
        return ExtractAssociatedIcon((HINSTANCE)Process::getCurrentModuleInstanceHandle(), name, &iconNum);
    }
    ...
    {
        if (cursorHandle != nullptr && !isStandard) DestroyCursor((HCURSOR)cursorHandle);
    }
    ...
    void* MouseCursor::createStandardMouseCursor(const MouseCursor::StandardCursorType type)
    {
        ...
        case DraggingHandCursor:
        {
            ...
            if (dragHandCursor == nullptr)
            {
                ...
                dragHandCursor =
                    CustomMouseCursorInfo(ImageFileFormat::loadFrom(dragHandData, sizeof(dragHandData)), {8, 7}).create();
            }
        ...
        if (cursorH == 0) cursorH = LoadCursor(0, IDC_ARROW);
