    ...
    public:
            ...
            : owner(fileChooser)
        {
            if (currentFileChooser == nullptr)
            {
                ...
                const char* action =
                    (selectsDirectories ? "android.intent.action.OPEN_DOCUMENT_TREE"
                                        : (saveMode ? "android.intent.action.CREATE_DOCUMENT"
                                                    : (sdkVersion >= 19 ? "android.intent.action.OPEN_DOCUMENT"
                                                                        : "android.intent.action.GET_CONTENT")));

                intent =
                    GlobalRef(env->NewObject(AndroidIntent, AndroidIntent.constructWithString, javaString(action).get()));
                ...
                if (owner.startingFile != File())
                {
                    ...
                    LocalRef<jobject> uri(
                        env->CallStaticObjectMethod(AndroidUri, AndroidUri.parse, javaString(url.toString(true)).get()));

                    if (uri)
                        env->CallObjectMethod(intent.get(), AndroidIntent.putExtraParcelable,
                                              javaString("android.provider.extra.INITIAL_URI").get(), uri.get());
                }
                ...
                if (!selectsDirectories)
                {
                    ...
                    else
                    {
                        ...
                        if (mimeTypes.size() > 0)
                        {
                            ...
                            LocalRef<jobjectArray> jMimeTypes(
                                env->NewObjectArray(mimeTypes.size(), JavaString, javaString("").get()));
                            ...
                            env->CallObjectMethod(intent.get(), AndroidIntent.putExtraStrings,
                                                  javaString("android.intent.extra.MIME_TYPES").get(), jMimeTypes.get());

                            if (!allMimeTypesHaveSameGroup) mimeGroup = "*";
                        }
        ...
        {
            if (currentFileChooser != nullptr)
                android.activity.callVoidMethod(JuceAppActivity.startActivityForResult, intent.get(),
        ...
        void completed(int resultCode, jobject intentData)
        {
            ...
            if (resultCode == /*Activity.RESULT_OK*/ -1 && intentData != nullptr)
            {
                ...
                if (uri != nullptr)
                {
                    ...
                    if (jStr != nullptr) chosenURLs.add(URL(juceString(env, jStr)));
                }
            }
