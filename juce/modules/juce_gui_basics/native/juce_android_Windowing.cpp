...
//==============================================================================
    ...
    //==============================================================================
    JUCE_JNI_CALLBACK(JUCE_ANDROID_ACTIVITY_CLASSNAME, launchApp, void,
                      (JNIEnv * env, jobject activity, jstring appFile, jstring appDataDir))
    {
        setEnv(env);
        ...
        {
            if (!app->initialiseApp()) exit(app->shutdownApp());
        }
        else
        {
    ...
    JUCE_JNI_CALLBACK(JUCE_ANDROID_ACTIVITY_CLASSNAME, suspendApp, void, (JNIEnv * env, jobject))
    {
        ...
        if (auto* app = JUCEApplicationBase::getInstance()) app->suspended();
    }
    ...
    JUCE_JNI_CALLBACK(JUCE_ANDROID_ACTIVITY_CLASSNAME, resumeApp, void, (JNIEnv * env, jobject))
    {
        ...
        if (auto* app = JUCEApplicationBase::getInstance()) app->resumed();
    }
    ...
    JUCE_JNI_CALLBACK(JUCE_ANDROID_ACTIVITY_CLASSNAME, appActivityResult, void,
                      (JNIEnv * env, jobject, jint requestCode, jint resultCode, jobject intentData))
    {
        setEnv(env);

#if JUCE_IN_APP_PURCHASES && JUCE_MODULE_AVAILABLE_juce_product_unlocking
        if (requestCode == 1001) juce_inAppPurchaseCompleted(intentData);
#endif

#if !JUCE_DISABLE_NATIVE_FILECHOOSERS
        if (requestCode == /*READ_REQUEST_CODE*/ 42) juce_fileChooserCompleted(resultCode, intentData);
#endif

        if (requestCode == 1003) juce_contentSharingCompleted(resultCode);
...
#if JUCE_PUSH_NOTIFICATIONS && JUCE_MODULE_AVAILABLE_juce_gui_extra
        if (juce_handleNotificationIntent((void*)intentData)) return;
    ...
    JUCE_JNI_CALLBACK(JUCE_FIREBASE_INSTANCE_ID_SERVICE_CLASSNAME, firebaseInstanceIdTokenRefreshed, void,
                      (JNIEnv * env, jobject /*activity*/, jstring token))
    {
        setEnv(env);
    ...
    JUCE_JNI_CALLBACK(JUCE_FIREBASE_MESSAGING_SERVICE_CLASSNAME, firebaseRemoteMessageReceived, void,
                      (JNIEnv * env, jobject /*activity*/, jobject remoteMessage))
    {
        setEnv(env);
    ...
    JUCE_JNI_CALLBACK(JUCE_FIREBASE_MESSAGING_SERVICE_CLASSNAME, firebaseRemoteMessagesDeleted, void,
    ...
    JUCE_JNI_CALLBACK(JUCE_FIREBASE_MESSAGING_SERVICE_CLASSNAME, firebaseRemoteMessageSent, void,
                      (JNIEnv * env, jobject /*activity*/, jstring messageId))
    {
        setEnv(env);
    ...
    JUCE_JNI_CALLBACK(JUCE_FIREBASE_MESSAGING_SERVICE_CLASSNAME, firebaseRemoteMessageSendError, void,
                      (JNIEnv * env, jobject /*activity*/, jstring messageId, jstring error))
    {
        setEnv(env);
...
#undef JNI_CLASS_MEMBERS
    ...
    //==============================================================================
    class AndroidComponentPeer : public ComponentPeer, private Timer
    {
    public:
            ...
            , scale((float)Desktop::getInstance().getDisplays().getMainDisplay().scale)
        {
            ...
            view = GlobalRef(android.activity.callObjectMethod(JuceAppActivity.createNewView, (jboolean)component.isOpaque(),
                                                               (jlong)this));

            if (isFocused()) handleFocusGain();
        }
        ...
        void setBounds(const Rectangle<int>& userRect, bool isNowFullScreen) override
        {
            ...
            if (MessageManager::getInstance()->isThisTheMessageThread())
            {
                ...
                view.callVoidMethod(AndroidView.layout, r.getX(), r.getY(), r.getRight(), r.getBottom());
            }
            else
            {
                ...
                {
                public:
                    ViewMover(const GlobalRef& v, const Rectangle<int>& boundsToUse)
                        : view(v)
                        , bounds(boundsToUse)
                    {
                    }
                    ...
                    void messageCallback() override
                    {
                        view.callVoidMethod(AndroidView.layout, bounds.getX(), bounds.getY(), bounds.getRight(),
                                            bounds.getBottom());
                    }
        ...
        {
            return (Rectangle<float>(view.callIntMethod(AndroidView.getLeft), view.callIntMethod(AndroidView.getTop),
                                     view.callIntMethod(AndroidView.getWidth), view.callIntMethod(AndroidView.getHeight))
                    / scale)
                .toNearestInt();
        }
        ...
        {
            return Point<float>(view.callIntMethod(AndroidView.getLeft), view.callIntMethod(AndroidView.getTop)) / scale;
        }
        ...
        bool shouldNavBarsBeHidden(bool shouldBeFullScreen) const
        {
            if (shouldBeFullScreen)
                if (Component* kiosk = Desktop::getInstance().getKioskModeComponent())
                    if (kiosk->getPeer() == this) return true;
        ...
        void setFullScreen(bool shouldBeFullScreen) override
        {
            ...
            if ((!shouldBeFullScreen) && r.isEmpty()) r = getBounds();

            // (can't call the component's setBounds method because that'll reset our fullscreen flag)
            if (!r.isEmpty()) setBounds(r, shouldBeFullScreen);
        ...
        bool contains(Point<int> localPos, bool trueIfInAChildWindow) const override
        {
            return isPositiveAndBelow(localPos.x, component.getWidth())
                   ...
                   && ((!trueIfInAChildWindow)
                       || view.callBooleanMethod(ComponentPeerView.containsPoint, localPos.x * scale, localPos.y * scale));
        }
        ...
        void handleMouseDownCallback(int index, Point<float> sysPos, int64 time)
        {
            ...
            if (isValidPeer(this)) handleMouseDragCallback(index, sysPos, time);
        }
        ...
        void handleMouseDragCallback(int index, Point<float> pos, int64 time)
        {
            ...
            handleMouseEvent(MouseInputSource::InputSourceType::touch, pos,
                             currentModifiers.withoutMouseButtons().withFlags(ModifierKeys::leftButtonModifier),
                             MouseInputSource::invalidPressure, MouseInputSource::invalidOrientation, time, {}, index);
        }
        ...
        void handleMouseUpCallback(int index, Point<float> pos, int64 time)
        {
            ...
            if (touchesDown == 0) currentModifiers = currentModifiers.withoutMouseButtons();
        ...
        {
            if (auto* app = JUCEApplicationBase::getInstance()) app->backButtonPressed();
        }

        void handleAppResumedCallback()
        {
            if (Component* kiosk = Desktop::getInstance().getKioskModeComponent())
                if (kiosk->getPeer() == this) setNavBarsHidden(navBarsHidden);
        }

        //==============================================================================
        bool isFocused() const override
        {
            if (view != nullptr) return view.callBooleanMethod(AndroidView.hasFocus);
        ...
        {
            if (view != nullptr) view.callBooleanMethod(AndroidView.requestFocus);
        }
        ...
        static const char* getVirtualKeyboardType(TextInputTarget::VirtualKeyboardType type) noexcept
        {
            ...
            case TextInputTarget::numericKeyboard:
                return "number";
            case TextInputTarget::decimalKeyboard:
        ...
        void handlePaintCallback(JNIEnv* env, jobject canvas, jobject paint)
        {
            ...
            if (jint* dest = env->GetIntArrayElements((jintArray)buffer.get(), 0))
            {
                ...
                env->CallVoidMethod(canvas, CanvasMinimal.drawBitmap, (jintArray)buffer.get(), 0, clip.getWidth(),
                                    (jfloat)clip.getX(), (jfloat)clip.getY(), clip.getWidth(), clip.getHeight(), true, paint);
            }
        }
        ...
        void repaint(const Rectangle<int>& userArea) override
        {
            ...
            else
            {
                struct ViewRepainter : public CallbackMessage
                {
                    ...
                    void messageCallback() override
                    {
                        view.callVoidMethod(AndroidView.invalidate, area.getX(), area.getY(), area.getRight(),
                                            area.getBottom());
                    }
    ...
    private:
        ...
        struct PreallocatedImage : public ImagePixelData
        {
            PreallocatedImage(const int width_, const int height_, jint* data_, bool hasAlpha_)
                ...
                , hasAlpha(hasAlpha_)
            {
                if (hasAlpha_) zeromem(data_, static_cast<size_t>(width * height) * sizeof(jint));
            }
...
#define JUCE_VIEW_CALLBACK(returnType, javaMethodName, params, juceMethodInvocation)                                         \
    JUCE_JNI_CALLBACK(JUCE_JOIN_MACRO(JUCE_ANDROID_ACTIVITY_CLASSNAME, _00024ComponentPeerView), javaMethodName, returnType, \
                      params)                                                                                                \
    {                                                                                                                        \
        setEnv(env);                                                                                                         \
        if (AndroidComponentPeer* peer = (AndroidComponentPeer*)(pointer_sized_uint)host) peer->juceMethodInvocation;        \
    }

    JUCE_VIEW_CALLBACK(void, handlePaint, (JNIEnv * env, jobject /*view*/, jlong host, jobject canvas, jobject paint),
                       handlePaintCallback(env, canvas, paint))
    JUCE_VIEW_CALLBACK(void, handleMouseDown,
                       (JNIEnv * env, jobject /*view*/, jlong host, jint i, jfloat x, jfloat y, jlong time),
                       handleMouseDownCallback(i, Point<float>((float)x, (float)y), (int64)time))
    JUCE_VIEW_CALLBACK(void, handleMouseDrag,
                       (JNIEnv * env, jobject /*view*/, jlong host, jint i, jfloat x, jfloat y, jlong time),
                       handleMouseDragCallback(i, Point<float>((float)x, (float)y), (int64)time))
    JUCE_VIEW_CALLBACK(void, handleMouseUp,
                       (JNIEnv * env, jobject /*view*/, jlong host, jint i, jfloat x, jfloat y, jlong time),
                       handleMouseUpCallback(i, Point<float>((float)x, (float)y), (int64)time))
    JUCE_VIEW_CALLBACK(void, viewSizeChanged, (JNIEnv * env, jobject /*view*/, jlong host), handleMovedOrResized())
    JUCE_VIEW_CALLBACK(void, focusChanged, (JNIEnv * env, jobject /*view*/, jlong host, jboolean hasFocus),
                       handleFocusChangeCallback(hasFocus))
    JUCE_VIEW_CALLBACK(void, handleKeyDown, (JNIEnv * env, jobject /*view*/, jlong host, jint k, jint kc),
                       handleKeyDownCallback((int)k, (int)kc))
    JUCE_VIEW_CALLBACK(void, handleKeyUp, (JNIEnv * env, jobject /*view*/, jlong host, jint k, jint kc),
                       handleKeyUpCallback((int)k, (int)kc))
...
//==============================================================================
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD) METHOD(getRotation, "getRotation", "()I")
...
#undef JNI_CLASS_MEMBERS
    ...
    Desktop::DisplayOrientation Desktop::getCurrentOrientation() const
    {
        ...
        LocalRef<jobject> windowManager = LocalRef<jobject>(
            env->CallObjectMethod(android.activity, JuceAppActivity.getSystemService, windowServiceString.get()));

        if (windowManager.get() != 0)
        {
            LocalRef<jobject> display =
                LocalRef<jobject>(env->CallObjectMethod(windowManager, WindowManager.getDefaultDisplay));
    ...
    {
        android.activity.callVoidMethod(JuceAppActivity.showMessageBox, javaString(title).get(), javaString(message).get(),
                                                         ...
                                                         ModalComponentManager::Callback* callback)
    {
        ...
        android.activity.callVoidMethod(JuceAppActivity.showOkCancelBox, javaString(title).get(), javaString(message).get(),
                                        (jlong)(pointer_sized_int)callback, javaString(TRANS("OK")).get(),
                                        javaString(TRANS("Cancel")).get());
        return false;
    }
                                                     ...
                                                     ModalComponentManager::Callback* callback)
    {
        ...
        android.activity.callVoidMethod(JuceAppActivity.showOkCancelBox, javaString(title).get(), javaString(message).get(),
                                        (jlong)(pointer_sized_int)callback, javaString(TRANS("Yes")).get(),
                                        javaString(TRANS("No")).get());
        return 0;
    }

    JUCE_JNI_CALLBACK(JUCE_ANDROID_ACTIVITY_CLASSNAME, alertDismissed, void,
                      (JNIEnv * env, jobject /*activity*/, jlong callbackAsLong, jint result))
    {
        setEnv(env);
    ...
    static jint getAndroidOrientationFlag(int orientations) noexcept
    {
        ...
        case Desktop::upright:
            return (jint)SCREEN_ORIENTATION_PORTRAIT;
        case Desktop::upsideDown:
            return (jint)SCREEN_ORIENTATION_REVERSE_PORTRAIT;
        case Desktop::upright + Desktop::upsideDown:
            return (jint)SCREEN_ORIENTATION_USER_PORTRAIT;
        case Desktop::rotatedAntiClockwise:
            return (jint)SCREEN_ORIENTATION_LANDSCAPE;
        case Desktop::rotatedClockwise:
            return (jint)SCREEN_ORIENTATION_REVERSE_LANDSCAPE;
        case Desktop::rotatedClockwise + Desktop::rotatedAntiClockwise:
            return (jint)SCREEN_ORIENTATION_USER_LANDSCAPE;
        default:
            return (jint)SCREEN_ORIENTATION_USER;
        }
    }
    ...
    void Desktop::Displays::findDisplays(float masterScale)
    {
        ...
        d.userArea = d.totalArea = Rectangle<int>(android.screenWidth, android.screenHeight) / d.scale;

        displays.add(d);
    }

    JUCE_JNI_CALLBACK(JUCE_ANDROID_ACTIVITY_CLASSNAME, setScreenSize, void,
                      (JNIEnv * env, jobject /*activity*/, jint screenWidth, jint screenHeight, jint dpi))
    {
        setEnv(env);
