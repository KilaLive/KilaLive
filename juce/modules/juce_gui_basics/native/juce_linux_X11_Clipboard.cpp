    ...
    namespace ClipboardHelpers
    {
        ...
        static String readWindowProperty(::Display* display, Window window, Atom prop)
        {
            ...
            if (display != nullptr)
            {
                ...
                if (XGetWindowProperty(display, window, prop, 0L /* offset */, 1000000 /* length (max) */, False,
                                       AnyPropertyType /* format */, &actualType, &actualFormat, &numItems, &bytesLeft,
        ...
        static bool requestSelectionContent(::Display* display, String& selectionContent, Atom selection,
                                            Atom requestedFormat)
        {
            ...
            XConvertSelection(display, selection, requestedFormat, property_name, juce_messageWindowHandle, CurrentTime);
        ...
        static void handleSelection(XSelectionRequestEvent& evt)
        {
            ...
            if (data != nullptr)
            {
                ...
                {
                    XChangeProperty(evt.display, evt.requestor, evt.property, evt.target, propertyFormat /* 8 or 32 */,
                                    PropModeReplace, reinterpret_cast<const unsigned char*>(data.getData()),
                                    (int)numDataItems);
            ...
            */
            ...
            if (selectionOwner != None)
            {
                ...
                else
                {
                    ...
                    bool ok = ClipboardHelpers::requestSelectionContent(display, content, selection,
                                                                        ClipboardHelpers::atom_UTF8_STRING);

                    if (!ok)
                    {
                        // second chance, ask for a good old locale-dependent string ..
                        ok = ClipboardHelpers::requestSelectionContent(display, content, selection, XA_STRING);
                    }
                }
            }
        }
