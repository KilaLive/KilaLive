...
namespace juce
{
    namespace RelativeRectangleHelpers
    {
        ...
        {
            if (e.getType() == Expression::operatorType && e.getSymbolOrFunction() == ".") return true;
            ...
            else
            {
                for (int i = e.getNumInputs(); --i >= 0;)
                    if (dependsOnSymbolsOtherThanThis(e.getInput(i))) return true;
            }
    ...
    bool RelativeRectangle::isDynamic() const
    {
        ...
        return dependsOnSymbolsOtherThanThis(left.getExpression()) || dependsOnSymbolsOtherThanThis(right.getExpression())
               || dependsOnSymbolsOtherThanThis(top.getExpression()) || dependsOnSymbolsOtherThanThis(bottom.getExpression());
    }
    ...
    void RelativeRectangle::renameSymbol(const Expression::Symbol& oldSymbol, const String& newName,
    ...
    public:
        ...
        void applyToComponentBounds() override
        {
            for (int i = 32; --i >= 0;)
            {
                ...
                if (newBounds == getComponent().getBounds()) return;
    ...
    void RelativeRectangle::applyToComponent(Component& component) const
    {
        if (isDynamic())
        {
            RelativeRectangleComponentPositioner* current =
                dynamic_cast<RelativeRectangleComponentPositioner*>(component.getPositioner());
