...
namespace juce
{
    struct MarkerListScope : public Expression::Scope
    {
        ...
        static const MarkerList::Marker* findMarker(Component& component, const String& name, MarkerList*& list)
        {
            ...
            if (mlh != nullptr)
            {
                ...
                if (list != nullptr) marker = list->getMarker(name);
            }
    ...
    {
        if (auto* targetComp = (scopeName == RelativeCoordinate::Strings::parent) ? component.getParentComponent()
    ...
    {
        if (Component* const parent = component.getParentComponent()) return parent->findChildWithID(componentID);
    ...
    public:
        ...
        Expression getSymbolValue(const String& symbol) const override
        {
            ...
            default:
                if (auto* parent = component.getParentComponent())
                {
                    ...
                    {
                        // The marker we want doesn't exist, so watch all lists in case they change and the marker appears
                        // later..
        ...
        void visitRelativeScope(const String& scopeName, Visitor& visitor) const override
        {
            ...
            else
            {
                ...
                if (Component* const parent = component.getParentComponent()) positioner.registerComponentListener(*parent);
    ...
    {
        if (getComponent().getParentComponent() == &changed && !registeredOk) apply();
    }
    ...
    {
        for (int i = sourceComponents.size(); --i >= 0;) sourceComponents.getUnchecked(i)->removeComponentListener(this);

        for (int i = sourceMarkerLists.size(); --i >= 0;) sourceMarkerLists.getUnchecked(i)->removeListener(this);
