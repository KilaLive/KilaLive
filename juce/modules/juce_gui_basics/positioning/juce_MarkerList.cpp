...
namespace juce
{
    ...
    {
        if (other.markers.size() != markers.size()) return false;
        ...
        for (int i = markers.size(); --i >= 0;)
        {
            ...
            if (m2 == nullptr || *m1 != *m2) return false;
        }
    ...
    {
        if (parentComponent == nullptr) return marker.position.resolve(nullptr);
    ...
    void MarkerList::ValueTreeWrapper::applyTo(MarkerList& markerList)
    {
        ...
        for (int i = markerList.getNumMarkers(); --i >= 0;)
            if (!updatedMarkers.contains(markerList.getMarker(i)->name)) markerList.removeMarker(i);
    }
    ...
    void MarkerList::ValueTreeWrapper::readFrom(const MarkerList& markerList, UndoManager* undoManager)
    {
        ...
        for (int i = 0; i < markerList.getNumMarkers(); ++i) setMarker(*markerList.getMarker(i), undoManager);
    }
