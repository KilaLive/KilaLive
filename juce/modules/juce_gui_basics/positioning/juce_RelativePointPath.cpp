...
namespace juce
{
    ...
    RelativePointPath::RelativePointPath(const RelativePointPath& other)
        ...
        , containsDynamicPoints(false)
    {
        for (int i = 0; i < other.elements.size(); ++i) elements.add(other.elements.getUnchecked(i)->clone());
    }
    ...
    {
        if (elements.size() != other.elements.size() || usesNonZeroWinding != other.usesNonZeroWinding
        ...
        for (int i = 0; i < elements.size(); ++i)
        {
            ...
            for (int j = numPoints1; --j >= 0;)
                if (points1[j] != points2[j]) return false;
        }
    ...
    {
        for (int i = 0; i < elements.size(); ++i) elements.getUnchecked(i)->addToPath(path, scope);
    }
    ...
    //==============================================================================
    RelativePointPath::ElementBase::ElementBase(const ElementType type_)
        : type(type_)
    {
    }
    ...
    bool RelativePointPath::ElementBase::isDynamic()
    {
        ...
        for (int i = numPoints; --i >= 0;)
            if (points[i].isDynamic()) return true;
    ...
    RelativePointPath::StartSubPath::StartSubPath(const RelativePoint& pos)
        : ElementBase(startSubPathElement)
        , startPos(pos)
    {
    }
    ...
    {
        path.quadraticTo(controlPoints[0].resolve(scope), controlPoints[1].resolve(scope));
    }
    ...
    //==============================================================================
    RelativePointPath::CubicTo::CubicTo(const RelativePoint& controlPoint1, const RelativePoint& controlPoint2,
    ...
    {
        path.cubicTo(controlPoints[0].resolve(scope), controlPoints[1].resolve(scope), controlPoints[2].resolve(scope));
    }
