...
namespace juce
{
    ...
    void ShapeButton::setShape(const Path& newShape, const bool resizeNowToFitThisShape, const bool maintainShapeProportions_,
                               const bool hasShadow)
    {
        ...
        if (resizeNowToFitThisShape)
        {
            ...
            if (hasShadow) newBounds = newBounds.expanded(4.0f);
    ...
    void ShapeButton::paintButton(Graphics& g, bool isMouseOverButton, bool isButtonDown)
    {
        ...
        if (getComponentEffect() != nullptr) r = r.reduced(2.0f);

        if (isButtonDown)
        {
            const float sizeReductionWhenPressed = 0.04f;

            r = r.reduced(sizeReductionWhenPressed * r.getWidth(), sizeReductionWhenPressed * r.getHeight());
        }

        const AffineTransform trans(shape.getTransformToScaleToFit(r, maintainShapeProportions));

        if (isButtonDown)
            g.setColour(getToggleState() && useOnColours ? downColourOn : downColour);
        else if (isMouseOverButton)
            g.setColour(getToggleState() && useOnColours ? overColourOn : overColour);
        else
            g.setColour(getToggleState() && useOnColours ? normalColourOn : normalColour);
