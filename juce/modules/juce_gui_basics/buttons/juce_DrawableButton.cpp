...
namespace juce
{
    ...
    void DrawableButton::setImages(const Drawable* normal, const Drawable* over, const Drawable* down,
                                   const Drawable* disabled, const Drawable* normalOn, const Drawable* overOn,
                                   const Drawable* downOn, const Drawable* disabledOn)
    {
    ...
    void DrawableButton::resized()
    {
        ...
        if (currentImage != nullptr)
        {
            if (style == ImageRaw)
                ...
                currentImage->setTransformToFit(getImageBounds(), style == ImageStretched ? RectanglePlacement::stretchToFit
    ...
    void DrawableButton::buttonStateChanged()
    {
        ...
        {
            imageToDraw = getToggleState() ? disabledImageOn.get() : disabledImage.get();
        ...
        if (currentImage != nullptr) currentImage->setAlpha(opacity);
    }
    ...
    void DrawableButton::paintButton(Graphics& g, const bool isMouseOverButton, const bool isButtonDown)
    {
    ...
    {
        return (getToggleState() && normalImageOn != nullptr) ? normalImageOn.get() : normalImage.get();
    }
    ...
    {
        if (auto* d = getToggleState() ? downImageOn.get() : downImage.get()) return d;
