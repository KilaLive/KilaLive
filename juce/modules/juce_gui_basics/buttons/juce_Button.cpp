...
namespace juce
{
                                    ...
                                    public KeyListener
    {
        ...
        {
            if (value.refersToSameSourceAs(button.isOn)) button.setToggleState(button.isOn.getValue(), dontSendNotification);
        }
    ...
    Button::~Button()
    {
        ...
        if (commandManagerToUse != nullptr) commandManagerToUse->removeListener(callbackHelper.get());
    ...
    void Button::updateAutomaticTooltip(const ApplicationCommandInfo& info)
    {
        ...
        {
            auto tt = info.description.isNotEmpty() ? info.description : info.shortName;
    ...
    void Button::setRadioGroupId(const int newGroupId, NotificationType notification)
    {
        if (radioGroupId != newGroupId)
        {
            ...
            if (lastToggleState) turnOffOtherButtonsInGroup(notification);
        }
    }
    ...
    void Button::removeListener(Listener* l)
    {
    ...
    void Button::mouseDown(const MouseEvent& e)
    {
        ...
        if (isDown())
        {
            if (autoRepeatDelay >= 0) callbackHelper->startTimer(autoRepeatDelay);

            if (triggerOnMouseDown) internalClickCallback(e.mods);
        }
    }
    ...
    void Button::mouseUp(const MouseEvent& e)
    {
        ...
        {
            if (lastStatePainted != buttonDown) flashButtonState();
    ...
    void Button::mouseDrag(const MouseEvent& e)
    {
        ...
        if (autoRepeatDelay >= 0 && buttonState != oldState && isDown()) callbackHelper->startTimer(autoRepeatSpeed);
    }

    bool Button::isMouseOrTouchOver(const MouseEvent& e)
    {
        if (e.source.isTouch()) return getLocalBounds().toFloat().contains(e.position);
    ...
    void Button::parentHierarchyChanged()
    {
        ...
        {
            if (keySource != nullptr) keySource->removeKeyListener(callbackHelper.get());

            keySource = newKeySource;

            if (keySource != nullptr) keySource->addKeyListener(callbackHelper.get());
        }
    }

    //==============================================================================
    void Button::setCommandToTrigger(ApplicationCommandManager* const newCommandManager, const CommandID newCommandID,
                                     const bool generateTip)
    {
        ...
        if (commandManagerToUse != newCommandManager)
        {
            ...
            if (commandManagerToUse != nullptr) commandManagerToUse->addListener(callbackHelper.get());
    ...
    bool Button::isShortcutPressed() const
    {
        if (isShowing() && !isCurrentlyBlockedByAnotherModalComponent())
            for (auto& s : shortcuts)
                if (s.isCurrentlyDown()) return true;
    ...
    bool Button::keyStateChangedCallback()
    {
        ...
        if (autoRepeatDelay >= 0 && (isKeyDown && !wasDown)) callbackHelper->startTimer(autoRepeatDelay);
    ...
    //==============================================================================
    void Button::setRepeatSpeed(const int initialDelayMillisecs, const int repeatMillisecs,
    ...
    void Button::repeatTimerCallback()
    {
        ...
        else if (autoRepeatSpeed > 0 && (isKeyDown || (updateState() == buttonDown)))
        {
            ...
            if (lastRepeatTime != 0 && (int)(now - lastRepeatTime) > repeatSpeed * 2) repeatSpeed = jmax(1, repeatSpeed / 2);
