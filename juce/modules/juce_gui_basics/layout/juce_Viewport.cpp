...
namespace juce
{
    ...
    bool Viewport::canScrollVertically() const noexcept
    {
        return contentComp->getY() < 0 || contentComp->getBottom() > getHeight();
    }
    bool Viewport::canScrollHorizontally() const noexcept
    {
    ...
    {
        if (contentComp != nullptr) contentComp->setTopLeftPosition(viewportPosToCompPos(newPosition));
    }
    ...
    struct Viewport::DragToScrollListener : private MouseListener, private ViewportDragPosition::Listener
    {
        ...
        bool doesMouseEventComponentBlockViewportDrag(const Component* eventComp)
        {
            for (auto c = eventComp; c != nullptr && c != &viewport; c = c->getParentComponent())
                if (c->getViewportIgnoreDragFlag()) return true;
    ...
    void Viewport::updateVisibleArea()
    {
        ...
        for (int i = 3; --i >= 0;)
        {
            ...
            {
                hBarVisible = canShowHBar
                              && (hBarVisible || contentComp->getX() < 0 || contentComp->getRight() > contentArea.getWidth());
                vBarVisible =
                    canShowVBar
            ...
            if (oldContentBounds == contentComp->getBounds()) break;
        }

        Rectangle<int> contentBounds;
        if (contentComp != nullptr) contentBounds = contentHolder.getLocalArea(contentComp, contentComp->getLocalBounds());
        ...
        if (canShowHBar && !hBarVisible) visibleOrigin.setX(0);
        ...
        if (canShowVBar && !vBarVisible) visibleOrigin.setY(0);
    ...
    void Viewport::setScrollBarsShown(const bool showVerticalScrollbarIfNeeded, const bool showHorizontalScrollbarIfNeeded,
    ...
    {
        if (!useMouseWheelMoveIfNeeded(e, wheel)) Component::mouseWheelMove(e, wheel);
    }
    ...
    static int rescaleMouseWheelDistance(float distance, int singleStepSize) noexcept
    {
        ...
        return roundToInt(distance < 0 ? jmin(distance, -1.0f) : jmax(distance, 1.0f));
    }
    ...
    {
        return key == KeyPress::upKey || key == KeyPress::downKey || key == KeyPress::pageUpKey
               || key == KeyPress::pageDownKey || key == KeyPress::homeKey || key == KeyPress::endKey;
    }

    static bool isLeftRightKeyPress(const KeyPress& key)
    {
        return key == KeyPress::leftKey || key == KeyPress::rightKey;
    }
    ...
    bool Viewport::keyPressed(const KeyPress& key)
    {
        ...
        if (getVerticalScrollBar().isVisible() && isUpDownKey) return getVerticalScrollBar().keyPressed(key);
