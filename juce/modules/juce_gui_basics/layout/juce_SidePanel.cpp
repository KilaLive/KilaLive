...
namespace juce
{
    SidePanel::SidePanel(StringRef title, int width, bool positionOnLeft, Component* contentToDisplay,
        ...
        , panelWidth(width)
    {
        ...
        if (contentToDisplay != nullptr) setContent(contentToDisplay, deleteComponentWhenNoLongerNeeded);
    ...
    SidePanel::~SidePanel()
    {
        if (parent != nullptr) parent->removeComponentListener(this);
    }
    ...
    void SidePanel::showOrHide(bool show)
    {
        if (parent != nullptr)
        {
            ...
            Desktop::getInstance().getAnimator().animateComponent(this, calculateBoundsInParent(*parent), 1.0f, 250, true,
                                                                  1.0, 0.0);
        }
    }
    ...
    void SidePanel::resized()
    {
        ...
        titleLabel.setBounds(isOnLeft ? titleBounds.withTrimmedRight(40) : titleBounds.withTrimmedLeft(40));

        if (contentComponent != nullptr) contentComponent->setBounds(bounds);
    }
    ...
    void SidePanel::paint(Graphics& g)
    {
        ...
        g.setGradientFill(ColourGradient(
            shadowColour.withAlpha(0.7f), (isOnLeft ? shadowArea.getTopLeft() : shadowArea.getTopRight()).toFloat(),
            shadowColour.withAlpha(0.0f), (isOnLeft ? shadowArea.getTopRight() : shadowArea.getTopLeft()).toFloat(), false));
    ...
    void SidePanel::mouseDrag(const MouseEvent& e)
    {
        ...
        else if (isShowing)
        {
            ...
            if (!getLocalBounds().contains(relativeMouseDownPosition) && getLocalBounds().contains(relativeMouseDragPosition))
            {
                shouldResize   = true;
    ...
    void SidePanel::componentMovedOrResized(Component& component, bool wasMoved, bool wasResized)
    {
        ...
        if (wasResized && (&component == parent)) setBounds(calculateBoundsInParent(component));
    }
    ...
    {
        shadowArea = isOnLeft ? bounds.removeFromRight(shadowWidth) : bounds.removeFromLeft(shadowWidth);
    }

    bool SidePanel::isMouseEventInThisOrChildren(Component* eventComponent)
    {
        if (eventComponent == this) return true;

        for (auto& child : getChildren())
            if (eventComponent == child) return true;
