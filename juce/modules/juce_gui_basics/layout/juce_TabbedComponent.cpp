...
namespace juce
{
    namespace TabbedComponentHelpers
    {
        ...
        {
            if (comp != nullptr && (bool)comp->getProperties()[deleteComponentId]) delete comp;
        }
    ...
    void TabbedComponent::addTab(const String& tabName, Colour tabBackgroundColour, Component* const contentComponent,
                                 const bool deleteComponentWhenNotNeeded, const int insertIndex)
    {
    ...
    void TabbedComponent::setTabBackgroundColour(const int tabIndex, Colour newColour)
    {
        ...
        if (getCurrentTabIndex() == tabIndex) repaint();
    }
    ...
    void TabbedComponent::resized()
    {
        ...
        for (int i = contentComponents.size(); --i >= 0;)
            if (Component* c = contentComponents.getReference(i)) c->setBounds(content);
    }

    void TabbedComponent::lookAndFeelChanged()
    {
        for (int i = contentComponents.size(); --i >= 0;)
            if (Component* c = contentComponents.getReference(i)) c->lookAndFeelChanged();
    }
