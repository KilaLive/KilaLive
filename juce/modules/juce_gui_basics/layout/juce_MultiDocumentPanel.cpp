...
namespace juce
{
    MultiDocumentPanelWindow::MultiDocumentPanelWindow(Colour backgroundColour)
        : DocumentWindow(String(), backgroundColour, DocumentWindow::maximiseButton | DocumentWindow::closeButton, false)
    {
    }
    ...
    {
        if (auto* owner = getOwner()) owner->updateOrder();
    }
    ...
    struct MultiDocumentPanel::TabbedComponentInternal : public TabbedComponent
    {
        ...
        {
            if (auto* owner = findParentComponentOfClass<MultiDocumentPanel>()) owner->updateOrder();
        }
    };
    ...
    bool MultiDocumentPanel::closeAllDocuments(const bool checkItsOkToCloseFirst)
    {
        while (!components.isEmpty())
            if (!closeDocument(components.getLast(), checkItsOkToCloseFirst)) return false;
    ...
    void MultiDocumentPanel::addWindow(Component* component)
    {
        ...
        if (auto* topComp = getChildren().getLast())
            if (topComp->getX() == x && topComp->getY() == x) x += 16;

        dw->setTopLeftPosition(x, x);

        auto pos = component->getProperties()["mdiDocumentPos_"];
        if (pos.toString().isNotEmpty()) dw->restoreWindowStateFromString(pos.toString());

        addAndMakeVisible(dw);
        dw->toFront(true);
    }

    bool MultiDocumentPanel::addDocument(Component* const component, Colour docColour, const bool deleteWhenRemoved)
    {
        ...
        if (component == nullptr || (maximumNumDocuments > 0 && components.size() >= maximumNumDocuments)) return false;
        ...
        if (mode == FloatingWindows)
        {
            if (isFullscreenWhenOneDocument())
            {
                ...
                {
                    if (components.size() == 2) addWindow(components.getFirst());
        ...
        else
        {
            if (tabComponent == nullptr && components.size() > numDocsBeforeTabsUsed)
            {
                ...
                for (auto& c : temp) tabComponent->addTab(c->getName(), docColour, c, false);
    ...
    bool MultiDocumentPanel::closeDocument(Component* component, const bool checkItsOkToCloseFirst)
    {
        if (components.contains(component))
        {
            if (checkItsOkToCloseFirst && !tryToCloseDocument(component)) return false;
            ...
            if (mode == FloatingWindows)
            {
                ...
                if (isFullscreenWhenOneDocument() && components.size() == 1)
                {
                    ...
                    {
                        ScopedPointer<MultiDocumentPanelWindow> dw(
                            dynamic_cast<MultiDocumentPanelWindow*>(getChildComponent(i)));

                        if (dw != nullptr) dw->clearContentComponent();
                    }
            ...
            else
            {
                ...
                if (tabComponent != nullptr)
                {
                    for (int i = tabComponent->getNumTabs(); --i >= 0;)
                        if (tabComponent->getTabContentComponent(i) == component) tabComponent->removeTab(i);
                }
                else
                {
                ...
                if (tabComponent != nullptr && tabComponent->getNumTabs() <= numDocsBeforeTabsUsed) tabComponent.reset();

                components.removeFirstMatchingValue(component);

                if (components.size() > 0 && tabComponent == nullptr) addAndMakeVisible(components.getFirst());
            }

            resized();

            // This ensures that the active tab is painted properly when a tab is closed!
            if (auto* activeComponent = getActiveDocument()) setActiveDocument(activeComponent);
    ...
    Component* MultiDocumentPanel::getActiveDocument() const noexcept
    {
        if (mode == FloatingWindows)
        {
            for (auto* child : getChildren())
                if (auto* dw = dynamic_cast<MultiDocumentPanelWindow*>(child))
                    if (dw->isActiveWindow()) return dw->getContentComponent();
        }
    ...
    void MultiDocumentPanel::setActiveDocument(Component* component)
    {
        ...
        if (mode == FloatingWindows)
        {
            ...
            if (component != nullptr) component->toFront(true);
        }
    ...
    void MultiDocumentPanel::resized()
    {
        ...
        {
            for (auto* child : getChildren()) child->setBounds(getLocalBounds());
        }
    ...
    Component* MultiDocumentPanel::getContainerComp(Component* c) const
    {
        if (mode == FloatingWindows)
        {
            for (auto* child : getChildren())
                if (auto* dw = dynamic_cast<MultiDocumentPanelWindow*>(child))
                    if (dw->getContentComponent() == c) return dw;
        }
    ...
    void MultiDocumentPanel::updateOrder()
    {
        ...
        if (mode == FloatingWindows)
        {
            ...
            for (auto* child : getChildren())
                if (auto* dw = dynamic_cast<MultiDocumentPanelWindow*>(child)) components.add(dw->getContentComponent());
        }
        else
        {
            if (tabComponent != nullptr)
        ...
        if (components != oldList) activeDocumentChanged();
    }
