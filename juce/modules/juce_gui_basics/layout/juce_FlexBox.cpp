...
namespace juce
{
    struct FlexBoxLayoutCalculation
    {
        ...
        void createStates()
        {
            ...
            for (auto& item : owner.items) itemStates.add(item);

            std::stable_sort(itemStates.begin(), itemStates.end(), [](const ItemWithState& i1, const ItemWithState& i2) {
        ...
        void calculateCrossSizesByLine() noexcept
        {
            for (int row = 0; row < numberOfRows; ++row)
            {
                ...
                for (int column = 0; column < numColumns; ++column)
                {
                    ...
                    maxSize =
                        jmax(maxSize, isRowDirection ? item.lockedHeight + item.lockedMarginTop + item.lockedMarginBottom
        ...
        void alignLinesPerAlignContent() noexcept
        {
            ...
            else if (owner.alignContent == FlexBox::AlignContent::flexEnd)
            {
                for (int row = 0; row < numberOfRows; ++row)
                {
                    ...
                    for (int row2 = row; row2 < numberOfRows; ++row2) crossHeights += lineInfo[row2].crossSize;
            ...
            else
            {
                ...
                for (int row = 0; row < numberOfRows; ++row) totalHeight += lineInfo[row].crossSize;
                ...
                else if (owner.alignContent == FlexBox::AlignContent::center)
                {
                    ...
                    for (int row = 0; row < numberOfRows; ++row)
                        lineInfo[row].lineY =
                            row == 0 ? additionalength : lineInfo[row - 1].lineY + lineInfo[row - 1].crossSize;
                }
                else if (owner.alignContent == FlexBox::AlignContent::spaceBetween)
                {
                    const auto additionalength =
                        numberOfRows <= 1
                            ? Coord()
                            : jmax(Coord(), (containerCrossLength - totalHeight) / static_cast<Coord>(numberOfRows - 1));
                    lineInfo[0].lineY = 0;
                ...
                {
                    const auto additionalength = numberOfRows <= 1
                                                     ? Coord()
                ...
                for (int column = 0; column < numColumns; ++column)
                {
                    ...
                    else if (isAuto(item.item->margin.top))
                    {
                        item.lockedMarginLeft =
                            jmax(Coord(), crossSizeForLine - item.lockedHeight - item.item->margin.bottom);
                    }
                }
            }
        }
                        ...
                        else if (owner.alignItems == FlexBox::AlignItems::center)
                        {
                            if (isRowDirection)
                                item.lockedMarginTop =
                                    (lineSize - item.lockedHeight - item.item->margin.top - item.item->margin.bottom) / 2;
                            else
                                item.lockedMarginLeft =
                                    (lineSize - item.lockedWidth - item.item->margin.left - item.item->margin.right) / 2;
                        }
                    }
                }
            }
                ...
                for (int column = 0; column < numColumns; ++column)
                {
                    ...
                    if (!isAuto(item.item->margin.top))
                    {
                        ...
                        else if (item.item->alignSelf == FlexItem::AlignSelf::center)
                        {
                            if (isRowDirection)
                                item.lockedMarginTop =
                                    item.item->margin.top
                                    + (lineSize - item.lockedHeight - item.item->margin.top - item.item->margin.bottom) / 2;
                            else
                                item.lockedMarginLeft =
                                    item.item->margin.left
                                    + (lineSize - item.lockedWidth - item.item->margin.left - item.item->margin.right) / 2;
                        }
                        else if (item.item->alignSelf == FlexItem::AlignSelf::stretch)
                        {
                            ...
                            if (isRowDirection)
                                item.setHeightChecked(isAssigned(item.item->height)
                                                          ? getPreferredHeight(item)
                                                          : lineSize - item.item->margin.top - item.item->margin.bottom);
                            else
                                item.setWidthChecked(isAssigned(item.item->width)
                                                         ? getPreferredWidth(item)
        ...
        void layoutAllItems() noexcept
        {
            for (int row = 0; row < numberOfRows; ++row)
            {
                ...
                for (int column = 0; column < numColumns; ++column)
                {
                    ...
                    item.item->currentBounds.setSize((float)item.lockedWidth, (float)item.lockedHeight);
                }
            }
    ...
    private:
        void resetRowItems(const int row) noexcept
        {
            ...
            for (int column = 0; column < numColumns; ++column) resetItem(getItem(column, row));
        }
        ...
        bool layoutRowItems(const int row) noexcept
        {
            ...
            {
                if (totalFlexGrow != 0.0) changeUnit = difference / totalFlexGrow;
            }
            else
            {
                if (totalFlexShrink != 0.0) changeUnit = difference / totalFlexShrink;
            }
            ...
            for (int column = 0; column < numColumns; ++column)
            {
                ...
                if (!item.locked)
                    if (!addToItemLength(
                            item, (positiveFlexibility ? item.item->flexGrow : item.item->flexShrink) * changeUnit, row))
                        ok = false;
            }
        ...
        void recalculateTotalItemLengthPerLineArray() noexcept
        {
            for (int row = 0; row < numberOfRows; ++row)
            {
                ...
                for (int column = 0; column < numColumns; ++column)
                {
                    ...
                    lineInfo[row].totalLength += isRowDirection
                                                     ? item.lockedWidth + item.lockedMarginLeft + item.lockedMarginRight
    ...
    FlexBox::FlexBox(JustifyContent jc) noexcept
        : justifyContent(jc)
    {
    }
        ...
        if (!items.isEmpty())
        {
            ...
            for (auto& item : items)
            {
                ...
                if (auto* comp = item.associatedComponent)
                    comp->setBounds(Rectangle<int>::leftTopRightBottom(
                        (int)item.currentBounds.getX(), (int)item.currentBounds.getY(), (int)item.currentBounds.getRight(),
                        (int)item.currentBounds.getBottom()));

                if (auto* box = item.associatedFlexBox) box->performLayout(item.currentBounds);
            }
        }
    }
    ...
    FlexItem::FlexItem(float w, float h) noexcept
        : currentBounds(w, h)
        , minWidth(w)
        , minHeight(h)
    {
    }
    FlexItem::FlexItem(float w, float h, Component& c) noexcept
        : FlexItem(w, h)
    {
        associatedComponent = &c;
    }
    FlexItem::FlexItem(float w, float h, FlexBox& fb) noexcept
        : FlexItem(w, h)
    {
        associatedFlexBox = &fb;
    }
    FlexItem::FlexItem(Component& c) noexcept
        : associatedComponent(&c)
    {
    }
    FlexItem::FlexItem(FlexBox& fb) noexcept
        : associatedFlexBox(&fb)
    {
    }
    ...
    FlexItem::Margin::Margin(float v) noexcept
        : left(v)
        , right(v)
        , top(v)
        , bottom(v)
    ...
    FlexItem::Margin::Margin(float t, float r, float b, float l) noexcept
        : left(l)
        , right(r)
        , top(t)
        , bottom(b)
