...
namespace juce
{
    ...
    {
        if (component != nullptr) component->removeComponentListener(this);
    ...
    void ComponentMovementWatcher::componentParentHierarchyChanged(Component&)
    {
        if (component != nullptr && !reentrant)
        {
            ...
            if (component != nullptr) componentVisibilityChanged(*component);
        }
    }
    ...
    void ComponentMovementWatcher::componentMovedOrResized(Component&, bool wasMoved, bool wasResized)
    {
        if (component != nullptr)
        {
            ...
            if (wasMoved || wasResized) componentMovedOrResized(wasMoved, wasResized);
        }
    }
    ...
    {
        for (auto* c : registeredParentComps) c->removeComponentListener(this);
