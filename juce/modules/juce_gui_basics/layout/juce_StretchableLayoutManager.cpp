...
namespace juce
{
    ...
    void StretchableLayoutManager::setItemLayout(const int itemIndex, const double minimumSize, const double maximumSize,
                                                 const double preferredSize)
    {
        ...
        if (layout == nullptr)
        {
            ...
            for (i = 0; i < items.size(); ++i)
                if (items.getUnchecked(i)->itemIndex > itemIndex) break;
    ...
    bool StretchableLayoutManager::getItemLayout(const int itemIndex, double& minimumSize, double& maximumSize,
    ...
    int StretchableLayoutManager::getItemCurrentPosition(const int itemIndex) const
    {
        ...
        for (int i = 0; i < itemIndex; ++i)
            if (auto* layout = getInfoFor(i)) pos += layout->currentSize;
    ...
    {
        if (auto* layout = getInfoFor(itemIndex)) return layout->currentSize;
    ...
    {
        if (auto* layout = getInfoFor(itemIndex)) return -layout->currentSize / (double)totalSize;

        return 0;
    }

    void StretchableLayoutManager::setItemPosition(const int itemIndex, int newPosition)
    {
    ...
    //==============================================================================
    void StretchableLayoutManager::layOutComponents(Component** const components, int numComponents, int x, int y, int w,
                                                    int h, const bool vertically, const bool resizeOtherDimension)
    {
    ...
    {
        for (auto* i : items)
            if (i->itemIndex == itemIndex) return i;

        return nullptr;
    }

    int StretchableLayoutManager::fitComponentsIntoSpace(const int startIndex, const int endIndex, const int availableSpace,
                                                         int startPos)
    {
        ...
        if (totalIdealSize <= 0) totalIdealSize = 1.0;
        ...
        while (extraSpace > 0)
        {
            ...
            for (int i = startIndex; i < endIndex; ++i)
            {
                ...
                auto bestSize =
                    jlimit(layout->currentSize, jmax(layout->currentSize, sizeToRealSize(layout->maxSize, totalSize)),
                           roundToInt(sizeWanted * availableSpace / totalIdealSize));

                if (bestSize > layout->currentSize) ++numWantingMoreSpace;
            }
            ...
            for (int i = startIndex; i < endIndex; ++i)
            {
                ...
                if (extraWanted > 0)
                {
                    auto extraAllowed = jmin(extraWanted, extraSpace / jmax(1, numWantingMoreSpace));
            ...
            if (numHavingTakenExtraSpace <= 0) break;
        }
    ...
    int StretchableLayoutManager::getMinimumSizeOfItems(const int startIndex, const int endIndex) const
    {
        int totalMinimums = 0;
