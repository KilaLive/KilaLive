...
namespace juce
{
    ...
    void StretchableObjectResizer::addItem(const double size, const double minSize, const double maxSize, const int order)
    {
    ...
    {
        return isPositiveAndBelow(index, items.size()) ? items.getReference(index).size : 0.0;
    }
        ...
        for (;;)
        {
            ...
            if (thisIterationTarget >= currentSize)
            {
                ...
                for (int i = 0; i < items.size(); ++i)
                {
                    ...
                    if (it.order <= order) it.size = jlimit(it.minSize, it.maxSize, it.size + (it.maxSize - it.size) * scale);
                }
            }
            else
            {
                ...
                for (int i = 0; i < items.size(); ++i)
                {
                    ...
                    if (it.order <= order) it.size = jmax(it.minSize, it.minSize + (it.size - it.minSize) * scale);
                }
            }
