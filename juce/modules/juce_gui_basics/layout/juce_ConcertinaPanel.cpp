...
namespace juce
{
    struct ConcertinaPanel::PanelSizes
    {
        ...
        PanelSizes withMovedPanel(const int index, int targetPosition, int totalSpace) const
        {
            ...
            newSizes.stretchRange(
                index, num, totalSpace - newSizes.getTotalSize(0, index) - newSizes.getTotalSize(index, num), stretchFirst);
            return newSizes;
        }
    ...
    private:
        ...
        void growRangeFirst(const int start, const int end, int spaceDiff) noexcept
        {
            for (int attempts = 4; --attempts >= 0 && spaceDiff > 0;)
                for (int i = start; i < end && spaceDiff > 0; ++i) spaceDiff -= get(i).expand(spaceDiff);
        }

        void growRangeLast(const int start, const int end, int spaceDiff) noexcept
        {
            for (int attempts = 4; --attempts >= 0 && spaceDiff > 0;)
                for (int i = end; --i >= start && spaceDiff > 0;) spaceDiff -= get(i).expand(spaceDiff);
        }
        ...
        void growRangeAll(const int start, const int end, int spaceDiff) noexcept
        {
            ...
            for (int i = start; i < end; ++i)
                if (get(i).canExpand() && !get(i).isMinimised()) expandableItems.add(&get(i));
        ...
        {
            for (int i = start; i < end && spaceDiff > 0; ++i) spaceDiff -= get(i).reduce(spaceDiff);
        }

        void shrinkRangeLast(const int start, const int end, int spaceDiff) noexcept
        {
            for (int i = end; --i >= start && spaceDiff > 0;) spaceDiff -= get(i).reduce(spaceDiff);
        }

        void stretchRange(const int start, const int end, const int amountToAdd, const ExpandMode expandMode) noexcept
        {
            if (end > start)
            {
                if (amountToAdd > 0)
                {
                    if (expandMode == stretchAll)
                        growRangeAll(start, end, amountToAdd);
                    else if (expandMode == stretchFirst)
                        growRangeFirst(start, end, amountToAdd);
                    else if (expandMode == stretchLast)
                        growRangeLast(start, end, amountToAdd);
                }
                else
                {
                    if (expandMode == stretchFirst)
                        shrinkRangeFirst(start, end, -amountToAdd);
                    else
                        shrinkRangeLast(start, end, -amountToAdd);
                }
            }
        }
    ...
    public:
        ...
        void paint(Graphics& g) override
        {
            if (customHeaderComponent == nullptr)
            {
                ...
                getLookAndFeel().drawConcertinaPanelHeader(g, area, isMouseOver(), isMouseButtonDown(), getPanel(),
                                                           *component);
            }
        }
        ...
        void resized() override
        {
            ...
            if (customHeaderComponent != nullptr) customHeaderComponent->setBounds(headerBounds);
        ...
        void mouseDrag(const MouseEvent& e) override
        {
            ...
            panel.setLayout(dragStartSizes.withMovedPanel(panel.holders.indexOf(this),
                                                          mouseDownY + e.getDistanceFromDragStartY(), panel.getHeight()),
                            false);
        }
    ...
    {
        if (PanelHolder* h = holders[index]) return h->component;
    ...
    void ConcertinaPanel::addPanel(int insertIndex, Component* component, bool takeOwnership)
    {
        ...
        currentSizes->sizes.insert(insertIndex,
                                   PanelSizes::Panel(headerHeight, headerHeight, std::numeric_limits<int>::max()));
    ...
    void ConcertinaPanel::setCustomPanelHeader(Component* component, Component* customComponent, bool takeOwnership)
    {
        ...
        if (index >= 0) holders.getUnchecked(index)->setCustomHeaderComponent(optional.release(), takeOwnership);
    }
    ...
    int ConcertinaPanel::indexOfComp(Component* comp) const noexcept
    {
        for (int i = 0; i < holders.size(); ++i)
            if (holders.getUnchecked(i)->component == comp) return i;
    ...
    {
        if (!animate) animator.cancelAllAnimations(false);
    ...
    {
        if (!expandPanelFully(component, true)) setPanelSize(component, 0, true);
    }
