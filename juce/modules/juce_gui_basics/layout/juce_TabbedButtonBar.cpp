...
namespace juce
{
    ...
    bool TabBarButton::hitTest(int mx, int my)
    {
        ...
        if (owner.isVertical())
        {
            if (isPositiveAndBelow(mx, getWidth()) && my >= area.getY() + overlapPixels
                && my < area.getBottom() - overlapPixels)
                return true;
        }
        else
        {
            if (isPositiveAndBelow(my, getHeight()) && mx >= area.getX() + overlapPixels
                && mx < area.getRight() - overlapPixels)
                return true;
        }
        ...
        return p.contains((float)(mx - area.getX()), (float)(my - area.getY()));
    }
    ...
    void TabBarButton::resized()
    {
        if (extraComponent != nullptr)
        {
            ...
            if (!extraComp.isEmpty()) extraComponent->setBounds(extraComp);
        }
    }
    ...
    void TabbedButtonBar::setOrientation(const Orientation newOrientation)
    {
        ...
        for (auto* child : getChildren()) child->resized();
    ...
    void TabbedButtonBar::addTab(const String& tabName, Colour tabBackgroundColour, int insertIndex)
    {
        jassert(tabName.isNotEmpty());  // you have to give them all a name..

        if (tabName.isNotEmpty())
        {
            if (!isPositiveAndBelow(insertIndex, tabs.size())) insertIndex = tabs.size();
    ...
    {
        if (auto* tab = tabs[currentTabIndex]) return tab->name;
    ...
    StringArray TabbedButtonBar::getTabNames() const
    {
        ...
        for (auto* t : tabs) names.add(t->name);
    ...
    void TabbedButtonBar::setCurrentTabIndex(int newIndex, bool shouldSendChangeMessage)
    {
        ...
        {
            if (!isPositiveAndBelow(newIndex, tabs.size())) newIndex = -1;
            ...
            if (shouldSendChangeMessage) sendChangeMessage();
    ...
    {
        if (auto* tab = tabs[index]) return static_cast<TabBarButton*>(tab->button.get());
    ...
    int TabbedButtonBar::indexOfTabButton(const TabBarButton* button) const
    {
        for (int i = tabs.size(); --i >= 0;)
            if (tabs.getUnchecked(i)->button.get() == button) return i;
    ...
    {
        if (button == nullptr || indexOfTabButton(button) == -1) return {};
    ...
    void TabbedButtonBar::updateTabPositions(bool animate)
    {
        ...
        if (!isVertical()) std::swap(depth, length);
        ...
        if (totalLength > length) scale = jmax(minimumScale, length / (double)totalLength);
        ...
        for (int i = 0; i < tabs.size(); ++i)
        {
            if (auto* tb = getTabButton(i))
            {
                ...
                if (i < numVisibleButtons)
                {
                    ...
                    if (i == currentTabIndex) frontTab = tb;
    ...
    {
        if (auto* tab = tabs[tabIndex]) return tab->colour;
    ...
    {
        if (bar != nullptr && result > 0) bar->setCurrentTabIndex(result - 1);
    }
    ...
    void TabbedButtonBar::showExtraItemsMenu()
    {
        ...
        for (int i = 0; i < tabs.size(); ++i)
        {
            ...
            if (!tab->button->isVisible()) m.addItem(i + 1, tab->name, true, i == currentTabIndex);
        }
