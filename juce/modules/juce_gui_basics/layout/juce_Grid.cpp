...
namespace juce
{
    struct Grid::SizeCalculation
    {
        static float getTotalAbsoluteSize(const juce::Array<Grid::TrackInfo>& tracks, Px gapSize) noexcept
        {
            ...
            for (const auto& trackInfo : tracks)
                if (!trackInfo.isFraction || trackInfo.hasKeyword) totalCellSize += trackInfo.size;

            float totalGap = tracks.size() > 1 ? static_cast<float>((tracks.size() - 1) * gapSize.pixels) : 0.0f;
        ...
        static float getRelativeUnitSize(float size, float totalAbsolute, const juce::Array<Grid::TrackInfo>& tracks) noexcept
        {
            ...
            for (const auto& trackInfo : tracks)
                if (trackInfo.isFraction) factorsSum += trackInfo.size;
        ...
        {
            for (auto& t : tracks)
                if (t.isFraction) return true;

            return false;
        }

        void computeSizes(float gridWidth, float gridHeight, Px columnGapToUse, Px rowGapToUse,
                          const juce::Array<Grid::TrackInfo>& columnTracks, const juce::Array<Grid::TrackInfo>& rowTracks)
        {
    ...
    struct Grid::PlacementHelpers
    {
        ...
        //==============================================================================
        static int deduceAbsoluteLineNumberFromLineName(GridItem::Property prop, const juce::Array<Grid::TrackInfo>& tracks)
        {
            ...
            for (int i = 0; i < lines.size(); i++)
            {
                ...
                if (count == prop.number) return i + 1;
            }
        ...
        static int deduceAbsoluteLineNumber(GridItem::Property prop, const juce::Array<Grid::TrackInfo>& tracks)
        {
            jassert(prop.hasAbsolute());

            if (prop.hasName()) return deduceAbsoluteLineNumberFromLineName(prop, tracks);

            return prop.number > 0 ? prop.number : tracks.size() + 2 + prop.number;
        }

        static int deduceAbsoluteLineNumberFromNamedSpan(int startLineNumber, GridItem::Property propertyWithSpan,
                                                         const juce::Array<Grid::TrackInfo>& tracks)
        {
            ...
            for (int i = startLineNumber; i < lines.size(); i++)
            {
                ...
                if (count == propertyWithSpan.number) return i + 1;
            }
        ...
        static int deduceAbsoluteLineNumberBasedOnSpan(int startLineNumber, GridItem::Property propertyWithSpan,
                                                       const juce::Array<Grid::TrackInfo>& tracks)
        {
            jassert(propertyWithSpan.hasSpan());
        ...
        static LineArea deduceLineArea(const GridItem& item, const Grid& grid,
                                       const std::map<juce::String, LineArea>& namedAreas)
        {
            if (item.area.isNotEmpty() && !grid.templateAreas.isEmpty()) return namedAreas.at(item.area);

            return {deduceLineRange(item.column, grid.templateColumns), deduceLineRange(item.row, grid.templateRows)};
        }
        ...
        static juce::Array<juce::StringArray> parseAreasProperty(const juce::StringArray& areasStrings)
        {
            ...
            for (const auto& areaString : areasStrings) strings.add(juce::StringArray::fromTokens(areaString, false));
        ...
        static juce::Rectangle<float> getCellBounds(int columnNumber, int rowNumber,
                                                    ...
                                                    Grid::SizeCalculation calculation, Px columnGap, Px rowGap)
        {
            ...
            const float width =
                columnTrackInfo.isFraction ? columnTrackInfo.size * calculation.relativeWidthUnit : columnTrackInfo.size;

            const auto& rowTrackInfo = rowTracks.getReference(rowNumber - 1);
            const float height =
                rowTrackInfo.isFraction ? rowTrackInfo.size * calculation.relativeHeightUnit : rowTrackInfo.size;

            return {x, y, width, height};
        }

        static juce::Rectangle<float> alignCell(juce::Rectangle<float> area, int columnNumber, int rowNumber,
                                                int numberOfColumns, int numberOfRows, Grid::SizeCalculation calculation,
                                                Grid::AlignContent alignContent, Grid::JustifyContent justifyContent)
        {
            if (alignContent == Grid::AlignContent::end) area.setY(area.getY() + calculation.remainingHeight);

            if (justifyContent == Grid::JustifyContent::end) area.setX(area.getX() + calculation.remainingWidth);

            if (alignContent == Grid::AlignContent::center) area.setY(area.getY() + calculation.remainingHeight / 2);

            if (justifyContent == Grid::JustifyContent::center) area.setX(area.getX() + calculation.remainingWidth / 2);
        ...
        static juce::Rectangle<float> getAreaBounds(int columnLineNumberStart, int columnLineNumberEnd,
                                                    ...
                                                    const juce::Array<Grid::TrackInfo>& columnTracks,
                                                    const juce::Array<Grid::TrackInfo>& rowTracks,
                                                    Grid::SizeCalculation calculation, Grid::AlignContent alignContent,
                                                    Grid::JustifyContent justifyContent, Px columnGap, Px rowGap)
        {
            auto startCell = getCellBounds(columnLineNumberStart, rowLineNumberStart, columnTracks, rowTracks, calculation,
                                           columnGap, rowGap);

            auto endCell = getCellBounds(columnLineNumberEnd - 1, rowLineNumberEnd - 1, columnTracks, rowTracks, calculation,
                                         columnGap, rowGap);

            startCell = alignCell(startCell, columnLineNumberStart, rowLineNumberStart, columnTracks.size(), rowTracks.size(),
                                  calculation, alignContent, justifyContent);

            endCell = alignCell(endCell, columnLineNumberEnd - 1, rowLineNumberEnd - 1, columnTracks.size(), rowTracks.size(),
                                calculation, alignContent, justifyContent);
    ...
    struct Grid::AutoPlacement
    {
        ...
        struct OccupancyPlane
        {
            ...
            Grid::PlacementHelpers::LineArea setCell(Cell cell, int columnSpan, int rowSpan)
            {
                for (int i = 0; i < columnSpan; i++)
                    for (int j = 0; j < rowSpan; j++) setCell(cell.column + i, cell.row + j);
            ...
            {
                return setCell(start, std::abs(end.column - start.column), std::abs(end.row - start.row));
            }
            ...
            {
                if (columnFirst && (rowNumber + rowSpan) > highestCrossDimension) highestCrossDimension = rowNumber + rowSpan;

                while (isOccupied(referenceCell, columnSpan, rowSpan) || (referenceCell.row != rowNumber))
                    referenceCell = advance(referenceCell);
            ...
            Cell nextAvailableOnColumn(Cell referenceCell, int columnSpan, int rowSpan, int columnNumber)
            {
                ...
                while (isOccupied(referenceCell, columnSpan, rowSpan) || (referenceCell.column != columnNumber))
                    referenceCell = advance(referenceCell);
        ...
        private:
            struct SortableCell
            {
                ...
                bool operator<(const SortableCell& other) const
                {
                    if (columnFirst)
                    {
                        if (row == other.row) return column < other.column;
            ...
            bool isOccupied(Cell cell, int columnSpan, int rowSpan) const
            {
                for (int i = 0; i < columnSpan; i++)
                    for (int j = 0; j < rowSpan; j++)
                        if (isOccupied({cell.column + i, cell.row + j})) return true;
            ...
            int getHighestCrossDimension() const
            {
                ...
                if (occupiedCells.size() > 0) cell = {occupiedCells.crbegin()->column, occupiedCells.crbegin()->row};
            ...
            {
                if (columnFirst) return {mainDimension, crossDimension};
        ...
        {
            if (item.area.isNotEmpty()) return true;

            if (isFixed(item.column) && isFixed(item.row)) return true;
        ...
        {
            if (item.area.isNotEmpty()) return false;
        ...
        {
            return autoFlow == Grid::AutoFlow::columnDense || autoFlow == Grid::AutoFlow::rowDense;
        }

        static bool isColumnAutoFlow(Grid::AutoFlow autoFlow)
        {
            return autoFlow == Grid::AutoFlow::column || autoFlow == Grid::AutoFlow::columnDense;
        }

        //==============================================================================
        static int getSpanFromAuto(GridItem::StartAndEndProperty prop)
        {
            if (prop.end.hasSpan()) return prop.end.number;

            if (prop.start.hasSpan()) return prop.start.number;
        ...
        ItemPlacementArray deduceAllItems(Grid& grid) const
        {
            ...
            OccupancyPlane plane(juce::jmax(grid.templateColumns.size() + 1, 2), juce::jmax(grid.templateRows.size() + 1, 2),
                                 isColumnAutoFlow(grid.autoFlow));

            ItemPlacementArray     itemPlacementArray;
            juce::Array<GridItem*> sortedItems;

            for (auto& item : grid.items) sortedItems.add(&item);
            ...
            for (auto* item : sortedItems)
            {
                if (hasPartialFixedPlacement(*item))
                {
                    if (isFixed(item->column))
                    {
                        ...
                        const auto insertionCell =
                            hasDenseAutoFlow(grid.autoFlow) ? OccupancyPlane::Cell{p.start, 1} : lastInsertionCell;
                        const auto nextAvailableCell =
                            plane.nextAvailableOnColumn(insertionCell, columnSpan, rowSpan, p.start);
                    ...
                    else if (isFixed(item->row))
                    {
                        ...
                        const auto insertionCell =
                            hasDenseAutoFlow(grid.autoFlow) ? OccupancyPlane::Cell{1, p.start} : lastInsertionCell;
            ...
            for (auto* item : sortedItems)
            {
                if (hasAutoPlacement(*item))
                {
                    ...
                    if (!hasDenseAutoFlow(grid.autoFlow)) lastInsertionCell = nextAvailableCell;
        ...
        //==============================================================================
        static void applySizeForAutoTracks(juce::Array<Grid::TrackInfo>& columns, juce::Array<Grid::TrackInfo>& rows,
            ...
            auto getHighestItemOnRow = [isSpan](int rowNumber, const ItemPlacementArray& itemPlacementArrayToUse) -> float {
                ...
                for (const auto& i : itemPlacementArrayToUse)
                    if (!isSpan(i.second.row) && i.second.row.start == rowNumber)
                        highestRowSize =
                            std::max(highestRowSize, i.first->height + i.first->margin.top + i.first->margin.bottom);

                return highestRowSize;
            };

            auto getHighestItemOnColumn = [isSpan](int                       rowNumber,
                                                   const ItemPlacementArray& itemPlacementArrayToUse) -> float {
                float highestColumnSize = 0.0f;
                for (const auto& i : itemPlacementArrayToUse)
                    if (!isSpan(i.second.column) && i.second.column.start == rowNumber)
                        highestColumnSize =
                            std::max(highestColumnSize, i.first->width + i.first->margin.left + i.first->margin.right);
    ...
    {
        static juce::Rectangle<float> alignItem(const GridItem& item, const Grid& grid, juce::Rectangle<float> area)
        {
            ...
            if (item.width != GridItem::notAssigned) r.setWidth(item.width);

            if (item.height != GridItem::notAssigned) r.setHeight(item.height);

            if (alignType == Grid::AlignItems::start && justifyType == Grid::JustifyItems::start) return r;
    ...
    //==============================================================================
    Grid::TrackInfo::TrackInfo() noexcept
        : hasKeyword(true)
    {
    }
    ...
    Grid::TrackInfo::TrackInfo(Px sizeInPixels, const juce::String& endLineNameToUse) noexcept
        : Grid::TrackInfo(sizeInPixels)
    {
        endLineName = endLineNameToUse;
    }

    Grid::TrackInfo::TrackInfo(Fr fractionOfFreeSpace, const juce::String& endLineNameToUse) noexcept
        : Grid::TrackInfo(fractionOfFreeSpace)
    {
        endLineName = endLineNameToUse;
    }

    Grid::TrackInfo::TrackInfo(const juce::String& startLineNameToUse, Px sizeInPixels) noexcept
        : Grid::TrackInfo(sizeInPixels)
    {
        startLineName = startLineNameToUse;
    }

    Grid::TrackInfo::TrackInfo(const juce::String& startLineNameToUse, Fr fractionOfFreeSpace) noexcept
        : Grid::TrackInfo(fractionOfFreeSpace)
    {
        startLineName = startLineNameToUse;
    }

    Grid::TrackInfo::TrackInfo(const juce::String& startLineNameToUse, Px sizeInPixels,
                               const juce::String& endLineNameToUse) noexcept
        : Grid::TrackInfo(startLineNameToUse, sizeInPixels)
    {
        endLineName = endLineNameToUse;
    }

    Grid::TrackInfo::TrackInfo(const juce::String& startLineNameToUse, Fr fractionOfFreeSpace,
                               const juce::String& endLineNameToUse) noexcept
    ...
    void Grid::performLayout(juce::Rectangle<int> targetArea)
    {
        ...
        calculation.computeSizes(targetArea.toFloat().getWidth(), targetArea.toFloat().getHeight(), columnGap, rowGap,
                                 columnTracks, rowTracks);

        for (auto& itemAndArea : itemsAndAreas)
        {
            const auto a          = itemAndArea.second;
            const auto areaBounds = Grid::PlacementHelpers::getAreaBounds(a.column.start, a.column.end, a.row.start,
                                                                          a.row.end, columnTracks, rowTracks, calculation,
                                                                          alignContent, justifyContent, columnGap, rowGap);

            auto* item = itemAndArea.first;
            item->currentBounds =
                Grid::BoxAlignment::alignItem(*item, *this, areaBounds) + targetArea.toFloat().getPosition();

            if (auto* c = item->associatedComponent) c->setBounds(item->currentBounds.toNearestInt());
        }
    }
