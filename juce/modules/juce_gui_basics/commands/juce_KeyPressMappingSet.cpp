...
namespace juce
{
    ...
    Array<KeyPress> KeyPressMappingSet::getKeyPressesAssignedToCommand(const CommandID commandID) const
    {
        for (int i = 0; i < mappings.size(); ++i)
            if (mappings.getUnchecked(i)->commandID == commandID) return mappings.getUnchecked(i)->keypresses;
    ...
    void KeyPressMappingSet::addKeyPress(const CommandID commandID, const KeyPress& newKeyPress, int insertIndex)
    {
        ...
        jassert(
            !(CharacterFunctions::isUpperCase(newKeyPress.getTextCharacter()) && !newKeyPress.getModifiers().isShiftDown()));
    ...
    void KeyPressMappingSet::resetToDefaultMappings()
    {
        ...
        for (int i = 0; i < commandManager.getNumCommands(); ++i) addKeyPresses(*this, commandManager.getCommandForIndex(i));
    ...
    void KeyPressMappingSet::resetToDefaultMapping(const CommandID commandID)
    {
        ...
        if (const ApplicationCommandInfo* const ci = commandManager.getCommandForID(commandID)) addKeyPresses(*this, ci);
    }
    ...
    {
        for (int i = 0; i < mappings.size(); ++i)
            if (mappings.getUnchecked(i)->keypresses.contains(keyPress)) return mappings.getUnchecked(i)->commandID;
    ...
    void KeyPressMappingSet::invokeCommand(const CommandID commandID, const KeyPress& key, const bool isKeyDown,
                                           const int millisecsSinceKeyPressed, Component* const originatingComponent) const
    {
    ...
    XmlElement* KeyPressMappingSet::createXml(const bool saveDifferencesFromDefaultSet) const
    {
        ...
        for (int i = 0; i < mappings.size(); ++i)
        {
            ...
            {
                if (defaultSet == nullptr || !defaultSet->containsMapping(cm.commandID, cm.keypresses.getReference(j)))
                {
    ...
    bool KeyPressMappingSet::keyPressed(const KeyPress& key, Component* const originatingComponent)
    {
        ...
        if (originatingComponent != nullptr && commandWasDisabled) originatingComponent->getLookAndFeel().playAlertSound();
                ...
                for (int j = cm.keypresses.size(); --j >= 0;)
                {
                    ...
                    if (isDown != wasDown)
                    {
                        ...
                        else
                        {
                            ...
                            if (now > pressTime) millisecs = (int)(now - pressTime);
    ...
    {
        if (focusedComponent != nullptr) focusedComponent->keyStateChanged(false);
    }
