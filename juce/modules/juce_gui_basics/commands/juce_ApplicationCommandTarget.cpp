...
namespace juce
{
    ...
    public:
        ...
        {
            if (ApplicationCommandTarget* const target = owner) target->tryToInvoke(info, false);
        }
    ...
    ApplicationCommandTarget* ApplicationCommandTarget::getTargetForCommand(const CommandID commandID)
    {
        ...
        while (target != nullptr)
        {
            ...
            if (commandIDs.contains(commandID)) return target;
            ...
            if (depth > 100 || target == this) break;
        }
    ...
    bool ApplicationCommandTarget::invoke(const InvocationInfo& info, const bool async)
    {
        ...
        while (target != nullptr)
        {
            if (target->tryToInvoke(info, async)) return true;
        ...
        if (target == nullptr)
        {
            ...
            if (target != nullptr) return target->tryToInvoke(info, async);
        }
