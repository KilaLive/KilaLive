...
namespace juce
{
    ...
    private:
        void run() override
        {
            for (const auto& image : images)
            {
                ...
                if (!tempFile.create().wasOk()) break;

                ScopedPointer<FileOutputStream> outputStream = tempFile.createOutputStream();

                if (outputStream == nullptr) break;

                if (imageFileFormat->writeImageToStream(image, *outputStream)) owner.temporaryFiles.add(tempFile);
            }
    ...
    void ContentSharer::shareFiles(const Array<URL>& files, std::function<void(bool, const String&)> callbackToUse)
    {
        ...
        if (callbackToUse) callbackToUse(false, "Content sharing is not available on this platform!");
#endif
    }
    ...
    void ContentSharer::filesToSharePrepared()
    {
        ...
        for (const auto& tempFile : temporaryFiles) urls.add(URL(tempFile));
    ...
    void ContentSharer::shareData(const MemoryBlock& mb, std::function<void(bool, const String&)> callbackToUse)
    {
        ...
        if (callbackToUse) callbackToUse(false, "Content sharing not available on this platform!");
#endif
    }
    ...
    void ContentSharer::sharingFinished(bool succeeded, const String& errorDescription)
    {
        ...
        if (cb) cb(succeeded, errorDescription);
    }

    void ContentSharer::deleteTemporaryFiles()
    {
        for (auto& f : temporaryFiles) f.deleteFile();
