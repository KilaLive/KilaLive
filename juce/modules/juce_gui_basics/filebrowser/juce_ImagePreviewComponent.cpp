...
namespace juce
{
    ...
    void ImagePreviewComponent::getThumbSize(int& w, int& h) const
    {
        ...
        const double scale = jmin(1.0, availableW / (double)w, availableH / (double)h);
        ...
        if (in != nullptr && in->getFile().existsAsFile())
        {
            if (ImageFileFormat* const format = ImageFileFormat::findImageFormatForStream(*in))
            {
                ...
                if (currentThumbnail.isValid())
                {
                    ...
                    currentDetails << fileToLoad.getFileName() << "\n"
    ...
    void ImagePreviewComponent::paint(Graphics& g)
    {
        if (currentThumbnail.isValid())
        {
            ...
            g.drawImageWithin(currentThumbnail, (getWidth() - w) / 2, y, w, h,
                              RectanglePlacement::centred | RectanglePlacement::onlyReduceInSize, false);

            g.drawFittedText(currentDetails, 0, y + h + 4, getWidth(), 100, Justification::centredTop, numLines);
        }
    }
