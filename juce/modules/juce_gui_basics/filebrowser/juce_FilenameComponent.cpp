...
namespace juce
{
    FilenameComponent::FilenameComponent(const String& name, const File& currentFile, bool canEditFilename, bool isDirectory,
                                         bool isForSaving, const String& fileBrowserWildcard, const String& suffix,
    ...
    {
        return getCurrentFile() == File() ? defaultBrowseFile : getCurrentFile();
    }

    void FilenameComponent::showChooser()
    {
#if JUCE_MODAL_LOOPS_PERMITTED
        FileChooser fc(isDir ? TRANS("Choose a new directory") : TRANS("Choose a new file"), getLocationToBrowse(), wildcard);

        if (isDir ? fc.browseForDirectory() : (isSaving ? fc.browseForFileToSave(false) : fc.browseForFileToOpen()))
        {
    ...
    void FilenameComponent::filesDropped(const StringArray& filenames, int, int)
    {
        ...
        if (f.exists() && (f.isDirectory() == isDir)) setCurrentFile(f, true);
    }
    ...
    File FilenameComponent::getCurrentFile() const
    {
        ...
        if (enforcedSuffix.isNotEmpty()) f = f.withFileExtension(enforcedSuffix);

        return f;
    }

    void FilenameComponent::setCurrentFile(File newFile, const bool addToRecentlyUsedList, NotificationType notification)
    {
        if (enforcedSuffix.isNotEmpty()) newFile = newFile.withFileExtension(enforcedSuffix);

        if (newFile.getFullPathName() != lastFilename)
        {
            lastFilename = newFile.getFullPathName();

            if (addToRecentlyUsedList) addRecentlyUsedFile(newFile);
    ...
    StringArray FilenameComponent::getRecentlyUsedFilenames() const
    {
        ...
        for (int i = 0; i < filenameBox.getNumItems(); ++i) names.add(filenameBox.getItemText(i));
    ...
    void FilenameComponent::setRecentlyUsedFilenames(const StringArray& filenames)
    {
        if (filenames != getRecentlyUsedFilenames())
        {
            ...
            for (int i = 0; i < jmin(filenames.size(), maxRecentFiles); ++i) filenameBox.addItem(filenames[i], i + 1);
        }
    }
