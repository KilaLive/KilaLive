...
namespace juce
{
    ...
    //==============================================================================
    FileChooserDialogBox::FileChooserDialogBox(const String& name, const String& instructions,
                                               FileBrowserComponent& chooserComponent, bool shouldWarn,
...
//==============================================================================
    ...
    {
        if (auto* previewComp = content->chooserComponent.getPreviewComponent()) return 400 + previewComp->getWidth();
    ...
    {
        if (result != 0 && box != nullptr) box->exitModalState(1);
    }

    void FileChooserDialogBox::okButtonPressed()
    {
        if (warnAboutOverwritingExistingFiles && content->chooserComponent.isSaveMode()
    ...
    void FileChooserDialogBox::createNewFolder()
    {
        ...
        {
            auto* aw = new AlertWindow(TRANS("New Folder"), TRANS("Please enter the name for the folder"),
            ...
            aw->enterModalState(
                true,
                ModalCallbackFunction::forComponent(createNewFolderCallback, this, Component::SafePointer<AlertWindow>(aw)),
                true);
        }
    }
    ...
    void FileChooserDialogBox::createNewFolderConfirmed(const String& nameFromDialog)
    {
        ...
        if (!name.isEmpty())
        {
            ...
            if (!parent.getChildFile(name).createDirectory())
                AlertWindow::showMessageBoxAsync(AlertWindow::WarningIcon, TRANS("New Folder"),
