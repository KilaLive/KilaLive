...
namespace juce
{
    FileBrowserComponent::FileBrowserComponent(int flags_, const File& initialFileOrDirectory, const FileFilter* fileFilter_,
        ...
        , wasProcessActive(true)
    {
        ...
        if ((flags & useTreeView) != 0)
        {
            ...
            if ((flags & canSelectMultipleItems) != 0) tree->setMultiSelectEnabled(true);
        ...
        else
        {
            ...
            if ((flags & canSelectMultipleItems) != 0) list->setMultipleSelectionEnabled(true);
        ...
        filenameBox.onFocusLost  = [this] {
            if (!isSaveMode()) selectionChanged();
        };
        ...
        if (previewComp != nullptr) addAndMakeVisible(previewComp);
    ...
    {
        if (chosenFiles.isEmpty() && currentFileIsValid()) return 1;
    ...
    {
        if ((flags & canSelectDirectories) != 0 && filenameBox.getText().isEmpty()) return currentRoot;

        if (!filenameBox.isReadOnly()) return currentRoot.getChildFile(filenameBox.getText());
    ...
    bool FileBrowserComponent::currentFileIsValid() const
    {
        ...
        if (isSaveMode()) return (flags & canSelectDirectories) != 0 || !f.isDirectory();
    ...
    {
        if (f.isDirectory())
            return (flags & canSelectDirectories) != 0 && (fileFilter == nullptr || fileFilter->isDirectorySuitable(f));

        return (flags & canSelectFiles) != 0 && f.exists() && (fileFilter == nullptr || fileFilter->isFileSuitable(f));
    }
    ...
    void FileBrowserComponent::setRoot(const File& newRootDirectory)
    {
        ...
        if (currentRoot != newRootDirectory)
        {
            ...
            if (path.isEmpty()) path = File::getSeparatorString();
            ...
            if (!rootPaths.contains(path, true))
            {
                ...
                if (!alreadyListed) currentPathBox.addItem(path, currentPathBox.getNumItems() + 2);
            }
        }
        ...
        if (auto* tree = dynamic_cast<FileTreeComponent*>(fileListComponent.get())) tree->refresh();

        auto currentRootName = currentRoot.getFullPathName();

        if (currentRootName.isEmpty()) currentRootName = File::getSeparatorString();
    ...
    {
        return isSaveMode() ? ((flags & canSelectDirectories) != 0 ? TRANS("Choose") : TRANS("Save")) : TRANS("Open");
    }
    ...
    {
        getLookAndFeel().layoutFileBrowserComponent(*this, fileListComponent.get(), previewComp, &currentPathBox,
                                                    &filenameBox, goUpButton.get());
    }
    ...
    void FileBrowserComponent::sendListenerChangeMessage()
    {
        ...
        if (previewComp != nullptr) previewComp->selectedFileChanged(getSelectedFile(0));
    ...
    void FileBrowserComponent::selectionChanged()
    {
        ...
        if (newFilenames.size() > 0) filenameBox.setText(newFilenames.joinIntoString(", "), false);
    ...
    void FileBrowserComponent::fileDoubleClicked(const File& f)
    {
        if (f.isDirectory())
        {
            ...
            if ((flags & canSelectDirectories) != 0 && (flags & doNotClearFileNameOnRootChange) == 0) filenameBox.setText({});
        }
        else
        {
            Component::BailOutChecker checker(this);
    ...
    bool FileBrowserComponent::keyPressed(const KeyPress& key)
    {
        ...
        if (key.getModifiers().isCommandDown() && (key.getKeyCode() == 'H' || key.getKeyCode() == 'h'))
        {
    ...
    void FileBrowserComponent::changeFilename()
    {
        if (filenameBox.getText().containsChar(File::getSeparatorChar()))
        {
            ...
            if (f.isDirectory())
            {
                ...
                if ((flags & doNotClearFileNameOnRootChange) == 0) filenameBox.setText({});
            }
            else
            {
        ...
        if (newText.isNotEmpty())
        {
            ...
            else
            {
                ...
                for (;;)
                {
                    ...
                    if (f.getParentDirectory() == f) break;
    ...
    void FileBrowserComponent::getDefaultRoots(StringArray& rootNames, StringArray& rootPaths)
    {
        ...
        for (int i = 0; i < roots.size(); ++i)
        {
            ...
            if (drive.isOnHardDisk())
            {
                ...
                if (volume.isEmpty()) volume = TRANS("Hard Drive");
    ...
    void FileBrowserComponent::timerCallback()
    {
        ...
        if (wasProcessActive != isProcessActive)
        {
            ...
            if (isProcessActive && fileList != nullptr) refresh();
        }
    }
