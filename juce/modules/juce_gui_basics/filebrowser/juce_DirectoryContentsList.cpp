...
namespace juce
{
    ...
    //==============================================================================
    void DirectoryContentsList::setDirectory(const File& directory, const bool includeDirectories, const bool includeFiles)
    {
        ...
        if (includeDirectories)
            newFlags |= File::findDirectories;
        else
            newFlags &= ~File::findDirectories;
        if (includeFiles)
            newFlags |= File::findFiles;
        else
            newFlags &= ~File::findFiles;
    ...
    File DirectoryContentsList::getFile(const int index) const
    {
        ...
        if (auto* info = files[index]) return root.getChildFile(info->filename);
    ...
    bool DirectoryContentsList::contains(const File& targetFile) const
    {
        ...
        for (int i = files.size(); --i >= 0;)
            if (root.getChildFile(files.getUnchecked(i)->filename) == targetFile) return true;
    ...
    int DirectoryContentsList::useTimeSlice()
    {
        ...
        for (int i = 100; --i >= 0;)
        {
            ...
            if (shouldStop || (Time::getApproximateMillisecondCounter() > startTime + 150)) break;
        }
    ...
    bool DirectoryContentsList::checkNextFile(bool& hasChanged)
    {
        if (fileFindHandle != nullptr)
        {
            ...
            if (fileFindHandle->next(&fileFoundIsDir, &isHidden, &fileSize, &modTime, &creationTime, &isReadOnly))
            {
                if (addFile(fileFindHandle->getFile(), fileFoundIsDir, fileSize, modTime, creationTime, isReadOnly))
                {
                    hasChanged = true;
                }
    ...
    bool DirectoryContentsList::addFile(const File& file, const bool isDir, const int64 fileSize, Time modTime,
                                        Time creationTime, const bool isReadOnly)
    {
        const ScopedLock sl(fileListLock);

        if (fileFilter == nullptr || ((!isDir) && fileFilter->isFileSuitable(file))
            || (isDir && fileFilter->isDirectorySuitable(file)))
        {
            ...
            for (int i = files.size(); --i >= 0;)
                if (files.getUnchecked(i)->filename == info->filename) return false;
