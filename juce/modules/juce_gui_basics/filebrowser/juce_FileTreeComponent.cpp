...
namespace juce
{
    ...
    //==============================================================================
    class FileListTreeItem : public TreeViewItem, private TimeSliceClient, private AsyncUpdater, private ChangeListener
    {
    public:
        FileListTreeItem(FileTreeComponent& treeComp, DirectoryContentsList* parentContents, int indexInContents,
                         const File& f, TimeSliceThread& t)
            : file(f)
            , owner(treeComp)
            ...
            if (isNowOpen)
            {
                ...
                if (isDirectory)
                {
                    if (subContentsList == nullptr)
                    {
                        ...
                        l->setDirectory(file, parentContentsList->isFindingDirectories(),
        ...
        void paintItem(Graphics& g, int width, int height) override
        {
            ...
            if (file != File())
            {
                ...
                if (icon.isNull()) thread.addTimeSliceClient(this);
            }

            owner.getLookAndFeel().drawFileBrowserRow(g, width, height, file, file.getFileName(), &icon, fileSize, modTime,
                                                      isDirectory, isSelected(), indexInContentsList, owner);
        }
        ...
        void updateIcon(const bool onlyUpdateIfCached)
        {
            if (icon.isNull())
            {
                ...
                if (im.isNull() && !onlyUpdateIfCached)
                {
                    ...
                    if (im.isValid()) ImageCache::addImageToCache(im, hashCode);
                }
    ...
    {
        if (auto* item = dynamic_cast<const FileListTreeItem*>(getSelectedItem(index))) return item->file;
    ...
    void FileTreeComponent::setItemHeight(int newHeight)
    {
        if (itemHeight != newHeight)
        {
            ...
            if (auto* root = getRootItem()) root->treeHasChanged();
        }
    }
