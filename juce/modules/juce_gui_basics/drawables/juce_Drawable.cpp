...
namespace juce
{
        ...
        : Component(other.getName())
    {
        ...
        if (auto* clipPath = other.drawableClipPath.get()) setClipPath(clipPath->createCopy());
    }
    ...
    void Drawable::applyDrawableClipPath(Graphics& g)
    {
        if (drawableClipPath != nullptr)
        {
            ...
            if (!clipPath.isEmpty()) g.getInternalContext().clipToPath(clipPath, {});
        }
    }
    ...
    void Drawable::nonConstDraw(Graphics& g, float opacity, const AffineTransform& transform)
    {
        ...
        g.addTransform(
            AffineTransform::translation((float)-(originRelativeToComponent.x), (float)-(originRelativeToComponent.y))
    ...
    void Drawable::drawWithin(Graphics& g, Rectangle<float> destArea, RectanglePlacement placement, float opacity) const
    {
    ...
    void Drawable::setBoundsToEnclose(Rectangle<float> area)
    {
        ...
        if (auto* parent = getParent()) parentOrigin = parent->originRelativeToComponent;
    ...
    bool Drawable::replaceColour(Colour original, Colour replacement)
    {
        ...
        for (auto* c : getChildren())
            if (auto* d = dynamic_cast<Drawable*>(c)) changed = d->replaceColour(original, replacement) || changed;
    ...
    {
        if (!area.isEmpty()) setTransform(placement.getTransformToFit(getDrawableBounds(), area));
    }
    ...
    Drawable* Drawable::createFromImageData(const void* data, const size_t numBytes)
    {
        ...
        else
        {
            ...
            if (outer != nullptr && outer->hasTagName("svg"))
            {
                ...
                if (svg != nullptr) result = Drawable::createFromSVG(*svg);
            }
        }
