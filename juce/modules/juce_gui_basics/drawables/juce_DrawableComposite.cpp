...
namespace juce
{
        ...
        , contentArea(other.contentArea)
    {
        for (auto* c : other.getChildren())
            if (auto* d = dynamic_cast<const Drawable*>(c)) addAndMakeVisible(d->createCopy());
    }
    ...
    void DrawableComposite::setBoundingBox(Parallelogram<float> newBounds)
    {
        if (bounds != newBounds)
        {
            ...
            auto t = AffineTransform::fromTargetPoints(contentArea.getTopLeft(), bounds.topLeft, contentArea.getTopRight(),
                                                       bounds.topRight, contentArea.getBottomLeft(), bounds.bottomLeft);

            if (t.isSingularity()) t = {};
    ...
    {
        if (auto* parent = getParent()) originRelativeToComponent = parent->originRelativeToComponent - getPosition();
    }
    ...
    void DrawableComposite::updateBoundsToFitChildren()
    {
        if (!updateBoundsReentrant)
        {
            ...
            for (auto* c : getChildren()) childArea = childArea.getUnion(c->getBoundsInParent());
            ...
            if (childArea != getBounds())
            {
                if (!delta.isOrigin())
                {
                    ...
                    for (auto* c : getChildren()) c->setBounds(c->getBounds() - delta);
                }
    ...
    Path DrawableComposite::getOutlineAsPath() const
    {
        ...
        for (auto* c : getChildren())
            if (auto* d = dynamic_cast<Drawable*>(c)) p.addPath(d->getOutlineAsPath());
