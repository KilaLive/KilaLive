...
namespace juce
{
    ...
    void DrawableShape::strokeChanged()
    {
        ...
        if (dashLengths.isEmpty())
            ...
            strokeType.createDashedStroke(strokePath, path, dashLengths.getRawDataPointer(), dashLengths.size(),
    ...
    {
        if (isStrokeVisible()) return strokePath.getBounds();
    ...
    bool DrawableShape::hitTest(int x, int y)
    {
        ...
        if (!allowsClicksOnThisComponent) return false;

        auto globalX = (float)(x - originRelativeToComponent.x);
        auto globalY = (float)(y - originRelativeToComponent.y);

        return path.contains(globalX, globalY) || (isStrokeVisible() && strokePath.contains(globalX, globalY));
    }
