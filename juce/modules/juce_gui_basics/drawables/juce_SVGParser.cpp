...
namespace juce
{
    ...
    public:
        ...
        struct XmlPath
        {
            ...
            bool applyOperationToChildWithID(const String& id, OperationType& op) const
            {
                forEachXmlChildElement(*xml, e)
                {
                    ...
                    if (e->compareAttribute("id", id) && !child->hasTagName("defs")) return op(child);

                    if (child.applyOperationToChildWithID(id, op)) return true;
                }
        ...
        struct GetFillTypeOp
        {
            ...
            {
                if (xml->hasTagNameIgnoringNamespace("linearGradient") || xml->hasTagNameIgnoringNamespace("radialGradient"))
                {
        ...
        Drawable* parseSVGElement(const XmlPath& xml)
        {
            ...
            if (xml->hasAttribute("transform")) newState.addTransform(xml);
            ...
            if (xml->hasAttribute("viewBox"))
            {
                ...
                if (parseCoords(viewParams, viewboxXY, true) && parseCoords(viewParams, vwh, true) && vwh.x > 0 && vwh.y > 0)
                {
        ...
        void parsePathString(Path& path, const String& pathString) const
        {
            ...
            while (!d.isEmpty())
            {
                ...
                case 'C':
                case 'c':
                    if (parseCoordsOrSkip(d, p1, false) && parseCoordsOrSkip(d, p2, false) && parseCoordsOrSkip(d, p3, false))
                    {
                        if (isRelative)
                        {
                            p1 += last;
                ...
                case 'S':
                case 's':
                    if (parseCoordsOrSkip(d, p1, false) && parseCoordsOrSkip(d, p3, false))
                    {
                        if (isRelative)
                        {
                            p1 += last;
                ...
                case 'Q':
                case 'q':
                    if (parseCoordsOrSkip(d, p1, false) && parseCoordsOrSkip(d, p2, false))
                    {
                        if (isRelative)
                        {
                            p1 += last;
                ...
                case 't':
                    if (parseCoordsOrSkip(d, p1, false))
                    {
                        ...
                        p2 = CharPointer_ASCII("QqTt").indexOf(previousCommand) >= 0 ? last + (last - last2) : p1;
                                ...
                                if (parseNextNumber(d, num, false))
                                {
                                    ...
                                    if (parseCoordsOrSkip(d, p2, false))
                                    {
                                        ...
                                        if (last != p2)
                                        {
                                            ...
                                            endpointToCentreParameters(last.x, last.y, p2.x, p2.y, angle, largeArc, sweep, rx,
                                                                       ry, centreX, centreY, startAngle, deltaAngle);

                                            path.addCentredArc((float)centreX, (float)centreY, (float)rx, (float)ry, angle,
                                                               (float)startAngle, (float)(startAngle + deltaAngle), false);
            ...
            if (path.getCurrentPosition() == subpathStart) path.closeSubPath();
        }
    ...
    private:
        ...
        static void setCommonAttributes(Drawable& d, const XmlPath& xml)
        {
            ...
            if (isNone(xml->getStringAttribute("display"))) d.setVisible(false);
        }
        ...
        void parseSubElements(const XmlPath& xml, DrawableComposite& parentDrawable, const bool shouldParseClip = true)
        {
            forEachXmlChildElement(*xml, e)
            {
                ...
                if (auto* drawable = parseSubElement(child))
                {
                    ...
                    if (!isNone(getStyleAttribute(child, "display"))) drawable->setVisible(true);

                    if (shouldParseClip) parseClipPath(child, *drawable);
                }
            }
        }
        ...
        Drawable* parseSubElement(const XmlPath& xml)
        {
            {
                Path path;
                if (parsePathElement(xml, path)) return parseShape(xml, path);
            }
        ...
        {
            if (auto* group = xml->getChildByName("g")) return parseGroupElement(xml.getChild(group), true);
        ...
        void parsePath(const XmlPath& xml, Path& path) const
        {
            ...
            if (getStyleAttribute(xml, "fill-rule").trim().equalsIgnoreCase("evenodd")) path.setUsingNonZeroWinding(false);
        }
        ...
        void parseRect(const XmlPath& xml, Path& rect) const
        {
            ...
            if (hasRX || hasRY)
            {
                ...
                rect.addRoundedRectangle(getCoordLength(xml, "x", viewBoxW), getCoordLength(xml, "y", viewBoxH),
                                         getCoordLength(xml, "width", viewBoxW), getCoordLength(xml, "height", viewBoxH), rx,
                                         ry);
            }
            else
            {
                rect.addRectangle(getCoordLength(xml, "x", viewBoxW), getCoordLength(xml, "y", viewBoxH),
                                  getCoordLength(xml, "width", viewBoxW), getCoordLength(xml, "height", viewBoxH));
            }
        }
        ...
        void parsePolygon(const XmlPath& xml, const bool isPolyline, Path& path) const
        {
            ...
            if (parseCoords(points, p, true))
            {
                ...
                if ((!isPolyline) || first == last) path.closeSubPath();
            }
        }
        ...
        static String getLinkedID(const XmlPath& xml)
        {
            ...
            if (link.startsWithChar('#')) return link.substring(1);
        ...
        static String parseURL(const String& str)
        {
            if (str.startsWithIgnoreCase("url"))
                return str.fromFirstOccurrenceOf("#", false, false).upToLastOccurrenceOf(")", false, false).trim();

            return {};
        }

        //==============================================================================
        Drawable* parseShape(const XmlPath& xml, Path& path, const bool shouldParseTransform = true,
                             AffineTransform* additonalTransform = nullptr) const
        {
            ...
            if (additonalTransform != nullptr) path.applyTransform(*additonalTransform);

            dp->setPath(path);

            dp->setFill(getPathFillType(path, xml, "fill", getStyleAttribute(xml, "fill-opacity"),
                                        getStyleAttribute(xml, "opacity"),
                                        pathContainsClosedSubPath(path) ? Colours::black : Colours::transparentBlack));

            auto strokeType = getStyleAttribute(xml, "stroke");

            if (strokeType.isNotEmpty() && !isNone(strokeType))
            {
                dp->setStrokeFill(getPathFillType(path, xml, "stroke", getStyleAttribute(xml, "stroke-opacity"),
                                                  getStyleAttribute(xml, "opacity"), Colours::transparentBlack));
            ...
            if (strokeDashArray.isNotEmpty()) parseDashArray(strokeDashArray, *dp);
        ...
        static bool pathContainsClosedSubPath(const Path& path) noexcept
        {
            for (Path::Iterator iter(path); iter.next();)
                if (iter.elementType == Path::Iterator::closePath) return true;
        ...
        {
            if (dashList.equalsIgnoreCase("null") || isNone(dashList)) return;
            ...
            {
                float value;
                if (!parseCoord(t, value, true, true)) break;
            ...
            if (dashLengths.size() > 0)
            {
                ...
                for (int i = 0; i < dashLengths.size(); ++i)
                {
                    if (dashes[i] <= 0)  // SVG uses zero-length dashes to mean a dotted line
                    {
                        ...
                        if (isPositiveAndBelow(pairedIndex, dashLengths.size()) && dashes[pairedIndex] > nonZeroLength)
        ...
        bool addGradientStopsIn(ColourGradient& cg, const XmlPath& fillXml) const
        {
            ...
            if (fillXml.xml != nullptr)
            {
                forEachXmlChildElementWithTagName(*fillXml, e, "stop")
                {
                    ...
                    if (e->getStringAttribute("offset").containsChar('%')) offset *= 0.01;
        ...
        FillType getGradientFillType(const XmlPath& fillXml, const Path& path, const float opacity) const
        {
            ...
            {
                if (gradient.getColourPosition(0) > 0) gradient.addColour(0.0, gradient.getColour(0));
            ...
            if (opacity < 1.0f) gradient.multiplyOpacity(opacity);
            ...
            if (gradient.isRadial)
            {
                if (userSpace)
                    ...
                    gradient.point1.setXY(
                        dx + gradientWidth * getCoordLength(fillXml->getStringAttribute("cx", "50%"), 1.0f),
            ...
            else
            {
                ...
                {
                    gradient.point1.setXY(
                        dx + gradientWidth * getCoordLength(fillXml->getStringAttribute("x1", "0%"), 1.0f),
                        dy + gradientHeight * getCoordLength(fillXml->getStringAttribute("y1", "0%"), 1.0f));

                    gradient.point2.setXY(
                        dx + gradientWidth * getCoordLength(fillXml->getStringAttribute("x2", "100%"), 1.0f),
                        dy + gradientHeight * getCoordLength(fillXml->getStringAttribute("y2", "0%"), 1.0f));
                }

                if (gradient.point1 == gradient.point2) return Colour(gradient.getColour(gradient.getNumColours() - 1));
            }

            FillType type(gradient);

            auto gradientTransform = parseTransform(fillXml->getStringAttribute("gradientTransform")).followedBy(transform);
            ...
            else
            {
                ...
                auto perpendicular =
                    Point<float>(gradient.point2.y - gradient.point1.y, gradient.point1.x - gradient.point2.x)
                ...
                const float scale =
                    perpendicular.getDotProduct(newGradPoint2 - newGradPoint1) / perpendicular.getDotProduct(perpendicular);
        ...
        FillType getPathFillType(const Path& path, const XmlPath& xml, StringRef fillAttribute, const String& fillOpacity,
                                 const String& overallOpacity, const Colour defaultColour) const
        {
            float opacity = 1.0f;

            if (overallOpacity.isNotEmpty()) opacity = jlimit(0.0f, 1.0f, overallOpacity.getFloatValue());

            if (fillOpacity.isNotEmpty()) opacity *= (jlimit(0.0f, 1.0f, fillOpacity.getFloatValue()));
            ...
            if (urlID.isNotEmpty())
            {
                ...
                if (topLevelXml.applyOperationToChildWithID(urlID, op)) return op.fillType;
            }

            if (isNone(fill)) return Colours::transparentBlack;
        ...
        Drawable* useText(const XmlPath& xml) const
        {
            ...
            if (linkedID.isNotEmpty()) topLevelXml.applyOperationToChildWithID(linkedID, op);
                            ...
                            AffineTransform* additonalTransform = nullptr) const
        {
            ...
            if (xml->hasTagName("use")) return useText(xml);

            if (!xml->hasTagName("text")) return nullptr;
            ...
            forEachXmlChildElement(*xml, e)
            {
                if (e->isTextElement())
                {
                    ...
                    Rectangle<float> bounds(xCoords[0], yCoords[0] - font.getAscent(), font.getStringWidthFloat(text),
                                            font.getHeight());
        ...
        Font getFont(const XmlPath& xml) const
        {
            ...
            if (family.isNotEmpty()) f.setTypefaceName(family);

            if (getStyleAttribute(xml, "font-style").containsIgnoreCase("italic")) f.setItalic(true);

            if (getStyleAttribute(xml, "font-weight").containsIgnoreCase("bold")) f.setBold(true);
                             ...
                             AffineTransform* additionalTransform = nullptr) const
        {
            ...
            if (xml->hasTagName("use")) return useImage(xml);

            if (!xml->hasTagName("image")) return nullptr;
            ...
            else
            {
                ...
                if (linkedFile.existsAsFile()) inputStream.reset(linkedFile.createInputStream());
            }
        ...
        {
            transform = parseTransform(xml->getStringAttribute("transform")).followedBy(transform);
        }
        ...
        {
            return parseCoord(s, p.x, allowUnits, true) && parseCoord(s, p.y, allowUnits, false);
        }

        bool parseCoordsOrSkip(String::CharPointerType& s, Point<float>& p, const bool allowUnits) const
        {
            if (parseCoords(s, p, allowUnits)) return true;
        ...
        void getCoordList(Array<float>& coords, const String& list, bool allowUnits, const bool isX) const
        {
            ...
            while (parseCoord(text, value, allowUnits, isX)) coords.add(value);
        }
        ...
        {
            if (auto* style = xml->getChildByName("style")) parseCSSStyle(xml.getChild(style));
        }
        ...
        static String::CharPointerType findStyleItem(String::CharPointerType source, String::CharPointerType name)
        {
            ...
            {
                if (source.getAndAdvance() == '.' && CharacterFunctions::compareIgnoreCaseUpTo(source, name, nameLength) == 0)
                {
                    auto endOfName = (source + nameLength).findEndOfWhitespace();

                    if (*endOfName == '{') return endOfName;

                    if (*endOfName == ',') return CharacterFunctions::find(endOfName, (juce_wchar)'{');
                }
            }
        ...
        {
            if (xml->hasAttribute(attributeName)) return xml->getStringAttribute(attributeName, defaultValue);
            ...
            else if (xml->hasAttribute("class"))
            {
                for (auto i = cssStyleText.getCharPointer();;)
                {
                    ...
                    if (openBrace.isEmpty()) break;

                    auto closeBrace = CharacterFunctions::find(openBrace, (juce_wchar)'}');

                    if (closeBrace.isEmpty()) break;

                    auto value = getAttributeFromStyleList(String(openBrace + 1, closeBrace), attributeName, defaultValue);
                    if (value.isNotEmpty()) return value;
            ...
            if (xml.parent != nullptr) return getStyleAttribute(*xml.parent, attributeName, defaultValue);
        ...
        {
            if (xml->hasAttribute(attributeName)) return xml->getStringAttribute(attributeName);

            if (xml.parent != nullptr) return getInheritedAttribute(*xml.parent, attributeName);
        ...
        static int parsePlacementFlags(const String& align) noexcept
        {
            ...
            if (isNone(align)) return RectanglePlacement::stretchToFit;

            return (align.containsIgnoreCase("slice") ? RectanglePlacement::fillDestination : 0)
                   | (align.containsIgnoreCase("xMin")
                          ? RectanglePlacement::xLeft
                          : (align.containsIgnoreCase("xMax") ? RectanglePlacement::xRight : RectanglePlacement::xMid))
                   | (align.containsIgnoreCase("yMin")
                          ? RectanglePlacement::yTop
                          : (align.containsIgnoreCase("yMax") ? RectanglePlacement::yBottom : RectanglePlacement::yMid));
        }
        ...
        Colour parseColour(const XmlPath& xml, StringRef attributeName, const Colour defaultColour) const
        {
            ...
            if (text.startsWithChar('#'))
            {
                ...
                if (numChars <= 3) return Colour((uint8)(hex[0] * 0x11), (uint8)(hex[1] * 0x11), (uint8)(hex[2] * 0x11));

                return Colour((uint8)((hex[0] << 4) + hex[1]), (uint8)((hex[2] << 4) + hex[3]),
                              (uint8)((hex[4] << 4) + hex[5]));
            }
            ...
            if (text.startsWith("rgb"))
            {
                ...
                if (openBracket >= 3 && closeBracket > openBracket)
                {
                    ...
                    return Colour((uint8)tokens[0].getIntValue(), (uint8)tokens[1].getIntValue(),
        ...
        static AffineTransform parseTransform(String t)
        {
            ...
            {
                StringArray tokens;
                tokens.addTokens(t.fromFirstOccurrenceOf("(", false, false).upToFirstOccurrenceOf(")", false, false), ", ",
                                 "");
                ...
                for (int i = 0; i < numElementsInArray(numbers); ++i) numbers[i] = tokens[i].getFloatValue();
        ...
        static void endpointToCentreParameters(double x1, double y1, double x2, double y2, double angle, bool largeArc,
                                               bool sweep, double& rx, double& ry, double& centreX, double& centreY,
                                               double& startAngle, double& deltaAngle) noexcept
        {
            ...
            if (uy < 0) startAngle = -startAngle;

            startAngle += MathConstants<double>::halfPi;

            deltaAngle = acos(jlimit(-1.0, 1.0, ((ux * vx) + (uy * vy)) / (length * juce_hypot(vx, vy))));

            if ((ux * vy) - (uy * vx) < 0) deltaAngle = -deltaAngle;

            if (sweep)
            {
                if (deltaAngle < 0) deltaAngle += MathConstants<double>::twoPi;
            }
            else
            {
                if (deltaAngle > 0) deltaAngle -= MathConstants<double>::twoPi;
    ...
    {
        if (!svgDocument.hasTagNameIgnoringNamespace("svg")) return nullptr;
