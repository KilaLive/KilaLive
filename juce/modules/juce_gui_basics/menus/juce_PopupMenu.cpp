...
namespace juce
{
    ...
    struct PopupMenu::HelperClasses
    {
        ...
        static const Colour* getColour(const PopupMenu::Item& item) noexcept
        {
            return item.colour != Colour() ? &item.colour : nullptr;
        }
        static bool hasSubMenu(const PopupMenu::Item& item) noexcept
        {
        ...
        struct ItemComponent : public Component
        {
            ...
            {
                if (item.isSectionHeader) customComp = new HeaderItemComponent(item.text);
            ...
            void getIdealSize(int& idealWidth, int& idealHeight, const int standardItemHeight)
            {
                if (customComp != nullptr)
                    ...
                    getLookAndFeel().getIdealPopupMenuItemSize(getTextForMeasurement(), item.isSeparator, standardItemHeight,
            ...
            {
                if (customComp == nullptr)
                    getLookAndFeel().drawPopupMenuItem(g, getLocalBounds(), item.isSeparator, item.isEnabled, isHighlighted,
                                                       item.isTicked, hasSubMenu(item), item.text,
                                                       item.shortcutKeyDescription, item.image.get(), getColour(item));
            }
            ...
            void setHighlighted(bool shouldBeHighlighted)
            {
                ...
                if (isHighlighted != shouldBeHighlighted)
                {
                    ...
                    if (customComp != nullptr) customComp->setHighlighted(shouldBeHighlighted);
        ...
        private:
            ...
            {
                if (item.commandManager != nullptr && item.itemID != 0 && item.shortcutKeyDescription.isEmpty())
                {
                    String shortcutKey;

                    for (auto& keypress : item.commandManager->getKeyMappings()->getKeyPressesAssignedToCommand(item.itemID))
                    {
            ...
            {
                return item.shortcutKeyDescription.isNotEmpty() ? item.text + "   " + item.shortcutKeyDescription : item.text;
            }
        ...
        {
        public:
            MenuWindow(const PopupMenu& menu, MenuWindow* parentWindow, const Options& opts, bool alignToRectangle,
                       bool shouldDismissOnMouseUp, ApplicationCommandManager** manager, float parentScaleFactor = 1.0f)
                : Component("menu")
                , parent(parentWindow)
                , options(opts)
                ...
                , scaleFactor(parentWindow != nullptr ? parentScaleFactor : 1.0f)
            {
                ...
                setLookAndFeel(parent != nullptr ? &(parent->getLookAndFeel()) : menu.lookAndFeel.get());

                auto& lf = getLookAndFeel();

                parentComponent = lf.getParentComponentForMenuOptions(options);

                if (parentComponent == nullptr && parentWindow == nullptr
                ...
                {
                    auto targetPosition = parentComponent != nullptr
                                              ? parentComponent->getLocalPoint(nullptr, targetArea.getTopLeft())
                ...
                getMouseState(Desktop::getInstance()
                                  .getMainMouseSource());  // forces creation of a mouse source watcher for the main mouse
            }
            ...
            void paint(Graphics& g) override
            {
                if (isOpaque()) g.fillAll(Colours::white);
            ...
            void hide(const PopupMenu::Item* item, bool makeInvisible)
            {
                if (isVisible())
                {
                    ...
                    if (item != nullptr && item->commandManager != nullptr && item->itemID != 0)
                    {
                    ...
                    if (makeInvisible && (deletionChecker != nullptr)) setVisible(false);
                }
            }
            ...
            static int getResultItemID(const PopupMenu::Item* item)
            {
                ...
                if (auto* cc = item->customCallback.get())
                    if (!cc->menuItemTriggered()) return 0;
            ...
            bool keyPressed(const KeyPress& key) override
            {
                ...
                else if (key.isKeyCode(KeyPress::leftKey))
                {
                    if (parent != nullptr)
                    {
                        ...
                        if (parentWindow != nullptr) parentWindow->setCurrentlyHighlightedChild(currentChildOfParent);
            ...
            void inputAttemptWhenModal() override
            {
                ...
                for (auto* ms : mouseSourceStates)
                {
                    ...
                    if (deletionChecker == nullptr) return;
                }
            ...
            void handleCommandMessage(int commandId) override
            {
                ...
                if (commandId == PopupMenuSettings::dismissCommandId) dismissMenu(nullptr);
            }
            ...
            bool windowIsStillValid()
            {
                ...
                if (auto* currentlyModalWindow = dynamic_cast<MenuWindow*>(Component::getCurrentlyModalComponent()))
                    if (!treeContains(currentlyModalWindow)) return false;
            ...
            {
                return parent != nullptr ? parent->isOverAnyMenu() : isOverChildren();
            }

            bool isOverChildren() const
            {
                return isVisible() && (isAnyMouseOver() || (activeSubMenu != nullptr && activeSubMenu->isOverChildren()));
            }

            bool isAnyMouseOver() const
            {
                for (auto* ms : mouseSourceStates)
                    if (ms->isOver()) return true;
            ...
            bool treeContains(const MenuWindow* const window) const noexcept
            {
                ...
                while (mw->parent != nullptr) mw = mw->parent;

                while (mw != nullptr)
                {
                    if (mw == window) return true;
            ...
            {
                auto parentArea = Desktop::getInstance()
                                      .getDisplays()
                ...
                if (parentComponent == nullptr) return parentArea;

                return parentComponent->getLocalArea(nullptr, parentComponent->getScreenBounds()
            ...
            void calculateWindowPos(Rectangle<int> target, const bool alignToRectangle)
            {
                ...
                else
                {
                    ...
                    auto biggestSpace =
                        jmax(parentArea.getRight() - target.getRight(), target.getX() - parentArea.getX()) - 32;

                    if (biggestSpace < widthToUse)
                    {
                        layoutMenuItems(biggestSpace + target.getWidth() / 3, maxMenuHeight, widthToUse, heightToUse);

                        if (numColumns > 1) layoutMenuItems(biggestSpace - 4, maxMenuHeight, widthToUse, heightToUse);
                    ...
                    if (getLookAndFeel().getPopupMenuBorderSize()
                        == 0)  // workaround for dismissing the window on mouse up when border size is 0
                ...
                hideOnExit = parent != nullptr && parent->windowPos.intersects(windowPos.expanded(-4, -4));
            }
            ...
            void layoutMenuItems(const int maxMenuW, const int maxMenuH, int& width, int& height)
            {
                ...
                for (;;)
                {
                    ...
                    if (totalW > maxMenuW / 2 || contentHeight < maxMenuH || numColumns >= maximumNumColumns) break;
            ...
            int workOutBestSize(const int maxMenuW)
            {
                ...
                for (int col = 0; col < numColumns; ++col)
                {
                    ...
                    auto numChildren = jmin(items.size() - childNum, (items.size() + numColumns - 1) / numColumns);
                ...
                if (totalW < minWidth)
                {
                    ...
                    for (int col = 0; col < numColumns; ++col) columnWidths.set(0, totalW / numColumns);
                }
            ...
            void ensureItemIsVisible(const int itemID, int wantedY)
            {
                ...
                for (int i = items.size(); --i >= 0;)
                {
                    ...
                    {
                        if (m->item.itemID == itemID && windowPos.getHeight() > PopupMenuSettings::scrollZone * 4)
                        {
                            ...
                            if (wantedY > 0 || currentY < 0 || m->getBottom() > windowPos.getHeight())
                            {
                                ...
                                auto newY = jlimit(parentArea.getY(), parentArea.getBottom() - windowPos.getHeight(),
            ...
            void resizeToBestWindowPos()
            {
                ...
                else if (childYOffset > 0)
                {
                    ...
                    if (spaceAtBottom > 0) r.setSize(r.getWidth(), r.getHeight() - spaceAtBottom);
                }
            ...
            void alterChildYPos(int delta)
            {
                if (canScroll())
                {
                    ...
                    if (delta < 0)
                        ...
                        childYOffset = jmin(
                            childYOffset, contentHeight - windowPos.getHeight() + getLookAndFeel().getPopupMenuBorderSize());
            ...
            {
                if (currentChild != nullptr) currentChild->setHighlighted(false);
            ...
            bool showSubMenuFor(ItemComponent* childComp)
            {
                ...
                {
                    activeSubMenu.reset(
                        new HelperClasses::MenuWindow(*(childComp->item.subMenu), this,
                    ...
                    activeSubMenu->setVisible(
                        true);  // (must be called before enterModalState on Windows to avoid DropShadower confusion)
            ...
            void disableTimerUntilMouseMoves()
            {
                ...
                if (parent != nullptr) parent->disableTimerUntilMouseMoves();
            }
            ...
            static float getApproximateScaleFactorForTargetComponent(Component* targetComponent)
            {
                ...
                for (auto* target = targetComponent; target != nullptr; target = target->getParentComponent())
                {
                    ...
                    if (target->isOnDesktop()) transform = transform.scaled(target->getDesktopScaleFactor());
                }
        ...
        public:
            ...
            void handleMouseEvent(const MouseEvent& e)
            {
                if (!window.windowIsStillValid()) return;
            ...
            void timerCallback() override
            {
                ...
                if ((source.isTouch() || source.isPen()) && !isValidMousePosition()) return;
#endif

                if (window.windowIsStillValid()) handleMousePosition(source.getScreenPosition().roundToInt());
            }
        ...
        private:
            ...
            void handleMousePosition(Point<int> globalMousePos)
            {
                ...
                if (timeNow > window.timeEnteredCurrentChildComp + 100 && window.reallyContains(localMousePos, true)
                    && window.currentChild != nullptr && !(window.disableMouseMoves || window.isSubMenuVisible()))
                {
            ...
            void checkButtonState(Point<int> localMousePos, const uint32 timeNow, const bool wasDown,
                                  const bool overScrollArea, const bool isOverAny)
            {
            ...
            void highlightItemUnderMouse(Point<int> globalMousePos, Point<int> localMousePos, const uint32 timeNow)
            {
                if (globalMousePos != lastMousePos || timeNow > lastMouseMoveTime + 350)
                {
                    ...
                    if (isMouseOver) window.hasBeenOver = true;

                    if (lastMousePos.getDistanceFrom(globalMousePos) > 2)
                    {
                        lastMouseMoveTime = timeNow;

                        if (window.disableMouseMoves && isMouseOver) window.disableMouseMoves = false;
                    }

                    if (window.disableMouseMoves
                        || (window.activeSubMenu != nullptr && window.activeSubMenu->isOverChildren()))
                        return;

                    const bool isMovingTowardsMenu =
                        isMouseOver && globalMousePos != lastMousePos && isMovingTowardsSubmenu(globalMousePos);
                    ...
                    if (!isMovingTowardsMenu)
                    {
                            ...
                            && (isMouseOver || (window.activeSubMenu == nullptr) || !window.activeSubMenu->isVisible()))
                        {
                            ...
                            if (!isMouseOver) itemUnderMouse = nullptr;
            ...
            {
                if (window.activeSubMenu == nullptr) return false;
                ...
                {
                    oldGlobalPos -=
                        Point<int>(2, 0);  // to enlarge the triangle a bit, in case the mouse only moves a couple of pixels
                }
                else
                {
                ...
                areaTowardsSubMenu.addTriangle((float)oldGlobalPos.x, (float)oldGlobalPos.y, subX,
                                               (float)itemScreenBounds.getY(), subX, (float)itemScreenBounds.getBottom());
            ...
            {
                if (window.canScroll() && isPositiveAndBelow(localMousePos.x, window.getWidth())
                    && (isPositiveAndBelow(localMousePos.y, window.getHeight()) || source.isDragging()))
                {
                    ...
                    if (window.isBottomScrollZoneActive()
            ...
            bool isValidMousePosition()
            {
                ...
                if (localPos.x < 0 && localPos.y < 0) return false;
        ...
        struct NormalComponentWrapper : public PopupMenu::CustomComponent
        {
            ...
            void resized() override
            {
                if (auto* child = getChildComponent(0)) child->setBounds(getLocalBounds());
            }
    ...
    void PopupMenu::addItem(const Item& newItem)
    {
        ...
        jassert(newItem.itemID != 0 || newItem.isSeparator || newItem.isSectionHeader || newItem.subMenu != nullptr);
    ...
    void PopupMenu::addCommandItem(ApplicationCommandManager* commandManager, const CommandID commandID,
                                   const String& displayName, Drawable* iconToUse)
    {
    ...
    void PopupMenu::addColouredItem(int itemResultID, const String& itemText, Colour itemTextColour, bool isActive,
                                    bool isTicked, Drawable* iconToUse)
    {
        Item i;
        i.text      = itemText;
        i.itemID    = itemResultID;
    ...
    void PopupMenu::addColouredItem(int itemResultID, const String& itemText, Colour itemTextColour, bool isActive,
                                    bool isTicked, const Image& iconToUse)
    {
        Item i;
        i.text      = itemText;
        i.itemID    = itemResultID;
    ...
    void PopupMenu::addSubMenu(const String& subMenuName, const PopupMenu& subMenu, bool isActive, const Image& iconToUse,
                               bool isTicked, int itemResultID)
    {
        addSubMenu(subMenuName, subMenu, isActive, createDrawableFromImage(iconToUse), isTicked, itemResultID);
    }

    void PopupMenu::addSubMenu(const String& subMenuName, const PopupMenu& subMenu, bool isActive, Drawable* iconToUse,
                               bool isTicked, int itemResultID)
    {
        Item i;
        i.text   = subMenuName;
    ...
    PopupMenu::Options PopupMenu::Options::withTargetComponent(Component* comp) const noexcept
    {
        ...
        if (comp != nullptr) o.targetArea = comp->getScreenBounds();
    ...
    Component* PopupMenu::createWindow(const Options& options, ApplicationCommandManager** managerOfChosenCommand) const
    {
        return items.isEmpty() ? nullptr
                               : new HelperClasses::MenuWindow(
                                     *this, nullptr, options, !options.getTargetScreenArea().isEmpty(),
                                     ModifierKeys::getCurrentModifiers().isAnyMouseButtonDown(), managerOfChosenCommand);
    }
    ...
    struct PopupMenuCompletionCallback : public ModalComponentManager::Callback
    {
        ...
        void modalStateFinished(int result) override
        {
            ...
            {
                if (prevTopLevel != nullptr) prevTopLevel->toFront(true);

                if (prevFocused != nullptr && prevFocused->isShowing()) prevFocused->grabKeyboardFocus();
            }
        }
                                            ...
                                            const bool canBeModal)
    {
        ...
        if (auto* window = createWindow(options, &(callback->managerOfChosenCommand)))
        {
            ...
            if (userCallback == nullptr && canBeModal) return window->runModalLoop();
#else
...
//==============================================================================
#if JUCE_MODAL_LOOPS_PERMITTED
    int PopupMenu::show(int itemIDThatMustBeVisible, int minimumWidth, int maximumNumColumns, int standardItemHeight,
                        ModalComponentManager::Callback* callback)
    {
        return showWithOptionalCallback(Options()
    ...
    int PopupMenu::showAt(Rectangle<int> screenAreaToAttachTo, int itemIDThatMustBeVisible, int minimumWidth,
                          int maximumNumColumns, int standardItemHeight, ModalComponentManager::Callback* callback)
    {
        return showWithOptionalCallback(Options()
                                            .withTargetScreenArea(screenAreaToAttachTo)
                                            .withItemThatMustBeVisible(itemIDThatMustBeVisible)
                                            .withMinimumWidth(minimumWidth)
                                            .withMaximumNumColumns(maximumNumColumns)
                                            .withStandardItemHeight(standardItemHeight),
    ...
    int PopupMenu::showAt(Component* componentToAttachTo, int itemIDThatMustBeVisible, int minimumWidth,
                          int maximumNumColumns, int standardItemHeight, ModalComponentManager::Callback* callback)
    {
        ...
        if (componentToAttachTo != nullptr) options = options.withTargetComponent(componentToAttachTo);
    ...
    bool JUCE_CALLTYPE PopupMenu::dismissAllActiveMenus()
    {
        ...
        for (int i = numWindows; --i >= 0;)
            if (auto* pmw = windows[i]) pmw->dismissMenu(nullptr);
    ...
    bool PopupMenu::containsAnyActiveItems() const noexcept
    {
        for (auto* mi : items)
        {
            ...
            {
                if (mi->subMenu->containsAnyActiveItems()) return true;
            }
    ...
    //==============================================================================
    PopupMenu::MenuItemIterator::MenuItemIterator(const PopupMenu& m, bool recurse)
        : searchRecursively(recurse)
    {
        index.add(0);
        menus.add(&m);
    }
    ...
    {
        if (index.size() == 0 || menus.getLast()->items.size() == 0) return false;
        ...
        while (index.size() > 0 && index.getLast() >= menus.getLast()->items.size())
        {
            ...
            if (index.size() > 0) index.setUnchecked(index.size() - 1, index.getLast() + 1);
        }
