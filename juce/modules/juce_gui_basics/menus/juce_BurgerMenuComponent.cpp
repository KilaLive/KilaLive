...
namespace juce
{
    ...
    struct CustomMenuBarItemHolder : public Component
    {
        ...
        void update(const ReferenceCountedObjectPtr<PopupMenu::CustomComponent>& newComponent)
        {
            ...
            {
                if (custom != nullptr) removeChildComponent(custom);
    ...
    void BurgerMenuComponent::paintListBoxItem(int rowIndex, Graphics& g, int w, int h, bool highlight)
    {
        ...
        auto row = (rowIndex < rows.size() ? rows.getReference(rowIndex) : Row{true, 0, {}});
        ...
        else
        {
            ...
            if (item.customComponent == nullptr)
                lf.drawPopupMenuItem(g, r.reduced(20, 0), item.isSeparator, item.isEnabled, highlight, item.isTicked,
                                     hasSubMenu(item), item.text, item.shortcutKeyDescription, item.image.get(), colour);
        }
    }
    ...
    Component* BurgerMenuComponent::refreshComponentForRow(int rowIndex, bool isRowSelected, Component* existing)
    {
        ...
        if (existing == nullptr && hasCustomComponent) return new CustomMenuBarItemHolder(row.item.customComponent);
    ...
    void BurgerMenuComponent::mouseUp(const MouseEvent& event)
    {
        ...
        if (rowIndex == lastRowClicked && rowIndex < rows.size() && event.source.getIndex() == inputSourceIndexOfLastClick)
        {
