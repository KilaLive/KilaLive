...
namespace juce
{
    ...
    void MenuBarComponent::setModel(MenuBarModel* const newModel)
    {
        if (model != newModel)
        {
            if (model != nullptr) model->removeListener(this);

            model = newModel;

            if (model != nullptr) model->addListener(this);
    ...
    void MenuBarComponent::paint(Graphics& g)
    {
        ...
        if (model != nullptr)
        {
            for (int i = 0; i < menuNames.size(); ++i)
            {
                ...
                getLookAndFeel().drawMenuBarItem(g, xPositions[i + 1] - xPositions[i], getHeight(), i, menuNames[i],
                                                 i == itemUnderMouse, i == currentPopupIndex, isMouseOverBar, *this);
            }
        }
    }
    ...
    int MenuBarComponent::getItemAt(Point<int> p)
    {
        for (int i = 0; i < xPositions.size(); ++i)
            if (p.x >= xPositions[i] && p.x < xPositions[i + 1]) return reallyContains(p, true) ? i : -1;
    ...
    void MenuBarComponent::showMenu(int index)
    {
        if (index != currentPopupIndex)
        {
            ...
            if (index >= 0)
            {
                PopupMenu m(model->getMenuForIndex(itemUnderMouse, menuNames[itemUnderMouse]));

                if (m.lookAndFeel == nullptr) m.setLookAndFeel(&getLookAndFeel());
    ...
    {
        if (bar != nullptr) bar->menuDismissed(topLevelIndex, result);
    }
    ...
    void MenuBarComponent::handleCommandMessage(int commandId)
    {
        ...
        if (currentPopupIndex == topLevelIndexClicked) setOpenItem(-1);

        if (commandId != 0 && model != nullptr) model->menuItemSelected(commandId, topLevelIndexClicked);
    }

    //==============================================================================
    void MenuBarComponent::mouseEnter(const MouseEvent& e)
    {
        if (e.eventComponent == this) updateItemUnderMouse(e.getPosition());
    }
    ...
    void MenuBarComponent::menuBarItemsChanged(MenuBarModel* /*menuBarModel*/)
    {
        ...
        if (model != nullptr) newNames = model->getMenuBarNames();
    ...
    {
        if (model == nullptr || (info.commandFlags & ApplicationCommandInfo::dontTriggerVisualFeedback) != 0) return;
