...
namespace juce
{
    class TableListBox::RowComp : public Component, public TooltipClient
    {
    public:
            ...
            if (auto* tableModel = owner.getModel())
            {
                ...
                for (int i = 0; i < numColumns; ++i)
                {
                    if (columnComponents[i] == nullptr)
                    {
                        ...
                        if (columnRect.getX() >= clipBounds.getRight()) break;
                        ...
                        if (columnRect.getRight() > clipBounds.getX())
                        {
                            ...
                            if (g.reduceClipRegion(columnRect))
                            {
                                ...
                                tableModel->paintCell(g, row, headerComp.getColumnIdOfIndex(i, true), columnRect.getWidth(),
                                                      columnRect.getHeight(), isSelected);
                            }
                        }
                    }
                }
        ...
        {
            for (int i = columnComponents.size(); --i >= 0;) resizeCustomComp(i);
        }

        void resizeCustomComp(int index)
        {
            if (auto* c = columnComponents.getUnchecked(index))
                c->setBounds(owner.getHeader().getColumnPosition(index).withY(0).withHeight(getHeight()));
        }
        ...
        void mouseDown(const MouseEvent& e) override
        {
            ...
            if (isEnabled())
            {
                if (!isSelected)
                {
                    ...
                    if (columnId != 0)
                        if (auto* m = owner.getModel()) m->cellClicked(row, columnId, e);
                }
                else
                {
        ...
        {
            if (isEnabled() && owner.getModel() != nullptr && e.mouseWasDraggedSinceMouseDown() && !isDragging)
            {
        ...
        void mouseUp(const MouseEvent& e) override
        {
            if (selectRowOnMouseUp && e.mouseWasClicked() && isEnabled())
            {
                ...
                if (columnId != 0)
                    if (TableListBoxModel* m = owner.getModel()) m->cellClicked(row, columnId, e);
            }
        }
        ...
        void mouseDoubleClick(const MouseEvent& e) override
        {
            ...
            if (columnId != 0)
                if (auto* m = owner.getModel()) m->cellDoubleClicked(row, columnId, e);
        }
        ...
        String getTooltip() override
        {
            ...
            if (columnId != 0)
                if (auto* m = owner.getModel()) return m->getCellTooltip(row, columnId);
    ...
    private:
        ...
        enum
        {
            autoSizeColumnId = 0xf836743,
            autoSizeAllId    = 0xf836744
        };
    ...
    void TableListBox::setHeader(TableHeaderComponent* newHeader)
    {
        ...
        if (header != nullptr) newBounds = header->getBounds();
    ...
    void TableListBox::autoSizeColumn(int columnId)
    {
        ...
        if (width > 0) header->setColumnWidth(columnId, width);
    }

    void TableListBox::autoSizeAllColumns()
    {
        for (int i = 0; i < header->getNumColumns(true); ++i) autoSizeColumn(header->getColumnIdOfIndex(i, true));
    }
    ...
    Rectangle<int> TableListBox::getCellPosition(int columnId, int rowNumber, bool relativeToComponentTopLeft) const
    {
        ...
        if (relativeToComponentTopLeft) headerCell.translate(header->getX(), 0);
    ...
    {
        if (existingComponentToUpdate == nullptr) existingComponentToUpdate = new RowComp(*this);
    ...
    {
        if (model != nullptr) model->selectedRowsChanged(row);
    }

    void TableListBox::deleteKeyPressed(int row)
    {
        if (model != nullptr) model->deleteKeyPressed(row);
    }

    void TableListBox::returnKeyPressed(int row)
    {
        if (model != nullptr) model->returnKeyPressed(row);
    }

    void TableListBox::backgroundClicked(const MouseEvent& e)
    {
        if (model != nullptr) model->backgroundClicked(e);
    }

    void TableListBox::listWasScrolled()
    {
        if (model != nullptr) model->listWasScrolled();
    }
    ...
    {
        if (model != nullptr) model->sortOrderChanged(header->getSortColumnId(), header->isSortedForwards());
    }
    ...
    void TableListBox::updateColumnComponents() const
    {
        ...
        for (int i = firstRow + getNumRowsOnScreen() + 2; --i >= firstRow;)
            if (auto* rowComp = dynamic_cast<RowComp*>(getComponentForRowNumber(i))) rowComp->resized();
    }
