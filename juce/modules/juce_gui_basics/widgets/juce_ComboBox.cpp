...
namespace juce
{
    ...
    void ComboBox::addItem(const String& newItemText, int newItemId)
    {
        ...
        if (newItemText.isNotEmpty() && newItemId != 0) currentMenu.addItem(newItemId, newItemText, true, false);
    }

    void ComboBox::addItemList(const StringArray& itemsToAdd, int firstItemID)
    {
        for (auto& i : itemsToAdd) currentMenu.addItem(firstItemID++, i);
    }
    ...
    void ComboBox::addSectionHeading(const String& headingName)
    {
        ...
        if (headingName.isNotEmpty()) currentMenu.addSectionHeader(headingName);
    }

    void ComboBox::setItemEnabled(int itemId, bool shouldBeEnabled)
    {
        if (auto* item = getItemForId(itemId)) item->isEnabled = shouldBeEnabled;
    }

    bool ComboBox::isItemEnabled(int itemId) const noexcept
    {
        if (auto* item = getItemForId(itemId)) return item->isEnabled;
    ...
    void ComboBox::clear(const NotificationType notification)
    {
        ...
        if (!label->isEditable()) setSelectedItemIndex(-1, notification);
    }
    ...
    {
        if (auto* item = getItemForIndex(index)) return item->text;
    ...
    {
        if (auto* item = getItemForIndex(index)) return item->itemID;
    ...
    int ComboBox::getSelectedItemIndex() const
    {
        ...
        if (getText() != getItemText(index)) index = -1;
    ...
    int ComboBox::getSelectedId() const noexcept
    {
        if (auto* item = getItemForId(currentId.getValue()))
            if (getText() == item->text) return item->itemID;
    ...
    bool ComboBox::nudgeSelectedItem(int delta)
    {
        for (int i = getSelectedItemIndex() + delta; isPositiveAndBelow(i, getNumItems()); i += delta)
            if (selectIfEnabled(i)) return true;
    ...
    {
        if (lastCurrentId != (int)currentId.getValue()) setSelectedId(currentId.getValue());
    }
    ...
    void ComboBox::setText(const String& newText, const NotificationType notification)
    {
        for (PopupMenu::MenuItemIterator iterator(currentMenu, true); iterator.next();)
        {
            ...
            if (item.itemID != 0 && item.text == newText)
            {
    ...
    {
        getLookAndFeel().drawComboBox(g, getWidth(), getHeight(), isButtonDown, label->getRight(), 0,
                                      getWidth() - label->getRight(), getHeight(), *this);
        ...
        if (textWhenNothingSelected.isNotEmpty() && label->getText().isEmpty() && !label->isBeingEdited())
        {
            ...
            g.drawFittedText(textWhenNothingSelected, label->getBounds().reduced(2, 1), label->getJustificationType(),
    ...
    {
        if (getHeight() > 0 && getWidth() > 0) getLookAndFeel().positionComboBoxText(*this, *label);
    }
    ...
    bool ComboBox::keyStateChanged(const bool isKeyDown)
    {
        ...
        return isKeyDown
               && (KeyPress::isKeyCurrentlyDown(KeyPress::upKey) || KeyPress::isKeyCurrentlyDown(KeyPress::leftKey)
                   || KeyPress::isKeyCurrentlyDown(KeyPress::downKey) || KeyPress::isKeyCurrentlyDown(KeyPress::rightKey));
    }
    ...
    void ComboBox::showPopupIfNotActive()
    {
        if (!menuActive)
        {
            ...
            // close themselves
            MessageManager::callAsync([safePointer]() mutable {
                if (safePointer != nullptr) safePointer->showPopup();
            });
        }
    }
    ...
    static void comboBoxPopupMenuFinishedCallback(int result, ComboBox* combo)
    {
        if (combo != nullptr)
        {
            ...
            if (result != 0) combo->setSelectedId(result);
        }
    }
    ...
    void ComboBox::showPopup()
    {
        ...
        if (menu.getNumItems() > 0)
        {
            ...
            for (PopupMenu::MenuItemIterator iterator(menu, true); iterator.next();)
            {
                ...
                if (item.itemID != 0) item.isTicked = (item.itemID == selectedId);
            }
        }
        else
        {
    ...
    void ComboBox::mouseDown(const MouseEvent& e)
    {
        ...
        if (isButtonDown && (e.eventComponent == this || !label->isEditable())) showPopupIfNotActive();
    }
    ...
    void ComboBox::mouseDrag(const MouseEvent& e)
    {
        ...
        if (isButtonDown && e.mouseWasDraggedSinceMouseDown()) showPopupIfNotActive();
    }
    ...
    void ComboBox::mouseUp(const MouseEvent& e2)
    {
        if (isButtonDown)
        {
            ...
            if (reallyContains(e.getPosition(), true) && (e2.eventComponent == this || !label->isEditable()))
            {
                showPopupIfNotActive();
            }
        }
    ...
    //==============================================================================
    void ComboBox::addListener(ComboBox::Listener* l)
    {
        listeners.add(l);
    }
    void ComboBox::removeListener(ComboBox::Listener* l)
    {
        listeners.remove(l);
    }
    ...
    {
        if (notification != dontSendNotification) triggerAsyncUpdate();

        if (notification == sendNotificationSync) handleUpdateNowIfNeeded();
    }
