...
namespace juce
{
    ...
    public:
        ...
        bool getToolbarItemSizes(int toolbarThickness, bool /*isToolbarVertical*/, int& preferredSize, int& minSize,
                                 int& maxSize) override
        {
            ...
            else
            {
                ...
                if (getEditingMode() == editableOnPalette) preferredSize = maxSize = toolbarThickness / (drawBar ? 3 : 2);
            }
    ...
    void Toolbar::addItemInternal(ToolbarItemFactory& factory, const int itemId, const int insertIndex)
    {
    ...
    void Toolbar::addDefaultItems(ToolbarItemFactory& factoryToUse)
    {
        ...
        for (auto i : ids) addItemInternal(factoryToUse, i, -1);
    ...
    {
        if (auto* tc = getItemComponent(itemIndex)) return tc->getItemId();
    ...
    String Toolbar::toString() const
    {
        ...
        for (int i = 0; i < getNumItems(); ++i) s << getItemId(i) << ' ';

        return s.trimEnd();
    }

    bool Toolbar::restoreFromString(ToolbarItemFactory& factoryToUse, const String& savedVersion)
    {
        if (!savedVersion.startsWith("TB:")) return false;
        ...
        for (auto& t : tokens) addItemInternal(factoryToUse, t.getIntValue(), -1);
    ...
    void Toolbar::updateAllItemPositions(bool animate)
    {
        if (getWidth() > 0 && getHeight() > 0)
        {
            ...
            for (auto* tc : items)
            {
                ...
                if (tc->getToolbarItemSizes(getThickness(), isVertical(), preferredSize, minSize, maxSize))
                {
                    tc->isActive = true;
                    resizer.addItem(preferredSize, minSize, maxSize, spacer != nullptr ? spacer->getResizeOrder() : 2);
                }
                else
                {
                    tc->isActive = false;
            ...
            for (int i = 0; i < resizer.getNumItems(); ++i) totalLength += (int)resizer.getItemSize(i);
            ...
            if (vertical)
                missingItemsButton->setCentrePosition(getWidth() / 2, getHeight() - 4 - extrasButtonSize / 2);
            else
                missingItemsButton->setCentrePosition(getWidth() - 4 - extrasButtonSize / 2, getHeight() / 2);

            auto maxLength =
                itemsOffTheEnd ? (vertical ? missingItemsButton->getY() : missingItemsButton->getX()) - 4 : getLength();
            ...
            for (auto* tc : items)
            {
                if (tc->isActive)
                {
                    ...
                    tc->setVisible(
                        pos <= maxLength
                        && ((!tc->isBeingDragged) || tc->getEditingMode() == ToolbarItemComponent::editableOnPalette));
                }
            }
        }
    }
    ...
    void Toolbar::itemDragMove(const SourceDetails& dragSourceDetails)
    {
        if (auto* tc = dynamic_cast<ToolbarItemComponent*>(dragSourceDetails.sourceComponent.get()))
        {
            if (!items.contains(tc))
            {
                ...
                {
                    if (auto* palette = tc->findParentComponentOfClass<ToolbarItemPalette>()) palette->replaceComponent(*tc);
                }
                else
                {
        ...
        public:
            CustomiserPanel(ToolbarItemFactory& tbf, Toolbar& bar, int optionFlags)
                ...
                , instructions({}, TRANS("You can drag the items above and drop them onto a toolbar to add them.") + "\n\n"
                                       + TRANS("Items on the toolbar can also be dragged around to change their order, or "
                                               "dragged off the edge to delete them."))
                , defaultButton(TRANS("Restore to default set of items"))
            {
                addAndMakeVisible(palette);

                if ((optionFlags
                     & (Toolbar::allowIconsOnlyChoice | Toolbar::allowIconsWithTextChoice | Toolbar::allowTextOnlyChoice))
                    != 0)
                {
            ...
            void paint(Graphics& g) override
            {
                ...
                if (auto* dw = findParentComponentOfClass<DialogWindow>()) background = dw->getBackgroundColour();
    ...
    void Toolbar::showCustomisationDialog(ToolbarItemFactory& factory, const int optionFlags)
    {
        ...
        (new CustomisationDialog(factory, *this, optionFlags))->enterModalState(true, nullptr, true);
    }
