...
namespace juce
{
    ...
    struct TextAtom
    {
        ...
        {
            if (passwordCharacter == 0) return atomText;

            return String::repeatedString(String::charToString(passwordCharacter), atomText.length());
        }

        String getTrimmedText(const juce_wchar passwordCharacter) const
        {
            if (passwordCharacter == 0) return atomText.substring(0, numChars);
    ...
    public:
        ...
        UniformTextSection* split(int indexToBreakAt, juce_wchar passwordChar)
        {
            ...
            for (int i = 0; i < atoms.size(); ++i)
            {
                ...
                {
                    for (int j = i; j < atoms.size(); ++j) section2->atoms.add(atoms.getUnchecked(j));
                ...
                if (indexToBreakAt >= index && indexToBreakAt < nextIndex)
                {
                    ...
                    for (int j = i + 1; j < atoms.size(); ++j) section2->atoms.add(atoms.getUnchecked(j));
        ...
        {
            for (auto& atom : atoms) mo << atom.atomText;
        }
        ...
        void appendSubstring(MemoryOutputStream& mo, Range<int> range) const
        {
            ...
            for (auto& atom : atoms)
            {
                ...
                {
                    if (range.getEnd() <= index) break;

                    auto r = (range - index).getIntersectionWith({0, (int)atom.numChars});

                    if (!r.isEmpty()) mo << atom.atomText.substring(r.getStart(), r.getEnd());
                }
        ...
        int getTotalLength() const noexcept
        {
            ...
            for (auto& atom : atoms) total += atom.numChars;
        ...
        void setFont(const Font& newFont, const juce_wchar passwordChar)
        {
            if (font != newFont)
            {
                ...
                for (auto& atom : atoms) atom.width = newFont.getStringWidthFloat(atom.getText(passwordChar));
            }
        }
    ...
    struct TextEditor::Iterator
    {
            ...
            , lineSpacing(ed.lineSpacing)
        {
            ...
            if (!sections.isEmpty())
            {
                ...
                if (currentSection != nullptr) beginNewLine();
            }
        }
        ...
        bool next()
        {
            if (atom == &tempAtom)
            {
                ...
                if (numRemaining > 0)
                {
                    ...
                    if (tempAtom.numChars > 0) lineY += lineHeight * lineSpacing;
                    ...
                    for (split = 0; split < g.getNumGlyphs(); ++split)
                        if (shouldWrap(g.getGlyph(split).getRight())) break;
                ...
                else
                {
                    ...
                    if (!lastAtom.isWhitespace())
                    {
                        ...
                        for (int section = sectionIndex + 1; section < sections.size(); ++section)
                        {
                            ...
                            if (s->atoms.size() == 0) break;

                            auto& nextAtom = s->atoms.getReference(0);

                            if (nextAtom.isWhitespace()) break;
            ...
            if (atom != nullptr)
            {
                ...
                if (atom->isNewLine()) beginNewLine();
            }
            ...
            if (shouldWrap(atomRight) || forceNewLine)
            {
                ...
                else
                {
                    if (shouldWrap(atom->width))  // atom too big to fit on a line, so break it up..
                    {
                        ...
                        if (atomX > justificationOffset) beginNewLine();
        ...
        void beginNewLine()
        {
            ...
            while (!shouldWrap(nextLineWidth))
            {
                ...
                if (tempSectionIndex >= sections.size()) break;
                ...
                if (!isPositiveAndBelow(tempAtomIndex, section->atoms.size())) break;

                auto& nextAtom = section->atoms.getReference(tempAtomIndex);
                nextLineWidth += nextAtom.width;

                if (shouldWrap(nextLineWidth) || nextAtom.isNewLine()) break;
        ...
        void draw(Graphics& g, const UniformTextSection*& lastSection) const
        {
            if (passwordCharacter != 0 || !atom->isWhitespace())
            {
                ...
                GlyphArrangement ga;
                ga.addLineOfText(currentSection->font, atom->getTrimmedText(passwordCharacter), atomX,
        ...
        {
            if (indexToFind <= indexInText) return atomX;

            if (indexToFind >= indexInText + atom->numChars) return atomRight;

            GlyphArrangement g;
            g.addLineOfText(currentSection->font, atom->getText(passwordCharacter), atomX, 0.0f);

            if (indexToFind - indexInText >= g.getNumGlyphs()) return atomRight;
        ...
        {
            if (xToFind <= atomX || atom->isNewLine()) return indexInText;

            if (xToFind >= atomRight) return indexInText + atom->numChars;
            ...
            for (j = 0; j < numGlyphs; ++j)
            {
                ...
                if ((pg.getLeft() + pg.getRight()) / 2 > xToFind) break;
            }
    ...
    {
        InsertAction(TextEditor& ed, const String& newText, int insertPos, const Font& newFont, Colour newColour,
                     int oldCaret, int newCaret)
            : owner(ed)
            , text(newText)
    ...
    struct TextEditor::RemoveAction : public UndoableAction
    {
        ...
        int getSizeInUnits() override
        {
            ...
            for (auto* s : removedSections) n += s->getTotalLength();
    ...
    {
        if (multiline != shouldBeMultiLine || wordWrap != (shouldWordWrap && shouldBeMultiLine))
        {
    ...
    {
        if (changeCurrentFont) currentFont = newFont;
    ...
    {
        for (auto* uts : sections) uts->colour = newColour;
    ...
    {
        if (caret != nullptr) caret->setCaretPosition(getCaretRectangle().translated(leftIndent, topIndent));
    }
    ...
    String TextEditor::LengthAndCharacterRestriction::filterNewText(TextEditor& ed, const String& newInput)
    {
        ...
        if (allowedCharacters.isNotEmpty()) t = t.retainCharacters(allowedCharacters);

        if (maxLength > 0) t = t.substring(0, maxLength - (ed.getTotalNumChars() - ed.getHighlightedRegion().getLength()));
    ...
    void TextEditor::setText(const String& newText, bool sendTextChangeMessage)
    {
        ...
        {
            if (!sendTextChangeMessage) textValue.removeListener(textHolder);
            ...
            if (cursorWasAtEnd && !isMultiLine()) oldCursorPos = getTotalNumChars();
    ...
    {
        if (textValue.getValueSource().getReferenceCount() > 1) setText(textValue.getValue());
    }
    ...
    void TextEditor::textChanged()
    {
        ...
        if (listeners.size() != 0 || onTextChange != nullptr) postCommandMessage(TextEditorDefs::textChangeMessageId);
    ...
    void TextEditor::addListener(Listener* l)
    {
        listeners.add(l);
    }
    void TextEditor::removeListener(Listener* l)
    {
        listeners.remove(l);
    }
    ...
    {
        if (hasKeyboardFocus(false) && !isCurrentlyBlockedByAnotherModalComponent()) wasFocused = true;

        auto now = Time::getApproximateMillisecondCounter();

        if (now > lastTransactionTime + 200) newTransaction();
    }
    ...
    void TextEditor::scrollEditorToPositionCaret(const int desiredCaretX, const int desiredCaretY)
    ...
    {
        return wordWrap ? getJustificationWidth() : std::numeric_limits<float>::max();
    }
    ...
    void TextEditor::updateTextHolderSize()
    {
        if (getWordWrapWidth() > 0)
        {
            ...
            while (i.next()) maxWidth = jmax(maxWidth, i.atomRight);

            auto w = leftIndent + roundToInt(maxWidth);
            auto h = topIndent + roundToInt(jmax(i.lineY + i.lineHeight, currentFont.getHeight()));

            textHolder->setSize(w + rightEdgeSpace,
                                h + 1);  // (allows a bit of space for the cursor to be at the right-hand-edge)
        }
    }
    ...
    void TextEditor::scrollToMakeSureCursorIsVisible()
    {
        ...
        if (keepCaretOnScreen)
        {
            ...
            {
                viewPos.x +=
                    relativeCursor.x + (isMultiLine() ? proportionOfWidth(0.2f) : 10) - viewport->getMaximumVisibleWidth();
            }
    ...
    void TextEditor::moveCaretTo(const int newPosition, const bool isSelecting)
    {
        if (isSelecting)
        {
            ...
            {
                if (getCaretPosition() >= selection.getEnd()) dragType = draggingSelectionEnd;

                selection = Range<int>::between(getCaretPosition(), selection.getEnd());
            }
            else
            {
                if (getCaretPosition() < selection.getStart()) dragType = draggingSelectionStart;
    ...
    void TextEditor::insertTextAtCaret(const String& t)
    {
        ...
        remove(selection, getUndoManager(), newText.isNotEmpty() ? newCaretPos - 1 : newCaretPos);
    ...
    void TextEditor::copy()
    {
        if (passwordCharacter == 0)
        {
            ...
            if (selectedText.isNotEmpty()) SystemClipboard::copyTextToClipboard(selectedText);
        }
    }
    ...
    void TextEditor::paste()
    {
        if (!isReadOnly())
        {
            ...
            if (clip.isNotEmpty()) insertTextAtCaret(clip);
        }
    }
    ...
    void TextEditor::paintOverChildren(Graphics& g)
    {
        if (textToShowWhenEmpty.isNotEmpty() && (!hasKeyboardFocus(false)) && getTotalNumChars() == 0)
        {
            ...
            if (isMultiLine())
                ...
                g.drawText(textToShowWhenEmpty, leftIndent, 0, viewport->getWidth() - leftIndent, getHeight(),
    ...
    void TextEditor::mouseDown(const MouseEvent& e)
    {
        ...
        if (wasFocused || !selectAllTextWhenFocused)
        {
            ...
            {
                moveCaretTo(getTextIndexAt(e.x, e.y), e.mods.isShiftDown());
            }
            else
            {
                PopupMenu m;
                ...
                m.showMenuAsync(PopupMenu::Options(), [safeThis](int menuResult) {
                    if (auto* editor = safeThis.getComponent())
                    {
                        editor->menuActive = false;

                        if (menuResult != 0) editor->performPopupMenuAction(menuResult);
                    }
                });
            }
        }
    ...
    {
        if (wasFocused || !selectAllTextWhenFocused)
            if (!(popupMenuEnabled && e.mods.isPopupMenu())) moveCaretTo(getTextIndexAt(e.x, e.y), true);
    }
    ...
    void TextEditor::mouseUp(const MouseEvent& e)
    {
        ...
        if (wasFocused || !selectAllTextWhenFocused)
            if (e.mouseWasClicked() && !(popupMenuEnabled && e.mods.isPopupMenu())) moveCaret(getTextIndexAt(e.x, e.y));
    ...
    {
        if (!viewport->useMouseWheelMoveIfNeeded(e, wheel)) Component::mouseWheelMove(e, wheel);
    }
    ...
    {
        if (!isMultiLine()) return moveCaretToStartOfLine(selecting);
    ...
    {
        if (!isMultiLine()) return moveCaretToEndOfLine(selecting);
    ...
    bool TextEditor::pageUp(bool selecting)
    {
        ...
        auto caretPos = getCaretRectangleFloat();
        return moveCaretWithTransaction(indexAtPosition(caretPos.getX(), caretPos.getY() - viewport->getViewHeight()),
                                        selecting);
    }
    ...
    bool TextEditor::pageDown(bool selecting)
    {
        ...
        auto caretPos = getCaretRectangleFloat();
        return moveCaretWithTransaction(indexAtPosition(caretPos.getX(), caretPos.getBottom() + viewport->getViewHeight()),
                                        selecting);
    }
    ...
    bool TextEditor::keyStateChanged(const bool isKeyDown)
    {
        ...
        if ((!consumeEscAndReturnKeys)
            && (KeyPress(KeyPress::escapeKey).isCurrentlyDown() || KeyPress(KeyPress::returnKey).isCurrentlyDown()))
            return false;
    ...
    void TextEditor::focusGained(FocusChangeType)
    {
        ...
        if (auto* peer = getPeer())
            if (!isReadOnly()) peer->textInputRequired(peer->globalToLocal(getScreenPosition()), *this);
    }
    ...
    void TextEditor::handleCommandMessage(const int commandId)
    {
        ...
        case TextEditorDefs::textChangeMessageId:
            ...
            if (!checker.shouldBailOut() && onTextChange != nullptr) onTextChange();
        ...
        case TextEditorDefs::returnKeyMessageId:
            ...
            if (!checker.shouldBailOut() && onReturnKey != nullptr) onReturnKey();
        ...
        case TextEditorDefs::escapeKeyMessageId:
            ...
            if (!checker.shouldBailOut() && onEscapeKey != nullptr) onEscapeKey();
        ...
        case TextEditorDefs::focusLossMessageId:
            ...
            if (!checker.shouldBailOut() && onFocusLost != nullptr) onFocusLost();
    ...
    void TextEditor::insert(const String& text, int insertIndex, const Font& font, Colour colour, UndoManager* um,
                            int caretPositionToMoveTo)
    {
        if (text.isNotEmpty())
        {
            if (um != nullptr)
            {
                if (um->getNumActionsInCurrentTransaction() > TextEditorDefs::maxActionsPerTransaction) newTransaction();

                um->perform(new InsertAction(*this, text, insertIndex, font, colour, caretPosition, caretPositionToMoveTo));
            }
            else
            {
                ...
                if (nextIndex == insertIndex) sections.add(new UniformTextSection(text, font, colour, passwordCharacter));
    ...
    void TextEditor::reinsert(int insertIndex, const OwnedArray<UniformTextSection>& sectionsToInsert)
    {
        ...
        if (nextIndex == insertIndex)
            for (auto* s : sectionsToInsert) sections.add(new UniformTextSection(*s));
    ...
    void TextEditor::remove(Range<int> range, UndoManager* const um, const int caretPositionToMoveTo)
    {
        if (!range.isEmpty())
        {
            ...
            if (um != nullptr)
            {
                ...
                {
                    if (range.getEnd() <= range.getStart()) break;
                ...
                um->perform(new RemoveAction(*this, range, caretPosition, caretPositionToMoveTo, removedSections));
            }
            else
            {
                ...
                for (int i = 0; i < sections.size(); ++i)
                {
                    ...
                    if (remainingRange.getStart() <= index && remainingRange.getEnd() >= nextIndex)
                    {
                        ...
                        if (remainingRange.isEmpty()) break;
    ...
    String TextEditor::getText() const
    {
        ...
        for (auto* s : sections) s->appendAllText(mo);
    ...
    int TextEditor::getTotalNumChars() const
    {
        if (totalNumChars < 0)
        {
            ...
            for (auto* s : sections) totalNumChars += s->getTotalLength();
        }
    ...
    int TextEditor::indexAtPosition(const float x, const float y)
    {
        if (getWordWrapWidth() > 0)
        {
            for (Iterator i(*this); i.next();)
            {
                ...
                {
                    if (y < i.lineY) return jmax(0, i.indexInText - 1);

                    if (x <= i.atomX || i.atom->isNewLine()) return i.indexInText;
    ...
    int TextEditor::findWordBreakAfter(const int position) const
    {
        ...
        while (i < totalLength && CharacterFunctions::isWhitespace(t[i])) ++i;

        auto type = TextEditorDefs::getCharacterCategory(t[i]);

        while (i < totalLength && type == TextEditorDefs::getCharacterCategory(t[i])) ++i;
    ...
    int TextEditor::findWordBreakBefore(const int position) const
    {
        ...
        while (i > 0 && CharacterFunctions::isWhitespace(t[i - 1])) --i;

        if (i > 0)
        {
            auto type = TextEditorDefs::getCharacterCategory(t[i - 1]);

            while (i > 0 && type == TextEditorDefs::getCharacterCategory(t[i - 1])) --i;
        }
    ...
    void TextEditor::splitSection(const int sectionIndex, const int charToSplitAt)
    {
        ...
        sections.insert(sectionIndex + 1, sections.getUnchecked(sectionIndex)->split(charToSplitAt, passwordCharacter));
    }
