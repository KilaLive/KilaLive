...
namespace juce
{
    ...
    public:
        ...
        {
            if (auto* m = owner.getModel()) m->paintListBoxItem(row, g, getWidth(), getHeight(), selected);
        }
        ...
        void performSelection(const MouseEvent& e, bool isMouseUp)
        {
            ...
            if (auto* m = owner.getModel()) m->listBoxItemClicked(row, e);
        }
        ...
        {
            if (isEnabled() && selectRowOnMouseUp && !(isDragging || isDraggingToScroll)) performSelection(e, true);
        }

        void mouseDoubleClick(const MouseEvent& e) override
        {
            if (isEnabled())
                if (auto* m = owner.getModel()) m->listBoxItemDoubleClicked(row, e);
        }
        ...
        void mouseDrag(const MouseEvent& e) override
        {
            ...
            if (!isDraggingToScroll)
                if (auto* vp = owner.getViewport()) isDraggingToScroll = vp->isCurrentlyScrollingOnDrag();
        }

        void resized() override
        {
            if (customComponent != nullptr) customComponent->setBounds(getLocalBounds());
        }

        String getTooltip() override
        {
            if (auto* m = owner.getModel()) return m->getTooltipForRow(row);
    ...
    public:
        ...
        int getRowNumberOfComponent(Component* const rowComponent) const noexcept
        {
            ...
            for (int i = num; --i >= 0;)
                if (((firstIndex + i) % jmax(1, num)) == index) return firstIndex + i;
        ...
        void visibleAreaChanged(const Rectangle<int>&) override
        {
            ...
            if (auto* m = owner.getModel()) m->listWasScrolled();
        }
        ...
        void updateContents()
        {
            ...
            if (owner.headerComponent != nullptr)
                owner.headerComponent->setBounds(owner.outlineThickness + content.getX(), owner.outlineThickness,
                                                 jmax(owner.getWidth() - owner.outlineThickness * 2, content.getWidth()),
                                                 owner.headerComponent->getHeight());
        }

        void selectRow(const int row, const int rowH, const bool dontScroll, const int lastSelectedRow, const int totalRows,
                       const bool isMouseClick)
        {
            ...
            else if (row >= lastWholeIndex && !dontScroll)
            {
                ...
                {
                    setViewPosition(getViewPositionX(), jlimit(0, jmax(0, totalRows - rowsOnScreen), row) * rowH);
                }
                else
                {
                    setViewPosition(getViewPositionX(), jmax(0, (row + 1) * rowH - getMaximumVisibleHeight()));
                }
            }

            if (!hasUpdated) updateContents();
        }
        ...
        void paint(Graphics& g) override
        {
            if (isOpaque()) g.fillAll(owner.findColour(ListBox::backgroundColourId));
        }
    ...
    void ListBox::setMouseMoveSelectsRows(bool b)
    {
        if (b)
        {
            if (mouseMoveSelector == nullptr) mouseMoveSelector.reset(new ListBoxMouseMoveSelector(*this));
        }
        else
        {
    ...
    {
        if (!hasDoneInitialUpdate) updateContent();
    ...
    void ListBox::resized()
    {
        viewport->setBoundsInset(
            BorderSize<int>(outlineThickness + (headerComponent != nullptr ? headerComponent->getHeight() : 0),
    ...
    void ListBox::updateContent()
    {
        ...
        if (selectionChanged && model != nullptr) model->selectedRowsChanged(lastRowSelected);
    }
    ...
    void ListBox::selectRowInternal(const int row, bool dontScroll, bool deselectOthersFirst, bool isMouseClick)
    {
        ...
        if ((!isRowSelected(row)) || (deselectOthersFirst && getNumSelectedRows() > 1))
        {
            ...
            {
                if (deselectOthersFirst) deselectAllRows();
            }
        }
    }
                                  ...
                                  const NotificationType sendNotificationEventToModel)
    {
        ...
        if (model != nullptr && sendNotificationEventToModel == sendNotification) model->selectedRowsChanged(lastRowSelected);
    }
    ...
    void ListBox::deselectAllRows()
    {
        if (!selected.isEmpty())
        {
            ...
            if (model != nullptr) model->selectedRowsChanged(lastRowSelected);
        }
    }
    ...
    int ListBox::getRowContainingPosition(const int x, const int y) const noexcept
    {
        if (isPositiveAndBelow(x, getWidth()))
        {
            ...
            if (isPositiveAndBelow(row, totalItems)) return row;
        }
    ...
    {
        if (auto* listRowComp = viewport->getComponentForRowIfOnscreen(row)) return listRowComp->customComponent.get();
    ...
    Rectangle<int> ListBox::getRowPosition(int rowNumber, bool relativeToComponentTopLeft) const noexcept
    {
        ...
        if (relativeToComponentTopLeft) y -= viewport->getViewPositionY();
    ...
    bool ListBox::keyPressed(const KeyPress& key)
    {
        ...
        {
            if (model != nullptr) model->returnKeyPressed(lastRowSelected);
        }
        else if ((key.isKeyCode(KeyPress::deleteKey) || key.isKeyCode(KeyPress::backspaceKey))
                 && isRowSelected(lastRowSelected))
        {
            if (model != nullptr) model->deleteKeyPressed(lastRowSelected);
        }
    ...
    {
        if (e.mouseWasClicked() && model != nullptr) model->backgroundClicked(e);
    }
    ...
    void ListBox::startDragAndDrop(const MouseEvent& e, const SparseSet<int>& rowsToDrag, const var& dragDescription,
