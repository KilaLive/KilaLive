...
namespace juce
{
    ...
    public:
        ...
        {
            return style == LinearVertical || style == LinearBarVertical || style == TwoValueVertical
                   || style == ThreeValueVertical;
        }

        bool isRotary() const noexcept
        {
            return style == Rotary || style == RotaryHorizontalDrag || style == RotaryVerticalDrag
        ...
        {
            if (isHorizontal() || isVertical()) return getLinearSliderPos(value);
        ...
        void setValue(double newValue, NotificationType notification)
        {
            ...
            if (style == ThreeValueHorizontal || style == ThreeValueVertical)
            {
                ...
                newValue =
                    jlimit(static_cast<double>(valueMin.getValue()), static_cast<double>(valueMax.getValue()), newValue);
            }

            if (newValue != lastCurrentValue)
            {
                if (valueBox != nullptr) valueBox->hideEditor(true);
        ...
        void setMinValue(double newValue, NotificationType notification, bool allowNudgingOfOtherValues)
        {
            ...
            {
                if (allowNudgingOfOtherValues && newValue > lastCurrentValue) setValue(newValue, notification);
        ...
        void setMinAndMaxValues(double newMinValue, double newMaxValue, NotificationType notification)
        {
            ...
            if (newMaxValue < newMinValue) std::swap(newMaxValue, newMinValue);
        ...
        void handleAsyncUpdate() override
        {
            ...
            if (owner.onValueChange != nullptr) owner.onValueChange();
        }
        ...
        void sendDragStart()
        {
            ...
            if (owner.onDragStart != nullptr) owner.onDragStart();
        }
        ...
        void sendDragEnd()
        {
            ...
            if (owner.onDragEnd != nullptr) owner.onDragEnd();
        }

        struct DragInProgress
        {
            DragInProgress(Pimpl& p)
                : owner(p)
            {
                owner.sendDragStart();
            }
        ...
        void updateText()
        {
            if (valueBox != nullptr)
            {
                ...
                if (newValue != valueBox->getText()) valueBox->setText(newValue, dontSendNotification);
            }
        }

        double constrainedValue(double value) const
        {
            if (interval > 0) value = minimum + interval * std::floor((value - minimum) / interval + 0.5);
        ...
        float getLinearSliderPos(double value) const
        {
            ...
            if (isVertical() || style == IncDecButtons) pos = 1.0 - pos;
        ...
        void setVelocityModeParameters(double sensitivity, int threshold, double offset, bool userCanPressKeyToSwapMode,
        ...
        {
            if (maximum > minimum)
                skewFactor = std::log(0.5) / std::log((sliderValueToShowAtMidPoint - minimum) / (maximum - minimum));
        }
        ...
        void showTextBox()
        {
            ...
            if (valueBox != nullptr) valueBox->showEditor();
        }
        ...
        void hideTextBox(bool discardCurrentEditorContents)
        {
            if (valueBox != nullptr)
            {
                ...
                if (discardCurrentEditorContents) updateText();
            }
        }
        ...
        void updateTextBoxEnablement()
        {
            if (valueBox != nullptr)
            {
                ...
                if (valueBox->isEditable()
                    != shouldBeEditable)  // (to avoid changing the single/double click flags unless we need to)
        ...
        void lookAndFeelChanged(LookAndFeel& lf)
        {
            ...
            {
                auto previousTextBoxContent =
                    (valueBox != nullptr ? valueBox->getText() : owner.getTextFromValue(currentValue.getValue()));
        ...
        void showPopupMenu()
        {
            ...
            m.showMenuAsync(PopupMenu::Options(), ModalCallbackFunction::forComponent(sliderMenuCallback, &owner));
        }
        ...
        int getThumbIndexAt(const MouseEvent& e)
        {
            ...
            if (isTwoValue || isThreeValue)
            {
                ...
                auto minPosDistance =
                    std::abs(getLinearSliderPos(valueMin.getValue()) + (isVertical() ? 0.1f : -0.1f) - mousePos);
                auto maxPosDistance =
                    std::abs(getLinearSliderPos(valueMax.getValue()) + (isVertical() ? -0.1f : 0.1f) - mousePos);

                if (isTwoValue) return maxPosDistance <= minPosDistance ? 2 : 1;

                if (normalPosDistance >= minPosDistance && maxPosDistance >= minPosDistance) return 1;

                if (normalPosDistance >= maxPosDistance) return 2;
            }
        ...
        void handleRotaryDrag(const MouseEvent& e)
        {
            ...
            if (dx * dx + dy * dy > 25.0f)
            {
                ...
                while (angle < 0.0) angle += MathConstants<double>::twoPi;
                ...
                {
                    while (angle < rotaryParams.startAngleRadians) angle += MathConstants<double>::twoPi;
                ...
                auto proportion = (angle - rotaryParams.startAngleRadians)
        ...
        void handleAbsoluteDrag(const MouseEvent& e)
        {
            ...
            if (style == RotaryHorizontalDrag || style == RotaryVerticalDrag || style == IncDecButtons
                || ((style == LinearHorizontal || style == LinearVertical || style == LinearBar || style == LinearBarVertical)
                    && !snapsToMousePos))
            {
                auto mouseDiff = (style == RotaryHorizontalDrag || style == LinearHorizontal || style == LinearBar
            ...
            {
                auto mouseDiff = (e.position.x - mouseDragStartPos.x) + (mouseDragStartPos.y - e.position.y);
            ...
            else
            {
                ...
                if (isVertical()) newPos = 1.0 - newPos;
            }
        ...
        {
            bool hasHorizontalStyle = (isHorizontal() || style == RotaryHorizontalDrag
                                       || (style == IncDecButtons && incDecDragDirectionIsHorizontal()));
            ...
            if (speed != 0.0)
            {
                speed = 0.2 * velocityModeSensitivity
                        * (1.0
                           + std::sin(MathConstants<double>::pi
                                      * (1.5
                                         + jmin(0.5, velocityModeOffset
                                                         + jmax(0.0, (double)(speed - velocityModeThreshold)) / maxSpeed))));

                if (mouseDiff < 0) speed = -speed;
        ...
        void mouseDown(const MouseEvent& e)
        {
            ...
            if (owner.isEnabled())
            {
                ...
                else if (canDoubleClickToValue() && e.mods.withoutMouseButtons() == ModifierKeys(ModifierKeys::altModifier))
                {
                    mouseDoubleClick();
                }
                else if (maximum > minimum)
                {
                    ...
                    valueWhenLastDragged =
                        (sliderBeingDragged == 2 ? valueMax : (sliderBeingDragged == 1 ? valueMin : currentValue)).getValue();
                    ...
                    if (showPopupOnDrag || showPopupOnHover)
                    {
                        ...
                        if (popupDisplay != nullptr) popupDisplay->stopTimer();
                    }
        ...
        void mouseDrag(const MouseEvent& e)
        {
            if (useDragEvents && maximum > minimum
                && !((style == LinearBar || style == LinearBarVertical) && e.mouseWasClicked() && valueBox != nullptr
                     && valueBox->isEditable()))
            {
                ...
                else
                {
                    ...
                    {
                        if (e.getDistanceFromDragStart() < 10 || !e.mouseWasDraggedSinceMouseDown()) return;
        ...
        void mouseUp()
        {
            if (owner.isEnabled() && useDragEvents && (maximum > minimum) && (style != IncDecButtons || incDecDragged))
            {
        ...
        void mouseMove()
        {
            ...
            auto shouldShowPopup = showPopupOnHover && (Time::getMillisecondCounterHiRes() - lastPopupDismissal) > 250;

            if (shouldShowPopup && !isTwoValue && !isThreeValue)
            {
                if (owner.isMouseOver(true))
                {
                    if (popupDisplay == nullptr) showPopupDisplay();

                    if (popupDisplay != nullptr && popupHoverTimeout != -1) popupDisplay->startTimer(popupHoverTimeout);
                }
            }
        }
        ...
        void showPopupDisplay()
        {
            ...
            if (popupDisplay == nullptr)
            {
                ...
                if (style == SliderStyle::TwoValueHorizontal || style == SliderStyle::TwoValueVertical)
                {
                    updatePopupDisplay(sliderBeingDragged == 2 ? getMaxValue() : getMinValue());
                }
                else
                {
        ...
        {
            if (popupDisplay != nullptr) popupDisplay->updatePosition(owner.getTextFromValue(valueToShow));
        }

        bool canDoubleClickToValue() const
        {
            return doubleClickToValue && style != IncDecButtons && minimum <= doubleClickReturnValue
        ...
        {
            if (style == IncDecButtons) return interval * wheelAmount;
        ...
        {
            if (scrollWheelEnabled && style != TwoValueHorizontal && style != TwoValueVertical)
            {
                ...
                if (e.eventTime != lastMouseWheelTime)
                {
                    ...
                    {
                        if (valueBox != nullptr) valueBox->hideEditor(false);

                        auto value = static_cast<double>(currentValue.getValue());
                        auto delta = getMouseWheelDelta(
                            value, (std::abs(wheel.deltaX) > std::abs(wheel.deltaY) ? -wheel.deltaX : wheel.deltaY)
        ...
        {
            if (style != IncDecButtons && style != Rotary && isAbsoluteDragMode(modifiers)) restoreMouseIfHidden();
        }
            ...
            for (auto& ms : Desktop::getInstance().getMouseSources())
            {
                if (ms.isUnboundedMouseMovementEnabled())
                {
                    ...
                    if (isRotary())
                    {
                        ...
                        auto delta = (float)(pixelsForFullDragExtent
                                             * (owner.valueToProportionOfLength(valueOnMouseDown)
                                                - owner.valueToProportionOfLength(pos)));

                        if (style == RotaryHorizontalDrag)
                            mousePos += Point<float>(-delta, 0.0f);
                        else if (style == RotaryVerticalDrag)
                            mousePos += Point<float>(0.0f, delta);
                        else
                            mousePos += Point<float>(delta / -2.0f, delta / 2.0f);
                    ...
                    else
                    {
                        ...
                        mousePos =
                            owner.localPointToGlobal(Point<float>(isHorizontal() ? pixelPos : (owner.getWidth() / 2.0f),
        ...
        void paint(Graphics& g, LookAndFeel& lf)
        {
            if (style != IncDecButtons)
            {
                if (isRotary())
                {
                    ...
                    lf.drawRotarySlider(g, sliderRect.getX(), sliderRect.getY(), sliderRect.getWidth(),
                                        sliderRect.getHeight(), sliderPos, rotaryParams.startAngleRadians,
                                        rotaryParams.endAngleRadians, owner);
                }
                else
                {
                    lf.drawLinearSlider(g, sliderRect.getX(), sliderRect.getY(), sliderRect.getWidth(),
                                        sliderRect.getHeight(), getLinearSliderPos(lastCurrentValue),
                                        getLinearSliderPos(lastValueMin), getLinearSliderPos(lastValueMax), style, owner);
                }
        ...
        void resized(LookAndFeel& lf)
        {
            ...
            if (valueBox != nullptr) valueBox->setBounds(layout.textBoxBounds);
        ...
        //==============================================================================
        struct PopupDisplayComponent : public BubbleComponent, public Timer
        {
        ...
        {
            return jmin(std::abs(a1 - a2), std::abs(a1 + MathConstants<double>::twoPi - a2),
    ...
    //==============================================================================
    void Slider::addListener(Listener* l)
    {
        pimpl->listeners.add(l);
    }
    void Slider::removeListener(Listener* l)
    {
    ...
    void Slider::setRotaryParameters(RotaryParameters p) noexcept
    {
        ...
        jassert(p.startAngleRadians < MathConstants<float>::pi * 4.0f && p.endAngleRadians < MathConstants<float>::pi * 4.0f);
                                           ...
                                           ModifierKeys::Flags modifierToSwapModes)
    {
        ...
        pimpl->setVelocityModeParameters(sensitivity, threshold, offset, userCanPressKeyToSwapMode, modifierToSwapModes);
    }
    ...
    bool Slider::isSymmetricSkew() const noexcept
    {
        return pimpl->symmetricSkew;
    }
    ...
    void Slider::setTextBoxStyle(TextEntryBoxPosition newPosition, bool isReadOnly, int textEntryBoxWidth,
    ...
    //==============================================================================
    Range<double> Slider::getRange() const noexcept
    {
        return {pimpl->minimum, pimpl->maximum};
    }
    ...
    void Slider::setRange(Range<double> newRange, double newInt)
    {
    ...
    {
        if (getNumDecimalPlacesToDisplay() > 0) return String(v, getNumDecimalPlacesToDisplay()) + getTextValueSuffix();
    ...
    double Slider::getValueFromText(const String& text)
    {
        ...
        if (t.endsWith(getTextValueSuffix())) t = t.substring(0, t.length() - getTextValueSuffix().length());
    ...
    double Slider::proportionOfLengthToValue(double proportion)
    {
        ...
        if (!isSymmetricSkew())
        {
            if (skew != 1.0 && proportion > 0.0) proportion = std::exp(std::log(proportion) / skew);
        ...
        if (skew != 1.0 && distanceFromMiddle != 0.0)
            distanceFromMiddle = std::exp(std::log(std::abs(distanceFromMiddle)) / skew) * (distanceFromMiddle < 0 ? -1 : 1);
    ...
    double Slider::valueToProportionOfLength(double value)
    {
        ...
        if (skew == 1.0) return n;

        if (!isSymmetricSkew()) return std::pow(n, skew);
    ...
    void Slider::mouseMove(const MouseEvent&)
    {
        pimpl->mouseMove();
    }
    ...
    {
        if (isEnabled()) pimpl->modifierKeysChanged(modifiers);
    }

    void Slider::mouseDrag(const MouseEvent& e)
    {
        if (isEnabled()) pimpl->mouseDrag(e);
    }

    void Slider::mouseDoubleClick(const MouseEvent&)
    {
        if (isEnabled()) pimpl->mouseDoubleClick();
    }

    void Slider::mouseWheelMove(const MouseEvent& e, const MouseWheelDetails& wheel)
    {
        if (!(isEnabled() && pimpl->mouseWheelMove(e, wheel))) Component::mouseWheelMove(e, wheel);
    }
