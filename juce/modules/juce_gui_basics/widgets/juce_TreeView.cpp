...
namespace juce
{
    class TreeView::ContentComponent : public Component, public TooltipClient, public AsyncUpdater
    {
    public:
        ...
        void mouseDown(const MouseEvent& e) override
        {
            ...
            if (auto* item = findItemAt(e.y, pos))
            {
                if (isEnabled())
                {
                    ...
                    {
                        if (e.x >= pos.getX() - owner.getIndentSize()) item->setOpen(!item->isOpen());
                    ...
                    else
                    {
                        ...
                        if (e.x >= pos.getX()) item->itemClicked(e.withNewPosition(e.position - pos.getPosition().toFloat()));
                    }
                }
            }
        }
        ...
        void mouseUp(const MouseEvent& e) override
        {
            ...
            if (needSelectionOnMouseUp && e.mouseWasClicked() && isEnabled())
            {
                ...
                if (auto* item = findItemAt(e.y, pos)) selectBasedOnModifiers(item, e.mods);
            }
        }
        ...
        {
            if (isEnabled()
                && !(isDragging || e.mouseWasClicked() || e.getDistanceFromDragStart() < 5 || e.mods.isPopupMenu()))
            {
                isDragging = true;
                Rectangle<int> pos;
        ...
        void paint(Graphics& g) override
        {
            if (owner.rootItem != nullptr)
            {
                ...
                if (!owner.rootItemVisible) g.setOrigin(0, -owner.rootItem->itemHeight);
        ...
        TreeViewItem* findItemAt(int y, Rectangle<int>& itemPosition) const
        {
            if (owner.rootItem != nullptr)
            {
                ...
                if (!owner.rootItemVisible) y += owner.rootItem->itemHeight;
        ...
        void updateComponents()
        {
            ...
            for (auto* i : items) i->shouldKeep = false;
        ...
        String getTooltip() override
        {
            ...
            if (auto* item = findItemAt(getMouseXYRelative().y, pos)) return item->getTooltip();
    ...
    private:
        ...
        void selectBasedOnModifiers(TreeViewItem* const item, const ModifierKeys modifiers)
        {
            ...
            if (modifiers.isShiftDown() && ((firstSelected = owner.getSelectedItem(0)) != nullptr))
            {
                ...
                if (ourRow > otherEnd) std::swap(ourRow, otherEnd);

                for (int i = ourRow; i <= otherEnd; ++i) owner.getItemOnRow(i)->setSelected(true, false);
            }
            else
            {
            ...
            if (owner.openCloseButtonsVisible)
            {
                ...
                if (auto* item = findItemAt(e.y, pos))
                {
                    if (e.x < pos.getX() && e.x >= pos.getX() - owner.getIndentSize())
                    {
                        ...
                        if (!newItem->mightContainSubItems()) newItem = nullptr;
                    }
                }
            }
            ...
            for (auto& ms : Desktop::getInstance().getMouseSources())
                if (ms.isDragging())
                    if (auto* underMouse = ms.getComponentUnderMouse())
                        if (comp == underMouse || comp->isParentOf(underMouse)) return true;
    ...
    public:
        ...
        bool keyPressed(const KeyPress& key) override
        {
            if (auto* tree = getParentComponent())
                if (tree->keyPressed(key)) return true;
    ...
    TreeView::~TreeView()
    {
        if (rootItem != nullptr) rootItem->setOwnerView(nullptr);
    }
    ...
    void TreeView::setRootItem(TreeViewItem* const newRootItem)
    {
        if (rootItem != newRootItem)
        {
            if (newRootItem != nullptr)
            {
                ...
                if (newRootItem->ownerView != nullptr) newRootItem->ownerView->setRootItem(nullptr);
            }
            ...
            if (newRootItem != nullptr) newRootItem->setOwnerView(this);
    ...
    {
        return indentSize >= 0 ? indentSize : getLookAndFeel().getTreeViewIndentSize(*this);
    }
    ...
    {
        if (rootItem != nullptr) rootItem->deselectAllRecursively(nullptr);
    }
    ...
    {
        if (!rootItemVisible) ++index;

        if (rootItem != nullptr && index >= 0) return rootItem->getItemOnRow(index);
    ...
    {
        if (rootItem == nullptr) return nullptr;
    ...
    {
        if (item->isSelected()) parent.createNewChildElement("SELECTED")->setAttribute("id", item->getItemIdentifierString());

        auto numSubItems = item->getNumSubItems();

        for (int i = 0; i < numSubItems; ++i) addAllSelectedItemIds(item->getSubItem(i), parent);
    }
    ...
    XmlElement* TreeView::getOpennessState(const bool alsoIncludeScrollPosition) const
    {
        ...
        if (rootItem != nullptr)
        {
            ...
            if (e != nullptr)
            {
                if (alsoIncludeScrollPosition) e->setAttribute("scrollPos", viewport->getViewPositionY());
    ...
    void TreeView::restoreOpennessState(const XmlElement& newState, const bool restoreStoredSelection)
    {
        if (rootItem != nullptr)
        {
            ...
            if (newState.hasAttribute("scrollPos"))
                viewport->setViewPosition(viewport->getViewPositionX(), newState.getIntAttribute("scrollPos"));
            ...
            if (restoreStoredSelection)
            {
                ...
                forEachXmlChildElementWithTagName(
                    newState, e,
                    "SELECTED") if (auto* item = rootItem->findItemFromIdentifierString(e->getStringAttribute("id")))
    ...
    void TreeView::moveSelectedRow(const int delta)
    {
        ...
        if (numRowsInTree > 0)
        {
            ...
            if (auto* firstSelected = getSelectedItem(0)) rowSelected = firstSelected->getRowNumberInTree();
    ...
    void TreeView::scrollToKeepItemVisible(TreeViewItem* item)
    {
        if (item != nullptr && item->ownerView == this)
        {
            ...
            {
                viewport->setViewPosition(viewport->getViewPositionX(), (y + item->itemHeight) - viewport->getViewHeight());
            }
        }
    }
    ...
    void TreeView::moveOutOfSelectedItem()
    {
        if (auto* firstSelected = getSelectedItem(0))
        {
            ...
            else
            {
                ...
                if ((!rootItemVisible) && parent == rootItem) parent = nullptr;
    ...
    void TreeView::moveByPages(int numPages)
    {
        if (auto* currentItem = getSelectedItem(0))
        {
            ...
            for (;;)
            {
                ...
                if ((numPages < 0 && y <= targetY) || (numPages > 0 && y >= targetY)) break;
    ...
    void TreeView::recalculateIfNeeded()
    {
        if (needsRecalculating)
        {
            ...
            if (rootItem != nullptr) rootItem->updatePositions(rootItemVisible ? 0 : -rootItem->itemHeight);

            viewport->updateComponents(false);

            if (rootItem != nullptr)
            {
                viewport->getViewedComponent()->setSize(jmax(viewport->getMaximumVisibleWidth(), rootItem->totalWidth + 50),
    ...
    public:
        ...
        void setTargetPosition(const InsertPoint& insertPos, const int width) noexcept
        {
            ...
            setBounds(insertPos.pos.x - offset, insertPos.pos.y - offset, width - (insertPos.pos.x - offset), getHeight());
        }
    ...
    public:
        ...
        {
            setBounds(item->getItemPosition(true).withHeight(item->getItemHeight()));
        }
    ...
    {
        const bool scrolled =
            viewport->autoScroll(dragSourceDetails.localPosition.x, dragSourceDetails.localPosition.y, 20, 10);

        InsertPoint insertPos(*this, files, dragSourceDetails);

        if (insertPos.item != nullptr)
        {
            if (scrolled || dragInsertPointHighlight == nullptr || dragInsertPointHighlight->lastItem != insertPos.item
    ...
    void TreeView::handleDrop(const StringArray& files, const SourceDetails& dragSourceDetails)
    {
        ...
        if (insertPos.item == nullptr) insertPos.item = rootItem;

        if (insertPos.item != nullptr)
        {
            if (files.size() > 0)
            {
                if (insertPos.item->isInterestedInFileDrag(files)) insertPos.item->filesDropped(files, insertPos.insertIndex);
            }
            else
            {
    ...
    {
        for (int i = subItems.size(); --i >= 0;) removeSubItemFromList(i, true);
    }
    ...
    void TreeViewItem::addSubItem(TreeViewItem* const newItem, const int insertPosition)
    {
        if (newItem != nullptr)
        {
            ...
            if (ownerView != nullptr)
            {
                ...
                if (newItem->isOpen()) newItem->itemOpennessChanged(true);
            }
            else
            {
                subItems.insert(insertPosition, newItem);
    ...
    void TreeViewItem::removeSubItem(int index, bool deleteItem)
    {
        if (ownerView != nullptr)
        {
            ...
            if (removeSubItemFromList(index, deleteItem)) treeHasChanged();
        }
        else
        {
    ...
    {
        if (openness == opennessDefault) return ownerView != nullptr && ownerView->defaultOpenness;
    ...
    {
        if (isOpen() != shouldBeOpen) setOpenness(shouldBeOpen ? opennessOpen : opennessClosed);
    }
    ...
    {
        if (this != itemToIgnore) setSelected(false, false);

        for (auto* i : subItems) i->deselectAllRecursively(itemToIgnore);
    }

    void TreeViewItem::setSelected(const bool shouldBeSelected, const bool deselectOtherItemsFirst,
                                   const NotificationType notify)
    {
        if (shouldBeSelected && !canBeSelected()) return;

        if (deselectOtherItemsFirst) getTopLevelItem()->deselectAllRecursively(this);

        if (shouldBeSelected != selected)
        {
            selected = shouldBeSelected;

            if (ownerView != nullptr) ownerView->repaint();

            if (notify != dontSendNotification) itemSelectionChanged(shouldBeSelected);
        }
    }

    void TreeViewItem::paintItem(Graphics&, int, int) {}

    void TreeViewItem::paintOpenCloseButton(Graphics& g, const Rectangle<float>& area, Colour backgroundColour,
                                            bool isMouseOver)
    {
        getOwnerView()->getLookAndFeel().drawTreeviewPlusMinusBox(g, area, backgroundColour, isOpen(), isMouseOver);
    }
    ...
    {
        if (mightContainSubItems()) setOpen(!isOpen());
    }
    ...
    Rectangle<int> TreeViewItem::getItemPosition(const bool relativeToTreeViewTopLeft) const noexcept
    {
        ...
        if (ownerView != nullptr && width < 0) width = ownerView->viewport->getViewWidth() - indentX;

        Rectangle<int> r(indentX, y, jmax(0, width), totalHeight);

        if (relativeToTreeViewTopLeft && ownerView != nullptr) r -= ownerView->viewport->getViewPosition();
    ...
    {
        if (ownerView != nullptr) ownerView->itemsChanged();
    }

    void TreeViewItem::repaintItem() const
    {
        if (ownerView != nullptr && areAllParentsOpen()) ownerView->viewport->repaint(getItemPosition(true).withLeft(0));
    }

    bool TreeViewItem::areAllParentsOpen() const noexcept
    {
        return parentItem == nullptr || (parentItem->isOpen() && parentItem->areAllParentsOpen());
    }
    ...
    int TreeViewItem::getIndentX() const noexcept
    {
        ...
        if (!ownerView->openCloseButtonsVisible) --x;

        for (auto* p = parentItem; p != nullptr; p = p->parentItem) ++x;
    ...
    namespace TreeViewHelpers
    {
        static int calculateDepth(const TreeViewItem* item, const bool rootIsVisible) noexcept
        {
            ...
            for (auto* p = item->getParentItem(); p != nullptr; p = p->getParentItem()) ++depth;
    ...
    void TreeViewItem::paintRecursively(Graphics& g, int width)
    {
...

        {
            ...
            if (g.reduceClipRegion(drawsInLeftMargin ? -indent : 0, 0, drawsInLeftMargin ? itemW + indent : itemW,
                                   itemHeight))
            {
                if (isSelected())
        ...
        if (depth >= 0 && ownerView->openCloseButtonsVisible)
        {
            ...
            if (mightContainSubItems())
            {
                ...
                paintOpenCloseButton(g,
                                     Rectangle<float>((float)(depth * indentWidth), 0, (float)indentWidth, (float)itemHeight),
        ...
        if (isOpen())
        {
            ...
            for (auto* ti : subItems)
            {
                ...
                if (relY + ti->totalHeight >= clip.getY())
                {
                    ...
                    if (g.reduceClipRegion(0, 0, width, ti->totalHeight)) ti->paintRecursively(g, width);
                }
            }
        }
    }
    ...
    {
        return parentItem == nullptr || parentItem->subItems.getLast() == this;
    }

    int TreeViewItem::getIndexInParent() const noexcept
    {
        return parentItem == nullptr ? 0 : parentItem->subItems.indexOf(this);
    }

    TreeViewItem* TreeViewItem::getTopLevelItem() noexcept
    {
        return parentItem == nullptr ? this : parentItem->getTopLevelItem();
    }
    ...
    int TreeViewItem::getNumRows() const noexcept
    {
        ...
        if (isOpen())
            for (auto* i : subItems) num += i->getNumRows();
    ...
    TreeViewItem* TreeViewItem::getItemOnRow(int index) noexcept
    {
        ...
        if (index > 0 && isOpen())
        {
            ...
            for (auto* i : subItems)
            {
                ...
                if (numRows > index) return i->getItemOnRow(index);
    ...
    TreeViewItem* TreeViewItem::findItemRecursively(int targetY) noexcept
    {
        if (isPositiveAndBelow(targetY, totalHeight))
        {
            ...
            if (isOpen())
            {
                ...
                for (auto* i : subItems)
                {
                    if (targetY < i->totalHeight) return i->findItemRecursively(targetY);
    ...
    int TreeViewItem::countSelectedItemsRecursively(int depth) const noexcept
    {
        ...
        if (depth != 0)
            for (auto* i : subItems) total += i->countSelectedItemsRecursively(depth - 1);
    ...
    TreeViewItem* TreeViewItem::getSelectedItemWithIndex(int index) noexcept
    {
        ...
        if (index >= 0)
        {
            for (auto* i : subItems)
            {
                if (auto* found = i->getSelectedItemWithIndex(index)) return found;
    ...
    int TreeViewItem::getRowNumberInTree() const noexcept
    {
        if (parentItem != nullptr && ownerView != nullptr)
        {
            ...
            while (--ourIndex >= 0) n += parentItem->subItems[ourIndex]->getNumRows();

            if (parentItem->parentItem == nullptr && !ownerView->rootItemVisible) --n;
    ...
    {
        if (recurse && isOpen() && !subItems.isEmpty()) return subItems.getFirst();

        if (parentItem != nullptr)
        {
            const int nextIndex = parentItem->subItems.indexOf(this) + 1;

            if (nextIndex >= parentItem->subItems.size()) return parentItem->getNextVisibleItem(false);
    ...
    String TreeViewItem::getItemIdentifierString() const
    {
        ...
        if (parentItem != nullptr) s = parentItem->getItemIdentifierString();
    ...
    TreeViewItem* TreeViewItem::findItemFromIdentifierString(const String& identifierString)
    {
        ...
        if (thisId == identifierString) return this;
        ...
        if (identifierString.startsWith(thisId + "/"))
        {
            ...
            for (auto* i : subItems)
                if (auto* item = i->findItemFromIdentifierString(remainingPath)) return item;
    ...
    void TreeViewItem::restoreOpennessState(const XmlElement& e)
    {
        ...
        else if (e.hasTagName("OPEN"))
        {
            ...
            for (auto* i : items) i->restoreToDefaultOpenness();
        }
    }
    ...
    XmlElement* TreeViewItem::getOpennessState(const bool canReturnNull) const
    {
        ...
        if (name.isNotEmpty())
        {
            ...
            if (isOpen())
            {
                if (canReturnNull && ownerView != nullptr && ownerView->defaultOpenness && isFullyOpen()) return nullptr;
            ...
            {
                if (canReturnNull && ownerView != nullptr && !ownerView->defaultOpenness) return nullptr;
    ...
    {
        if (oldOpenness != nullptr) treeViewItem.restoreOpennessState(*oldOpenness);
    }
