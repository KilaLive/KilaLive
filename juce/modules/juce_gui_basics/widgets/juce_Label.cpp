...
namespace juce
{
    ...
    Label::~Label()
    {
        ...
        if (ownerComponent != nullptr) ownerComponent->removeComponentListener(this);
    ...
    void Label::setText(const String& newText, NotificationType notification)
    {
        ...
        if (lastTextValue != newText)
        {
            ...
            if (ownerComponent != nullptr) componentMovedOrResized(*ownerComponent, true, true);

            if (notification != dontSendNotification) callChangeListeners();
        }
    }
    ...
    void Label::setEditable(bool editOnSingleClick, bool editOnDoubleClick, bool lossOfFocusDiscards)
    {
    ...
    void Label::componentMovedOrResized(Component& component, bool /*wasMoved*/, bool /*wasResized*/)
    {
        ...
        {
            auto width =
                jmin(roundToInt(f.getStringWidthFloat(textValue.toString()) + 0.5f) + getBorderSize().getLeftAndRight(),
                     component.getX());
    ...
    {
        if (auto* parent = component.getParentComponent()) parent->addChildComponent(this);
    }
    ...
    void Label::hideEditor(bool discardCurrentEditorContents)
    {
        if (editor != nullptr)
        {
            ...
            const bool changed = (!discardCurrentEditorContents) && updateFromTextEditorContents(*outgoingEditor);
            outgoingEditor.reset();
            repaint();

            if (changed) textWasEdited();

            if (deletionChecker != nullptr) exitModalState(0);

            if (changed && deletionChecker != nullptr) callChangeListeners();
        }
    }
    ...
    {
        if (editSingleClick && isEnabled() && contains(e.getPosition())
    ...
    {
        if (editDoubleClick && isEnabled() && !e.mods.isPopupMenu()) showEditor();
    }
    ...
    {
        if (editSingleClick && isEnabled() && cause == focusChangedByTabKey) showEditor();
    }
    ...
    public:
        ...
        {
            return dynamic_cast<TextEditor*>(current) != nullptr ? current->getParentComponent() : current;
        }
    };
    ...
    //==============================================================================
    void Label::addListener(Label::Listener* l)
    {
        listeners.add(l);
    }
    void Label::removeListener(Label::Listener* l)
    {
        listeners.remove(l);
    }
    ...
    void Label::textEditorReturnKeyPressed(TextEditor& ed)
    {
        if (editor != nullptr)
        {
            ...
            if (changed && deletionChecker != nullptr)
            {
                ...
                if (deletionChecker != nullptr) callChangeListeners();
            }
        }
    }
