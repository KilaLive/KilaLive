...
namespace juce
{
    ...
    {
        if (isMouseOver || isMouseDragging) g.fillAll(Colours::yellow.withAlpha(0.4f));
    }
                                       ...
                                       bool isMouseDown)
    {
        ...
        if (isMouseOver || isMouseDown) thumbCol = thumbCol.withMultipliedAlpha(2.0f);
    ...
    void LookAndFeel_V3::drawConcertinaPanelHeader(Graphics& g, const Rectangle<int>& area, bool      isMouseOver,
    ...
    static void drawButtonShape(Graphics& g, const Path& outline, Colour baseColour, float height)
    {
        ...
        g.setGradientFill(ColourGradient::vertical(baseColour.brighter(0.2f), 0.0f, baseColour.darker(0.25f), height));
        g.fillPath(outline);
    ...
    void LookAndFeel_V3::createTabTextLayout(const TabBarButton& button, float length, float depth, Colour colour,
    ...
    void LookAndFeel_V3::drawTabAreaBehindFrontButton(TabbedButtonBar& bar, Graphics& g, const int w, const int h)
    {
        ...
        ColourGradient gradient(Colours::black.withAlpha(bar.isEnabled() ? 0.08f : 0.04f), 0, 0, Colours::transparentBlack, 0,
                                0, false);
    ...
    void LookAndFeel_V3::drawTreeviewPlusMinusBox(Graphics& g, const Rectangle<float>& area, Colour backgroundColour,
    ...
    void LookAndFeel_V3::drawComboBox(Graphics& g, int width, int height, const bool /*isButtonDown*/, int buttonX,
    ...
    void LookAndFeel_V3::drawLinearSlider(Graphics& g, int x, int y, int width, int height, float sliderPos,
                                          ...
                                          Slider& slider)
    {
        ...
        if (style == Slider::LinearBar || style == Slider::LinearBarVertical)
        {
            ...
            g.setGradientFill(
                ColourGradient::vertical(baseColour.brighter(0.08f), 0.0f, baseColour.darker(0.08f), (float)height));
            g.fillPath(p);
    ...
    void LookAndFeel_V3::drawLinearSliderBackground(Graphics& g, int x, int y, int width, int height, float /*sliderPos*/,
    ...
    void LookAndFeel_V3::drawMenuBarBackground(Graphics& g, int width, int height, bool, MenuBarComponent& menuBar)
    {
    ...
    void LookAndFeel_V3::drawKeymapChangeButton(Graphics& g, int width, int height, Button& button,
    ...
    public:
        ...
        void paintButton(Graphics& g, bool isMouseOverButton, bool isButtonDown) override
        {
            ...
            if (ResizableWindow* rw = findParentComponentOfClass<ResizableWindow>()) background = rw->getBackgroundColour();
            ...
            float scale = 0.55f;
            g.fillPath(p, p.getTransformToScaleToFit(cx - diam * scale, cy - diam * scale, diam * 2.0f * scale,
                                                     diam * 2.0f * scale, true));
        }
    ...
    Path LookAndFeel_V3::getCrossShape(const float height)
    {
        static const unsigned char pathData[] = {
            110, 109, 88,  57, 198, 65,  29,  90,  171, 65,  108, 63,  53,  154, 65,  8,   172, 126, 65,  108, 76,  55, 198,
            65,  215, 163, 38, 65,  108, 141, 151, 175, 65,  82,  184, 242, 64,  108, 117, 147, 131, 65,  90,  100, 81, 65,
            108, 184, 30,  47, 65,  82,  184, 242, 64,  108, 59,  223, 1,   65,  215, 163, 38,  65,  108, 84,  227, 89, 65,
            8,   172, 126, 65, 108, 35,  219, 1,   65,  29,  90,  171, 65,  108, 209, 34,  47,  65,  231, 251, 193, 65, 108,
            117, 147, 131, 65, 207, 247, 149, 65,  108, 129, 149, 175, 65,  231, 251, 193, 65,  99,  101, 0,   0};
