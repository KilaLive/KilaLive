...
namespace juce
{
    ...
    {
        if (isPositiveAndBelow(index, numColours)) return palette[index];
    ...
    LookAndFeel_V4::LookAndFeel_V4()
        : currentColourScheme(getDarkColourScheme())
    {
        initialiseColours();
    }

    LookAndFeel_V4::LookAndFeel_V4(ColourScheme scheme)
    ...
    {
        return {0xff323e44, 0xff263238, 0xff323e44, 0xff8e989b, 0xffffffff, 0xff42a2c8, 0xffffffff, 0xff181f22, 0xffffffff};
    }

    LookAndFeel_V4::ColourScheme LookAndFeel_V4::getMidnightColourScheme()
    {
        return {0xff2f2f3a, 0xff191926, 0xffd0d0d0, 0xff66667c, 0xc8ffffff, 0xffd8d8d8, 0xffffffff, 0xff606073, 0xff000000};
    }

    LookAndFeel_V4::ColourScheme LookAndFeel_V4::getGreyColourScheme()
    {
        return {0xff505050, 0xff424242, 0xff606060, 0xffa6a6a6, 0xffffffff, 0xff21ba90, 0xff000000, 0xffffffff, 0xffffffff};
    }

    LookAndFeel_V4::ColourScheme LookAndFeel_V4::getLightColourScheme()
    {
        return {0xffefefef, 0xffffffff, 0xffffffff, 0xffdddddd, 0xff000000, 0xffa9a9a9, 0xffffffff, 0xff42a2c8, 0xff000000};
    }
    ...
    public:
        ...
        void paintButton(Graphics& g, bool isMouseOverButton, bool isButtonDown) override
        {
            ...
            g.setColour((!isEnabled() || isButtonDown) ? colour.withAlpha(0.6f) : colour);
    ...
    void LookAndFeel_V4::positionDocumentWindowButtons(DocumentWindow&, int titleBarX, int titleBarY, int titleBarW,
    ...
    void LookAndFeel_V4::drawDocumentWindowTitleBar(DocumentWindow& window, Graphics& g, int w, int h, int titleSpaceX,
                                              ...
                                              bool isMouseOverButton, bool isButtonDown)
    {
        ...
        if (isButtonDown || isMouseOverButton) baseColour = baseColour.contrasting(isButtonDown ? 0.2f : 0.05f);
        ...
        if (button.isConnectedOnLeft() || button.isConnectedOnRight())
        {
            ...
            path.addRoundedRectangle(bounds.getX(), bounds.getY(), bounds.getWidth(), bounds.getHeight(), cornerSize,
                                     cornerSize, !button.isConnectedOnLeft(), !button.isConnectedOnRight(),
    ...
    void LookAndFeel_V4::drawToggleButton(Graphics& g, ToggleButton& button, bool isMouseOverButton, bool isButtonDown)
    {
    ...
    void LookAndFeel_V4::drawTickBox(Graphics& g, Component& component, float x, float y, float w, float h, const bool ticked,
    ...
    //==============================================================================
    AlertWindow* LookAndFeel_V4::createAlertWindow(const String& title, const String& message, const String& button1,
                                                   ...
                                                   Component* associatedComponent)
    {
        ...
        auto* aw = LookAndFeel_V2::createAlertWindow(title, message, button1, button2, button3, iconType, numButtons,
                                                     associatedComponent);
        ...
        for (auto* child : aw->getChildren())
            if (auto* button = dynamic_cast<TextButton*>(child)) button->setBounds(button->getBounds() + Point<int>(25, 40));

        return aw;
    }

    void LookAndFeel_V4::drawAlertBox(Graphics& g, AlertWindow& alert, const Rectangle<int>& textArea, TextLayout& textLayout)
    {
        ...
        if (alert.getAlertType() != AlertWindow::NoIcon)
        {
            ...
            GlyphArrangement ga;
            ga.addFittedText({iconRect.getHeight() * 0.9f, Font::bold}, String::charToString((juce_wchar)(uint8)character),
        ...
        Rectangle<int> alertBounds(bounds.getX() + iconSpaceUsed, 30, bounds.getWidth(),
    ...
    //==============================================================================
    void LookAndFeel_V4::drawProgressBar(Graphics& g, ProgressBar& progressBar, int width, int height, double progress,
                                               ...
                                               const String& textToShow)
    {
        ...
        else
        {
            ...
            for (auto x = static_cast<float>(-position); x < width + stripeWidth; x += stripeWidth)
                p.addQuadrilateral(x, 0.0f, x + stripeWidth * 0.5f, 0.0f, x, static_cast<float>(height),
    ...
    void LookAndFeel_V4::drawCircularProgressBar(Graphics& g, ProgressBar& progressBar, const String& progressText)
    {
        ...
        g.setColour(background);
        Path arcPath2;
        arcPath2.addCentredArc(barBounds.getCentreX(), barBounds.getCentreY(), barBounds.getWidth() * 0.5f,
                               barBounds.getHeight() * 0.5f, 0.0f, 0.0f, MathConstants<float>::twoPi, true);
        g.strokePath(arcPath2, PathStrokeType(4.0f));

        g.setColour(foreground);
        Path arcPath;
        arcPath.addCentredArc(barBounds.getCentreX(), barBounds.getCentreY(), barBounds.getWidth() * 0.5f,
                              barBounds.getHeight() * 0.5f, 0.0f, degreesToRadians(startInDegrees),
                              degreesToRadians(endInDegrees), true);
    ...
    {
        static const unsigned char pathData[] = {110, 109, 32,  210, 202, 64,  126, 183, 148, 64, 108, 39, 244, 247, 64,  245,
                                                 76,  124, 64,  108, 178, 131, 27,  65,  246, 76, 252, 64, 108, 175, 242, 4,
                                                 65,  246, 76,  252, 64,  108, 236, 5,   68,  65, 0,   0,  160, 180, 108, 240,
                                                 150, 90,  65,  21,  136, 52,  63,  108, 48,  59, 16,  65, 0,   0,   32,  65,
                                                 108, 32,  210, 202, 64,  126, 183, 148, 64,  99, 101, 0,  0};
    ...
    Path LookAndFeel_V4::getCrossShape(float height)
    {
        static const unsigned char pathData[] = {
            110, 109, 51, 51,  255, 66,  0,   0,   0,   0,   108, 205, 204, 13,  67,  51,  51,  99,  65,  108, 0,
            0,   170, 66, 205, 204, 141, 66,  108, 51,  179, 13,  67,  52,  51,  255, 66,  108, 0,   0,   255, 66,
            205, 204, 13, 67,  108, 205, 204, 141, 66,  0,   0,   170, 66,  108, 52,  51,  99,  65,  51,  179, 13,
            67,  108, 0,  0,   0,   0,   51,  51,  255, 66,  108, 205, 204, 98,  66,  204, 204, 141, 66,  108, 0,
            0,   0,   0,  51,  51,  99,  65,  108, 51,  51,  99,  65,  0,   0,   0,   0,   108, 205, 204, 141, 66,
            205, 204, 98, 66,  108, 51,  51,  255, 66,  0,   0,   0,   0,   99,  101, 0,   0};
                                                    ...
                                                    TextEditor* filenameBox, Button* goUpButton)
    {
        ...
        if (previewComp != nullptr) previewComp->setBounds(b.removeFromRight(b.getWidth() / 3));

        if (auto* listAsComp = dynamic_cast<Component*>(fileListComponent)) listAsComp->setBounds(b.reduced(0, 10));
    }

    void LookAndFeel_V4::drawFileBrowserRow(Graphics& g, int width, int height, const File& file, const String& filename,
    ...
    {
        LookAndFeel_V2::drawFileBrowserRow(g, width, height, file, filename, icon, fileSizeDescription, fileTimeDescription,
                                           isDirectory, isItemSelected, itemIndex, dcc);
    }

    //==============================================================================
    void LookAndFeel_V4::drawPopupMenuItem(Graphics& g, const Rectangle<int>& area, const bool isSeparator,
    ...
    void LookAndFeel_V4::getIdealPopupMenuItemSize(const String& text, const bool isSeparator, int standardMenuItemHeight,
    ...
    void LookAndFeel_V4::drawMenuBarBackground(Graphics& g, int width, int height, bool, MenuBarComponent& menuBar)
    {
    ...
    void LookAndFeel_V4::drawMenuBarItem(Graphics& g, int width, int height, int itemIndex, const String& itemText,
    ...
    //==============================================================================
    void LookAndFeel_V4::drawComboBox(Graphics& g, int width, int height, bool, int, int, int, int, ComboBox& box)
    {
    ...
    void LookAndFeel_V4::drawLinearSlider(Graphics& g, int x, int y, int width, int height, float sliderPos,
                                          ...
                                          Slider& slider)
    {
        ...
        else
        {
            ...
            if (isTwoVal || isThreeVal)
            {
                ...
                {
                    drawPointer(g, minSliderPos - sr, jmax(0.0f, y + height * 0.5f - trackWidth * 2.0f), trackWidth * 2.0f,
                                pointerColour, 2);

                    drawPointer(g, maxSliderPos - trackWidth, jmin(y + height - trackWidth * 2.0f, y + height * 0.5f),
                                trackWidth * 2.0f, pointerColour, 4);
                }
                else
                {
                    drawPointer(g, jmax(0.0f, x + width * 0.5f - trackWidth * 2.0f), minSliderPos - trackWidth,
    ...
    void LookAndFeel_V4::drawPointer(Graphics& g, const float x, const float y, const float diameter, const Colour& colour,
    ...
    //==============================================================================
    void LookAndFeel_V4::drawConcertinaPanelHeader(Graphics& g, const Rectangle<int>& area, bool isMouseOver,
                                                   bool /*isMouseDown*/, ConcertinaPanel& concertina, Component& panel)
    {
        ...
        Path p;
        p.addRoundedRectangle(bounds.getX(), bounds.getY(), bounds.getWidth(), bounds.getHeight(), cornerSize, cornerSize,
                              isTopPanel, isTopPanel, false, false);

        auto bkg = Colours::grey;

        g.setGradientFill(ColourGradient::vertical(Colours::white.withAlpha(isMouseOver ? 0.4f : 0.2f),
                                                   static_cast<float>(area.getY()), Colours::darkgrey.withAlpha(0.1f),
    ...
    void LookAndFeel_V4::drawLevelMeter(Graphics& g, int width, int height, float level)
    {
        ...
        for (auto i = 0; i < totalBlocks; ++i)
        {
            ...
            g.fillRoundedRectangle(outerBorderWidth + (i * blockWidth) + blockRectSpacing, outerBorderWidth, blockRectWidth,
    ...
    void LookAndFeel_V4::paintToolbarBackground(Graphics& g, int w, int h, Toolbar& toolbar)
    {
        ...
        g.setGradientFill({background, 0.0f, 0.0f, background.darker(0.2f), toolbar.isVertical() ? w - 1.0f : 0.0f,
                           toolbar.isVertical() ? 0.0f : h - 1.0f, false});
        g.fillAll();
    }

    void LookAndFeel_V4::paintToolbarButtonLabel(Graphics& g, int x, int y, int width, int height, const String& text,
    ...
    //==============================================================================
    void LookAndFeel_V4::drawPropertyPanelSectionHeader(Graphics& g, const String& name, bool isOpen, int width, int height)
    {
    ...
    void LookAndFeel_V4::drawPropertyComponentLabel(Graphics& g, int width, int height, PropertyComponent& component)
    {
        ...
        g.drawFittedText(component.getName(), indent, r.getY(), r.getX() - 5, r.getHeight(), Justification::centredLeft, 2);
    }
    ...
    //==============================================================================
    void LookAndFeel_V4::drawCallOutBoxBackground(CallOutBox& box, Graphics& g, const Path& path, Image& cachedImage)
    {
    ...
    void LookAndFeel_V4::initialiseColours()
    {
        ...
        const uint32 coloursToUse[] = {
            ...
            currentColourScheme.getUIColour(ColourScheme::UIColour::widgetBackground).getARGB(),
            TextButton::buttonOnColourId,
            currentColourScheme.getUIColour(ColourScheme::UIColour::highlightedFill).getARGB(),
            TextButton::textColourOnId,
            currentColourScheme.getUIColour(ColourScheme::UIColour::highlightedText).getARGB(),
            TextButton::textColourOffId,
            currentColourScheme.getUIColour(ColourScheme::UIColour::defaultText).getARGB(),
            ...
            currentColourScheme.getUIColour(ColourScheme::UIColour::defaultText).withAlpha(0.5f).getARGB(),
            ...
            currentColourScheme.getUIColour(ColourScheme::UIColour::defaultFill).withAlpha(0.4f).getARGB(),
            TextEditor::highlightedTextColourId,
            currentColourScheme.getUIColour(ColourScheme::UIColour::highlightedText).getARGB(),
            TextEditor::outlineColourId,
            currentColourScheme.getUIColour(ColourScheme::UIColour::outline).getARGB(),
            ...
            currentColourScheme.getUIColour(ColourScheme::UIColour::defaultFill).getARGB(),
            ...
            currentColourScheme.getUIColour(ColourScheme::UIColour::menuBackground).getARGB(),
            PopupMenu::textColourId,
            currentColourScheme.getUIColour(ColourScheme::UIColour::menuText).getARGB(),
            ...
            currentColourScheme.getUIColour(ColourScheme::UIColour::widgetBackground).withAlpha(0.0f).getARGB(),
            ...
            currentColourScheme.getUIColour(ColourScheme::UIColour::windowBackground).getARGB(),

            DocumentWindow::textColourId,
            currentColourScheme.getUIColour(ColourScheme::UIColour::defaultText).getARGB(),
            ...
            currentColourScheme.getUIColour(ColourScheme::UIColour::outline).withAlpha(0.5f).getARGB(),
            TabbedButtonBar::frontOutlineColourId,
            currentColourScheme.getUIColour(ColourScheme::UIColour::outline).getARGB(),

            Toolbar::backgroundColourId,
            currentColourScheme.getUIColour(ColourScheme::UIColour::widgetBackground).withAlpha(0.4f).getARGB(),
            Toolbar::separatorColourId,
            currentColourScheme.getUIColour(ColourScheme::UIColour::outline).getARGB(),
            Toolbar::buttonMouseOverBackgroundColourId,
            currentColourScheme.getUIColour(ColourScheme::UIColour::widgetBackground).contrasting(0.2f).getARGB(),
            Toolbar::buttonMouseDownBackgroundColourId,
            currentColourScheme.getUIColour(ColourScheme::UIColour::widgetBackground).contrasting(0.5f).getARGB(),
            ...
            currentColourScheme.getUIColour(ColourScheme::UIColour::defaultText)
                .interpolatedWith(Colours::blue, 0.4f)
                .getARGB(),
            ...
            currentColourScheme.getUIColour(ColourScheme::UIColour::defaultFill)
                .getARGB(),
            0x1000441,
            /*LassoComponent::lassoOutlineColourId*/
            currentColourScheme.getUIColour(ColourScheme::UIColour::outline)
                .getARGB(),
            ...
            currentColourScheme.getUIColour(ColourScheme::UIColour::widgetBackground)
                .getARGB(),
            0x1004502,
            ...
            currentColourScheme.getUIColour(ColourScheme::UIColour::highlightedFill)
                .withAlpha(0.5f)
                .getARGB(),
            0x1004505,
            ...
            currentColourScheme.getUIColour(ColourScheme::UIColour::widgetBackground).darker().getARGB(),
            SidePanel::dismissButtonNormalColour,
            currentColourScheme.getUIColour(ColourScheme::UIColour::defaultFill).getARGB(),
            SidePanel::dismissButtonOverColour,
            currentColourScheme.getUIColour(ColourScheme::UIColour::defaultFill).darker().getARGB(),
            SidePanel::dismissButtonDownColour,
            currentColourScheme.getUIColour(ColourScheme::UIColour::defaultFill).brighter().getARGB(),
